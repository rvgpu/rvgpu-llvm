...
//
/// \file
/// This pass does misc. RVGPU optimizations on IR *just* before instruction
/// selection.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "rvgpu-late-codegenprepare"
...
static cl::opt<bool>
    WidenLoads("rvgpu-late-codegenprepare-widen-constant-loads",
               cl::desc("Widen sub-dword constant address space loads in "
                        "RVGPULateCodeGenPrepare"),
               cl::ReallyHidden, cl::init(true));

namespace {

class RVGPULateCodeGenPrepare
    : public FunctionPass,
      public InstVisitor<RVGPULateCodeGenPrepare, bool> {
  Module *Mod = nullptr;
...
public:
  ...
  RVGPULateCodeGenPrepare() : FunctionPass(ID) {}

  StringRef getPassName() const override {
    return "RVGPU IR late optimizations";
  }
...
bool RVGPULateCodeGenPrepare::doInitialization(Module &M) {
  Mod = &M;
  DL = &Mod->getDataLayout();
  return false;
}

bool RVGPULateCodeGenPrepare::runOnFunction(Function &F) {
  if (skipFunction(F))
    return false;
...
bool RVGPULateCodeGenPrepare::canWidenScalarExtLoad(LoadInst &LI) const {
...
bool RVGPULateCodeGenPrepare::visitLoadInst(LoadInst &LI) {
  if (!WidenLoads)
    return false;
...
INITIALIZE_PASS_BEGIN(RVGPULateCodeGenPrepare, DEBUG_TYPE,
                      "RVGPU IR late optimizations", false, false)
INITIALIZE_PASS_DEPENDENCY(AssumptionCacheTracker)
INITIALIZE_PASS_DEPENDENCY(UniformityInfoWrapperPass)
INITIALIZE_PASS_END(RVGPULateCodeGenPrepare, DEBUG_TYPE,
                    "RVGPU IR late optimizations", false, false)

char RVGPULateCodeGenPrepare::ID = 0;

FunctionPass *llvm::createRVGPULateCodeGenPreparePass() {
  return new RVGPULateCodeGenPrepare();
}
