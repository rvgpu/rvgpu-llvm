...
static cl::opt<bool> EnableOCLManglingMismatchWA(
    "rvgpu-enable-ocl-mangling-mismatch-workaround", cl::init(true),
    cl::ReallyHidden,
...
public:
  ...
  static unsigned toIndex(ID Id) {
    assert(static_cast<unsigned>(Id) >
               static_cast<unsigned>(RVGPULibFunc::EI_LAST_MANGLED) &&
  ...
  static ID toFuncId(unsigned Index) {
    ...
    return static_cast<ID>(
        Index + 1 + static_cast<unsigned>(RVGPULibFunc::EI_LAST_MANGLED));
  }
...
// corresponds to the EFuncId enum at RVGPULibFunc.h
//
...
//    same order should be preserved for arguments in the RVGPULibFunc structure
...
static RVGPULibFunc::Param getRetType(RVGPULibFunc::EFuncId id,
                                       const RVGPULibFunc::Param (&Leads)[2]) {
  RVGPULibFunc::Param Res = Leads[0];
  // TBD - This switch may require to be extended for other intrinsics
  switch (id) {
  case RVGPULibFunc::EI_SINCOS:
    Res.PtrKind = RVGPULibFunc::BYVALUE;
    break;
  default:
    break;
  }
...
class ParamIterator {
  ...
  int Index;
public:
  ParamIterator(const RVGPULibFunc::Param (&leads)[2],
                const ManglingRule& rule)
    : Leads(leads), Rule(rule), Index(0) {}

  RVGPULibFunc::Param getNextParam();
};

RVGPULibFunc::Param ParamIterator::getNextParam() {
  RVGPULibFunc::Param P;
  ...
  case E_NONE:     break;
  case EX_UINT:
    P.ArgType = RVGPULibFunc::U32; break;
  case EX_INTV4:
    P.ArgType = RVGPULibFunc::I32; P.VectorSize = 4; break;
  case EX_UINTV4:
    P.ArgType = RVGPULibFunc::U32; P.VectorSize = 4; break;
  case EX_FLOAT4:
    P.ArgType = RVGPULibFunc::F32; P.VectorSize = 4; break;
  case EX_SIZET:
    P.ArgType = RVGPULibFunc::U64; break;
  case EX_EVENT:
    P.ArgType = RVGPULibFunc::EVENT;   break;
  case EX_SAMPLER:
    P.ArgType = RVGPULibFunc::SAMPLER; break;
  ...
  default:
    ...
    case E_POINTEE:
      P.PtrKind = RVGPULibFunc::BYVALUE; break;
    case E_V2_OF_POINTEE:
      P.VectorSize = 2; P.PtrKind = RVGPULibFunc::BYVALUE; break;
    case E_V3_OF_POINTEE:
      P.VectorSize = 3; P.PtrKind = RVGPULibFunc::BYVALUE; break;
    case E_V4_OF_POINTEE:
      P.VectorSize = 4; P.PtrKind = RVGPULibFunc::BYVALUE; break;
    case E_V8_OF_POINTEE:
      P.VectorSize = 8; P.PtrKind = RVGPULibFunc::BYVALUE; break;
    case E_V16_OF_POINTEE:
      P.VectorSize = 16; P.PtrKind = RVGPULibFunc::BYVALUE; break;
    case E_CONSTPTR_ANY:
      P.PtrKind |= RVGPULibFunc::CONST; break;
    case E_VLTLPTR_ANY:
      P.PtrKind |= RVGPULibFunc::VOLATILE; break;
    case E_SETBASE_I32:
      P.ArgType = RVGPULibFunc::I32; break;
    case E_SETBASE_U32:
      P.ArgType = RVGPULibFunc::U32; break;

    case E_MAKEBASE_UNS:
      P.ArgType &= ~RVGPULibFunc::BASE_TYPE_MASK;
      P.ArgType |= RVGPULibFunc::UINT;
      break;

    case E_IMAGECOORDS:
      switch (P.ArgType) {
      case RVGPULibFunc::IMG1DA: P.VectorSize = 2; break;
      case RVGPULibFunc::IMG1DB: P.VectorSize = 1; break;
      case RVGPULibFunc::IMG2DA: P.VectorSize = 4; break;
      case RVGPULibFunc::IMG1D:  P.VectorSize = 1; break;
      case RVGPULibFunc::IMG2D:  P.VectorSize = 2; break;
      case RVGPULibFunc::IMG3D:  P.VectorSize = 4; break;
      }
      P.PtrKind = RVGPULibFunc::BYVALUE;
      P.ArgType = RVGPULibFunc::I32;
      break;

    case E_CONSTPTR_SWAPGL: {
      unsigned AS = RVGPULibFunc::getAddrSpaceFromEPtrKind(P.PtrKind);
      switch (AS) {
      case RVGPUAS::GLOBAL_ADDRESS: AS = RVGPUAS::LOCAL_ADDRESS; break;
      case RVGPUAS::LOCAL_ADDRESS:  AS = RVGPUAS::GLOBAL_ADDRESS; break;
      }
      P.PtrKind = RVGPULibFunc::getEPtrKindFromAddrSpace(AS);
      P.PtrKind |= RVGPULibFunc::CONST;
      break;
    }
...
RVGPUMangledLibFunc::RVGPUMangledLibFunc() {
  FuncId = EI_NONE;
  FKind = NOPFX;
  Leads[0].reset();
  Leads[1].reset();
...
RVGPUUnmangledLibFunc::RVGPUUnmangledLibFunc() {
  FuncId = EI_NONE;
  FuncTy = nullptr;
}

RVGPUMangledLibFunc::RVGPUMangledLibFunc(
    EFuncId id, const RVGPUMangledLibFunc &copyFrom) {
  FuncId = id;
...
RVGPUMangledLibFunc::RVGPUMangledLibFunc(EFuncId id, FunctionType *FT,
                                           bool SignedInts) {
  FuncId = id;
...
static RVGPULibFunc::ENamePrefix parseNamePrefix(StringRef& mangledName) {
  std::pair<StringRef, StringRef> const P = mangledName.split('_');
  RVGPULibFunc::ENamePrefix Pfx =
    StringSwitch<RVGPULibFunc::ENamePrefix>(P.first)
    .Case("native", RVGPULibFunc::NATIVE)
    .Case("half"  , RVGPULibFunc::HALF)
    .Default(RVGPULibFunc::NOPFX);

  if (Pfx != RVGPULibFunc::NOPFX)
...
bool RVGPUMangledLibFunc::parseUnmangledName(StringRef FullName) {
...
struct ItaniumParamParser {
  RVGPULibFunc::Param Prev;
  bool parseItaniumParam(StringRef& param, RVGPULibFunc::Param &res);
};
} // namespace

bool ItaniumParamParser::parseItaniumParam(StringRef& param,
                                           RVGPULibFunc::Param &res) {
  res.reset();
  if (param.empty()) return false;

  // parse pointer prefix
  if (eatTerm(param, 'P')) {
    if (eatTerm(param, 'K')) res.PtrKind |= RVGPULibFunc::CONST;
    if (eatTerm(param, 'V')) res.PtrKind |= RVGPULibFunc::VOLATILE;
    unsigned AS;
    ...
    res.PtrKind |= RVGPULibFuncBase::getEPtrKindFromAddrSpace(AS);
  } else {
    res.PtrKind = RVGPULibFunc::BYVALUE;
  }
  ...
  if (isDigit(TC)) {
    res.ArgType = StringSwitch<RVGPULibFunc::EType>
      (eatLengthPrefixedName(param))
      .Case("ocl_image1darray" , RVGPULibFunc::IMG1DA)
      .Case("ocl_image1dbuffer", RVGPULibFunc::IMG1DB)
      .Case("ocl_image2darray" , RVGPULibFunc::IMG2DA)
      .Case("ocl_image1d"      , RVGPULibFunc::IMG1D)
      .Case("ocl_image2d"      , RVGPULibFunc::IMG2D)
      .Case("ocl_image3d"      , RVGPULibFunc::IMG3D)
      .Case("ocl_event"        , RVGPULibFunc::DUMMY)
      .Case("ocl_sampler"      , RVGPULibFunc::DUMMY)
      .Default(RVGPULibFunc::DUMMY);
  } else {
    drop_front(param);
    switch (TC) {
    case 'h': res.ArgType =  RVGPULibFunc::U8; break;
    case 't': res.ArgType = RVGPULibFunc::U16; break;
    case 'j': res.ArgType = RVGPULibFunc::U32; break;
    case 'm': res.ArgType = RVGPULibFunc::U64; break;
    case 'c': res.ArgType =  RVGPULibFunc::I8; break;
    case 's': res.ArgType = RVGPULibFunc::I16; break;
    case 'i': res.ArgType = RVGPULibFunc::I32; break;
    case 'l': res.ArgType = RVGPULibFunc::I64; break;
    case 'f': res.ArgType = RVGPULibFunc::F32; break;
    case 'd': res.ArgType = RVGPULibFunc::F64; break;
    case 'D': if (!eatTerm(param, 'h')) return false;
              res.ArgType = RVGPULibFunc::F16; break;
    case 'S':
      if (!eatTerm(param, '_')) {
        eatNumber(param);
...
bool RVGPUMangledLibFunc::parseFuncName(StringRef &mangledName) {
...
bool RVGPUUnmangledLibFunc::parseFuncName(StringRef &Name) {
...
bool RVGPULibFunc::parse(StringRef FuncName, RVGPULibFunc &F) {
  if (FuncName.empty()) {
    F.Impl = std::unique_ptr<RVGPULibFuncImpl>();
    return false;
  }

  if (eatTerm(FuncName, "_Z"))
    F.Impl = std::make_unique<RVGPUMangledLibFunc>();
  else
    F.Impl = std::make_unique<RVGPUUnmangledLibFunc>();
...
StringRef RVGPUMangledLibFunc::getUnmangledName(StringRef mangledName) {
...
void RVGPUMangledLibFunc::writeName(Stream &OS) const {
  const char *Pfx = "";
  switch (FKind) {
...
std::string RVGPUMangledLibFunc::mangle() const { return mangleNameItanium(); }

///////////////////////////////////////////////////////////////////////////////
// Itanium Mangling

static const char *getItaniumTypeName(RVGPULibFunc::EType T) {
  switch (T) {
  case RVGPULibFunc::U8:      return "h";
  case RVGPULibFunc::U16:     return "t";
  case RVGPULibFunc::U32:     return "j";
  case RVGPULibFunc::U64:     return "m";
  case RVGPULibFunc::I8:      return "c";
  case RVGPULibFunc::I16:     return "s";
  case RVGPULibFunc::I32:     return "i";
  case RVGPULibFunc::I64:     return "l";
  case RVGPULibFunc::F16:     return "Dh";
  case RVGPULibFunc::F32:     return "f";
  case RVGPULibFunc::F64:     return "d";
  case RVGPULibFunc::IMG1DA:  return "16ocl_image1darray";
  case RVGPULibFunc::IMG1DB:  return "17ocl_image1dbuffer";
  case RVGPULibFunc::IMG2DA:  return "16ocl_image2darray";
  case RVGPULibFunc::IMG1D:   return "11ocl_image1d";
  case RVGPULibFunc::IMG2D:   return "11ocl_image2d";
  case RVGPULibFunc::IMG3D:   return "11ocl_image3d";
  case RVGPULibFunc::SAMPLER: return "11ocl_sampler";
  case RVGPULibFunc::EVENT:   return "9ocl_event";
  default:
...
class ItaniumMangler {
  SmallVector<RVGPULibFunc::Param, 10> Str; // list of accumulated substitutions
  bool  UseAddrSpace;

  int findSubst(const RVGPULibFunc::Param& P) const {
    for(unsigned I = 0; I < Str.size(); ++I) {
      const RVGPULibFunc::Param& T = Str[I];
  ...
  template <typename Stream>
  bool trySubst(Stream& os, const RVGPULibFunc::Param& p) {
...
public:
  ...
  template <typename Stream>
  void operator()(Stream& os, RVGPULibFunc::Param p) {
    ...
    RVGPULibFunc::Param Ptr;

    if (p.PtrKind) {
      if (trySubst(os, p)) return;
      os << 'P';
      if (p.PtrKind & RVGPULibFunc::CONST) os << 'K';
      if (p.PtrKind & RVGPULibFunc::VOLATILE) os << 'V';
      unsigned AS = UseAddrSpace
                        ? RVGPULibFuncBase::getAddrSpaceFromEPtrKind(p.PtrKind)
                        : 0;
    ...
    os << getItaniumTypeName((RVGPULibFunc::EType)p.ArgType);
...
std::string RVGPUMangledLibFunc::mangleNameItanium() const {
  SmallString<128> Buf;
...
RVGPULibFuncBase::Param RVGPULibFuncBase::Param::getFromTy(Type *Ty,
                                                             bool Signed) {
  Param P;
  ...
  switch (Ty->getTypeID()) {
  case Type::FloatTyID:
    P.ArgType = RVGPULibFunc::F32;
    break;
  case Type::DoubleTyID:
    P.ArgType = RVGPULibFunc::F64;
    break;
  case Type::HalfTyID:
    P.ArgType = RVGPULibFunc::F16;
    break;
  case Type::IntegerTyID:
    switch (cast<IntegerType>(Ty)->getBitWidth()) {
    case 8:
      P.ArgType = Signed ? RVGPULibFunc::I8 : RVGPULibFunc::U8;
      break;
    case 16:
      P.ArgType = Signed ? RVGPULibFunc::I16 : RVGPULibFunc::U16;
      break;
    case 32:
      P.ArgType = Signed ? RVGPULibFunc::I32 : RVGPULibFunc::U32;
      break;
    case 64:
      P.ArgType = Signed ? RVGPULibFunc::I64 : RVGPULibFunc::U64;
      break;
    default:
...
static Type* getIntrinsicParamType(
  LLVMContext& C,
  const RVGPULibFunc::Param& P,
  bool useAddrSpace) {
  Type* T = nullptr;
  switch (P.ArgType) {
  case RVGPULibFunc::U8:
  case RVGPULibFunc::I8:   T = Type::getInt8Ty(C);   break;
  case RVGPULibFunc::U16:
  case RVGPULibFunc::I16:  T = Type::getInt16Ty(C);  break;
  case RVGPULibFunc::U32:
  case RVGPULibFunc::I32:  T = Type::getInt32Ty(C);  break;
  case RVGPULibFunc::U64:
  case RVGPULibFunc::I64:  T = Type::getInt64Ty(C);  break;
  case RVGPULibFunc::F16:  T = Type::getHalfTy(C);   break;
  case RVGPULibFunc::F32:  T = Type::getFloatTy(C);  break;
  case RVGPULibFunc::F64:  T = Type::getDoubleTy(C); break;

  case RVGPULibFunc::IMG1DA:
  case RVGPULibFunc::IMG1DB:
  case RVGPULibFunc::IMG2DA:
  case RVGPULibFunc::IMG1D:
  case RVGPULibFunc::IMG2D:
  case RVGPULibFunc::IMG3D:
    T = StructType::create(C,"ocl_image")->getPointerTo(); break;
  case RVGPULibFunc::SAMPLER:
    T = StructType::create(C,"ocl_sampler")->getPointerTo(); break;
  case RVGPULibFunc::EVENT:
  ...
  if (P.PtrKind != RVGPULibFunc::BYVALUE)
    T = useAddrSpace ? T->getPointerTo((P.PtrKind & RVGPULibFunc::ADDR_SPACE)
                                       - 1)
                     : T->getPointerTo();
  return T;
}

FunctionType *RVGPUMangledLibFunc::getFunctionType(Module &M) const {
...
unsigned RVGPUMangledLibFunc::getNumArgs() const {
  return manglingRules[FuncId].getNumArgs();
}

unsigned RVGPUUnmangledLibFunc::getNumArgs() const {
  return UnmangledFuncInfo::getNumArgs(FuncId);
}

std::string RVGPUMangledLibFunc::getName() const {
  SmallString<128> Buf;
...
bool RVGPULibFunc::isCompatibleSignature(const FunctionType *FuncTy) const {
  // TODO: Validate types make sense
  return !FuncTy->isVarArg() && FuncTy->getNumParams() == getNumArgs();
}

Function *RVGPULibFunc::getFunction(Module *M, const RVGPULibFunc &fInfo) {
...
FunctionCallee RVGPULibFunc::getOrInsertFunction(Module *M,
                                                  const RVGPULibFunc &fInfo) {
...
bool UnmangledFuncInfo::lookup(StringRef Name, ID &Id) {
  ...
  Id = RVGPULibFunc::EI_NONE;
  return false;
}

RVGPULibFunc::RVGPULibFunc(const RVGPULibFunc &F) {
  if (auto *MF = dyn_cast<RVGPUMangledLibFunc>(F.Impl.get()))
    Impl.reset(new RVGPUMangledLibFunc(*MF));
  else if (auto *UMF = dyn_cast<RVGPUUnmangledLibFunc>(F.Impl.get()))
    Impl.reset(new RVGPUUnmangledLibFunc(*UMF));
  else
    Impl = std::unique_ptr<RVGPULibFuncImpl>();
}

RVGPULibFunc &RVGPULibFunc::operator=(const RVGPULibFunc &F) {
  if (this == &F)
    return *this;
  new (this) RVGPULibFunc(F);
  return *this;
}

RVGPULibFunc::RVGPULibFunc(EFuncId Id, const RVGPULibFunc &CopyFrom) {
  assert(RVGPULibFuncBase::isMangled(Id) && CopyFrom.isMangled() &&
         "not supported");
  Impl.reset(new RVGPUMangledLibFunc(
      Id, *cast<RVGPUMangledLibFunc>(CopyFrom.Impl.get())));
}

RVGPULibFunc::RVGPULibFunc(EFuncId Id, FunctionType *FT, bool SignedInts) {
  Impl.reset(new RVGPUMangledLibFunc(Id, FT, SignedInts));
}

RVGPULibFunc::RVGPULibFunc(StringRef Name, FunctionType *FT) {
  Impl.reset(new RVGPUUnmangledLibFunc(Name, FT));
}

void RVGPULibFunc::initMangled() { Impl.reset(new RVGPUMangledLibFunc()); }

RVGPULibFunc::Param *RVGPULibFunc::getLeads() {
  if (!Impl)
    initMangled();
  return cast<RVGPUMangledLibFunc>(Impl.get())->Leads;
}

const RVGPULibFunc::Param *RVGPULibFunc::getLeads() const {
  return cast<const RVGPUMangledLibFunc>(Impl.get())->Leads;
}
