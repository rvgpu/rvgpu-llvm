...
/// \file
///
/// This file defines the RVGPUCustomBehaviour class which inherits from
...
#ifndef LLVM_LIB_TARGET_RVGPU_MCA_RVGPUCUSTOMBEHAVIOUR_H
#define LLVM_LIB_TARGET_RVGPU_MCA_RVGPUCUSTOMBEHAVIOUR_H
...
class RVGPUInstrPostProcess : public InstrPostProcess {
  void processWaitCnt(std::unique_ptr<Instruction> &Inst, const MCInst &MCI);

public:
  RVGPUInstrPostProcess(const MCSubtargetInfo &STI, const MCInstrInfo &MCII)
      : InstrPostProcess(STI, MCII) {}

  ~RVGPUInstrPostProcess() = default;
...
class RVGPUCustomBehaviour : public CustomBehaviour {
...
public:
  RVGPUCustomBehaviour(const MCSubtargetInfo &STI,
                        const mca::SourceMgr &SrcMgr, const MCInstrInfo &MCII);

  ~RVGPUCustomBehaviour() = default;
