...
/// \file
///
/// This file implements methods from the RVGPUCustomBehaviour class.
///
//===----------------------------------------------------------------------===//

#include "RVGPUCustomBehaviour.h"
...
void RVGPUInstrPostProcess::postProcessInstruction(
    std::unique_ptr<Instruction> &Inst, const MCInst &MCI) {
  switch (MCI.getOpcode()) {
  case RVGPU::S_WAITCNT:
  case RVGPU::S_WAITCNT_soft:
  case RVGPU::S_WAITCNT_EXPCNT:
  case RVGPU::S_WAITCNT_LGKMCNT:
  case RVGPU::S_WAITCNT_VMCNT:
  case RVGPU::S_WAITCNT_VSCNT:
  case RVGPU::S_WAITCNT_VSCNT_soft:
  case RVGPU::S_WAITCNT_EXPCNT_gfx10:
  case RVGPU::S_WAITCNT_LGKMCNT_gfx10:
  case RVGPU::S_WAITCNT_VMCNT_gfx10:
  case RVGPU::S_WAITCNT_VSCNT_gfx10:
  case RVGPU::S_WAITCNT_gfx10:
  case RVGPU::S_WAITCNT_gfx6_gfx7:
  case RVGPU::S_WAITCNT_vi:
...
void RVGPUInstrPostProcess::processWaitCnt(std::unique_ptr<Instruction> &Inst,
                                            const MCInst &MCI) {
...
RVGPUCustomBehaviour::RVGPUCustomBehaviour(const MCSubtargetInfo &STI,
...
unsigned RVGPUCustomBehaviour::checkCustomHazard(ArrayRef<InstRef> IssuedInst,
                                                  const InstRef &IR) {
  ...
  default:
    return 0;
  case RVGPU::S_WAITCNT: // This instruction
  case RVGPU::S_WAITCNT_soft:
  case RVGPU::S_WAITCNT_EXPCNT:
  case RVGPU::S_WAITCNT_LGKMCNT:
  case RVGPU::S_WAITCNT_VMCNT:
  case RVGPU::S_WAITCNT_VSCNT:
  case RVGPU::S_WAITCNT_VSCNT_soft: // to this instruction are all pseudo.
  case RVGPU::S_WAITCNT_EXPCNT_gfx10:
  case RVGPU::S_WAITCNT_LGKMCNT_gfx10:
  case RVGPU::S_WAITCNT_VMCNT_gfx10:
  case RVGPU::S_WAITCNT_VSCNT_gfx10:
...
unsigned RVGPUCustomBehaviour::handleWaitCnt(ArrayRef<InstRef> IssuedInst,
                                              const InstRef &IR) {
...
void RVGPUCustomBehaviour::computeWaitCnt(const InstRef &IR, unsigned &Vmcnt,
                                           unsigned &Expcnt, unsigned &Lgkmcnt,
                                           unsigned &Vscnt) {
  RVGPU::IsaVersion IV = RVGPU::getIsaVersion(STI.getCPU());
  const Instruction &Inst = *IR.getInstruction();
  unsigned Opcode = Inst.getOpcode();
  ...
  case RVGPU::S_WAITCNT_VSCNT_gfx10: {
    ...
    if (OpReg->getReg() != RVGPU::SGPR_NULL) {
  ...
  case RVGPU::S_WAITCNT_vi:
    ...
    RVGPU::decodeWaitcnt(IV, WaitCnt, Vmcnt, Expcnt, Lgkmcnt);
    return;
  }
}

void RVGPUCustomBehaviour::generateWaitCntInfo() {
  ...
  for (const auto &EN : llvm::enumerate(SrcMgr.getInstructions())) {
    ...
    if ((MCID.TSFlags & SIInstrFlags::DS) &&
      ...
      if (isAlwaysGDS(Opcode) || hasModifiersSet(Inst, RVGPU::OpName::gds))
      ...
      InstrWaitCntInfo[Index].LgkmCnt = true;
      if (!STI.hasFeature(RVGPU::FeatureVscnt))
    ...
    } else if (isVMEM(MCID) && !RVGPU::getMUBUFIsBufferInv(Opcode)) {
      if (!STI.hasFeature(RVGPU::FeatureVscnt))
        InstrWaitCntInfo[Index].VmCnt = true;
...
// taken from RVInstrInfo::isVMEM()
bool RVGPUCustomBehaviour::isVMEM(const MCInstrDesc &MCID) {
  return MCID.TSFlags & SIInstrFlags::MUBUF ||
         MCID.TSFlags & SIInstrFlags::MTBUF ||
         MCID.TSFlags & SIInstrFlags::MIMG;
}

// taken from RVInstrInfo::hasModifiersSet()
bool RVGPUCustomBehaviour::hasModifiersSet(
    const std::unique_ptr<Instruction> &Inst, unsigned OpName) const {
  int Idx = RVGPU::getNamedOperandIdx(Inst->getOpcode(), OpName);
  if (Idx == -1)
    return false;
...
// taken from RVInstrInfo::isGWS()
bool RVGPUCustomBehaviour::isGWS(uint16_t Opcode) const {
  const MCInstrDesc &MCID = MCII.get(Opcode);
  return MCID.TSFlags & SIInstrFlags::GWS;
}

// taken from RVInstrInfo::isAlwaysGDS()
bool RVGPUCustomBehaviour::isAlwaysGDS(uint16_t Opcode) const {
...
createRVGPUCustomBehaviour(const MCSubtargetInfo &STI,
                            const mca::SourceMgr &SrcMgr,
                            const MCInstrInfo &MCII) {
  return new RVGPUCustomBehaviour(STI, SrcMgr, MCII);
}

static InstrPostProcess *
createRVGPUInstrPostProcess(const MCSubtargetInfo &STI,
                             const MCInstrInfo &MCII) {
  return new RVGPUInstrPostProcess(STI, MCII);
}

/// Extern function to initialize the targets for the RVGPU backend

extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeRVGPUTargetMCA() {
  TargetRegistry::RegisterCustomBehaviour(getTheRVGPUTarget(),
                                          createRVGPUCustomBehaviour);
  TargetRegistry::RegisterInstrPostProcess(getTheRVGPUTarget(),
                                           createRVGPUInstrPostProcess);
}
