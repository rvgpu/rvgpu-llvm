//===- RVGPUInsertDelayAlu.cpp - Insert s_delay_alu instructions ---------===//
//
...
#define DEBUG_TYPE "rvgpu-insert-delay-alu"

namespace {

class RVGPUInsertDelayAlu : public MachineFunctionPass {
public:
  static char ID;
  ...
  RVGPUInsertDelayAlu() : MachineFunctionPass(ID) {}
  ...
  static bool instructionWaitsForVALU(const MachineInstr &MI) {
    ...
    if (MI.getDesc().TSFlags & VA_VDST_0)
      return true;
    if (MI.getOpcode() == RVGPU::S_SENDMSG_RTN_B32 ||
        MI.getOpcode() == RVGPU::S_SENDMSG_RTN_B64)
      return true;
    if (MI.getOpcode() == RVGPU::S_WAITCNT_DEPCTR &&
        RVGPU::DepCtr::decodeFieldVaVdst(MI.getOperand(0).getImm()) == 0)
      return true;
    return false;
  }
  ...
  MachineInstr *emitDelayAlu(MachineInstr &MI, DelayInfo Delay,
    ...
    MachineInstr *DelayAlu =
        BuildMI(MBB, MI, DebugLoc(), SII->get(RVGPU::S_DELAY_ALU)).addImm(Imm);
  ...
  bool runOnMachineBasicBlock(MachineBasicBlock &MBB, bool Emit) {
    ...
    for (auto &MI : MBB.instrs()) {
      ...
      if (instructionWaitsForVALU(MI)) {
        ...
        for (const auto &Op : MI.explicit_uses()) {
          if (Op.isReg()) {
            ...
            // ignore this operand.
            if (MI.getOpcode() == RVGPU::V_WRITELANE_B32 && Op.isTied())
              continue;
      ...
      unsigned Cycles = RVInstrInfo::getNumWaitStates(MI);
  ...
  bool runOnMachineFunction(MachineFunction &MF) override {
    ...
    LLVM_DEBUG(dbgs() << "RVGPUInsertDelayAlu running on " << MF.getName()
                      << "\n");
...
char RVGPUInsertDelayAlu::ID = 0;

char &llvm::RVGPUInsertDelayAluID = RVGPUInsertDelayAlu::ID;

INITIALIZE_PASS(RVGPUInsertDelayAlu, DEBUG_TYPE, "RVGPU Insert Delay ALU",
                false, false)
