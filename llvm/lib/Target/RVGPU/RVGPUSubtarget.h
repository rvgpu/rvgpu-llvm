//=====-- RVGPUSubtarget.h - Define Subtarget for RVGPU -------*- C++ -*-===//
//
...
//
/// \file
/// Base class for RVGPU specific classes of TargetSubtarget.
//
//===----------------------------------------------------------------------===//
...
enum RVGPUDwarfFlavour : unsigned;
class Function;
class Instruction;
...
public:
  RVGPUSubtarget(const Triple &TT);

  static const RVGPUSubtarget &get(const MachineFunction &MF);
  static const RVGPUSubtarget &get(const TargetMachine &TM,
                                    const Function &F);
  ...
  /// requested using "rvgpu-flat-work-group-size" attribute attached to
  /// function \p F.
  ///
  ...
  /// execution unit explicitly requested using "rvgpu-waves-per-eu" attribute
  ...
  bool isRvHsaOrMesa(const Function &F) const {
    return isRvHsaOS() || isMesaKernel(F);
  }

  bool isGCN() const {
    return TargetTriple.getArch() == Triple::rvgpu;
  }
  ...
  Align getAlignmentForImplicitArgPtr() const {
    return isRvHsaOS() ? Align(8) : Align(4);
  }
  ...
  /// \p WavefrontSize.
  RVGPUDwarfFlavour getRVGPUDwarfFlavour() const;

  virtual ~RVGPUSubtarget() = default;
};

} // end namespace llvm

#endif // LLVM_LIB_TARGET_RVGPU_RVGPUSUBTARGET_H
