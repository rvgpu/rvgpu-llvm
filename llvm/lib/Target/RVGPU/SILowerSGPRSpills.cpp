  ...
  for (auto Reg : MFI->getSGPRSpillVGPRs()) {
    for (MachineBasicBlock *SaveBlock : SaveBlocks) {
      ...
      auto MIB = BuildMI(*SaveBlock, *InsertBefore, InsertBefore->getDebugLoc(),
                         TII->get(RVGPU::IMPLICIT_DEF), Reg);
      MFI->setFlag(Reg, RVGPU::VirtRegFlag::WWM_REG);
      // Set SGPR_SPILL asm printer flag
      MIB->setAsmPrinterFlag(RVGPU::SGPR_SPILL);
      if (LIS) {
...
bool SILowerSGPRSpills::runOnMachineFunction(MachineFunction &MF) {
  ...
  if (HasSGPRSpillToVGPR) {
    ...
    for (MachineBasicBlock &MBB : MF) {
      for (MachineInstr &MI : llvm::make_early_inc_range(MBB)) {
        ...
        int FI = TII->getNamedOperand(MI, RVGPU::OpName::addr)->getIndex();
  ...
  if (SpilledToVirtVGPRLanes) {
    ...
    FuncInfo->setSGPRForEXECCopy(RVGPU::NoRegister);
  }
