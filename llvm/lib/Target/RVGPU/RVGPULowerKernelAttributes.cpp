...
#define DEBUG_TYPE "rvgpu-lower-kernel-attributes"
...
class RVGPULowerKernelAttributes : public ModulePass {
public:
  static char ID;

  RVGPULowerKernelAttributes() : ModulePass(ID) {}

  bool runOnModule(Module &M) override;

  StringRef getPassName() const override {
    return "RVGPU Kernel Attributes";
  }
...
Function *getBasePtrIntrinsic(Module &M, bool IsV5OrAbove) {
  auto IntrinsicId = IsV5OrAbove ? Intrinsic::rvgpu_implicitarg_ptr
                                 : Intrinsic::rvgpu_dispatch_ptr;
...
static bool processUse(CallInst *CI, bool IsV5OrAbove) {
  ...
  if (IsV5OrAbove && HasUniformWorkGroupSize) {
    ...
    for (int I = 0; I < 3; ++I) {
      ...
      auto GroupIDIntrin =
          I == 0 ? m_Intrinsic<Intrinsic::rvgpu_workgroup_id_x>()
                 : (I == 1 ? m_Intrinsic<Intrinsic::rvgpu_workgroup_id_y>()
                           : m_Intrinsic<Intrinsic::rvgpu_workgroup_id_z>());
...
INITIALIZE_PASS_BEGIN(RVGPULowerKernelAttributes, DEBUG_TYPE,
                      "RVGPU Kernel Attributes", false, false)
INITIALIZE_PASS_END(RVGPULowerKernelAttributes, DEBUG_TYPE,
                    "RVGPU Kernel Attributes", false, false)

char RVGPULowerKernelAttributes::ID = 0;

ModulePass *llvm::createRVGPULowerKernelAttributesPass() {
  return new RVGPULowerKernelAttributes();
}
