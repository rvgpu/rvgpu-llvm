//=== lib/CodeGen/GlobalISel/RVGPURegBankCombiner.cpp ---------------===//
//
...
#define DEBUG_TYPE "rvgpu-regbank-combiner"
...
RVGPURegBankCombinerImpl::RVGPURegBankCombinerImpl(
    MachineFunction &MF, CombinerInfo &CInfo, const TargetPassConfig *TPC,
    GISelKnownBits &KB, GISelCSEInfo *CSEInfo,
    const RVGPURegBankCombinerImplRuleConfig &RuleConfig,
...
bool RVGPURegBankCombinerImpl::isVgprRegBank(Register Reg) const {
  return RBI.getRegBank(Reg, MRI, TRI)->getID() == RVGPU::VGPRRegBankID;
}

Register RVGPURegBankCombinerImpl::getAsVgpr(Register Reg) const {
  ...
  for (MachineInstr &Use : MRI.use_instructions(Reg)) {
    ...
    if (Use.getOpcode() == RVGPU::COPY && isVgprRegBank(Def))
      return Def;
  }

  // Copy Reg to vgpr.
  Register VgprReg = B.buildCopy(MRI.getType(Reg), Reg).getReg(0);
  MRI.setRegBank(VgprReg, RBI.getRegBank(RVGPU::VGPRRegBankID));
  return VgprReg;
}

RVGPURegBankCombinerImpl::MinMaxMedOpc
RVGPURegBankCombinerImpl::getMinMaxPair(unsigned Opc) const {
  switch (Opc) {
  default:
    llvm_unreachable("Unsupported opcode");
  case RVGPU::G_SMAX:
  case RVGPU::G_SMIN:
    return {RVGPU::G_SMIN, RVGPU::G_SMAX, RVGPU::G_RVGPU_SMED3};
  case RVGPU::G_UMAX:
  case RVGPU::G_UMIN:
    return {RVGPU::G_UMIN, RVGPU::G_UMAX, RVGPU::G_RVGPU_UMED3};
  case RVGPU::G_FMAXNUM:
  case RVGPU::G_FMINNUM:
    return {RVGPU::G_FMINNUM, RVGPU::G_FMAXNUM, RVGPU::G_RVGPU_FMED3};
  case RVGPU::G_FMAXNUM_IEEE:
  case RVGPU::G_FMINNUM_IEEE:
    return {RVGPU::G_FMINNUM_IEEE, RVGPU::G_FMAXNUM_IEEE,
            RVGPU::G_RVGPU_FMED3};
  }
}

template <class m_Cst, typename CstTy>
bool RVGPURegBankCombinerImpl::matchMed(MachineInstr &MI,
...
bool RVGPURegBankCombinerImpl::matchIntMinMaxToMed3(
  ...
  if (OpcodeTriple.Med == RVGPU::G_RVGPU_SMED3 && K0->Value.sgt(K1->Value))
    return false;
  if (OpcodeTriple.Med == RVGPU::G_RVGPU_UMED3 && K0->Value.ugt(K1->Value))
    return false;
...
bool RVGPURegBankCombinerImpl::matchFPMinMaxToMed3(
    MachineInstr &MI, Med3MatchInfo &MatchInfo) const {
  Register Dst = MI.getOperand(0).getReg();
  LLT Ty = MRI.getType(Dst);
...
bool RVGPURegBankCombinerImpl::matchFPMinMaxToClamp(MachineInstr &MI,
                                                     Register &Reg) const {
...
bool RVGPURegBankCombinerImpl::matchFPMed3ToClamp(MachineInstr &MI,
                                                   Register &Reg) const {
  // In llvm-ir, clamp is often represented as an intrinsic call to
  // @llvm.rvgpu.fmed3.f32(%Val, 0.0, 1.0). Check for other operand orders.
...
void RVGPURegBankCombinerImpl::applyClamp(MachineInstr &MI,
                                           Register &Reg) const {
  B.setInstrAndDebugLoc(MI);
  B.buildInstr(RVGPU::G_RVGPU_CLAMP, {MI.getOperand(0)}, {Reg},
               MI.getFlags());
  MI.eraseFromParent();
}

void RVGPURegBankCombinerImpl::applyMed3(MachineInstr &MI,
...
SIModeRegisterDefaults RVGPURegBankCombinerImpl::getMode() const {
  return MF.getInfo<RVMachineFunctionInfo>()->getMode();
}

bool RVGPURegBankCombinerImpl::getIEEE() const { return getMode().IEEE; }

bool RVGPURegBankCombinerImpl::getDX10Clamp() const {
  return getMode().DX10Clamp;
}

bool RVGPURegBankCombinerImpl::isFminnumIeee(const MachineInstr &MI) const {
  return MI.getOpcode() == RVGPU::G_FMINNUM_IEEE;
}

bool RVGPURegBankCombinerImpl::isFCst(MachineInstr *MI) const {
  return MI->getOpcode() == RVGPU::G_FCONSTANT;
}

bool RVGPURegBankCombinerImpl::isClampZeroToOne(MachineInstr *K0,
...
class RVGPURegBankCombiner : public MachineFunctionPass {
public:
  static char ID;

  RVGPURegBankCombiner(bool IsOptNone = false);

  StringRef getPassName() const override { return "RVGPURegBankCombiner"; }
...
private:
  bool IsOptNone;
  RVGPURegBankCombinerImplRuleConfig RuleConfig;
};
} // end anonymous namespace

void RVGPURegBankCombiner::getAnalysisUsage(AnalysisUsage &AU) const {
...
RVGPURegBankCombiner::RVGPURegBankCombiner(bool IsOptNone)
    : MachineFunctionPass(ID), IsOptNone(IsOptNone) {
  initializeRVGPURegBankCombinerPass(*PassRegistry::getPassRegistry());

  if (!RuleConfig.parseCommandLineOption())
    report_fatal_error("Invalid rule identifier");
}

bool RVGPURegBankCombiner::runOnMachineFunction(MachineFunction &MF) {
  ...
  RVGPURegBankCombinerImpl Impl(MF, CInfo, TPC, *KB, /*CSEInfo*/ nullptr,
                                 RuleConfig, ST, MDT, LI);
  return Impl.combineMachineInstrs();
}

char RVGPURegBankCombiner::ID = 0;
INITIALIZE_PASS_BEGIN(RVGPURegBankCombiner, DEBUG_TYPE,
                      "Combine RVGPU machine instrs after regbankselect",
                      false, false)
INITIALIZE_PASS_DEPENDENCY(TargetPassConfig)
INITIALIZE_PASS_DEPENDENCY(GISelKnownBitsAnalysis)
INITIALIZE_PASS_END(RVGPURegBankCombiner, DEBUG_TYPE,
                    "Combine RVGPU machine instrs after regbankselect", false,
                    false)

namespace llvm {
FunctionPass *createRVGPURegBankCombiner(bool IsOptNone) {
  return new RVGPURegBankCombiner(IsOptNone);
}
} // end namespace llvm
