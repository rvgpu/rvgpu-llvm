...
static cl::opt<bool>
    EnablePreallocateSGPRSpillVGPRs("rvgpu-prealloc-sgpr-spill-vgprs",
...
SIPreAllocateWWMRegs::printWWMInfo(const MachineInstr &MI) {
  ...
  if (Opc == RVGPU::ENTER_STRICT_WWM || Opc == RVGPU::ENTER_STRICT_WQM ||
      Opc == RVGPU::ENTER_PSEUDO_WM) {
    dbgs() << "Entering ";
  } else {
    assert(Opc == RVGPU::EXIT_STRICT_WWM || Opc == RVGPU::EXIT_STRICT_WQM ||
           Opc == RVGPU::EXIT_PSEUDO_WM);
    dbgs() << "Exiting ";
  }

  if (Opc == RVGPU::ENTER_STRICT_WWM || Opc == RVGPU::EXIT_STRICT_WWM) {
    dbgs() << "Strict WWM ";
  } else if (Opc == RVGPU::ENTER_PSEUDO_WM || Opc == RVGPU::EXIT_PSEUDO_WM) {
    dbgs() << "Pseudo WWM/WQM ";
  } else {
    assert(Opc == RVGPU::ENTER_STRICT_WQM || Opc == RVGPU::EXIT_STRICT_WQM);
    dbgs() << "Strict WQM ";
  }
...
bool SIPreAllocateWWMRegs::runOnMachineFunction(MachineFunction &MF) {
  ...
  bool PreallocateSGPRSpillVGPRs =
      ...
      MF.getFunction().hasFnAttribute("rvgpu-prealloc-sgpr-spill-vgprs");
  ...
  for (MachineBasicBlock *MBB : RPOT) {
    ...
    for (MachineInstr &MI : *MBB) {
      ...
      if (MI.getOpcode() == RVGPU::ENTER_STRICT_WWM ||
          MI.getOpcode() == RVGPU::ENTER_STRICT_WQM ||
          MI.getOpcode() == RVGPU::ENTER_PSEUDO_WM) {
        LLVM_DEBUG(printWWMInfo(MI));
        InWWM = true;
        continue;
      }

      if (MI.getOpcode() == RVGPU::EXIT_STRICT_WWM ||
          MI.getOpcode() == RVGPU::EXIT_STRICT_WQM ||
          MI.getOpcode() == RVGPU::EXIT_PSEUDO_WM) {
        LLVM_DEBUG(printWWMInfo(MI));
        InWWM = false;
      }
