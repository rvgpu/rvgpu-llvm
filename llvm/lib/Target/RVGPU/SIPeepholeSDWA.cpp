...
public:
  ...
  virtual MachineInstr *potentialToConvert(const RVInstrInfo *TII) = 0;
  virtual bool convertToSDWA(MachineInstr &MI, const RVInstrInfo *TII) = 0;
...
public:
  ...
  MachineInstr *potentialToConvert(const RVInstrInfo *TII) override;
  bool convertToSDWA(MachineInstr &MI, const RVInstrInfo *TII) override;
  ...
  uint64_t getSrcMods(const RVInstrInfo *TII,
...
uint64_t SDWASrcOperand::getSrcMods(const RVInstrInfo *TII,
                                    const MachineOperand *SrcOp) const {
  uint64_t Mods = 0;
  const auto *MI = SrcOp->getParent();
  if (TII->getNamedOperand(*MI, RVGPU::OpName::src0) == SrcOp) {
    if (auto *Mod = TII->getNamedOperand(*MI, RVGPU::OpName::src0_modifiers)) {
      Mods = Mod->getImm();
    }
  } else if (TII->getNamedOperand(*MI, RVGPU::OpName::src1) == SrcOp) {
    if (auto *Mod = TII->getNamedOperand(*MI, RVGPU::OpName::src1_modifiers)) {
      Mods = Mod->getImm();
    }
  }
  if (Abs || Neg) {
...
MachineInstr *SDWASrcOperand::potentialToConvert(const RVInstrInfo *TII) {
...
bool SDWASrcOperand::convertToSDWA(MachineInstr &MI, const RVInstrInfo *TII) {
  ...
  MachineOperand *Src = TII->getNamedOperand(MI, RVGPU::OpName::src0);
  MachineOperand *SrcSel = TII->getNamedOperand(MI, RVGPU::OpName::src0_sel);
  ...
  if (!isSameReg(*Src, *getReplacedOperand())) {
    ...
    Src = TII->getNamedOperand(MI, RVGPU::OpName::src1);
    SrcSel = TII->getNamedOperand(MI, RVGPU::OpName::src1_sel);
    SrcMods = TII->getNamedOperand(MI, RVGPU::OpName::src1_modifiers);
    ...
    if (!Src ||
      ...
      MachineOperand *Dst = TII->getNamedOperand(MI, RVGPU::OpName::vdst);
      MachineOperand *DstUnused =
        TII->getNamedOperand(MI, RVGPU::OpName::dst_unused);

      if (Dst &&
          DstUnused->getImm() == RVGPU::SDWA::DstUnused::UNUSED_PRESERVE) {
        ...
        SdwaSel DstSel = static_cast<SdwaSel>(
            TII->getNamedImmOperand(MI, RVGPU::OpName::dst_sel));
        if (DstSel == RVGPU::SDWA::SdwaSel::WORD_1 &&
            getSrcSel() == RVGPU::SDWA::SdwaSel::WORD_0) {
          IsPreserveSrc = true;
          auto DstIdx = RVGPU::getNamedOperandIdx(MI.getOpcode(),
                                                   RVGPU::OpName::vdst);
    ...
    if ((MI.getOpcode() == RVGPU::V_FMAC_F16_sdwa ||
         MI.getOpcode() == RVGPU::V_FMAC_F32_sdwa ||
         MI.getOpcode() == RVGPU::V_MAC_F16_sdwa ||
         MI.getOpcode() == RVGPU::V_MAC_F32_sdwa) &&
         !isSameReg(*Src, *getReplacedOperand())) {
...
MachineInstr *SDWADstOperand::potentialToConvert(const RVInstrInfo *TII) {
...
bool SDWADstOperand::convertToSDWA(MachineInstr &MI, const RVInstrInfo *TII) {
  ...
  if ((MI.getOpcode() == RVGPU::V_FMAC_F16_sdwa ||
       ...
       MI.getOpcode() == RVGPU::V_MAC_F16_sdwa ||
       MI.getOpcode() == RVGPU::V_MAC_F32_sdwa) &&
      getDstSel() != RVGPU::SDWA::DWORD) {
    // v_mac_f16/32_sdwa allow dst_sel to be equal only to DWORD
    return false;
  }

  MachineOperand *Operand = TII->getNamedOperand(MI, RVGPU::OpName::vdst);
  assert(Operand &&
         Operand->isReg() &&
         isSameReg(*Operand, *getReplacedOperand()));
  copyRegOperand(*Operand, *getTargetOperand());
  MachineOperand *DstSel= TII->getNamedOperand(MI, RVGPU::OpName::dst_sel);
  assert(DstSel);
  DstSel->setImm(getDstSel());
  MachineOperand *DstUnused= TII->getNamedOperand(MI, RVGPU::OpName::dst_unused);
  assert(DstUnused);
...
bool SDWADstPreserveOperand::convertToSDWA(MachineInstr &MI,
  ...
  MI.tieOperands(RVGPU::getNamedOperandIdx(MI.getOpcode(), RVGPU::OpName::vdst),
                 MI.getNumOperands() - 1);
...
SIPeepholeSDWA::matchSDWAOperand(MachineInstr &MI) {
  ...
  case RVGPU::V_LSHLREV_B32_e64: {
    ...
    if (Opcode == RVGPU::V_LSHLREV_B32_e32 ||
        Opcode == RVGPU::V_LSHLREV_B32_e64) {
      ...
      return std::make_unique<SDWASrcOperand>(
          ...
          Opcode != RVGPU::V_LSHRREV_B32_e32 &&
          Opcode != RVGPU::V_LSHRREV_B32_e64);
    }
    break;
  }
  ...
  case RVGPU::V_LSHLREV_B16_e64: {
    ...
    if (Opcode == RVGPU::V_LSHLREV_B16_e32 ||
        Opcode == RVGPU::V_LSHLREV_B16_e64) {
      return std::make_unique<SDWADstOperand>(Dst, Src1, BYTE_1, UNUSED_PAD);
    } else {
      return std::make_unique<SDWASrcOperand>(
            Src1, Dst, BYTE_1, false, false,
            Opcode != RVGPU::V_LSHRREV_B16_e32 &&
            Opcode != RVGPU::V_LSHRREV_B16_e64);
    }
    break;
  }

  case RVGPU::V_BFE_I32_e64:
  case RVGPU::V_BFE_U32_e64: {
    // e.g.:
    ...
    MachineOperand *Src2 = TII->getNamedOperand(MI, RVGPU::OpName::src2);
    ...
    return std::make_unique<SDWASrcOperand>(
          Src0, Dst, SrcSel, false, false, Opcode != RVGPU::V_BFE_U32_e64);
  }
  ...
  case RVGPU::V_OR_B32_e64: {
    ...
    MachineOperand *OrSDWA = TII->getNamedOperand(MI, RVGPU::OpName::src0);
    MachineOperand *OrOther = TII->getNamedOperand(MI, RVGPU::OpName::src1);
    assert(OrSDWA && OrOther);
    auto Res = CheckOROperandsForSDWA(OrSDWA, OrOther);
    if (!Res) {
      OrSDWA = TII->getNamedOperand(MI, RVGPU::OpName::src1);
      OrOther = TII->getNamedOperand(MI, RVGPU::OpName::src0);
      assert(OrSDWA && OrOther);
    ...
    SdwaSel DstSel = static_cast<SdwaSel>(
        TII->getNamedImmOperand(*SDWAInst, RVGPU::OpName::dst_sel));
    SdwaSel OtherDstSel = static_cast<SdwaSel>(
      TII->getNamedImmOperand(*OtherInst, RVGPU::OpName::dst_sel));
    ...
    DstUnused OtherDstUnused = static_cast<DstUnused>(
      TII->getNamedImmOperand(*OtherInst, RVGPU::OpName::dst_unused));
    if (OtherDstUnused != DstUnused::UNUSED_PAD)
      break;

    // Create DstPreserveOperand
    MachineOperand *OrDst = TII->getNamedOperand(MI, RVGPU::OpName::vdst);
...
void SIPeepholeSDWA::pseudoOpConvertToVOP2(MachineInstr &MI,
  ...
  if (!TII->canShrink(MI, *MRI))
    return;
  Opc = RVGPU::getVOPe32(Opc);
  // Find the related ADD instruction.
  const MachineOperand *Sdst = TII->getNamedOperand(MI, RVGPU::OpName::sdst);
  if (!Sdst)
    return;
  ...
  MachineOperand *CarryIn = TII->getNamedOperand(MISucc, RVGPU::OpName::src2);
  if (!CarryIn)
    return;
  MachineOperand *CarryOut = TII->getNamedOperand(MISucc, RVGPU::OpName::sdst);
  if (!CarryOut)
    return;
  ...
  auto Liveness = MBB.computeRegisterLiveness(TRI, RVGPU::VCC, MI, 25);
  if (Liveness != MachineBasicBlock::LQR_Dead)
    return;
  // Check if VCC is referenced in range of (MI,MISucc].
  for (auto I = std::next(MI.getIterator()), E = MISucc.getIterator();
       I != E; ++I) {
    if (I->modifiesRegister(RVGPU::VCC, TRI))
      return;
  }

  // Replace MI with V_{SUB|ADD}_I32_e32
  BuildMI(MBB, MI, MI.getDebugLoc(), TII->get(Opc))
    .add(*TII->getNamedOperand(MI, RVGPU::OpName::vdst))
    .add(*TII->getNamedOperand(MI, RVGPU::OpName::src0))
    .add(*TII->getNamedOperand(MI, RVGPU::OpName::src1))
...
bool SIPeepholeSDWA::isConvertibleToSDWA(MachineInstr &MI,
  ...
  if (RVGPU::getSDWAOp(Opc) == -1)
    Opc = RVGPU::getVOPe32(Opc);

  if (RVGPU::getSDWAOp(Opc) == -1)
    return false;

  if (!ST.hasSDWAOmod() && TII->hasModifiersSet(MI, RVGPU::OpName::omod))
    return false;

  if (TII->isVOPC(Opc)) {
    if (!ST.hasSDWASdst()) {
      const MachineOperand *SDst = TII->getNamedOperand(MI, RVGPU::OpName::sdst);
      if (SDst && (SDst->getReg() != RVGPU::VCC &&
                   SDst->getReg() != RVGPU::VCC_LO))
        return false;
    }
  ...
  } else if (TII->getNamedOperand(MI, RVGPU::OpName::sdst) ||
             !TII->getNamedOperand(MI, RVGPU::OpName::vdst)) {
    return false;
  }

  if (!ST.hasSDWAMac() && (Opc == RVGPU::V_FMAC_F16_e32 ||
                           Opc == RVGPU::V_FMAC_F32_e32 ||
                           Opc == RVGPU::V_MAC_F16_e32 ||
                           Opc == RVGPU::V_MAC_F32_e32))
    return false;
  ...
  if (Opc == RVGPU::V_CNDMASK_B32_e32)
    return false;

  if (MachineOperand *Src0 = TII->getNamedOperand(MI, RVGPU::OpName::src0)) {
    if (!Src0->isReg() && !Src0->isImm())
      return false;
  }

  if (MachineOperand *Src1 = TII->getNamedOperand(MI, RVGPU::OpName::src1)) {
...
bool SIPeepholeSDWA::convertToSDWA(MachineInstr &MI,
  ...
  if (TII->isSDWA(Opcode)) {
    ...
    SDWAOpcode = RVGPU::getSDWAOp(Opcode);
    if (SDWAOpcode == -1)
      SDWAOpcode = RVGPU::getSDWAOp(RVGPU::getVOPe32(Opcode));
  }
  assert(SDWAOpcode != -1);
  ...
  MachineOperand *Dst = TII->getNamedOperand(MI, RVGPU::OpName::vdst);
  if (Dst) {
    assert(RVGPU::hasNamedOperand(SDWAOpcode, RVGPU::OpName::vdst));
    SDWAInst.add(*Dst);
  } else if ((Dst = TII->getNamedOperand(MI, RVGPU::OpName::sdst))) {
    assert(Dst && RVGPU::hasNamedOperand(SDWAOpcode, RVGPU::OpName::sdst));
    SDWAInst.add(*Dst);
  } else {
    assert(RVGPU::hasNamedOperand(SDWAOpcode, RVGPU::OpName::sdst));
  ...
  MachineOperand *Src0 = TII->getNamedOperand(MI, RVGPU::OpName::src0);
  assert(Src0 && RVGPU::hasNamedOperand(SDWAOpcode, RVGPU::OpName::src0) &&
         RVGPU::hasNamedOperand(SDWAOpcode, RVGPU::OpName::src0_modifiers));
  if (auto *Mod = TII->getNamedOperand(MI, RVGPU::OpName::src0_modifiers))
  ...
  MachineOperand *Src1 = TII->getNamedOperand(MI, RVGPU::OpName::src1);
  if (Src1) {
    assert(RVGPU::hasNamedOperand(SDWAOpcode, RVGPU::OpName::src1) &&
           RVGPU::hasNamedOperand(SDWAOpcode, RVGPU::OpName::src1_modifiers));
    if (auto *Mod = TII->getNamedOperand(MI, RVGPU::OpName::src1_modifiers))
      SDWAInst.addImm(Mod->getImm());
    else
      SDWAInst.addImm(0);
  ...
  if (SDWAOpcode == RVGPU::V_FMAC_F16_sdwa ||
      SDWAOpcode == RVGPU::V_FMAC_F32_sdwa ||
      SDWAOpcode == RVGPU::V_MAC_F16_sdwa ||
      SDWAOpcode == RVGPU::V_MAC_F32_sdwa) {
  ...
  assert(RVGPU::hasNamedOperand(SDWAOpcode, RVGPU::OpName::clamp));
  MachineOperand *Clamp = TII->getNamedOperand(MI, RVGPU::OpName::clamp);
  if (Clamp) {
    SDWAInst.add(*Clamp);
  } else {
    SDWAInst.addImm(0);
  ...
  if (RVGPU::hasNamedOperand(SDWAOpcode, RVGPU::OpName::omod)) {
    MachineOperand *OMod = TII->getNamedOperand(MI, RVGPU::OpName::omod);
    if (OMod) {
      SDWAInst.add(*OMod);
    } else {
      SDWAInst.addImm(0);
  ...
  if (RVGPU::hasNamedOperand(SDWAOpcode, RVGPU::OpName::dst_sel)) {
    MachineOperand *DstSel = TII->getNamedOperand(MI, RVGPU::OpName::dst_sel);
    if (DstSel) {
      SDWAInst.add(*DstSel);
    } else {
      SDWAInst.addImm(RVGPU::SDWA::SdwaSel::DWORD);
    }
  }

  // Copy dst_unused if present, initialize otherwise if needed
  if (RVGPU::hasNamedOperand(SDWAOpcode, RVGPU::OpName::dst_unused)) {
    MachineOperand *DstUnused = TII->getNamedOperand(MI, RVGPU::OpName::dst_unused);
    if (DstUnused) {
      SDWAInst.add(*DstUnused);
    } else {
      SDWAInst.addImm(RVGPU::SDWA::DstUnused::UNUSED_PAD);
    }
  }

  // Copy src0_sel if present, initialize otherwise
  assert(RVGPU::hasNamedOperand(SDWAOpcode, RVGPU::OpName::src0_sel));
  MachineOperand *Src0Sel = TII->getNamedOperand(MI, RVGPU::OpName::src0_sel);
  if (Src0Sel) {
  ...
  if (Src1) {
    assert(RVGPU::hasNamedOperand(SDWAOpcode, RVGPU::OpName::src1_sel));
    MachineOperand *Src1Sel = TII->getNamedOperand(MI, RVGPU::OpName::src1_sel);
    if (Src1Sel) {
  ...
  auto DstUnused = TII->getNamedOperand(MI, RVGPU::OpName::dst_unused);
  if (DstUnused &&
      DstUnused->getImm() == RVGPU::SDWA::DstUnused::UNUSED_PRESERVE) {
    ...
    auto PreserveDstIdx = RVGPU::getNamedOperandIdx(SDWAOpcode, RVGPU::OpName::vdst);
...
void SIPeepholeSDWA::legalizeScalarOperands(MachineInstr &MI,
  ...
  for (MachineOperand &Op : MI.explicit_uses()) {
    ...
    Register VGPR = MRI->createVirtualRegister(&RVGPU::VGPR_32RegClass);
    auto Copy = BuildMI(*MI.getParent(), MI.getIterator(), MI.getDebugLoc(),
                        TII->get(RVGPU::V_MOV_B32_e32), VGPR);
    if (Op.isImm())
  ...
  for (MachineBasicBlock &MBB : MF) {
    ...
    do {
      ...
      for (const auto &OperandPair : SDWAOperands) {
        ...
        if (PotentialMI &&
           (PotentialMI->getOpcode() == RVGPU::V_ADD_CO_U32_e64 ||
            PotentialMI->getOpcode() == RVGPU::V_SUB_CO_U32_e64))
