...
static bool updateVMCntOnly(const MachineInstr &Inst) {
  return RVInstrInfo::isVMEM(Inst) || RVInstrInfo::isFLATGlobal(Inst) ||
         RVInstrInfo::isFLATScratch(Inst);
}

VmemType getVmemType(const MachineInstr &Inst) {
  assert(updateVMCntOnly(Inst));
  if (!RVInstrInfo::isMIMG(Inst))
    return VMEM_NOSAMPLER;
  const RVGPU::MIMGInfo *Info = RVGPU::getMIMGInfo(Inst.getOpcode());
...
void addWait(RVGPU::Waitcnt &Wait, InstCounterType T, unsigned Count) {
  switch (T) {
  case VM_CNT:
...
public:
  ...
  RegInterval getRegInterval(const MachineInstr *MI,
                             ...
                             const RVRegisterInfo *TRI, unsigned OpNo) const;

  bool counterOutOfOrder(InstCounterType T) const;
  void simplifyWaitcnt(RVGPU::Waitcnt &Wait) const;
  void simplifyWaitcnt(InstCounterType T, unsigned &Count) const;
  void determineWait(InstCounterType T, int RegNo, RVGPU::Waitcnt &Wait) const;
  void applyWaitcnt(const RVGPU::Waitcnt &Wait);
  void applyWaitcnt(InstCounterType T, unsigned Count);
  void updateByEvent(const RVInstrInfo *TII, const RVRegisterInfo *TRI,
...
private:
  ...
  void setExpScore(const MachineInstr *MI, const RVInstrInfo *TII,
...
private:
  ...
  // because of rvgpu-waitcnt-forcezero flag
...
// For debug builds, get the debug counter info and adjust if need be
  ...
  WaitEventType getVmemWaitEventType(const MachineInstr &Inst) const {
    assert(RVInstrInfo::isVMEM(Inst) || RVInstrInfo::isFLAT(Inst));
    // LDS DMA loads are also stores, but on the LDS side. On the VMEM side
    // these should use VM_CNT.
    if (!ST->hasVscnt() || RVInstrInfo::mayWriteLDSThroughDMA(Inst))
      return VMEM_ACCESS;
    if (Inst.mayStore() && !RVInstrInfo::isAtomicRet(Inst)) {
      // FLAT and SCRATCH instructions may access scratch. Other VMEM
      // instructions do not.
      if (RVInstrInfo::isFLAT(Inst) && mayAccessScratchThroughFlat(Inst))
  ...
  bool generateWaitcntBlockEnd(MachineBasicBlock &Block,
                               WaitcntBrackets &ScoreBrackets,
                               MachineInstr *OldWaitcntInstr);
  bool generateWaitcnt(RVGPU::Waitcnt Wait,
...
RegInterval WaitcntBrackets::getRegInterval(const MachineInstr *MI,
  ...
  unsigned Reg = TRI->getEncodingValue(RVGPU::getMCReg(Op.getReg(), *ST)) &
...
void WaitcntBrackets::updateByEvent(const RVInstrInfo *TII,
                                    const RVRegisterInfo *TRI,
                                    const MachineRegisterInfo *MRI,
  ...
  if (T == EXP_CNT) {
    ...
    if (TII->isDS(Inst) && (Inst.mayStore() || Inst.mayLoad())) {
      int AddrOpIdx =
          RVGPU::getNamedOperandIdx(Inst.getOpcode(), RVGPU::OpName::addr);
      ...
      if (Inst.mayStore()) {
        if (RVGPU::hasNamedOperand(Inst.getOpcode(), RVGPU::OpName::data0)) {
          setExpScore(
              &Inst, TII, TRI, MRI,
              RVGPU::getNamedOperandIdx(Inst.getOpcode(), RVGPU::OpName::data0),
              CurrScore);
        }
        if (RVGPU::hasNamedOperand(Inst.getOpcode(), RVGPU::OpName::data1)) {
          setExpScore(&Inst, TII, TRI, MRI,
                      RVGPU::getNamedOperandIdx(Inst.getOpcode(),
                                                 RVGPU::OpName::data1),
                      CurrScore);
        }
      } else if (RVInstrInfo::isAtomicRet(Inst) && !RVInstrInfo::isGWS(Inst) &&
                 Inst.getOpcode() != RVGPU::DS_APPEND &&
                 Inst.getOpcode() != RVGPU::DS_CONSUME &&
                 Inst.getOpcode() != RVGPU::DS_ORDERED_COUNT) {
      ...
      if (Inst.mayStore()) {
        setExpScore(
            &Inst, TII, TRI, MRI,
            RVGPU::getNamedOperandIdx(Inst.getOpcode(), RVGPU::OpName::data),
            CurrScore);
      } else if (RVInstrInfo::isAtomicRet(Inst)) {
        setExpScore(
            &Inst, TII, TRI, MRI,
            RVGPU::getNamedOperandIdx(Inst.getOpcode(), RVGPU::OpName::data),
            CurrScore);
      ...
      setExpScore(
          &Inst, TII, TRI, MRI,
          RVGPU::getNamedOperandIdx(Inst.getOpcode(), RVGPU::OpName::vdst),
          CurrScore);
    } else {
      if (TII->isEXP(Inst)) {
        ...
        for (unsigned I = 0, E = Inst.getNumOperands(); I != E; ++I) {
          ...
          if (DefMO.isReg() && DefMO.isDef() &&
            ...
            setRegScore(
                TRI->getEncodingValue(RVGPU::getMCReg(DefMO.getReg(), *ST)),
                EXP_CNT, CurrScore);
          }
        }
      }
  ...
  } else if (Inst.getOpcode() == RVGPU::BUFFER_STORE_DWORD ||
       Inst.getOpcode() == RVGPU::BUFFER_STORE_DWORDX2 ||
       Inst.getOpcode() == RVGPU::BUFFER_STORE_DWORDX4) {
    MachineOperand *MO = TII->getNamedOperand(Inst, RVGPU::OpName::data);
...
void WaitcntBrackets::simplifyWaitcnt(RVGPU::Waitcnt &Wait) const {
...
void WaitcntBrackets::applyWaitcnt(const RVGPU::Waitcnt &Wait) {
...
static bool updateOperandIfDifferent(MachineInstr &MI, uint16_t OpName,
                                     unsigned NewEnc) {
  int OpIdx = RVGPU::getNamedOperandIdx(MI.getOpcode(), OpName);
  assert(OpIdx >= 0);
...
bool SIInsertWaitcnts::promoteSoftWaitCnt(MachineInstr *Waitcnt) const {
  ...
  Waitcnt->setDesc(TII->get(RVInstrInfo::getNonSoftWaitcntOpcode(Opcode)));
  return true;
}
...
bool SIInsertWaitcnts::applyPreexistingWaitcnt(
    ...
    RVGPU::Waitcnt &Wait, MachineBasicBlock::instr_iterator It) const {
  bool Modified = false;
  ...
  for (auto &II :
    ...
    bool IsSoft = RVInstrInfo::isSoftWaitcnt(Opcode);

    if (RVInstrInfo::isWaitcnt(Opcode)) {
      // Update required wait count. If this is a soft waitcnt (= it was added
      // by an earlier pass), it may be entirely removed.
      unsigned IEnc = II.getOperand(0).getImm();
      RVGPU::Waitcnt OldWait = RVGPU::decodeWaitcnt(IV, IEnc);
      if (IsSoft)
      ...
      assert(RVInstrInfo::isWaitcntVsCnt(Opcode));
      assert(II.getOperand(0).getReg() == RVGPU::SGPR_NULL);

      unsigned OldVSCnt =
          TII->getNamedOperand(II, RVGPU::OpName::simm16)->getImm();
      if (IsSoft)
  ...
  if (WaitcntInstr) {
    Modified |= updateOperandIfDifferent(*WaitcntInstr, RVGPU::OpName::simm16,
                                         RVGPU::encodeWaitcnt(IV, Wait));
  ...
  if (WaitcntVsCntInstr) {
    Modified |= updateOperandIfDifferent(*WaitcntVsCntInstr,
                                         RVGPU::OpName::simm16, Wait.VsCnt);
...
static bool readsVCCZ(const MachineInstr &MI) {
  ...
  return (Opc == RVGPU::S_CBRANCH_VCCNZ || Opc == RVGPU::S_CBRANCH_VCCZ) &&
...
bool SIInsertWaitcnts::generateWaitcntInstBefore(MachineInstr &MI,
  ...
  if (MI.getOpcode() == RVGPU::BUFFER_WBINVL1 ||
      MI.getOpcode() == RVGPU::BUFFER_WBINVL1_SC ||
      MI.getOpcode() == RVGPU::BUFFER_WBINVL1_VOL ||
      MI.getOpcode() == RVGPU::BUFFER_GL0_INV ||
      MI.getOpcode() == RVGPU::BUFFER_GL1_INV) {
    Wait.VmCnt = 0;
  }
  ...
  if (MI.getOpcode() == RVGPU::SI_RETURN_TO_EPILOG ||
      MI.getOpcode() == RVGPU::SI_RETURN ||
      MI.getOpcode() == RVGPU::S_SETPC_B64_return ||
      (MI.isReturn() && MI.isCall() && !callWaitsOnFunctionEntry(MI))) {
    Wait = Wait.combined(RVGPU::Waitcnt::allZeroExceptVsCnt());
  }
  ...
  // before overwriting the EXEC mask.
    ...
    if (MI.isCall() && callWaitsOnFunctionEntry(MI)) {
      ...
      if (MI.getOperand(CallAddrOpIdx).isReg()) {
        ...
        int RtnAddrOpIdx =
          RVGPU::getNamedOperandIdx(MI.getOpcode(), RVGPU::OpName::dst);
        if (RtnAddrOpIdx != -1) {
      ...
      for (const MachineMemOperand *Memop : MI.memoperands()) {
        ...
        if (AS != RVGPUAS::LOCAL_ADDRESS && AS != RVGPUAS::FLAT_ADDRESS)
          continue;
  ...
  if (MI.getOpcode() == RVGPU::S_BARRIER &&
      !ST->hasAutoWaitcntBeforeBarrier() && !ST->supportsBackOffBarrier()) {
    Wait = Wait.combined(RVGPU::Waitcnt::allZero(ST->hasVscnt()));
  }
  ...
  if (ForceEmitZeroWaitcnts)
    Wait = RVGPU::Waitcnt::allZeroExceptVsCnt();
...
bool SIInsertWaitcnts::generateWaitcnt(RVGPU::Waitcnt Wait,
                                       MachineBasicBlock::instr_iterator It,
  ...
  if (Wait.ExpCnt != ~0u && It != Block.instr_end() &&
      RVInstrInfo::isVINTERP(*It)) {
    MachineOperand *WaitExp =
        TII->getNamedOperand(*It, RVGPU::OpName::waitexp);
  ...
  if (Wait.hasWaitExceptVsCnt()) {
    unsigned Enc = RVGPU::encodeWaitcnt(IV, Wait);
    [[maybe_unused]] auto SWaitInst =
        BuildMI(Block, It, DL, TII->get(RVGPU::S_WAITCNT)).addImm(Enc);
    Modified = true;
  ...
  if (Wait.hasWaitVsCnt()) {
    ...
    [[maybe_unused]] auto SWaitInst = BuildMI(Block, It, DL, TII->get(RVGPU::S_WAITCNT_VSCNT))
...
bool SIInsertWaitcnts::mayAccessVMEMThroughFlat(const MachineInstr &MI) const {
  ...
  for (const MachineMemOperand *Memop : MI.memoperands()) {
    unsigned AS = Memop->getAddrSpace();
    assert(AS != RVGPUAS::REGION_ADDRESS);
...
void SIInsertWaitcnts::updateEventWaitcntAfter(MachineInstr &Inst,
  ...
  if (TII->isDS(Inst) && TII->usesLGKM_CNT(Inst)) {
    if (TII->isAlwaysGDS(Inst.getOpcode()) ||
        TII->hasModifiersSet(Inst, RVGPU::OpName::gds)) {
  ...
  } else if (RVInstrInfo::isVMEM(Inst) &&
             !llvm::RVGPU::getMUBUFIsBufferInv(Inst.getOpcode())) {
    ScoreBrackets->updateByEvent(TII, TRI, MRI, getVmemWaitEventType(Inst),
                                 Inst);

    if (ST->vmemWriteNeedsExpWaitcnt() &&
        (Inst.mayStore() || RVInstrInfo::isAtomicRet(Inst))) {
    ...
    if (callWaitsOnFunctionReturn(Inst)) {
      ...
      ScoreBrackets->applyWaitcnt(RVGPU::Waitcnt::allZeroExceptVsCnt());
    } else {
      // May need to way wait for anything.
      ScoreBrackets->applyWaitcnt(RVGPU::Waitcnt());
    }
  } else if (RVInstrInfo::isLDSDIR(Inst)) {
    ScoreBrackets->updateByEvent(TII, TRI, MRI, EXP_LDS_ACCESS, Inst);
  } else if (TII->isVINTERP(Inst)) {
    int64_t Imm = TII->getNamedOperand(Inst, RVGPU::OpName::waitexp)->getImm();
    ScoreBrackets->applyWaitcnt(EXP_CNT, Imm);
  } else if (RVInstrInfo::isEXP(Inst)) {
    unsigned Imm = TII->getNamedOperand(Inst, RVGPU::OpName::tgt)->getImm();
    if (Imm >= RVGPU::Exp::ET_PARAM0 && Imm <= RVGPU::Exp::ET_PARAM31)
      ScoreBrackets->updateByEvent(TII, TRI, MRI, EXP_PARAM_ACCESS, Inst);
    else if (Imm >= RVGPU::Exp::ET_POS0 && Imm <= RVGPU::Exp::ET_POS_LAST)
    ...
    case RVGPU::S_SENDMSG_RTN_B32:
    case RVGPU::S_SENDMSG_RTN_B64:
    case RVGPU::S_SENDMSGHALT:
      ScoreBrackets->updateByEvent(TII, TRI, MRI, SQ_MESSAGE, Inst);
      break;
    case RVGPU::S_MEMTIME:
    case RVGPU::S_MEMREALTIME:
    case RVGPU::S_BARRIER_SIGNAL_ISFIRST_M0:
    case RVGPU::S_BARRIER_SIGNAL_ISFIRST_IMM:
    case RVGPU::S_BARRIER_LEAVE:
    case RVGPU::S_GET_BARRIER_STATE_M0:
    case RVGPU::S_GET_BARRIER_STATE_IMM:
      ScoreBrackets->updateByEvent(TII, TRI, MRI, SMEM_ACCESS, Inst);
      break;
    }
  }
...
static bool isWaitInstr(MachineInstr &Inst) {
  ...
  return RVInstrInfo::isWaitcnt(Opcode) ||
         (RVInstrInfo::isWaitcntVsCnt(Opcode) && Inst.getOperand(0).isReg() &&
          Inst.getOperand(0).getReg() == RVGPU::SGPR_NULL);
}
...
bool SIInsertWaitcnts::insertWaitcntInBlock(MachineFunction &MF,
  ...
  for (MachineBasicBlock::instr_iterator Iter = Block.instr_begin(),
    ...
    if (ST->hasReadVCCZBug() || !ST->partialVCCWritesUpdateVCCZ()) {
      if (Inst.definesRegister(RVGPU::VCC_LO) ||
          Inst.definesRegister(RVGPU::VCC_HI)) {
        // Up to gfx9, writes to vcc_lo and vcc_hi don't update vccz.
        if (!ST->partialVCCWritesUpdateVCCZ())
          VCCZCorrect = false;
      } else if (Inst.definesRegister(RVGPU::VCC)) {
    ...
    if (RestoreVCCZ) {
      ...
      BuildMI(Block, Inst, Inst.getDebugLoc(),
              TII->get(ST->isWave32() ? RVGPU::S_MOV_B32 : RVGPU::S_MOV_B64),
              TRI->getVCC())
          .addReg(TRI->getVCC());
      VCCZCorrect = true;
      Modified = true;
...
bool SIInsertWaitcnts::isVMEMOrFlatVMEM(const MachineInstr &MI) const {
  return RVInstrInfo::isVMEM(MI) ||
         (RVInstrInfo::isFLAT(MI) && mayAccessVMEMThroughFlat(MI));
}
...
bool SIInsertWaitcnts::runOnMachineFunction(MachineFunction &MF) {
  ...
  MRI = &MF.getRegInfo();
  IV = RVGPU::getIsaVersion(ST->getCPU());
  ...
  HardwareLimits Limits = {};
  Limits.VmcntMax = RVGPU::getVmcntBitMask(IV);
  Limits.ExpcntMax = RVGPU::getExpcntBitMask(IV);
  Limits.LgkmcntMax = RVGPU::getLgkmcntBitMask(IV);
  ...
  RegisterEncoding Encoding = {};
  Encoding.VGPR0 =
      TRI->getEncodingValue(RVGPU::VGPR0) & RVGPU::HWEncoding::REG_IDX_MASK;
  Encoding.VGPRL = Encoding.VGPR0 + NumVGPRsMax - 1;
  Encoding.SGPR0 =
      TRI->getEncodingValue(RVGPU::SGPR0) & RVGPU::HWEncoding::REG_IDX_MASK;
  ...
  if (!MFI->isEntryFunction()) {
    ...
    for (MachineBasicBlock::iterator E = EntryBB.end();
      ...
      ;
    BuildMI(EntryBB, I, DebugLoc(), TII->get(RVGPU::S_WAITCNT)).addImm(0);
  ...
  if (ST->hasScalarStores()) {
    ...
    for (MachineBasicBlock &MBB : MF) {
      for (MachineInstr &MI : MBB) {
        ...
        if (MI.getOpcode() == RVGPU::S_ENDPGM ||
    ...
    if (HaveScalarStores) {
      ...
      for (MachineBasicBlock *MBB : EndPgmBlocks) {
        ...
        for (MachineBasicBlock::iterator I = MBB->begin(), E = MBB->end();
             I != E; ++I) {
          if (I->getOpcode() == RVGPU::S_DCACHE_WB)
            SeenDCacheWB = true;
          else if (TII->isScalarStore(*I))
            SeenDCacheWB = false;

          // FIXME: It would be better to insert this before a waitcnt if any.
          if ((I->getOpcode() == RVGPU::S_ENDPGM ||
               I->getOpcode() == RVGPU::SI_RETURN_TO_EPILOG) &&
              !SeenDCacheWB) {
            Modified = true;
            BuildMI(*MBB, I, I->getDebugLoc(), TII->get(RVGPU::S_DCACHE_WB));
          }
        }
      }
    }
  ...
  for (MachineInstr *MI : ReleaseVGPRInsts) {
    if (ST->requiresNopBeforeDeallocVGPRs()) {
      BuildMI(*MI->getParent(), MI, DebugLoc(), TII->get(RVGPU::S_NOP))
          .addImm(0);
    }
