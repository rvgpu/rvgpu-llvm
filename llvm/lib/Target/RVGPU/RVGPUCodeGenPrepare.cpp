...
//
/// \file
/// This pass does misc. RVGPU optimizations on IR before instruction
/// selection.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "rvgpu-codegenprepare"
...
static cl::opt<bool> WidenLoads(
  "rvgpu-codegenprepare-widen-constant-loads",
  cl::desc("Widen sub-dword constant address space loads in RVGPUCodeGenPrepare"),
  cl::ReallyHidden,
  cl::init(false));

static cl::opt<bool> Widen16BitOps(
  "rvgpu-codegenprepare-widen-16-bit-ops",
  cl::desc("Widen uniform 16-bit instructions to 32-bit in RVGPUCodeGenPrepare"),
  cl::ReallyHidden,
  cl::init(true));

static cl::opt<bool>
    BreakLargePHIs("rvgpu-codegenprepare-break-large-phis",
                   cl::desc("Break large PHI nodes for DAGISel"),
                   cl::ReallyHidden, cl::init(true));

static cl::opt<bool>
    ForceBreakLargePHIs("rvgpu-codegenprepare-force-break-large-phis",
...
static cl::opt<unsigned> BreakLargePHIsThreshold(
    "rvgpu-codegenprepare-break-large-phis-threshold",
...
static cl::opt<bool> UseMul24Intrin(
  "rvgpu-codegenprepare-mul24",
  cl::desc("Introduce mul24 intrinsics in RVGPUCodeGenPrepare"),
  cl::ReallyHidden,
  cl::init(true));

// Legalize 64-bit division by using the generic IR expansion.
static cl::opt<bool> ExpandDiv64InIR(
  "rvgpu-codegenprepare-expand-div64",
  cl::desc("Expand 64-bit division in RVGPUCodeGenPrepare"),
  cl::ReallyHidden,
  cl::init(false));
...
static cl::opt<bool> DisableIDivExpand(
  "rvgpu-codegenprepare-disable-idiv-expansion",
  cl::desc("Prevent expanding integer division in RVGPUCodeGenPrepare"),
  cl::ReallyHidden,
  cl::init(false));

// Disable processing of fdiv so we can better test the backend implementations.
static cl::opt<bool> DisableFDivExpand(
  "rvgpu-codegenprepare-disable-fdiv-expansion",
  cl::desc("Prevent expanding floating point division in RVGPUCodeGenPrepare"),
  cl::ReallyHidden,
  cl::init(false));

class RVGPUCodeGenPrepareImpl
    : public InstVisitor<RVGPUCodeGenPrepareImpl, bool> {
public:
  ...
  Function *getSqrtF32() const {
    ...
    SqrtF32 = Intrinsic::getDeclaration(Mod, Intrinsic::rvgpu_sqrt,
                                        {Type::getFloatTy(Ctx)});
    return SqrtF32;
  }
  ...
  /// Replace mul instructions with llvm.rvgpu.mul.u24 or llvm.rvgpu.mul.s24.
...
class RVGPUCodeGenPrepare : public FunctionPass {
private:
  RVGPUCodeGenPrepareImpl Impl;

public:
  static char ID;
  RVGPUCodeGenPrepare() : FunctionPass(ID) {
    initializeRVGPUCodeGenPreparePass(*PassRegistry::getPassRegistry());
  }
  ...
  StringRef getPassName() const override { return "RVGPU IR optimizations"; }
};

} // end anonymous namespace

bool RVGPUCodeGenPrepareImpl::run(Function &F) {
...
unsigned RVGPUCodeGenPrepareImpl::getBaseElementBitWidth(const Type *T) const {
...
Type *RVGPUCodeGenPrepareImpl::getI32Ty(IRBuilder<> &B, const Type *T) const {
  assert(needsPromotionToI32(T) && "T does not need promotion to i32");
...
bool RVGPUCodeGenPrepareImpl::isSigned(const BinaryOperator &I) const {
  return I.getOpcode() == Instruction::AShr ||
      I.getOpcode() == Instruction::SDiv || I.getOpcode() == Instruction::SRem;
}

bool RVGPUCodeGenPrepareImpl::isSigned(const SelectInst &I) const {
  return isa<ICmpInst>(I.getOperand(0)) ?
      cast<ICmpInst>(I.getOperand(0))->isSigned() : false;
}

bool RVGPUCodeGenPrepareImpl::needsPromotionToI32(const Type *T) const {
  if (!Widen16BitOps)
    return false;
...
bool RVGPUCodeGenPrepareImpl::isLegalFloatingTy(const Type *Ty) const {
...
bool RVGPUCodeGenPrepareImpl::canWidenScalarExtLoad(LoadInst &I) const {
  Type *Ty = I.getType();
...
bool RVGPUCodeGenPrepareImpl::promoteUniformOpToI32(BinaryOperator &I) const {
...
bool RVGPUCodeGenPrepareImpl::promoteUniformOpToI32(ICmpInst &I) const {
...
bool RVGPUCodeGenPrepareImpl::promoteUniformOpToI32(SelectInst &I) const {
  assert(needsPromotionToI32(I.getType()) &&
         "I does not need promotion to i32");
...
bool RVGPUCodeGenPrepareImpl::promoteUniformBitreverseToI32(
...
unsigned RVGPUCodeGenPrepareImpl::numBitsUnsigned(Value *Op) const {
  return computeKnownBits(Op, *DL, 0, AC).countMaxActiveBits();
}

unsigned RVGPUCodeGenPrepareImpl::numBitsSigned(Value *Op) const {
...
bool RVGPUCodeGenPrepareImpl::replaceMulWithMul24(BinaryOperator &I) const {
  ...
  for (int I = 0, E = LHSVals.size(); I != E; ++I) {
    ...
    Intrinsic::ID ID =
        IsSigned ? Intrinsic::rvgpu_mul_i24 : Intrinsic::rvgpu_mul_u24;
...
bool RVGPUCodeGenPrepareImpl::foldBinOpIntoSelect(BinaryOperator &BO) const {
...
RVGPUCodeGenPrepareImpl::getFrexpResults(IRBuilder<> &Builder,
                                          Value *Src) const {
  Type *Ty = Src->getType();
  ...
  Value *FrexpExp =
      ST->hasFractBug()
          ? Builder.CreateIntrinsic(Intrinsic::rvgpu_frexp_exp,
...
Value *RVGPUCodeGenPrepareImpl::emitRcpIEEE1ULP(IRBuilder<> &Builder,
                                                 Value *Src,
                                                 bool IsNegative) const {
  ...
  Value *Rcp = Builder.CreateUnaryIntrinsic(Intrinsic::rvgpu_rcp, FrexpMant);
  return Builder.CreateCall(getLdexpF32(), {Rcp, ScaleFactor});
}

/// Emit a 2ulp expansion for fdiv by using frexp for input scaling.
Value *RVGPUCodeGenPrepareImpl::emitFrexpDiv(IRBuilder<> &Builder, Value *LHS,
                                              Value *RHS,
                                              FastMathFlags FMF) const {
  ...
  Value *Rcp =
      Builder.CreateUnaryIntrinsic(Intrinsic::rvgpu_rcp, FrexpMantRHS);
...
Value *RVGPUCodeGenPrepareImpl::emitSqrtIEEE2ULP(IRBuilder<> &Builder,
                                                  Value *Src,
                                                  FastMathFlags FMF) const {
  Type *Ty = Src->getType();
...
static Value *emitRsqIEEE1ULP(IRBuilder<> &Builder, Value *Src,
  ...
  Value *Rsq = Builder.CreateUnaryIntrinsic(Intrinsic::rvgpu_rsq, ScaledInput);
...
bool RVGPUCodeGenPrepareImpl::canOptimizeWithRsq(const FPMathOperator *SqrtOp,
...
Value *RVGPUCodeGenPrepareImpl::optimizeWithRsq(
  ...
  if (CLHS->isExactlyValue(1.0) || (IsNegative = CLHS->isExactlyValue(-1.0))) {
    ...
    if ((DivFMF.approxFunc() && SqrtFMF.approxFunc()) || HasUnsafeFPMath ||
      ...
      Value *Result = Builder.CreateUnaryIntrinsic(Intrinsic::rvgpu_rsq, Den);
...
Value *
RVGPUCodeGenPrepareImpl::optimizeWithRcp(IRBuilder<> &Builder, Value *Num,
  ...
  if (const ConstantFP *CLHS = dyn_cast<ConstantFP>(Num)) {
    ...
    if (CLHS->isExactlyValue(1.0) ||
      ...
      if (HasFP32DenormalFlush || FMF.approxFunc()) {
        ...
        // 1.0 / x -> rcp(x)
        return Builder.CreateUnaryIntrinsic(Intrinsic::rvgpu_rcp, Src);
      }
  ...
  if (FMF.allowReciprocal()) {
    ...
    if (HasFP32DenormalFlush || FMF.approxFunc()) {
      Value *Recip = Builder.CreateUnaryIntrinsic(Intrinsic::rvgpu_rcp, Den);
...
Value *RVGPUCodeGenPrepareImpl::optimizeWithFDivFast(
  ...
  return Builder.CreateIntrinsic(Intrinsic::rvgpu_fdiv_fast, {}, {Num, Den});
}

Value *RVGPUCodeGenPrepareImpl::visitFDivElement(
...
bool RVGPUCodeGenPrepareImpl::visitFDiv(BinaryOperator &FDiv) {
...
int RVGPUCodeGenPrepareImpl::getDivNumBits(BinaryOperator &I, Value *Num,
...
Value *RVGPUCodeGenPrepareImpl::expandDivRem24(IRBuilder<> &Builder,
...
Value *RVGPUCodeGenPrepareImpl::expandDivRem24Impl(
  ...
  Function *RcpDecl = Intrinsic::getDeclaration(Mod, Intrinsic::rvgpu_rcp,
                                                Builder.getFloatTy());
  ...
  auto FMAD = !ST->hasMadMacF32Insts()
                  ? Intrinsic::fma
                  : (Intrinsic::ID)Intrinsic::rvgpu_fmad_ftz;
...
bool RVGPUCodeGenPrepareImpl::divHasSpecialOptimization(BinaryOperator &I,
                                                         Value *Num,
                                                         Value *Den) const {
...
Value *RVGPUCodeGenPrepareImpl::expandDivRem32(IRBuilder<> &Builder,
  ...
  Function *Rcp = Intrinsic::getDeclaration(Mod, Intrinsic::rvgpu_rcp, F32Ty);
...
Value *RVGPUCodeGenPrepareImpl::shrinkDivRem64(IRBuilder<> &Builder,
                                                BinaryOperator &I, Value *Num,
                                                Value *Den) const {
...
void RVGPUCodeGenPrepareImpl::expandDivRem64(BinaryOperator &I) const {
  Instruction::BinaryOps Opc = I.getOpcode();
...
bool RVGPUCodeGenPrepareImpl::visitBinaryOperator(BinaryOperator &I) {
...
bool RVGPUCodeGenPrepareImpl::visitLoadInst(LoadInst &I) {
  if (!WidenLoads)
    return false;

  if ((I.getPointerAddressSpace() == RVGPUAS::CONSTANT_ADDRESS ||
       I.getPointerAddressSpace() == RVGPUAS::CONSTANT_ADDRESS_32BIT) &&
...
bool RVGPUCodeGenPrepareImpl::visitICmpInst(ICmpInst &I) {
  bool Changed = false;
...
bool RVGPUCodeGenPrepareImpl::visitSelectInst(SelectInst &I) {
...
bool RVGPUCodeGenPrepareImpl::canBreakPHINode(const PHINode &I) {
...
bool RVGPUCodeGenPrepareImpl::visitPHINode(PHINode &I) {
...
bool RVGPUCodeGenPrepareImpl::visitIntrinsicInst(IntrinsicInst &I) {
...
bool RVGPUCodeGenPrepareImpl::visitBitreverseIntrinsicInst(IntrinsicInst &I) {
  bool Changed = false;
...
Value *RVGPUCodeGenPrepareImpl::matchFractPat(IntrinsicInst &I) {
  if (ST->hasFractBug())
    return nullptr;
...
Value *RVGPUCodeGenPrepareImpl::applyFractPat(IRBuilder<> &Builder,
                                               Value *FractArg) {
  ...
  for (unsigned I = 0, E = FractVals.size(); I != E; ++I) {
    ResultVals[I] =
        Builder.CreateIntrinsic(Intrinsic::rvgpu_fract, {Ty}, {FractVals[I]});
  }

  return insertValues(Builder, FractArg->getType(), ResultVals);
}

bool RVGPUCodeGenPrepareImpl::visitMinNum(IntrinsicInst &I) {
...
bool RVGPUCodeGenPrepareImpl::visitSqrt(IntrinsicInst &Sqrt) {
...
bool RVGPUCodeGenPrepare::doInitialization(Module &M) {
  Impl.Mod = &M;
...
bool RVGPUCodeGenPrepare::runOnFunction(Function &F) {
  if (skipFunction(F))
    return false;
  ...
  const RVGPUTargetMachine &TM = TPC->getTM<RVGPUTargetMachine>();
  Impl.TLInfo = &getAnalysis<TargetLibraryInfoWrapperPass>().getTLI(F);
  Impl.ST = &TM.getSubtarget<RVSubtarget>(F);
...
PreservedAnalyses RVGPUCodeGenPreparePass::run(Function &F,
...
INITIALIZE_PASS_BEGIN(RVGPUCodeGenPrepare, DEBUG_TYPE,
                      "RVGPU IR optimizations", false, false)
INITIALIZE_PASS_DEPENDENCY(AssumptionCacheTracker)
INITIALIZE_PASS_DEPENDENCY(TargetLibraryInfoWrapperPass)
INITIALIZE_PASS_DEPENDENCY(UniformityInfoWrapperPass)
INITIALIZE_PASS_END(RVGPUCodeGenPrepare, DEBUG_TYPE, "RVGPU IR optimizations",
                    false, false)

char RVGPUCodeGenPrepare::ID = 0;

FunctionPass *llvm::createRVGPUCodeGenPreparePass() {
  return new RVGPUCodeGenPrepare();
}
