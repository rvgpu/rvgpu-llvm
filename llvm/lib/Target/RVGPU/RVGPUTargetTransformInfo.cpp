//===- RVGPUTargetTransformInfo.cpp - RVGPU specific TTI pass -----------===//
//
...
static cl::opt<unsigned> UnrollThresholdPrivate(
  "rvgpu-unroll-threshold-private",
  cl::desc("Unroll threshold for RVGPU if private memory used in a loop"),
  cl::init(2700), cl::Hidden);

static cl::opt<unsigned> UnrollThresholdLocal(
  "rvgpu-unroll-threshold-local",
  cl::desc("Unroll threshold for RVGPU if local memory used in a loop"),
  cl::init(1000), cl::Hidden);

static cl::opt<unsigned> UnrollThresholdIf(
  "rvgpu-unroll-threshold-if",
  cl::desc("Unroll threshold increment for RVGPU for each if statement inside loop"),
  cl::init(200), cl::Hidden);

static cl::opt<bool> UnrollRuntimeLocal(
  "rvgpu-unroll-runtime-local",
  cl::desc("Allow runtime unroll for RVGPU if local memory used in a loop"),
  cl::init(true), cl::Hidden);

static cl::opt<unsigned> UnrollMaxBlockToAnalyze(
    "rvgpu-unroll-max-block-to-analyze",
    cl::desc("Inner loop block size threshold to analyze in unroll for RVGPU"),
    cl::init(32), cl::Hidden);

static cl::opt<unsigned> ArgAllocaCost("rvgpu-inline-arg-alloca-cost",
...
void RVGPUTTIImpl::getUnrollingPreferences(Loop *L, ScalarEvolution &SE,
  ...
  UP.Threshold =
      F.getFnAttributeAsParsedInteger("rvgpu-unroll-threshold", 300);
  ...
  // If this loop has the rvgpu.loop.unroll.threshold metadata we will use the
  // provided threshold value as the default for Threshold
  if (MDNode *LoopUnrollThreshold =
          findOptionMDForLoop(L, "rvgpu.loop.unroll.threshold")) {
    ...
    for (const Instruction &I : *BB) {
      ...
      if (AS == RVGPUAS::PRIVATE_ADDRESS) {
        ...
        if (AllocaSize > MaxAlloca)
          continue;
      } else if (AS == RVGPUAS::LOCAL_ADDRESS ||
                 AS == RVGPUAS::REGION_ADDRESS) {
        LocalGEPsSeen++;
...
int64_t RVGPUTTIImpl::getMaxMemIntrinsicInlineSizeThreshold() const {
  return 1024;
}
...
const FeatureBitset RVTTIImpl::InlineFeatureIgnoreList = {
    ...
    RVGPU::FeaturePromoteAlloca, RVGPU::FeatureUnalignedScratchAccess,
    RVGPU::FeatureUnalignedAccessMode,

    RVGPU::FeatureAutoWaitcntBeforeBarrier,

    // Property of the kernel/environment which can't actually differ.
    RVGPU::FeatureSGPRInitBug, RVGPU::FeatureXNACK,
    RVGPU::FeatureTrapHandler,
...
unsigned RVTTIImpl::getLoadStoreVecRegBitWidth(unsigned AddrSpace) const {
  if (AddrSpace == RVGPUAS::GLOBAL_ADDRESS ||
      ...
      AddrSpace == RVGPUAS::BUFFER_FAT_POINTER ||
      AddrSpace == RVGPUAS::BUFFER_RESOURCE ||
      AddrSpace == RVGPUAS::BUFFER_STRIDED_POINTER) {
    return 512;
  }
...
Type *RVTTIImpl::getMemcpyLoopLoweringType(
  ...
  if (SrcAddrSpace == RVGPUAS::LOCAL_ADDRESS ||
      SrcAddrSpace == RVGPUAS::REGION_ADDRESS ||
      DestAddrSpace == RVGPUAS::LOCAL_ADDRESS ||
      DestAddrSpace == RVGPUAS::REGION_ADDRESS) {
...
bool RVTTIImpl::isSourceOfDivergence(const Value *V) const {
  ...
  if (const LoadInst *Load = dyn_cast<LoadInst>(V))
    return Load->getPointerAddressSpace() == RVGPUAS::PRIVATE_ADDRESS ||
           Load->getPointerAddressSpace() == RVGPUAS::FLAT_ADDRESS;
  ...
  if (const IntrinsicInst *Intrinsic = dyn_cast<IntrinsicInst>(V)) {
    ...
    return RVGPU::isIntrinsicSourceOfDivergence(Intrinsic->getIntrinsicID());
  }
...
bool RVTTIImpl::isAlwaysUniform(const Value *V) const {
  ...
  uint64_t C;
  if (match(V, m_LShr(m_Intrinsic<Intrinsic::rvgpu_workitem_id_x>(),
                      m_ConstantInt(C))) ||
      match(V, m_AShr(m_Intrinsic<Intrinsic::rvgpu_workitem_id_x>(),
                      m_ConstantInt(C)))) {
  ...
  Value *Mask;
  if (match(V, m_c_And(m_Intrinsic<Intrinsic::rvgpu_workitem_id_x>(),
                       m_Value(Mask)))) {
    const Function *F = cast<Instruction>(V)->getFunction();
...
Value *RVTTIImpl::rewriteIntrinsicWithAddressSpace(IntrinsicInst *II,
  ...
  case Intrinsic::rvgpu_is_shared:
  case Intrinsic::rvgpu_is_private: {
    unsigned TrueAS = IntrID == Intrinsic::rvgpu_is_shared ?
  ...
  case Intrinsic::rvgpu_flat_atomic_fmin_num: {
    ...
    if (!RVGPU::isExtendedGlobalAddrSpace(NewAS))
      return nullptr;
...
static unsigned adjustInliningThresholdUsingCallee(const CallBase *CB,
  ...
  for (const Use &A : CB->args()) {
    ...
    for (auto ArgVT : ValueVTs) {
      ...
      if (RVGPU::isArgPassedInSGPR(CB, CB->getArgOperandNo(&A)))
        SGPRsInUse += CCRegNum;
      else
        VGPRsInUse += CCRegNum;
    }
...
unsigned RVTTIImpl::getCallerAllocaCost(const CallBase *CB,
  ...
  // inliner-cost in here. The vector-bonus is 0 on RVGPU.
