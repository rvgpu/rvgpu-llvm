...
static unsigned macToMad(unsigned Opc) {
  ...
  case RVGPU::V_MAC_F32_e64:
    return RVGPU::V_MAD_F32_e64;
  case RVGPU::V_MAC_F16_e64:
    return RVGPU::V_MAD_F16_e64;
  case RVGPU::V_FMAC_F32_e64:
    return RVGPU::V_FMA_F32_e64;
  case RVGPU::V_FMAC_F16_e64:
    return RVGPU::V_FMA_F16_gfx9_e64;
  case RVGPU::V_FMAC_F16_t16_e64:
    return RVGPU::V_FMA_F16_gfx9_e64;
  case RVGPU::V_FMAC_LEGACY_F32_e64:
    return RVGPU::V_FMA_LEGACY_F32_e64;
  case RVGPU::V_FMAC_F64_e64:
    return RVGPU::V_FMA_F64_e64;
  }
  return RVGPU::INSTRUCTION_LIST_END;
}
...
bool SIFoldOperands::frameIndexMayFold(const MachineInstr &UseMI, int OpNo,
  ...
  if (TII->isMUBUF(UseMI))
    return OpNo == RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::vaddr);
  if (!TII->isFLATScratch(UseMI))
    return false;

  int SIdx = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::saddr);
  if (OpNo == SIdx)
    return true;

  int VIdx = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::vaddr);
...
bool SIFoldOperands::canUseImmWithOpSel(FoldCandidate &Fold) const {
  ...
  if (!(TSFlags & SIInstrFlags::IsPacked) || (TSFlags & SIInstrFlags::IsMAI) ||
      ...
      !RVGPU::isFoldableLiteralV216(Fold.ImmToFold, ST->hasInv2PiInlineImm()))
    return false;
...
bool SIFoldOperands::tryFoldImmWithOpSel(FoldCandidate &Fold) const {
  ...
  // already set.
  int ModIdx = -1;
  if (OpNo == RVGPU::getNamedOperandIdx(Opcode, RVGPU::OpName::src0))
    ModIdx = RVGPU::OpName::src0_modifiers;
  else if (OpNo == RVGPU::getNamedOperandIdx(Opcode, RVGPU::OpName::src1))
    ModIdx = RVGPU::OpName::src1_modifiers;
  else if (OpNo == RVGPU::getNamedOperandIdx(Opcode, RVGPU::OpName::src2))
    ModIdx = RVGPU::OpName::src2_modifiers;
  assert(ModIdx != -1);
  ModIdx = RVGPU::getNamedOperandIdx(Opcode, ModIdx);
...
bool SIFoldOperands::updateOperand(FoldCandidate &Fold) const {
  ...
  if ((Fold.isImm() || Fold.isFI() || Fold.isGlobal()) && Fold.needsShrink()) {
    ...
    auto Liveness = MBB->computeRegisterLiveness(TRI, RVGPU::VCC, MI, 16);
    ...
    if (HaveNonDbgCarryUse) {
      BuildMI(*MBB, MI, MI->getDebugLoc(), TII->get(RVGPU::COPY),
              Dst1.getReg())
        .addReg(RVGPU::VCC, RegState::Kill);
    }
    ...
    for (unsigned I = MI->getNumOperands() - 1; I > 0; --I)
      MI->removeOperand(I);
    MI->setDesc(TII->get(RVGPU::IMPLICIT_DEF));
  ...
  if (Fold.isImm()) {
    if (Old.isTied()) {
      int NewMFMAOpc = RVGPU::getMFMAEarlyClobberOp(MI->getOpcode());
      if (NewMFMAOpc == -1)
        return false;
...
bool SIFoldOperands::tryAddToFoldList(SmallVectorImpl<FoldCandidate> &FoldList,
  ...
  auto tryToFoldAsFMAAKorMK = [&]() {
    ...
    const unsigned NewOpc = TryAK ? RVGPU::S_FMAAK_F32 : RVGPU::S_FMAMK_F32;
  ...
  if (!IsLegal) {
    ...
    if (NewOpc != RVGPU::INSTRUCTION_LIST_END) {
      // Check if changing this to a v_mad_{f16, f32} instruction will allow us
      // to fold the operand.
      MI->setDesc(TII->get(NewOpc));
      bool AddOpSel = !RVGPU::hasNamedOperand(Opc, RVGPU::OpName::op_sel) &&
                      RVGPU::hasNamedOperand(NewOpc, RVGPU::OpName::op_sel);
      if (AddOpSel)
    ...
    if (Opc == RVGPU::S_FMAC_F32 && OpNo == 3) {
    ...
    if (OpToFold->isImm()) {
      unsigned ImmOpc = 0;
      if (Opc == RVGPU::S_SETREG_B32)
        ImmOpc = RVGPU::S_SETREG_IMM32_B32;
      else if (Opc == RVGPU::S_SETREG_B32_mode)
        ImmOpc = RVGPU::S_SETREG_IMM32_B32_mode;
      if (ImmOpc) {
    ...
    if (!TII->isOperandLegal(*MI, CommuteOpNo, OpToFold)) {
      if ((Opc != RVGPU::V_ADD_CO_U32_e64 && Opc != RVGPU::V_SUB_CO_U32_e64 &&
           Opc != RVGPU::V_SUBREV_CO_U32_e64) || // FIXME
      ...
      Op32 = RVGPU::getVOPe32(MaybeCommutedOpc);
    }
  ...
  if ((Opc == RVGPU::S_FMAAK_F32 || Opc == RVGPU::S_FMAMK_F32) &&
      !OpToFold->isReg() && !TII->isInlineConstant(*OpToFold)) {
    unsigned ImmIdx = Opc == RVGPU::S_FMAAK_F32 ? 3 : 2;
...
bool SIFoldOperands::tryToFoldACImm(
  ...
  if (!RVGPU::isSISrcInlinableOperand(Desc, UseOpIdx))
    return false;
...
void SIFoldOperands::foldOperand(
  ...
  if (UseOp.isReg() && OpToFold.isReg() &&
      (UseOp.isImplicit() || UseOp.getSubReg() != RVGPU::NoSubRegister))
    return;
  ...
  if (frameIndexMayFold(*UseMI, UseOpIdx, OpToFold)) {
    ...
    if (TII->isMUBUF(*UseMI)) {
      if (TII->getNamedOperand(*UseMI, RVGPU::OpName::srsrc)->getReg() !=
      ...
      // wave.
      MachineOperand &SOff =
          *TII->getNamedOperand(*UseMI, RVGPU::OpName::soffset);
    ...
    const unsigned Opc = UseMI->getOpcode();
    if (TII->isFLATScratch(*UseMI) &&
        RVGPU::hasNamedOperand(Opc, RVGPU::OpName::vaddr) &&
        !RVGPU::hasNamedOperand(Opc, RVGPU::OpName::saddr)) {
      unsigned NewOpc = RVGPU::getFlatScratchInstSSfromSV(Opc);
  ...
  if (FoldingImmLike && UseMI->isCopy()) {
    ...
    if (!DestReg.isPhysical()) {
      if (DestRC == &RVGPU::AGPR_32RegClass &&
          TII->isInlineConstant(OpToFold, RVGPU::OPERAND_REG_INLINE_C_INT32)) {
        UseMI->setDesc(TII->get(RVGPU::V_ACCVGPR_WRITE_B32_e64));
    ...
    if (UseMI->isCopy() && OpToFold.isReg() &&
      ...
      if (Size > 4 && TRI->isAGPR(*MRI, UseMI->getOperand(0).getReg()) &&
          getRegSeqInit(Defs, UseReg, RVGPU::OPERAND_REG_INLINE_C_INT32)) {
        const DebugLoc &DL = UseMI->getDebugLoc();
        MachineBasicBlock &MBB = *UseMI->getParent();

        UseMI->setDesc(TII->get(RVGPU::REG_SEQUENCE));
        ...
        for (unsigned I = 0; I < Size / 4; ++I) {
          ...
          if (Def->isImm() &&
              TII->isInlineConstant(*Def, RVGPU::OPERAND_REG_INLINE_C_INT32)) {
            int64_t Imm = Def->getImm();

            auto Tmp = MRI->createVirtualRegister(&RVGPU::AGPR_32RegClass);
            BuildMI(MBB, UseMI, DL,
                    TII->get(RVGPU::V_ACCVGPR_WRITE_B32_e64), Tmp).addImm(Imm);
            B.addReg(Tmp);
            ...
            if (TRI->isSGPRReg(*MRI, Src.Reg)) {
              ...
              auto Tmp = MRI->createVirtualRegister(&RVGPU::AGPR_32RegClass);
              BuildMI(MBB, UseMI, DL, TII->get(RVGPU::COPY), Tmp).add(*Def);
              B.addReg(Tmp);
            }
          }
          ...
          if (CopyToVGPR.Reg) {
            ...
            if (VGPRCopies.count(CopyToVGPR)) {
              ...
              Vgpr = MRI->createVirtualRegister(&RVGPU::VGPR_32RegClass);
              BuildMI(MBB, UseMI, DL, TII->get(RVGPU::COPY), Vgpr).add(*Def);
              VGPRCopies[CopyToVGPR] = Vgpr;
            }
            auto Tmp = MRI->createVirtualRegister(&RVGPU::AGPR_32RegClass);
            BuildMI(MBB, UseMI, DL,
                    TII->get(RVGPU::V_ACCVGPR_WRITE_B32_e64), Tmp).addReg(Vgpr);
            B.addReg(Tmp);
          }
      ...
      if (TRI->isAGPR(*MRI, Reg0) && TRI->isVGPR(*MRI, Reg1))
        ...
        UseMI->setDesc(TII->get(RVGPU::V_ACCVGPR_READ_B32_e64));
      else if (ST->hasGFX90AInsts() && TRI->isAGPR(*MRI, Reg0) &&
               TRI->isAGPR(*MRI, Reg1))
        UseMI->setDesc(TII->get(RVGPU::V_ACCVGPR_MOV_B32));
      return;
    }

    unsigned UseOpc = UseMI->getOpcode();
    if (UseOpc == RVGPU::V_READFIRSTLANE_B32 ||
        (UseOpc == RVGPU::V_READLANE_B32 &&
         (int)UseOpIdx ==
         RVGPU::getNamedOperandIdx(UseOpc, RVGPU::OpName::src0))) {
      // %vgpr = V_MOV_B32 imm
      ...
      if (FoldingImmLike) {
        ...
        UseMI->setDesc(TII->get(RVGPU::S_MOV_B32));
      ...
      if (OpToFold.isReg() && TRI->isSGPRReg(*MRI, OpToFold.getReg())) {
        ...
        // =>
        // %sgpr1 = COPY %sgpr0
        UseMI->setDesc(TII->get(RVGPU::COPY));
  ...
  if (UseOp.getSubReg() && RVGPU::getRegBitWidth(*FoldRC) == 64) {
    Register UseReg = UseOp.getReg();
    const TargetRegisterClass *UseRC = MRI->getRegClass(UseReg);
    if (RVGPU::getRegBitWidth(*UseRC) != 64)
      return;

    APInt Imm(64, OpToFold.getImm());
    if (UseOp.getSubReg() == RVGPU::sub0) {
      Imm = Imm.getLoBits(32);
    } else {
      assert(UseOp.getSubReg() == RVGPU::sub1);
...
static bool evalBinaryInstruction(unsigned Opcode, int32_t &Result,
  ...
  switch (Opcode) {
  case RVGPU::V_AND_B32_e64:
  case RVGPU::V_AND_B32_e32:
  case RVGPU::S_AND_B32:
    Result = LHS & RHS;
    return true;
  case RVGPU::V_OR_B32_e64:
  case RVGPU::V_OR_B32_e32:
  case RVGPU::S_OR_B32:
    Result = LHS | RHS;
    return true;
  case RVGPU::V_XOR_B32_e64:
  case RVGPU::V_XOR_B32_e32:
  case RVGPU::S_XOR_B32:
    Result = LHS ^ RHS;
    return true;
  ...
  case RVGPU::S_ORN2_B32:
    Result = LHS | ~RHS;
    return true;
  case RVGPU::V_LSHL_B32_e64:
  case RVGPU::V_LSHL_B32_e32:
  ...
  case RVGPU::V_LSHLREV_B32_e32:
    Result = RHS << (LHS & 31);
    return true;
  case RVGPU::V_LSHR_B32_e64:
  case RVGPU::V_LSHR_B32_e32:
  ...
  case RVGPU::V_LSHRREV_B32_e32:
    Result = RHS >> (LHS & 31);
    return true;
  case RVGPU::V_ASHR_I32_e64:
  case RVGPU::V_ASHR_I32_e32:
...
static unsigned getMovOpc(bool IsScalar) {
  return IsScalar ? RVGPU::S_MOV_B32 : RVGPU::V_MOV_B32_e32;
}
...
SIFoldOperands::getImmOrMaterializedImm(MachineOperand &Op) const {
  ...
  if (!Op.isReg() || Op.getSubReg() != RVGPU::NoSubRegister ||
      !Op.getReg().isVirtual())
    return &Op;
...
bool SIFoldOperands::tryConstantFoldOp(MachineInstr *MI) const {
  ...
  if ((Opc == RVGPU::V_NOT_B32_e64 || Opc == RVGPU::V_NOT_B32_e32 ||
       Opc == RVGPU::S_NOT_B32) &&
      Src0->isImm()) {
    MI->getOperand(1).ChangeToImmediate(~Src0->getImm());
    mutateCopyOp(*MI, TII->get(getMovOpc(Opc == RVGPU::S_NOT_B32)));
    return true;
  }
  ...
  if (Opc == RVGPU::V_OR_B32_e64 ||
      Opc == RVGPU::V_OR_B32_e32 ||
      Opc == RVGPU::S_OR_B32) {
    if (Src1Val == 0) {
      // y = or x, 0 => y = copy x
      MI->removeOperand(Src1Idx);
      mutateCopyOp(*MI, TII->get(RVGPU::COPY));
    } else if (Src1Val == -1) {
      // y = or x, -1 => y = v_mov_b32 -1
      MI->removeOperand(Src1Idx);
      mutateCopyOp(*MI, TII->get(getMovOpc(Opc == RVGPU::S_OR_B32)));
    } else
      return false;
  ...
  if (Opc == RVGPU::V_AND_B32_e64 || Opc == RVGPU::V_AND_B32_e32 ||
      Opc == RVGPU::S_AND_B32) {
    if (Src1Val == 0) {
      // y = and x, 0 => y = v_mov_b32 0
      MI->removeOperand(Src0Idx);
      mutateCopyOp(*MI, TII->get(getMovOpc(Opc == RVGPU::S_AND_B32)));
    } else if (Src1Val == -1) {
      // y = and x, -1 => y = copy x
      MI->removeOperand(Src1Idx);
      mutateCopyOp(*MI, TII->get(RVGPU::COPY));
  ...
  if (Opc == RVGPU::V_XOR_B32_e64 || Opc == RVGPU::V_XOR_B32_e32 ||
      Opc == RVGPU::S_XOR_B32) {
    if (Src1Val == 0) {
      // y = xor x, 0 => y = copy x
      MI->removeOperand(Src1Idx);
...
bool SIFoldOperands::tryFoldCndMask(MachineInstr &MI) const {
  unsigned Opc = MI.getOpcode();
  if (Opc != RVGPU::V_CNDMASK_B32_e32 && Opc != RVGPU::V_CNDMASK_B32_e64 &&
      Opc != RVGPU::V_CNDMASK_B64_PSEUDO)
    return false;

  MachineOperand *Src0 = TII->getNamedOperand(MI, RVGPU::OpName::src0);
  ...
  int Src1ModIdx =
      RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::src1_modifiers);
  int Src0ModIdx =
      RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::src0_modifiers);
  ...
  auto &NewDesc =
      TII->get(Src0->isReg() ? (unsigned)RVGPU::COPY : getMovOpc(false));
  int Src2Idx = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::src2);
  if (Src2Idx != -1)
    MI.removeOperand(Src2Idx);
  MI.removeOperand(RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::src1));
  if (Src1ModIdx != -1)
...
bool SIFoldOperands::tryFoldZeroHighBits(MachineInstr &MI) const {
  if (MI.getOpcode() != RVGPU::V_AND_B32_e64 &&
      MI.getOpcode() != RVGPU::V_AND_B32_e32)
    return false;
...
bool SIFoldOperands::tryFoldFoldableCopy(
  ...
  if (MI.getOperand(0).getReg() == RVGPU::M0) {
...
const MachineOperand *SIFoldOperands::isClamp(const MachineInstr &MI) const {
  ...
  switch (Op) {
  case RVGPU::V_MAX_F32_e64:
  case RVGPU::V_MAX_F16_e64:
  case RVGPU::V_MAX_F16_t16_e64:
  case RVGPU::V_MAX_F16_fake16_e64:
  case RVGPU::V_MAX_F64_e64:
  case RVGPU::V_PK_MAX_F16: {
    if (!TII->getNamedOperand(MI, RVGPU::OpName::clamp)->getImm())
      return nullptr;

    // Make sure sources are identical.
    const MachineOperand *Src0 = TII->getNamedOperand(MI, RVGPU::OpName::src0);
    const MachineOperand *Src1 = TII->getNamedOperand(MI, RVGPU::OpName::src1);
    if (!Src0->isReg() || !Src1->isReg() ||
        Src0->getReg() != Src1->getReg() ||
        Src0->getSubReg() != Src1->getSubReg() ||
        Src0->getSubReg() != RVGPU::NoSubRegister)
      return nullptr;

    // Can't fold up if we have modifiers.
    if (TII->hasModifiersSet(MI, RVGPU::OpName::omod))
      return nullptr;

    unsigned Src0Mods
      = TII->getNamedOperand(MI, RVGPU::OpName::src0_modifiers)->getImm();
    unsigned Src1Mods
      = TII->getNamedOperand(MI, RVGPU::OpName::src1_modifiers)->getImm();

    // Having a 0 op_sel_hi would require swizzling the output in the source
    // instruction, which we can't do.
    unsigned UnsetMods = (Op == RVGPU::V_PK_MAX_F16) ? SISrcMods::OP_SEL_1
                                                      : 0u;
...
bool SIFoldOperands::tryFoldClamp(MachineInstr &MI) {
  ...
  MachineOperand *DefClamp = TII->getNamedOperand(*Def, RVGPU::OpName::clamp);
  if (!DefClamp)
    return false;
...
static int getOModValue(unsigned Opc, int64_t Val) {
  ...
  case RVGPU::V_MUL_F16_e64:
  case RVGPU::V_MUL_F16_t16_e64:
  case RVGPU::V_MUL_F16_fake16_e64: {
...
SIFoldOperands::isOMod(const MachineInstr &MI) const {
  ...
  case RVGPU::V_MUL_F16_e64: {
    ...
    if ((Op == RVGPU::V_MUL_F32_e64 &&
         MFI->getMode().FP32Denormals.Output != DenormalMode::PreserveSign) ||
        ((Op == RVGPU::V_MUL_F64_e64 || Op == RVGPU::V_MUL_F16_e64 ||
          Op == RVGPU::V_MUL_F16_t16_e64 ||
          Op == RVGPU::V_MUL_F16_fake16_e64) &&
    ...
    if (OMod == SIOutMods::NONE ||
        TII->hasModifiersSet(MI, RVGPU::OpName::src0_modifiers) ||
        TII->hasModifiersSet(MI, RVGPU::OpName::src1_modifiers) ||
        TII->hasModifiersSet(MI, RVGPU::OpName::omod) ||
        TII->hasModifiersSet(MI, RVGPU::OpName::clamp))
      return std::pair(nullptr, SIOutMods::NONE);

    return std::pair(RegOp, OMod);
  }
  case RVGPU::V_ADD_F64_e64:
  case RVGPU::V_ADD_F32_e64:
  case RVGPU::V_ADD_F16_e64:
  case RVGPU::V_ADD_F16_t16_e64:
  case RVGPU::V_ADD_F16_fake16_e64: {
    // If output denormals are enabled, omod is ignored.
    if ((Op == RVGPU::V_ADD_F32_e64 &&
         MFI->getMode().FP32Denormals.Output != DenormalMode::PreserveSign) ||
        ((Op == RVGPU::V_ADD_F64_e64 || Op == RVGPU::V_ADD_F16_e64 ||
          Op == RVGPU::V_ADD_F16_t16_e64 ||
          Op == RVGPU::V_ADD_F16_fake16_e64) &&
         MFI->getMode().FP64FP16Denormals.Output != DenormalMode::PreserveSign))
      return std::pair(nullptr, SIOutMods::NONE);
...
bool SIFoldOperands::tryFoldRegSequence(MachineInstr &MI) {
  ...
  auto RS = BuildMI(*MI.getParent(), MI, MI.getDebugLoc(),
                    TII->get(RVGPU::REG_SEQUENCE), Dst);
...
static bool isAGPRCopy(const RVRegisterInfo &TRI,
  ...
  if (!OtherCopySrcReg.isVirtual() ||
      CopySrcDef->getOperand(0).getSubReg() != RVGPU::NoSubRegister ||
      OtherCopySrc.getSubReg() != RVGPU::NoSubRegister ||
...
bool SIFoldOperands::tryFoldPhiAGPR(MachineInstr &PHI) {
  ...
  for (unsigned K = 1; K < PHI.getNumExplicitOperands(); K += 2) {
    ...
    Register AGPRSrc;
    unsigned AGPRRegMask = RVGPU::NoSubRegister;
  ...
  bool IsAGPR32 = (ARC == &RVGPU::AGPR_32RegClass);
  ...
  for (unsigned K = 1; K < PHI.getNumExplicitOperands(); K += 2) {
    ...
    if (MachineInstr *Def = MRI->getVRegDef(Reg)) {
      ...
      if (Def->isCopy()) {
        Register AGPRSrc;
        unsigned AGPRSubReg = RVGPU::NoSubRegister;
        ...
        if (IsAGPR32 && !ST->hasGFX90AInsts() && !MRI->hasOneNonDBGUse(Reg) &&
          ...
          CopyOpc = RVGPU::V_ACCVGPR_WRITE_B32_e64;
      }
  ...
  for (const auto &[Entry, MOs] : RegToMO) {
    ...
    MachineInstr *VGPRCopy =
        BuildMI(*DefMBB, ++Def->getIterator(), Def->getDebugLoc(),
                TII->get(RVGPU::V_ACCVGPR_READ_B32_e64), TempVGPR)
...
bool SIFoldOperands::runOnMachineFunction(MachineFunction &MF) {
  ...
  for (MachineBasicBlock *MBB : depth_first(&MF)) {
    ...
    for (auto &MI : make_early_inc_range(*MBB)) {
      ...
      if (CurrentKnownM0Val && MI.modifiesRegister(RVGPU::M0, TRI))
