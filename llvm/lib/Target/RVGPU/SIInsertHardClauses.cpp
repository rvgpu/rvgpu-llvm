...
public:
      ...
      if (ST->getGeneration() == RVGPUSubtarget::GFX10) {
        if (RVInstrInfo::isVMEM(MI) || RVInstrInfo::isSegmentSpecificFLAT(MI)) {
          if (ST->hasNSAClauseBug()) {
            ...
            if (Info && Info->MIMGEncoding == RVGPU::MIMGEncGfx10NSA)
  ...
  bool emitClause(const ClauseInfo &CI, const RVInstrInfo *SII) {
    if (CI.First == CI.Last)
      return false;
    ...
    auto ClauseMI =
        BuildMI(MBB, *CI.First, DebugLoc(), SII->get(RVGPU::S_CLAUSE))
            .addImm(CI.Length - 1);
        ...
        if (CI.Length == MaxInstructionsInClause ||
            (CI.Length && Type != HARDCLAUSE_INTERNAL &&
             ...
             (Type != CI.Type ||
              ...
              // as they aren't used in the RVInstrInfo implementation.
