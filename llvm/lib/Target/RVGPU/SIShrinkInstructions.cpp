...
bool SIShrinkInstructions::foldImmediates(MachineInstr &MI,
  ...
  int Src0Idx = RVGPU::getNamedOperandIdx(MI.getOpcode(), RVGPU::OpName::src0);
  ...
  for (unsigned I = 0, E = MI.getNumExplicitOperands(); I != E; ++I) {
    ...
    if (MO.isReg()) {
      ...
      if (RVGPU::VGPR_32RegClass.contains(Reg) &&
          !RVGPU::VGPR_32_Lo128RegClass.contains(Reg))
        return false;
    }
  }
  return true;
...
void SIShrinkInstructions::shrinkScalarCompare(MachineInstr &MI) const {
  ...
  int SOPKOpc = RVGPU::getSOPKOp(MI.getOpcode());
  if (SOPKOpc == -1)
    return;

  // eq/ne is special because the imm16 can be treated as signed or unsigned,
  // and initially selected to the unsigned versions.
  if (SOPKOpc == RVGPU::S_CMPK_EQ_U32 || SOPKOpc == RVGPU::S_CMPK_LG_U32) {
    bool HasUImm;
    if (isKImmOrKUImmOperand(Src1, HasUImm)) {
      if (!HasUImm) {
        SOPKOpc = (SOPKOpc == RVGPU::S_CMPK_EQ_U32) ?
          RVGPU::S_CMPK_EQ_I32 : RVGPU::S_CMPK_LG_I32;
...
void SIShrinkInstructions::shrinkMIMG(MachineInstr &MI) const {
  ...
  if (Info->VAddrDwords == 2) {
    RC = &RVGPU::VReg_64RegClass;
  } else if (Info->VAddrDwords == 3) {
    RC = &RVGPU::VReg_96RegClass;
  } else if (Info->VAddrDwords == 4) {
    RC = &RVGPU::VReg_128RegClass;
  } else if (Info->VAddrDwords == 5) {
    RC = &RVGPU::VReg_160RegClass;
  } else if (Info->VAddrDwords == 6) {
    RC = &RVGPU::VReg_192RegClass;
  } else if (Info->VAddrDwords == 7) {
    RC = &RVGPU::VReg_224RegClass;
  } else if (Info->VAddrDwords == 8) {
    RC = &RVGPU::VReg_256RegClass;
  } else if (Info->VAddrDwords == 9) {
    RC = &RVGPU::VReg_288RegClass;
  } else if (Info->VAddrDwords == 10) {
    RC = &RVGPU::VReg_320RegClass;
  } else if (Info->VAddrDwords == 11) {
    RC = &RVGPU::VReg_352RegClass;
  } else if (Info->VAddrDwords == 12) {
    RC = &RVGPU::VReg_384RegClass;
  } else {
    RC = &RVGPU::VReg_512RegClass;
    NewAddrDwords = 16;
  }
  ...
  // enabled
  int TFEIdx = RVGPU::getNamedOperandIdx(MI.getOpcode(), RVGPU::OpName::tfe);
  int LWEIdx = RVGPU::getNamedOperandIdx(MI.getOpcode(), RVGPU::OpName::lwe);
  ...
  unsigned NewOpcode = RVGPU::getMIMGOpcode(Info->BaseOpcode, NewEncoding,
...
void SIShrinkInstructions::shrinkMadFma(MachineInstr &MI) const {
  ...
  unsigned NewOpcode = RVGPU::INSTRUCTION_LIST_END;
  ...
  if (Src2.isImm() && !TII->isInlineConstant(Src2)) {
    ...
    case RVGPU::V_MAD_F32_e64:
      NewOpcode = RVGPU::V_MADAK_F32;
      break;
    case RVGPU::V_FMA_F32_e64:
      NewOpcode = RVGPU::V_FMAAK_F32;
      break;
    case RVGPU::V_MAD_F16_e64:
      NewOpcode = RVGPU::V_MADAK_F16;
      break;
    case RVGPU::V_FMA_F16_e64:
    case RVGPU::V_FMA_F16_gfx9_e64:
      NewOpcode = ST->hasTrue16BitInsts() ? RVGPU::V_FMAAK_F16_t16
                                          : RVGPU::V_FMAAK_F16;
      break;
    }
  }
  ...
  if (Src2.isReg() && TRI->isVGPR(*MRI, Src2.getReg())) {
    ...
    default:
      llvm_unreachable("Unexpected mad/fma opcode!");
    case RVGPU::V_MAD_F32_e64:
      NewOpcode = RVGPU::V_MADMK_F32;
      break;
    case RVGPU::V_FMA_F32_e64:
      NewOpcode = RVGPU::V_FMAMK_F32;
      break;
    case RVGPU::V_MAD_F16_e64:
      NewOpcode = RVGPU::V_MADMK_F16;
      break;
    case RVGPU::V_FMA_F16_e64:
    case RVGPU::V_FMA_F16_gfx9_e64:
      NewOpcode = ST->hasTrue16BitInsts() ? RVGPU::V_FMAMK_F16_t16
                                          : RVGPU::V_FMAMK_F16;
      break;
    }
  }
  ...
  if (RVGPU::isTrue16Inst(NewOpcode) && !shouldShrinkTrue16(MI))
    return;
...
bool SIShrinkInstructions::shrinkScalarLogicOp(MachineInstr &MI) const {
  ...
  if (!SrcImm->isImm() ||
      RVGPU::isInlinableLiteral32(SrcImm->getImm(), ST->hasInv2PiInlineImm()))
    return false;

  uint32_t Imm = static_cast<uint32_t>(SrcImm->getImm());
  uint32_t NewImm = 0;

  if (Opc == RVGPU::S_AND_B32) {
    if (isPowerOf2_32(~Imm)) {
      NewImm = llvm::countr_one(Imm);
      Opc = RVGPU::S_BITSET0_B32;
    } else if (RVGPU::isInlinableLiteral32(~Imm, ST->hasInv2PiInlineImm())) {
      NewImm = ~Imm;
      Opc = RVGPU::S_ANDN2_B32;
    }
  } else if (Opc == RVGPU::S_OR_B32) {
    if (isPowerOf2_32(Imm)) {
      NewImm = llvm::countr_zero(Imm);
      Opc = RVGPU::S_BITSET1_B32;
    } else if (RVGPU::isInlinableLiteral32(~Imm, ST->hasInv2PiInlineImm())) {
      NewImm = ~Imm;
      Opc = RVGPU::S_ORN2_B32;
    }
  } else if (Opc == RVGPU::S_XOR_B32) {
    if (RVGPU::isInlinableLiteral32(~Imm, ST->hasInv2PiInlineImm())) {
      NewImm = ~Imm;
      Opc = RVGPU::S_XNOR_B32;
    }
  } else {
...
void SIShrinkInstructions::dropInstructionKeepingImpDefs(
  ...
  for (unsigned i = MI.getDesc().getNumOperands() +
    ...
    BuildMI(*MI.getParent(), MI.getIterator(), MI.getDebugLoc(),
            TII->get(RVGPU::IMPLICIT_DEF), Op.getReg());
  }
...
MachineInstr *SIShrinkInstructions::matchSwap(MachineInstr &MovT) const {
  assert(MovT.getOpcode() == RVGPU::V_MOV_B32_e32 ||
         MovT.getOpcode() == RVGPU::COPY);
  ...
  for (auto Iter = std::next(MovT.getIterator()),
    ...
    if ((MovY->getOpcode() != RVGPU::V_MOV_B32_e32 &&
         MovY->getOpcode() != RVGPU::COPY) ||
    ...
    for (auto IY = MovY->getIterator(), I = std::next(MovT.getIterator());
      ...
      if (MovX ||
          (I->getOpcode() != RVGPU::V_MOV_B32_e32 &&
           I->getOpcode() != RVGPU::COPY) ||
    ...
    for (unsigned I = 0; I < Size; ++I) {
      ...
      auto MIB = BuildMI(MBB, MovX->getIterator(), MovT.getDebugLoc(),
                         TII->get(RVGPU::V_SWAP_B32))
        .addDef(X1.Reg, 0, X1.SubReg)
        .addDef(Y1.Reg, 0, Y1.SubReg)
        .addReg(Y1.Reg, 0, Y1.SubReg)
        .addReg(X1.Reg, 0, X1.SubReg).getInstr();
      if (MovX->hasRegisterImplicitUseOperand(RVGPU::EXEC)) {
        // Drop implicit EXEC.
...
bool SIShrinkInstructions::tryReplaceDeadSDST(MachineInstr &MI) const {
  ...
  MachineOperand *Op = TII->getNamedOperand(MI, RVGPU::OpName::sdst);
  if (!Op)
    return false;
  ...
  Op->setReg(ST->isWave32() ? RVGPU::SGPR_NULL : RVGPU::SGPR_NULL64);
  return true;
}
...
bool SIShrinkInstructions::runOnMachineFunction(MachineFunction &MF) {
  ...
  unsigned VCCReg = ST->isWave32() ? RVGPU::VCC_LO : RVGPU::VCC;
  ...
  for (MachineFunction::iterator BI = MF.begin(), BE = MF.end();
    ...
    for (I = MBB.begin(); I != MBB.end(); I = Next) {
      ...
      if (MI.getOpcode() == RVGPU::V_MOV_B32_e32) {
        ...
        if (Src.isImm() && MI.getOperand(0).getReg().isPhysical()) {
          ...
          if (isReverseInlineImm(Src, ReverseImm)) {
            MI.setDesc(TII->get(RVGPU::V_BFREV_B32_e32));
      ...
      if (ST->hasSwap() && (MI.getOpcode() == RVGPU::V_MOV_B32_e32 ||
                            MI.getOpcode() == RVGPU::COPY)) {
      ...
      if (MI.getOpcode() == RVGPU::S_ADD_I32 ||
          MI.getOpcode() == RVGPU::S_MUL_I32) {
        const MachineOperand *Dest = &MI.getOperand(0);
        MachineOperand *Src0 = &MI.getOperand(1);
        MachineOperand *Src1 = &MI.getOperand(2);
        ...
        if (Src0->isReg() && Src0->getReg() == Dest->getReg()) {
          if (Src1->isImm() && isKImmOperand(*Src1)) {
            unsigned Opc = (MI.getOpcode() == RVGPU::S_ADD_I32) ?
              RVGPU::S_ADDK_I32 : RVGPU::S_MULK_I32;
      ...
      if (MI.getOpcode() == RVGPU::S_MOV_B32) {
      ...
      if (MI.getOpcode() == RVGPU::S_AND_B32 ||
          MI.getOpcode() == RVGPU::S_OR_B32 ||
          MI.getOpcode() == RVGPU::S_XOR_B32) {
      ...
      if (MI.getOpcode() == RVGPU::V_MAD_F32_e64 ||
          MI.getOpcode() == RVGPU::V_FMA_F32_e64 ||
          MI.getOpcode() == RVGPU::V_MAD_F16_e64 ||
          MI.getOpcode() == RVGPU::V_FMA_F16_e64 ||
          MI.getOpcode() == RVGPU::V_FMA_F16_gfx9_e64) {
        shrinkMadFma(MI);
        continue;
      }
      ...
      int Op32 = RVGPU::getVOPe32(MI.getOpcode());
      ...
      if (Op32 == RVGPU::V_CNDMASK_B32_e32) {
      ...
      if (ST->hasTrue16BitInsts() && RVGPU::isTrue16Inst(MI.getOpcode()) &&
