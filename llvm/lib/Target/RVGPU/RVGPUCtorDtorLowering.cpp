...
static Function *createInitOrFiniKernelFunction(Module &M, bool IsCtor) {
  StringRef InitOrFiniKernelName = "rvgpu.device.init";
  if (!IsCtor)
    InitOrFiniKernelName = "rvgpu.device.fini";
  ...
  InitOrFiniKernel->setCallingConv(CallingConv::RVGPU_KERNEL);
  InitOrFiniKernel->addFnAttr("rvgpu-flat-work-group-size", "1,1");
  if (IsCtor)
...
static void createInitOrFiniCalls(Function &F, bool IsCtor) {
  ...
  Type *PtrTy = IRB.getPtrTy(RVGPUAS::GLOBAL_ADDRESS);
...
char &llvm::RVGPUCtorDtorLoweringLegacyPassID =
    RVGPUCtorDtorLoweringLegacy::ID;
