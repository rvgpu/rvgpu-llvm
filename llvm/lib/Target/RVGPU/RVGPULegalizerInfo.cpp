...
#define DEBUG_TYPE "rvgpu-legalinfo"
...
static cl::opt<bool> EnableNewLegality(
  "rvgpu-global-isel-new-legality",
...
static LegalizeMutation moreElementsToNextExistingRegClass(unsigned TypeIdx) {
  return [=](const LegalityQuery &Query) {
    ...
    for (NewNumElts = NumElts; NewNumElts < MaxNumElts; ++NewNumElts) {
      if (RVRegisterInfo::getSGPRClassForBitWidth(NewNumElts * EltSize))
        break;
    }
...
static LegalityPredicate isIllegalRegisterType(unsigned TypeIdx) {
  return [=](const LegalityQuery &Query) {
    ...
    return isRegisterType(Ty) &&
           !RVRegisterInfo::getSGPRClassForBitWidth(Ty.getSizeInBits());
  };
}
...
static unsigned maxSizeForAddrSpace(const RVSubtarget &ST, unsigned AS,
...
static bool isLoadStoreSizeLegal(const RVSubtarget &ST,
                                 const LegalityQuery &Query) {
  const LLT Ty = Query.Types[0];

  // Handle G_LOAD, G_ZEXTLOAD, G_SEXTLOAD
  const bool IsLoad = Query.Opcode != RVGPU::G_STORE;
  ...
  if (AS == RVGPUAS::CONSTANT_ADDRESS_32BIT)
    return false;
  ...
  if (AlignBits < MemSize) {
    const RVTargetLowering *TLI = ST.getTargetLowering();
...
static bool hasBufferRsrcWorkaround(const LLT Ty) {
  if (Ty.isPointer() && Ty.getAddressSpace() == RVGPUAS::BUFFER_RESOURCE)
    return true;
  if (Ty.isVector()) {
...
static bool isLoadStoreLegal(const RVSubtarget &ST, const LegalityQuery &Query) {
  const LLT Ty = Query.Types[0];
...
/// to a different type.
static bool shouldBitcastLoadStoreType(const RVSubtarget &ST, const LLT Ty,
                                       const LLT MemTy) {
...
static bool shouldWidenLoad(const RVSubtarget &ST, LLT MemoryTy,
...
static bool shouldWidenLoad(const RVSubtarget &ST, const LegalityQuery &Query,
                            unsigned Opcode) {
...
RVGPULegalizerInfo::RVGPULegalizerInfo(const RVSubtarget &ST_,
  ...
  const LLT GlobalPtr = GetAddrSpacePtr(RVGPUAS::GLOBAL_ADDRESS);
  const LLT ConstantPtr = GetAddrSpacePtr(RVGPUAS::CONSTANT_ADDRESS);
  const LLT Constant32Ptr = GetAddrSpacePtr(RVGPUAS::CONSTANT_ADDRESS_32BIT);
  const LLT LocalPtr = GetAddrSpacePtr(RVGPUAS::LOCAL_ADDRESS);
  const LLT RegionPtr = GetAddrSpacePtr(RVGPUAS::REGION_ADDRESS);
  const LLT FlatPtr = GetAddrSpacePtr(RVGPUAS::FLAT_ADDRESS);
  const LLT PrivatePtr = GetAddrSpacePtr(RVGPUAS::PRIVATE_ADDRESS);
  const LLT BufferFatPtr = GetAddrSpacePtr(RVGPUAS::BUFFER_FAT_POINTER);
  const LLT RsrcPtr = GetAddrSpacePtr(RVGPUAS::BUFFER_RESOURCE);
  const LLT BufferStridedPtr =
      GetAddrSpacePtr(RVGPUAS::BUFFER_STRIDED_POINTER);
  ...
  if (ST.has16BitInsts()) {
    getActionDefinitionsBuilder(
        ...
        .clampScalar(0, S16, S64)
        .scalarize(0);
  } else if (ST.getGeneration() >= RVGPUSubtarget::SEA_ISLANDS) {
    getActionDefinitionsBuilder(
        {G_INTRINSIC_TRUNC, G_FCEIL, G_INTRINSIC_ROUNDEVEN})
        .legalFor({S32, S64})
        .clampScalar(0, S32, S64)
    ...
    getActionDefinitionsBuilder(Op)
      .customIf([=](const LegalityQuery &Query) {
          ...
          const bool isLegalVecType =
              !!RVRegisterInfo::getSGPRClassForBitWidth(VecTy.getSizeInBits());
...
bool RVGPULegalizerInfo::legalizeCustom(LegalizerHelper &Helper,
...
Register RVGPULegalizerInfo::getSegmentAperture(
  unsigned AS,
  ...
  assert(AS == RVGPUAS::LOCAL_ADDRESS || AS == RVGPUAS::PRIVATE_ADDRESS);
  ...
  if (ST.hasApertureRegs()) {
    ...
    MRI.setRegClass(Dst, &RVGPU::SReg_64RegClass);
    B.buildInstr(RVGPU::S_MOV_B64, {Dst}, {Register(ApertureRegNo)});
  ...
  if (!loadInputValue(QueuePtr, B, RVGPUFunctionArgInfo::QUEUE_PTR))
    return Register();
...
bool RVGPULegalizerInfo::legalizeAddrSpaceCast(
  ...
  if (DestAS == RVGPUAS::FLAT_ADDRESS &&
      (SrcAS == RVGPUAS::LOCAL_ADDRESS ||
  ...
  if (DestAS == RVGPUAS::CONSTANT_ADDRESS_32BIT &&
...
bool RVGPULegalizerInfo::legalizeFroundeven(MachineInstr &MI,
                                             MachineRegisterInfo &MRI,
                                             MachineIRBuilder &B) const {
  Register Src = MI.getOperand(1).getReg();
...
bool RVGPULegalizerInfo::legalizeFceil(
  MachineInstr &MI, MachineRegisterInfo &MRI,
  MachineIRBuilder &B) const {
...
bool RVGPULegalizerInfo::legalizeFrem(
  MachineInstr &MI, MachineRegisterInfo &MRI,
  MachineIRBuilder &B) const {
...
static MachineInstrBuilder extractF64Exponent(Register Hi,
  ...
  auto ExpPart = B.buildIntrinsic(Intrinsic::rvgpu_ubfe, {S32})
                     .addUse(Hi)
...
bool RVGPULegalizerInfo::legalizeIntrinsicTrunc(
  MachineInstr &MI, MachineRegisterInfo &MRI,
  MachineIRBuilder &B) const {
  const LLT S1 = LLT::scalar(1);
  const LLT S32 = LLT::scalar(32);
...
bool RVGPULegalizerInfo::legalizeITOFP(
  MachineInstr &MI, MachineRegisterInfo &MRI,
  ...
  if (Signed) {
    ...
    auto LS = B.buildIntrinsic(Intrinsic::rvgpu_sffbh, {S32})
...
// actually works.
bool RVGPULegalizerInfo::legalizeFPTOI(MachineInstr &MI,
                                        MachineRegisterInfo &MRI,
                                        MachineIRBuilder &B,
                                        bool Signed) const {
...
bool RVGPULegalizerInfo::legalizeMinNumMaxNum(LegalizerHelper &Helper,
                                               MachineInstr &MI) const {
  MachineFunction &MF = Helper.MIRBuilder.getMF();
  const RVMachineFunctionInfo *MFI = MF.getInfo<RVMachineFunctionInfo>();

  const bool IsIEEEOp = MI.getOpcode() == RVGPU::G_FMINNUM_IEEE ||
                        MI.getOpcode() == RVGPU::G_FMAXNUM_IEEE;
...
bool RVGPULegalizerInfo::legalizeExtractVectorElt(
  MachineInstr &MI, MachineRegisterInfo &MRI,
  MachineIRBuilder &B) const {
...
bool RVGPULegalizerInfo::legalizeInsertVectorElt(
  MachineInstr &MI, MachineRegisterInfo &MRI,
  MachineIRBuilder &B) const {
  // TODO: Should move some of this into LegalizerHelper.
...
bool RVGPULegalizerInfo::legalizeSinCos(
  MachineInstr &MI, MachineRegisterInfo &MRI,
  MachineIRBuilder &B) const {
  ...
  if (ST.hasTrigReducedRange()) {
    ...
    TrigVal = B.buildIntrinsic(Intrinsic::rvgpu_fract, {Ty})
  ...
  Intrinsic::ID TrigIntrin = MI.getOpcode() == RVGPU::G_FSIN ?
    Intrinsic::rvgpu_sin : Intrinsic::rvgpu_cos;
...
bool RVGPULegalizerInfo::buildPCRelGlobalAddress(Register DstReg, LLT PtrTy,
                                                  MachineIRBuilder &B,
  ...
  LLT ConstPtrTy = LLT::pointer(RVGPUAS::CONSTANT_ADDRESS, 64);

  Register PCReg = PtrTy.getSizeInBits() != 32 ? DstReg :
    B.getMRI()->createGenericVirtualRegister(ConstPtrTy);

  MachineInstrBuilder MIB = B.buildInstr(RVGPU::SI_PC_ADD_REL_OFFSET)
    .addDef(PCReg);
  ...
  if (!B.getMRI()->getRegClassOrNull(PCReg))
    B.getMRI()->setRegClass(PCReg, &RVGPU::SReg_64RegClass);
...
void RVGPULegalizerInfo::buildAbsGlobalAddress(
  ...
  if (!MRI.getRegClassOrNull(AddrLo))
    MRI.setRegClass(AddrLo, &RVGPU::SReg_32RegClass);

  // Write the lower half.
  B.buildInstr(RVGPU::S_MOV_B32)
      .addDef(AddrLo)
      .addGlobalAddress(GV, 0, RVInstrInfo::MO_ABS32_LO);
  ...
  if (RequiresHighHalf) {
    ...
    MRI.setRegClass(AddrHi, &RVGPU::SReg_32RegClass);

    B.buildInstr(RVGPU::S_MOV_B32)
        .addDef(AddrHi)
        .addGlobalAddress(GV, 0, RVInstrInfo::MO_ABS32_HI);
    ...
    if (!MRI.getRegClassOrNull(AddrDst))
      MRI.setRegClass(AddrDst, &RVGPU::SReg_64RegClass);
...
bool RVGPULegalizerInfo::legalizeGlobalValue(
  MachineInstr &MI, MachineRegisterInfo &MRI,
  MachineIRBuilder &B) const {
  Register DstReg = MI.getOperand(0).getReg();
  LLT Ty = MRI.getType(DstReg);
  ...
  if (AS == RVGPUAS::LOCAL_ADDRESS || AS == RVGPUAS::REGION_ADDRESS) {
    ...
    if (!TLI->shouldUseLDSConstAddress(GV)) {
      MI.getOperand(1).setTargetFlags(RVInstrInfo::MO_ABS32_LO);
      return true; // Leave in place;
    }

    if (AS == RVGPUAS::LOCAL_ADDRESS && GV->hasExternalLinkage()) {
      ...
      if (B.getDataLayout().getTypeAllocSize(Ty).isZero()) {
        ...
        LLT S32 = LLT::scalar(32);
        auto Sz = B.buildIntrinsic(Intrinsic::rvgpu_groupstaticsize, {S32});
  ...
  if (TLI->shouldEmitPCReloc(GV)) {
    buildPCRelGlobalAddress(DstReg, Ty, B, GV, 0, RVInstrInfo::MO_REL32);
    MI.eraseFromParent();
    return true;
  }
  ...
  buildPCRelGlobalAddress(GOTAddr, PtrTy, B, GV, 0, RVInstrInfo::MO_GOTPCREL32);
...
bool RVGPULegalizerInfo::legalizeLoad(LegalizerHelper &Helper,
                                       MachineInstr &MI) const {
  MachineIRBuilder &B = Helper.MIRBuilder;
  MachineRegisterInfo &MRI = *B.getMRI();
  ...
  if (AddrSpace == RVGPUAS::CONSTANT_ADDRESS_32BIT) {
    LLT ConstPtr = LLT::pointer(RVGPUAS::CONSTANT_ADDRESS, 64);
...
bool RVGPULegalizerInfo::legalizeStore(LegalizerHelper &Helper,
                                        MachineInstr &MI) const {
  MachineIRBuilder &B = Helper.MIRBuilder;
  MachineRegisterInfo &MRI = *B.getMRI();
  GISelChangeObserver &Observer = Helper.Observer;
...
bool RVGPULegalizerInfo::legalizeFMad(
  MachineInstr &MI, MachineRegisterInfo &MRI,
  MachineIRBuilder &B) const {
...
bool RVGPULegalizerInfo::legalizeAtomicCmpXChg(
  ...
  assert(RVGPU::isFlatGlobalAddrSpace(MRI.getType(PtrReg).getAddressSpace()) &&
  ...
  B.buildInstr(RVGPU::G_RVGPU_ATOMIC_CMPXCHG)
    .addDef(DstReg)
    .addUse(PtrReg)
    .addUse(PackedVal)
...
RVGPULegalizerInfo::getScaledLogInput(MachineIRBuilder &B, Register Src,
...
bool RVGPULegalizerInfo::legalizeFlog2(MachineInstr &MI,
                                        MachineIRBuilder &B) const {
  ...
  if (Ty == LLT::scalar(16)) {
    ...
    auto Log2 = B.buildIntrinsic(Intrinsic::rvgpu_log, {F32})
                    .addUse(Ext.getReg(0))
                    .setMIFlags(Flags);
  ...
  if (!ScaledInput) {
    B.buildIntrinsic(Intrinsic::rvgpu_log, {MI.getOperand(0)})
        .addUse(Src)
        .setMIFlags(Flags);
    MI.eraseFromParent();
    return true;
  ...
  auto Log2 = B.buildIntrinsic(Intrinsic::rvgpu_log, {Ty})
                  .addUse(ScaledInput)
                  .setMIFlags(Flags);
...
bool RVGPULegalizerInfo::legalizeFlogCommon(MachineInstr &MI,
                                             MachineIRBuilder &B) const {
  ...
  auto Y =
      B.buildIntrinsic(Intrinsic::rvgpu_log, {Ty}).addUse(X).setMIFlags(Flags);
...
bool RVGPULegalizerInfo::legalizeFlogUnsafe(MachineIRBuilder &B, Register Dst,
  ...
  if (Ty == LLT::scalar(32)) {
    ...
    if (ScaledInput) {
      auto LogSrc = B.buildIntrinsic(Intrinsic::rvgpu_log, {Ty})
                        .addUse(Src)
                        .setMIFlags(Flags);
  ...
  auto Log2Operand = Ty == LLT::scalar(16)
                         ...
                         : B.buildIntrinsic(Intrinsic::rvgpu_log, {Ty})
                               .addUse(Src)
                               .setMIFlags(Flags);
...
bool RVGPULegalizerInfo::legalizeFExp2(MachineInstr &MI,
                                        MachineIRBuilder &B) const {
  ...
  if (Ty == F16) {
    // Nothing in half is a denormal when promoted to f32.
    auto Ext = B.buildFPExt(F32, Src, Flags);
    auto Log2 = B.buildIntrinsic(Intrinsic::rvgpu_exp2, {F32})
                    .addUse(Ext.getReg(0))
                    .setMIFlags(Flags);
    B.buildFPTrunc(Dst, Log2, Flags);
    MI.eraseFromParent();
  ...
  if (!needsDenormHandlingF32(B.getMF(), Src, Flags)) {
    B.buildIntrinsic(Intrinsic::rvgpu_exp2, ArrayRef<Register>{Dst})
        .addUse(Src)
        .setMIFlags(Flags);
    MI.eraseFromParent();
    return true;
  ...
  auto Exp2 = B.buildIntrinsic(Intrinsic::rvgpu_exp2, {Ty})
...
bool RVGPULegalizerInfo::legalizeFExpUnsafe(MachineIRBuilder &B, Register Dst,
...
bool RVGPULegalizerInfo::legalizeFExp(MachineInstr &MI,
                                       MachineIRBuilder &B) const {
  Register Dst = MI.getOperand(0).getReg();
  Register X = MI.getOperand(1).getReg();
...
bool RVGPULegalizerInfo::legalizeFPow(MachineInstr &MI,
                                       MachineIRBuilder &B) const {
  Register Dst = MI.getOperand(0).getReg();
  ...
  if (Ty == F32) {
    ...
    auto Mul = B.buildIntrinsic(Intrinsic::rvgpu_fmul_legacy, {F32})
                   .addUse(Log.getReg(0))
                   .addUse(Src1)
                   .setMIFlags(Flags);
...
static Register stripAnySourceMods(Register OrigSrc, MachineRegisterInfo &MRI) {
  ...
  if (MachineInstr *SrcFNeg = getOpcodeDef(RVGPU::G_FNEG, ModSrc, MRI)) {
    ModSrc = SrcFNeg->getOperand(1).getReg();
    if (MachineInstr *SrcFAbs = getOpcodeDef(RVGPU::G_FABS, ModSrc, MRI))
      ModSrc = SrcFAbs->getOperand(1).getReg();
  } else if (MachineInstr *SrcFAbs = getOpcodeDef(RVGPU::G_FABS, ModSrc, MRI))
    ModSrc = SrcFAbs->getOperand(1).getReg();
  return ModSrc;
}

bool RVGPULegalizerInfo::legalizeFFloor(MachineInstr &MI,
                                         MachineRegisterInfo &MRI,
                                         MachineIRBuilder &B) const {
  ...
  auto Fract = B.buildIntrinsic(Intrinsic::rvgpu_fract, {F64})
                   .addUse(OrigSrc)
                   .setMIFlags(Flags);
...
bool RVGPULegalizerInfo::legalizeBuildVector(
  MachineInstr &MI, MachineRegisterInfo &MRI, MachineIRBuilder &B) const {
  Register Dst = MI.getOperand(0).getReg();
  const LLT S32 = LLT::scalar(32);
  const LLT S16 = LLT::scalar(16);
  ...
  if (MI.getOpcode() == RVGPU::G_BUILD_VECTOR_TRUNC) {
...
void RVGPULegalizerInfo::buildMultiply(LegalizerHelper &Helper,
      ...
      [&](MutableArrayRef<Register> LocalAccum, unsigned DstIndex, Carry &CarryIn)
        ...
        if (j0 <= DstIndex) {
          ...
          do {
            ...
            auto Mad = B.buildInstr(RVGPU::G_RVGPU_MAD_U64_U32, {S64, S1},
                                    {Src0[j0], Src1[j1], Tmp});
            Tmp = Mad.getReg(0);
            if (!HaveSmallAccum)
...
bool RVGPULegalizerInfo::legalizeMul(LegalizerHelper &Helper,
                                      MachineInstr &MI) const {
  assert(ST.hasMad64_32());
  ...
  const bool UsePartialMad64_32 = ST.getGeneration() < RVGPUSubtarget::GFX10;
...
bool RVGPULegalizerInfo::legalizeCTLZ_CTTZ(MachineInstr &MI,
                                            MachineRegisterInfo &MRI,
                                            MachineIRBuilder &B) const {
  Register Dst = MI.getOperand(0).getReg();
  Register Src = MI.getOperand(1).getReg();
  ...
  unsigned NewOpc = MI.getOpcode() == RVGPU::G_CTLZ
                        ? RVGPU::G_RVGPU_FFBH_U32
                        : RVGPU::G_RVGPU_FFBL_B32;
...
verifyCFIntrinsic(MachineInstr &MI, MachineRegisterInfo &MRI, MachineInstr *&Br,
  ...
  if (UseMI->getParent() != Parent || UseMI->getOpcode() != RVGPU::G_BRCOND)
    return nullptr;
...
bool RVGPULegalizerInfo::loadInputValue(
  ...
  if (!Arg) {
    if (ArgType == RVGPUFunctionArgInfo::KERNARG_SEGMENT_PTR) {
...
bool RVGPULegalizerInfo::legalizePreloadedArgIntrin(
...
bool RVGPULegalizerInfo::legalizeWorkitemIDIntrinsic(
    MachineInstr &MI, MachineRegisterInfo &MRI, MachineIRBuilder &B,
    unsigned Dim, RVGPUFunctionArgInfo::PreloadedValue ArgType) const {
...
Register RVGPULegalizerInfo::getKernargParameterPtr(MachineIRBuilder &B,
...
/// legacy intrinsics.
bool RVGPULegalizerInfo::legalizeKernargMemParameter(MachineInstr &MI,
                                                      MachineIRBuilder &B,
                                                      uint64_t Offset,
...
bool RVGPULegalizerInfo::legalizeFDIV(MachineInstr &MI,
                                       MachineRegisterInfo &MRI,
                                       MachineIRBuilder &B) const {
  Register Dst = MI.getOperand(0).getReg();
  LLT DstTy = MRI.getType(Dst);
...
void RVGPULegalizerInfo::legalizeUnsignedDIV_REM32Impl(MachineIRBuilder &B,
                                                        Register DstDivReg,
                                                        Register DstRemReg,
                                                        Register X,
                                                        Register Y) const {
  ...
  auto RcpIFlag = B.buildInstr(RVGPU::G_RVGPU_RCP_IFLAG, {S32}, {FloatY});
...
static std::pair<Register, Register> emitReciprocalU64(MachineIRBuilder &B,
  ...
  auto Rcp = B.buildInstr(RVGPU::G_RVGPU_RCP_IFLAG, {S32}, {Mad});
...
void RVGPULegalizerInfo::legalizeUnsignedDIV_REM64Impl(MachineIRBuilder &B,
                                                        Register DstDivReg,
                                                        Register DstRemReg,
                                                        Register Numer,
...
bool RVGPULegalizerInfo::legalizeUnsignedDIV_REM(MachineInstr &MI,
                                                  MachineRegisterInfo &MRI,
                                                  MachineIRBuilder &B) const {
...
bool RVGPULegalizerInfo::legalizeSignedDIV_REM(MachineInstr &MI,
                                                MachineRegisterInfo &MRI,
                                                MachineIRBuilder &B) const {
  const LLT S64 = LLT::scalar(64);
  const LLT S32 = LLT::scalar(32);
...
bool RVGPULegalizerInfo::legalizeFastUnsafeFDIV(MachineInstr &MI,
                                                 MachineRegisterInfo &MRI,
                                                 MachineIRBuilder &B) const {
  ...
  if (auto CLHS = getConstantFPVRegVal(LHS, MRI)) {
    ...
    if (CLHS->isExactlyValue(1.0)) {
      B.buildIntrinsic(Intrinsic::rvgpu_rcp, Res)
          .addUse(RHS)
          .setMIFlags(Flags);
  ...
  // x / y -> x * (1.0 / y)
  auto RCP = B.buildIntrinsic(Intrinsic::rvgpu_rcp, {ResTy})
                 .addUse(RHS)
                 .setMIFlags(Flags);
...
bool RVGPULegalizerInfo::legalizeFastUnsafeFDIV64(MachineInstr &MI,
                                                   MachineRegisterInfo &MRI,
                                                   MachineIRBuilder &B) const {
  Register Res = MI.getOperand(0).getReg();
  Register X = MI.getOperand(1).getReg();
  ...
  auto R = B.buildIntrinsic(Intrinsic::rvgpu_rcp, {ResTy})
               .addUse(Y)
               .setMIFlags(Flags);
...
bool RVGPULegalizerInfo::legalizeFDIV16(MachineInstr &MI,
                                         MachineRegisterInfo &MRI,
                                         MachineIRBuilder &B) const {
  ...
  auto RCP = B.buildIntrinsic(Intrinsic::rvgpu_rcp, {S32})
  ...
  B.buildIntrinsic(Intrinsic::rvgpu_div_fixup, Res)
...
static const unsigned SPDenormModeBitField =
    RVGPU::Hwreg::ID_MODE | (4 << RVGPU::Hwreg::OFFSET_SHIFT_) |
...
static void toggleSPDenormMode(bool Enable, MachineIRBuilder &B,
  ...
  if (ST.hasDenormModeInst()) {
    ...
    B.buildInstr(RVGPU::S_DENORM_MODE)
      .addImm(NewDenormModeValue);

  } else {
    B.buildInstr(RVGPU::S_SETREG_IMM32_B32)
      .addImm(SPDenormMode)
      .addImm(SPDenormModeBitField);
  }
}

bool RVGPULegalizerInfo::legalizeFDIV32(MachineInstr &MI,
                                         MachineRegisterInfo &MRI,
                                         MachineIRBuilder &B) const {
  if (legalizeFastUnsafeFDIV(MI, MRI, B))
    return true;
  ...
  auto DenominatorScaled =
      B.buildIntrinsic(Intrinsic::rvgpu_div_scale, {S32, S1})
          .addUse(LHS)
          .addUse(RHS)
          .addImm(0)
          .setMIFlags(Flags);
  ...
  auto ApproxRcp = B.buildIntrinsic(Intrinsic::rvgpu_rcp, {S32})
  ...
  if (!PreservesDenormals) {
    if (HasDynamicDenormals) {
      SavedSPDenormMode = MRI.createVirtualRegister(&RVGPU::SReg_32RegClass);
      B.buildInstr(RVGPU::S_GETREG_B32)
  ...
  if (!PreservesDenormals) {
    if (HasDynamicDenormals) {
      ...
      B.buildInstr(RVGPU::S_SETREG_B32)
  ...
  auto Fmas = B.buildIntrinsic(Intrinsic::rvgpu_div_fmas, {S32})
...
bool RVGPULegalizerInfo::legalizeFDIV64(MachineInstr &MI,
                                         MachineRegisterInfo &MRI,
                                         MachineIRBuilder &B) const {
  ...
  auto DivScale0 = B.buildIntrinsic(Intrinsic::rvgpu_div_scale, {S64, S1})
                       .addUse(LHS)
                       .addUse(RHS)
                       .addImm(0)
                       .setMIFlags(Flags);
  ...
  auto Rcp = B.buildIntrinsic(Intrinsic::rvgpu_rcp, {S64})
  ...
  auto DivScale1 = B.buildIntrinsic(Intrinsic::rvgpu_div_scale, {S64, S1})
                       .addUse(LHS)
                       .addUse(RHS)
                       .addImm(1)
                       .setMIFlags(Flags);
  ...
  auto Fmas = B.buildIntrinsic(Intrinsic::rvgpu_div_fmas, {S64})
                  .addUse(Fma4.getReg(0))
                  .addUse(Fma3.getReg(0))
                  .addUse(Mul.getReg(0))
                  .addUse(Scale)
  ...
  B.buildIntrinsic(Intrinsic::rvgpu_div_fixup, ArrayRef(Res))
      .addUse(Fmas.getReg(0))
      .addUse(RHS)
      .addUse(LHS)
      .setMIFlags(Flags);
...
bool RVGPULegalizerInfo::legalizeFFREXP(MachineInstr &MI,
                                         MachineRegisterInfo &MRI,
                                         MachineIRBuilder &B) const {
  ...
  auto Mant = B.buildIntrinsic(Intrinsic::rvgpu_frexp_mant, {Ty})
                  .addUse(Val)
                  .setMIFlags(Flags);
  auto Exp = B.buildIntrinsic(Intrinsic::rvgpu_frexp_exp, {InstrExpTy})
                 .addUse(Val)
                 .setMIFlags(Flags);
...
bool RVGPULegalizerInfo::legalizeFDIVFastIntrin(MachineInstr &MI,
                                                 MachineRegisterInfo &MRI,
                                                 MachineIRBuilder &B) const {
  Register Res = MI.getOperand(0).getReg();
...
bool RVGPULegalizerInfo::legalizeFSQRTF16(MachineInstr &MI,
                                           MachineRegisterInfo &MRI,
                                           MachineIRBuilder &B) const {
  ...
  auto Log2 = B.buildIntrinsic(Intrinsic::rvgpu_sqrt, {F32})
    .addUse(Ext.getReg(0))
    .setMIFlags(Flags);
...
bool RVGPULegalizerInfo::legalizeFSQRTF32(MachineInstr &MI,
                                           MachineRegisterInfo &MRI,
                                           MachineIRBuilder &B) const {
  MachineFunction &MF = B.getMF();
  Register Dst = MI.getOperand(0).getReg();
  ...
  if (allowApproxFunc(MF, Flags)) {
    B.buildIntrinsic(Intrinsic::rvgpu_sqrt, ArrayRef<Register>({Dst}))
      .addUse(X)
      .setMIFlags(Flags);
    MI.eraseFromParent();
    return true;
  ...
  if (needsDenormHandlingF32(MF, X, Flags)) {
    B.buildIntrinsic(Intrinsic::rvgpu_sqrt, ArrayRef<Register>({SqrtS}))
    ...
    auto SqrtR =
        B.buildIntrinsic(Intrinsic::rvgpu_rsq, {F32}).addReg(SqrtX.getReg(0));
...
bool RVGPULegalizerInfo::legalizeFSQRTF64(MachineInstr &MI,
                                           MachineRegisterInfo &MRI,
                                           MachineIRBuilder &B) const {
  ...
  auto SqrtY =
      B.buildIntrinsic(Intrinsic::rvgpu_rsq, {F64}).addReg(SqrtX.getReg(0));
...
bool RVGPULegalizerInfo::legalizeFSQRT(MachineInstr &MI,
                                        MachineRegisterInfo &MRI,
                                        MachineIRBuilder &B) const {
  LLT Ty = MRI.getType(MI.getOperand(0).getReg());
  if (Ty == LLT::scalar(32))
...
// Expand llvm.rvgpu.rsq.clamp on targets that don't support the instruction.
...
// +-max_float.
bool RVGPULegalizerInfo::legalizeRsqClampIntrinsic(MachineInstr &MI,
                                                    MachineRegisterInfo &MRI,
                                                    MachineIRBuilder &B) const {
  ...
  auto Rsq = B.buildIntrinsic(Intrinsic::rvgpu_rsq, {Ty})
                 .addUse(Src)
                 .setMIFlags(Flags);
...
static unsigned getDSFPAtomicOpcode(Intrinsic::ID IID) {
  ...
  case Intrinsic::rvgpu_ds_fadd:
    return RVGPU::G_ATOMICRMW_FADD;
  case Intrinsic::rvgpu_ds_fmin:
    return RVGPU::G_RVGPU_ATOMIC_FMIN;
  case Intrinsic::rvgpu_ds_fmax:
    return RVGPU::G_RVGPU_ATOMIC_FMAX;
  default:
    llvm_unreachable("not a DS FP intrinsic");
  }
}

bool RVGPULegalizerInfo::legalizeDSAtomicFPIntrinsic(LegalizerHelper &Helper,
                                                      MachineInstr &MI,
...
bool RVGPULegalizerInfo::getImplicitArgPtr(Register DstReg,
                                            MachineRegisterInfo &MRI,
                                            MachineIRBuilder &B) const {
  uint64_t Offset =
    ST.getTargetLowering()->getImplicitParameterOffset(
      B.getMF(), RVGPUTargetLowering::FIRST_IMPLICIT);
...
bool RVGPULegalizerInfo::legalizePointerAsRsrcIntrin(
    MachineInstr &MI, MachineRegisterInfo &MRI, MachineIRBuilder &B) const {
...
bool RVGPULegalizerInfo::legalizeImplicitArgPtr(MachineInstr &MI,
                                                 MachineRegisterInfo &MRI,
                                                 MachineIRBuilder &B) const {
  const RVMachineFunctionInfo *MFI = B.getMF().getInfo<RVMachineFunctionInfo>();
...
bool RVGPULegalizerInfo::getLDSKernelId(Register DstReg,
                                         MachineRegisterInfo &MRI,
                                         MachineIRBuilder &B) const {
...
bool RVGPULegalizerInfo::legalizeLDSKernelId(MachineInstr &MI,
                                              MachineRegisterInfo &MRI,
                                              MachineIRBuilder &B) const {
...
bool RVGPULegalizerInfo::legalizeIsAddrSpace(MachineInstr &MI,
                                              MachineRegisterInfo &MRI,
                                              MachineIRBuilder &B,
...
RVGPULegalizerInfo::splitBufferOffsets(MachineIRBuilder &B,
                                        Register OrigOffset) const {
  const unsigned MaxImm = RVInstrInfo::getMaxMUBUFImmOffset(ST);
  Register BaseReg;
  unsigned ImmOffset;
  const LLT S32 = LLT::scalar(32);
  MachineRegisterInfo &MRI = *B.getMRI();
  ...
  std::tie(BaseReg, ImmOffset) =
      RVGPU::getBaseWithConstantOffset(MRI, OrigOffset);
...
Register RVGPULegalizerInfo::handleD16VData(MachineIRBuilder &B,
                                             MachineRegisterInfo &MRI,
                                             Register Reg,
...
Register RVGPULegalizerInfo::fixStoreSourceType(
...
bool RVGPULegalizerInfo::legalizeBufferStore(MachineInstr &MI,
                                              MachineRegisterInfo &MRI,
                                              MachineIRBuilder &B,
                                              bool IsTyped,
                                              bool IsFormat) const {
  ...
  unsigned Opc;
  if (IsTyped) {
    Opc = IsD16 ? RVGPU::G_RVGPU_TBUFFER_STORE_FORMAT_D16 :
                  RVGPU::G_RVGPU_TBUFFER_STORE_FORMAT;
  } else if (IsFormat) {
    Opc = IsD16 ? RVGPU::G_RVGPU_BUFFER_STORE_FORMAT_D16 :
                  RVGPU::G_RVGPU_BUFFER_STORE_FORMAT;
  } else {
    switch (MemSize) {
    case 1:
      Opc = RVGPU::G_RVGPU_BUFFER_STORE_BYTE;
      break;
    case 2:
      Opc = RVGPU::G_RVGPU_BUFFER_STORE_SHORT;
      break;
    default:
      Opc = RVGPU::G_RVGPU_BUFFER_STORE;
      break;
    }
  }
...
bool RVGPULegalizerInfo::legalizeBufferLoad(MachineInstr &MI,
                                             MachineRegisterInfo &MRI,
                                             MachineIRBuilder &B,
                                             bool IsFormat,
                                             bool IsTyped) const {
  ...
  if (IsTyped) {
    if (IsTFE)
      return false;
    Opc = IsD16 ? RVGPU::G_RVGPU_TBUFFER_LOAD_FORMAT_D16 :
                  RVGPU::G_RVGPU_TBUFFER_LOAD_FORMAT;
  } else if (IsFormat) {
    if (IsD16) {
      if (IsTFE)
        return false;
      Opc = RVGPU::G_RVGPU_BUFFER_LOAD_FORMAT_D16;
    } else {
      Opc = IsTFE ? RVGPU::G_RVGPU_BUFFER_LOAD_FORMAT_TFE
                  : RVGPU::G_RVGPU_BUFFER_LOAD_FORMAT;
    }
  } else {
    if (IsTFE)
      return false;
    ...
    case 8:
      Opc = RVGPU::G_RVGPU_BUFFER_LOAD_UBYTE;
      break;
    case 16:
      Opc = RVGPU::G_RVGPU_BUFFER_LOAD_USHORT;
      break;
    default:
      Opc = RVGPU::G_RVGPU_BUFFER_LOAD;
      break;
    }
  }
...
static unsigned getBufferAtomicPseudo(Intrinsic::ID IntrID) {
  ...
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_swap:
    return RVGPU::G_RVGPU_BUFFER_ATOMIC_SWAP;
  ...
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_add:
    return RVGPU::G_RVGPU_BUFFER_ATOMIC_ADD;
  ...
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_sub:
    return RVGPU::G_RVGPU_BUFFER_ATOMIC_SUB;
  ...
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_smin:
    return RVGPU::G_RVGPU_BUFFER_ATOMIC_SMIN;
  ...
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_umin:
    return RVGPU::G_RVGPU_BUFFER_ATOMIC_UMIN;
  ...
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_smax:
    return RVGPU::G_RVGPU_BUFFER_ATOMIC_SMAX;
  ...
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_umax:
    return RVGPU::G_RVGPU_BUFFER_ATOMIC_UMAX;
  ...
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_and:
    return RVGPU::G_RVGPU_BUFFER_ATOMIC_AND;
  ...
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_or:
    return RVGPU::G_RVGPU_BUFFER_ATOMIC_OR;
  ...
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_xor:
    return RVGPU::G_RVGPU_BUFFER_ATOMIC_XOR;
  ...
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_inc:
    return RVGPU::G_RVGPU_BUFFER_ATOMIC_INC;
  ...
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_dec:
    return RVGPU::G_RVGPU_BUFFER_ATOMIC_DEC;
  ...
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_cmpswap:
    return RVGPU::G_RVGPU_BUFFER_ATOMIC_CMPSWAP;
  ...
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_fadd:
    return RVGPU::G_RVGPU_BUFFER_ATOMIC_FADD;
  ...
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_fmin:
    return RVGPU::G_RVGPU_BUFFER_ATOMIC_FMIN;
  ...
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_fmax:
    return RVGPU::G_RVGPU_BUFFER_ATOMIC_FMAX;
  default:
    llvm_unreachable("unhandled atomic opcode");
  }
}

bool RVGPULegalizerInfo::legalizeBufferAtomic(MachineInstr &MI,
                                               MachineIRBuilder &B,
                                               Intrinsic::ID IID) const {
  const bool IsCmpSwap =
      IID == Intrinsic::rvgpu_raw_buffer_atomic_cmpswap ||
      IID == Intrinsic::rvgpu_struct_buffer_atomic_cmpswap ||
      IID == Intrinsic::rvgpu_raw_ptr_buffer_atomic_cmpswap ||
      IID == Intrinsic::rvgpu_struct_ptr_buffer_atomic_cmpswap;
...
static void convertImageAddrToPacked(MachineIRBuilder &B, MachineInstr &MI,
  ...
  for (int I = 1; I != NumVAddrs; ++I) {
    MachineOperand &SrcOp = MI.getOperand(DimIdx + I);
    if (SrcOp.isReg())
      MI.getOperand(DimIdx + I).setReg(RVGPU::NoRegister);
  }
}
...
bool RVGPULegalizerInfo::legalizeImageIntrinsic(
  ...
  const unsigned StoreOpcode = IsD16 ? RVGPU::G_RVGPU_INTRIN_IMAGE_STORE_D16
                                     : RVGPU::G_RVGPU_INTRIN_IMAGE_STORE;
  const unsigned LoadOpcode = IsD16 ? RVGPU::G_RVGPU_INTRIN_IMAGE_LOAD_D16
                                    : RVGPU::G_RVGPU_INTRIN_IMAGE_LOAD;
  ...
  if (BaseOpcode->Atomic) {
    ...
    if (BaseOpcode->AtomicX2) {
      ...
      MI.getOperand(3).setReg(RVGPU::NoRegister);
    }
  }
...
bool RVGPULegalizerInfo::legalizeSBufferLoad(
  ...
  // allowed to add one.
  MI.setDesc(B.getTII().get(RVGPU::G_RVGPU_S_BUFFER_LOAD));
...
bool RVGPULegalizerInfo::legalizeTrapIntrinsic(MachineInstr &MI,
                                                MachineRegisterInfo &MRI,
                                                MachineIRBuilder &B) const {
...
bool RVGPULegalizerInfo::legalizeTrapEndpgm(
    MachineInstr &MI, MachineRegisterInfo &MRI, MachineIRBuilder &B) const {
  ...
  if (BB.succ_empty() && std::next(MI.getIterator()) == BB.end()) {
    BuildMI(BB, BB.end(), DL, B.getTII().get(RVGPU::S_ENDPGM))
      .addImm(0);
    MI.eraseFromParent();
    return true;
  }
  ...
  MF->push_back(TrapBB);
  BuildMI(*TrapBB, TrapBB->end(), DL, B.getTII().get(RVGPU::S_ENDPGM))
    .addImm(0);
  BuildMI(BB, &MI, DL, B.getTII().get(RVGPU::S_CBRANCH_EXECNZ))
    .addMBB(TrapBB);
...
bool RVGPULegalizerInfo::legalizeTrapHsaQueuePtr(
    MachineInstr &MI, MachineRegisterInfo &MRI, MachineIRBuilder &B) const {
  MachineFunction &MF = B.getMF();
  const LLT S64 = LLT::scalar(64);
  ...
  // Reference: https://llvm.org/docs/RVGPUUsage.html#trap-handler-abi
  Register LiveIn =
    MRI.createGenericVirtualRegister(LLT::pointer(RVGPUAS::CONSTANT_ADDRESS, 64));
  if (!loadInputValue(LiveIn, B, RVGPUFunctionArgInfo::QUEUE_PTR))
    return false;
...
bool RVGPULegalizerInfo::legalizeDebugTrapIntrinsic(
    MachineInstr &MI, MachineRegisterInfo &MRI, MachineIRBuilder &B) const {
...
bool RVGPULegalizerInfo::legalizeBVHIntrinsic(MachineInstr &MI,
                                               MachineIRBuilder &B) const {
  MachineRegisterInfo &MRI = *B.getMRI();
  const LLT S16 = LLT::scalar(16);
  const LLT S32 = LLT::scalar(32);
  ...
  auto MIB = B.buildInstr(RVGPU::G_RVGPU_INTRIN_BVH_INTERSECT_RAY)
    .addDef(DstReg)
    .addImm(Opcode);
...
bool RVGPULegalizerInfo::legalizeStackSave(MachineInstr &MI,
                                            MachineIRBuilder &B) const {
  const RVTargetLowering *TLI = ST.getTargetLowering();
  Register StackPtr = TLI->getStackPointerRegisterToSaveRestore();
  Register DstReg = MI.getOperand(0).getReg();
  B.buildInstr(RVGPU::G_RVGPU_WAVE_ADDRESS, {DstReg}, {StackPtr});
  MI.eraseFromParent();
  return true;
}

bool RVGPULegalizerInfo::legalizeIntrinsic(LegalizerHelper &Helper,
                                            MachineInstr &MI) const {
  MachineIRBuilder &B = Helper.MIRBuilder;
  MachineRegisterInfo &MRI = *B.getMRI();
  ...
  case Intrinsic::rvgpu_else: {
    ...
    if (MachineInstr *BrCond =
      ...
      if (IntrID == Intrinsic::rvgpu_if) {
        B.buildInstr(RVGPU::SI_IF)
          .addDef(Def)
          .addUse(Use)
          .addMBB(UncondBrTarget);
      } else {
        B.buildInstr(RVGPU::SI_ELSE)
            .addDef(Def)
            .addUse(Use)
            .addMBB(UncondBrTarget);
      }
  ...
  case Intrinsic::rvgpu_kernarg_segment_ptr:
    if (!RVGPU::isKernel(B.getMF().getFunction().getCallingConv())) {
    ...
    return legalizePreloadedArgIntrin(
      MI, MRI, B, RVGPUFunctionArgInfo::KERNARG_SEGMENT_PTR);
  ...
  case Intrinsic::rvgpu_implicit_buffer_ptr:
    return legalizePreloadedArgIntrin(
      MI, MRI, B, RVGPUFunctionArgInfo::IMPLICIT_BUFFER_PTR);
  ...
  case Intrinsic::rvgpu_is_shared:
    return legalizeIsAddrSpace(MI, MRI, B, RVGPUAS::LOCAL_ADDRESS);
  case Intrinsic::rvgpu_is_private:
    return legalizeIsAddrSpace(MI, MRI, B, RVGPUAS::PRIVATE_ADDRESS);
  ...
  case Intrinsic::rvgpu_fmed3: {
    ...
    Observer.changingInstr(MI);
    MI.setDesc(B.getTII().get(RVGPU::G_RVGPU_FMED3));
    MI.removeOperand(1);
    Observer.changedInstr(MI);
    return true;
  }
