...
static cl::opt<bool> DisableLoopAlignment(
  "rvgpu-disable-loop-alignment",
  cl::desc("Do not align and prefetch loops"),
  cl::init(false));

static cl::opt<bool> UseDivergentRegisterIndexing(
  "rvgpu-use-divergent-register-indexing",
  cl::Hidden,
...
static unsigned findFirstFreeSGPR(CCState &CCInfo) {
  unsigned NumSGPRs = RVGPU::SGPR_32RegClass.getNumRegs();
  for (unsigned Reg = 0; Reg < NumSGPRs; ++Reg) {
    if (!CCInfo.isAllocated(RVGPU::SGPR0 + Reg)) {
      return RVGPU::SGPR0 + Reg;
    }
  }
...
RVTargetLowering::RVTargetLowering(const TargetMachine &TM,
  ...
  addRegisterClass(MVT::i64, &RVGPU::SReg_64RegClass);

  addRegisterClass(MVT::i32, &RVGPU::SReg_32RegClass);
  addRegisterClass(MVT::f32, &RVGPU::VGPR_32RegClass);

  addRegisterClass(MVT::v2i32, &RVGPU::SReg_64RegClass);

  const RVRegisterInfo *TRI = STI.getRegisterInfo();
  ...
  addRegisterClass(MVT::v3i32, &RVGPU::SGPR_96RegClass);
  addRegisterClass(MVT::v3f32, TRI->getVGPRClassForBitWidth(96));

  addRegisterClass(MVT::v2i64, &RVGPU::SGPR_128RegClass);
  addRegisterClass(MVT::v2f64, &RVGPU::SGPR_128RegClass);

  addRegisterClass(MVT::v4i32, &RVGPU::SGPR_128RegClass);
  addRegisterClass(MVT::v4f32, TRI->getVGPRClassForBitWidth(128));

  addRegisterClass(MVT::v5i32, &RVGPU::SGPR_160RegClass);
  addRegisterClass(MVT::v5f32, TRI->getVGPRClassForBitWidth(160));

  addRegisterClass(MVT::v6i32, &RVGPU::SGPR_192RegClass);
  addRegisterClass(MVT::v6f32, TRI->getVGPRClassForBitWidth(192));

  addRegisterClass(MVT::v3i64, &RVGPU::SGPR_192RegClass);
  addRegisterClass(MVT::v3f64, TRI->getVGPRClassForBitWidth(192));

  addRegisterClass(MVT::v7i32, &RVGPU::SGPR_224RegClass);
  addRegisterClass(MVT::v7f32, TRI->getVGPRClassForBitWidth(224));

  addRegisterClass(MVT::v8i32, &RVGPU::SGPR_256RegClass);
  addRegisterClass(MVT::v8f32, TRI->getVGPRClassForBitWidth(256));

  addRegisterClass(MVT::v4i64, &RVGPU::SGPR_256RegClass);
  addRegisterClass(MVT::v4f64, TRI->getVGPRClassForBitWidth(256));

  addRegisterClass(MVT::v9i32, &RVGPU::SGPR_288RegClass);
  addRegisterClass(MVT::v9f32, TRI->getVGPRClassForBitWidth(288));

  addRegisterClass(MVT::v10i32, &RVGPU::SGPR_320RegClass);
  addRegisterClass(MVT::v10f32, TRI->getVGPRClassForBitWidth(320));

  addRegisterClass(MVT::v11i32, &RVGPU::SGPR_352RegClass);
  addRegisterClass(MVT::v11f32, TRI->getVGPRClassForBitWidth(352));

  addRegisterClass(MVT::v12i32, &RVGPU::SGPR_384RegClass);
  addRegisterClass(MVT::v12f32, TRI->getVGPRClassForBitWidth(384));

  addRegisterClass(MVT::v16i32, &RVGPU::SGPR_512RegClass);
  addRegisterClass(MVT::v16f32, TRI->getVGPRClassForBitWidth(512));

  addRegisterClass(MVT::v8i64, &RVGPU::SGPR_512RegClass);
  addRegisterClass(MVT::v8f64, TRI->getVGPRClassForBitWidth(512));

  addRegisterClass(MVT::v16i64, &RVGPU::SGPR_1024RegClass);
  addRegisterClass(MVT::v16f64, TRI->getVGPRClassForBitWidth(1024));

  if (Subtarget->has16BitInsts()) {
    if (Subtarget->useRealTrue16Insts()) {
      addRegisterClass(MVT::i16, &RVGPU::VGPR_16RegClass);
      addRegisterClass(MVT::f16, &RVGPU::VGPR_16RegClass);
    } else {
      addRegisterClass(MVT::i16, &RVGPU::SReg_32RegClass);
      addRegisterClass(MVT::f16, &RVGPU::SReg_32RegClass);
    }

    // Unless there are also VOP3P operations, not operations are really legal.
    addRegisterClass(MVT::v2i16, &RVGPU::SReg_32RegClass);
    addRegisterClass(MVT::v2f16, &RVGPU::SReg_32RegClass);
    addRegisterClass(MVT::v4i16, &RVGPU::SReg_64RegClass);
    addRegisterClass(MVT::v4f16, &RVGPU::SReg_64RegClass);
    addRegisterClass(MVT::v8i16, &RVGPU::SGPR_128RegClass);
    addRegisterClass(MVT::v8f16, &RVGPU::SGPR_128RegClass);
    addRegisterClass(MVT::v16i16, &RVGPU::SGPR_256RegClass);
    addRegisterClass(MVT::v16f16, &RVGPU::SGPR_256RegClass);
    addRegisterClass(MVT::v32i16, &RVGPU::SGPR_512RegClass);
    addRegisterClass(MVT::v32f16, &RVGPU::SGPR_512RegClass);
  }

  addRegisterClass(MVT::v32i32, &RVGPU::VReg_1024RegClass);
  ...
  setStackPointerRegisterToSaveRestore(RVGPU::SGPR32);
...
bool RVTargetLowering::isFPExtFoldable(const SelectionDAG &DAG, unsigned Opcode,
...
bool RVTargetLowering::isFPExtFoldable(const MachineInstr &MI, unsigned Opcode,
...
bool RVTargetLowering::isShuffleMaskLegal(ArrayRef<int>, EVT) const {
  // SI has some legal vector types, but no legal vector operations. Say no
  // shuffles are legal in order to prefer scalarizing some vector operations.
  return false;
}

MVT RVTargetLowering::getRegisterTypeForCallingConv(LLVMContext &Context,
                                                    CallingConv::ID CC,
                                                    EVT VT) const {
...
unsigned RVTargetLowering::getNumRegistersForCallingConv(LLVMContext &Context,
                                                         CallingConv::ID CC,
                                                         EVT VT) const {
  if (CC == CallingConv::RVGPU_KERNEL)
...
unsigned RVTargetLowering::getVectorTypeBreakdownForCallingConv(
  LLVMContext &Context, CallingConv::ID CC,
  EVT VT, EVT &IntermediateVT,
  unsigned &NumIntermediates, MVT &RegisterVT) const {
  if (CC != CallingConv::RVGPU_KERNEL && VT.isVector()) {
    unsigned NumElts = VT.getVectorNumElements();
    EVT ScalarVT = VT.getScalarType();
    unsigned Size = ScalarVT.getSizeInBits();
...
/// modeling, to work.
MVT RVTargetLowering::getPointerTy(const DataLayout &DL, unsigned AS) const {
  if (RVGPUAS::BUFFER_FAT_POINTER == AS && DL.getPointerSizeInBits(AS) == 160)
    return MVT::v5i32;
  if (RVGPUAS::BUFFER_STRIDED_POINTER == AS &&
      DL.getPointerSizeInBits(AS) == 192)
    return MVT::v6i32;
  return RVGPUTargetLowering::getPointerTy(DL, AS);
}
...
MVT RVTargetLowering::getPointerMemTy(const DataLayout &DL, unsigned AS) const {
  if ((RVGPUAS::BUFFER_FAT_POINTER == AS &&
       DL.getPointerSizeInBits(AS) == 160) ||
      (RVGPUAS::BUFFER_STRIDED_POINTER == AS &&
       DL.getPointerSizeInBits(AS) == 192))
    return MVT::v8i32;
  return RVGPUTargetLowering::getPointerMemTy(DL, AS);
}

bool RVTargetLowering::getTgtMemIntrinsic(IntrinsicInfo &Info,
                                          const CallInst &CI,
                                          MachineFunction &MF,
  ...
  if (const RVGPU::RsrcIntrinsic *RsrcIntr =
    ...
    Info.fallbackAddressSpace = RVGPUAS::BUFFER_RESOURCE;
    ...
    if (auto *RsrcPtrTy = dyn_cast<PointerType>(RsrcArg->getType())) {
      if (RsrcPtrTy->getAddressSpace() == RVGPUAS::BUFFER_RESOURCE)
    ...
    if (ME.onlyReadsMemory()) {
      ...
      if (RsrcIntr->IsImage) {
        const RVGPU::ImageDimIntrinsicInfo *Intr
          = RVGPU::getImageDimIntrinsicInfo(IntrID);
  ...
  case Intrinsic::rvgpu_ds_fmin:
  case Intrinsic::rvgpu_ds_fmax: {
    Info.opc = ISD::INTRINSIC_W_CHAIN;
  ...
  case Intrinsic::rvgpu_buffer_atomic_fadd: {
    Info.opc = ISD::INTRINSIC_W_CHAIN;
  ...
  case Intrinsic::rvgpu_ds_sub_gs_reg_rtn: {
    ...
    Info.memVT = MVT::getVT(CI.getOperand(0)->getType());
    Info.ptrVal = nullptr;
    Info.fallbackAddressSpace = RVGPUAS::STREAMOUT_REGISTER;
    Info.flags = MachineMemOperand::MOLoad | MachineMemOperand::MOStore;
    return true;
  }
  ...
  case Intrinsic::rvgpu_image_bvh_intersect_ray: {
    Info.opc = ISD::INTRINSIC_W_CHAIN;
  ...
  case Intrinsic::rvgpu_ds_gws_sema_release_all: {
    ...
    if (IntrID == Intrinsic::rvgpu_ds_gws_barrier)
      Info.flags |= MachineMemOperand::MOLoad;
    else
      Info.flags |= MachineMemOperand::MOStore;
    return true;
...
bool RVTargetLowering::getAddrModeArguments(IntrinsicInst *II,
...
bool RVTargetLowering::isLegalFlatAddressingMode(const AddrMode &AM,
                                                 unsigned AddrSpace,
...
bool RVTargetLowering::isLegalGlobalAddressingMode(const AddrMode &AM) const {
  if (Subtarget->hasFlatGlobalInsts())
    return isLegalFlatAddressingMode(AM, RVGPUAS::GLOBAL_ADDRESS,
  ...
  if (!Subtarget->hasAddr64() || Subtarget->useFlatForGlobal()) {
    ...
    return isLegalFlatAddressingMode(AM, RVGPUAS::FLAT_ADDRESS,
                                     SIInstrFlags::FLAT);
  }
...
bool RVTargetLowering::isLegalMUBUFAddressingMode(const AddrMode &AM) const {
  ...
  const RVInstrInfo *TII = Subtarget->getInstrInfo();
...
bool RVTargetLowering::isLegalAddressingMode(const DataLayout &DL,
  ...
  if (AS == RVGPUAS::GLOBAL_ADDRESS)
    return isLegalGlobalAddressingMode(AM);

  if (AS == RVGPUAS::CONSTANT_ADDRESS ||
      AS == RVGPUAS::CONSTANT_ADDRESS_32BIT ||
      AS == RVGPUAS::BUFFER_FAT_POINTER || AS == RVGPUAS::BUFFER_RESOURCE ||
      AS == RVGPUAS::BUFFER_STRIDED_POINTER) {
    ...
    if (Subtarget->getGeneration() == RVGPUSubtarget::SOUTHERN_ISLANDS) {
      // SMRD instructions have an 8-bit, dword offset on SI.
      if (!isUInt<8>(AM.BaseOffs / 4))
        return false;
    } else if (Subtarget->getGeneration() == RVGPUSubtarget::SEA_ISLANDS) {
      // On CI+, this can also be a 32-bit literal constant offset. If it fits
      // in 8-bits, it can use a smaller encoding.
      if (!isUInt<32>(AM.BaseOffs / 4))
        return false;
    } else if (Subtarget->getGeneration() < RVGPUSubtarget::GFX9) {
      // On VI, these use the SMEM format and the offset is 20-bit in bytes.
      if (!isUInt<20>(AM.BaseOffs))
        return false;
    } else if (Subtarget->getGeneration() < RVGPUSubtarget::GFX12) {
  ...
  if (AS == RVGPUAS::PRIVATE_ADDRESS)
    return Subtarget->enableFlatScratch()
               ? isLegalFlatAddressingMode(AM, RVGPUAS::PRIVATE_ADDRESS,
                                           SIInstrFlags::FlatScratch)
               : isLegalMUBUFAddressingMode(AM);

  if (AS == RVGPUAS::LOCAL_ADDRESS ||
      (AS == RVGPUAS::REGION_ADDRESS && Subtarget->hasGDS())) {
  ...
  if (AS == RVGPUAS::FLAT_ADDRESS || AS == RVGPUAS::UNKNOWN_ADDRESS_SPACE) {
...
bool RVTargetLowering::canMergeStoresTo(unsigned AS, EVT MemVT,
                                        const MachineFunction &MF) const {
  if (AS == RVGPUAS::GLOBAL_ADDRESS || AS == RVGPUAS::FLAT_ADDRESS) {
    return (MemVT.getSizeInBits() <= 4 * 32);
  } else if (AS == RVGPUAS::PRIVATE_ADDRESS) {
    unsigned MaxPrivateBits = 8 * getSubtarget()->getMaxPrivateElementSize();
    return (MemVT.getSizeInBits() <= MaxPrivateBits);
  } else if (AS == RVGPUAS::LOCAL_ADDRESS || AS == RVGPUAS::REGION_ADDRESS) {
    return (MemVT.getSizeInBits() <= 2 * 32);
  }
  return true;
}

bool RVTargetLowering::allowsMisalignedMemoryAccessesImpl(
    unsigned Size, unsigned AddrSpace, Align Alignment,
    MachineMemOperand::Flags Flags, unsigned *IsFast) const {
  if (IsFast)
    *IsFast = 0;

  if (AddrSpace == RVGPUAS::LOCAL_ADDRESS ||
  ...
  if (AddrSpace == RVGPUAS::PRIVATE_ADDRESS) {
  ...
  if (AddrSpace == RVGPUAS::FLAT_ADDRESS &&
      !Subtarget->hasUnalignedScratchAccess()) {
    bool AlignedBy4 = Alignment >= Align(4);
    if (IsFast)
      *IsFast = AlignedBy4;
  ...
  if (RVGPU::isExtendedGlobalAddrSpace(AddrSpace)) {
    if (IsFast)
      *IsFast = Size;
...
bool RVTargetLowering::allowsMisalignedMemoryAccesses(
...
EVT RVTargetLowering::getOptimalMemOpType(
...
bool RVTargetLowering::isMemOpHasNoClobberedMemOperand(const SDNode *N) const {
  const MemSDNode *MemNode = cast<MemSDNode>(N);
  return MemNode->getMemOperand()->getFlags() & MONoClobber;
}

bool RVTargetLowering::isNonGlobalAddrSpace(unsigned AS) {
  return AS == RVGPUAS::LOCAL_ADDRESS || AS == RVGPUAS::REGION_ADDRESS ||
         AS == RVGPUAS::PRIVATE_ADDRESS;
}

bool RVTargetLowering::isFreeAddrSpaceCast(unsigned SrcAS,
...
bool RVTargetLowering::isMemOpUniform(const SDNode *N) const {
  const MemSDNode *MemNode = cast<MemSDNode>(N);

  return RVGPUInstrInfo::isUniformMMO(MemNode->getMemOperand());
}

TargetLoweringBase::LegalizeTypeAction
RVTargetLowering::getPreferredVectorAction(MVT VT) const {
...
bool RVTargetLowering::shouldConvertConstantLoadToIntImm(const APInt &Imm,
                                                         Type *Ty) const {
  // FIXME: Could be smarter if called for vector constants.
  return true;
}

bool RVTargetLowering::isExtractSubvectorCheap(EVT ResVT, EVT SrcVT,
...
bool RVTargetLowering::isTypeDesirableForOp(unsigned Op, EVT VT) const {
...
SDValue RVTargetLowering::lowerKernArgParameterPtr(SelectionDAG &DAG,
                                                   const SDLoc &SL,
                                                   SDValue Chain,
  ...
  std::tie(InputPtrReg, RC, ArgTy) =
      Info->getPreloadedValue(RVGPUFunctionArgInfo::KERNARG_SEGMENT_PTR);
...
SDValue RVTargetLowering::getImplicitArgPtr(SelectionDAG &DAG,
...
SDValue RVTargetLowering::getLDSKernelId(SelectionDAG &DAG,
                                         const SDLoc &SL) const {

  Function &F = DAG.getMachineFunction().getFunction();
  std::optional<uint32_t> KnownSize =
      RVGPUMachineFunction::getLDSKernelIdMetadata(F);
  if (KnownSize.has_value())
    return DAG.getConstant(*KnownSize, SL, MVT::i32);
  return SDValue();
}

SDValue RVTargetLowering::convertArgType(SelectionDAG &DAG, EVT VT, EVT MemVT,
...
SDValue RVTargetLowering::lowerStackParameter(SelectionDAG &DAG, CCValAssign &VA,
...
SDValue RVTargetLowering::getPreloadedValue(SelectionDAG &DAG,
  ...
  if (!Reg) {
    if (PVID == RVGPUFunctionArgInfo::PreloadedValue::KERNARG_SEGMENT_PTR) {
    ...
    // It's undefined behavior if a function marked with the rvgpu-no-*
...
static void processPSInputArgs(SmallVectorImpl<ISD::InputArg> &Splits,
  ...
  for (unsigned I = 0, E = Ins.size(), PSInputNum = 0; I != E; ++I) {
    ...
    if (CallConv == CallingConv::RVGPU_PS &&
...
void RVTargetLowering::allocateSpecialEntryInputVGPRs(CCState &CCInfo,
                                                      MachineFunction &MF,
...
static void allocateSGPR64Input(CCState &CCInfo, RvArgDescriptor &Arg) {
  if (Arg) {
    ...
    Arg = allocateSGPR32InputImpl(CCInfo, &RVGPU::SGPR_64RegClass, 16);
}

/// Allocate implicit function VGPR arguments at the end of allocated user
/// arguments.
void RVTargetLowering::allocateSpecialInputVGPRs(
...
void RVTargetLowering::allocateSpecialInputVGPRsFixed(
  CCState &CCInfo, MachineFunction &MF,
  const RVRegisterInfo &TRI, RVMachineFunctionInfo &Info) const {
  Register Reg = CCInfo.AllocateReg(RVGPU::VGPR31);
  if (!Reg)
...
void RVTargetLowering::allocateHSAUserSGPRs(CCState &CCInfo,
                                            MachineFunction &MF,
                                            const RVRegisterInfo &TRI,
                                            RVMachineFunctionInfo &Info) const {
  const RVUserSGPRUsageInfo &UserSGPRInfo = Info.getUserSGPRInfo();
  ...
  if (UserSGPRInfo.hasDispatchPtr()) {
    ...
    MF.addLiveIn(DispatchPtrReg, &RVGPU::SGPR_64RegClass);
  ...
  if (UserSGPRInfo.hasKernargSegmentPtr()) {
    ...
    Register VReg = MF.addLiveIn(InputPtrReg, &RVGPU::SGPR_64RegClass);
    MRI.setType(VReg, LLT::pointer(RVGPUAS::CONSTANT_ADDRESS, 64));
  }

  if (UserSGPRInfo.hasDispatchID()) {
    Register DispatchIDReg = Info.addDispatchID(TRI);
    MF.addLiveIn(DispatchIDReg, &RVGPU::SGPR_64RegClass);
    CCInfo.AllocateReg(DispatchIDReg);
  }

  if (UserSGPRInfo.hasFlatScratchInit() && !getSubtarget()->isRvPalOS()) {
    Register FlatScratchInitReg = Info.addFlatScratchInit(TRI);
    MF.addLiveIn(FlatScratchInitReg, &RVGPU::SGPR_64RegClass);
...
void RVTargetLowering::allocatePreloadKernArgSGPRs(
...
void RVTargetLowering::allocateLDSKernelId(CCState &CCInfo, MachineFunction &MF,
                                           const RVRegisterInfo &TRI,
                                           RVMachineFunctionInfo &Info) const {
  // Always allocate this last since it is a synthetic preload.
  if (Info.hasLDSKernelId()) {
    Register Reg = Info.addLDSKernelId();
    MF.addLiveIn(Reg, &RVGPU::SGPR_32RegClass);
    CCInfo.AllocateReg(Reg);
  }
}

// Allocate special input registers that are initialized per-wave.
void RVTargetLowering::allocateSystemSGPRs(CCState &CCInfo,
                                           MachineFunction &MF,
                                           RVMachineFunctionInfo &Info,
                                           CallingConv::ID CallConv,
                                           bool IsShader) const {
  ...
  if (Info.hasPrivateSegmentWaveByteOffset()) {
    ...
    if (IsShader) {
      ...
      // location.
      if (PrivateSegmentWaveByteOffsetReg == RVGPU::NoRegister) {
    ...
    MF.addLiveIn(PrivateSegmentWaveByteOffsetReg, &RVGPU::SGPR_32RegClass);
...
static void reservePrivateMemoryRegs(const TargetMachine &TM,
  ...
  if (!ST.enableFlatScratch()) {
    if (RequiresStackAccess && ST.isRvHsaOrMesa(MF.getFunction())) {
      // If we have stack objects, we unquestionably need the private buffer
      // resource. For the Code Object V2 ABI, this will be the first 4 user
      // SGPR inputs. We can reserve those and use them directly.

      Register PrivateSegmentBufferReg =
          Info.getPreloadedReg(RVGPUFunctionArgInfo::PRIVATE_SEGMENT_BUFFER);
  ...
  // registers.
  if (!MRI.isLiveIn(RVGPU::SGPR32)) {
    Info.setStackPtrOffsetReg(RVGPU::SGPR32);
  } else {
    assert(RVGPU::isShader(MF.getFunction().getCallingConv()));

    if (MFI.hasCalls())
      report_fatal_error("call in graphics shader with too many input SGPRs");

    for (unsigned Reg : RVGPU::SGPR_32RegClass) {
      if (!MRI.isLiveIn(Reg)) {
    ...
    if (Info.getStackPtrOffsetReg() == RVGPU::SP_REG)
  ...
  if (ST.getFrameLowering()->hasFP(MF)) {
    Info.setFrameOffsetReg(RVGPU::SGPR33);
  }
}

bool RVTargetLowering::supportSplitCSR(MachineFunction *MF) const {
  const RVMachineFunctionInfo *Info = MF->getInfo<RVMachineFunctionInfo>();
  return !Info->isEntryFunction();
}

void RVTargetLowering::initializeSplitCSR(MachineBasicBlock *Entry) const {

}

void RVTargetLowering::insertCopiesSplitCSR(
  MachineBasicBlock *Entry,
  const SmallVectorImpl<MachineBasicBlock *> &Exits) const {
  const RVRegisterInfo *TRI = getSubtarget()->getRegisterInfo();
  ...
  for (const MCPhysReg *I = IStart; *I; ++I) {
    ...
    if (RVGPU::SReg_64RegClass.contains(*I))
      RC = &RVGPU::SGPR_64RegClass;
    else if (RVGPU::SReg_32RegClass.contains(*I))
      RC = &RVGPU::SGPR_32RegClass;
    else
...
SDValue RVTargetLowering::LowerFormalArguments(
  ...
  if (Subtarget->isRvHsaOS() && RVGPU::isGraphics(CallConv)) {
  ...
  if (IsGraphics) {
    ...
    if (CallConv != CallingConv::RVGPU_CS || !Subtarget->hasArchitectedSGPRs())
  ...
  if (CallConv == CallingConv::RVGPU_PS) {
    ...
    if ((Info->getPSInputAddr() & 0x7F) == 0 ||
      ...
      CCInfo.AllocateReg(RVGPU::VGPR0);
      CCInfo.AllocateReg(RVGPU::VGPR1);
      Info->markPSInputAllocated(0);
      Info->markPSInputEnabled(0);
    }
    if (Subtarget->isRvPalOS()) {
      // For isRvPalOS, the user does not enable some bits after compilation
  ...
  if (!IsKernel) {
    ...
    if (!IsGraphics && !Subtarget->enableFlatScratch()) {
      CCInfo.AllocateRegBlock(ArrayRef<MCPhysReg>{RVGPU::SGPR0, RVGPU::SGPR1,
                                                  RVGPU::SGPR2, RVGPU::SGPR3},
                              4);
    }
  ...
  for (unsigned i = 0, e = Ins.size(), ArgIdx = 0; i != e; ++i) {
    ...
    if (IsEntryFunc && VA.isMemLoc()) {
      ...
      if (Arg.isOrigArg() && Info->getArgInfo().PreloadKernArgs.count(i)) {
        if (MemVT.getStoreSize() < 4 && Alignment < 4) {
          ...
          const RVMachineFunctionInfo *Info =
              MF.getInfo<RVMachineFunctionInfo>();
          MachineRegisterInfo &MRI = DAG.getMachineFunction().getRegInfo();
          Register Reg =
      ...
      if (Subtarget->getGeneration() == RVGPUSubtarget::SOUTHERN_ISLANDS &&
          ParamTy && (ParamTy->getAddressSpace() == RVGPUAS::LOCAL_ADDRESS ||
                      ParamTy->getAddressSpace() == RVGPUAS::REGION_ADDRESS)) {
    ...
    const TargetRegisterClass *RC = nullptr;
    if (RVGPU::VGPR_32RegClass.contains(Reg))
      RC = &RVGPU::VGPR_32RegClass;
    else if (RVGPU::SGPR_32RegClass.contains(Reg))
      RC = &RVGPU::SGPR_32RegClass;
    else
  ...
  auto &ArgUsageInfo =
    DAG.getPass()->getAnalysis<RVGPUArgumentUsageInfo>();
...
bool RVTargetLowering::CanLowerReturn(
  CallingConv::ID CallConv,
  ...
  if (RVGPU::isEntryFunctionCC(CallConv))
    return true;
  ...
  unsigned TotalNumVGPRs = RVGPU::VGPR_32RegClass.getNumRegs();
  for (unsigned i = MaxNumVGPRs; i < TotalNumVGPRs; ++i)
    if (CCInfo.isAllocated(RVGPU::VGPR_32RegClass.getRegister(i)))
      return false;
...
SDValue
RVTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                              bool isVarArg,
                              const SmallVectorImpl<ISD::OutputArg> &Outs,
  ...
  if (RVGPU::isKernel(CallConv)) {
    return RVGPUTargetLowering::LowerReturn(Chain, CallConv, isVarArg, Outs,
                                             OutVals, DL, DAG);
  }

  bool IsShader = RVGPU::isShader(CallConv);
  ...
  unsigned Opc = RVGPUISD::ENDPGM;
  if (!IsWaveEnd)
    Opc = IsShader ? RVGPUISD::RETURN_TO_EPILOG : RVGPUISD::RET_GLUE;
  return DAG.getNode(Opc, DL, MVT::Other, RetOps);
}

SDValue RVTargetLowering::LowerCallResult(
...
void RVTargetLowering::passSpecialInputs(
  ...
  const RVRegisterInfo *TRI = Subtarget->getRegisterInfo();
  const RVGPUFunctionArgInfo &CallerArgInfo = Info.getArgInfo();

  const RVGPUFunctionArgInfo *CalleeArgInfo
    = &RVGPUArgumentUsageInfo::FixedABIFunctionInfo;
  ...
  static constexpr std::pair<RVGPUFunctionArgInfo::PreloadedValue,
                             StringLiteral> ImplicitAttrs[] = {
    {RVGPUFunctionArgInfo::DISPATCH_PTR, "rvgpu-no-dispatch-ptr"},
    {RVGPUFunctionArgInfo::QUEUE_PTR, "rvgpu-no-queue-ptr" },
    {RVGPUFunctionArgInfo::IMPLICIT_ARG_PTR, "rvgpu-no-implicitarg-ptr"},
    {RVGPUFunctionArgInfo::DISPATCH_ID, "rvgpu-no-dispatch-id"},
    {RVGPUFunctionArgInfo::WORKGROUP_ID_X, "rvgpu-no-workgroup-id-x"},
    {RVGPUFunctionArgInfo::WORKGROUP_ID_Y,"rvgpu-no-workgroup-id-y"},
    {RVGPUFunctionArgInfo::WORKGROUP_ID_Z,"rvgpu-no-workgroup-id-z"},
    {RVGPUFunctionArgInfo::LDS_KERNEL_ID,"rvgpu-no-lds-kernel-id"},
  };
  ...
  for (auto Attr : ImplicitAttrs) {
    ...
    RVGPUFunctionArgInfo::PreloadedValue InputID = Attr.first;
    ...
    } else if (InputID == RVGPUFunctionArgInfo::IMPLICIT_ARG_PTR) {
    ...
    } else if (InputID == RVGPUFunctionArgInfo::LDS_KERNEL_ID) {
  ...
  std::tie(OutgoingArg, ArgRC, Ty) =
      CalleeArgInfo->getPreloadedValue(RVGPUFunctionArgInfo::WORKITEM_ID_X);
  if (!OutgoingArg)
    std::tie(OutgoingArg, ArgRC, Ty) =
        CalleeArgInfo->getPreloadedValue(RVGPUFunctionArgInfo::WORKITEM_ID_Y);
  if (!OutgoingArg)
    std::tie(OutgoingArg, ArgRC, Ty) =
        CalleeArgInfo->getPreloadedValue(RVGPUFunctionArgInfo::WORKITEM_ID_Z);
  if (!OutgoingArg)
    return;
  ...
  const bool NeedWorkItemIDX = !CLI.CB->hasFnAttr("rvgpu-no-workitem-id-x");
  const bool NeedWorkItemIDY = !CLI.CB->hasFnAttr("rvgpu-no-workitem-id-y");
  const bool NeedWorkItemIDZ = !CLI.CB->hasFnAttr("rvgpu-no-workitem-id-z");
...
bool RVTargetLowering::isEligibleForTailCallOptimization(
    SDValue Callee, CallingConv::ID CalleeCC, bool IsVarArg,
    const SmallVectorImpl<ISD::OutputArg> &Outs,
    const SmallVectorImpl<SDValue> &OutVals,
    const SmallVectorImpl<ISD::InputArg> &Ins, SelectionDAG &DAG) const {
  if (RVGPU::isChainCC(CalleeCC))
    return true;
  ...
  const RVMachineFunctionInfo *FuncInfo = MF.getInfo<RVMachineFunctionInfo>();
...
bool RVTargetLowering::mayBeEmittedAsTailCall(const CallInst *CI) const {
  if (!CI->isTailCall())
    return false;

  const Function *ParentFn = CI->getParent()->getParent();
  if (RVGPU::isEntryFunctionCC(ParentFn->getCallingConv()))
    return false;
  return true;
}

// The wave scratch offset register is used as the global base pointer.
SDValue RVTargetLowering::LowerCall(CallLoweringInfo &CLI,
                                    SmallVectorImpl<SDValue> &InVals) const {
  CallingConv::ID CallConv = CLI.CallConv;
  bool IsChainCallConv = RVGPU::isChainCC(CallConv);
  ...
  if (IsTailCall) {
    ...
    if (!IsTailCall &&
      ...
      report_fatal_error("failed to perform tail call elimination on a call "
                         "site marked musttail or on llvm.rvgpu.cs.chain");
    }
  ...
  if (CallConv != CallingConv::RVGPU_Gfx && !RVGPU::isChainCC(CallConv)) {
  ...
  if (!IsSibCall || IsChainCallConv) {
    if (!Subtarget->enableFlatScratch()) {
      ...
      RegsToPass.emplace_back(IsChainCallConv
                                  ? RVGPU::SGPR48_SGPR49_SGPR50_SGPR51
                                  : RVGPU::SGPR0_SGPR1_SGPR2_SGPR3,
                              ScratchRSrcReg);
      ...
      if (Outs[i].Flags.isByVal()) {
        ...
        SDValue Cpy =
            DAG.getMemcpy(Chain, DL, DstAddr, Arg, SizeNode,
                          ...
                          MachinePointerInfo(RVGPUAS::PRIVATE_ADDRESS));
  ...
  auto *TRI = static_cast<const RVRegisterInfo *>(Subtarget->getRegisterInfo());
  ...
  if (IsTailCall) {
    MFI.setHasTailCall();
    unsigned OPC = RVGPUISD::TC_RETURN;
    switch (CallConv) {
    case CallingConv::RVGPU_Gfx:
      OPC = RVGPUISD::TC_RETURN_GFX;
      break;
    case CallingConv::RVGPU_CS_Chain:
    case CallingConv::RVGPU_CS_ChainPreserve:
      OPC = RVGPUISD::TC_RETURN_CHAIN;
      break;
    }
  ...
  SDValue Call = DAG.getNode(RVGPUISD::CALL, DL, NodeTys, Ops);
...
SDValue RVTargetLowering::lowerDYNAMIC_STACKALLOCImpl(
...
SDValue RVTargetLowering::LowerDYNAMIC_STACKALLOC(SDValue Op,
  ...
  return RVGPUTargetLowering::LowerDYNAMIC_STACKALLOC(Op, DAG);
}

SDValue RVTargetLowering::LowerSTACKSAVE(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue VectorAddress =
      DAG.getNode(RVGPUISD::WAVE_ADDRESS, SL, MVT::i32, CopyFromSP);
  return DAG.getMergeValues({VectorAddress, CopyFromSP.getValue(1)}, SL);
}

SDValue RVTargetLowering::lowerGET_ROUNDING(SDValue Op,
                                            SelectionDAG &DAG) const {
  SDLoc SL(Op);
  assert(Op.getValueType() == MVT::i32);

  uint32_t BothRoundHwReg =
      RVGPU::Hwreg::encodeHwreg(RVGPU::Hwreg::ID_MODE, 0, 4);
  SDValue GetRoundBothImm = DAG.getTargetConstant(BothRoundHwReg, SL, MVT::i32);

  SDValue IntrinID =
      DAG.getTargetConstant(Intrinsic::rvgpu_s_getreg, SL, MVT::i32);
  ...
  SDValue BitTable =
      DAG.getConstant(RVGPU::FltRoundConversionTable, SL, MVT::i64);
...
SDValue RVTargetLowering::lowerPREFETCH(SDValue Op, SelectionDAG &DAG) const {
  if (Op->isDivergent())
    return SDValue();

  switch (cast<MemSDNode>(Op)->getAddressSpace()) {
  case RVGPUAS::FLAT_ADDRESS:
  case RVGPUAS::GLOBAL_ADDRESS:
  case RVGPUAS::CONSTANT_ADDRESS:
  case RVGPUAS::CONSTANT_ADDRESS_32BIT:
    break;
  default:
    return SDValue();
  }
...
Register RVTargetLowering::getRegisterByName(const char* RegName, LLT VT,
                                             const MachineFunction &MF) const {
  Register Reg = StringSwitch<Register>(RegName)
    ...
    .Case("flat_scratch", RVGPU::FLAT_SCR)
  ...
  if (!Subtarget->hasFlatScrRegister() &&
       Subtarget->getRegisterInfo()->regsOverlap(Reg, RVGPU::FLAT_SCR)) {
...
// proper terminator.
MachineBasicBlock *
RVTargetLowering::splitKillBlock(MachineInstr &MI,
                                 MachineBasicBlock *BB) const {
  MachineBasicBlock *SplitBB = BB->splitAt(MI, false /*UpdateLiveIns*/);
  const RVInstrInfo *TII = getSubtarget()->getInstrInfo();
...
void RVTargetLowering::bundleInstWithWaitcnt(MachineInstr &MI) const {
  MachineBasicBlock *MBB = MI.getParent();
  const RVInstrInfo *TII = getSubtarget()->getInstrInfo();
  auto I = MI.getIterator();
  auto E = std::next(I);

  BuildMI(*MBB, E, MI.getDebugLoc(), TII->get(RVGPU::S_WAITCNT))
    .addImm(0);
...
MachineBasicBlock *
RVTargetLowering::emitGWSMemViolTestLoop(MachineInstr &MI,
                                         MachineBasicBlock *BB) const {
  ...
  if (MachineOperand *Src = TII->getNamedOperand(MI, RVGPU::OpName::data0))
    Src->setIsKill(false);
  ...
  const unsigned EncodedReg = RVGPU::Hwreg::encodeHwreg(
    RVGPU::Hwreg::ID_TRAPSTS, RVGPU::Hwreg::OFFSET_MEM_VIOL, 1);

  // Clear TRAP_STS.MEM_VIOL
  BuildMI(*LoopBB, LoopBB->begin(), DL, TII->get(RVGPU::S_SETREG_IMM32_B32))
    .addImm(0)
    .addImm(EncodedReg);
  ...
  BuildMI(*LoopBB, I, DL, TII->get(RVGPU::S_GETREG_B32), Reg)
    .addImm(EncodedReg);

  // FIXME: Do we need to use an isel pseudo that may clobber scc?
  BuildMI(*LoopBB, I, DL, TII->get(RVGPU::S_CMP_LG_U32))
    .addReg(Reg, RegState::Kill)
    .addImm(0);
  BuildMI(*LoopBB, I, DL, TII->get(RVGPU::S_CBRANCH_SCC1))
    .addMBB(LoopBB);
...
emitLoadM0FromVGPRLoop(const RVInstrInfo *TII, MachineRegisterInfo &MRI,
  ...
  Register CurrentIdxReg = MRI.createVirtualRegister(&RVGPU::SGPR_32RegClass);
  ...
  BuildMI(LoopBB, I, DL, TII->get(RVGPU::V_READFIRSTLANE_B32), CurrentIdxReg)
      .addReg(Idx.getReg(), getUndefRegState(Idx.isUndef()));

  // Compare the just read M0 value to all possible Idx values.
  BuildMI(LoopBB, I, DL, TII->get(RVGPU::V_CMP_EQ_U32_e64), CondReg)
      .addReg(CurrentIdxReg)
      .addReg(Idx.getReg(), 0, Idx.getSubReg());

  // Update EXEC, save the original EXEC value to VCC.
  BuildMI(LoopBB, I, DL, TII->get(ST.isWave32() ? RVGPU::S_AND_SAVEEXEC_B32
                                                : RVGPU::S_AND_SAVEEXEC_B64),
          NewExec)
  ...
  if (UseGPRIdxMode) {
    if (Offset == 0) {
      ...
      SGPRIdxReg = MRI.createVirtualRegister(&RVGPU::SGPR_32RegClass);
      BuildMI(LoopBB, I, DL, TII->get(RVGPU::S_ADD_I32), SGPRIdxReg)
    ...
    if (Offset == 0) {
      BuildMI(LoopBB, I, DL, TII->get(RVGPU::S_MOV_B32), RVGPU::M0)
        .addReg(CurrentIdxReg, RegState::Kill);
    } else {
      BuildMI(LoopBB, I, DL, TII->get(RVGPU::S_ADD_I32), RVGPU::M0)
        .addReg(CurrentIdxReg, RegState::Kill)
        .addImm(Offset);
    }
  }
  ...
  MachineInstr *InsertPt =
    BuildMI(LoopBB, I, DL, TII->get(ST.isWave32() ? RVGPU::S_XOR_B32_term
                                                  : RVGPU::S_XOR_B64_term), Exec)
      .addReg(Exec)
      .addReg(NewExec);
  ...
  BuildMI(LoopBB, I, DL, TII->get(RVGPU::S_CBRANCH_EXECNZ))
    .addMBB(&LoopBB);
...
static MachineBasicBlock::iterator
loadM0FromVGPR(const RVInstrInfo *TII, MachineBasicBlock &MBB, MachineInstr &MI,
  ...
  const MachineOperand *Idx = TII->getNamedOperand(MI, RVGPU::OpName::idx);
...
computeIndirectRegAndOffset(const RVRegisterInfo &TRI,
  ...
  if (Offset >= NumElts || Offset < 0)
    return std::pair(RVGPU::sub0, Offset);

  return std::pair(RVRegisterInfo::getSubRegFromChannel(Offset), 0);
}

static void setM0ToIndexFromSGPR(const RVInstrInfo *TII,
  ...
  assert(Idx->getReg() != RVGPU::NoRegister);

  if (Offset == 0) {
    BuildMI(*MBB, I, DL, TII->get(RVGPU::S_MOV_B32), RVGPU::M0).add(*Idx);
  } else {
    BuildMI(*MBB, I, DL, TII->get(RVGPU::S_ADD_I32), RVGPU::M0)
        .add(*Idx)
        .addImm(Offset);
  }
}

static Register getIndirectSGPRIdx(const RVInstrInfo *TII,
                                   MachineRegisterInfo &MRI, MachineInstr &MI,
                                   int Offset) {
  MachineBasicBlock *MBB = MI.getParent();
  const DebugLoc &DL = MI.getDebugLoc();
  ...
  Register Tmp = MRI.createVirtualRegister(&RVGPU::SReg_32_XM0RegClass);
  BuildMI(*MBB, I, DL, TII->get(RVGPU::S_ADD_I32), Tmp)
      .add(*Idx)
      .addImm(Offset);
  return Tmp;
}
...
static MachineBasicBlock *emitIndirectSrc(MachineInstr &MI,
  ...
  const MachineOperand *Idx = TII->getNamedOperand(MI, RVGPU::OpName::idx);
  Register SrcReg = TII->getNamedOperand(MI, RVGPU::OpName::src)->getReg();
  int Offset = TII->getNamedOperand(MI, RVGPU::OpName::offset)->getImm();
  ...
  if (TII->getRegisterInfo().isSGPRClass(IdxRC)) {
    ...
    if (UseGPRIdxMode) {
      ...
      BuildMI(MBB, I, DL, TII->get(RVGPU::V_MOVRELS_B32_e32), Dst)
  ...
  Register PhiReg = MRI.createVirtualRegister(&RVGPU::VGPR_32RegClass);
  Register InitReg = MRI.createVirtualRegister(&RVGPU::VGPR_32RegClass);
  ...
  if (UseGPRIdxMode) {
    ...
    BuildMI(*LoopBB, InsPt, DL, TII->get(RVGPU::V_MOVRELS_B32_e32), Dst)
      .addReg(SrcReg, 0, SubReg)
      .addReg(SrcReg, RegState::Implicit);
  }
...
static MachineBasicBlock *emitIndirectDst(MachineInstr &MI,
  ...
  Register Dst = MI.getOperand(0).getReg();
  const MachineOperand *SrcVec = TII->getNamedOperand(MI, RVGPU::OpName::src);
  const MachineOperand *Idx = TII->getNamedOperand(MI, RVGPU::OpName::idx);
  const MachineOperand *Val = TII->getNamedOperand(MI, RVGPU::OpName::val);
  int Offset = TII->getNamedOperand(MI, RVGPU::OpName::offset)->getImm();
  ...
  if (Idx->getReg() == RVGPU::NoRegister) {
    MachineBasicBlock::iterator I(&MI);
    const DebugLoc &DL = MI.getDebugLoc();
...
static MachineBasicBlock *lowerWaveReduce(MachineInstr &MI,
  ...
  if (isSGPR) {
    ...
    BuildMI(BB, MI, DL, TII->get(RVGPU::S_MOV_B32), DstReg).addReg(SrcReg);
    RetBB = &BB;
  } else {
    ...
    unsigned ExecReg = IsWave32 ? RVGPU::EXEC_LO : RVGPU::EXEC;
    ...
    uint32_t InitalValue =
        (Opc == RVGPU::S_MIN_U32) ? std::numeric_limits<uint32_t>::max() : 0;
    auto TmpSReg =
        BuildMI(BB, I, DL, TII->get(MovOpc), LoopIterator).addReg(ExecReg);
    BuildMI(BB, I, DL, TII->get(RVGPU::S_MOV_B32), InitalValReg)
        .addImm(InitalValue);
    BuildMI(BB, I, DL, TII->get(RVGPU::S_BRANCH)).addMBB(ComputeLoop);

    // Start constructing ComputeLoop
    I = ComputeLoop->end();
    auto Accumulator =
        BuildMI(*ComputeLoop, I, DL, TII->get(RVGPU::PHI), AccumulatorReg)
            .addReg(InitalValReg)
            .addMBB(&BB);
    auto ActiveBits =
        BuildMI(*ComputeLoop, I, DL, TII->get(RVGPU::PHI), ActiveBitsReg)
            .addReg(TmpSReg->getOperand(0).getReg())
            .addMBB(&BB);

    // Perform the computations
    unsigned SFFOpc = IsWave32 ? RVGPU::S_FF1_I32_B32 : RVGPU::S_FF1_I32_B64;
    auto FF1 = BuildMI(*ComputeLoop, I, DL, TII->get(SFFOpc), FF1Reg)
                   .addReg(ActiveBits->getOperand(0).getReg());
    auto LaneValue = BuildMI(*ComputeLoop, I, DL,
                             TII->get(RVGPU::V_READLANE_B32), LaneValueReg)
                         .addReg(SrcReg)
    ...
    unsigned BITSETOpc =
        IsWave32 ? RVGPU::S_BITSET0_B32 : RVGPU::S_BITSET0_B64;
    auto NewActiveBits =
    ...
    // Creating branching
    unsigned CMPOpc = IsWave32 ? RVGPU::S_CMP_LG_U32 : RVGPU::S_CMP_LG_U64;
    BuildMI(*ComputeLoop, I, DL, TII->get(CMPOpc))
        .addReg(NewActiveBits->getOperand(0).getReg())
        .addImm(0);
    BuildMI(*ComputeLoop, I, DL, TII->get(RVGPU::S_CBRANCH_SCC1))
        .addMBB(ComputeLoop);
...
MachineBasicBlock *RVTargetLowering::EmitInstrWithCustomInserter(
  ...
  switch (MI.getOpcode()) {
  case RVGPU::WAVE_REDUCE_UMIN_PSEUDO_U32:
    return lowerWaveReduce(MI, *BB, *getSubtarget(), RVGPU::S_MIN_U32);
  case RVGPU::WAVE_REDUCE_UMAX_PSEUDO_U32:
    return lowerWaveReduce(MI, *BB, *getSubtarget(), RVGPU::S_MAX_U32);
  ...
  case RVGPU::S_USUBO_PSEUDO: {
    ...
    unsigned Opc = (MI.getOpcode() == RVGPU::S_UADDO_PSEUDO)
                       ? RVGPU::S_ADD_I32
                       : RVGPU::S_SUB_I32;
    BuildMI(*BB, MI, DL, TII->get(Opc), Dest0.getReg()).add(Src0).add(Src1);

    BuildMI(*BB, MI, DL, TII->get(RVGPU::S_CSELECT_B64), Dest1.getReg())
        .addImm(1)
        .addImm(0);
  ...
  case RVGPU::S_SUB_U64_PSEUDO: {
    ...
    bool IsAdd = (MI.getOpcode() == RVGPU::S_ADD_U64_PSEUDO);
    if (Subtarget->hasScalarAddSub64()) {
      unsigned Opc = IsAdd ? RVGPU::S_ADD_U64 : RVGPU::S_SUB_U64;
      ...
      Register DestSub0 = MRI.createVirtualRegister(&RVGPU::SReg_32RegClass);
      Register DestSub1 = MRI.createVirtualRegister(&RVGPU::SReg_32RegClass);

      MachineOperand Src0Sub0 = TII->buildExtractSubRegOrImm(
          MI, MRI, Src0, BoolRC, RVGPU::sub0, &RVGPU::SReg_32RegClass);
      MachineOperand Src0Sub1 = TII->buildExtractSubRegOrImm(
          MI, MRI, Src0, BoolRC, RVGPU::sub1, &RVGPU::SReg_32RegClass);

      MachineOperand Src1Sub0 = TII->buildExtractSubRegOrImm(
          MI, MRI, Src1, BoolRC, RVGPU::sub0, &RVGPU::SReg_32RegClass);
      MachineOperand Src1Sub1 = TII->buildExtractSubRegOrImm(
          MI, MRI, Src1, BoolRC, RVGPU::sub1, &RVGPU::SReg_32RegClass);

      unsigned LoOpc = IsAdd ? RVGPU::S_ADD_U32 : RVGPU::S_SUB_U32;
      unsigned HiOpc = IsAdd ? RVGPU::S_ADDC_U32 : RVGPU::S_SUBB_U32;
  ...
  case RVGPU::V_SUB_U64_PSEUDO: {
    ...
    bool IsAdd = (MI.getOpcode() == RVGPU::V_ADD_U64_PSEUDO);
    ...
    if (IsAdd && ST.hasLshlAddB64()) {
      auto Add = BuildMI(*BB, MI, DL, TII->get(RVGPU::V_LSHL_ADD_U64_e64),
                         Dest.getReg())
                     .add(Src0)
                     .addImm(0)
                     .add(Src1);
    ...
    const auto *CarryRC = TRI->getRegClass(RVGPU::SReg_1_XEXECRegClassID);

    Register DestSub0 = MRI.createVirtualRegister(&RVGPU::VGPR_32RegClass);
    Register DestSub1 = MRI.createVirtualRegister(&RVGPU::VGPR_32RegClass);
    ...
    const TargetRegisterClass *Src0RC = Src0.isReg()
                                            ...
                                            : &RVGPU::VReg_64RegClass;
    const TargetRegisterClass *Src1RC = Src1.isReg()
                                            ? MRI.getRegClass(Src1.getReg())
                                            : &RVGPU::VReg_64RegClass;

    const TargetRegisterClass *Src0SubRC =
        TRI->getSubRegisterClass(Src0RC, RVGPU::sub0);
    const TargetRegisterClass *Src1SubRC =
        TRI->getSubRegisterClass(Src1RC, RVGPU::sub1);

    MachineOperand SrcReg0Sub0 = TII->buildExtractSubRegOrImm(
        MI, MRI, Src0, Src0RC, RVGPU::sub0, Src0SubRC);
    MachineOperand SrcReg1Sub0 = TII->buildExtractSubRegOrImm(
        MI, MRI, Src1, Src1RC, RVGPU::sub0, Src1SubRC);

    MachineOperand SrcReg0Sub1 = TII->buildExtractSubRegOrImm(
        MI, MRI, Src0, Src0RC, RVGPU::sub1, Src0SubRC);
    MachineOperand SrcReg1Sub1 = TII->buildExtractSubRegOrImm(
        MI, MRI, Src1, Src1RC, RVGPU::sub1, Src1SubRC);

    unsigned LoOpc = IsAdd ? RVGPU::V_ADD_CO_U32_e64 : RVGPU::V_SUB_CO_U32_e64;
    ...
    unsigned HiOpc = IsAdd ? RVGPU::V_ADDC_U32_e64 : RVGPU::V_SUBB_U32_e64;
    MachineInstr *HiHalf =
        BuildMI(*BB, MI, DL, TII->get(HiOpc), DestSub1)
  ...
  case RVGPU::S_SUB_CO_PSEUDO: {
    ...
    unsigned Opc = (MI.getOpcode() == RVGPU::S_ADD_CO_PSEUDO)
                       ? RVGPU::S_ADDC_U32
                       : RVGPU::S_SUBB_U32;
    if (Src0.isReg() && TRI->isVectorRegister(MRI, Src0.getReg())) {
      Register RegOp0 = MRI.createVirtualRegister(&RVGPU::SReg_32RegClass);
      BuildMI(*BB, MII, DL, TII->get(RVGPU::V_READFIRSTLANE_B32), RegOp0)
          .addReg(Src0.getReg());
      Src0.setReg(RegOp0);
    }
    if (Src1.isReg() && TRI->isVectorRegister(MRI, Src1.getReg())) {
      Register RegOp1 = MRI.createVirtualRegister(&RVGPU::SReg_32RegClass);
      BuildMI(*BB, MII, DL, TII->get(RVGPU::V_READFIRSTLANE_B32), RegOp1)
          .addReg(Src1.getReg());
      Src1.setReg(RegOp1);
    }
    Register RegOp2 = MRI.createVirtualRegister(&RVGPU::SReg_32RegClass);
    if (TRI->isVectorRegister(MRI, Src2.getReg())) {
      BuildMI(*BB, MII, DL, TII->get(RVGPU::V_READFIRSTLANE_B32), RegOp2)
          .addReg(Src2.getReg());
      Src2.setReg(RegOp2);
    }
    ...
    if (WaveSize == 64) {
      if (ST.hasScalarCompareEq64()) {
        BuildMI(*BB, MII, DL, TII->get(RVGPU::S_CMP_LG_U64))
            .addReg(Src2.getReg())
            .addImm(0);
      } else {
        const TargetRegisterClass *SubRC =
            TRI->getSubRegisterClass(Src2RC, RVGPU::sub0);
        MachineOperand Src2Sub0 = TII->buildExtractSubRegOrImm(
            MII, MRI, Src2, Src2RC, RVGPU::sub0, SubRC);
        MachineOperand Src2Sub1 = TII->buildExtractSubRegOrImm(
            MII, MRI, Src2, Src2RC, RVGPU::sub1, SubRC);
        Register Src2_32 = MRI.createVirtualRegister(&RVGPU::SReg_32RegClass);

        BuildMI(*BB, MII, DL, TII->get(RVGPU::S_OR_B32), Src2_32)
            .add(Src2Sub0)
            .add(Src2Sub1);

        BuildMI(*BB, MII, DL, TII->get(RVGPU::S_CMP_LG_U32))
    ...
    unsigned SelOpc =
        (WaveSize == 64) ? RVGPU::S_CSELECT_B64 : RVGPU::S_CSELECT_B32;
  ...
  case RVGPU::SI_INIT_M0: {
    BuildMI(*BB, MI.getIterator(), MI.getDebugLoc(),
            TII->get(RVGPU::S_MOV_B32), RVGPU::M0)
        .add(MI.getOperand(0));
    MI.eraseFromParent();
    return BB;
  }
  ...
  case RVGPU::SI_INDIRECT_SRC_V1:
  case RVGPU::SI_INDIRECT_SRC_V2:
  case RVGPU::SI_INDIRECT_SRC_V4:
  case RVGPU::SI_INDIRECT_SRC_V8:
  case RVGPU::SI_INDIRECT_SRC_V9:
  case RVGPU::SI_INDIRECT_SRC_V10:
  case RVGPU::SI_INDIRECT_SRC_V11:
  case RVGPU::SI_INDIRECT_SRC_V12:
  case RVGPU::SI_INDIRECT_SRC_V16:
  case RVGPU::SI_INDIRECT_SRC_V32:
    return emitIndirectSrc(MI, *BB, *getSubtarget());
  case RVGPU::SI_INDIRECT_DST_V1:
  case RVGPU::SI_INDIRECT_DST_V2:
  case RVGPU::SI_INDIRECT_DST_V4:
  case RVGPU::SI_INDIRECT_DST_V8:
  case RVGPU::SI_INDIRECT_DST_V9:
  case RVGPU::SI_INDIRECT_DST_V10:
  case RVGPU::SI_INDIRECT_DST_V11:
  case RVGPU::SI_INDIRECT_DST_V12:
  case RVGPU::SI_INDIRECT_DST_V16:
  case RVGPU::SI_INDIRECT_DST_V32:
  ...
  case RVGPU::V_CNDMASK_B64_PSEUDO: {
    ...
    Register DstLo = MRI.createVirtualRegister(&RVGPU::VGPR_32RegClass);
    Register DstHi = MRI.createVirtualRegister(&RVGPU::VGPR_32RegClass);
    const auto *CondRC = TRI->getRegClass(RVGPU::SReg_1_XEXECRegClassID);
    ...
    BuildMI(*BB, MI, DL, TII->get(RVGPU::COPY), SrcCondCopy)
      .addReg(SrcCond);
    BuildMI(*BB, MI, DL, TII->get(RVGPU::V_CNDMASK_B32_e64), DstLo)
        .addImm(0)
        .add(Src0Sub0)
        .addImm(0)
        .add(Src1Sub0)
        .addReg(SrcCondCopy);
    BuildMI(*BB, MI, DL, TII->get(RVGPU::V_CNDMASK_B32_e64), DstHi)
        .addImm(0)
        .add(Src0Sub1)
        .addImm(0)
        .add(Src1Sub1)
    ...
    BuildMI(*BB, MI, DL, TII->get(RVGPU::REG_SEQUENCE), Dst)
      .addReg(DstLo)
      .addImm(RVGPU::sub0)
      .addReg(DstHi)
      .addImm(RVGPU::sub1);
  ...
  case RVGPU::SI_BR_UNDEF: {
    const RVInstrInfo *TII = getSubtarget()->getInstrInfo();
    const DebugLoc &DL = MI.getDebugLoc();
    MachineInstr *Br = BuildMI(*BB, MI, DL, TII->get(RVGPU::S_CBRANCH_SCC1))
                           .add(MI.getOperand(0));
    Br->getOperand(1).setIsUndef(); // read undef SCC
    MI.eraseFromParent();
    return BB;
  }
  case RVGPU::ADJCALLSTACKUP:
  case RVGPU::ADJCALLSTACKDOWN: {
    const RVMachineFunctionInfo *Info = MF->getInfo<RVMachineFunctionInfo>();
  ...
  case RVGPU::SI_CALL_ISEL: {
    ...
    MIB = BuildMI(*BB, MI, DL, TII->get(RVGPU::SI_CALL), ReturnAddrReg);
  ...
  case RVGPU::V_ADD_CO_U32_e32:
  case RVGPU::V_SUB_CO_U32_e32:
  case RVGPU::V_SUBREV_CO_U32_e32: {
  ...
  case RVGPU::V_ADDC_U32_e32:
  case RVGPU::V_SUBB_U32_e32:
  case RVGPU::V_SUBBREV_U32_e32:
    // These instructions have an implicit use of vcc which counts towards the
    // constant bus limit.
    TII->legalizeOperands(MI);
    return BB;
  case RVGPU::DS_GWS_INIT:
  case RVGPU::DS_GWS_SEMA_BR:
  case RVGPU::DS_GWS_BARRIER:
    TII->enforceOperandRCAlignment(MI, RVGPU::OpName::data0);
    [[fallthrough]];
  case RVGPU::DS_GWS_SEMA_V:
  case RVGPU::DS_GWS_SEMA_P:
  case RVGPU::DS_GWS_SEMA_RELEASE_ALL:
  ...
  case RVGPU::S_SETREG_B32: {
    ...
    RVGPU::Hwreg::decodeHwreg(MI.getOperand(1).getImm(), ID, Offset, Width);
    if (ID != RVGPU::Hwreg::ID_MODE)
      return BB;
    ...
    if (getSubtarget()->hasDenormModeInst()) {
      ...
      if (SetMask ==
          (RVGPU::Hwreg::FP_ROUND_MASK | RVGPU::Hwreg::FP_DENORM_MASK)) {
        // If this fully sets both the round and denorm mode, emit the two
        // dedicated instructions for these.
        SetRoundOp = RVGPU::S_ROUND_MODE;
        SetDenormOp = RVGPU::S_DENORM_MODE;
      } else if (SetMask == RVGPU::Hwreg::FP_ROUND_MASK) {
        SetRoundOp = RVGPU::S_ROUND_MODE;
      } else if (SetMask == RVGPU::Hwreg::FP_DENORM_MASK) {
        SetDenormOp = RVGPU::S_DENORM_MODE;
      }
    ...
    if ((SetMask & (RVGPU::Hwreg::FP_ROUND_MASK |
                    RVGPU::Hwreg::FP_DENORM_MASK)) == SetMask)
      MI.setDesc(TII->get(RVGPU::S_SETREG_B32_mode));
  ...
  case RVGPU::S_INVERSE_BALLOT_U64: {
    ...
    BuildMI(*BB, &MI, DL, TII->get(RVGPU::COPY), DstReg).addReg(MaskReg);
    MI.eraseFromParent();
    return BB;
  }
  case RVGPU::ENDPGM_TRAP: {
    const DebugLoc &DL = MI.getDebugLoc();
    if (BB->succ_empty() && std::next(MI.getIterator()) == BB->end()) {
      MI.setDesc(TII->get(RVGPU::S_ENDPGM));
    ...
    MF->push_back(TrapBB);
    BuildMI(*TrapBB, TrapBB->end(), DL, TII->get(RVGPU::S_ENDPGM))
      .addImm(0);
    BuildMI(*BB, &MI, DL, TII->get(RVGPU::S_CBRANCH_EXECNZ))
      .addMBB(TrapBB);
  ...
  default:
    return RVGPUTargetLowering::EmitInstrWithCustomInserter(MI, BB);
  }
}

bool RVTargetLowering::hasAtomicFaddRtnForTy(SDValue &Op) const {
...
bool RVTargetLowering::enableAggressiveFMAFusion(EVT VT) const {
...
bool RVTargetLowering::enableAggressiveFMAFusion(LLT Ty) const { return true; }

EVT RVTargetLowering::getSetCCResultType(const DataLayout &DL, LLVMContext &Ctx,
                                         EVT VT) const {
  if (!VT.isVector()) {
    return MVT::i1;
  }
...
MVT RVTargetLowering::getScalarShiftAmountTy(const DataLayout &, EVT VT) const {
  // TODO: Should i16 be used always if legal? For now it would force VALU
  // shifts.
  return (VT == MVT::i16) ? MVT::i16 : MVT::i32;
}

LLT RVTargetLowering::getPreferredShiftAmountTy(LLT Ty) const {
...
//
bool RVTargetLowering::isFMAFasterThanFMulAndFAdd(const MachineFunction &MF,
                                                  EVT VT) const {
  VT = VT.getScalarType();
...
bool RVTargetLowering::isFMADLegal(const MachineInstr &MI, LLT Ty) const {
  if (!Ty.isScalar())
    return false;
...
bool RVTargetLowering::isFMADLegal(const SelectionDAG &DAG,
...
SDValue RVTargetLowering::splitUnaryVectorOp(SDValue Op,
                                             SelectionDAG &DAG) const {
...
// Work around LegalizeDAG doing the wrong thing and fully scalarizing if the
// wider vector type is legal.
SDValue RVTargetLowering::splitBinaryVectorOp(SDValue Op,
                                              SelectionDAG &DAG) const {
  unsigned Opc = Op.getOpcode();
  EVT VT = Op.getValueType();
  assert(VT == MVT::v4i16 || VT == MVT::v4f16 || VT == MVT::v4f32 ||
...
SDValue RVTargetLowering::splitTernaryVectorOp(SDValue Op,
                                              SelectionDAG &DAG) const {
  unsigned Opc = Op.getOpcode();
  EVT VT = Op.getValueType();
...
SDValue RVTargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
  switch (Op.getOpcode()) {
  default: return RVGPUTargetLowering::LowerOperation(Op, DAG);
  ...
  case ISD::FPTRUNC_ROUND: {
    ...
    if (RoundMode == (int)RoundingMode::TowardPositive)
      Opc = RVGPUISD::FPTRUNC_ROUND_UPWARD;
    else if (RoundMode == (int)RoundingMode::TowardNegative)
      Opc = RVGPUISD::FPTRUNC_ROUND_DOWNWARD;
    else
      return SDValue();
...
SDValue RVTargetLowering::adjustLoadValueType(unsigned Opcode,
                                              MemSDNode *M,
                                              SelectionDAG &DAG,
...
SDValue RVTargetLowering::lowerIntrinsicLoad(MemSDNode *M, bool IsFormat,
                                             SelectionDAG &DAG,
  ...
  unsigned Opc;
  if (IsFormat) {
    Opc = IsTFE ? RVGPUISD::BUFFER_LOAD_FORMAT_TFE
                : RVGPUISD::BUFFER_LOAD_FORMAT;
  } else {
    // TODO: Support non-format TFE loads.
    if (IsTFE)
      return SDValue();
    Opc = RVGPUISD::BUFFER_LOAD;
  }

  if (IsD16) {
    return adjustLoadValueType(RVGPUISD::BUFFER_LOAD_FORMAT_D16, M, DAG, Ops);
  }
...
static SDValue lowerICMPIntrinsic(const RVTargetLowering &TLI,
  ...
  SDValue SetCC = DAG.getNode(RVGPUISD::SETCC, DL, CCVT, LHS, RHS,
...
static SDValue lowerFCMPIntrinsic(const RVTargetLowering &TLI,
                                  SDNode *N, SelectionDAG &DAG) {
  EVT VT = N->getValueType(0);
  const auto *CD = cast<ConstantSDNode>(N->getOperand(3));
  ...
  unsigned WavefrontSize = TLI.getSubtarget()->getWavefrontSize();
  EVT CCVT = EVT::getIntegerVT(*DAG.getContext(), WavefrontSize);
  SDValue SetCC = DAG.getNode(RVGPUISD::SETCC, SL, CCVT, Src0,
...
static SDValue lowerBALLOTIntrinsic(const RVTargetLowering &TLI, SDNode *N,
                                    SelectionDAG &DAG) {
  EVT VT = N->getValueType(0);
  ...
  if (Src.getOpcode() == ISD::SETCC) {
    // (ballot (ISD::SETCC ...)) -> (RVGPUISD::SETCC ...)
    return DAG.getNode(RVGPUISD::SETCC, SL, VT, Src.getOperand(0),
  ...
  // (ballot (i1 $src)) -> (RVGPUISD::SETCC (i32 (zext $src)) (i32 0)
  // ISD::SETNE)
  return DAG.getNode(
      RVGPUISD::SETCC, SL, VT, DAG.getZExtOrTrunc(Src, SL, MVT::i32),
      DAG.getConstant(0, SL, MVT::i32), DAG.getCondCode(ISD::SETNE));
}

void RVTargetLowering::ReplaceNodeResults(SDNode *N,
  ...
  case ISD::INTRINSIC_WO_CHAIN: {
    ...
    switch (IID) {
    case Intrinsic::rvgpu_make_buffer_rsrc:
    ...
    case Intrinsic::rvgpu_cvt_pkrtz: {
      ...
      SDValue Src1 = N->getOperand(2);
      SDLoc SL(N);
      SDValue Cvt = DAG.getNode(RVGPUISD::CVT_PKRTZ_F16_F32, SL, MVT::i32,
                                Src0, Src1);
    ...
    case Intrinsic::rvgpu_cvt_pk_u16: {
      ...
      if (IID == Intrinsic::rvgpu_cvt_pknorm_i16)
        Opcode = RVGPUISD::CVT_PKNORM_I16_F32;
      else if (IID == Intrinsic::rvgpu_cvt_pknorm_u16)
        Opcode = RVGPUISD::CVT_PKNORM_U16_F32;
      else if (IID == Intrinsic::rvgpu_cvt_pk_i16)
        Opcode = RVGPUISD::CVT_PK_I16_I32;
      else
        Opcode = RVGPUISD::CVT_PK_U16_U32;
  ...
  default:
    RVGPUTargetLowering::ReplaceNodeResults(N, Results, DAG);
    break;
  }
}
...
unsigned RVTargetLowering::isCFIntrinsic(const SDNode *Intr) const {
  if (Intr->getOpcode() == ISD::INTRINSIC_W_CHAIN) {
    switch (cast<ConstantSDNode>(Intr->getOperand(1))->getZExtValue()) {
    case Intrinsic::rvgpu_if:
      return RVGPUISD::IF;
    case Intrinsic::rvgpu_else:
      return RVGPUISD::ELSE;
    case Intrinsic::rvgpu_loop:
      return RVGPUISD::LOOP;
...
bool RVTargetLowering::shouldEmitFixup(const GlobalValue *GV) const {
  const Triple &TT = getTargetMachine().getTargetTriple();
  return (GV->getAddressSpace() == RVGPUAS::CONSTANT_ADDRESS ||
          GV->getAddressSpace() == RVGPUAS::CONSTANT_ADDRESS_32BIT) &&
         RVGPU::shouldEmitConstantsToTextSection(TT);
}

bool RVTargetLowering::shouldEmitGOTReloc(const GlobalValue *GV) const {
  if (Subtarget->isRvPalOS() || Subtarget->isMesa3DOS())
    return false;
...
bool RVTargetLowering::shouldEmitPCReloc(const GlobalValue *GV) const {
  return !shouldEmitFixup(GV) && !shouldEmitGOTReloc(GV);
}

bool RVTargetLowering::shouldUseLDSConstAddress(const GlobalValue *GV) const {
...
SDValue RVTargetLowering::LowerBRCOND(SDValue BRCOND,
                                      SelectionDAG &DAG) const {
  SDLoc DL(BRCOND);
...
SDValue RVTargetLowering::LowerRETURNADDR(SDValue Op,
                                          SelectionDAG &DAG) const {
...
SDValue RVTargetLowering::getFPExtOrFPRound(SelectionDAG &DAG,
                                            SDValue Op,
                                            const SDLoc &DL,
                                            EVT VT) const {
...
SDValue RVTargetLowering::lowerFP_ROUND(SDValue Op, SelectionDAG &DAG) const {
...
SDValue RVTargetLowering::lowerFMINNUM_FMAXNUM(SDValue Op,
                                               SelectionDAG &DAG) const {
  EVT VT = Op.getValueType();
  const MachineFunction &MF = DAG.getMachineFunction();
  const RVMachineFunctionInfo *Info = MF.getInfo<RVMachineFunctionInfo>();
...
SDValue RVTargetLowering::lowerFLDEXP(SDValue Op, SelectionDAG &DAG) const {
...
SDValue RVTargetLowering::lowerXMULO(SDValue Op, SelectionDAG &DAG) const {
  EVT VT = Op.getValueType();
  SDLoc SL(Op);
...
SDValue RVTargetLowering::lowerXMUL_LOHI(SDValue Op, SelectionDAG &DAG) const {
  if (Op->isDivergent()) {
...
SDValue RVTargetLowering::lowerTrapEndpgm(
    SDValue Op, SelectionDAG &DAG) const {
  SDLoc SL(Op);
  SDValue Chain = Op.getOperand(0);
  return DAG.getNode(RVGPUISD::ENDPGM_TRAP, SL, MVT::Other, Chain);
}

SDValue RVTargetLowering::loadImplicitKernelArgument(SelectionDAG &DAG, MVT VT,
...
SDValue RVTargetLowering::lowerTrapHsaQueuePtr(
    SDValue Op, SelectionDAG &DAG) const {
  SDLoc SL(Op);
  SDValue Chain = Op.getOperand(0);
  ...
  if (RVGPU::getCodeObjectVersion(*M) >= RVGPU::RVHSA_COV5) {
    ...
    if (UserSGPR == RVGPU::NoRegister) {
      // We probably are in a function incorrectly marked with
      // rvgpu-no-queue-ptr. This is undefined. We don't want to delete the
      // trap, so just use a null pointer.
      QueuePtr = DAG.getConstant(0, SL, MVT::i64);
    } else {
      QueuePtr = CreateLiveInRegister(DAG, &RVGPU::SReg_64RegClass, UserSGPR,
                                      MVT::i64);
    }
  }

  SDValue SGPR01 = DAG.getRegister(RVGPU::SGPR0_SGPR1, MVT::i64);
  ...
  return DAG.getNode(RVGPUISD::TRAP, SL, MVT::Other, Ops);
}

SDValue RVTargetLowering::lowerTrapHsa(
    SDValue Op, SelectionDAG &DAG) const {
  SDLoc SL(Op);
  SDValue Chain = Op.getOperand(0);
...
SDValue RVTargetLowering::lowerDEBUGTRAP(SDValue Op, SelectionDAG &DAG) const {
  SDLoc SL(Op);
  SDValue Chain = Op.getOperand(0);
  MachineFunction &MF = DAG.getMachineFunction();
...
SDValue RVTargetLowering::getSegmentAperture(unsigned AS, const SDLoc &DL,
                                             SelectionDAG &DAG) const {
  if (Subtarget->hasApertureRegs()) {
    const unsigned ApertureRegNo = (AS == RVGPUAS::LOCAL_ADDRESS)
                                       ? RVGPU::SRC_SHARED_BASE
                                       : RVGPU::SRC_PRIVATE_BASE;
    ...
    //  happening.
    SDNode *Mov = DAG.getMachineNode(RVGPU::S_MOV_B64, DL, MVT::i64,
  ...
  if (UserSGPR == RVGPU::NoRegister) {
    // We probably are in a function incorrectly marked with
    // rvgpu-no-queue-ptr. This is undefined.
    return DAG.getUNDEF(MVT::i32);
  }

  SDValue QueuePtr = CreateLiveInRegister(
    DAG, &RVGPU::SReg_64RegClass, UserSGPR, MVT::i64);

  // Offset into rv_queue_t for group_segment_aperture_base_hi /
  // private_segment_aperture_base_hi.
  uint32_t StructOffset = (AS == RVGPUAS::LOCAL_ADDRESS) ? 0x40 : 0x44;
...
static bool isKnownNonNull(SDValue Val, SelectionDAG &DAG,
                           const RVGPUTargetMachine &TM, unsigned AddrSpace) {
...
SDValue RVTargetLowering::lowerADDRSPACECAST(SDValue Op,
                                             SelectionDAG &DAG) const {
  SDLoc SL(Op);
  ...
  const RVGPUTargetMachine &TM =
    static_cast<const RVGPUTargetMachine &>(getTargetMachine());

  // flat -> local/private
  if (SrcAS == RVGPUAS::FLAT_ADDRESS) {
    unsigned DestAS = ASC->getDestAddressSpace();

    if (DestAS == RVGPUAS::LOCAL_ADDRESS ||
        DestAS == RVGPUAS::PRIVATE_ADDRESS) {
  ...
  // local/private -> flat
  if (ASC->getDestAddressSpace() == RVGPUAS::FLAT_ADDRESS) {
    if (SrcAS == RVGPUAS::LOCAL_ADDRESS ||
        SrcAS == RVGPUAS::PRIVATE_ADDRESS) {
  ...
  if (SrcAS == RVGPUAS::CONSTANT_ADDRESS_32BIT &&
      Op.getValueType() == MVT::i64) {
    const RVMachineFunctionInfo *Info =
        DAG.getMachineFunction().getInfo<RVMachineFunctionInfo>();
  ...
  if (ASC->getDestAddressSpace() == RVGPUAS::CONSTANT_ADDRESS_32BIT &&
...
// remains.
SDValue RVTargetLowering::lowerINSERT_SUBVECTOR(SDValue Op,
                                                SelectionDAG &DAG) const {
...
SDValue RVTargetLowering::lowerINSERT_VECTOR_ELT(SDValue Op,
                                                 SelectionDAG &DAG) const {
  SDValue Vec = Op.getOperand(0);
...
SDValue RVTargetLowering::lowerEXTRACT_VECTOR_ELT(SDValue Op,
                                                  SelectionDAG &DAG) const {
  SDLoc SL(Op);
...
SDValue RVTargetLowering::lowerVECTOR_SHUFFLE(SDValue Op,
                                              SelectionDAG &DAG) const {
  SDLoc SL(Op);
  EVT ResultVT = Op.getValueType();
...
SDValue RVTargetLowering::lowerSCALAR_TO_VECTOR(SDValue Op,
                                                SelectionDAG &DAG) const {
...
SDValue RVTargetLowering::lowerBUILD_VECTOR(SDValue Op,
                                            SelectionDAG &DAG) const {
  SDLoc SL(Op);
  EVT VT = Op.getValueType();
...
bool
RVTargetLowering::isOffsetFoldingLegal(const GlobalAddressSDNode *GA) const {
  // We can fold offsets for anything that doesn't require a GOT relocation.
  return (GA->getAddressSpace() == RVGPUAS::GLOBAL_ADDRESS ||
          GA->getAddressSpace() == RVGPUAS::CONSTANT_ADDRESS ||
          GA->getAddressSpace() == RVGPUAS::CONSTANT_ADDRESS_32BIT) &&
...
buildPCRelGlobalAddress(SelectionDAG &DAG, const GlobalValue *GV,
                        ...
                        unsigned GAFlags = RVInstrInfo::MO_NONE) {
  ...
  SDValue PtrHi;
  if (GAFlags == RVInstrInfo::MO_NONE)
    PtrHi = DAG.getTargetConstant(0, DL, MVT::i32);
  else
    PtrHi = DAG.getTargetGlobalAddress(GV, DL, MVT::i32, Offset, GAFlags + 1);
  return DAG.getNode(RVGPUISD::PC_ADD_REL_OFFSET, DL, PtrVT, PtrLo, PtrHi);
}

SDValue RVTargetLowering::LowerGlobalAddress(RVGPUMachineFunction *MFI,
                                             SDValue Op,
                                             SelectionDAG &DAG) const {
  ...
  const GlobalValue *GV = GSD->getGlobal();
  if ((GSD->getAddressSpace() == RVGPUAS::LOCAL_ADDRESS &&
       shouldUseLDSConstAddress(GV)) ||
      GSD->getAddressSpace() == RVGPUAS::REGION_ADDRESS ||
      GSD->getAddressSpace() == RVGPUAS::PRIVATE_ADDRESS) {
    if (GSD->getAddressSpace() == RVGPUAS::LOCAL_ADDRESS &&
      ...
      if (DAG.getDataLayout().getTypeAllocSize(Ty).isZero()) {
        ...
        return SDValue(
            DAG.getMachineNode(RVGPU::GET_GROUPSTATICSIZE, DL, PtrVT), 0);
      }
    }
    return RVGPUTargetLowering::LowerGlobalAddress(MFI, Op, DAG);
  }

  if (GSD->getAddressSpace() == RVGPUAS::LOCAL_ADDRESS) {
    SDValue GA = DAG.getTargetGlobalAddress(GV, DL, MVT::i32, GSD->getOffset(),
                                            RVInstrInfo::MO_ABS32_LO);
    return DAG.getNode(RVGPUISD::LDS, DL, MVT::i32, GA);
  }

  if (Subtarget->isRvPalOS() || Subtarget->isMesa3DOS()) {
    SDValue AddrLo = DAG.getTargetGlobalAddress(
        GV, DL, MVT::i32, GSD->getOffset(), RVInstrInfo::MO_ABS32_LO);
    AddrLo = {DAG.getMachineNode(RVGPU::S_MOV_B32, DL, MVT::i32, AddrLo), 0};

    SDValue AddrHi = DAG.getTargetGlobalAddress(
        GV, DL, MVT::i32, GSD->getOffset(), RVInstrInfo::MO_ABS32_HI);
    AddrHi = {DAG.getMachineNode(RVGPU::S_MOV_B32, DL, MVT::i32, AddrHi), 0};
  ...
  if (shouldEmitPCReloc(GV))
    return buildPCRelGlobalAddress(DAG, GV, DL, GSD->getOffset(), PtrVT,
                                   RVInstrInfo::MO_REL32);

  SDValue GOTAddr = buildPCRelGlobalAddress(DAG, GV, DL, 0, PtrVT,
                                            RVInstrInfo::MO_GOTPCREL32);

  Type *Ty = PtrVT.getTypeForEVT(*DAG.getContext());
  PointerType *PtrTy = PointerType::get(Ty, RVGPUAS::CONSTANT_ADDRESS);
...
SDValue RVTargetLowering::copyToM0(SelectionDAG &DAG, SDValue Chain,
  ...
  SDNode *M0 = DAG.getMachineNode(RVGPU::SI_INIT_M0, DL, MVT::Other, MVT::Glue,
                                  V, Chain);
  return SDValue(M0, 0);
}

SDValue RVTargetLowering::lowerImplicitZextParam(SelectionDAG &DAG,
                                                 SDValue Op,
                                                 MVT VT,
...
SDValue RVTargetLowering::lowerImage(SDValue Op,
                                     const RVGPU::ImageDimIntrinsicInfo *Intr,
  ...
  if (BaseOpcode->Gradients && IsG16 && ST->hasG16()) {
    // Activate g16
    const RVGPU::MIMGG16MappingInfo *G16MappingInfo =
        RVGPU::getMIMGG16MappingInfo(Intr->BaseOpcode);
  ...
  if (BaseOpcode->Atomic)
    CPol |= RVGPU::CPol::GLC; // TODO no-return optimization
  if (CPol & ~(IsGFX12Plus ? RVGPU::CPol::ALL : RVGPU::CPol::ALL_pregfx12))
    return Op;
  ...
  Ops.push_back(IsA16 &&  // r128, a16 for gfx9
                ST->hasFeature(RVGPU::FeatureR128A16) ? True : False);
  if (IsGFX10Plus)
  ...
  if (IsGFX12Plus) {
    ...
    Opcode = RVGPU::getMIMGOpcode(IntrOpcode,
                                   UseNSA ? RVGPU::MIMGEncGfx10NSA
                                          : RVGPU::MIMGEncGfx10Default,
                                   NumVDataDwords, NumVAddrDwords);
  } else {
    if (Subtarget->hasGFX90AInsts()) {
      Opcode = RVGPU::getMIMGOpcode(IntrOpcode, RVGPU::MIMGEncGfx90a,
                                     NumVDataDwords, NumVAddrDwords);
      if (Opcode == -1)
        report_fatal_error(
    ...
    if (Opcode == -1 &&
        Subtarget->getGeneration() >= RVGPUSubtarget::VOLCANIC_ISLANDS)
      Opcode = RVGPU::getMIMGOpcode(IntrOpcode, RVGPU::MIMGEncGfx8,
                                     NumVDataDwords, NumVAddrDwords);
    if (Opcode == -1)
      Opcode = RVGPU::getMIMGOpcode(IntrOpcode, RVGPU::MIMGEncGfx6,
                                     NumVDataDwords, NumVAddrDwords);
  }
  if (Opcode == -1)
    return Op;
...
SDValue RVTargetLowering::lowerSBuffer(EVT VT, SDLoc DL, SDValue Rsrc,
  ...
  if (!Offset->isDivergent()) {
    ...
    if (VT.isVector() && VT.getVectorNumElements() == 3 &&
      ...
      auto WidenedOp = DAG.getMemIntrinsicNode(
          RVGPUISD::SBUFFER_LOAD, DL, DAG.getVTList(WidenedVT), Ops, WidenedVT,
    ...
    return DAG.getMemIntrinsicNode(RVGPUISD::SBUFFER_LOAD, DL,
  ...
  for (unsigned i = 0; i < NumLoads; ++i) {
    ...
    Loads.push_back(getMemIntrinsicNode(RVGPUISD::BUFFER_LOAD, DL, VTList, Ops,
                                        LoadVT, MMO, DAG));
  }
...
SDValue RVTargetLowering::lowerWorkitemID(SelectionDAG &DAG, SDValue Op,
  ...
  SDValue Val = loadInputValue(DAG, &RVGPU::VGPR_32RegClass, MVT::i32,
...
SDValue RVTargetLowering::LowerINTRINSIC_WO_CHAIN(SDValue Op,
                                                  SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  auto MFI = MF.getInfo<RVMachineFunctionInfo>();
  ...
  switch (IntrinsicID) {
  case Intrinsic::rvgpu_implicit_buffer_ptr: {
    if (getSubtarget()->isRvHsaOrMesa(MF.getFunction()))
      return emitNonHSAIntrinsicError(DAG, DL, VT);
    return getPreloadedValue(DAG, *MFI, VT,
                             RVGPUFunctionArgInfo::IMPLICIT_BUFFER_PTR);
  }
  case Intrinsic::rvgpu_dispatch_ptr:
  case Intrinsic::rvgpu_queue_ptr: {
    if (!Subtarget->isRvHsaOrMesa(MF.getFunction())) {
    ...
    auto RegID = IntrinsicID == Intrinsic::rvgpu_dispatch_ptr ?
      RVGPUFunctionArgInfo::DISPATCH_PTR : RVGPUFunctionArgInfo::QUEUE_PTR;
  ...
  case Intrinsic::rvgpu_implicitarg_ptr: {
    ...
    return getPreloadedValue(DAG, *MFI, VT,
                             RVGPUFunctionArgInfo::IMPLICIT_ARG_PTR);
  }
  case Intrinsic::rvgpu_kernarg_segment_ptr: {
    if (!RVGPU::isKernel(MF.getFunction().getCallingConv())) {
      // This only makes sense to call in a kernel, so just lower to null.
      return DAG.getConstant(0, DL, VT);
    }

    return getPreloadedValue(DAG, *MFI, VT,
                             RVGPUFunctionArgInfo::KERNARG_SEGMENT_PTR);
  }
  case Intrinsic::rvgpu_dispatch_id: {
    return getPreloadedValue(DAG, *MFI, VT, RVGPUFunctionArgInfo::DISPATCH_ID);
  }
  case Intrinsic::rvgpu_rcp:
    return DAG.getNode(RVGPUISD::RCP, DL, VT, Op.getOperand(1));
  case Intrinsic::rvgpu_rsq:
    return DAG.getNode(RVGPUISD::RSQ, DL, VT, Op.getOperand(1));
  case Intrinsic::rvgpu_rsq_legacy:
    if (Subtarget->getGeneration() >= RVGPUSubtarget::VOLCANIC_ISLANDS)
      return emitRemovedIntrinsicError(DAG, DL, VT);
    return SDValue();
  case Intrinsic::rvgpu_rcp_legacy:
    if (Subtarget->getGeneration() >= RVGPUSubtarget::VOLCANIC_ISLANDS)
      return emitRemovedIntrinsicError(DAG, DL, VT);
    return DAG.getNode(RVGPUISD::RCP_LEGACY, DL, VT, Op.getOperand(1));
  case Intrinsic::rvgpu_rsq_clamp: {
    if (Subtarget->getGeneration() < RVGPUSubtarget::VOLCANIC_ISLANDS)
      return DAG.getNode(RVGPUISD::RSQ_CLAMP, DL, VT, Op.getOperand(1));

    Type *Type = VT.getTypeForEVT(*DAG.getContext());
    APFloat Max = APFloat::getLargest(Type->getFltSemantics());
    APFloat Min = APFloat::getLargest(Type->getFltSemantics(), true);

    SDValue Rsq = DAG.getNode(RVGPUISD::RSQ, DL, VT, Op.getOperand(1));
  ...
  case Intrinsic::rvgpu_workgroup_id_x:
    return getPreloadedValue(DAG, *MFI, VT,
                             RVGPUFunctionArgInfo::WORKGROUP_ID_X);
  case Intrinsic::rvgpu_workgroup_id_y:
    return getPreloadedValue(DAG, *MFI, VT,
                             RVGPUFunctionArgInfo::WORKGROUP_ID_Y);
  case Intrinsic::rvgpu_workgroup_id_z:
    return getPreloadedValue(DAG, *MFI, VT,
                             RVGPUFunctionArgInfo::WORKGROUP_ID_Z);
  case Intrinsic::rvgpu_lds_kernel_id: {
    if (MFI->isEntryFunction())
      return getLDSKernelId(DAG, DL);
    return getPreloadedValue(DAG, *MFI, VT,
                             RVGPUFunctionArgInfo::LDS_KERNEL_ID);
  }
  ...
  case Intrinsic::rvgpu_s_buffer_load: {
    ...
    if (CPol & ~((Subtarget->getGeneration() >= RVGPUSubtarget::GFX12)
                     ? RVGPU::CPol::ALL
                     : RVGPU::CPol::ALL_pregfx12))
      return Op;
    return lowerSBuffer(VT, DL, Op.getOperand(1), Op.getOperand(2), Op.getOperand(3),
                        DAG);
  }
  case Intrinsic::rvgpu_fdiv_fast:
    return lowerFDIV_FAST(Op, DAG);
  case Intrinsic::rvgpu_sin:
    return DAG.getNode(RVGPUISD::SIN_HW, DL, VT, Op.getOperand(1));

  case Intrinsic::rvgpu_cos:
    return DAG.getNode(RVGPUISD::COS_HW, DL, VT, Op.getOperand(1));

  case Intrinsic::rvgpu_mul_u24:
    return DAG.getNode(RVGPUISD::MUL_U24, DL, VT, Op.getOperand(1), Op.getOperand(2));
  case Intrinsic::rvgpu_mul_i24:
    return DAG.getNode(RVGPUISD::MUL_I24, DL, VT, Op.getOperand(1), Op.getOperand(2));
  ...
  case Intrinsic::rvgpu_fract:
    return DAG.getNode(RVGPUISD::FRACT, DL, VT, Op.getOperand(1));

  case Intrinsic::rvgpu_class:
    return DAG.getNode(RVGPUISD::FP_CLASS, DL, VT,
                       Op.getOperand(1), Op.getOperand(2));
  case Intrinsic::rvgpu_div_fmas:
    return DAG.getNode(RVGPUISD::DIV_FMAS, DL, VT,
                       Op.getOperand(1), Op.getOperand(2), Op.getOperand(3),
                       Op.getOperand(4));

  case Intrinsic::rvgpu_div_fixup:
    return DAG.getNode(RVGPUISD::DIV_FIXUP, DL, VT,
                       Op.getOperand(1), Op.getOperand(2), Op.getOperand(3));
  ...
  case Intrinsic::rvgpu_div_scale: {
    ...
    return DAG.getNode(RVGPUISD::DIV_SCALE, DL, Op->getVTList(), Src0,
  ...
  case Intrinsic::rvgpu_fmed3:
    return DAG.getNode(RVGPUISD::FMED3, DL, VT,
                       Op.getOperand(1), Op.getOperand(2), Op.getOperand(3));
  case Intrinsic::rvgpu_fdot2:
    return DAG.getNode(RVGPUISD::FDOT2, DL, VT,
                       Op.getOperand(1), Op.getOperand(2), Op.getOperand(3),
                       Op.getOperand(4));
  case Intrinsic::rvgpu_fmul_legacy:
    return DAG.getNode(RVGPUISD::FMUL_LEGACY, DL, VT,
                       Op.getOperand(1), Op.getOperand(2));
  case Intrinsic::rvgpu_sffbh:
    return DAG.getNode(RVGPUISD::FFBH_I32, DL, VT, Op.getOperand(1));
  case Intrinsic::rvgpu_sbfe:
    return DAG.getNode(RVGPUISD::BFE_I32, DL, VT,
                       Op.getOperand(1), Op.getOperand(2), Op.getOperand(3));
  case Intrinsic::rvgpu_ubfe:
    return DAG.getNode(RVGPUISD::BFE_U32, DL, VT,
                       Op.getOperand(1), Op.getOperand(2), Op.getOperand(3));
  case Intrinsic::rvgpu_cvt_pkrtz:
  case Intrinsic::rvgpu_cvt_pknorm_i16:
  case Intrinsic::rvgpu_cvt_pknorm_u16:
  ...
  case Intrinsic::rvgpu_cvt_pk_u16: {
    ...
    if (IntrinsicID == Intrinsic::rvgpu_cvt_pkrtz)
      Opcode = RVGPUISD::CVT_PKRTZ_F16_F32;
    else if (IntrinsicID == Intrinsic::rvgpu_cvt_pknorm_i16)
      Opcode = RVGPUISD::CVT_PKNORM_I16_F32;
    else if (IntrinsicID == Intrinsic::rvgpu_cvt_pknorm_u16)
      Opcode = RVGPUISD::CVT_PKNORM_U16_F32;
    else if (IntrinsicID == Intrinsic::rvgpu_cvt_pk_i16)
      Opcode = RVGPUISD::CVT_PK_I16_I32;
    else
      Opcode = RVGPUISD::CVT_PK_U16_U32;
  ...
  case Intrinsic::rvgpu_fmad_ftz:
    return DAG.getNode(RVGPUISD::FMAD_FTZ, DL, VT, Op.getOperand(1),
                       Op.getOperand(2), Op.getOperand(3));

  case Intrinsic::rvgpu_if_break:
    return SDValue(DAG.getMachineNode(RVGPU::SI_IF_BREAK, DL, VT,
  ...
  case Intrinsic::rvgpu_groupstaticsize: {
    ...
    const GlobalValue *GV =
        M->getNamedValue(Intrinsic::getName(Intrinsic::rvgpu_groupstaticsize));
    SDValue GA = DAG.getTargetGlobalAddress(GV, DL, MVT::i32, 0,
                                            RVInstrInfo::MO_ABS32_LO);
    return {DAG.getMachineNode(RVGPU::S_MOV_B32, DL, MVT::i32, GA), 0};
  }
  case Intrinsic::rvgpu_is_shared:
  case Intrinsic::rvgpu_is_private: {
    SDLoc SL(Op);
    unsigned AS = (IntrinsicID == Intrinsic::rvgpu_is_shared) ?
      RVGPUAS::LOCAL_ADDRESS : RVGPUAS::PRIVATE_ADDRESS;
  ...
  case Intrinsic::rvgpu_perm:
    return DAG.getNode(RVGPUISD::PERM, DL, MVT::i32, Op.getOperand(1),
                       Op.getOperand(2), Op.getOperand(3));
  ...
  default:
    if (const RVGPU::ImageDimIntrinsicInfo *ImageDimIntr =
            RVGPU::getImageDimIntrinsicInfo(IntrinsicID))
  ...
  if (Subtarget->hasRestrictedSOffset())
    if (auto SOffsetConst = dyn_cast<ConstantSDNode>(SOffset)) {
      if (SOffsetConst->isZero()) {
        return DAG.getRegister(RVGPU::SGPR_NULL, MVT::i32);
      }
    }
  return SOffset;
}

SDValue RVTargetLowering::lowerRawBufferAtomicIntrin(SDValue Op,
                                                     SelectionDAG &DAG,
...
SDValue
RVTargetLowering::lowerStructBufferAtomicIntrin(SDValue Op, SelectionDAG &DAG,
                                                unsigned NewOpcode) const {
  SDLoc DL(Op);
...
SDValue RVTargetLowering::LowerINTRINSIC_W_CHAIN(SDValue Op,
                                                 SelectionDAG &DAG) const {
  ...
  case Intrinsic::rvgpu_ds_ordered_swap: {
    ...
    unsigned Instruction = IntrID == Intrinsic::rvgpu_ds_ordered_add ? 0 : 1;
    unsigned ShaderType =
        RVInstrInfo::getDSShaderTypeValue(DAG.getMachineFunction());
    ...
    return DAG.getMemIntrinsicNode(RVGPUISD::DS_ORDERED_COUNT, DL,
  ...
  case Intrinsic::rvgpu_ds_fmax: {
    ...
    switch (IntrID) {
    case Intrinsic::rvgpu_ds_fmin:
      Opc = RVGPUISD::ATOMIC_LOAD_FMIN;
      break;
    case Intrinsic::rvgpu_ds_fmax:
      Opc = RVGPUISD::ATOMIC_LOAD_FMAX;
      break;
    default:
  ...
  case Intrinsic::rvgpu_buffer_load:
  case Intrinsic::rvgpu_buffer_load_format: {
    ...
    unsigned Opc = (IntrID == Intrinsic::rvgpu_buffer_load) ?
        RVGPUISD::BUFFER_LOAD : RVGPUISD::BUFFER_LOAD_FORMAT;
    ...
    if (LoadVT.getScalarType() == MVT::f16)
      return adjustLoadValueType(RVGPUISD::BUFFER_LOAD_FORMAT_D16,
                                 M, DAG, Ops);
  ...
  case Intrinsic::rvgpu_raw_buffer_load_format:
  case Intrinsic::rvgpu_raw_ptr_buffer_load_format: {
    const bool IsFormat =
        IntrID == Intrinsic::rvgpu_raw_buffer_load_format ||
        IntrID == Intrinsic::rvgpu_raw_ptr_buffer_load_format;
  ...
  case Intrinsic::rvgpu_struct_buffer_load_format:
  case Intrinsic::rvgpu_struct_ptr_buffer_load_format: {
    const bool IsFormat =
        IntrID == Intrinsic::rvgpu_struct_buffer_load_format ||
        IntrID == Intrinsic::rvgpu_struct_ptr_buffer_load_format;
  ...
  case Intrinsic::rvgpu_tbuffer_load: {
    MemSDNode *M = cast<MemSDNode>(Op);
    EVT LoadVT = Op.getValueType();
    ...
    if (LoadVT.getScalarType() == MVT::f16)
      return adjustLoadValueType(RVGPUISD::TBUFFER_LOAD_FORMAT_D16,
                                 M, DAG, Ops);
    return getMemIntrinsicNode(RVGPUISD::TBUFFER_LOAD_FORMAT, DL,
  ...
  case Intrinsic::rvgpu_buffer_atomic_swap:
  case Intrinsic::rvgpu_buffer_atomic_add:
  case Intrinsic::rvgpu_buffer_atomic_sub:
  case Intrinsic::rvgpu_buffer_atomic_csub:
  case Intrinsic::rvgpu_buffer_atomic_smin:
  case Intrinsic::rvgpu_buffer_atomic_umin:
  case Intrinsic::rvgpu_buffer_atomic_smax:
  case Intrinsic::rvgpu_buffer_atomic_umax:
  case Intrinsic::rvgpu_buffer_atomic_and:
  case Intrinsic::rvgpu_buffer_atomic_or:
  case Intrinsic::rvgpu_buffer_atomic_xor:
  case Intrinsic::rvgpu_buffer_atomic_fadd: {
    unsigned Slc = cast<ConstantSDNode>(Op.getOperand(6))->getZExtValue();
    ...
    switch (IntrID) {
    case Intrinsic::rvgpu_buffer_atomic_swap:
      Opcode = RVGPUISD::BUFFER_ATOMIC_SWAP;
      break;
    case Intrinsic::rvgpu_buffer_atomic_add:
      Opcode = RVGPUISD::BUFFER_ATOMIC_ADD;
      break;
    case Intrinsic::rvgpu_buffer_atomic_sub:
      Opcode = RVGPUISD::BUFFER_ATOMIC_SUB;
      break;
    case Intrinsic::rvgpu_buffer_atomic_csub:
      Opcode = RVGPUISD::BUFFER_ATOMIC_CSUB;
      break;
    case Intrinsic::rvgpu_buffer_atomic_smin:
      Opcode = RVGPUISD::BUFFER_ATOMIC_SMIN;
      break;
    case Intrinsic::rvgpu_buffer_atomic_umin:
      Opcode = RVGPUISD::BUFFER_ATOMIC_UMIN;
      break;
    case Intrinsic::rvgpu_buffer_atomic_smax:
      Opcode = RVGPUISD::BUFFER_ATOMIC_SMAX;
      break;
    case Intrinsic::rvgpu_buffer_atomic_umax:
      Opcode = RVGPUISD::BUFFER_ATOMIC_UMAX;
      break;
    case Intrinsic::rvgpu_buffer_atomic_and:
      Opcode = RVGPUISD::BUFFER_ATOMIC_AND;
      break;
    case Intrinsic::rvgpu_buffer_atomic_or:
      Opcode = RVGPUISD::BUFFER_ATOMIC_OR;
      break;
    case Intrinsic::rvgpu_buffer_atomic_xor:
      Opcode = RVGPUISD::BUFFER_ATOMIC_XOR;
      break;
    case Intrinsic::rvgpu_buffer_atomic_fadd:
      Opcode = RVGPUISD::BUFFER_ATOMIC_FADD;
      break;
    default:
  ...
  case Intrinsic::rvgpu_raw_buffer_atomic_fadd:
  case Intrinsic::rvgpu_raw_ptr_buffer_atomic_fadd:
    return lowerRawBufferAtomicIntrin(Op, DAG, RVGPUISD::BUFFER_ATOMIC_FADD);
  case Intrinsic::rvgpu_struct_buffer_atomic_fadd:
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_fadd:
    return lowerStructBufferAtomicIntrin(Op, DAG, RVGPUISD::BUFFER_ATOMIC_FADD);
  case Intrinsic::rvgpu_raw_buffer_atomic_fmin:
  case Intrinsic::rvgpu_raw_ptr_buffer_atomic_fmin:
    return lowerRawBufferAtomicIntrin(Op, DAG, RVGPUISD::BUFFER_ATOMIC_FMIN);
  case Intrinsic::rvgpu_struct_buffer_atomic_fmin:
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_fmin:
    return lowerStructBufferAtomicIntrin(Op, DAG, RVGPUISD::BUFFER_ATOMIC_FMIN);
  case Intrinsic::rvgpu_raw_buffer_atomic_fmax:
  case Intrinsic::rvgpu_raw_ptr_buffer_atomic_fmax:
    return lowerRawBufferAtomicIntrin(Op, DAG, RVGPUISD::BUFFER_ATOMIC_FMAX);
  case Intrinsic::rvgpu_struct_buffer_atomic_fmax:
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_fmax:
    return lowerStructBufferAtomicIntrin(Op, DAG, RVGPUISD::BUFFER_ATOMIC_FMAX);
  case Intrinsic::rvgpu_raw_buffer_atomic_swap:
  case Intrinsic::rvgpu_raw_ptr_buffer_atomic_swap:
    return lowerRawBufferAtomicIntrin(Op, DAG, RVGPUISD::BUFFER_ATOMIC_SWAP);
  case Intrinsic::rvgpu_raw_buffer_atomic_add:
  case Intrinsic::rvgpu_raw_ptr_buffer_atomic_add:
    return lowerRawBufferAtomicIntrin(Op, DAG, RVGPUISD::BUFFER_ATOMIC_ADD);
  case Intrinsic::rvgpu_raw_buffer_atomic_sub:
  case Intrinsic::rvgpu_raw_ptr_buffer_atomic_sub:
    return lowerRawBufferAtomicIntrin(Op, DAG, RVGPUISD::BUFFER_ATOMIC_SUB);
  case Intrinsic::rvgpu_raw_buffer_atomic_smin:
  case Intrinsic::rvgpu_raw_ptr_buffer_atomic_smin:
    return lowerRawBufferAtomicIntrin(Op, DAG, RVGPUISD::BUFFER_ATOMIC_SMIN);
  case Intrinsic::rvgpu_raw_buffer_atomic_umin:
  case Intrinsic::rvgpu_raw_ptr_buffer_atomic_umin:
    return lowerRawBufferAtomicIntrin(Op, DAG, RVGPUISD::BUFFER_ATOMIC_UMIN);
  case Intrinsic::rvgpu_raw_buffer_atomic_smax:
  case Intrinsic::rvgpu_raw_ptr_buffer_atomic_smax:
    return lowerRawBufferAtomicIntrin(Op, DAG, RVGPUISD::BUFFER_ATOMIC_SMAX);
  case Intrinsic::rvgpu_raw_buffer_atomic_umax:
  case Intrinsic::rvgpu_raw_ptr_buffer_atomic_umax:
    return lowerRawBufferAtomicIntrin(Op, DAG, RVGPUISD::BUFFER_ATOMIC_UMAX);
  case Intrinsic::rvgpu_raw_buffer_atomic_and:
  case Intrinsic::rvgpu_raw_ptr_buffer_atomic_and:
    return lowerRawBufferAtomicIntrin(Op, DAG, RVGPUISD::BUFFER_ATOMIC_AND);
  case Intrinsic::rvgpu_raw_buffer_atomic_or:
  case Intrinsic::rvgpu_raw_ptr_buffer_atomic_or:
    return lowerRawBufferAtomicIntrin(Op, DAG, RVGPUISD::BUFFER_ATOMIC_OR);
  case Intrinsic::rvgpu_raw_buffer_atomic_xor:
  case Intrinsic::rvgpu_raw_ptr_buffer_atomic_xor:
    return lowerRawBufferAtomicIntrin(Op, DAG, RVGPUISD::BUFFER_ATOMIC_XOR);
  case Intrinsic::rvgpu_raw_buffer_atomic_inc:
  case Intrinsic::rvgpu_raw_ptr_buffer_atomic_inc:
    return lowerRawBufferAtomicIntrin(Op, DAG, RVGPUISD::BUFFER_ATOMIC_INC);
  case Intrinsic::rvgpu_raw_buffer_atomic_dec:
  case Intrinsic::rvgpu_raw_ptr_buffer_atomic_dec:
    return lowerRawBufferAtomicIntrin(Op, DAG, RVGPUISD::BUFFER_ATOMIC_DEC);
  case Intrinsic::rvgpu_struct_buffer_atomic_swap:
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_swap:
    return lowerStructBufferAtomicIntrin(Op, DAG,
                                         RVGPUISD::BUFFER_ATOMIC_SWAP);
  case Intrinsic::rvgpu_struct_buffer_atomic_add:
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_add:
    return lowerStructBufferAtomicIntrin(Op, DAG, RVGPUISD::BUFFER_ATOMIC_ADD);
  case Intrinsic::rvgpu_struct_buffer_atomic_sub:
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_sub:
    return lowerStructBufferAtomicIntrin(Op, DAG, RVGPUISD::BUFFER_ATOMIC_SUB);
  case Intrinsic::rvgpu_struct_buffer_atomic_smin:
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_smin:
    return lowerStructBufferAtomicIntrin(Op, DAG,
                                         RVGPUISD::BUFFER_ATOMIC_SMIN);
  case Intrinsic::rvgpu_struct_buffer_atomic_umin:
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_umin:
    return lowerStructBufferAtomicIntrin(Op, DAG,
                                         RVGPUISD::BUFFER_ATOMIC_UMIN);
  case Intrinsic::rvgpu_struct_buffer_atomic_smax:
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_smax:
    return lowerStructBufferAtomicIntrin(Op, DAG,
                                         RVGPUISD::BUFFER_ATOMIC_SMAX);
  case Intrinsic::rvgpu_struct_buffer_atomic_umax:
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_umax:
    return lowerStructBufferAtomicIntrin(Op, DAG,
                                         RVGPUISD::BUFFER_ATOMIC_UMAX);
  case Intrinsic::rvgpu_struct_buffer_atomic_and:
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_and:
    return lowerStructBufferAtomicIntrin(Op, DAG, RVGPUISD::BUFFER_ATOMIC_AND);
  case Intrinsic::rvgpu_struct_buffer_atomic_or:
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_or:
    return lowerStructBufferAtomicIntrin(Op, DAG, RVGPUISD::BUFFER_ATOMIC_OR);
  case Intrinsic::rvgpu_struct_buffer_atomic_xor:
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_xor:
    return lowerStructBufferAtomicIntrin(Op, DAG, RVGPUISD::BUFFER_ATOMIC_XOR);
  case Intrinsic::rvgpu_struct_buffer_atomic_inc:
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_inc:
    return lowerStructBufferAtomicIntrin(Op, DAG, RVGPUISD::BUFFER_ATOMIC_INC);
  case Intrinsic::rvgpu_struct_buffer_atomic_dec:
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_dec:
    return lowerStructBufferAtomicIntrin(Op, DAG, RVGPUISD::BUFFER_ATOMIC_DEC);

  case Intrinsic::rvgpu_buffer_atomic_cmpswap: {
    ...
    return DAG.getMemIntrinsicNode(RVGPUISD::BUFFER_ATOMIC_CMPSWAP, DL,
                                   Op->getVTList(), Ops, VT, M->getMemOperand());
  }
  case Intrinsic::rvgpu_raw_buffer_atomic_cmpswap:
  case Intrinsic::rvgpu_raw_ptr_buffer_atomic_cmpswap: {
  ...
  case Intrinsic::rvgpu_struct_buffer_atomic_cmpswap:
  case Intrinsic::rvgpu_struct_ptr_buffer_atomic_cmpswap: {
    SDValue Rsrc = bufferRsrcPtrToVector(Op->getOperand(4), DAG);
  ...
  case Intrinsic::rvgpu_image_bvh_intersect_ray: {
    ...
    const unsigned BaseOpcodes[2][2] = {
        {RVGPU::IMAGE_BVH_INTERSECT_RAY, RVGPU::IMAGE_BVH_INTERSECT_RAY_a16},
        {RVGPU::IMAGE_BVH64_INTERSECT_RAY,
         RVGPU::IMAGE_BVH64_INTERSECT_RAY_a16}};
    int Opcode;
    if (UseNSA) {
  ...
  case Intrinsic::rvgpu_flat_atomic_fmax_num: {
    ...
    case Intrinsic::rvgpu_flat_atomic_fmin:
    case Intrinsic::rvgpu_flat_atomic_fmin_num: {
      Opcode = RVGPUISD::ATOMIC_LOAD_FMIN;
      break;
    }
    case Intrinsic::rvgpu_global_atomic_fmax:
    case Intrinsic::rvgpu_global_atomic_fmax_num:
    case Intrinsic::rvgpu_flat_atomic_fmax:
    case Intrinsic::rvgpu_flat_atomic_fmax_num: {
      Opcode = RVGPUISD::ATOMIC_LOAD_FMAX;
      break;
    }
    default:
      llvm_unreachable("unhandled atomic opcode");
  ...
  case Intrinsic::rvgpu_s_get_barrier_state: {
    ...
    if (isa<ConstantSDNode>(Op->getOperand(2))) {
      ...
      IsInlinableBarID = RVGPU::isInlinableIntLiteral(BarID);
    }

    if (IsInlinableBarID) {
      Opc = RVGPU::S_GET_BARRIER_STATE_IMM;
      SDValue K = DAG.getTargetConstant(BarID, DL, MVT::i32);
      Ops.push_back(K);
    } else {
      Opc = RVGPU::S_GET_BARRIER_STATE_M0;
...
SDValue RVTargetLowering::getMemIntrinsicNode(unsigned Opcode, const SDLoc &DL,
                                              SDVTList VTList,
...
SDValue RVTargetLowering::handleD16VData(SDValue VData, SelectionDAG &DAG,
...
SDValue RVTargetLowering::LowerINTRINSIC_VOID(SDValue Op,
                                              SelectionDAG &DAG) const {
  SDLoc DL(Op);
  SDValue Chain = Op.getOperand(0);
  ...
  case Intrinsic::rvgpu_exp_compr: {
    ...
    unsigned Opc = Done->isZero() ? RVGPU::EXP : RVGPU::EXP_DONE;
  ...
  case Intrinsic::rvgpu_s_barrier: {
    ...
    if (getTargetMachine().getOptLevel() > CodeGenOptLevel::None) {
      ...
      if (WGSize <= ST.getWavefrontSize())
        return SDValue(DAG.getMachineNode(RVGPU::WAVE_BARRIER, DL, MVT::Other,
                                          Op.getOperand(0)), 0);
    }
    ...
    if (ST.hasSplitBarriers()) {
      SDValue K =
          DAG.getTargetConstant(RVGPU::Barrier::WORKGROUP, DL, MVT::i32);
      SDValue BarSignal =
          SDValue(DAG.getMachineNode(RVGPU::S_BARRIER_SIGNAL_IMM, DL,
                                     MVT::Other, K, Op.getOperand(0)),
                  0);
      SDValue BarWait =
          SDValue(DAG.getMachineNode(RVGPU::S_BARRIER_WAIT, DL, MVT::Other, K,
  ...
  case Intrinsic::rvgpu_tbuffer_store: {
    SDValue VData = Op.getOperand(2);
    ...
    unsigned Opc = IsD16 ? RVGPUISD::TBUFFER_STORE_FORMAT_D16 :
                           RVGPUISD::TBUFFER_STORE_FORMAT;
    MemSDNode *M = cast<MemSDNode>(Op);
  ...
  case Intrinsic::rvgpu_buffer_store:
  case Intrinsic::rvgpu_buffer_store_format: {
    SDValue VData = Op.getOperand(2);
    bool IsD16 = (VData.getValueType().getScalarType() == MVT::f16);
    if (IsD16)
      VData = handleD16VData(VData, DAG);
    ...
    unsigned Opc = IntrinsicID == Intrinsic::rvgpu_buffer_store ?
                   RVGPUISD::BUFFER_STORE : RVGPUISD::BUFFER_STORE_FORMAT;
    Opc = IsD16 ? RVGPUISD::BUFFER_STORE_FORMAT_D16 : Opc;
    MemSDNode *M = cast<MemSDNode>(Op);
  ...
  case Intrinsic::rvgpu_raw_ptr_buffer_store_format: {
    const bool IsFormat =
        IntrinsicID == Intrinsic::rvgpu_raw_buffer_store_format ||
        IntrinsicID == Intrinsic::rvgpu_raw_ptr_buffer_store_format;
    ...
    unsigned Opc =
        IsFormat ? RVGPUISD::BUFFER_STORE_FORMAT : RVGPUISD::BUFFER_STORE;
    Opc = IsD16 ? RVGPUISD::BUFFER_STORE_FORMAT_D16 : Opc;
    MemSDNode *M = cast<MemSDNode>(Op);
  ...
  case Intrinsic::rvgpu_struct_buffer_store_format:
  case Intrinsic::rvgpu_struct_ptr_buffer_store_format: {
    const bool IsFormat =
        IntrinsicID == Intrinsic::rvgpu_struct_buffer_store_format ||
        IntrinsicID == Intrinsic::rvgpu_struct_ptr_buffer_store_format;
    ...
    unsigned Opc =
        !IsFormat ? RVGPUISD::BUFFER_STORE : RVGPUISD::BUFFER_STORE_FORMAT;
    Opc = IsD16 ? RVGPUISD::BUFFER_STORE_FORMAT_D16 : Opc;
    MemSDNode *M = cast<MemSDNode>(Op);
  ...
  case Intrinsic::rvgpu_struct_ptr_buffer_load_lds: {
    unsigned Opc;
    bool HasVIndex =
        IntrinsicID == Intrinsic::rvgpu_struct_buffer_load_lds ||
        IntrinsicID == Intrinsic::rvgpu_struct_ptr_buffer_load_lds;
    ...
    default:
      return SDValue();
    case 1:
      Opc = HasVIndex ? HasVOffset ? RVGPU::BUFFER_LOAD_UBYTE_LDS_BOTHEN
                                   : RVGPU::BUFFER_LOAD_UBYTE_LDS_IDXEN
                      : HasVOffset ? RVGPU::BUFFER_LOAD_UBYTE_LDS_OFFEN
                                   : RVGPU::BUFFER_LOAD_UBYTE_LDS_OFFSET;
      break;
    case 2:
      Opc = HasVIndex ? HasVOffset ? RVGPU::BUFFER_LOAD_USHORT_LDS_BOTHEN
                                   : RVGPU::BUFFER_LOAD_USHORT_LDS_IDXEN
                      : HasVOffset ? RVGPU::BUFFER_LOAD_USHORT_LDS_OFFEN
                                   : RVGPU::BUFFER_LOAD_USHORT_LDS_OFFSET;
      break;
    case 4:
      Opc = HasVIndex ? HasVOffset ? RVGPU::BUFFER_LOAD_DWORD_LDS_BOTHEN
                                   : RVGPU::BUFFER_LOAD_DWORD_LDS_IDXEN
                      : HasVOffset ? RVGPU::BUFFER_LOAD_DWORD_LDS_OFFEN
                                   : RVGPU::BUFFER_LOAD_DWORD_LDS_OFFSET;
      break;
    }
    ...
    Ops.push_back(
      DAG.getTargetConstant(Aux & RVGPU::CPol::ALL, DL, MVT::i8)); // cpol
    Ops.push_back(
    ...
    LoadPtrI.V = PoisonValue::get(
        PointerType::get(*DAG.getContext(), RVGPUAS::GLOBAL_ADDRESS));
    LoadPtrI.AddrSpace = RVGPUAS::GLOBAL_ADDRESS;
    StorePtrI.AddrSpace = RVGPUAS::LOCAL_ADDRESS;
  ...
  case Intrinsic::rvgpu_global_load_lds: {
    ...
    default:
      return SDValue();
    case 1:
      Opc = RVGPU::GLOBAL_LOAD_LDS_UBYTE;
      break;
    case 2:
      Opc = RVGPU::GLOBAL_LOAD_LDS_USHORT;
      break;
    case 4:
      Opc = RVGPU::GLOBAL_LOAD_LDS_DWORD;
      break;
    }
    ...
    Ops.push_back(Addr);
    if (!Addr->isDivergent()) {
      Opc = RVGPU::getGlobalSaddrOp(Opc);
      if (!VOffset)
        VOffset = SDValue(
            DAG.getMachineNode(RVGPU::V_MOV_B32_e32, DL, MVT::i32,
  ...
  case Intrinsic::rvgpu_end_cf:
    return SDValue(DAG.getMachineNode(RVGPU::SI_END_CF, DL, MVT::Other,
  ...
  case Intrinsic::rvgpu_s_wakeup_barrier: {
    ...
    if (isa<ConstantSDNode>(BarOp)) {
      ...
      IsInlinableBarID = RVGPU::isInlinableIntLiteral(BarVal);
    }
    ...
    if (IsInlinableBarID) {
      ...
      default:
        return SDValue();
      case Intrinsic::rvgpu_s_barrier_init:
        Opc = RVGPU::S_BARRIER_INIT_IMM;
        break;
      case Intrinsic::rvgpu_s_barrier_join:
        Opc = RVGPU::S_BARRIER_JOIN_IMM;
        break;
      case Intrinsic::rvgpu_s_wakeup_barrier:
        Opc = RVGPU::S_WAKEUP_BARRIER_IMM;
        break;
      }
      ...
      default:
        return SDValue();
      case Intrinsic::rvgpu_s_barrier_init:
        Opc = RVGPU::S_BARRIER_INIT_M0;
        break;
      case Intrinsic::rvgpu_s_barrier_join:
        Opc = RVGPU::S_BARRIER_JOIN_M0;
        break;
      case Intrinsic::rvgpu_s_wakeup_barrier:
        Opc = RVGPU::S_WAKEUP_BARRIER_M0;
        break;
      }
    }

    if (IntrinsicID == Intrinsic::rvgpu_s_barrier_init) {
      SDValue M0Val;
      ...
      if (!IsInlinableBarID) {
        ...
        // include it in M0.
        M0Val = SDValue(DAG.getMachineNode(RVGPU::S_OR_B32, DL, MVT::i32,
...
std::pair<SDValue, SDValue> RVTargetLowering::splitBufferOffsets(
...
// Analyze a combined offset from an rvgpu_buffer_ intrinsic and store the
// three offsets (voffset, soffset and instoffset) into the SDValue[3] array
// pointed to by Offsets.
void RVTargetLowering::setBufferOffsets(SDValue CombinedOffset,
  ...
  SDValue SOffsetZero = Subtarget->hasRestrictedSOffset()
                            ? DAG.getRegister(RVGPU::SGPR_NULL, MVT::i32)
...
SDValue RVTargetLowering::bufferRsrcPtrToVector(SDValue MaybePointer,
                                                SelectionDAG &DAG) const {
...
SDValue RVTargetLowering::lowerPointerAsRsrcIntrin(SDNode *Op,
                                                   SelectionDAG &DAG) const {
  SDLoc Loc(Op);
...
SDValue RVTargetLowering::handleByteShortBufferLoads(SelectionDAG &DAG,
                                                     EVT LoadVT, SDLoc DL,
                                                     ArrayRef<SDValue> Ops,
                                                     MemSDNode *M) const {
  EVT IntVT = LoadVT.changeTypeToInteger();
  unsigned Opc = (LoadVT.getScalarType() == MVT::i8) ?
         RVGPUISD::BUFFER_LOAD_UBYTE : RVGPUISD::BUFFER_LOAD_USHORT;
...
SDValue RVTargetLowering::handleByteShortBufferStores(SelectionDAG &DAG,
  ...
  unsigned Opc = (VDataType == MVT::i8) ? RVGPUISD::BUFFER_STORE_BYTE :
                                 RVGPUISD::BUFFER_STORE_SHORT;
...
SDValue RVTargetLowering::widenLoad(LoadSDNode *Ld, DAGCombinerInfo &DCI) const {
  ...
  if (AS != RVGPUAS::CONSTANT_ADDRESS &&
      AS != RVGPUAS::CONSTANT_ADDRESS_32BIT &&
      (AS != RVGPUAS::GLOBAL_ADDRESS || !Ld->isInvariant()))
    return SDValue();
...
SDValue RVTargetLowering::LowerLOAD(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  if (Subtarget->hasLDSMisalignedBug() && AS == RVGPUAS::FLAT_ADDRESS &&
...
SDValue RVTargetLowering::LowerSELECT(SDValue Op, SelectionDAG &DAG) const {
  EVT VT = Op.getValueType();
...
// instructions.
SDValue RVTargetLowering::lowerFastUnsafeFDIV(SDValue Op,
                                              SelectionDAG &DAG) const {
  SDLoc SL(Op);
  SDValue LHS = Op.getOperand(0);
  SDValue RHS = Op.getOperand(1);
  ...
  if (const ConstantFPSDNode *CLHS = dyn_cast<ConstantFPSDNode>(LHS)) {
    ...
    if (CLHS->isExactlyValue(1.0)) {
      ...
      // 1.0 / x -> rcp(x)
      return DAG.getNode(RVGPUISD::RCP, SL, VT, RHS);
    }
    ...
    if (CLHS->isExactlyValue(-1.0)) {
      ...
      return DAG.getNode(RVGPUISD::RCP, SL, VT, FNegRHS);
    }
  }
  ...
  // x / y -> x * (1.0 / y)
  SDValue Recip = DAG.getNode(RVGPUISD::RCP, SL, VT, RHS);
  return DAG.getNode(ISD::FMUL, SL, VT, LHS, Recip, Flags);
}

SDValue RVTargetLowering::lowerFastUnsafeFDIV64(SDValue Op,
                                                SelectionDAG &DAG) const {
  SDLoc SL(Op);
  ...
  SDValue R = DAG.getNode(RVGPUISD::RCP, SL, VT, Y);
...
SDValue RVTargetLowering::LowerFDIV16(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue RcpSrc1 = DAG.getNode(RVGPUISD::RCP, SL, MVT::f32, CvtSrc1);
  ...
  return DAG.getNode(RVGPUISD::DIV_FIXUP, SL, MVT::f16, BestQuot, Src1, Src0);
}

// Faster 2.5 ULP division that does not support denormals.
SDValue RVTargetLowering::lowerFDIV_FAST(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue r0 = DAG.getNode(RVGPUISD::RCP, SL, MVT::f32, r1, Flags);
...
SDValue RVTargetLowering::LowerFDIV32(SDValue Op, SelectionDAG &DAG) const {
  if (SDValue FastLowered = lowerFastUnsafeFDIV(Op, DAG))
    return FastLowered;
  ...
  SDValue DenominatorScaled = DAG.getNode(RVGPUISD::DIV_SCALE, SL, ScaleVT,
                                          {RHS, RHS, LHS}, Flags);
  SDValue NumeratorScaled = DAG.getNode(RVGPUISD::DIV_SCALE, SL, ScaleVT,
                                        {LHS, RHS, LHS}, Flags);

  // Denominator is scaled to not be denormal, so using rcp is ok.
  SDValue ApproxRcp = DAG.getNode(RVGPUISD::RCP, SL, MVT::f32,
                                  DenominatorScaled, Flags);
  SDValue NegDivScale0 = DAG.getNode(ISD::FNEG, SL, MVT::f32,
                                     DenominatorScaled, Flags);

  const unsigned Denorm32Reg = RVGPU::Hwreg::ID_MODE |
                               (4 << RVGPU::Hwreg::OFFSET_SHIFT_) |
                               (1 << RVGPU::Hwreg::WIDTH_M1_SHIFT_);
  ...
  if (!PreservesDenormals) {
    ...
    if (HasDynamicDenormals) {
      SDNode *GetReg = DAG.getMachineNode(RVGPU::S_GETREG_B32, SL,
    ...
    if (Subtarget->hasDenormModeInst()) {
      ...
      EnableDenorm = DAG.getNode(RVGPUISD::DENORM_MODE, SL, BindParamVTs, Glue,
                                 EnableDenormValue)
                         .getNode();
    } else {
      const SDValue EnableDenormValue = DAG.getConstant(FP_DENORM_FLUSH_NONE,
                                                        SL, MVT::i32);
      EnableDenorm = DAG.getMachineNode(RVGPU::S_SETREG_B32, SL, BindParamVTs,
  ...
  if (!PreservesDenormals) {
    ...
    if (!HasDynamicDenormals && Subtarget->hasDenormModeInst()) {
      ...
      DisableDenorm = DAG.getNode(RVGPUISD::DENORM_MODE, SL, MVT::Other,
      ...
      DisableDenorm = DAG.getMachineNode(
          RVGPU::S_SETREG_B32, SL, MVT::Other,
  ...
  SDValue Fmas = DAG.getNode(RVGPUISD::DIV_FMAS, SL, MVT::f32,
                             {Fma4, Fma1, Fma3, Scale}, Flags);

  return DAG.getNode(RVGPUISD::DIV_FIXUP, SL, MVT::f32, Fmas, RHS, LHS, Flags);
}

SDValue RVTargetLowering::LowerFDIV64(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue DivScale0 = DAG.getNode(RVGPUISD::DIV_SCALE, SL, ScaleVT, Y, Y, X);

  SDValue NegDivScale0 = DAG.getNode(ISD::FNEG, SL, MVT::f64, DivScale0);

  SDValue Rcp = DAG.getNode(RVGPUISD::RCP, SL, MVT::f64, DivScale0);
  ...
  SDValue DivScale1 = DAG.getNode(RVGPUISD::DIV_SCALE, SL, ScaleVT, X, Y, X);
  ...
  SDValue Fmas = DAG.getNode(RVGPUISD::DIV_FMAS, SL, MVT::f64,
                             Fma4, Fma3, Mul, Scale);

  return DAG.getNode(RVGPUISD::DIV_FIXUP, SL, MVT::f64, Fmas, Y, X);
}

SDValue RVTargetLowering::LowerFDIV(SDValue Op, SelectionDAG &DAG) const {
  EVT VT = Op.getValueType();
...
SDValue RVTargetLowering::LowerFFREXP(SDValue Op, SelectionDAG &DAG) const {
  SDLoc dl(Op);
  ...
  SDValue Mant = DAG.getNode(
      ...
      DAG.getTargetConstant(Intrinsic::rvgpu_frexp_mant, dl, MVT::i32), Val);

  SDValue Exp = DAG.getNode(
      ISD::INTRINSIC_WO_CHAIN, dl, InstrExpVT,
      DAG.getTargetConstant(Intrinsic::rvgpu_frexp_exp, dl, MVT::i32), Val);
...
SDValue RVTargetLowering::LowerSTORE(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue RVTargetLowering::lowerFSQRTF16(SDValue Op, SelectionDAG &DAG) const {
  SDLoc SL(Op);
  ...
  SDValue SqrtID = DAG.getTargetConstant(Intrinsic::rvgpu_sqrt, SL, MVT::i32);
  SDValue Sqrt =
...
SDValue RVTargetLowering::lowerFSQRTF32(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  SDNodeFlags Flags = Op->getFlags();
  ...
  if (allowApproxFunc(DAG, Flags)) {
    ...
    return DAG.getNode(
        ISD::INTRINSIC_WO_CHAIN, DL, VT,
        DAG.getTargetConstant(Intrinsic::rvgpu_sqrt, DL, MVT::i32), X, Flags);
  }
  ...
  if (needsDenormHandlingF32(DAG, X, Flags)) {
    SDValue SqrtID =
        DAG.getTargetConstant(Intrinsic::rvgpu_sqrt, DL, MVT::i32);
    ...
    SDValue SqrtR = DAG.getNode(RVGPUISD::RSQ, DL, VT, SqrtX, Flags);
...
SDValue RVTargetLowering::lowerFSQRTF64(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue SqrtY = DAG.getNode(RVGPUISD::RSQ, DL, MVT::f64, SqrtX);
...
SDValue RVTargetLowering::LowerTrig(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  EVT VT = Op.getValueType();
  ...
  if (Subtarget->hasTrigReducedRange()) {
    ...
    TrigVal = DAG.getNode(RVGPUISD::FRACT, DL, VT, MulVal, Flags);
  } else {
  ...
  switch (Op.getOpcode()) {
  case ISD::FCOS:
    return DAG.getNode(RVGPUISD::COS_HW, SDLoc(Op), VT, TrigVal, Flags);
  case ISD::FSIN:
    return DAG.getNode(RVGPUISD::SIN_HW, SDLoc(Op), VT, TrigVal, Flags);
  default:
    llvm_unreachable("Wrong trig opcode");
  }
}

SDValue RVTargetLowering::LowerATOMIC_CMP_SWAP(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getMemIntrinsicNode(RVGPUISD::ATOMIC_CMP_SWAP, DL, Op->getVTList(),
...
SDValue RVTargetLowering::performUCharToFloatCombine(SDNode *N,
  ...
  if (DCI.isAfterLegalizeDAG() && SrcVT == MVT::i32) {
    if (DAG.MaskedValueIsZero(Src, APInt::getHighBitsSet(32, 24))) {
      SDValue Cvt = DAG.getNode(RVGPUISD::CVT_F32_UBYTE0, DL, MVT::f32, Src);
      DCI.AddToWorklist(Cvt.getNode());
...
SDValue RVTargetLowering::performFCopySignCombine(SDNode *N,
                                                  DAGCombinerInfo &DCI) const {
...
//
SDValue RVTargetLowering::performSHLPtrCombine(SDNode *N,
                                               unsigned AddrSpace,
                                               EVT MemVT,
                                               DAGCombinerInfo &DCI) const {
...
SDValue RVTargetLowering::performMemSDNodeCombine(MemSDNode *N,
                                                  DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
  SDLoc SL(N);
...
// immediate.
SDValue RVTargetLowering::splitBinaryBitConstantOp(
  DAGCombinerInfo &DCI,
  const SDLoc &SL,
...
SDValue RVTargetLowering::performAndCombine(SDNode *N,
                                            DAGCombinerInfo &DCI) const {
  ...
  if (CRHS && VT == MVT::i32) {
    ...
    if (getSubtarget()->hasSDWA() && LHS->getOpcode() == ISD::SRL &&
      ...
      if (auto *CShift = dyn_cast<ConstantSDNode>(LHS->getOperand(1))) {
        ...
        if ((Offset & (Bits - 1)) == 0) { // Starts at a byte or word boundary.
          SDLoc SL(N);
          SDValue BFE = DAG.getNode(RVGPUISD::BFE_U32, SL, MVT::i32,
                                    LHS->getOperand(0),
    ...
    if (LHS.hasOneUse() && LHS.getOpcode() == RVGPUISD::PERM &&
      ...
      SDLoc DL(N);
      return DAG.getNode(RVGPUISD::PERM, DL, MVT::i32, LHS.getOperand(0),
  ...
  if (LHS.getOpcode() == ISD::SETCC && RHS.getOpcode() == ISD::SETCC) {
    ...
    if (LCC == ISD::SETO) {
      ...
      if (RCC == ISD::SETUNE) {
        ...
        SDLoc DL(N);
        return DAG.getNode(RVGPUISD::FP_CLASS, DL, MVT::i1,
                           X, DAG.getConstant(Mask, DL, MVT::i32));
      }
    }
  }

  if (RHS.getOpcode() == ISD::SETCC && LHS.getOpcode() == RVGPUISD::FP_CLASS)
    std::swap(LHS, RHS);

  if (LHS.getOpcode() == ISD::SETCC && RHS.getOpcode() == RVGPUISD::FP_CLASS &&
      RHS.hasOneUse()) {
    ISD::CondCode LCC = cast<CondCodeSDNode>(LHS.getOperand(2))->get();
    ...
    if ((LCC == ISD::SETO || LCC == ISD::SETUO) && Mask &&
      ...
      SDLoc DL(N);
      return DAG.getNode(RVGPUISD::FP_CLASS, DL, MVT::i1, RHS.getOperand(0),
  ...
  if (VT == MVT::i32 && LHS.hasOneUse() && RHS.hasOneUse() &&
      N->isDivergent() && TII->pseudoToMCOpcode(RVGPU::V_PERM_B32_e64) != -1) {
    ...
    if (LHSMask != ~0u && RHSMask != ~0u) {
      ...
      if (!(LHSUsedLanes & RHSUsedLanes) &&
        ...
        return DAG.getNode(RVGPUISD::PERM, DL, MVT::i32,
...
static SDValue matchPERM(SDNode *N, TargetLowering::DAGCombinerInfo &DCI) {
  ...
  if (hasNon16BitAccesses(PermMask, Op, OtherOp)) {
    ...
    return DAG.getNode(RVGPUISD::PERM, DL, MVT::i32, Op, OtherOp,
...
SDValue RVTargetLowering::performOrCombine(SDNode *N,
                                           DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
  SDValue LHS = N->getOperand(0);
  SDValue RHS = N->getOperand(1);
  ...
  if (VT == MVT::i1) {
    ...
    if (LHS.getOpcode() == RVGPUISD::FP_CLASS &&
        RHS.getOpcode() == RVGPUISD::FP_CLASS) {
...
SDValue RVTargetLowering::performXorCombine(SDNode *N,
                                            DAGCombinerInfo &DCI) const {
...
SDValue RVTargetLowering::performZeroExtendCombine(SDNode *N,
                                                   DAGCombinerInfo &DCI) const {
  if (!Subtarget->has16BitInsts() ||
...
SDValue RVTargetLowering::performSignExtendInRegCombine(SDNode *N,
                                                        DAGCombinerInfo &DCI)
                                                        const {
  SDValue Src = N->getOperand(0);
  auto *VTSign = cast<VTSDNode>(N->getOperand(1));

  if (((Src.getOpcode() == RVGPUISD::BUFFER_LOAD_UBYTE &&
      VTSign->getVT() == MVT::i8) ||
      (Src.getOpcode() == RVGPUISD::BUFFER_LOAD_USHORT &&
    ...
    unsigned Opc = (Src.getOpcode() == RVGPUISD::BUFFER_LOAD_UBYTE) ?
                   RVGPUISD::BUFFER_LOAD_BYTE : RVGPUISD::BUFFER_LOAD_SHORT;
...
SDValue RVTargetLowering::performClassCombine(SDNode *N,
                                              DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
...
SDValue RVTargetLowering::performRcpCombine(SDNode *N,
                                            DAGCombinerInfo &DCI) const {
  EVT VT = N->getValueType(0);
  SDValue N0 = N->getOperand(0);
  ...
  if (VT == MVT::f32 && (N0.getOpcode() == ISD::UINT_TO_FP ||
    ...
    return DCI.DAG.getNode(RVGPUISD::RCP_IFLAG, SDLoc(N), VT, N0,
                           N->getFlags());
  }

  // TODO: Could handle f32 + rvgpu.sqrt but probably never reaches here.
  if ((VT == MVT::f16 && N0.getOpcode() == ISD::FSQRT) &&
      N->getFlags().hasAllowContract() && N0->getFlags().hasAllowContract()) {
    return DCI.DAG.getNode(RVGPUISD::RSQ, SDLoc(N), VT,
                           N0.getOperand(0), N->getFlags());
  }

  return RVGPUTargetLowering::performRcpCombine(N, DCI);
}

bool RVTargetLowering::isCanonicalized(SelectionDAG &DAG, SDValue Op,
  ...
  case ISD::FP_EXTEND:
  case ISD::FLDEXP:
  case RVGPUISD::FMUL_LEGACY:
  case RVGPUISD::FMAD_FTZ:
  case RVGPUISD::RCP:
  case RVGPUISD::RSQ:
  case RVGPUISD::RSQ_CLAMP:
  case RVGPUISD::RCP_LEGACY:
  case RVGPUISD::RCP_IFLAG:
  case RVGPUISD::LOG:
  case RVGPUISD::EXP:
  case RVGPUISD::DIV_SCALE:
  case RVGPUISD::DIV_FMAS:
  case RVGPUISD::DIV_FIXUP:
  case RVGPUISD::FRACT:
  case RVGPUISD::CVT_PKRTZ_F16_F32:
  case RVGPUISD::CVT_F32_UBYTE0:
  case RVGPUISD::CVT_F32_UBYTE1:
  case RVGPUISD::CVT_F32_UBYTE2:
  case RVGPUISD::CVT_F32_UBYTE3:
    return true;
  ...
  case RVGPUISD::FMAX3:
  case RVGPUISD::FMIN3:
  case RVGPUISD::FMAXIMUM3:
  case RVGPUISD::FMINIMUM3: {
...
bool RVTargetLowering::isCanonicalized(Register Reg, MachineFunction &MF,
                                       unsigned MaxDepth) const {
  MachineRegisterInfo &MRI = MF.getRegInfo();
  MachineInstr *MI = MRI.getVRegDef(Reg);
  unsigned Opcode = MI->getOpcode();

  if (Opcode == RVGPU::G_FCANONICALIZE)
    return true;
  ...
  case RVGPU::G_FFLOOR:
  case RVGPU::G_FRINT:
  case RVGPU::G_FNEARBYINT:
  case RVGPU::G_INTRINSIC_FPTRUNC_ROUND:
...
SDValue RVTargetLowering::getCanonicalConstantFP(
...
SDValue RVTargetLowering::performFCanonicalizeCombine(
  SDNode *N,
  DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
  SDValue N0 = N->getOperand(0);
...
static unsigned minMaxOpcToMin3Max3Opc(unsigned Opc) {
  ...
  case ISD::FMAXNUM:
  case ISD::FMAXNUM_IEEE:
    return RVGPUISD::FMAX3;
  case ISD::FMAXIMUM:
    return RVGPUISD::FMAXIMUM3;
  case ISD::SMAX:
    return RVGPUISD::SMAX3;
  case ISD::UMAX:
    return RVGPUISD::UMAX3;
  case ISD::FMINNUM:
  case ISD::FMINNUM_IEEE:
    return RVGPUISD::FMIN3;
  case ISD::FMINIMUM:
    return RVGPUISD::FMINIMUM3;
  case ISD::SMIN:
    return RVGPUISD::SMIN3;
  case ISD::UMIN:
    return RVGPUISD::UMIN3;
  default:
    llvm_unreachable("Not a min/max opcode");
  }
}

SDValue RVTargetLowering::performIntMed3ImmCombine(SelectionDAG &DAG,
  ...
  unsigned Med3Opc = Signed ? RVGPUISD::SMED3 : RVGPUISD::UMED3;
...
SDValue RVTargetLowering::performFPMed3ImmCombine(SelectionDAG &DAG,
                                                  const SDLoc &SL,
                                                  SDValue Op0,
                                                  SDValue Op1) const {
  ...
  if (Info->getMode().DX10Clamp) {
    ...
    if (K1->isExactlyValue(1.0) && K0->isExactlyValue(0.0))
      return DAG.getNode(RVGPUISD::CLAMP, SL, VT, Op0.getOperand(0));
  }
  ...
  if (VT == MVT::f32 || (VT == MVT::f16 && Subtarget->hasMed3_16())) {
    ...
    if ((!K0->hasOneUse() ||
      ...
      return DAG.getNode(RVGPUISD::FMED3, SL, K0->getValueType(0),
...
SDValue RVTargetLowering::performMinMaxCombine(SDNode *N,
                                               DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
  ...
  if (Opc != RVGPUISD::FMIN_LEGACY && Opc != RVGPUISD::FMAX_LEGACY &&
      !VT.isVector() &&
  ...
  if (((Opc == ISD::FMINNUM && Op0.getOpcode() == ISD::FMAXNUM) ||
       ...
       (Opc == RVGPUISD::FMIN_LEGACY &&
        Op0.getOpcode() == RVGPUISD::FMAX_LEGACY)) &&
...
SDValue RVTargetLowering::performFMed3Combine(SDNode *N,
                                              DAGCombinerInfo &DCI) const {
  EVT VT = N->getValueType(0);
  ...
  if (isClampZeroToOne(Src0, Src1)) {
    ...
    // nans.
    // FIXME: Should this be allowing -0.0?
    return DAG.getNode(RVGPUISD::CLAMP, SL, VT, Src2);
  }
  ...
  if (Info->getMode().DX10Clamp) {
    ...
    if (isClampZeroToOne(Src1, Src2))
      return DAG.getNode(RVGPUISD::CLAMP, SL, VT, Src0);
  }

  return SDValue();
}

SDValue RVTargetLowering::performCvtPkRTZCombine(SDNode *N,
                                                 DAGCombinerInfo &DCI) const {
  SDValue Src0 = N->getOperand(0);
  SDValue Src1 = N->getOperand(1);
...
bool RVTargetLowering::shouldExpandVectorDynExt(SDNode *N) const {
  ...
  return RVTargetLowering::shouldExpandVectorDynExt(
      EltSize, NumElem, Idx->isDivergent(), getSubtarget());
}

SDValue RVTargetLowering::performExtractVectorEltCombine(
...
SDValue
RVTargetLowering::performInsertVectorEltCombine(SDNode *N,
                                                DAGCombinerInfo &DCI) const {
  SDValue Vec = N->getOperand(0);
...
SDValue RVTargetLowering::performFPRoundCombine(SDNode *N,
                                                DAGCombinerInfo &DCI) const {
  ...
  if (TruncSrc.getOpcode() != RVGPUISD::FMED3 ||
...
unsigned RVTargetLowering::getFusedOpcode(const SelectionDAG &DAG,
                                          const SDNode *N0,
...
SDValue RVTargetLowering::reassociateScalarOps(SDNode *N,
                                               SelectionDAG &DAG) const {
  EVT VT = N->getValueType(0);
...
static SDValue getMad64_32(SelectionDAG &DAG, const SDLoc &SL,
                           ...
                           bool Signed) {
  unsigned MadOpc = Signed ? RVGPUISD::MAD_I64_I32 : RVGPUISD::MAD_U64_U32;
...
// instead of a tree.
SDValue RVTargetLowering::tryFoldToMad64_32(SDNode *N,
                                            DAGCombinerInfo &DCI) const {
...
resolveSources(SelectionDAG &DAG, SDLoc SL,
  ...
  if (Srcs.size() == 1) {
    ...
    return DAG.getNode(RVGPUISD::PERM, SL, MVT::i32, EltVal, EltVal,
  ...
  while (true) {
    ...
    Perms.push_back(DAG.getNode(RVGPUISD::PERM, SL, MVT::i32, FirstVal,
                                SecondVal,
    ...
    if (SecondElt == Srcs.end()) {
      ...
      Perms.push_back(
          DAG.getNode(RVGPUISD::PERM, SL, MVT::i32, EltVal, EltVal,
...
static bool isMul(const SDValue Op) {
  ...
  return (Opcode == ISD::MUL || Opcode == RVGPUISD::MUL_U24 ||
          Opcode == RVGPUISD::MUL_I24);
}
...
SDValue RVTargetLowering::performAddCombine(SDNode *N,
                                            DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
  EVT VT = N->getValueType(0);
  SDLoc SL(N);
  ...
  if ((isMul(LHS) || isMul(RHS)) && Subtarget->hasDot7Insts() &&
    ...
    SDValue IID = DAG.getTargetConstant(*IsSigned ? Intrinsic::rvgpu_sdot4
                                                  : Intrinsic::rvgpu_udot4,
                                        SL, MVT::i64);
...
SDValue RVTargetLowering::performSubCombine(SDNode *N,
                                            DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
  EVT VT = N->getValueType(0);
...
SDValue RVTargetLowering::performAddCarrySubCarryCombine(SDNode *N,
  DAGCombinerInfo &DCI) const {
...
SDValue RVTargetLowering::performFAddCombine(SDNode *N,
                                             DAGCombinerInfo &DCI) const {
...
SDValue RVTargetLowering::performFSubCombine(SDNode *N,
                                             DAGCombinerInfo &DCI) const {
  if (DCI.getDAGCombineLevel() < AfterLegalizeDAG)
    return SDValue();
...
SDValue RVTargetLowering::performFDivCombine(SDNode *N,
                                             DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
  SDLoc SL(N);
  EVT VT = N->getValueType(0);
  ...
  if (const ConstantFPSDNode *CLHS = dyn_cast<ConstantFPSDNode>(LHS)) {
    ...
    if (CLHS->isExactlyValue(1.0) ||
      ...
      if (RHS.getOpcode() == ISD::FSQRT) {
        ...
        SDValue Rsq =
            DAG.getNode(RVGPUISD::RSQ, SL, VT, RHS.getOperand(0), Flags);
...
SDValue RVTargetLowering::performFMACombine(SDNode *N,
                                            DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
  EVT VT = N->getValueType(0);
  SDLoc SL(N);
  ...
  if (Options.AllowFPOpFusion == FPOpFusion::Fast || Options.UnsafeFPMath ||
    ...
    if ((Vec1 == Vec3 && Vec2 == Vec4) ||
        (Vec1 == Vec4 && Vec2 == Vec3)) {
      return DAG.getNode(RVGPUISD::FDOT2, SL, MVT::f32, Vec1, Vec2, FMAAcc,
...
SDValue RVTargetLowering::performSetCCCombine(SDNode *N,
                                              DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
  SDLoc SL(N);
  ...
  if ((CC == ISD::SETOEQ || CC == ISD::SETONE) && LHS.getOpcode() == ISD::FABS) {
    ...
    if (APF.isInfinity() && !APF.isNegative()) {
      ...
      return DAG.getNode(RVGPUISD::FP_CLASS, SL, MVT::i1, LHS.getOperand(0),
...
SDValue RVTargetLowering::performCvtF32UByteNCombine(SDNode *N,
                                                     DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
  SDLoc SL(N);
  unsigned Offset = N->getOpcode() - RVGPUISD::CVT_F32_UBYTE0;
  ...
  if (Shift.getOpcode() == ISD::SRL || Shift.getOpcode() == ISD::SHL) {
    ...
    if (auto *C = dyn_cast<ConstantSDNode>(Shift.getOperand(1))) {
      ...
      if (ShiftOffset < 32 && (ShiftOffset % 8) == 0) {
        return DAG.getNode(RVGPUISD::CVT_F32_UBYTE0 + ShiftOffset / 8, SL,
                           MVT::f32, Shifted);
      }
    }
  }
...
SDValue RVTargetLowering::performClampCombine(SDNode *N,
                                              DAGCombinerInfo &DCI) const {
  ...
  if (F < Zero ||
      (F.isNaN() && MF.getInfo<RVMachineFunctionInfo>()->getMode().DX10Clamp)) {
...
SDValue RVTargetLowering::PerformDAGCombine(SDNode *N,
                                            DAGCombinerInfo &DCI) const {
  if (getTargetMachine().getOptLevel() == CodeGenOptLevel::None)
    return SDValue();
  switch (N->getOpcode()) {
  ...
  case ISD::UMAX:
  case ISD::UMIN:
  case RVGPUISD::FMIN_LEGACY:
  case RVGPUISD::FMAX_LEGACY:
  ...
  case ISD::FSHR: {
    ...
    if (N->getValueType(0) == MVT::i32 && N->isDivergent() &&
        TII->pseudoToMCOpcode(RVGPU::V_PERM_B32_e64) != -1) {
  ...
  return RVGPUTargetLowering::PerformDAGCombine(N, DCI);
}
...
static unsigned SubIdx2Lane(unsigned Idx) {
  switch (Idx) {
  default: return ~0u;
  case RVGPU::sub0: return 0;
  case RVGPU::sub1: return 1;
  case RVGPU::sub2: return 2;
  case RVGPU::sub3: return 3;
  case RVGPU::sub4: return 4; // Possible with TFE/LWE
  }
}

/// Adjust the writemask of MIMG, VIMAGE or VSAMPLE instructions
SDNode *RVTargetLowering::adjustWritemask(MachineSDNode *&Node,
                                          SelectionDAG &DAG) const {
  unsigned Opcode = Node->getMachineOpcode();

  // Subtract 1 because the vdata output is not a MachineSDNode operand.
  int D16Idx = RVGPU::getNamedOperandIdx(Opcode, RVGPU::OpName::d16) - 1;
  if (D16Idx >= 0 && Node->getConstantOperandVal(D16Idx))
    return Node; // not implemented for D16

  SDNode *Users[5] = { nullptr };
  unsigned Lane = 0;
  unsigned DmaskIdx = RVGPU::getNamedOperandIdx(Opcode, RVGPU::OpName::dmask) - 1;
  unsigned OldDmask = Node->getConstantOperandVal(DmaskIdx);
  unsigned NewDmask = 0;
  unsigned TFEIdx = RVGPU::getNamedOperandIdx(Opcode, RVGPU::OpName::tfe) - 1;
  unsigned LWEIdx = RVGPU::getNamedOperandIdx(Opcode, RVGPU::OpName::lwe) - 1;
  ...
  int NewOpcode =
      RVGPU::getMaskedMIMGOp(Node->getMachineOpcode(), NewChannels);
  assert(NewOpcode != -1 &&
  ...
  for (unsigned i = 0, Idx = RVGPU::sub0; i < 5; ++i) {
    SDNode *User = Users[i];
    if (!User) {
    ...
    switch (Idx) {
    default: break;
    case RVGPU::sub0: Idx = RVGPU::sub1; break;
    case RVGPU::sub1: Idx = RVGPU::sub2; break;
    case RVGPU::sub2: Idx = RVGPU::sub3; break;
    case RVGPU::sub3: Idx = RVGPU::sub4; break;
    }
  }
...
SDNode *RVTargetLowering::legalizeTargetIndependentNode(SDNode *Node,
                                                        SelectionDAG &DAG) const {
  if (Node->getOpcode() == ISD::CopyToReg) {
    ...
    if (SrcVal.getValueType() == MVT::i1 && DestReg->getReg().isPhysical()) {
      ...
      SDValue VReg = DAG.getRegister(
        MRI.createVirtualRegister(&RVGPU::VReg_1RegClass), MVT::i1);
  ...
  for (unsigned i = 0; i < Node->getNumOperands(); ++i) {
    ...
    SDLoc DL(Node);
    Ops.push_back(SDValue(DAG.getMachineNode(RVGPU::S_MOV_B32, DL,
...
SDNode *RVTargetLowering::PostISelFolding(MachineSDNode *Node,
                                          SelectionDAG &DAG) const {
  const RVInstrInfo *TII = getSubtarget()->getInstrInfo();
  unsigned Opcode = Node->getMachineOpcode();
  ...
  if (TII->isImage(Opcode) && !TII->get(Opcode).mayStore() &&
      ...
      RVGPU::hasNamedOperand(Opcode, RVGPU::OpName::dmask)) {
    return adjustWritemask(Node, DAG);
  }

  if (Opcode == RVGPU::INSERT_SUBREG ||
      Opcode == RVGPU::REG_SEQUENCE) {
    legalizeTargetIndependentNode(Node, DAG);
    return Node;
  }

  switch (Opcode) {
  case RVGPU::V_DIV_SCALE_F32_e64:
  case RVGPU::V_DIV_SCALE_F64_e64: {
    ...
    if ((Src0.isMachineOpcode() &&
         Src0.getMachineOpcode() != RVGPU::IMPLICIT_DEF) &&
        (Src0 == Src1 || Src0 == Src2))
      break;
    ...
    if (Src0.isMachineOpcode() &&
      ...
      if (Src1.isMachineOpcode() &&
          Src1.getMachineOpcode() != RVGPU::IMPLICIT_DEF)
        Src0 = Src1;
      else if (Src2.isMachineOpcode() &&
               Src2.getMachineOpcode() != RVGPU::IMPLICIT_DEF)
        Src0 = Src2;
      else {
        assert(Src1.getMachineOpcode() == RVGPU::IMPLICIT_DEF);
        Src0 = UndefReg;
        Src1 = UndefReg;
      }
    } else
...
// img instruction.
void RVTargetLowering::AddIMGInit(MachineInstr &MI) const {
  const RVInstrInfo *TII = getSubtarget()->getInstrInfo();
  const RVRegisterInfo &TRI = TII->getRegisterInfo();
  MachineRegisterInfo &MRI = MI.getMF()->getRegInfo();
  MachineBasicBlock &MBB = *MI.getParent();

  MachineOperand *TFE = TII->getNamedOperand(MI, RVGPU::OpName::tfe);
  MachineOperand *LWE = TII->getNamedOperand(MI, RVGPU::OpName::lwe);
  MachineOperand *D16 = TII->getNamedOperand(MI, RVGPU::OpName::d16);
  ...
  MachineOperand *MO_Dmask = TII->getNamedOperand(MI, RVGPU::OpName::dmask);
  ...
  BuildMI(MBB, MI, DL, TII->get(RVGPU::IMPLICIT_DEF), PrevDst);
  for (; SizeLeft; SizeLeft--, CurrIdx++) {
    NewDst = MRI.createVirtualRegister(TII->getOpRegClass(MI, DstIdx));
    // Initialize dword
    Register SubReg = MRI.createVirtualRegister(&RVGPU::VGPR_32RegClass);
    BuildMI(MBB, MI, DL, TII->get(RVGPU::V_MOV_B32_e32), SubReg)
      .addImm(0);
    // Insert into the super-reg
    BuildMI(MBB, MI, DL, TII->get(TargetOpcode::INSERT_SUBREG), NewDst)
      .addReg(PrevDst)
      .addReg(SubReg)
      .addImm(RVRegisterInfo::getSubRegFromChannel(CurrIdx));
...
void RVTargetLowering::AdjustInstrPostInstrSelection(MachineInstr &MI,
                                                     SDNode *Node) const {
  const RVInstrInfo *TII = getSubtarget()->getInstrInfo();
  ...
  if (TII->isVOP3(MI.getOpcode())) {
    ...
    if (!MI.getDesc().operands().empty()) {
      ...
      const RVRegisterInfo *TRI = Subtarget->getRegisterInfo();
      int16_t Src2Idx = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::src2);
      for (auto I :
           {RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::src0),
            RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::src1), Src2Idx}) {
        if (I == -1)
          break;
      ...
      if (auto *Src2 = TII->getNamedOperand(MI, RVGPU::OpName::src2)) {
  ...
  if (TII->isImage(MI)) {
    if (!MI.mayStore())
      AddIMGInit(MI);
    TII->enforceOperandRCAlignment(MI, RVGPU::OpName::vaddr);
  }
}
...
static SDValue buildSMovImm32(SelectionDAG &DAG, const SDLoc &DL,
  ...
  return SDValue(DAG.getMachineNode(RVGPU::S_MOV_B32, DL, MVT::i32, K), 0);
}

MachineSDNode *RVTargetLowering::wrapAddr64Rsrc(SelectionDAG &DAG,
                                                const SDLoc &DL,
                                                SDValue Ptr) const {
  const RVInstrInfo *TII = getSubtarget()->getInstrInfo();
  ...
  const SDValue Ops0[] = {
    DAG.getTargetConstant(RVGPU::SGPR_64RegClassID, DL, MVT::i32),
    buildSMovImm32(DAG, DL, 0),
    DAG.getTargetConstant(RVGPU::sub0, DL, MVT::i32),
    buildSMovImm32(DAG, DL, TII->getDefaultRsrcDataFormat() >> 32),
    DAG.getTargetConstant(RVGPU::sub1, DL, MVT::i32)
  };

  SDValue SubRegHi = SDValue(DAG.getMachineNode(RVGPU::REG_SEQUENCE, DL,
                                                MVT::v2i32, Ops0), 0);

  // Combine the constants and the pointer.
  const SDValue Ops1[] = {
    DAG.getTargetConstant(RVGPU::SGPR_128RegClassID, DL, MVT::i32),
    Ptr,
    DAG.getTargetConstant(RVGPU::sub0_sub1, DL, MVT::i32),
    SubRegHi,
    DAG.getTargetConstant(RVGPU::sub2_sub3, DL, MVT::i32)
  };

  return DAG.getMachineNode(RVGPU::REG_SEQUENCE, DL, MVT::v4i32, Ops1);
}
...
MachineSDNode *RVTargetLowering::buildRSRC(SelectionDAG &DAG, const SDLoc &DL,
                                           SDValue Ptr, uint32_t RsrcDword1,
                                           uint64_t RsrcDword2And3) const {
  SDValue PtrLo = DAG.getTargetExtractSubreg(RVGPU::sub0, DL, MVT::i32, Ptr);
  SDValue PtrHi = DAG.getTargetExtractSubreg(RVGPU::sub1, DL, MVT::i32, Ptr);
  if (RsrcDword1) {
    PtrHi = SDValue(DAG.getMachineNode(RVGPU::S_OR_B32, DL, MVT::i32, PtrHi,
  ...
  const SDValue Ops[] = {
    ...
    DAG.getTargetConstant(RVGPU::sub1, DL, MVT::i32),
    DataLo,
    DAG.getTargetConstant(RVGPU::sub2, DL, MVT::i32),
    DataHi,
    DAG.getTargetConstant(RVGPU::sub3, DL, MVT::i32)
  };

  return DAG.getMachineNode(RVGPU::REG_SEQUENCE, DL, MVT::v4i32, Ops);
}
...
RVTargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI_,
                                               StringRef Constraint,
                                               MVT VT) const {
  const RVRegisterInfo *TRI = static_cast<const RVRegisterInfo *>(TRI_);
  ...
  if (Constraint.size() == 1) {
    ...
    case 'r':
      ...
      case 64:
        ...
        break;
      default:
        RC = RVRegisterInfo::getSGPRClassForBitWidth(BitWidth);
        if (!RC)
    ...
    if (RC) {
      ...
      if (RegName.consume_front("[")) {
        ...
        if (!Failed) {
          ...
          if (RVRegisterInfo::isVGPRClass(RC))
            RC = TRI->getVGPRClassForBitWidth(Width);
          else if (RVRegisterInfo::isSGPRClass(RC))
            RC = TRI->getSGPRClassForBitWidth(Width);
          else if (RVRegisterInfo::isAGPRClass(RC))
            RC = TRI->getAGPRClassForBitWidth(Width);
          if (RC) {
            Reg = TRI->getMatchingSuperReg(Reg, RVGPU::sub0, RC);
...
RVTargetLowering::ConstraintType
RVTargetLowering::getConstraintType(StringRef Constraint) const {
  if (Constraint.size() == 1) {
    switch (Constraint[0]) {
    default: break;
    case 's':
...
void RVTargetLowering::LowerAsmOperandForConstraint(SDValue Op,
                                                    StringRef Constraint,
                                                    std::vector<SDValue> &Ops,
                                                    SelectionDAG &DAG) const {
  if (isImmConstraint(Constraint)) {
...
bool RVTargetLowering::getAsmOperandConstVal(SDValue Op, uint64_t &Val) const {
...
bool RVTargetLowering::checkAsmConstraintVal(SDValue Op, StringRef Constraint,
                                             uint64_t Val) const {
  if (Constraint.size() == 1) {
    switch (Constraint[0]) {
    case 'I':
      return RVGPU::isInlinableIntLiteral(Val);
    case 'J':
      return isInt<16>(Val);
    case 'A':
    ...
    case 'C':
      return isUInt<32>(clearUnusedBits(Val, Op.getScalarValueSizeInBits())) ||
             RVGPU::isInlinableIntLiteral(Val);
    default:
      break;
    }
...
bool RVTargetLowering::checkAsmConstraintValA(SDValue Op,
                                              uint64_t Val,
                                              unsigned MaxSize) const {
  unsigned Size = std::min<unsigned>(Op.getScalarValueSizeInBits(), MaxSize);
  bool HasInv2Pi = Subtarget->hasInv2PiInlineImm();
  if ((Size == 16 && RVGPU::isInlinableLiteral16(Val, HasInv2Pi)) ||
      (Size == 32 && RVGPU::isInlinableLiteral32(Val, HasInv2Pi)) ||
      (Size == 64 && RVGPU::isInlinableLiteral64(Val, HasInv2Pi))) {
    return true;
  }
  return false;
}
...
static int getAlignedAGPRClassID(unsigned UnalignedClassID) {
  ...
  case RVGPU::VReg_64RegClassID:
    return RVGPU::VReg_64_Align2RegClassID;
  case RVGPU::VReg_96RegClassID:
    return RVGPU::VReg_96_Align2RegClassID;
  case RVGPU::VReg_128RegClassID:
    return RVGPU::VReg_128_Align2RegClassID;
  case RVGPU::VReg_160RegClassID:
    return RVGPU::VReg_160_Align2RegClassID;
  case RVGPU::VReg_192RegClassID:
    return RVGPU::VReg_192_Align2RegClassID;
  case RVGPU::VReg_224RegClassID:
    return RVGPU::VReg_224_Align2RegClassID;
  case RVGPU::VReg_256RegClassID:
    return RVGPU::VReg_256_Align2RegClassID;
  case RVGPU::VReg_288RegClassID:
    return RVGPU::VReg_288_Align2RegClassID;
  case RVGPU::VReg_320RegClassID:
    return RVGPU::VReg_320_Align2RegClassID;
  case RVGPU::VReg_352RegClassID:
    return RVGPU::VReg_352_Align2RegClassID;
  case RVGPU::VReg_384RegClassID:
    return RVGPU::VReg_384_Align2RegClassID;
  case RVGPU::VReg_512RegClassID:
    return RVGPU::VReg_512_Align2RegClassID;
  case RVGPU::VReg_1024RegClassID:
    return RVGPU::VReg_1024_Align2RegClassID;
  case RVGPU::AReg_64RegClassID:
    return RVGPU::AReg_64_Align2RegClassID;
  case RVGPU::AReg_96RegClassID:
    return RVGPU::AReg_96_Align2RegClassID;
  case RVGPU::AReg_128RegClassID:
    return RVGPU::AReg_128_Align2RegClassID;
  case RVGPU::AReg_160RegClassID:
    return RVGPU::AReg_160_Align2RegClassID;
  case RVGPU::AReg_192RegClassID:
    return RVGPU::AReg_192_Align2RegClassID;
  case RVGPU::AReg_256RegClassID:
    return RVGPU::AReg_256_Align2RegClassID;
  case RVGPU::AReg_512RegClassID:
    return RVGPU::AReg_512_Align2RegClassID;
  case RVGPU::AReg_1024RegClassID:
    return RVGPU::AReg_1024_Align2RegClassID;
  default:
    return -1;
  }
}
...
void RVTargetLowering::finalizeLowering(MachineFunction &MF) const {
  ...
  Register SReg = ST.isWave32()
                      ? RVGPU::SGPR_32RegClass.getRegister(MaxNumSGPRs - 1)
  ...
  if (Info->getStackPtrOffsetReg() != RVGPU::SP_REG)
    MRI.replaceRegWith(RVGPU::SP_REG, Info->getStackPtrOffsetReg());

  // We need to worry about replacing the default register with itself in case
  // of MIR testcases missing the MFI.
  if (Info->getScratchRSrcReg() != RVGPU::PRIVATE_RSRC_REG)
    MRI.replaceRegWith(RVGPU::PRIVATE_RSRC_REG, Info->getScratchRSrcReg());

  if (Info->getFrameOffsetReg() != RVGPU::FP_REG)
    MRI.replaceRegWith(RVGPU::FP_REG, Info->getFrameOffsetReg());
...
void RVTargetLowering::computeKnownBitsForTargetNode(const SDValue Op,
                                                     KnownBits &Known,
  ...
  return RVGPUTargetLowering::computeKnownBitsForTargetNode(
      Op, Known, DemandedElts, DAG, Depth);
}

void RVTargetLowering::computeKnownBitsForFrameIndex(
...
void RVTargetLowering::computeKnownBitsForTargetInstr(
...
Align RVTargetLowering::computeKnownAlignForTargetInstr(
...
Align RVTargetLowering::getPrefLoopAlignment(MachineLoop *ML) const {
  ...
  for (MachineLoop *P = ML->getParentLoop(); P; P = P->getParentLoop()) {
    if (MachineBasicBlock *Exit = P->getExitBlock()) {
      ...
      if (I != Exit->end() && I->getOpcode() == RVGPU::S_INST_PREFETCH)
        return CacheLineAlign;
    }
  }
  ...
  if (Pre && Exit) {
    ...
    if (PreTerm == Pre->begin() ||
        std::prev(PreTerm)->getOpcode() != RVGPU::S_INST_PREFETCH)
      BuildMI(*Pre, PreTerm, DebugLoc(), TII->get(RVGPU::S_INST_PREFETCH))
          .addImm(1); // prefetch 2 lines behind PC

    auto ExitHead = Exit->getFirstNonDebugInstr();
    if (ExitHead == Exit->end() ||
        ExitHead->getOpcode() != RVGPU::S_INST_PREFETCH)
      BuildMI(*Exit, ExitHead, DebugLoc(), TII->get(RVGPU::S_INST_PREFETCH))
...
bool RVTargetLowering::isSDNodeSourceOfDivergence(const SDNode *N,
  ...
  case ISD::LOAD: {
    ...
    return AS == RVGPUAS::PRIVATE_ADDRESS || AS == RVGPUAS::FLAT_ADDRESS;
  }
  case ISD::CALLSEQ_END:
    return true;
  case ISD::INTRINSIC_WO_CHAIN:
    return RVGPU::isIntrinsicSourceOfDivergence(
        cast<ConstantSDNode>(N->getOperand(0))->getZExtValue());
  case ISD::INTRINSIC_W_CHAIN:
    return RVGPU::isIntrinsicSourceOfDivergence(
        cast<ConstantSDNode>(N->getOperand(1))->getZExtValue());
  case RVGPUISD::ATOMIC_CMP_SWAP:
  case RVGPUISD::ATOMIC_LOAD_FMIN:
  case RVGPUISD::ATOMIC_LOAD_FMAX:
  case RVGPUISD::BUFFER_ATOMIC_SWAP:
  case RVGPUISD::BUFFER_ATOMIC_ADD:
  case RVGPUISD::BUFFER_ATOMIC_SUB:
  case RVGPUISD::BUFFER_ATOMIC_SMIN:
  case RVGPUISD::BUFFER_ATOMIC_UMIN:
  case RVGPUISD::BUFFER_ATOMIC_SMAX:
  case RVGPUISD::BUFFER_ATOMIC_UMAX:
  case RVGPUISD::BUFFER_ATOMIC_AND:
  case RVGPUISD::BUFFER_ATOMIC_OR:
  case RVGPUISD::BUFFER_ATOMIC_XOR:
  case RVGPUISD::BUFFER_ATOMIC_INC:
  case RVGPUISD::BUFFER_ATOMIC_DEC:
  case RVGPUISD::BUFFER_ATOMIC_CMPSWAP:
  case RVGPUISD::BUFFER_ATOMIC_CSUB:
  case RVGPUISD::BUFFER_ATOMIC_FADD:
  case RVGPUISD::BUFFER_ATOMIC_FMIN:
  case RVGPUISD::BUFFER_ATOMIC_FMAX:
...
bool RVTargetLowering::denormalsEnabledForType(const SelectionDAG &DAG,
                                               EVT VT) const {
...
bool RVTargetLowering::denormalsEnabledForType(LLT Ty,
...
bool RVTargetLowering::isKnownNeverNaNForTargetNode(SDValue Op,
                                                    const SelectionDAG &DAG,
                                                    bool SNaN,
                                                    unsigned Depth) const {
  if (Op.getOpcode() == RVGPUISD::CLAMP) {
    const MachineFunction &MF = DAG.getMachineFunction();
    const RVMachineFunctionInfo *Info = MF.getInfo<RVMachineFunctionInfo>();
  ...
  return RVGPUTargetLowering::isKnownNeverNaNForTargetNode(Op, DAG,
                                                            SNaN, Depth);
}
...
// The rvgpu-unsafe-fp-atomics attribute enables generation of unsafe
...
RVTargetLowering::shouldExpandAtomicRMWInIR(AtomicRMWInst *RMW) const {
  ...
  case AtomicRMWInst::FAdd: {
    ...
    if (AS == RVGPUAS::LOCAL_ADDRESS && Subtarget->hasLDSFPAtomicAdd()) {
      if (!Ty->isDoubleTy())
        return AtomicExpansionKind::None;
      ...
      return RMW->getFunction()
                         ->getFnAttribute("rvgpu-unsafe-fp-atomics")
  ...
  return RVGPUTargetLowering::shouldExpandAtomicRMWInIR(RMW);
}

TargetLowering::AtomicExpansionKind
RVTargetLowering::shouldExpandAtomicLoadInIR(LoadInst *LI) const {
  return LI->getPointerAddressSpace() == RVGPUAS::PRIVATE_ADDRESS
             ? AtomicExpansionKind::NotAtomic
             : AtomicExpansionKind::None;
}

TargetLowering::AtomicExpansionKind
RVTargetLowering::shouldExpandAtomicStoreInIR(StoreInst *SI) const {
  return SI->getPointerAddressSpace() == RVGPUAS::PRIVATE_ADDRESS
             ? AtomicExpansionKind::NotAtomic
             : AtomicExpansionKind::None;
}

TargetLowering::AtomicExpansionKind
RVTargetLowering::shouldExpandAtomicCmpXchgInIR(AtomicCmpXchgInst *CmpX) const {
  return CmpX->getPointerAddressSpace() == RVGPUAS::PRIVATE_ADDRESS
             ? AtomicExpansionKind::NotAtomic
             : AtomicExpansionKind::None;
}

const TargetRegisterClass *
RVTargetLowering::getRegClassFor(MVT VT, bool isDivergent) const {
  const TargetRegisterClass *RC = TargetLoweringBase::getRegClassFor(VT, false);
  const RVRegisterInfo *TRI = Subtarget->getRegisterInfo();
  if (RC == &RVGPU::VReg_1RegClass && !isDivergent)
    return Subtarget->getWavefrontSize() == 64 ? &RVGPU::SReg_64RegClass
                                               : &RVGPU::SReg_32RegClass;
...
bool RVTargetLowering::requiresUniformRegister(MachineFunction &MF,
                                               const Value *V) const {
  if (const CallInst *CI = dyn_cast<CallInst>(V)) {
    if (CI->isInlineAsm()) {
      ...
      const RVRegisterInfo *SIRI = Subtarget->getRegisterInfo();
...
bool RVTargetLowering::hasMemSDNodeUser(SDNode *N) const {
...
bool RVTargetLowering::isReassocProfitable(SelectionDAG &DAG, SDValue N0,
                                           SDValue N1) const {
  if (!N0.hasOneUse())
    return false;
...
bool RVTargetLowering::isReassocProfitable(MachineRegisterInfo &MRI,
                                           Register N0, Register N1) const {
  return MRI.hasOneNonDBGUse(N0); // FIXME: handle regbanks
}

MachineMemOperand::Flags
RVTargetLowering::getTargetMMOFlags(const Instruction &I) const {
  // Propagate metadata set by RVGPUAnnotateUniformValues to the MMO of a load.
  if (I.getMetadata("rvgpu.noclobber"))
    return MONoClobber;
  return MachineMemOperand::MONone;
}

bool RVTargetLowering::checkForPhysRegDependency(
  ...
  if (II.isCompare() && II.hasImplicitDefOfPhysReg(RVGPU::SCC)) {
    PhysReg = RVGPU::SCC;
    const TargetRegisterClass *RC =
...
void RVTargetLowering::emitExpandAtomicRMW(AtomicRMWInst *AI) const {
  assert(Subtarget->hasAtomicFaddInsts() &&
         "target should have atomic fadd instructions");
  assert(AI->getType()->isFloatTy() &&
         AI->getPointerAddressSpace() == RVGPUAS::FLAT_ADDRESS &&
  ...
  //   %is.shared = call i1 @llvm.rvgpu.is.shared(ptr %addr)
  ...
  //   %is.private = call i1 @llvm.rvgpu.is.private(ptr %int8ptr)
  ...
  CallInst *IsShared = Builder.CreateIntrinsic(Intrinsic::rvgpu_is_shared, {},
                                               {Addr}, nullptr, "is.shared");
  Builder.CreateCondBr(IsShared, SharedBB, CheckPrivateBB);

  Builder.SetInsertPoint(SharedBB);
  Value *CastToLocal = Builder.CreateAddrSpaceCast(
      Addr, PointerType::get(Ctx, RVGPUAS::LOCAL_ADDRESS));
  ...
  CallInst *IsPrivate = Builder.CreateIntrinsic(
      Intrinsic::rvgpu_is_private, {}, {Addr}, nullptr, "is.private");
  Builder.CreateCondBr(IsPrivate, PrivateBB, GlobalBB);

  Builder.SetInsertPoint(PrivateBB);
  Value *CastToPrivate = Builder.CreateAddrSpaceCast(
      Addr, PointerType::get(Ctx, RVGPUAS::PRIVATE_ADDRESS));
  Value *LoadedPrivate =
  ...
  Value *CastToGlobal = Builder.CreateAddrSpaceCast(
      Addr, PointerType::get(Ctx, RVGPUAS::GLOBAL_ADDRESS));
...
LoadInst *
RVTargetLowering::lowerIdempotentRMWIntoFencedLoad(AtomicRMWInst *AI) const {
  IRBuilder<> Builder(AI);
