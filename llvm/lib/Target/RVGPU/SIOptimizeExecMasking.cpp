...
static Register isLogicalOpOnExec(const MachineInstr &MI) {
  ...
  case RVGPU::S_XNOR_B64: {
    ...
    if (Src1.isReg() && Src1.getReg() == RVGPU::EXEC)
      return MI.getOperand(0).getReg();
    const MachineOperand &Src2 = MI.getOperand(2);
    if (Src2.isReg() && Src2.getReg() == RVGPU::EXEC)
      return MI.getOperand(0).getReg();
    break;
  }
  ...
  case RVGPU::S_XNOR_B32: {
    const MachineOperand &Src1 = MI.getOperand(1);
    if (Src1.isReg() && Src1.getReg() == RVGPU::EXEC_LO)
      return MI.getOperand(0).getReg();
    const MachineOperand &Src2 = MI.getOperand(2);
    if (Src2.isReg() && Src2.getReg() == RVGPU::EXEC_LO)
      return MI.getOperand(0).getReg();
    break;
  }
  }
...
static unsigned getSaveExecOp(unsigned Opc) {
  switch (Opc) {
  case RVGPU::S_AND_B64:
    return RVGPU::S_AND_SAVEEXEC_B64;
  case RVGPU::S_OR_B64:
    return RVGPU::S_OR_SAVEEXEC_B64;
  case RVGPU::S_XOR_B64:
    return RVGPU::S_XOR_SAVEEXEC_B64;
  case RVGPU::S_ANDN2_B64:
    return RVGPU::S_ANDN2_SAVEEXEC_B64;
  case RVGPU::S_ORN2_B64:
    return RVGPU::S_ORN2_SAVEEXEC_B64;
  case RVGPU::S_NAND_B64:
    return RVGPU::S_NAND_SAVEEXEC_B64;
  case RVGPU::S_NOR_B64:
    return RVGPU::S_NOR_SAVEEXEC_B64;
  case RVGPU::S_XNOR_B64:
    return RVGPU::S_XNOR_SAVEEXEC_B64;
  case RVGPU::S_AND_B32:
    return RVGPU::S_AND_SAVEEXEC_B32;
  case RVGPU::S_OR_B32:
    return RVGPU::S_OR_SAVEEXEC_B32;
  case RVGPU::S_XOR_B32:
    return RVGPU::S_XOR_SAVEEXEC_B32;
  case RVGPU::S_ANDN2_B32:
    return RVGPU::S_ANDN2_SAVEEXEC_B32;
  case RVGPU::S_ORN2_B32:
    return RVGPU::S_ORN2_SAVEEXEC_B32;
  case RVGPU::S_NAND_B32:
    return RVGPU::S_NAND_SAVEEXEC_B32;
  case RVGPU::S_NOR_B32:
    return RVGPU::S_NOR_SAVEEXEC_B32;
  case RVGPU::S_XNOR_B32:
    return RVGPU::S_XNOR_SAVEEXEC_B32;
  default:
...
bool SIOptimizeExecMasking::removeTerminatorBit(MachineInstr &MI) const {
  ...
  case RVGPU::S_MOV_B32_term: {
    ...
    MI.setDesc(TII->get(RegSrc ? RVGPU::COPY : RVGPU::S_MOV_B32));
    return true;
  }
  case RVGPU::S_MOV_B64_term: {
    bool RegSrc = MI.getOperand(1).isReg();
    MI.setDesc(TII->get(RegSrc ? RVGPU::COPY : RVGPU::S_MOV_B64));
    return true;
  }
  case RVGPU::S_XOR_B64_term: {
    // This is only a terminator to get the correct spill code placement during
    // register allocation.
    MI.setDesc(TII->get(RVGPU::S_XOR_B64));
    return true;
  }
  case RVGPU::S_XOR_B32_term: {
    // This is only a terminator to get the correct spill code placement during
    // register allocation.
    MI.setDesc(TII->get(RVGPU::S_XOR_B32));
    return true;
  }
  case RVGPU::S_OR_B64_term: {
    // This is only a terminator to get the correct spill code placement during
    // register allocation.
    MI.setDesc(TII->get(RVGPU::S_OR_B64));
    return true;
  }
  case RVGPU::S_OR_B32_term: {
    // This is only a terminator to get the correct spill code placement during
    // register allocation.
    MI.setDesc(TII->get(RVGPU::S_OR_B32));
    return true;
  }
  case RVGPU::S_ANDN2_B64_term: {
    // This is only a terminator to get the correct spill code placement during
    // register allocation.
    MI.setDesc(TII->get(RVGPU::S_ANDN2_B64));
    return true;
  }
  case RVGPU::S_ANDN2_B32_term: {
    // This is only a terminator to get the correct spill code placement during
    // register allocation.
    MI.setDesc(TII->get(RVGPU::S_ANDN2_B32));
    return true;
  }
  case RVGPU::S_AND_B64_term: {
    // This is only a terminator to get the correct spill code placement during
    // register allocation.
    MI.setDesc(TII->get(RVGPU::S_AND_B64));
    return true;
  }
  case RVGPU::S_AND_B32_term: {
    // This is only a terminator to get the correct spill code placement during
    // register allocation.
    MI.setDesc(TII->get(RVGPU::S_AND_B32));
    return true;
  }
  default:
    return false;
  ...
  for (MachineBasicBlock &MBB : *MF) {
    ...
    for (MachineBasicBlock::iterator
      ...
      if (J->modifiesRegister(CopyToExec, TRI)) {
        ...
        if (SaveExecOp == RVGPU::INSTRUCTION_LIST_END)
          break;
    ...
    for (MachineInstr *OtherInst : OtherUseInsts) {
      OtherInst->substituteRegister(CopyToExec, Exec, RVGPU::NoSubRegister,
                                    *TRI);
    }
...
bool SIOptimizeExecMasking::optimizeVCMPSaveExecSequence(
  ...
  const int NewOpcode = RVGPU::getVCMPXOpFromVCMP(VCmp.getOpcode());

  if (NewOpcode == -1)
    return false;

  MachineOperand *Src0 = TII->getNamedOperand(VCmp, RVGPU::OpName::src0);
  MachineOperand *Src1 = TII->getNamedOperand(VCmp, RVGPU::OpName::src1);
  ...
  if (!SaveExecInstr.uses().empty()) {
    ...
    unsigned MovOpcode = IsSGPR32 ? RVGPU::S_MOV_B32 : RVGPU::S_MOV_B64;
  ...
  TryAddImmediateValueFromNamedOperand(RVGPU::OpName::src0_modifiers);
  Builder.add(*Src0);

  TryAddImmediateValueFromNamedOperand(RVGPU::OpName::src1_modifiers);
  Builder.add(*Src1);

  TryAddImmediateValueFromNamedOperand(RVGPU::OpName::clamp);
...
void SIOptimizeExecMasking::tryRecordVCmpxAndSaveexecSequence(
  ...
  MachineOperand *SaveExecSrc0 = TII->getNamedOperand(MI, RVGPU::OpName::src0);
  ...
  VCmp = findInstrBackwards(
      MI,
      [&](MachineInstr *Check) {
        return RVGPU::getVCMPXOpFromVCMP(Check->getOpcode()) != -1 &&
  ...
  MachineOperand *VCmpDest = TII->getNamedOperand(*VCmp, RVGPU::OpName::sdst);
...
void SIOptimizeExecMasking::tryRecordOrSaveexecXorSequence(MachineInstr &MI) {
  const unsigned XorOpcode =
      ST->isWave32() ? RVGPU::S_XOR_B32 : RVGPU::S_XOR_B64;
...
bool SIOptimizeExecMasking::optimizeOrSaveexecXorSequences() {
  ...
  bool Changed = false;
  const unsigned Andn2Opcode = ST->isWave32() ? RVGPU::S_ANDN2_SAVEEXEC_B32
                                              : RVGPU::S_ANDN2_SAVEEXEC_B64;
