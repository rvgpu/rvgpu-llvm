...
//
// RV OpenCL only sets flush none and reports CL_FP_DENORM for double
...
extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeRVGPUAsmPrinter() {
  TargetRegistry::RegisterAsmPrinter(getTheRVGPUTarget(),
...
void RVGPUAsmPrinter::emitFunctionBodyStart() {
  ...
  if (STM.isMesaKernel(F) &&
      (F.getCallingConv() == CallingConv::RVGPU_KERNEL ||
       F.getCallingConv() == CallingConv::SPIR_KERNEL)) {
    rv_kernel_code_t KernelCode;
    getRvKernelCode(KernelCode, CurrentProgramInfo, *MF);
    getTargetStreamer()->EmitRVKernelCodeT(KernelCode);
  }
  ...
  if (MFI.getNumKernargPreloadedSGPRs() > 0) {
    assert(RVGPU::hasKernargPreload(STM));
...
void RVGPUAsmPrinter::emitImplicitDef(const MachineInstr *MI) const {
  ...
  if (MI->getAsmPrinterFlags() & RVGPU::SGPR_SPILL)
...
void RVGPUAsmPrinter::emitFunctionEntryLabel() {
  ...
  const RVMachineFunctionInfo *MFI = MF->getInfo<RVMachineFunctionInfo>();
  const RVSubtarget &STM = MF->getSubtarget<RVSubtarget>();
  if (MFI->isEntryFunction() && STM.isRvHsaOrMesa(MF->getFunction())) {
...
void RVGPUAsmPrinter::emitGlobalVariable(const GlobalVariable *GV) {
  if (GV->getAddressSpace() == RVGPUAS::LOCAL_ADDRESS) {
    ...
    TS->emitRVGPULDS(GVSym, Size, Alignment);
    return;
  }
...
bool RVGPUAsmPrinter::runOnMachineFunction(MachineFunction &MF) {
  ...
  if (!STM.isRvHsaOS() && !STM.isRvPalOS()) {
    MCSectionELF *ConfigSection =
        Context.getELFSection(".RVGPU.config", ELF::SHT_PROGBITS, 0);
  ...
  if (isVerbose()) {
    ...
    if (!MFI->isEntryFunction()) {
      ...
      emitCommonFunctionComments(
          ...
          Info.getTotalNumSGPRs(MF.getSubtarget<RVSubtarget>()),
...
void RVGPUAsmPrinter::initializeTargetID(const Module &M) {
  ...
  for (auto &F : M) {
    ...
    const RVSubtarget &STM = TM.getSubtarget<RVSubtarget>(F);
    const IsaInfo::RVGPUTargetID &STMTargetID = STM.getTargetID();
...
uint64_t RVGPUAsmPrinter::getFunctionCodeSize(const MachineFunction &MF) const {
  const RVSubtarget &STM = MF.getSubtarget<RVSubtarget>();
  const RVInstrInfo *TII = STM.getInstrInfo();
...
void RVGPUAsmPrinter::getRVProgramInfo(RVProgramInfo &ProgInfo,
  ...
  // duplicated in part in RVGPUAsmParser::calculateGPRBlocks, and could be
  // unified.
  ...
  if (STM.getGeneration() >= RVGPUSubtarget::VOLCANIC_ISLANDS &&
      !STM.hasSGPRInitBug()) {
  ...
  if (isShader(F.getCallingConv())) {
    bool IsPixelShader =
        F.getCallingConv() == CallingConv::RVGPU_PS && !STM.isRvHsaOS();
    ...
    if (IsPixelShader) {
      ...
      assert((InputEna || InputAddr) &&
             ...
             "never both be 0 for RVGPU_PS shaders");
  ...
  if (STM.getGeneration() <= RVGPUSubtarget::SEA_ISLANDS ||
      STM.hasSGPRInitBug()) {
    unsigned MaxAddressableNumSGPRs = STM.getAddressableNumSGPRs();
    if (ProgInfo.NumSGPR > MaxAddressableNumSGPRs) {
  ...
  if (STM.hasSGPRInitBug()) {
    ProgInfo.NumSGPR =
        RVGPU::IsaInfo::FIXED_NUM_SGPRS_FOR_INIT_BUG;
...
static unsigned getRsrcReg(CallingConv::ID CallConv) {
  switch (CallConv) {
  default: [[fallthrough]];
  case CallingConv::RVGPU_CS: return R_00B848_COMPUTE_PGM_RSRC1;
  case CallingConv::RVGPU_LS: return R_00B528_SPI_SHADER_PGM_RSRC1_LS;
  case CallingConv::RVGPU_HS: return R_00B428_SPI_SHADER_PGM_RSRC1_HS;
  case CallingConv::RVGPU_ES: return R_00B328_SPI_SHADER_PGM_RSRC1_ES;
  case CallingConv::RVGPU_GS: return R_00B228_SPI_SHADER_PGM_RSRC1_GS;
  case CallingConv::RVGPU_VS: return R_00B128_SPI_SHADER_PGM_RSRC1_VS;
  case CallingConv::RVGPU_PS: return R_00B028_SPI_SHADER_PGM_RSRC1_PS;
  }
}
...
// is RVPAL.  It stores each compute/SPI register setting and other PAL
// metadata items into the RVPALMD::Metadata, combining with any provided by the
...
void RVGPUAsmPrinter::emitPALFunctionMetadata(const MachineFunction &MF) {
  auto *MD = getTargetStreamer()->getPALMetadata();
  ...
  MD->setRsrc1(CallingConv::RVGPU_CS,
               CurrentProgramInfo.getPGMRSrc1(CallingConv::RVGPU_CS, ST));
  MD->setRsrc2(CallingConv::RVGPU_CS, CurrentProgramInfo.getComputePGMRSrc2());
...
static rv_element_byte_size_t getElementByteSizeValue(unsigned Size) {
  switch (Size) {
  case 4:
    return RV_ELEMENT_4_BYTES;
  case 8:
    return RV_ELEMENT_8_BYTES;
  case 16:
    return RV_ELEMENT_16_BYTES;
  default:
...
void RVGPUAsmPrinter::getRvKernelCode(rv_kernel_code_t &Out,
  ...
  RVGPU::initDefaultRVKernelCodeT(Out, &STM);

  Out.compute_pgm_resource_registers =
      CurrentProgramInfo.getComputePGMRSrc1(STM) |
      (CurrentProgramInfo.getComputePGMRSrc2() << 32);
  Out.code_properties |= RV_CODE_PROPERTY_IS_PTR64;

  if (CurrentProgramInfo.DynamicCallStack)
    Out.code_properties |= RV_CODE_PROPERTY_IS_DYNAMIC_CALLSTACK;

  RV_HSA_BITS_SET(Out.code_properties,
  ...
  if (UserSGPRInfo.hasKernargSegmentPtr())
    Out.code_properties |= RV_CODE_PROPERTY_ENABLE_SGPR_KERNARG_SEGMENT_PTR;

  if (UserSGPRInfo.hasDispatchID())
    Out.code_properties |= RV_CODE_PROPERTY_ENABLE_SGPR_DISPATCH_ID;

  if (UserSGPRInfo.hasFlatScratchInit())
    Out.code_properties |= RV_CODE_PROPERTY_ENABLE_SGPR_FLAT_SCRATCH_INIT;

  if (UserSGPRInfo.hasDispatchPtr())
    Out.code_properties |= RV_CODE_PROPERTY_ENABLE_SGPR_DISPATCH_PTR;

  if (STM.isXNACKEnabled())
    Out.code_properties |= RV_CODE_PROPERTY_IS_XNACK_SUPPORTED;
