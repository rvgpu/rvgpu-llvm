...
public:
  ...
  void processBlockPhase1(MachineBasicBlock &MBB, const RVInstrInfo *TII);

  void processBlockPhase2(MachineBasicBlock &MBB, const RVInstrInfo *TII);

  void processBlockPhase3(MachineBasicBlock &MBB, const RVInstrInfo *TII);

  Status getInstructionMode(MachineInstr &MI, const RVInstrInfo *TII);

  void insertSetreg(MachineBasicBlock &MBB, MachineInstr *I,
                    const RVInstrInfo *TII, Status InstrMode);
};
...
Status SIModeRegister::getInstructionMode(MachineInstr &MI,
  ...
  if (TII->usesFPDPRounding(MI) ||
      MI.getOpcode() == RVGPU::FPTRUNC_UPWARD_PSEUDO ||
      MI.getOpcode() == RVGPU::FPTRUNC_DOWNWARD_PSEUDO) {
    switch (MI.getOpcode()) {
    case RVGPU::V_INTERP_P1LL_F16:
    case RVGPU::V_INTERP_P1LV_F16:
    case RVGPU::V_INTERP_P2_F16:
    ...
    case RVGPU::FPTRUNC_UPWARD_PSEUDO: {
      ...
      if (TII->getSubtarget().hasTrue16BitInsts()) {
        ...
        MI.setDesc(TII->get(RVGPU::V_CVT_F16_F32_t16_e64));
        ...
        MI.setDesc(TII->get(RVGPU::V_CVT_F16_F32_e32));
      return Status(FP_ROUND_MODE_DP(3),
                    FP_ROUND_MODE_DP(FP_ROUND_ROUND_TO_INF));
    }
    case RVGPU::FPTRUNC_DOWNWARD_PSEUDO: {
      // Replacing the pseudo by a real instruction in place
      if (TII->getSubtarget().hasTrue16BitInsts()) {
        MachineBasicBlock &MBB = *MI.getParent();
        MachineInstrBuilder B(*MBB.getParent(), MI);
...
void SIModeRegister::insertSetreg(MachineBasicBlock &MBB, MachineInstr *MI,
  ...
  while (InstrMode.Mask) {
    ...
    BuildMI(MBB, MI, nullptr, TII->get(RVGPU::S_SETREG_IMM32_B32))
        .addImm(Value)
        .addImm(((Width - 1) << RVGPU::Hwreg::WIDTH_M1_SHIFT_) |
                (Offset << RVGPU::Hwreg::OFFSET_SHIFT_) |
                (RVGPU::Hwreg::ID_MODE << RVGPU::Hwreg::ID_SHIFT_));
    ++NumSetregInserted;
    Changed = true;
...
void SIModeRegister::processBlockPhase1(MachineBasicBlock &MBB,
  ...
  for (MachineInstr &MI : MBB) {
    ...
    if (MI.getOpcode() == RVGPU::S_SETREG_B32 ||
        MI.getOpcode() == RVGPU::S_SETREG_B32_mode ||
        MI.getOpcode() == RVGPU::S_SETREG_IMM32_B32 ||
        MI.getOpcode() == RVGPU::S_SETREG_IMM32_B32_mode) {
      ...
      unsigned Dst = TII->getNamedOperand(MI, RVGPU::OpName::simm16)->getImm();
      if (((Dst & RVGPU::Hwreg::ID_MASK_) >> RVGPU::Hwreg::ID_SHIFT_) !=
          RVGPU::Hwreg::ID_MODE)
        continue;

      unsigned Width = ((Dst & RVGPU::Hwreg::WIDTH_M1_MASK_) >>
                        RVGPU::Hwreg::WIDTH_M1_SHIFT_) +
                       1;
      unsigned Offset =
          (Dst & RVGPU::Hwreg::OFFSET_MASK_) >> RVGPU::Hwreg::OFFSET_SHIFT_;
      ...
      // as unknown.
      if (MI.getOpcode() == RVGPU::S_SETREG_IMM32_B32 ||
          MI.getOpcode() == RVGPU::S_SETREG_IMM32_B32_mode) {
        unsigned Val = TII->getNamedOperand(MI, RVGPU::OpName::imm)->getImm();
