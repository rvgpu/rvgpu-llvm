//==- RVMachineFunctionInfo.h - RVMachineFunctionInfo interface --*- C++ -*-==//
//
...
#ifndef LLVM_LIB_TARGET_RVGPU_SIMACHINEFUNCTIONINFO_H
#define LLVM_LIB_TARGET_RVGPU_SIMACHINEFUNCTIONINFO_H
...
class RVGPUPseudoSourceValue : public PseudoSourceValue {
public:
  enum RVGPUPSVKind : unsigned {
    PSVImage = PseudoSourceValue::TargetCustom,
    GWSResource
  };

protected:
  RVGPUPseudoSourceValue(unsigned Kind, const RVGPUTargetMachine &TM)
...
class RVGPUGWSResourcePseudoSourceValue final : public RVGPUPseudoSourceValue {
public:
  explicit RVGPUGWSResourcePseudoSourceValue(const RVGPUTargetMachine &TM)
      : RVGPUPseudoSourceValue(GWSResource, TM) {}
...
struct RVMachineFunctionInfo final : public yaml::MachineFunctionInfo {
  ...
  RVMachineFunctionInfo() = default;
  RVMachineFunctionInfo(const llvm::RVMachineFunctionInfo &,
...
template <> struct MappingTraits<RVMachineFunctionInfo> {
  static void mapping(IO &YamlIO, RVMachineFunctionInfo &MFI) {
...
class RVMachineFunctionInfo final : public RVGPUMachineFunction,
  ...
  Register ScratchRSrcReg = RVGPU::PRIVATE_RSRC_REG;
  ...
  Register FrameOffsetReg = RVGPU::FP_REG;
  ...
  Register StackPtrOffsetReg = RVGPU::SP_REG;
  ...
  const RVGPUGWSResourcePseudoSourceValue GWSResourcePSV;
...
private:
  ...
  // for RVPAL OS type. 0xffffffff represents no hard-wired high half, since
...
private:
  ...
  DenseMap<int, std::vector<RVRegisterInfo::SpilledReg>>
...
public:
  ...
  bool initializeBaseYamlFields(const yaml::RVMachineFunctionInfo &YamlMFI,
  ...
  ArrayRef<RVRegisterInfo::SpilledReg>
  ...
  MCPhysReg getVGPRToAGPRSpill(int FrameIndex, unsigned Lane) const {
    ...
    return (I == VGPRToAGPRSpills.end()) ? (MCPhysReg)RVGPU::NoRegister
                                         : I->second.Lanes[Lane];
  }
  ...
  int getScavengeFI(MachineFrameInfo &MFI, const RVRegisterInfo &TRI);
  ...
  // Add user SGPRs.
  Register addPrivateSegmentBuffer(const RVRegisterInfo &TRI);
  Register addDispatchPtr(const RVRegisterInfo &TRI);
  Register addQueuePtr(const RVRegisterInfo &TRI);
  Register addKernargSegmentPtr(const RVRegisterInfo &TRI);
  Register addDispatchID(const RVRegisterInfo &TRI);
  Register addFlatScratchInit(const RVRegisterInfo &TRI);
  Register addImplicitBufferPtr(const RVRegisterInfo &TRI);
  Register addLDSKernelId();
  SmallVectorImpl<MCRegister> *
  addPreloadedKernArg(const RVRegisterInfo &TRI, const TargetRegisterClass *RC,
  ...
  RVGPUFunctionArgInfo &getArgInfo() {
    return ArgInfo;
  }

  const RVGPUFunctionArgInfo &getArgInfo() const {
    return ArgInfo;
  }
  ...
  MCRegister getPreloadedReg(RVGPUFunctionArgInfo::PreloadedValue Value) const {
  ...
  // Note the unset value for this is RVGPU::SP_REG rather than
  ...
  const RVGPUGWSResourcePseudoSourceValue *
  getGWSPSV(const RVGPUTargetMachine &TM) {
...
#endif // LLVM_LIB_TARGET_RVGPU_SIMACHINEFUNCTIONINFO_H
