//===-- RVGPUInstPrinter.cpp - RVGPU MC Inst -> ASM ---------------------===//
//
...
void RVGPUInstPrinter::printU4ImmOperand(const MCInst *MI, unsigned OpNo,
                                          const MCSubtargetInfo &STI,
                                          raw_ostream &O) {
  O << formatHex(MI->getOperand(OpNo).getImm() & 0xf);
}

void RVGPUInstPrinter::printU16ImmOperand(const MCInst *MI, unsigned OpNo,
                                           const MCSubtargetInfo &STI,
                                           raw_ostream &O) {
...
void RVGPUInstPrinter::printU4ImmDecOperand(const MCInst *MI, unsigned OpNo,
                                             raw_ostream &O) {
  O << formatDec(MI->getOperand(OpNo).getImm() & 0xf);
}

void RVGPUInstPrinter::printU8ImmDecOperand(const MCInst *MI, unsigned OpNo,
                                             raw_ostream &O) {
  O << formatDec(MI->getOperand(OpNo).getImm() & 0xff);
}

void RVGPUInstPrinter::printU16ImmDecOperand(const MCInst *MI, unsigned OpNo,
                                              raw_ostream &O) {
  O << formatDec(MI->getOperand(OpNo).getImm() & 0xffff);
}

void RVGPUInstPrinter::printU32ImmOperand(const MCInst *MI, unsigned OpNo,
                                           const MCSubtargetInfo &STI,
                                           raw_ostream &O) {
  O << formatHex(MI->getOperand(OpNo).getImm() & 0xffffffff);
}

void RVGPUInstPrinter::printNamedBit(const MCInst *MI, unsigned OpNo,
...
void RVGPUInstPrinter::printOffset(const MCInst *MI, unsigned OpNo,
                                    const MCSubtargetInfo &STI,
                                    raw_ostream &O) {
  ...
  if (Imm != 0) {
    ...
    if (RVGPU::isGFX12(STI) && IsVBuffer)
...
void RVGPUInstPrinter::printFlatOffset(const MCInst *MI, unsigned OpNo,
                                        const MCSubtargetInfo &STI,
                                        raw_ostream &O) {
  uint32_t Imm = MI->getOperand(OpNo).getImm();
  if (Imm != 0) {
    ...
    if (AllowNegative) // Signed offset
      O << formatDec(SignExtend32(Imm, RVGPU::getNumFlatOffsetBits(STI)));
    else // Unsigned offset
      printU16ImmDecOperand(MI, OpNo, O);
  }
}

void RVGPUInstPrinter::printOffset0(const MCInst *MI, unsigned OpNo,
                                     const MCSubtargetInfo &STI,
                                     raw_ostream &O) {
  if (MI->getOperand(OpNo).getImm()) {
    O << " offset0:";
...
void RVGPUInstPrinter::printOffset1(const MCInst *MI, unsigned OpNo,
                                     const MCSubtargetInfo &STI,
                                     raw_ostream &O) {
  if (MI->getOperand(OpNo).getImm()) {
    O << " offset1:";
...
void RVGPUInstPrinter::printSMRDOffset8(const MCInst *MI, unsigned OpNo,
                                        const MCSubtargetInfo &STI,
                                        raw_ostream &O) {
  printU32ImmOperand(MI, OpNo, STI, O);
}

void RVGPUInstPrinter::printSMEMOffset(const MCInst *MI, unsigned OpNo,
                                        const MCSubtargetInfo &STI,
                                        raw_ostream &O) {
  O << formatHex(MI->getOperand(OpNo).getImm());
}

void RVGPUInstPrinter::printSMEMOffsetMod(const MCInst *MI, unsigned OpNo,
                                           const MCSubtargetInfo &STI,
                                           raw_ostream &O) {
  O << " offset:";
...
void RVGPUInstPrinter::printSMRDLiteralOffset(const MCInst *MI, unsigned OpNo,
                                               const MCSubtargetInfo &STI,
                                               raw_ostream &O) {
  printU32ImmOperand(MI, OpNo, STI, O);
}

void RVGPUInstPrinter::printCPol(const MCInst *MI, unsigned OpNo,
                                  const MCSubtargetInfo &STI, raw_ostream &O) {
  auto Imm = MI->getOperand(OpNo).getImm();

  if (RVGPU::isGFX12Plus(STI)) {
  ...
  if (Imm & CPol::GLC)
    O << ((RVGPU::isGFX940(STI) &&
           !(MII.get(MI->getOpcode()).TSFlags & SIInstrFlags::SMRD)) ? " sc0"
                                                                     : " glc");
  if (Imm & CPol::SLC)
    O << (RVGPU::isGFX940(STI) ? " nt" : " slc");
  if ((Imm & CPol::DLC) && RVGPU::isGFX10Plus(STI))
    O << " dlc";
  if ((Imm & CPol::SCC) && RVGPU::isGFX90A(STI))
    O << (RVGPU::isGFX940(STI) ? " sc1" : " scc");
  if (Imm & ~CPol::ALL)
    O << " /* unexpected cache policy bit */";
}

void RVGPUInstPrinter::printTH(const MCInst *MI, int64_t TH, int64_t Scope,
                                raw_ostream &O) {
  ...
  if (IsAtomic) {
    O << "TH_ATOMIC_";
    if (TH & RVGPU::CPol::TH_ATOMIC_CASCADE) {
      if (Scope >= RVGPU::CPol::SCOPE_DEV)
        O << "CASCADE" << (TH & RVGPU::CPol::TH_ATOMIC_NT ? "_NT" : "_RT");
      else
        O << formatHex(TH);
    } else if (TH & RVGPU::CPol::TH_ATOMIC_NT)
      O << "NT" << (TH & RVGPU::CPol::TH_ATOMIC_RETURN ? "_RETURN" : "");
    else if (TH & RVGPU::CPol::TH_ATOMIC_RETURN)
      O << "RETURN";
    else
      O << formatHex(TH);
  } else {
    if (!IsStore && TH == RVGPU::CPol::TH_RESERVED)
      O << formatHex(TH);
    else {
      ...
      switch (TH) {
      case RVGPU::CPol::TH_NT:
        O << "NT";
        break;
      case RVGPU::CPol::TH_HT:
        O << "HT";
        break;
      case RVGPU::CPol::TH_BYPASS: // or LU or RT_WB
        O << (Scope == RVGPU::CPol::SCOPE_SYS ? "BYPASS"
                                               : (IsStore ? "RT_WB" : "LU"));
        break;
      case RVGPU::CPol::TH_NT_RT:
        O << "NT_RT";
        break;
      case RVGPU::CPol::TH_RT_NT:
        O << "RT_NT";
        break;
      case RVGPU::CPol::TH_NT_HT:
        O << "NT_HT";
        break;
      case RVGPU::CPol::TH_NT_WB:
        O << "NT_WB";
        break;
      default:
...
void RVGPUInstPrinter::printScope(int64_t Scope, raw_ostream &O) {
...
void RVGPUInstPrinter::printDMask(const MCInst *MI, unsigned OpNo,
                                   const MCSubtargetInfo &STI, raw_ostream &O) {
  if (MI->getOperand(OpNo).getImm()) {
    O << " dmask:";
...
void RVGPUInstPrinter::printDim(const MCInst *MI, unsigned OpNo,
                                 const MCSubtargetInfo &STI, raw_ostream &O) {
...
void RVGPUInstPrinter::printR128A16(const MCInst *MI, unsigned OpNo,
                                  const MCSubtargetInfo &STI, raw_ostream &O) {
  if (STI.hasFeature(RVGPU::FeatureR128A16))
    printNamedBit(MI, OpNo, O, "a16");
  else
    printNamedBit(MI, OpNo, O, "r128");
}

void RVGPUInstPrinter::printFORMAT(const MCInst *MI, unsigned OpNo,
                                    const MCSubtargetInfo &STI,
                                    raw_ostream &O) {
}

void RVGPUInstPrinter::printSymbolicFormat(const MCInst *MI,
                                            const MCSubtargetInfo &STI,
                                            raw_ostream &O) {
  using namespace llvm::RVGPU::MTBUFFormat;

  int OpNo =
    RVGPU::getNamedOperandIdx(MI->getOpcode(), RVGPU::OpName::format);
  assert(OpNo != -1);

  unsigned Val = MI->getOperand(OpNo).getImm();
  if (RVGPU::isGFX10Plus(STI)) {
    if (Val == UFMT_DEFAULT)
      return;
...
void RVGPUInstPrinter::printVOPDst(const MCInst *MI, unsigned OpNo,
                                    const MCSubtargetInfo &STI, raw_ostream &O) {
  ...
  case RVGPU::V_ADD_CO_CI_U32_e32_gfx10:
  case RVGPU::V_SUB_CO_CI_U32_e32_gfx10:
  case RVGPU::V_SUBREV_CO_CI_U32_e32_gfx10:
  case RVGPU::V_ADD_CO_CI_U32_sdwa_gfx10:
  case RVGPU::V_SUB_CO_CI_U32_sdwa_gfx10:
  case RVGPU::V_SUBREV_CO_CI_U32_sdwa_gfx10:
  case RVGPU::V_ADD_CO_CI_U32_dpp_gfx10:
  case RVGPU::V_SUB_CO_CI_U32_dpp_gfx10:
  case RVGPU::V_SUBREV_CO_CI_U32_dpp_gfx10:
  ...
  case RVGPU::V_ADD_CO_CI_U32_dpp_gfx12:
  case RVGPU::V_SUB_CO_CI_U32_dpp_gfx12:
  case RVGPU::V_SUBREV_CO_CI_U32_dpp_gfx12:
  case RVGPU::V_ADD_CO_CI_U32_dpp8_gfx12:
  case RVGPU::V_SUB_CO_CI_U32_dpp8_gfx12:
  case RVGPU::V_SUBREV_CO_CI_U32_dpp8_gfx12:
    printDefaultVccOperand(false, STI, O);
    break;
  }
}

void RVGPUInstPrinter::printVINTRPDst(const MCInst *MI, unsigned OpNo,
                                       const MCSubtargetInfo &STI, raw_ostream &O) {
  if (RVGPU::isSI(STI) || RVGPU::isCI(STI))
    O << " ";
  else
    O << "_e32 ";
...
void RVGPUInstPrinter::printImmediateInt16(uint32_t Imm,
                                            const MCSubtargetInfo &STI,
                                            raw_ostream &O) {
...
void RVGPUInstPrinter::printImmediate16(uint32_t Imm,
                                         const MCSubtargetInfo &STI,
                                         raw_ostream &O) {
  int16_t SImm = static_cast<int16_t>(Imm);
  if (isInlinableIntLiteral(SImm)) {
...
void RVGPUInstPrinter::printImmediateV216(uint32_t Imm,
                                           const MCSubtargetInfo &STI,
                                           raw_ostream &O) {
...
void RVGPUInstPrinter::printImmediate32(uint32_t Imm,
                                         const MCSubtargetInfo &STI,
                                         raw_ostream &O) {
...
void RVGPUInstPrinter::printImmediate64(uint64_t Imm,
                                         const MCSubtargetInfo &STI,
  ...
  if (Imm == llvm::bit_cast<uint64_t>(0.0))
    ...
    assert(RVGPU::isValid32BitLiteral(Imm, true));
...
void RVGPUInstPrinter::printBLGP(const MCInst *MI, unsigned OpNo,
                                  const MCSubtargetInfo &STI,
                                  raw_ostream &O) {
...
void RVGPUInstPrinter::printCBSZ(const MCInst *MI, unsigned OpNo,
                                  const MCSubtargetInfo &STI,
                                  raw_ostream &O) {
  unsigned Imm = MI->getOperand(OpNo).getImm();
  if (!Imm)
...
void RVGPUInstPrinter::printABID(const MCInst *MI, unsigned OpNo,
                                  const MCSubtargetInfo &STI,
                                  raw_ostream &O) {
  unsigned Imm = MI->getOperand(OpNo).getImm();
  if (!Imm)
...
void RVGPUInstPrinter::printDefaultVccOperand(bool FirstOperand,
                                               const MCSubtargetInfo &STI,
                                               raw_ostream &O) {
  if (!FirstOperand)
    O << ", ";
  printRegOperand(STI.hasFeature(RVGPU::FeatureWavefrontSize64)
                      ? RVGPU::VCC
                      : RVGPU::VCC_LO,
                  O, MRI);
  if (FirstOperand)
...
void RVGPUInstPrinter::printWaitVDST(const MCInst *MI, unsigned OpNo,
                                      const MCSubtargetInfo &STI,
                                      raw_ostream &O) {
  O << " wait_vdst:";
...
void RVGPUInstPrinter::printWaitEXP(const MCInst *MI, unsigned OpNo,
                                    const MCSubtargetInfo &STI,
                                    raw_ostream &O) {
  O << " wait_exp:";
  printU4ImmDecOperand(MI, OpNo, O);
...
bool RVGPUInstPrinter::needsImpliedVcc(const MCInstrDesc &Desc,
                                        unsigned OpNo) const {
  return OpNo == 0 && (Desc.TSFlags & SIInstrFlags::DPP) &&
         (Desc.TSFlags & SIInstrFlags::VOPC) &&
         (Desc.hasImplicitDefOfPhysReg(RVGPU::VCC) ||
          Desc.hasImplicitDefOfPhysReg(RVGPU::VCC_LO));
}
...
void RVGPUInstPrinter::printRegularOperand(const MCInst *MI, unsigned OpNo,
                                            const MCSubtargetInfo &STI,
                                            raw_ostream &O) {
  const MCInstrDesc &Desc = MII.get(MI->getOpcode());
  ...
  if (Op.isReg()) {
    ...
    case RVGPU::OPERAND_REG_IMM_V2FP16:
      if (!isUInt<16>(Op.getImm()) &&
          STI.hasFeature(RVGPU::FeatureVOP3Literal)) {
        printImmediate32(Op.getImm(), STI, O);
        break;
      }

      //  Deal with 16-bit FP inline immediates not working.
      if (OpTy == RVGPU::OPERAND_REG_IMM_V2FP16) {
    ...
    if (Value == 0.0)
      ...
      const MCInstrDesc &Desc = MII.get(MI->getOpcode());
      int RCID = Desc.operands()[OpNo].RegClass;
      unsigned RCBits = RVGPU::getRegBitWidth(MRI.getRegClass(RCID));
      if (RCBits == 32)
  ...
  case RVGPU::V_CNDMASK_B32_e32_gfx10:
  case RVGPU::V_ADD_CO_CI_U32_e32_gfx10:
  case RVGPU::V_SUB_CO_CI_U32_e32_gfx10:
  case RVGPU::V_SUBREV_CO_CI_U32_e32_gfx10:
  case RVGPU::V_ADD_CO_CI_U32_dpp_gfx10:
  case RVGPU::V_SUB_CO_CI_U32_dpp_gfx10:
  case RVGPU::V_SUBREV_CO_CI_U32_dpp_gfx10:
  case RVGPU::V_CNDMASK_B32_dpp8_gfx10:
  case RVGPU::V_ADD_CO_CI_U32_dpp8_gfx10:
  case RVGPU::V_SUB_CO_CI_U32_dpp8_gfx10:
  case RVGPU::V_SUBREV_CO_CI_U32_dpp8_gfx10:
  ...
  case RVGPU::V_SUB_CO_CI_U32_e32_gfx12:
  case RVGPU::V_SUBREV_CO_CI_U32_e32_gfx12:
  case RVGPU::V_CNDMASK_B32_dpp_gfx12:
  case RVGPU::V_ADD_CO_CI_U32_dpp_gfx12:
  case RVGPU::V_SUB_CO_CI_U32_dpp_gfx12:
  case RVGPU::V_SUBREV_CO_CI_U32_dpp_gfx12:
  case RVGPU::V_CNDMASK_B32_dpp8_gfx12:
  case RVGPU::V_ADD_CO_CI_U32_dpp8_gfx12:
  case RVGPU::V_SUB_CO_CI_U32_dpp8_gfx12:
  case RVGPU::V_SUBREV_CO_CI_U32_dpp8_gfx12:

  case RVGPU::V_CNDMASK_B32_e32_gfx6_gfx7:
  case RVGPU::V_CNDMASK_B32_e32_vi:
    if ((int)OpNo == RVGPU::getNamedOperandIdx(MI->getOpcode(),
                                                RVGPU::OpName::src1))
...
void RVGPUInstPrinter::printOperandAndFPInputMods(const MCInst *MI,
                                                   unsigned OpNo,
                                                   const MCSubtargetInfo &STI,
                                                   raw_ostream &O) {
  const MCInstrDesc &Desc = MII.get(MI->getOpcode());
...
void RVGPUInstPrinter::printOperandAndIntInputMods(const MCInst *MI,
                                                    unsigned OpNo,
                                                    const MCSubtargetInfo &STI,
                                                    raw_ostream &O) {
  const MCInstrDesc &Desc = MII.get(MI->getOpcode());
  ...
  case RVGPU::V_SUB_CO_CI_U32_sdwa_gfx10:
  case RVGPU::V_SUBREV_CO_CI_U32_sdwa_gfx10:
    if ((int)OpNo + 1 == RVGPU::getNamedOperandIdx(MI->getOpcode(),
                                                    RVGPU::OpName::src1))
      printDefaultVccOperand(OpNo == 0, STI, O);
    break;
  }
...
void RVGPUInstPrinter::printDPP8(const MCInst *MI, unsigned OpNo,
                                  const MCSubtargetInfo &STI,
                                  raw_ostream &O) {
  if (!RVGPU::isGFX10Plus(STI))
...
void RVGPUInstPrinter::printDPPCtrl(const MCInst *MI, unsigned OpNo,
                                     const MCSubtargetInfo &STI,
                                     raw_ostream &O) {
  ...
  if (!RVGPU::isLegalDPALU_DPPControl(Imm) && RVGPU::isDPALU_DPP(Desc)) {
...
void RVGPUInstPrinter::printDppRowMask(const MCInst *MI, unsigned OpNo,
                                        const MCSubtargetInfo &STI,
                                        raw_ostream &O) {
  O << " row_mask:";
...
void RVGPUInstPrinter::printDppBankMask(const MCInst *MI, unsigned OpNo,
                                         const MCSubtargetInfo &STI,
                                         raw_ostream &O) {
  O << " bank_mask:";
  printU4ImmOperand(MI, OpNo, STI, O);
...
void RVGPUInstPrinter::printDppBoundCtrl(const MCInst *MI, unsigned OpNo,
                                          const MCSubtargetInfo &STI,
                                          raw_ostream &O) {
  unsigned Imm = MI->getOperand(OpNo).getImm();
  if (Imm) {
...
void RVGPUInstPrinter::printDppFI(const MCInst *MI, unsigned OpNo,
                                   const MCSubtargetInfo &STI, raw_ostream &O) {
...
void RVGPUInstPrinter::printSDWASel(const MCInst *MI, unsigned OpNo,
                                     raw_ostream &O) {
...
void RVGPUInstPrinter::printSDWADstSel(const MCInst *MI, unsigned OpNo,
                                        const MCSubtargetInfo &STI,
                                        raw_ostream &O) {
  O << "dst_sel:";
...
void RVGPUInstPrinter::printSDWASrc0Sel(const MCInst *MI, unsigned OpNo,
                                         const MCSubtargetInfo &STI,
                                         raw_ostream &O) {
  O << "src0_sel:";
  printSDWASel(MI, OpNo, O);
...
void RVGPUInstPrinter::printSDWASrc1Sel(const MCInst *MI, unsigned OpNo,
                                         const MCSubtargetInfo &STI,
                                         raw_ostream &O) {
  O << "src1_sel:";
  printSDWASel(MI, OpNo, O);
...
void RVGPUInstPrinter::printSDWADstUnused(const MCInst *MI, unsigned OpNo,
                                           const MCSubtargetInfo &STI,
                                           raw_ostream &O) {
  using namespace llvm::RVGPU::SDWA;
...
void RVGPUInstPrinter::printExpSrcN(const MCInst *MI, unsigned OpNo,
                                     const MCSubtargetInfo &STI, raw_ostream &O,
                                     unsigned N) {
  unsigned Opc = MI->getOpcode();
  int EnIdx = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::en);
  unsigned En = MI->getOperand(EnIdx).getImm();

  int ComprIdx = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::compr);
...
void RVGPUInstPrinter::printExpSrc0(const MCInst *MI, unsigned OpNo,
                                     const MCSubtargetInfo &STI,
                                     raw_ostream &O) {
  printExpSrcN(MI, OpNo, STI, O, 0);
}

void RVGPUInstPrinter::printExpSrc1(const MCInst *MI, unsigned OpNo,
                                     const MCSubtargetInfo &STI,
                                     raw_ostream &O) {
  printExpSrcN(MI, OpNo, STI, O, 1);
}

void RVGPUInstPrinter::printExpSrc2(const MCInst *MI, unsigned OpNo,
                                     const MCSubtargetInfo &STI,
                                     raw_ostream &O) {
  printExpSrcN(MI, OpNo, STI, O, 2);
}

void RVGPUInstPrinter::printExpSrc3(const MCInst *MI, unsigned OpNo,
                                     const MCSubtargetInfo &STI,
                                     raw_ostream &O) {
  printExpSrcN(MI, OpNo, STI, O, 3);
}

void RVGPUInstPrinter::printExpTgt(const MCInst *MI, unsigned OpNo,
                                    const MCSubtargetInfo &STI,
                                    raw_ostream &O) {
...
void RVGPUInstPrinter::printPackedModifier(const MCInst *MI,
                                            StringRef Name,
                                            unsigned Mod,
                                            raw_ostream &O) {
  unsigned Opc = MI->getOpcode();
  ...
  for (int OpName : { RVGPU::OpName::src0_modifiers,
                      RVGPU::OpName::src1_modifiers,
                      RVGPU::OpName::src2_modifiers }) {
    int Idx = RVGPU::getNamedOperandIdx(Opc, OpName);
    if (Idx == -1)
      break;
...
void RVGPUInstPrinter::printOpSel(const MCInst *MI, unsigned,
                                   const MCSubtargetInfo &STI,
                                   raw_ostream &O) {
  unsigned Opc = MI->getOpcode();
  if (isPermlane16(Opc)) {
    auto FIN = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::src0_modifiers);
    auto BCN = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::src1_modifiers);
...
void RVGPUInstPrinter::printOpSelHi(const MCInst *MI, unsigned OpNo,
                                     const MCSubtargetInfo &STI,
                                     raw_ostream &O) {
  printPackedModifier(MI, " op_sel_hi:[", SISrcMods::OP_SEL_1, O);
}

void RVGPUInstPrinter::printNegLo(const MCInst *MI, unsigned OpNo,
                                   const MCSubtargetInfo &STI,
                                   raw_ostream &O) {
  printPackedModifier(MI, " neg_lo:[", SISrcMods::NEG, O);
}

void RVGPUInstPrinter::printNegHi(const MCInst *MI, unsigned OpNo,
                                   const MCSubtargetInfo &STI,
                                   raw_ostream &O) {
  printPackedModifier(MI, " neg_hi:[", SISrcMods::NEG_HI, O);
}

void RVGPUInstPrinter::printInterpSlot(const MCInst *MI, unsigned OpNum,
                                        const MCSubtargetInfo &STI,
                                        raw_ostream &O) {
...
void RVGPUInstPrinter::printInterpAttr(const MCInst *MI, unsigned OpNum,
                                        const MCSubtargetInfo &STI,
                                        raw_ostream &O) {
...
void RVGPUInstPrinter::printInterpAttrChan(const MCInst *MI, unsigned OpNum,
                                        const MCSubtargetInfo &STI,
                                        raw_ostream &O) {
...
void RVGPUInstPrinter::printGPRIdxMode(const MCInst *MI, unsigned OpNo,
                                        const MCSubtargetInfo &STI,
                                        raw_ostream &O) {
...
void RVGPUInstPrinter::printMemOperand(const MCInst *MI, unsigned OpNo,
                                        const MCSubtargetInfo &STI,
                                        raw_ostream &O) {
  printRegularOperand(MI, OpNo, STI, O);
  O  << ", ";
...
void RVGPUInstPrinter::printIfSet(const MCInst *MI, unsigned OpNo,
...
void RVGPUInstPrinter::printOModSI(const MCInst *MI, unsigned OpNo,
                                    const MCSubtargetInfo &STI,
                                    raw_ostream &O) {
...
void RVGPUInstPrinter::printSendMsg(const MCInst *MI, unsigned OpNo,
                                     const MCSubtargetInfo &STI,
                                     raw_ostream &O) {
...
void RVGPUInstPrinter::printSwizzle(const MCInst *MI, unsigned OpNo,
                                     const MCSubtargetInfo &STI,
                                     raw_ostream &O) {
  using namespace llvm::RVGPU::Swizzle;
...
void RVGPUInstPrinter::printSWaitCnt(const MCInst *MI, unsigned OpNo,
                                      const MCSubtargetInfo &STI,
                                      raw_ostream &O) {
  RVGPU::IsaVersion ISA = RVGPU::getIsaVersion(STI.getCPU());
...
void RVGPUInstPrinter::printDepCtr(const MCInst *MI, unsigned OpNo,
                                    const MCSubtargetInfo &STI,
                                    raw_ostream &O) {
...
void RVGPUInstPrinter::printSDelayALU(const MCInst *MI, unsigned OpNo,
                                       const MCSubtargetInfo &STI,
                                       raw_ostream &O) {
...
void RVGPUInstPrinter::printHwreg(const MCInst *MI, unsigned OpNo,
                                   const MCSubtargetInfo &STI, raw_ostream &O) {
  unsigned Id;
  unsigned Offset;
  unsigned Width;
...
void RVGPUInstPrinter::printEndpgm(const MCInst *MI, unsigned OpNo,
                                    const MCSubtargetInfo &STI,
                                    raw_ostream &O) {
  uint16_t Imm = MI->getOperand(OpNo).getImm();
  if (Imm == 0) {
