...
static bool isDefBetween(const RVRegisterInfo &TRI,
...
bool SIOptimizeExecMaskingPreRA::optimizeVcndVcmpPair(MachineBasicBlock &MBB) {
  auto I = llvm::find_if(MBB.terminators(), [](const MachineInstr &MI) {
                           ...
                           return Opc == RVGPU::S_CBRANCH_VCCZ ||
                                  Opc == RVGPU::S_CBRANCH_VCCNZ; });
  if (I == MBB.terminators().end())
    return false;

  auto *And =
      TRI->findReachingDef(CondReg, RVGPU::NoSubRegister, *I, *MRI, LIS);
  ...
  if (!Cmp || !(Cmp->getOpcode() == RVGPU::V_CMP_NE_U32_e32 ||
                Cmp->getOpcode() == RVGPU::V_CMP_NE_U32_e64) ||
      Cmp->getParent() != And->getParent())
    return false;

  MachineOperand *Op1 = TII->getNamedOperand(*Cmp, RVGPU::OpName::src0);
  MachineOperand *Op2 = TII->getNamedOperand(*Cmp, RVGPU::OpName::src1);
  ...
  if (!Sel || Sel->getOpcode() != RVGPU::V_CNDMASK_B32_e64)
    return false;

  if (TII->hasModifiersSet(*Sel, RVGPU::OpName::src0_modifiers) ||
      TII->hasModifiersSet(*Sel, RVGPU::OpName::src1_modifiers))
    return false;

  Op1 = TII->getNamedOperand(*Sel, RVGPU::OpName::src0);
  Op2 = TII->getNamedOperand(*Sel, RVGPU::OpName::src1);
  MachineOperand *CC = TII->getNamedOperand(*Sel, RVGPU::OpName::src2);
  ...
  assert(AndSCC.getReg() == RVGPU::SCC);
  MachineOperand &Andn2SCC = Andn2->getOperand(3);
  assert(Andn2SCC.getReg() == RVGPU::SCC);
...
bool SIOptimizeExecMaskingPreRA::runOnMachineFunction(MachineFunction &MF) {
  ...
  AndOpc = Wave32 ? RVGPU::S_AND_B32 : RVGPU::S_AND_B64;
  Andn2Opc = Wave32 ? RVGPU::S_ANDN2_B32 : RVGPU::S_ANDN2_B64;
  OrSaveExecOpc =
      Wave32 ? RVGPU::S_OR_SAVEEXEC_B32 : RVGPU::S_OR_SAVEEXEC_B64;
  XorTermrOpc = Wave32 ? RVGPU::S_XOR_B32_term : RVGPU::S_XOR_B64_term;
  CondReg = MCRegister::from(Wave32 ? RVGPU::VCC_LO : RVGPU::VCC);
  ExecReg = MCRegister::from(Wave32 ? RVGPU::EXEC_LO : RVGPU::EXEC);

  DenseSet<Register> RecalcRegs({RVGPU::EXEC_LO, RVGPU::EXEC_HI});
  bool Changed = false;

  for (MachineBasicBlock &MBB : MF) {

    if (optimizeElseBranch(MBB)) {
      RecalcRegs.insert(RVGPU::SCC);
      Changed = true;
    }

    if (optimizeVcndVcmpPair(MBB)) {
      RecalcRegs.insert(RVGPU::VCC_LO);
      RecalcRegs.insert(RVGPU::VCC_HI);
      RecalcRegs.insert(RVGPU::SCC);
      Changed = true;
    }
    ...
    if (MBB.succ_empty()) {
      ...
      if (Term.getOpcode() != RVGPU::S_ENDPGM || Term.getNumOperands() != 1)
        continue;
      ...
      while (!Blocks.empty()) {
        ...
        if (I != E) {
          if (I->isUnconditionalBranch() || I->getOpcode() == RVGPU::S_ENDPGM)
            ++I;
          else if (I->isBranch())
            continue;
        }
