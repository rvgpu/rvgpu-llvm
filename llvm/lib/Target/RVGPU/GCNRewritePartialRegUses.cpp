...
private:
  ...
  struct SubRegInfo {
    ...
    unsigned SubReg = RVGPU::NoSubRegister;
...
GCNRewritePartialRegUses::getMinSizeReg(const TargetRegisterClass *RC,
                                        SubRegMap &SubRegs) const {
  unsigned CoverSubreg = RVGPU::NoSubRegister;
  ...
  for (auto [SubReg, SRI] : SubRegs) {
    ...
    if (SubRegOffset < Offset) {
      Offset = SubRegOffset;
      CoverSubreg = RVGPU::NoSubRegister;
    }
    if (SubRegEnd > End) {
      End = SubRegEnd;
      CoverSubreg = RVGPU::NoSubRegister;
  ...
  if (CoverSubreg != RVGPU::NoSubRegister)
...
bool GCNRewritePartialRegUses::rewriteReg(Register Reg) const {
  ...
  if (Range.empty() || any_of(Range, [](MachineOperand &MO) {
        return MO.getSubReg() == RVGPU::NoSubRegister; // Whole reg used. [1]
      }))
    return false;
  ...
  for (MachineOperand &MO : Range) {
    ...
    assert(SubReg != RVGPU::NoSubRegister); // Due to [1].
  ...
  for (auto &MO : make_early_inc_range(MRI->reg_operands(Reg))) {
    ...
    MO.setSubReg(SubReg);
    if (SubReg == RVGPU::NoSubRegister && MO.isDef())
      MO.setIsUndef(false);
  }
...
bool GCNRewritePartialRegUses::runOnMachineFunction(MachineFunction &MF) {
  MRI = &MF.getRegInfo();
  TRI = static_cast<const RVRegisterInfo *>(MRI->getTargetRegisterInfo());
