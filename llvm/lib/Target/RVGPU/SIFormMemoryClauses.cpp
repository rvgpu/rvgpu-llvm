...
MaxClause("rvgpu-max-memory-clause", cl::Hidden, cl::init(15),
...
static bool isVMEMClauseInst(const MachineInstr &MI) {
  return RVInstrInfo::isFLAT(MI) || RVInstrInfo::isVMEM(MI);
}
...
bool SIFormMemoryClauses::runOnMachineFunction(MachineFunction &MF) {
  ...
  MaxVGPRs = TRI->getAllocatableSet(MF, &RVGPU::VGPR_32RegClass).count();
  MaxSGPRs = TRI->getAllocatableSet(MF, &RVGPU::SGPR_32RegClass).count();
  unsigned FuncMaxClause = MF.getFunction().getFnAttributeAsParsedInteger(
      "rvgpu-max-memory-clause", MaxClause);
