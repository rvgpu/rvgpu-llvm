...
//  call float @llvm.rvgpu.image.load.2dmsaa.f32.i32(i32 1, i32 %s, i32 %t, i32 0, <8 x i32> %rsrc, i32 0, i32 0)
//  call float @llvm.rvgpu.image.load.2dmsaa.f32.i32(i32 1, i32 %s, i32 %t, i32 1, <8 x i32> %rsrc, i32 0, i32 0)
//  call float @llvm.rvgpu.image.load.2dmsaa.f32.i32(i32 1, i32 %s, i32 %t, i32 2, <8 x i32> %rsrc, i32 0, i32 0)
//  call float @llvm.rvgpu.image.load.2dmsaa.f32.i32(i32 1, i32 %s, i32 %t, i32 3, <8 x i32> %rsrc, i32 0, i32 0)
// ==>
//  call <4 x float> @llvm.rvgpu.image.msaa.load.2dmsaa.v4f32.i32(i32 1, i32 %s, i32 %t, i32 0, <8 x i32> %rsrc, i32 0, i32 0)
//
// clang-format on
//
// Future improvements:
...
#define DEBUG_TYPE "rvgpu-image-intrinsic-opt"

namespace {
class RVGPUImageIntrinsicOptimizer : public FunctionPass {
  const TargetMachine *TM;

public:
  static char ID;

  RVGPUImageIntrinsicOptimizer(const TargetMachine *TM = nullptr)
      : FunctionPass(ID), TM(TM) {}

  bool runOnFunction(Function &F) override;

}; // End of class RVGPUImageIntrinsicOptimizer
} // End anonymous namespace

INITIALIZE_PASS(RVGPUImageIntrinsicOptimizer, DEBUG_TYPE,
                "RVGPU Image Intrinsic Optimizer", false, false)

char RVGPUImageIntrinsicOptimizer::ID = 0;
...
BasicBlock::iterator rvCollectMergeableInsts(
  ...
  for (; I != E; ++I) {
    ...
    if (IntrinsicInst *II = dyn_cast<IntrinsicInst>(I)) {
      ...
      if (IntrinID != Intrinsic::rvgpu_image_load_2dmsaa &&
          IntrinID != Intrinsic::rvgpu_image_load_2darraymsaa)
        continue;

      // Check for constant FragId.
      const auto *ImageDimIntr = RVGPU::getImageDimIntrinsicInfo(IntrinID);
...
bool rvOptimizeSection(ArrayRef<SmallVector<IntrinsicInst *, 4>> MergeableInsts) {
  ...
  for (const auto &IIList : MergeableInsts) {
    ...
    const RVGPU::ImageDimIntrinsicInfo *ImageDimIntr =
        RVGPU::getImageDimIntrinsicInfo(IntrinID);
    ...
    while (DMaskVal != 0) {
      ...
      if (IntrinID == Intrinsic::rvgpu_image_load_2dmsaa)
        NewIntrinID = Intrinsic::rvgpu_image_msaa_load_2dmsaa;
      else
        NewIntrinID = Intrinsic::rvgpu_image_msaa_load_2darraymsaa;
...
static bool imageIntrinsicOptimizerImpl(Function &F, const TargetMachine *TM) {
  ...
  if (std::none_of(M->begin(), M->end(), [](Function &F) {
        return !F.users().empty() &&
               (F.getIntrinsicID() == Intrinsic::rvgpu_image_load_2dmsaa ||
                F.getIntrinsicID() == Intrinsic::rvgpu_image_load_2darraymsaa);
      }))
    return false;
...
bool RVGPUImageIntrinsicOptimizer::runOnFunction(Function &F) {
  if (skipFunction(F))
    return false;
...
FunctionPass *
llvm::createRVGPUImageIntrinsicOptimizerPass(const TargetMachine *TM) {
  return new RVGPUImageIntrinsicOptimizer(TM);
}

PreservedAnalyses
RVGPUImageIntrinsicOptimizerPass::run(Function &F,
