...
#define DEBUG_TYPE "rvgpu-lower-kernel-arguments"
...
public:
  ...
  PreloadKernelArgInfo(Function &F, const RVSubtarget &ST) : F(F), ST(ST) {
...
class RVGPULowerKernelArguments : public FunctionPass {
public:
  static char ID;

  RVGPULowerKernelArguments() : FunctionPass(ID) {}
...
static bool lowerKernelArguments(Function &F, const TargetMachine &TM) {
  ...
  CallInst *KernArgSegment =
      Builder.CreateIntrinsic(Intrinsic::rvgpu_kernarg_segment_ptr, {}, {},
  ...
  for (Argument &Arg : F.args()) {
    ...
    if (PointerType *PT = dyn_cast<PointerType>(ArgTy)) {
      ...
      // integer types.
      if ((PT->getAddressSpace() == RVGPUAS::LOCAL_ADDRESS ||
           PT->getAddressSpace() == RVGPUAS::REGION_ADDRESS) &&
...
bool RVGPULowerKernelArguments::runOnFunction(Function &F) {
  auto &TPC = getAnalysis<TargetPassConfig>();
  const TargetMachine &TM = TPC.getTM<TargetMachine>();
  return lowerKernelArguments(F, TM);
}

INITIALIZE_PASS_BEGIN(RVGPULowerKernelArguments, DEBUG_TYPE,
                      "RVGPU Lower Kernel Arguments", false, false)
INITIALIZE_PASS_END(RVGPULowerKernelArguments, DEBUG_TYPE, "RVGPU Lower Kernel Arguments",
                    false, false)

char RVGPULowerKernelArguments::ID = 0;

FunctionPass *llvm::createRVGPULowerKernelArgumentsPass() {
  return new RVGPULowerKernelArguments();
}

PreservedAnalyses
RVGPULowerKernelArgumentsPass::run(Function &F, FunctionAnalysisManager &AM) {
