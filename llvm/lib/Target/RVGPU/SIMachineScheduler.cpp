...
// Some figures taken from RV docs:
...
SUnit* SIScheduleBlock::pickNode() {
  ...
  for (SUnit* SU : TopReadySUs) {
    ...
    TryCand.SGPRUsage = pressure[RVGPU::RegisterPressureSets::SReg_32];
    TryCand.VGPRUsage = pressure[RVGPU::RegisterPressureSets::VGPR_32];
...
void SIScheduleBlock::printDebug(bool full) {
  ...
  if (Scheduled) {
    dbgs() << "LiveInPressure "
           << LiveInPressure[RVGPU::RegisterPressureSets::SReg_32] << ' '
           << LiveInPressure[RVGPU::RegisterPressureSets::VGPR_32] << '\n';
    dbgs() << "LiveOutPressure "
           << LiveOutPressure[RVGPU::RegisterPressureSets::SReg_32] << ' '
           << LiveOutPressure[RVGPU::RegisterPressureSets::VGPR_32] << "\n\n";
    dbgs() << "LiveIns:\n";
...
void SIScheduleBlockCreator::colorExports() {
  ...
  for (unsigned SUNum : DAG->TopDownIndex2SU) {
    const SUnit &SU = DAG->SUnits[SUNum];
    if (RVInstrInfo::isEXP(*SU.getInstr())) {
      ...
      for (const SDep &SuccDep : SU.Succs) {
        ...
        if (!RVInstrInfo::isEXP(*SuccSU->getInstr())) {
  ...
  for (std::vector<SIScheduleBlock*>::iterator I = ReadyBlocks.begin(),
    ...
    TryCand.VGPRUsageDiff =
      checkRegUsageImpact(TryCand.Block->getInRegs(),
          TryCand.Block->getOutRegs())[RVGPU::RegisterPressureSets::VGPR_32];
...
SIScheduleDAGMI::SIScheduleDAGMI(MachineSchedContext *C) :
  ...
  SITII = static_cast<const RVInstrInfo*>(TII);
  SITRI = static_cast<const RVRegisterInfo*>(TRI);
}
...
void SIScheduleDAGMI::moveLowLatencies() {
   ...
   for (unsigned i = 0, e = ScheduledSUnits.size(); i != e; ++i) {
    ...
    } else if (SU->getInstr()->getOpcode() == RVGPU::COPY) {
...
SIScheduleDAGMI::fillVgprSgprCost(_Iterator First, _Iterator End,
  ...
  for (_Iterator RegI = First; RegI != End; ++RegI) {
    ...
    for (; PSetI.isValid(); ++PSetI) {
      if (*PSetI == RVGPU::RegisterPressureSets::VGPR_32)
        VgprUsage += PSetI.getWeight();
      else if (*PSetI == RVGPU::RegisterPressureSets::SReg_32)
