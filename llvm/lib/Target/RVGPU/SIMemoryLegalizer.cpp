...
/// found here:
///   http://llvm.org/docs/RVGPUUsage.html#memory-model
//
//===----------------------------------------------------------------------===//
...
static cl::opt<bool> RvgpuSkipCacheInvalidations(
    "rvgpu-skip-cache-invalidations", cl::init(false), cl::Hidden,
...
/// The atomic synchronization scopes supported by the RVGPU target.
...
/// The distinct address spaces supported by the RVGPU target for
...
private:
  RVGPUMachineModuleInfo *MMI = nullptr;
...
protected:
  ...
  bool enableDLCBit(const MachineBasicBlock::iterator &MI) const {
    return enableNamedBit(MI, RVGPU::CPol::DLC);
  }
...
private:
  ...
  bool isAtomicRet(const MachineInstr &MI) const {
    return RVInstrInfo::isAtomicRet(MI);
  }
...
SIMemOpAccess::SIMemOpAccess(MachineFunction &MF) {
  MMI = &MF.getMMI().getObjFileInfo<RVGPUMachineModuleInfo>();
}
...
SIMemOpAccess::getAtomicFenceInfo(const MachineBasicBlock::iterator &MI) const {
  ...
  if (MI->getOpcode() != RVGPU::ATOMIC_FENCE)
    return std::nullopt;
...
bool SICacheControl::enableNamedBit(const MachineBasicBlock::iterator MI,
                                    RVGPU::CPol::CPol Bit) const {
  MachineOperand *CPol = TII->getNamedOperand(*MI, RVGPU::OpName::cpol);
  if (!CPol)
    return false;
...
std::unique_ptr<SICacheControl> SICacheControl::create(const RVSubtarget &ST) {
  ...
  if (Generation <= RVGPUSubtarget::SOUTHERN_ISLANDS)
...
bool SIGfx6CacheControl::insertWait(MachineBasicBlock::iterator &MI,
  ...
  if (VMCnt || LGKMCnt) {
    unsigned WaitCntImmediate =
      RVGPU::encodeWaitcnt(IV,
                            VMCnt ? 0 : getVmcntBitMask(IV),
                            getExpcntBitMask(IV),
                            LGKMCnt ? 0 : getLgkmcntBitMask(IV));
    BuildMI(MBB, MI, DL, TII->get(RVGPU::S_WAITCNT_soft))
...
bool SIGfx6CacheControl::insertAcquire(MachineBasicBlock::iterator &MI,
  ...
  if ((AddrSpace & SIAtomicAddrSpace::GLOBAL) != SIAtomicAddrSpace::NONE) {
    ...
    case SIAtomicScope::SYSTEM:
    case SIAtomicScope::AGENT:
      BuildMI(MBB, MI, DL, TII->get(RVGPU::BUFFER_WBINVL1));
      Changed = true;
      break;
    case SIAtomicScope::WORKGROUP:
    case SIAtomicScope::WAVEFRONT:
...
bool SIGfx90ACacheControl::insertAcquire(MachineBasicBlock::iterator &MI,
  ...
  if ((AddrSpace & SIAtomicAddrSpace::GLOBAL) != SIAtomicAddrSpace::NONE) {
    ...
    case SIAtomicScope::SYSTEM:
      ...
      BuildMI(MBB, MI, DL, TII->get(RVGPU::BUFFER_INVL2));
...
bool SIGfx90ACacheControl::insertRelease(MachineBasicBlock::iterator &MI,
  ...
  if ((AddrSpace & SIAtomicAddrSpace::GLOBAL) != SIAtomicAddrSpace::NONE) {
    ...
    case SIAtomicScope::SYSTEM:
      ...
      BuildMI(MBB, MI, DL, TII->get(RVGPU::BUFFER_WBL2))
        // Set SC bits to indicate system scope.
        .addImm(RVGPU::CPol::SC0 | RVGPU::CPol::SC1);
...
bool SIGfx940CacheControl::insertAcquire(MachineBasicBlock::iterator &MI,
  ...
  if ((AddrSpace & SIAtomicAddrSpace::GLOBAL) != SIAtomicAddrSpace::NONE) {
    ...
    case SIAtomicScope::SYSTEM:
      ...
      // stale local VMEM data with MTYPE NC. Local VMEM data with MTYPE RW and
      // CC will never be stale due to the local memory probes.
      BuildMI(MBB, MI, DL, TII->get(RVGPU::BUFFER_INV))
          // Set SC bits to indicate system scope.
          .addImm(RVGPU::CPol::SC0 | RVGPU::CPol::SC1);
      // Inserting a "S_WAITCNT vmcnt(0)" after is not required because the
      // hardware does not reorder memory operations by the same wave with
...
bool SIGfx10CacheControl::insertWait(MachineBasicBlock::iterator &MI,
  ...
  if (VSCnt) {
    BuildMI(MBB, MI, DL, TII->get(RVGPU::S_WAITCNT_VSCNT_soft))
...
bool SIGfx10CacheControl::insertAcquire(MachineBasicBlock::iterator &MI,
  ...
  if ((AddrSpace & SIAtomicAddrSpace::GLOBAL) != SIAtomicAddrSpace::NONE) {
    ...
    case SIAtomicScope::SYSTEM:
    case SIAtomicScope::AGENT:
      BuildMI(MBB, MI, DL, TII->get(RVGPU::BUFFER_GL0_INV));
      BuildMI(MBB, MI, DL, TII->get(RVGPU::BUFFER_GL1_INV));
      Changed = true;
      break;
    case SIAtomicScope::WORKGROUP:
      // In WGP mode the waves of a work-group can be executing on either CU of
...
bool SIMemoryLegalizer::expandAtomicFence(const SIMemOpInfo &MOI,
                                          MachineBasicBlock::iterator &MI) {
  assert(MI->getOpcode() == RVGPU::ATOMIC_FENCE);
