...
void SIAnnotateControlFlow::initialize(Module &M, const RVSubtarget &ST) {
  ...
  If = Intrinsic::getDeclaration(&M, Intrinsic::rvgpu_if, { IntMask });
  Else = Intrinsic::getDeclaration(&M, Intrinsic::rvgpu_else,
                                   { IntMask, IntMask });
  IfBreak = Intrinsic::getDeclaration(&M, Intrinsic::rvgpu_if_break,
                                      { IntMask });
  Loop = Intrinsic::getDeclaration(&M, Intrinsic::rvgpu_loop, { IntMask });
  EndCf = Intrinsic::getDeclaration(&M, Intrinsic::rvgpu_end_cf, { IntMask });
}
...
bool SIAnnotateControlFlow::hasKill(const BasicBlock *BB) {
  for (const Instruction &I : *BB) {
    if (const CallInst *CI = dyn_cast<CallInst>(&I))
      if (CI->getIntrinsicID() == Intrinsic::rvgpu_kill)
        return true;
  }
  return false;
}
