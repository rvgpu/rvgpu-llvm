//===- RVGPUMCInstLower.cpp - Lower RVGPU MachineInstr to an MCInst -----===//
//
...
//
/// \file
/// Code to lower RVGPU MachineInstrs to their corresponding MCInst.
//
//===----------------------------------------------------------------------===//
//

#include "RVGPUMCInstLower.h"
#include "RVGPU.h"
...
//#include "RVGPUGenMCPseudoLowering.inc"

RVGPUMCInstLower::RVGPUMCInstLower(MCContext &ctx,
...
static MCSymbolRefExpr::VariantKind getVariantKind(unsigned MOFlags) {
  ...
  case RVGPUInstrInfo::MO_GOTPCREL:
    return MCSymbolRefExpr::VK_GOTPCREL;
  case RVGPUInstrInfo::MO_GOTPCREL32_LO:
    return MCSymbolRefExpr::VK_RVGPU_GOTPCREL32_LO;
  case RVGPUInstrInfo::MO_GOTPCREL32_HI:
    return MCSymbolRefExpr::VK_RVGPU_GOTPCREL32_HI;
  case RVGPUInstrInfo::MO_REL32_LO:
    return MCSymbolRefExpr::VK_RVGPU_REL32_LO;
  case RVGPUInstrInfo::MO_REL32_HI:
    return MCSymbolRefExpr::VK_RVGPU_REL32_HI;
  case RVGPUInstrInfo::MO_ABS32_LO:
    return MCSymbolRefExpr::VK_RVGPU_ABS32_LO;
  case RVGPUInstrInfo::MO_ABS32_HI:
    return MCSymbolRefExpr::VK_RVGPU_ABS32_HI;
  }
}

bool RVGPUMCInstLower::lowerOperand(const MachineOperand &MO,
  ...
  case MachineOperand::MO_MCSymbol:
    if (MO.getTargetFlags() == RVGPUInstrInfo::MO_FAR_BRANCH_OFFSET) {
...
void RVGPUMCInstLower::lower(const MachineInstr *MI, MCInst &OutMI) const {
  unsigned Opcode = MI->getOpcode();
  const auto *TII = static_cast<const RVGPUInstrInfo*>(ST.getInstrInfo());
#if 0
  ...
  if (Opcode == RVGPU::S_SETPC_B64_return)
    Opcode = RVGPU::S_SETPC_B64;
  else if (Opcode == RVGPU::SI_CALL) {
    // SI_CALL is just S_SWAPPC_B64 with an additional operand to track the
    // called function (which we need to remove here).
    OutMI.setOpcode(TII->pseudoToMCOpcode(RVGPU::S_SWAPPC_B64));
    MCOperand Dest, Src;
    ...
    return;
  } else if (Opcode == RVGPU::SI_TCRETURN ||
             Opcode == RVGPU::SI_TCRETURN_GFX) {
  ...
  if (MCOpcode == -1) {
    ...
    C.emitError("RVGPUMCInstLower::lower - Pseudo instruction doesn't have "
                "a target-specific version: " + Twine(MI->getOpcode()));
  }
#endif 
  int MCOpcode = MI->getOpcode();
...
#if 0
  int FIIdx = RVGPU::getNamedOperandIdx(MCOpcode, RVGPU::OpName::fi);
...
bool RVGPUAsmPrinter::lowerOperand(const MachineOperand &MO,
                                    MCOperand &MCOp) const {
  const RVGPUSubtarget &STI = MF->getSubtarget<RVGPUSubtarget>();
  RVGPUMCInstLower MCInstLowering(OutContext, STI, *this);
  return MCInstLowering.lowerOperand(MO, MCOp);
}

const MCExpr *RVGPUAsmPrinter::lowerConstant(const Constant *CV) {

  // Intercept LDS variables with known addresses
  if (const GlobalVariable *GV = dyn_cast<const GlobalVariable>(CV)) {
    if (std::optional<uint32_t> Address =
            RVGPUMachineFunctionInfo::getLDSAbsoluteAddress(*GV)) {
...
void RVGPUAsmPrinter::emitInstruction(const MachineInstr *MI) {
  ...
  if (MI->isBundle()) {
    ...
    if (MI->getOpcode() == RVGPU::WAVE_BARRIER) {
      if (isVerbose())
        OutStreamer->emitRawComment(" wave barrier");
      return;
    }

    if (MI->getOpcode() == RVGPU::SCHED_BARRIER) {
      if (isVerbose()) {
    ...
    if (MI->getOpcode() == RVGPU::SCHED_GROUP_BARRIER) {
      if (isVerbose()) {
        std::string HexString;
        raw_string_ostream HexStream(HexString);
        HexStream << format_hex(MI->getOperand(0).getImm(), 10, true);
    ...
    if (MI->getOpcode() == RVGPU::IGLP_OPT) {
      if (isVerbose()) {
        std::string HexString;
        raw_string_ostream HexStream(HexString);
        HexStream << format_hex(MI->getOperand(0).getImm(), 10, true);
    ...
    if (MI->getOpcode() == RVGPU::SI_MASKED_UNREACHABLE) {
      if (isVerbose())
    ...
    if (!MI->isPseudo() && STI.isCPUStringValid(STI.getCPU()) &&
      ...
      std::unique_ptr<MCCodeEmitter> InstEmitter(createRVGPUMCCodeEmitter(
    ...
    if (DumpCodeInstEmitter) {
      ...
      RVGPUInstPrinter InstPrinter(*TM.getMCAsmInfo(), *STI.getInstrInfo(),
                                    *STI.getRegisterInfo());
