...
static cl::opt<bool>
RemoveRedundantEndcf("rvgpu-remove-redundant-endcf",
...
static void setImpSCCDefDead(MachineInstr &MI, bool IsDead) {
  ...
  assert(ImpDefSCC.getReg() == RVGPU::SCC && ImpDefSCC.isDef());
...
void SILowerControlFlow::emitIf(MachineInstr &MI) {
  ...
  assert(Cond.getSubReg() == RVGPU::NoSubRegister);
  ...
  MachineInstr *CopyExec =
    BuildMI(MBB, I, DL, TII->get(RVGPU::COPY), CopyReg)
    .addReg(Exec)
  ...
  MachineInstr *NewBr = BuildMI(MBB, I, DL, TII->get(RVGPU::S_CBRANCH_EXECZ))
                            .add(MI.getOperand(2));
...
void SILowerControlFlow::emitElse(MachineInstr &MI) {
  ...
  MachineInstr *Branch =
      BuildMI(MBB, ElsePt, DL, TII->get(RVGPU::S_CBRANCH_EXECZ))
          .addMBB(DestBB);
...
void SILowerControlFlow::emitLoop(MachineInstr &MI) {
  ...
  MachineInstr *Branch =
      BuildMI(MBB, BranchPt, DL, TII->get(RVGPU::S_CBRANCH_EXECNZ))
          .add(MI.getOperand(1));
...
void SILowerControlFlow::optimizeEndCf() {
  ...
  for (MachineInstr *MI : reverse(LoweredEndCf)) {
    ...
    Register SavedExec
      = TII->getNamedOperand(*Next, RVGPU::OpName::src1)->getReg();
    ...
    if (Def && LoweredIf.count(SavedExec)) {
      ...
      Register Reg;
      if (LV)
        Reg = TII->getNamedOperand(*MI, RVGPU::OpName::src1)->getReg();
      MI->eraseFromParent();
      if (LV)
...
MachineBasicBlock *SILowerControlFlow::process(MachineInstr &MI) {
  ...
  case RVGPU::SI_WATERFALL_LOOP:
    MI.setDesc(TII->get(RVGPU::S_CBRANCH_EXECNZ));
    break;

  case RVGPU::SI_END_CF:
...
void SILowerControlFlow::lowerInitExec(MachineBasicBlock *MBB,
  ...
  if (MI.getOpcode() == RVGPU::SI_INIT_EXEC) {
    ...
    MachineInstr *InitMI = BuildMI(*MBB, MBB->begin(), MI.getDebugLoc(),
            TII->get(IsWave32 ? RVGPU::S_MOV_B32 : RVGPU::S_MOV_B64), Exec)
  ...
  Register CountReg = MRI->createVirtualRegister(&RVGPU::SGPR_32RegClass);
  auto BfeMI = BuildMI(*MBB, FirstMI, DL, TII->get(RVGPU::S_BFE_U32), CountReg)
                   .addReg(InputReg)
  ...
  auto BfmMI =
      BuildMI(*MBB, FirstMI, DL,
              TII->get(IsWave32 ? RVGPU::S_BFM_B32 : RVGPU::S_BFM_B64), Exec)
          .addReg(CountReg)
          .addImm(0);
  auto CmpMI = BuildMI(*MBB, FirstMI, DL, TII->get(RVGPU::S_CMP_EQ_U32))
  ...
  auto CmovMI =
      BuildMI(*MBB, FirstMI, DL,
              TII->get(IsWave32 ? RVGPU::S_CMOV_B32 : RVGPU::S_CMOV_B64),
              Exec)
          .addImm(-1);
...
bool SILowerControlFlow::removeMBBifRedundant(MachineBasicBlock &MBB) {
  ...
  if (FallThrough && !FallThrough->isLayoutSuccessor(Succ)) {
    ...
    MachineInstr *BranchMI = BuildMI(*FallThrough, FallThrough->end(),
            FallThrough->findBranchDebugLoc(), TII->get(RVGPU::S_BRANCH))
        .addMBB(Succ);
    if (LIS)
...
bool SILowerControlFlow::runOnMachineFunction(MachineFunction &MF) {
  ...
  if (ST.isWave32()) {
    ...
    OrOpc = RVGPU::S_OR_B32;
    XorOpc = RVGPU::S_XOR_B32;
    MovTermOpc = RVGPU::S_MOV_B32_term;
    Andn2TermOpc = RVGPU::S_ANDN2_B32_term;
    XorTermrOpc = RVGPU::S_XOR_B32_term;
    OrTermrOpc = RVGPU::S_OR_B32_term;
    OrSaveExecOpc = RVGPU::S_OR_SAVEEXEC_B32;
    Exec = RVGPU::EXEC_LO;
  } else {
    AndOpc = RVGPU::S_AND_B64;
    OrOpc = RVGPU::S_OR_B64;
    XorOpc = RVGPU::S_XOR_B64;
    MovTermOpc = RVGPU::S_MOV_B64_term;
    Andn2TermOpc = RVGPU::S_ANDN2_B64_term;
    XorTermrOpc = RVGPU::S_XOR_B64_term;
    OrTermrOpc = RVGPU::S_OR_B64_term;
    OrSaveExecOpc = RVGPU::S_OR_SAVEEXEC_B64;
    Exec = RVGPU::EXEC;
  }
  ...
  for (MachineFunction::iterator BI = MF.begin();
    ...
    for (I = MBB->begin(); I != E; I = Next) {
      ...
      case RVGPU::SI_INIT_EXEC:
      case RVGPU::SI_INIT_EXEC_FROM_INPUT:
