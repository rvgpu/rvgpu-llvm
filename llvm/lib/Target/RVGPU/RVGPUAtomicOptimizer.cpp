...
#define DEBUG_TYPE "rvgpu-atomic-optimizer"
...
class RVGPUAtomicOptimizer : public FunctionPass {
public:
  static char ID;
  ScanOptions ScanImpl;
  RVGPUAtomicOptimizer(ScanOptions ScanImpl)
...
class RVGPUAtomicOptimizerImpl
    : public InstVisitor<RVGPUAtomicOptimizerImpl> {
private:
...
public:
  RVGPUAtomicOptimizerImpl() = delete;

  RVGPUAtomicOptimizerImpl(const UniformityInfo *UA, const DataLayout *DL,
                            DomTreeUpdater &DTU, const RVSubtarget *ST,
...
char RVGPUAtomicOptimizer::ID = 0;

char &llvm::RVGPUAtomicOptimizerID = RVGPUAtomicOptimizer::ID;

bool RVGPUAtomicOptimizer::runOnFunction(Function &F) {
  if (skipFunction(F)) {
    return false;
  }
  ...
  bool IsPixelShader = F.getCallingConv() == CallingConv::RVGPU_PS;

  return RVGPUAtomicOptimizerImpl(UA, DL, DTU, ST, IsPixelShader, ScanImpl)
      .run(F);
}

PreservedAnalyses RVGPUAtomicOptimizerPass::run(Function &F,
  ...
  bool IsChanged =
      RVGPUAtomicOptimizerImpl(UA, DL, DTU, ST, IsPixelShader, ScanImpl)
          .run(F);
...
bool RVGPUAtomicOptimizerImpl::run(Function &F) {
...
void RVGPUAtomicOptimizerImpl::visitIntrinsicInst(IntrinsicInst &I) {
  AtomicRMWInst::BinOp Op;
...
Value *RVGPUAtomicOptimizerImpl::buildReduction(IRBuilder<> &B,
                                                 AtomicRMWInst::BinOp Op,
                                                 Value *V,
                                                 Value *const Identity) const {
  ...
  Function *UpdateDPP =
      Intrinsic::getDeclaration(M, Intrinsic::rvgpu_update_dpp, AtomicTy);
  ...
  if (ST->hasPermLane64()) {
    ...
    Value *Permlane64Call =
        B.CreateIntrinsic(Intrinsic::rvgpu_permlane64, {}, V);
  ...
  Function *ReadLane =
      Intrinsic::getDeclaration(M, Intrinsic::rvgpu_readlane, {});
  V = B.CreateBitCast(V, IntNTy);
...
// all lanes active.
Value *RVGPUAtomicOptimizerImpl::buildScan(IRBuilder<> &B,
  ...
  if (ST->hasDPPBroadcasts()) {
    ...
    if (!ST->isWave32()) {
      ...
      V = B.CreateBitCast(V, IntNTy);
      Value *const Lane31 = B.CreateIntrinsic(Intrinsic::rvgpu_readlane, {},
                                              {V, B.getInt32(31)});
...
Value *RVGPUAtomicOptimizerImpl::buildShiftRight(IRBuilder<> &B, Value *V,
                                                  Value *Identity) const {
  Type *AtomicTy = V->getType();
  Type *IntNTy = B.getIntNTy(AtomicTy->getPrimitiveSizeInBits());
  ...
  if (ST->hasDPPWavefrontShifts()) {
    ...
    Function *ReadLane =
        Intrinsic::getDeclaration(M, Intrinsic::rvgpu_readlane, {});
    Function *WriteLane =
        Intrinsic::getDeclaration(M, Intrinsic::rvgpu_writelane, {});
...
std::pair<Value *, Value *> RVGPUAtomicOptimizerImpl::buildScanIteratively(
  ...
  auto *Ballot =
      B.CreateIntrinsic(Intrinsic::rvgpu_ballot, WaveTy, B.getTrue());
  ...
  V = B.CreateBitCast(V, IntNTy);
  Value *LaneValue =
      B.CreateIntrinsic(Intrinsic::rvgpu_readlane, {}, {V, LaneIdxInt});
  ...
  if (NeedResult) {
    OldValue =
        B.CreateIntrinsic(Intrinsic::rvgpu_writelane, {},
...
void RVGPUAtomicOptimizerImpl::optimizeAtomic(Instruction &I,
                                               AtomicRMWInst::BinOp Op,
                                               unsigned ValIdx,
                                               bool ValDivergent) const {
  ...
  if (IsPixelShader) {
    ...
    Value *const Cond = B.CreateIntrinsic(Intrinsic::rvgpu_ps_live, {}, {});
  ...
  Value *Mbcnt;
  if (ST->isWave32()) {
    Mbcnt = B.CreateIntrinsic(Intrinsic::rvgpu_mbcnt_lo, {},
                              {Ballot, B.getInt32(0)});
  } else {
    ...
    Mbcnt =
        B.CreateIntrinsic(Intrinsic::rvgpu_mbcnt_hi, {}, {ExtractHi, Mbcnt});
  }
  ...
  if (ValDivergent) {
    if (ScanImpl == ScanOptions::DPP) {
      ...
      NewV = B.CreateIntrinsic(Intrinsic::rvgpu_set_inactive, IntNTy,
                               {V, Identity});
      ...
      if (!NeedResult && ST->hasPermLaneX16()) {
        ...
        NewV = B.CreateIntrinsic(Intrinsic::rvgpu_readlane, {},
                                 {NewV, LastLaneIdx});
        NewV = B.CreateBitCast(NewV, Ty);
      }
      // Finally mark the readlanes in the WWM section.
      NewV = B.CreateIntrinsic(Intrinsic::rvgpu_strict_wwm, Ty, NewV);
  ...
  if (NeedResult) {
    ...
    if (TyBitWidth == 64) {
      ...
      CallInst *const ReadFirstLaneLo =
          B.CreateIntrinsic(Intrinsic::rvgpu_readfirstlane, {}, ExtractLo);
      CallInst *const ReadFirstLaneHi =
          B.CreateIntrinsic(Intrinsic::rvgpu_readfirstlane, {}, ExtractHi);
      ...
      BroadcastI =
          B.CreateIntrinsic(Intrinsic::rvgpu_readfirstlane, {}, CastedPhi);
    ...
    if (ValDivergent) {
      if (ScanImpl == ScanOptions::DPP) {
        LaneOffset =
            B.CreateIntrinsic(Intrinsic::rvgpu_strict_wwm, Ty, ExclScan);
      } else if (ScanImpl == ScanOptions::Iterative) {
        LaneOffset = ExclScan;
      } else {
        llvm_unreachable("Atomic Optimzer is disabled for None strategy");
...
INITIALIZE_PASS_BEGIN(RVGPUAtomicOptimizer, DEBUG_TYPE,
                      "RVGPU atomic optimizations", false, false)
INITIALIZE_PASS_DEPENDENCY(UniformityInfoWrapperPass)
INITIALIZE_PASS_DEPENDENCY(TargetPassConfig)
INITIALIZE_PASS_END(RVGPUAtomicOptimizer, DEBUG_TYPE,
                    "RVGPU atomic optimizations", false, false)

FunctionPass *llvm::createRVGPUAtomicOptimizerPass(ScanOptions ScanStrategy) {
  return new RVGPUAtomicOptimizer(ScanStrategy);
}
