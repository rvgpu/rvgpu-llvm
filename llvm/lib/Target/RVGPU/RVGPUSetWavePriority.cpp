//===- RVGPUSetWavePriority.cpp - Set wave priority ----------------------===//
//
...
#define DEBUG_TYPE "rvgpu-set-wave-priority"

static cl::opt<unsigned> DefaultVALUInstsThreshold(
    "rvgpu-set-wave-priority-valu-insts-threshold",
...
class RVGPUSetWavePriority : public MachineFunctionPass {
public:
  static char ID;

  RVGPUSetWavePriority() : MachineFunctionPass(ID) {}
...
INITIALIZE_PASS(RVGPUSetWavePriority, DEBUG_TYPE, "Set wave priority", false,
                false)

char RVGPUSetWavePriority::ID = 0;

FunctionPass *llvm::createRVGPUSetWavePriorityPass() {
  return new RVGPUSetWavePriority();
}

MachineInstr *
RVGPUSetWavePriority::BuildSetprioMI(MachineBasicBlock &MBB,
                                      MachineBasicBlock::iterator I,
                                      unsigned priority) const {
  return BuildMI(MBB, I, DebugLoc(), TII->get(RVGPU::S_SETPRIO))
      .addImm(priority);
}
...
static bool isVMEMLoad(const MachineInstr &MI) {
  return RVInstrInfo::isVMEM(MI) && MI.mayLoad();
}

bool RVGPUSetWavePriority::runOnMachineFunction(MachineFunction &MF) {
  ...
  Attribute A = F.getFnAttribute("rvgpu-wave-priority-threshold");
  if (A.isValid())
  ...
  while (I != E && !RVInstrInfo::isVALU(*I) && !I->isTerminator())
    ++I;
