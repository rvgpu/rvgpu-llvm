//===- RVGPUInsertSingleUseVDST.cpp - Insert s_singleuse_vdst instructions ==//
//
...
#define DEBUG_TYPE "rvgpu-insert-single-use-vdst"

namespace {
class RVGPUInsertSingleUseVDST : public MachineFunctionPass {
private:
...
public:
  ...
  RVGPUInsertSingleUseVDST() : MachineFunctionPass(ID) {}
  ...
  void emitSingleUseVDST(MachineInstr &MI) const {
    ...
    BuildMI(*MI.getParent(), MI, DebugLoc(), SII->get(RVGPU::S_SINGLEUSE_VDST))
        .addImm(0x1);
  }
  ...
  bool runOnMachineFunction(MachineFunction &MF) override {
    ...
    for (MachineBasicBlock &MBB : MF) {
      ...
      for (MachineInstr &MI : reverse(MBB.instrs())) {
        ...
        if (AllProducerOperandsAreSingleUse && RVInstrInfo::isVALU(MI)) {
...
char RVGPUInsertSingleUseVDST::ID = 0;

char &llvm::RVGPUInsertSingleUseVDSTID = RVGPUInsertSingleUseVDST::ID;

INITIALIZE_PASS(RVGPUInsertSingleUseVDST, DEBUG_TYPE,
                "RVGPU Insert SingleUseVDST", false, false)
