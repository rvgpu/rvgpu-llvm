...
// rvgpu-long-branch-factor cl argument which sets LongBranchFactor.
//===----------------------------------------------------------------------===//
#include "RVGPU.h"
...
#define DEBUG_TYPE "rvgpu-pre-ra-long-branch-reg"

namespace {

static cl::opt<double> LongBranchFactor(
    "rvgpu-long-branch-factor", cl::init(1.0), cl::Hidden,
...
public:
  ...
  StringRef getPassName() const override {
    return "RVGPU Pre-RA Long Branch Reg";
  }
...
INITIALIZE_PASS(GCNPreRALongBranchReg, DEBUG_TYPE,
                "RVGPU Pre-RA Long Branch Reg", false, false)
...
bool GCNPreRALongBranchReg::runOnMachineFunction(MachineFunction &MF) {
  ...
  const RVRegisterInfo *TRI = STM.getRegisterInfo();
  ...
  Register LongBranchReservedReg = TRI->findUnusedRegister(
      MRI, &RVGPU::SGPR_64RegClass, MF, ReserveHighestRegister);
