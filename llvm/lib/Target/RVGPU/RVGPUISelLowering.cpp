//===-- RVGPUISelLowering.cpp - RVGPU Common DAG lowering functions -----===//
//
...
#include "RVGPUGenCallingConv.inc"

static cl::opt<bool> RVGPUBypassSlowDiv(
  "rvgpu-bypass-slow-div",
  cl::desc("Skip 64-bit divide for dynamic 32-bit values"),
  cl::init(true));

// Find a larger type to do a load / store of a vector with.
EVT RVGPUTargetLowering::getEquivalentMemType(LLVMContext &Ctx, EVT VT) {
...
unsigned RVGPUTargetLowering::numBitsUnsigned(SDValue Op, SelectionDAG &DAG) {
  return DAG.computeKnownBits(Op).countMaxActiveBits();
}

unsigned RVGPUTargetLowering::numBitsSigned(SDValue Op, SelectionDAG &DAG) {
  // In order for this to be a signed 24-bit value, bit 23, must
  // be a sign bit.
  return DAG.ComputeMaxSignificantBits(Op);
}

RVGPUTargetLowering::RVGPUTargetLowering(const TargetMachine &TM,
  ...
  for (MVT VT : ScalarIntVTs) {
    ...
    // RVGPU uses ADDC/SUBC/ADDE/SUBE
...
bool RVGPUTargetLowering::mayIgnoreSignedZero(SDValue Op) const {
...
static bool fnegFoldsIntoOpcode(unsigned Opc) {
  ...
  case RVGPUISD::FMED3:
    // TODO: handle llvm.rvgpu.fma.legacy
    return true;
  case ISD::BITCAST:
...
bool RVGPUTargetLowering::allUsesHaveSourceMods(const SDNode *N,
...
EVT RVGPUTargetLowering::getTypeForExtReturn(LLVMContext &Context, EVT VT,
...
MVT RVGPUTargetLowering::getVectorIdxTy(const DataLayout &) const {
  return MVT::i32;
}

bool RVGPUTargetLowering::isSelectSupported(SelectSupportKind SelType) const {
  return true;
}

// The backend supports 32 and 64 bit floating point immediates.
// FIXME: Why are we reporting vectors of FP immediates as legal?
bool RVGPUTargetLowering::isFPImmLegal(const APFloat &Imm, EVT VT,
...
bool RVGPUTargetLowering::ShouldShrinkFPConstant(EVT VT) const {
  EVT ScalarVT = VT.getScalarType();
  return (ScalarVT != MVT::f32 && ScalarVT != MVT::f64);
}

bool RVGPUTargetLowering::shouldReduceLoadWidth(SDNode *N,
  ...
  if (OldSize >= 32 && NewSize < 32 && MN->getAlign() >= Align(4) &&
      (AS == RVGPUAS::CONSTANT_ADDRESS ||
       ...
       (isa<LoadSDNode>(N) && AS == RVGPUAS::GLOBAL_ADDRESS &&
        MN->isInvariant())) &&
      RVGPUInstrInfo::isUniformMMO(MN->getMemOperand()))
    return false;
...
bool RVGPUTargetLowering::isLoadBitCastBeneficial(EVT LoadTy, EVT CastTy,
...
// speculate things.
bool RVGPUTargetLowering::isCheapToSpeculateCttz(Type *Ty) const {
  return true;
}

bool RVGPUTargetLowering::isCheapToSpeculateCtlz(Type *Ty) const {
  return true;
}

bool RVGPUTargetLowering::isSDNodeAlwaysUniform(const SDNode *N) const {
  switch (N->getOpcode()) {
  case ISD::EntryToken:
  ...
  case ISD::LOAD:
    if (cast<LoadSDNode>(N)->getMemOperand()->getAddrSpace() ==
        RVGPUAS::CONSTANT_ADDRESS_32BIT)
      return true;
    return false;
  case RVGPUISD::SETCC: // ballot-style instruction
    return true;
  }
  return false;
}

SDValue RVGPUTargetLowering::getNegatedExpression(
  ...
  case RVGPUISD::RCP: {
    ...
    if (NegSrc)
      return DAG.getNode(RVGPUISD::RCP, SL, VT, NegSrc, Op->getFlags());
    return SDValue();
  }
  default:
    break;
...
bool RVGPUTargetLowering::isFAbsFree(EVT VT) const {
...
bool RVGPUTargetLowering::isFNegFree(EVT VT) const {
  assert(VT.isFloatingPoint());
...
bool RVGPUTargetLowering:: storeOfVectorConstantIsCheap(bool IsZero, EVT MemVT,
                                                         unsigned NumElem,
                                                         unsigned AS) const {
  return true;
}

bool RVGPUTargetLowering::aggressivelyPreferBuildVectorSources(EVT VecVT) const {
...
bool RVGPUTargetLowering::isTruncateFree(EVT Source, EVT Dest) const {
...
bool RVGPUTargetLowering::isTruncateFree(Type *Source, Type *Dest) const {
  // Truncate is just accessing a subregister.
...
bool RVGPUTargetLowering::isZExtFree(Type *Src, Type *Dest) const {
...
bool RVGPUTargetLowering::isZExtFree(EVT Src, EVT Dest) const {
...
bool RVGPUTargetLowering::isNarrowingProfitable(EVT SrcVT, EVT DestVT) const {
...
bool RVGPUTargetLowering::isDesirableToCommuteWithShift(
...
CCAssignFn *RVGPUCallLowering::CCAssignFnForCall(CallingConv::ID CC,
                                                  bool IsVarArg) {
  switch (CC) {
  ...
  case CallingConv::RVGPU_CS_ChainPreserve:
    return CC_RVGPU_CS_CHAIN;
  case CallingConv::C:
  case CallingConv::Fast:
  case CallingConv::Cold:
    return CC_RVGPU_Func;
...
CCAssignFn *RVGPUCallLowering::CCAssignFnForReturn(CallingConv::ID CC,
                                                    bool IsVarArg) {
  switch (CC) {
  case CallingConv::RVGPU_KERNEL:
  case CallingConv::SPIR_KERNEL:
...
/// with illegal types.  However, this does not work for the RVGPU targets
...
/// the ValVT.
void RVGPUTargetLowering::analyzeFormalArgumentsCompute(
  CCState &State,
  const SmallVectorImpl<ISD::InputArg> &Ins) const {
  const MachineFunction &MF = State.getMachineFunction();
  const Function &Fn = MF.getFunction();
  LLVMContext &Ctx = Fn.getParent()->getContext();
  const RVGPUSubtarget &ST = RVGPUSubtarget::get(MF);
...
SDValue RVGPUTargetLowering::LowerReturn(
  ...
  return DAG.getNode(RVGPUISD::ENDPGM, DL, MVT::Other, Chain);
}
...
CCAssignFn *RVGPUTargetLowering::CCAssignFnForCall(CallingConv::ID CC,
                                                    bool IsVarArg) {
  return RVGPUCallLowering::CCAssignFnForCall(CC, IsVarArg);
}

CCAssignFn *RVGPUTargetLowering::CCAssignFnForReturn(CallingConv::ID CC,
                                                      bool IsVarArg) {
  return RVGPUCallLowering::CCAssignFnForReturn(CC, IsVarArg);
}

SDValue RVGPUTargetLowering::addTokenForArgument(SDValue Chain,
                                                  SelectionDAG &DAG,
...
SDValue RVGPUTargetLowering::lowerUnhandledCall(CallLoweringInfo &CLI,
...
SDValue RVGPUTargetLowering::LowerCall(CallLoweringInfo &CLI,
...
SDValue RVGPUTargetLowering::LowerOperation(SDValue Op,
                                             SelectionDAG &DAG) const {
  switch (Op.getOpcode()) {
  default:
    Op->print(errs(), &DAG);
...
void RVGPUTargetLowering::ReplaceNodeResults(SDNode *N,
...
SDValue RVGPUTargetLowering::LowerGlobalAddress(RVGPUMachineFunction* MFI,
                                                 SDValue Op,
                                                 SelectionDAG &DAG) const {
  ...
  if (!MFI->isModuleEntryFunction()) {
    if (std::optional<uint32_t> Address =
            RVGPUMachineFunction::getLDSAbsoluteAddress(*GV)) {
      return DAG.getConstant(*Address, SDLoc(Op), Op.getValueType());
    }
  }

  if (G->getAddressSpace() == RVGPUAS::LOCAL_ADDRESS ||
      G->getAddressSpace() == RVGPUAS::REGION_ADDRESS) {
    if (!MFI->isModuleEntryFunction() &&
        !GV->getName().equals("llvm.rvgpu.module.lds")) {
      SDLoc DL(Op);
      const Function &Fn = DAG.getMachineFunction().getFunction();
...
SDValue RVGPUTargetLowering::LowerCONCAT_VECTORS(SDValue Op,
                                                  SelectionDAG &DAG) const {
...
SDValue RVGPUTargetLowering::LowerEXTRACT_SUBVECTOR(SDValue Op,
                                                     SelectionDAG &DAG) const {
  SDLoc SL(Op);
  SmallVector<SDValue, 8> Args;
...
SDValue RVGPUTargetLowering::combineFMinMaxLegacyImpl(
  ...
  case ISD::SETULT: {
    if (LHS == True)
      return DAG.getNode(RVGPUISD::FMIN_LEGACY, DL, VT, RHS, LHS);
    return DAG.getNode(RVGPUISD::FMAX_LEGACY, DL, VT, LHS, RHS);
  }
  case ISD::SETOLE:
  case ISD::SETOLT:
  case ISD::SETLE:
  case ISD::SETLT: {
    ...
    if (LHS == True)
      return DAG.getNode(RVGPUISD::FMIN_LEGACY, DL, VT, LHS, RHS);
    return DAG.getNode(RVGPUISD::FMAX_LEGACY, DL, VT, RHS, LHS);
  }
  case ISD::SETUGE:
  case ISD::SETUGT: {
    if (LHS == True)
...
SDValue RVGPUTargetLowering::combineFMinMaxLegacy(const SDLoc &DL, EVT VT,
...
RVGPUTargetLowering::split64BitValue(SDValue Op, SelectionDAG &DAG) const {
  SDLoc SL(Op);
...
SDValue RVGPUTargetLowering::getLoHalf64(SDValue Op, SelectionDAG &DAG) const {
  SDLoc SL(Op);
...
SDValue RVGPUTargetLowering::getHiHalf64(SDValue Op, SelectionDAG &DAG) const {
  SDLoc SL(Op);
...
std::pair<EVT, EVT>
RVGPUTargetLowering::getSplitDestVTs(const EVT &VT, SelectionDAG &DAG) const {
  EVT LoVT, HiVT;
...
// scalar.
std::pair<SDValue, SDValue>
RVGPUTargetLowering::splitVector(const SDValue &N, const SDLoc &DL,
...
SDValue RVGPUTargetLowering::SplitVectorLoad(const SDValue Op,
                                              SelectionDAG &DAG) const {
...
SDValue RVGPUTargetLowering::WidenOrSplitVectorLoad(SDValue Op,
                                                     SelectionDAG &DAG) const {
  LoadSDNode *Load = cast<LoadSDNode>(Op);
  EVT VT = Op.getValueType();
  SDValue BasePtr = Load->getBasePtr();
...
SDValue RVGPUTargetLowering::SplitVectorStore(SDValue Op,
                                               SelectionDAG &DAG) const {
...
SDValue RVGPUTargetLowering::LowerDIVREM24(SDValue Op, SelectionDAG &DAG,
                                            bool Sign) const {
  SDLoc DL(Op);
  EVT VT = Op.getValueType();
  ...
  SDValue fq = DAG.getNode(ISD::FMUL, DL, FltVT,
                           fa, DAG.getNode(RVGPUISD::RCP, DL, FltVT, fb));
  ...
  unsigned OpCode = !Subtarget->hasMadMacF32Insts() ? (unsigned)ISD::FMA
                    : UseFmadFtz ? (unsigned)RVGPUISD::FMAD_FTZ
                                 : (unsigned)ISD::FMAD;
...
void RVGPUTargetLowering::LowerUDIVREM64(SDValue Op,
                                      SelectionDAG &DAG,
  ...
  if (isTypeLegal(MVT::i64)) {
    ...
    SDValue Mad1 = DAG.getNode(FMAD, DL, MVT::f32, Cvt_Hi,
      ...
      Cvt_Lo);
    SDValue Rcp = DAG.getNode(RVGPUISD::RCP, DL, MVT::f32, Mad1);
...
SDValue RVGPUTargetLowering::LowerUDIVREM(SDValue Op,
                                           SelectionDAG &DAG) const {
  SDLoc DL(Op);
  EVT VT = Op.getValueType();
  ...
  // See RVGPUCodeGenPrepare::expandDivRem32 for a description of the
  // algorithm used here.

  // Initial estimate of inv(y).
  SDValue Z = DAG.getNode(RVGPUISD::URECIP, DL, VT, Y);
...
SDValue RVGPUTargetLowering::LowerSDIVREM(SDValue Op,
                                           SelectionDAG &DAG) const {
  SDLoc DL(Op);
  EVT VT = Op.getValueType();
...
SDValue RVGPUTargetLowering::LowerFREM(SDValue Op, SelectionDAG &DAG) const {
  SDLoc SL(Op);
  EVT VT = Op.getValueType();
...
SDValue RVGPUTargetLowering::LowerFCEIL(SDValue Op, SelectionDAG &DAG) const {
  SDLoc SL(Op);
  SDValue Src = Op.getOperand(0);
...
static SDValue extractF64Exponent(SDValue Hi, const SDLoc &SL,
  ...
  SDValue ExpPart = DAG.getNode(RVGPUISD::BFE_U32, SL, MVT::i32,
                                Hi,
...
SDValue RVGPUTargetLowering::LowerFTRUNC(SDValue Op, SelectionDAG &DAG) const {
  SDLoc SL(Op);
  SDValue Src = Op.getOperand(0);
...
SDValue RVGPUTargetLowering::LowerFROUNDEVEN(SDValue Op,
                                              SelectionDAG &DAG) const {
  SDLoc SL(Op);
  SDValue Src = Op.getOperand(0);
...
SDValue RVGPUTargetLowering::LowerFNEARBYINT(SDValue Op,
                                              SelectionDAG &DAG) const {
...
SDValue RVGPUTargetLowering::LowerFRINT(SDValue Op, SelectionDAG &DAG) const {
...
SDValue RVGPUTargetLowering::LowerFFLOOR(SDValue Op, SelectionDAG &DAG) const {
  SDLoc SL(Op);
  SDValue Src = Op.getOperand(0);
...
bool RVGPUTargetLowering::allowApproxFunc(const SelectionDAG &DAG,
                                           SDNodeFlags Flags) {
...
bool RVGPUTargetLowering::needsDenormHandlingF32(const SelectionDAG &DAG,
                                                  SDValue Src,
                                                  SDNodeFlags Flags) {
...
SDValue RVGPUTargetLowering::getIsLtSmallestNormal(SelectionDAG &DAG,
                                                    SDValue Src,
...
SDValue RVGPUTargetLowering::getIsFinite(SelectionDAG &DAG, SDValue Src,
                                          SDNodeFlags Flags) const {
  SDLoc SL(Src);
  EVT VT = Src.getValueType();
  const fltSemantics &Semantics = SelectionDAG::EVTToAPFloatSemantics(VT);
...
std::pair<SDValue, SDValue>
RVGPUTargetLowering::getScaledLogInput(SelectionDAG &DAG, const SDLoc SL,
...
SDValue RVGPUTargetLowering::LowerFLOG2(SDValue Op, SelectionDAG &DAG) const {
  ...
  // log2 = rvgpu_log2 - (is_denormal ? 32.0 : 0.0)
  ...
  if (VT == MVT::f16) {
    ...
    SDValue Log = DAG.getNode(RVGPUISD::LOG, SL, MVT::f32, Ext, Flags);
  ...
  if (!ScaledInput)
    return DAG.getNode(RVGPUISD::LOG, SL, VT, Src, Flags);

  SDValue Log2 = DAG.getNode(RVGPUISD::LOG, SL, VT, ScaledInput, Flags);
...
SDValue RVGPUTargetLowering::LowerFLOGCommon(SDValue Op,
                                              SelectionDAG &DAG) const {
  SDValue X = Op.getOperand(0);
  EVT VT = Op.getValueType();
  SDNodeFlags Flags = Op->getFlags();
  ...
  SDValue Y = DAG.getNode(RVGPUISD::LOG, DL, VT, X, Flags);
...
SDValue RVGPUTargetLowering::LowerFLOG10(SDValue Op, SelectionDAG &DAG) const {
  return LowerFLOGCommon(Op, DAG);
}

// Do f32 fast math expansion for flog2 or flog10. This is accurate enough for a
// promote f16 operation.
SDValue RVGPUTargetLowering::LowerFLOGUnsafe(SDValue Src, const SDLoc &SL,
                                              SelectionDAG &DAG, bool IsLog10,
                                              SDNodeFlags Flags) const {
  EVT VT = Src.getValueType();
  unsigned LogOp =
      VT == MVT::f32 ? (unsigned)RVGPUISD::LOG : (unsigned)ISD::FLOG2;
  ...
  if (VT == MVT::f32) {
    ...
    if (ScaledInput) {
      SDValue LogSrc = DAG.getNode(RVGPUISD::LOG, SL, VT, ScaledInput, Flags);
...
SDValue RVGPUTargetLowering::lowerFEXP2(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (VT == MVT::f16) {
    ...
    assert(!Subtarget->has16BitInsts());
    SDValue Ext = DAG.getNode(ISD::FP_EXTEND, SL, MVT::f32, Src, Flags);
    SDValue Log = DAG.getNode(RVGPUISD::EXP, SL, MVT::f32, Ext, Flags);
    return DAG.getNode(ISD::FP_ROUND, SL, VT, Log,
                       DAG.getTargetConstant(0, SL, MVT::i32), Flags);
  }
  ...
  if (!needsDenormHandlingF32(DAG, Src, Flags))
    return DAG.getNode(RVGPUISD::EXP, SL, MVT::f32, Src, Flags);
  ...
  SDValue Exp2 = DAG.getNode(RVGPUISD::EXP, SL, VT, AddInput, Flags);
...
SDValue RVGPUTargetLowering::lowerFEXPUnsafe(SDValue X, const SDLoc &SL,
                                              SelectionDAG &DAG,
                                              SDNodeFlags Flags) const {
  ...
  if (VT != MVT::f32 || !needsDenormHandlingF32(DAG, X, Flags)) {
    ...
    return DAG.getNode(VT == MVT::f32 ? (unsigned)RVGPUISD::EXP
                                      : (unsigned)ISD::FEXP2,
                       SL, VT, Mul, Flags);
  }
  ...
  SDValue Exp2 = DAG.getNode(RVGPUISD::EXP, SL, VT, ExpInput, Flags);
...
/// handled correctly.
SDValue RVGPUTargetLowering::lowerFEXP10Unsafe(SDValue X, const SDLoc &SL,
                                                SelectionDAG &DAG,
                                                SDNodeFlags Flags) const {
  const EVT VT = X.getValueType();
  const unsigned Exp2Op = VT == MVT::f32 ? RVGPUISD::EXP : ISD::FEXP2;
...
SDValue RVGPUTargetLowering::lowerFEXP(SDValue Op, SelectionDAG &DAG) const {
  EVT VT = Op.getValueType();
  SDLoc SL(Op);
  SDValue X = Op.getOperand(0);
  SDNodeFlags Flags = Op->getFlags();
  ...
  SDValue Exp2 = DAG.getNode(RVGPUISD::EXP, SL, VT, A, Flags);
...
SDValue RVGPUTargetLowering::LowerCTLZ_CTTZ(SDValue Op, SelectionDAG &DAG) const {
  SDLoc SL(Op);
  SDValue Src = Op.getOperand(0);

  assert(isCtlzOpc(Op.getOpcode()) || isCttzOpc(Op.getOpcode()));
  bool Ctlz = isCtlzOpc(Op.getOpcode());
  unsigned NewOpc = Ctlz ? RVGPUISD::FFBH_U32 : RVGPUISD::FFBL_B32;
...
SDValue RVGPUTargetLowering::LowerINT_TO_FP32(SDValue Op, SelectionDAG &DAG,
                                               bool Signed) const {
  ...
  if (Signed && Subtarget->isGCN()) {
    ...
    ShAmt = DAG.getNode(RVGPUISD::FFBH_I32, SL, MVT::i32, Hi);
...
SDValue RVGPUTargetLowering::LowerINT_TO_FP64(SDValue Op, SelectionDAG &DAG,
                                               bool Signed) const {
  SDLoc SL(Op);
  SDValue Src = Op.getOperand(0);
...
SDValue RVGPUTargetLowering::LowerUINT_TO_FP(SDValue Op,
                                               SelectionDAG &DAG) const {
...
SDValue RVGPUTargetLowering::LowerSINT_TO_FP(SDValue Op,
                                              SelectionDAG &DAG) const {
  EVT DestVT = Op.getValueType();
...
SDValue RVGPUTargetLowering::LowerFP_TO_INT64(SDValue Op, SelectionDAG &DAG,
                                               bool Signed) const {
  SDLoc SL(Op);
...
SDValue RVGPUTargetLowering::LowerFP_TO_FP16(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  SDValue N0 = Op.getOperand(0);

  // Convert to target node to get known bits
  if (N0.getValueType() == MVT::f32)
    return DAG.getNode(RVGPUISD::FP_TO_FP16, DL, Op.getValueType(), N0);
...
SDValue RVGPUTargetLowering::LowerSIGN_EXTEND_INREG(SDValue Op,
                                                     SelectionDAG &DAG) const {
...
static bool isU24(SDValue Op, SelectionDAG &DAG) {
  return RVGPUTargetLowering::numBitsUnsigned(Op, DAG) <= 24;
}
...
static bool isI24(SDValue Op, SelectionDAG &DAG) {
  ...
  return VT.getSizeInBits() >= 24 && // Types less than 24-bit should be treated
         ...
         RVGPUTargetLowering::numBitsSigned(Op, DAG) <= 24;
}
...
static SDValue simplifyMul24(SDNode *Node24,
  ...
  if (IsIntrin) {
    ...
    case Intrinsic::rvgpu_mul_i24:
      NewOpcode = RVGPUISD::MUL_I24;
      break;
    case Intrinsic::rvgpu_mul_u24:
      NewOpcode = RVGPUISD::MUL_U24;
      break;
    case Intrinsic::rvgpu_mulhi_i24:
      NewOpcode = RVGPUISD::MULHI_I24;
      break;
    case Intrinsic::rvgpu_mulhi_u24:
      NewOpcode = RVGPUISD::MULHI_U24;
      break;
    default:
...
bool RVGPUTargetLowering::shouldCombineMemoryType(EVT VT) const {
...
// type.
SDValue RVGPUTargetLowering::performLoadCombine(SDNode *N,
                                                 DAGCombinerInfo &DCI) const {
...
// type.
SDValue RVGPUTargetLowering::performStoreCombine(SDNode *N,
                                                  DAGCombinerInfo &DCI) const {
  if (!DCI.isBeforeLegalize())
    return SDValue();
...
// issues.
SDValue RVGPUTargetLowering::performAssertSZExtCombine(SDNode *N,
                                                        DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
...
SDValue RVGPUTargetLowering::performIntrinsicWOChainCombine(
...
SDValue RVGPUTargetLowering::splitBinaryBitConstantOpImpl(
...
SDValue RVGPUTargetLowering::performShlCombine(SDNode *N,
                                                DAGCombinerInfo &DCI) const {
...
SDValue RVGPUTargetLowering::performSraCombine(SDNode *N,
                                                DAGCombinerInfo &DCI) const {
...
SDValue RVGPUTargetLowering::performSrlCombine(SDNode *N,
                                                DAGCombinerInfo &DCI) const {
...
SDValue RVGPUTargetLowering::performTruncateCombine(
  SDNode *N, DAGCombinerInfo &DCI) const {
  SDLoc SL(N);
  SelectionDAG &DAG = DCI.DAG;
  EVT VT = N->getValueType(0);
...
static SDValue getMul24(SelectionDAG &DAG, const SDLoc &SL,
  ...
  if (Size <= 32) {
    unsigned MulOpc = Signed ? RVGPUISD::MUL_I24 : RVGPUISD::MUL_U24;
    return DAG.getNode(MulOpc, SL, MVT::i32, N0, N1);
  }

  unsigned MulLoOpc = Signed ? RVGPUISD::MUL_I24 : RVGPUISD::MUL_U24;
  unsigned MulHiOpc = Signed ? RVGPUISD::MULHI_I24 : RVGPUISD::MULHI_U24;
...
SDValue RVGPUTargetLowering::performMulCombine(SDNode *N,
                                                DAGCombinerInfo &DCI) const {
  EVT VT = N->getValueType(0);
...
SDValue
RVGPUTargetLowering::performMulLoHiCombine(SDNode *N,
                                            DAGCombinerInfo &DCI) const {
  ...
  if (Subtarget->hasMulU24() && isU24(N0, DAG) && isU24(N1, DAG)) {
    N0 = DAG.getZExtOrTrunc(N0, DL, MVT::i32);
    N1 = DAG.getZExtOrTrunc(N1, DL, MVT::i32);
    LoOpcode = RVGPUISD::MUL_U24;
    HiOpcode = RVGPUISD::MULHI_U24;
  } else if (Subtarget->hasMulI24() && isI24(N0, DAG) && isI24(N1, DAG)) {
    N0 = DAG.getSExtOrTrunc(N0, DL, MVT::i32);
    N1 = DAG.getSExtOrTrunc(N1, DL, MVT::i32);
    LoOpcode = RVGPUISD::MUL_I24;
    HiOpcode = RVGPUISD::MULHI_I24;
  } else {
    return SDValue();
  }
...
SDValue RVGPUTargetLowering::performMulhsCombine(SDNode *N,
                                                  DAGCombinerInfo &DCI) const {
  EVT VT = N->getValueType(0);
  ...
  SDValue Mulhi = DAG.getNode(RVGPUISD::MULHI_I24, DL, MVT::i32, N0, N1);
  DCI.AddToWorklist(Mulhi.getNode());
  return DAG.getSExtOrTrunc(Mulhi, DL, VT);
}

SDValue RVGPUTargetLowering::performMulhuCombine(SDNode *N,
                                                  DAGCombinerInfo &DCI) const {
  EVT VT = N->getValueType(0);
  ...
  SDValue Mulhi = DAG.getNode(RVGPUISD::MULHI_U24, DL, MVT::i32, N0, N1);
  DCI.AddToWorklist(Mulhi.getNode());
  return DAG.getZExtOrTrunc(Mulhi, DL, VT);
}

SDValue RVGPUTargetLowering::getFFBX_U32(SelectionDAG &DAG,
                                          SDValue Op,
                                          const SDLoc &DL,
                                          unsigned Opc) const {
  EVT VT = Op.getValueType();
...
SDValue RVGPUTargetLowering::performCtlz_CttzCombine(const SDLoc &SL, SDValue Cond,
                                                 SDValue LHS, SDValue RHS,
                                                 DAGCombinerInfo &DCI) const {
  ...
  if (CCOpcode == ISD::SETEQ &&
    ...
    unsigned Opc =
        isCttzOpc(RHS.getOpcode()) ? RVGPUISD::FFBL_B32 : RVGPUISD::FFBH_U32;
  ...
  if (CCOpcode == ISD::SETNE &&
    ...
    unsigned Opc =
        isCttzOpc(LHS.getOpcode()) ? RVGPUISD::FFBL_B32 : RVGPUISD::FFBH_U32;
...
SDValue
RVGPUTargetLowering::foldFreeOpFromSelect(TargetLowering::DAGCombinerInfo &DCI,
                                           SDValue N) const {
  SelectionDAG &DAG = DCI.DAG;
  ...
  if ((LHS.getOpcode() == ISD::FABS && RHS.getOpcode() == ISD::FABS) ||
    ...
    if (!RVGPUTargetLowering::allUsesHaveSourceMods(N.getNode()))
      return SDValue();
...
SDValue RVGPUTargetLowering::performSelectCombine(SDNode *N,
                                                   DAGCombinerInfo &DCI) const {
...
RVGPUTargetLowering::getConstantNegateCost(const ConstantFPSDNode *C) const {
  if (C->isZero())
...
bool RVGPUTargetLowering::isConstantCostlierToNegate(SDValue N) const {
  if (const ConstantFPSDNode *C = isConstOrConstSplatFP(N))
    return getConstantNegateCost(C) == NegatibleCost::Expensive;
  return false;
}

bool RVGPUTargetLowering::isConstantCheaperToNegate(SDValue N) const {
  if (const ConstantFPSDNode *C = isConstOrConstSplatFP(N))
...
static unsigned inverseMinMax(unsigned Opc) {
  ...
  case ISD::FMINIMUM:
    return ISD::FMAXIMUM;
  case RVGPUISD::FMAX_LEGACY:
    return RVGPUISD::FMIN_LEGACY;
  case RVGPUISD::FMIN_LEGACY:
    return  RVGPUISD::FMAX_LEGACY;
  default:
...
/// instruction.
bool RVGPUTargetLowering::shouldFoldFNegIntoSrc(SDNode *N, SDValue N0) {
...
SDValue RVGPUTargetLowering::performFNegCombine(SDNode *N,
                                                 DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
  SDValue N0 = N->getOperand(0);
  EVT VT = N->getValueType(0);
  ...
  case RVGPUISD::FMED3: {
    ...
    SDValue Res = DAG.getNode(RVGPUISD::FMED3, SL, VT, Ops, N0->getFlags());
    if (Res.getOpcode() != RVGPUISD::FMED3)
      return SDValue(); // Op got folded away.
...
SDValue RVGPUTargetLowering::performFAbsCombine(SDNode *N,
                                                 DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
  SDValue N0 = N->getOperand(0);
...
SDValue RVGPUTargetLowering::performRcpCombine(SDNode *N,
                                                DAGCombinerInfo &DCI) const {
...
SDValue RVGPUTargetLowering::PerformDAGCombine(SDNode *N,
  ...
  case RVGPUISD::MUL_U24:
  case RVGPUISD::MUL_I24: {
    if (SDValue Simplified = simplifyMul24(N, DCI))
      return Simplified;
    return performMulCombine(N, DCI);
  }
  case RVGPUISD::MULHI_I24:
  case RVGPUISD::MULHI_U24:
    return simplifyMul24(N, DCI);
  case ISD::SMUL_LOHI:
  case ISD::UMUL_LOHI:
  ...
  case RVGPUISD::BFE_U32: {
    ...
    bool Signed = N->getOpcode() == RVGPUISD::BFE_I32;
...
SDValue RVGPUTargetLowering::CreateLiveInRegister(SelectionDAG &DAG,
...
SDValue RVGPUTargetLowering::loadStackInputValue(SelectionDAG &DAG,
                                                  EVT VT,
                                                  const SDLoc &SL,
...
SDValue RVGPUTargetLowering::storeStackInputValue(SelectionDAG &DAG,
                                                   const SDLoc &SL,
                                                   SDValue Chain,
                                                   SDValue ArgVal,
                                                   int64_t Offset) const {
...
uint32_t RVGPUTargetLowering::getImplicitParameterOffset(
  ...
  case FIRST_IMPLICIT:
    return ArgOffset;
  case PRIVATE_BASE:
    return ArgOffset + RVGPU::ImplicitArg::PRIVATE_BASE_OFFSET;
  case SHARED_BASE:
    return ArgOffset + RVGPU::ImplicitArg::SHARED_BASE_OFFSET;
  case QUEUE_PTR:
    return ArgOffset + RVGPU::ImplicitArg::QUEUE_PTR_OFFSET;
  }
...
const char* RVGPUTargetLowering::getTargetNodeName(unsigned Opcode) const {
  ...
  case RVGPUISD::FIRST_MEM_OPCODE_NUMBER: break;
  ...
  case RVGPUISD::LAST_RVGPU_ISD_NUMBER: break;
  }
  return nullptr;
}

SDValue RVGPUTargetLowering::getSqrtEstimate(SDValue Operand,
  ...
  if (VT == MVT::f32) {
    RefinementSteps = 0;
    return DAG.getNode(RVGPUISD::RSQ, SDLoc(Operand), VT, Operand);
  }
...
SDValue RVGPUTargetLowering::getRecipEstimate(SDValue Operand,
                                               SelectionDAG &DAG, int Enabled,
  ...
  if (VT == MVT::f32) {
    ...
    RefinementSteps = 0;
    return DAG.getNode(RVGPUISD::RCP, SDLoc(Operand), VT, Operand);
  }
...
void RVGPUTargetLowering::computeKnownBitsForTargetNode(
  ...
  case RVGPUISD::BFE_U32: {
    ...
    if (Opc == RVGPUISD::BFE_U32)
      Known.Zero = APInt::getHighBitsSet(32, 32 - Width);

    break;
  }
  case RVGPUISD::FP_TO_FP16: {
  ...
  case RVGPUISD::BUFFER_LOAD_UBYTE:  {
    Known.Zero.setHighBits(24);
    break;
  }
  case RVGPUISD::BUFFER_LOAD_USHORT: {
...
unsigned RVGPUTargetLowering::ComputeNumSignBitsForTargetNode(
  ...
  case RVGPUISD::BORROW:
    return 31;
  case RVGPUISD::BUFFER_LOAD_BYTE:
    return 25;
  case RVGPUISD::BUFFER_LOAD_SHORT:
    return 17;
  case RVGPUISD::BUFFER_LOAD_UBYTE:
    return 24;
  case RVGPUISD::BUFFER_LOAD_USHORT:
...
unsigned RVGPUTargetLowering::computeNumSignBitsForTargetInstr(
...
bool RVGPUTargetLowering::isKnownNeverNaNForTargetNode(SDValue Op,
                                                        const SelectionDAG &DAG,
                                                        bool SNaN,
                                                        unsigned Depth) const {
...
bool RVGPUTargetLowering::isReassocProfitable(MachineRegisterInfo &MRI,
                                               Register N0, Register N1) const {
  return MRI.hasOneNonDBGUse(N0); // FIXME: handle regbanks
}

TargetLowering::AtomicExpansionKind
RVGPUTargetLowering::shouldExpandAtomicRMWInIR(AtomicRMWInst *RMW) const {
...
bool RVGPUTargetLowering::shouldSinkOperands(
