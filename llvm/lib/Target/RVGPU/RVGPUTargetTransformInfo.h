...
class RVTargetLowering;
class Type;
class Value;

class RVGPUTTIImpl final : public BasicTTIImplBase<RVGPUTTIImpl> {
  using BaseT = BasicTTIImplBase<RVGPUTTIImpl>;
...
public:
  ...
  bool isValidAddrSpaceCast(unsigned FromAS, unsigned ToAS) const {
    ...
    if ((FromAS == RVGPUAS::CONSTANT_ADDRESS_32BIT &&
         ToAS == RVGPUAS::CONSTANT_ADDRESS) ||
        (FromAS == RVGPUAS::CONSTANT_ADDRESS &&
         ToAS == RVGPUAS::CONSTANT_ADDRESS_32BIT))
      return true;
    return false;
  }

  bool addrspacesMayAlias(unsigned AS0, unsigned AS1) const {
    return RVGPU::addrspacesMayAlias(AS0, AS1);
  }
...
#endif // LLVM_LIB_TARGET_RVGPU_RVGPUTARGETTRANSFORMINFO_H
