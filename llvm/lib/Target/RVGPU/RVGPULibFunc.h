...
#ifndef _RVGPU_LIBFUNC_H_
#define _RVGPU_LIBFUNC_H_
...
class RVGPULibFuncImpl : public RVGPULibFuncBase {
public:
  RVGPULibFuncImpl() = default;
  virtual ~RVGPULibFuncImpl() = default;
  ...
  bool isMangled() const { return RVGPULibFuncBase::isMangled(FuncId); }
...
/// Wrapper class for RVGPULIbFuncImpl
class RVGPULibFunc : public RVGPULibFuncBase {
public:
  explicit RVGPULibFunc() : Impl(std::unique_ptr<RVGPULibFuncImpl>()) {}
  RVGPULibFunc(const RVGPULibFunc &F);
  /// Clone a mangled library func with the Id \p Id and argument info from \p
  /// CopyFrom.
  explicit RVGPULibFunc(EFuncId Id, const RVGPULibFunc &CopyFrom);
  explicit RVGPULibFunc(EFuncId Id, FunctionType *FT, bool SignedInts);

  /// Construct an unmangled library function on the fly.
  explicit RVGPULibFunc(StringRef FName, FunctionType *FT);

  RVGPULibFunc &operator=(const RVGPULibFunc &F);
  ...
  static Function *getFunction(llvm::Module *M, const RVGPULibFunc &fInfo);

  static FunctionCallee getOrInsertFunction(llvm::Module *M,
                                            const RVGPULibFunc &fInfo);
  static bool parse(StringRef MangledName, RVGPULibFunc &Ptr);

private:
  /// Initialize as a mangled library function.
  void initMangled();
  std::unique_ptr<RVGPULibFuncImpl> Impl;
};

class RVGPUMangledLibFunc : public RVGPULibFuncImpl {
public:
  Param Leads[2];

  explicit RVGPUMangledLibFunc();
  explicit RVGPUMangledLibFunc(EFuncId id,
                                const RVGPUMangledLibFunc &copyFrom);
  explicit RVGPUMangledLibFunc(EFuncId id, FunctionType *FT,
  ...
  static bool classof(const RVGPULibFuncImpl *F) { return F->isMangled(); }
...
class RVGPUUnmangledLibFunc : public RVGPULibFuncImpl {
  FunctionType *FuncTy;

public:
  explicit RVGPUUnmangledLibFunc();
  explicit RVGPUUnmangledLibFunc(StringRef FName, FunctionType *FT) {
