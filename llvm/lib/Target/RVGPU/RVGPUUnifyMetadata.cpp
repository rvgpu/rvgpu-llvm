//===- RVGPUUnifyMetadata.cpp - Unify OpenCL metadata --------------------===//
//
...
namespace {
  ...
  class RVGPUUnifyMetadata : public ModulePass {
  public:
    static char ID;

    explicit RVGPUUnifyMetadata() : ModulePass(ID) {}
  ...
  char RVGPUUnifyMetadata::ID = 0;

  char &llvm::RVGPUUnifyMetadataID = RVGPUUnifyMetadata::ID;

  INITIALIZE_PASS(RVGPUUnifyMetadata, "rvgpu-unify-metadata",
                  "Unify multiple OpenCL metadata due to linking", false, false)

  ModulePass *llvm::createRVGPUUnifyMetadataPass() {
    return new RVGPUUnifyMetadata();
  }

  bool RVGPUUnifyMetadata::runOnModule(Module &M) {
    return unifyMetadataImpl(M);
  }

  PreservedAnalyses RVGPUUnifyMetadataPass::run(Module &M,
