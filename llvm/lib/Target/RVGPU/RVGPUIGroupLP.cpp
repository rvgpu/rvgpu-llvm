//===--- RVGPUIGroupLP.cpp - RVGPU IGroupLP  ------------===//
//
...
static cl::opt<bool> EnableExactSolver(
    "rvgpu-igrouplp-exact-solver", cl::Hidden,
...
static cl::opt<unsigned> CutoffForExact(
    "rvgpu-igrouplp-exact-solver-cutoff", cl::init(0), cl::Hidden,
    cl::desc("The maximum number of scheduling group conflicts "
             ...
             "the solver (e.g. by rvgpu-igrouplp-exact-solver"));

static cl::opt<uint64_t> MaxBranchesExplored(
    "rvgpu-igrouplp-exact-solver-max-branches", cl::init(0), cl::Hidden,
...
static cl::opt<bool> UseCostHeur(
    "rvgpu-igrouplp-exact-solver-cost-heur", cl::init(true), cl::Hidden,
...
public:
  ...
  InstructionRule(const RVInstrInfo *TII, unsigned SGID,
...
public:
  ...
  SchedGroup(SchedGroupMask SGMask, std::optional<unsigned> MaxSize,
             ScheduleDAGInstrs *DAG, const RVInstrInfo *TII)
...
static void resetEdges(SUnit &SU, ScheduleDAGInstrs *DAG) {
  assert(SU.getInstr()->getOpcode() == RVGPU::SCHED_BARRIER ||
         SU.getInstr()->getOpcode() == RVGPU::SCHED_GROUP_BARRIER ||
         SU.getInstr()->getOpcode() == RVGPU::IGLP_OPT);
  ...
  for (auto &SyncPipeline : CurrPipeline) {
    for (auto &SG : SyncPipeline) {
      ...
      auto SchedBarr = llvm::find_if(TempCollection, [](SUnit *SU) {
        return SU->getInstr()->getOpcode() == RVGPU::SCHED_GROUP_BARRIER;
      });
  ...
  for (auto &SyncPipeline : BestPipeline) {
    ...
    for (auto &SG : SyncPipeline) {
      ...
      for (auto &SU : SG.Collection) {
        if (SU->getInstr()->getOpcode() == RVGPU::SCHED_GROUP_BARRIER)
          SGBarr = SU;
...
public:
  ...
  IGLPStrategy(ScheduleDAGInstrs *DAG, const RVInstrInfo *TII)
      : DAG(DAG), TII(TII) {}
...
public:
  ...
  MFMASmallGemmOpt(ScheduleDAGInstrs *DAG, const RVInstrInfo *TII)
...
private:
  ...
  public:
    ...
    EnablesInitialMFMA(const RVInstrInfo *TII, unsigned SGID,
                       bool NeedsCache = false)
  ...
  public:
    bool apply(const SUnit *SU, const ArrayRef<SUnit *> Collection,
               SmallVectorImpl<SchedGroup> &SyncPipe) override {
      auto MI = SU->getInstr();
      if (MI->getOpcode() != RVGPU::V_PERM_B32_e64)
        return false;
    ...
    IsPermForDSW(const RVInstrInfo *TII, unsigned SGID, bool NeedsCache = false)
        : InstructionRule(TII, SGID, NeedsCache) {}
  };
  ...
  public:
    ...
    IsSuccOfPrevGroup(const RVInstrInfo *TII, unsigned SGID,
                      bool NeedsCache = false)
        : InstructionRule(TII, SGID, NeedsCache) {}
  };
  ...
  public:
    ...
    VMEMSize(const RVInstrInfo *TII, unsigned SGID, bool NeedsCache = false)
        : InstructionRule(TII, SGID, NeedsCache) {}
  };
  ...
  public:
    ...
    SharesPredWithPrevNthGroup(unsigned Distance, const RVInstrInfo *TII,
...
public:
  ...
  MFMASmallGemmSingleWaveOpt(ScheduleDAGInstrs *DAG, const RVInstrInfo *TII)
      : IGLPStrategy(DAG, TII) {
    IsBottomUp = 0;
  }
};
  ...
  if (!IsReentry) {
    ...
    for (; I != E; I++) {
      ...
      for (auto &Pred : (*I)->Preds) {
        if (Pred.getSUnit()->getInstr()->getOpcode() != RVGPU::V_PERM_B32_e64)
          continue;
...
int SchedGroup::link(SUnit &SU, bool MakePred,
  ...
  for (auto *A : Collection) {
    SUnit *B = &SU;
    if (A == B || A->getInstr()->getOpcode() == RVGPU::SCHED_GROUP_BARRIER)
      continue;
    if (MakePred)
      std::swap(A, B);
...
void SchedGroup::link(SUnit &SU, bool MakePred) {
  for (auto *A : Collection) {
    SUnit *B = &SU;
    if (A->getInstr()->getOpcode() == RVGPU::SCHED_GROUP_BARRIER)
      continue;
    if (MakePred)
      std::swap(A, B);
...
void IGroupLPDAGMutation::apply(ScheduleDAGInstrs *DAGInstrs) {
  ...
  for (auto R = DAG->SUnits.rbegin(), E = DAG->SUnits.rend(); R != E; ++R) {
    ...
    if (Opc == RVGPU::SCHED_BARRIER) {
      addSchedBarrierEdges(*R);
      foundSB = true;
    } else if (Opc == RVGPU::SCHED_GROUP_BARRIER) {
      initSchedGroupBarrierPipelineStage(R);
      foundSB = true;
    } else if (Opc == RVGPU::IGLP_OPT) {
      resetEdges(*R, DAG);
...
void IGroupLPDAGMutation::addSchedBarrierEdges(SUnit &SchedBarrier) {
  ...
  assert(MI.getOpcode() == RVGPU::SCHED_BARRIER);
...
void IGroupLPDAGMutation::initSchedGroupBarrierPipelineStage(
  ...
  assert(SGB.getOpcode() == RVGPU::SCHED_GROUP_BARRIER);
