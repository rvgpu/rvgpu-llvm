...
//
// To achieve optimal code generation for RVGPU, we assume that uniformity
...
#define DEBUG_TYPE "rvgpu-rewrite-undef-for-phi"

namespace {

class RVGPURewriteUndefForPHILegacy : public FunctionPass {
public:
  static char ID;
  RVGPURewriteUndefForPHILegacy() : FunctionPass(ID) {
    initializeRVGPURewriteUndefForPHILegacyPass(*PassRegistry::getPassRegistry());
  }
  bool runOnFunction(Function &F) override;
  StringRef getPassName() const override {
    return "RVGPU Rewrite Undef for PHI";
  }
...
char RVGPURewriteUndefForPHILegacy::ID = 0;

INITIALIZE_PASS_BEGIN(RVGPURewriteUndefForPHILegacy, DEBUG_TYPE,
...
bool RVGPURewriteUndefForPHILegacy::runOnFunction(Function &F) {
  UniformityInfo &UA =
...
FunctionPass *llvm::createRVGPURewriteUndefForPHILegacyPass() {
  return new RVGPURewriteUndefForPHILegacy();
}
