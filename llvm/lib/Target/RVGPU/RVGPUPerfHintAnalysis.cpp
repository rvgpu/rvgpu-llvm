//===- RVGPUPerfHintAnalysis.cpp - analysis of functions memory traffic --===//
//
...
#include "RVGPU.h"
#include "RVGPUPerfHintAnalysis.h"
...
#define DEBUG_TYPE "rvgpu-perf-hint"

static cl::opt<unsigned>
    MemBoundThresh("rvgpu-membound-threshold", cl::init(50), cl::Hidden,
                   cl::desc("Function mem bound threshold in %"));

static cl::opt<unsigned>
    LimitWaveThresh("rvgpu-limit-wave-threshold", cl::init(50), cl::Hidden,
                    cl::desc("Kernel limit wave threshold in %"));

static cl::opt<unsigned>
    IAWeight("rvgpu-indirect-access-weight", cl::init(1000), cl::Hidden,
             cl::desc("Indirect access memory instruction weight"));

static cl::opt<unsigned>
    LSWeight("rvgpu-large-stride-weight", cl::init(1000), cl::Hidden,
             cl::desc("Large stride memory access weight"));

static cl::opt<unsigned>
    LargeStrideThresh("rvgpu-large-stride-threshold", cl::init(64), cl::Hidden,
...
char llvm::RVGPUPerfHintAnalysis::ID = 0;
char &llvm::RVGPUPerfHintAnalysisID = RVGPUPerfHintAnalysis::ID;

INITIALIZE_PASS(RVGPUPerfHintAnalysis, DEBUG_TYPE,
                "Analysis if a function is memory bound", true, true)

namespace {

struct RVGPUPerfHint {
  friend RVGPUPerfHintAnalysis;

public:
  RVGPUPerfHint(RVGPUPerfHintAnalysis::FuncInfoMap &FIM_,
...
private:
  ...
  RVGPUPerfHintAnalysis::FuncInfoMap &FIM;

  const DataLayout *DL;

  const TargetLowering *TLI;

  RVGPUPerfHintAnalysis::FuncInfo *visit(const Function &F);
  static bool isMemBound(const RVGPUPerfHintAnalysis::FuncInfo &F);
  static bool needLimitWave(const RVGPUPerfHintAnalysis::FuncInfo &F);
...
bool RVGPUPerfHint::isIndirectAccess(const Instruction *Inst) const {
...
bool RVGPUPerfHint::isGlobalLoadUsedInBB(const Instruction &I) const {
...
RVGPUPerfHintAnalysis::FuncInfo *RVGPUPerfHint::visit(const Function &F) {
  RVGPUPerfHintAnalysis::FuncInfo &FI = FIM[&F];

  LLVM_DEBUG(dbgs() << "[RVGPUPerfHint] process " << F.getName() << '\n');
...
bool RVGPUPerfHint::runOnFunction(Function &F) {
  const Module &M = *F.getParent();
  DL = &M.getDataLayout();

  if (F.hasFnAttribute("rvgpu-wave-limiter") &&
      F.hasFnAttribute("rvgpu-memory-bound"))
    return false;

  const RVGPUPerfHintAnalysis::FuncInfo *Info = visit(F);
  ...
  if (isMemBound(*Info)) {
    ...
    NumMemBound++;
    F.addFnAttr("rvgpu-memory-bound", "true");
    Changed = true;
  }

  if (RVGPU::isEntryFunctionCC(F.getCallingConv()) && needLimitWave(*Info)) {
    LLVM_DEBUG(dbgs() << F.getName() << " needs limit wave\n");
    NumLimitWave++;
    F.addFnAttr("rvgpu-wave-limiter", "true");
    Changed = true;
  }
...
bool RVGPUPerfHint::isMemBound(const RVGPUPerfHintAnalysis::FuncInfo &FI) {
...
bool RVGPUPerfHint::needLimitWave(const RVGPUPerfHintAnalysis::FuncInfo &FI) {
  return ((FI.MemInstCost + FI.IAMInstCost * IAWeight +
           FI.LSMInstCost * LSWeight) * 100 / FI.InstCost) > LimitWaveThresh;
}

bool RVGPUPerfHint::isGlobalAddr(const Value *V) const {
  if (auto PT = dyn_cast<PointerType>(V->getType())) {
    unsigned As = PT->getAddressSpace();
    // Flat likely points to global too.
    return As == RVGPUAS::GLOBAL_ADDRESS || As == RVGPUAS::FLAT_ADDRESS;
  }
  return false;
}

bool RVGPUPerfHint::isLocalAddr(const Value *V) const {
  if (auto PT = dyn_cast<PointerType>(V->getType()))
    return PT->getAddressSpace() == RVGPUAS::LOCAL_ADDRESS;
  return false;
}

bool RVGPUPerfHint::isLargeStride(const Instruction *Inst) {
...
RVGPUPerfHint::MemAccessInfo
RVGPUPerfHint::makeMemAccessInfo(Instruction *Inst) const {
  MemAccessInfo MAI;
...
bool RVGPUPerfHint::MemAccessInfo::isLargeStride(
...
bool RVGPUPerfHintAnalysis::runOnSCC(CallGraphSCC &SCC) {
  ...
  for (CallGraphNode *I : SCC) {
    ...
    RVGPUPerfHint Analyzer(FIM, ST->getTargetLowering());
...
bool RVGPUPerfHintAnalysis::isMemoryBound(const Function *F) const {
  auto FI = FIM.find(F);
  if (FI == FIM.end())
    return false;

  return RVGPUPerfHint::isMemBound(FI->second);
}

bool RVGPUPerfHintAnalysis::needsWaveLimiter(const Function *F) const {
  auto FI = FIM.find(F);
  if (FI == FIM.end())
    return false;

  return RVGPUPerfHint::needLimitWave(FI->second);
}
