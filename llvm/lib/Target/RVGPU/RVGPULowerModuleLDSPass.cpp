...
// functions it calls through the intrinsic rvgpu_lds_kernel_id. The integer
...
// The RVGPUPromoteAlloca pass skips kernels that use dynamic LDS.
...
#define DEBUG_TYPE "rvgpu-lower-module-lds"
...
cl::opt<LoweringKind> LoweringKindLoc(
    "rvgpu-lower-module-lds-strategy",
...
bool isKernelLDS(const Function *F) {
  // Some weirdness here. RVGPU::isKernelCC does not call into
  // RVGPU::isKernel with the calling conv, it instead calls into
  // isModuleEntryFunction which returns true for more calling conventions
  // than RVGPU::isKernel does. There's a FIXME on RVGPU::isKernel.
  // There's also a test that checks that the LDS lowering does not hit on
  // a graphics shader, denoted rvgpu_ps, so stay with the limited case.
  // Putting LDS in the name of the function to draw attention to this.
  return RVGPU::isKernel(F->getCallingConv());
}
...
class RVGPULowerModuleLDS {
  ...
  static void markUsedByKernel(Function *Func, GlobalVariable *SGV) {
    // The llvm.rvgpu.module.lds instance is implicitly used by all kernels
  ...
  static bool eliminateConstantExprUsesOfLDSFromAllInstructions(Module &M) {
    ...
    for (auto &GV : M.globals())
      if (RVGPU::isLDSVariableToLower(GV))
...
public:
  RVGPULowerModuleLDS(const RVGPUTargetMachine &TM_) : TM(TM_) {}
  ...
  static GlobalVariable *buildLookupTable(
    ...
    return new GlobalVariable(
        ...
        "llvm.rvgpu.lds.offset.table", nullptr, GlobalValue::NotThreadLocal,
        RVGPUAS::CONSTANT_ADDRESS);
  }
  ...
  static void recordLDSAbsoluteAddress(Module *M, GlobalVariable *GV,
    ...
    LLVMContext &Ctx = M->getContext();
    auto *IntTy =
        M->getDataLayout().getIntPtrType(Ctx, RVGPUAS::LOCAL_ADDRESS);
  ...
  Value *getTableLookupKernelIndex(Module &M, Function *F) {
    // Accesses from a function use the rvgpu_lds_kernel_id intrinsic which
    ...
    if (Inserted) {
      Function *Decl =
          Intrinsic::getDeclaration(&M, Intrinsic::rvgpu_lds_kernel_id, {});
  ...
  static std::vector<Function *> assignLDSKernelIDToEachKernel(
    ...
    if (!KernelsThatAllocateTableLDS.empty() ||
      ...
      for (size_t i = 0; i < OrderedKernels.size(); i++) {
        ...
        OrderedKernels[i]->setMetadata("llvm.rvgpu.lds.kernel.id",
  ...
  static void partitionVariablesIntoIndirectStrategies(
    ...
    for (auto &K : LDSToKernelsThatNeedToAccessItIndirectly) {
      ...
      GlobalVariable *GV = K.first;
      assert(RVGPU::isLDSVariableToLower(*GV));
      assert(K.second.size() != 0);

      if (RVGPU::isDynamicLDS(*GV)) {
  ...
  static GlobalVariable *lowerModuleScopeStructVariables(
    ...
    LDSVariableReplacement ModuleScopeReplacement =
        createLDSVariableReplacement(M, "llvm.rvgpu.module.lds",
  ...
  lowerKernelScopeStructVariables(
    ...
    for (Function &Func : M.functions()) {
      ...
      std::string VarName =
          (Twine("llvm.rvgpu.kernel.") + Func.getName() + ".lds").str();
      ...
      // codegen in test/CodeGen/RVGPU/noclobber-barrier.ll
  ...
  buildRepresentativeDynamicLDSInstance(Module &M, LDSUsesInfoTy &LDSUsesInfo,
    ...
    auto UpdateMaxAlignment = [&MaxDynamicAlignment, &DL](GlobalVariable *GV) {
      if (RVGPU::isDynamicLDS(*GV)) {
        MaxDynamicAlignment =
            std::max(MaxDynamicAlignment, RVGPU::getAlign(DL, GV));
      }
    };
    ...
    GlobalVariable *N = new GlobalVariable(
        ...
        Twine("llvm.rvgpu." + func->getName() + ".dynlds"), nullptr, GlobalValue::NotThreadLocal, RVGPUAS::LOCAL_ADDRESS,
        false);
    N->setAlignment(MaxDynamicAlignment);

    assert(RVGPU::isDynamicLDS(*N));
    return N;
  }
  ...
  DenseMap<Function *, GlobalVariable *> lowerDynamicLDSVariables(
    ...
    if (!KernelsThatIndirectlyAllocateDynamicLDS.empty()) {
      ...
      GlobalVariable *table = new GlobalVariable(
          ...
          "llvm.rvgpu.dynlds.offset.table", nullptr,
          GlobalValue::NotThreadLocal, RVGPUAS::CONSTANT_ADDRESS);
  ...
  bool runOnModule(Module &M) {
    ...
    // addresses.
    {
      ...
      for (Function &Func : M.functions()) {
        ...
        // zero. They are allocated by RVGPUMachineFunction as one block.
        // Layout:
        //{
        //  module.lds
        //  alignment padding
        ...
        if (AllocateKernelScopeStruct) {
          ...
          Offset = alignTo(Offset, RVGPU::getAlign(DL, KernelStruct));
        ...
        if (AllocateDynamicVariable) {
          ...
          Offset = alignTo(Offset, RVGPU::getAlign(DL, DynamicVariable));
        ...
        if (Offset != 0) {
          ...
          Func.addFnAttr("rvgpu-lds-size", Buffer);
        }
      }
    }
...
private:
  ...
  static bool superAlignLDSGlobals(Module &M) {
    ...
    for (auto &GV : M.globals()) {
      if (GV.getType()->getPointerAddressSpace() != RVGPUAS::LOCAL_ADDRESS) {
      ...
      Align Alignment = RVGPU::getAlign(DL, &GV);
      ...
      if (Alignment != RVGPU::getAlign(DL, &GV)) {
        Changed = true;
  ...
  static LDSVariableReplacement createLDSVariableReplacement(
    ...
    Align StructAlign = RVGPU::getAlign(DL, LocalVars[0]);

    GlobalVariable *SGV = new GlobalVariable(
        M, LDSTy, false, GlobalValue::InternalLinkage, PoisonValue::get(LDSTy),
        VarName, nullptr, GlobalValue::NotThreadLocal, RVGPUAS::LOCAL_ADDRESS,
        false);
  ...
  static void replaceLDSVariablesWithStruct(
    ...
    // field of the instance that will be allocated by RVGPUMachineFunction
    for (size_t I = 0; I < NumberVars; I++) {
...
class RVGPULowerModuleLDSLegacy : public ModulePass {
public:
  const RVGPUTargetMachine *TM;
  static char ID;

  RVGPULowerModuleLDSLegacy(const RVGPUTargetMachine *TM_ = nullptr)
      : ModulePass(ID), TM(TM_) {
    initializeRVGPULowerModuleLDSLegacyPass(*PassRegistry::getPassRegistry());
  }
  ...
  bool runOnModule(Module &M) override {
    if (!TM) {
      ...
      TM = &TPC.getTM<RVGPUTargetMachine>();
    }

    return RVGPULowerModuleLDS(*TM).runOnModule(M);
  }
};

} // namespace
char RVGPULowerModuleLDSLegacy::ID = 0;

char &llvm::RVGPULowerModuleLDSLegacyPassID = RVGPULowerModuleLDSLegacy::ID;

INITIALIZE_PASS_BEGIN(RVGPULowerModuleLDSLegacy, DEBUG_TYPE,
                      "Lower uses of LDS variables from non-kernel functions",
                      false, false)
INITIALIZE_PASS_DEPENDENCY(TargetPassConfig)
INITIALIZE_PASS_END(RVGPULowerModuleLDSLegacy, DEBUG_TYPE,
                    "Lower uses of LDS variables from non-kernel functions",
                    false, false)

ModulePass *
llvm::createRVGPULowerModuleLDSLegacyPass(const RVGPUTargetMachine *TM) {
  return new RVGPULowerModuleLDSLegacy(TM);
}

PreservedAnalyses RVGPULowerModuleLDSPass::run(Module &M,
                                                ModuleAnalysisManager &) {
  return RVGPULowerModuleLDS(TM).runOnModule(M) ? PreservedAnalyses::none()
