...
RVModeRegisterDefaults::RVModeRegisterDefaults(const Function &F,
                                               const RVGPUSubtarget &ST) {
  *this = getDefaultForCallingConv(F.getCallingConv());

  StringRef IEEEAttr = F.getFnAttribute("rvgpu-ieee").getValueAsString();
  if (!IEEEAttr.empty())
    IEEE = IEEEAttr == "true";

  StringRef DX10ClampAttr =
      F.getFnAttribute("rvgpu-dx10-clamp").getValueAsString();
...
encodeFltRoundsTableSame(RVGPUFltRounds FltRoundsMode, uint32_t HWVal) {
  return encodeFltRoundsTable(FltRoundsMode, HWVal, HWVal);
}

// Convert mode register encoded rounding mode to RVGPUFltRounds
static constexpr RVGPUFltRounds
decodeIndexFltRoundConversionTable(uint32_t HWMode) {
  ...
  return static_cast<RVGPUFltRounds>(TableRead);
}
...
const uint64_t RVGPU::FltRoundConversionTable =
...
static_assert(decodeIndexFltRoundConversionTable(getModeRegisterRoundMode(
                  HWTowardZero, HWTowardZero)) == RVGPUFltRounds::TowardZero);
static_assert(decodeIndexFltRoundConversionTable(getModeRegisterRoundMode(
                  HWNearestTiesToEven, HWNearestTiesToEven)) ==
              RVGPUFltRounds::NearestTiesToEven);
static_assert(decodeIndexFltRoundConversionTable(getModeRegisterRoundMode(
                  HWTowardPositive, HWTowardPositive)) ==
              RVGPUFltRounds::TowardPositive);
static_assert(decodeIndexFltRoundConversionTable(getModeRegisterRoundMode(
                  HWTowardNegative, HWTowardNegative)) ==
              RVGPUFltRounds::TowardNegative);
