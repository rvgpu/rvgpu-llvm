...
void SIWholeQuadMode::markDefs(const MachineInstr &UseMI, LiveRange &LR,
  ...
  // Note: this code assumes that lane masks on RVGPU completely
  // cover registers.
...
void SIWholeQuadMode::markOperand(const MachineInstr &MI,
  ...
  if (Reg.isVirtual()) {
    ...
    for (MCRegUnit Unit : TRI->regunits(Reg.asMCReg())) {
      ...
      markDefs(MI, LR, Unit, RVGPU::NoSubRegister, Flag, Worklist);
    }
  }
}
...
char SIWholeQuadMode::scanInstructions(MachineFunction &MF,
                                       std::vector<WorkItem> &Worklist) {
  char GlobalFlags = 0;
  bool WQMOutputs = MF.getFunction().hasFnAttribute("rvgpu-ps-wqm-outputs");
  SmallVector<MachineInstr *, 4> SetInactiveInstrs;
  SmallVector<MachineInstr *, 4> SoftWQMInstrs;
  bool HasImplicitDerivatives =
      MF.getFunction().getCallingConv() == CallingConv::RVGPU_PS;
  ...
  for (MachineBasicBlock *MBB : RPOT) {
    ...
    for (MachineInstr &MI : *MBB) {
      ...
      if (TII->isWQM(Opcode)) {
        ...
        continue;
      } else if (Opcode == RVGPU::STRICT_WQM ||
        ...
        continue;
      } else if (Opcode == RVGPU::LDS_PARAM_LOAD ||
                 Opcode == RVGPU::LDS_DIRECT_LOAD) {
        ...
        GlobalFlags |= StateStrictWQM;
        continue;
      } else if (Opcode == RVGPU::V_SET_INACTIVE_B32 ||
                 Opcode == RVGPU::V_SET_INACTIVE_B64) {
        ...
        if (Opcode == RVGPU::SI_PS_LIVE || Opcode == RVGPU::SI_LIVE_MASK) {
          LiveMaskQueries.push_back(&MI);
        } else if (Opcode == RVGPU::SI_KILL_I1_TERMINATOR ||
                   Opcode == RVGPU::SI_KILL_F32_COND_IMM_TERMINATOR ||
                   Opcode == RVGPU::SI_DEMOTE_I1) {
  ...
  // semantics of @llvm.rvgpu.set.inactive.
  // Similarly for SOFT_WQM instructions, implementing @llvm.rvgpu.softwqm.
...
SIWholeQuadMode::saveSCC(MachineBasicBlock &MBB,
                         MachineBasicBlock::iterator Before) {
  Register SaveReg = MRI->createVirtualRegister(&RVGPU::SReg_32_XM0RegClass);

  MachineInstr *Save =
      BuildMI(MBB, Before, DebugLoc(), TII->get(RVGPU::COPY), SaveReg)
          .addReg(RVGPU::SCC);
  MachineInstr *Restore =
      BuildMI(MBB, Before, DebugLoc(), TII->get(RVGPU::COPY), RVGPU::SCC)
          .addReg(SaveReg);
...
MachineBasicBlock *SIWholeQuadMode::splitBlock(MachineBasicBlock *BB,
  ...
  case RVGPU::S_AND_B32:
    NewOpcode = RVGPU::S_AND_B32_term;
    break;
  case RVGPU::S_AND_B64:
    NewOpcode = RVGPU::S_AND_B64_term;
    break;
  case RVGPU::S_MOV_B32:
    NewOpcode = RVGPU::S_MOV_B32_term;
    break;
  case RVGPU::S_MOV_B64:
    NewOpcode = RVGPU::S_MOV_B64_term;
    break;
  default:
    break;
  }
  ...
  if (SplitBB != BB) {
    ...
    // Link blocks
    MachineInstr *MI =
        BuildMI(*BB, BB->end(), DebugLoc(), TII->get(RVGPU::S_BRANCH))
            .addMBB(SplitBB);
...
MachineInstr *SIWholeQuadMode::lowerKillF32(MachineBasicBlock &MBB,
  ...
  case ISD::SETUEQ:
    Opcode = RVGPU::V_CMP_LG_F32_e64;
    break;
  case ISD::SETUGT:
    Opcode = RVGPU::V_CMP_GE_F32_e64;
    break;
  case ISD::SETUGE:
    Opcode = RVGPU::V_CMP_GT_F32_e64;
    break;
  case ISD::SETULT:
    Opcode = RVGPU::V_CMP_LE_F32_e64;
    break;
  case ISD::SETULE:
    Opcode = RVGPU::V_CMP_LT_F32_e64;
    break;
  case ISD::SETUNE:
    Opcode = RVGPU::V_CMP_EQ_F32_e64;
    break;
  case ISD::SETO:
    Opcode = RVGPU::V_CMP_O_F32_e64;
    break;
  case ISD::SETUO:
    Opcode = RVGPU::V_CMP_U_F32_e64;
    break;
  case ISD::SETOEQ:
  case ISD::SETEQ:
    Opcode = RVGPU::V_CMP_NEQ_F32_e64;
    break;
  case ISD::SETOGT:
  case ISD::SETGT:
    Opcode = RVGPU::V_CMP_NLT_F32_e64;
    break;
  case ISD::SETOGE:
  case ISD::SETGE:
    Opcode = RVGPU::V_CMP_NLE_F32_e64;
    break;
  case ISD::SETOLT:
  case ISD::SETLT:
    Opcode = RVGPU::V_CMP_NGT_F32_e64;
    break;
  case ISD::SETOLE:
  case ISD::SETLE:
    Opcode = RVGPU::V_CMP_NGE_F32_e64;
    break;
  case ISD::SETONE:
  case ISD::SETNE:
    Opcode = RVGPU::V_CMP_NLG_F32_e64;
    break;
  default:
  ...
  Register VCC = ST->isWave32() ? RVGPU::VCC_LO : RVGPU::VCC;

  if (TRI->isVGPR(*MRI, Op0.getReg())) {
    Opcode = RVGPU::getVOPe32(Opcode);
  ...
  MachineInstr *EarlyTermMI =
      BuildMI(MBB, MI, DL, TII->get(RVGPU::SI_EARLY_TERMINATE_SCC0));

  MachineInstr *ExecMaskMI =
      BuildMI(MBB, MI, DL, TII->get(AndN2Opc), Exec).addReg(Exec).addReg(VCC);

  assert(MBB.succ_size() == 1);
  MachineInstr *NewTerm = BuildMI(MBB, MI, DL, TII->get(RVGPU::S_BRANCH))
...
MachineInstr *SIWholeQuadMode::lowerKillI1(MachineBasicBlock &MBB,
  ...
  const bool IsDemote = IsWQM && (MI.getOpcode() == RVGPU::SI_DEMOTE_I1);
  ...
  if (Op.isImm()) {
    if (Op.getImm() == KillVal) {
      ...
      if (MI.getOpcode() == RVGPU::SI_DEMOTE_I1) {
        LIS->RemoveMachineInstrFromMaps(MI);
      } else {
        assert(MBB.succ_size() == 1);
        NewTerm = BuildMI(MBB, MI, DL, TII->get(RVGPU::S_BRANCH))
                      .addMBB(*MBB.succ_begin());
...
void SIWholeQuadMode::lowerPseudoStrictMode(MachineBasicBlock &MBB,
                                            MachineInstr *Entry,
                                            MachineInstr *Exit) {
  assert(Entry->getOpcode() == RVGPU::ENTER_STRICT_WQM);
  assert(Exit->getOpcode() == RVGPU::EXIT_STRICT_WQM);

  Register SaveOrig = Entry->getOperand(0).getReg();

  MachineInstr *NewEntry =
    BuildMI(MBB, Entry, DebugLoc(), TII->get(RVGPU::ENTER_PSEUDO_WM));
  MachineInstr *NewExit =
    BuildMI(MBB, Exit, DebugLoc(), TII->get(RVGPU::EXIT_PSEUDO_WM));
...
MachineBasicBlock::iterator SIWholeQuadMode::prepareInsertion(
  ...
  LiveRange &LR =
      LIS->getRegUnit(*TRI->regunits(MCRegister::from(RVGPU::SCC)).begin());
  auto MBBE = MBB.end();
  ...
  while (MBBI != Last) {
    ...
    for (const MachineOperand &MO : MBBI->all_defs()) {
      IsExecDef |=
          MO.getReg() == RVGPU::EXEC_LO || MO.getReg() == RVGPU::EXEC;
    }
    if (!IsExecDef)
      break;
    MBBI++;
...
void SIWholeQuadMode::toWQM(MachineBasicBlock &MBB,
  ...
  if (SavedWQM) {
    MI = BuildMI(MBB, Before, DebugLoc(), TII->get(RVGPU::COPY), Exec)
             .addReg(SavedWQM);
  } else {
...
void SIWholeQuadMode::toStrictMode(MachineBasicBlock &MBB,
  ...
  if (StrictStateNeeded == StateStrictWWM) {
    MI = BuildMI(MBB, Before, DebugLoc(), TII->get(RVGPU::ENTER_STRICT_WWM),
                 SaveOrig)
             .addImm(-1);
  } else {
    MI = BuildMI(MBB, Before, DebugLoc(), TII->get(RVGPU::ENTER_STRICT_WQM),
                 SaveOrig)
             .addImm(-1);
  }
  LIS->InsertMachineInstrInMaps(*MI);
...
void SIWholeQuadMode::fromStrictMode(MachineBasicBlock &MBB,
  ...
  if (CurrentStrictState == StateStrictWWM) {
    MI = BuildMI(MBB, Before, DebugLoc(), TII->get(RVGPU::EXIT_STRICT_WWM),
                 Exec)
             .addReg(SavedOrig);
  } else {
    MI = BuildMI(MBB, Before, DebugLoc(), TII->get(RVGPU::EXIT_STRICT_WQM),
                 Exec)
             .addReg(SavedOrig);
  }
  LIS->InsertMachineInstrInMaps(*MI);
...
void SIWholeQuadMode::processBlock(MachineBasicBlock &MBB, bool IsEntry) {
  ...
  if (IsEntry) {
    ...
    if (II != IE && II->getOpcode() == RVGPU::COPY &&
...
void SIWholeQuadMode::lowerLiveMaskQueries() {
  for (MachineInstr *MI : LiveMaskQueries) {
    ...
    MachineInstr *Copy =
        BuildMI(*MI->getParent(), MI, DL, TII->get(RVGPU::COPY), Dest)
            .addReg(LiveMaskReg);
  ...
  for (MachineInstr *MI : LowerToMovInstrs) {
    ...
    if (TRI->isVGPRClass(regClass)) {
      ...
      assert(any_of(MI->implicit_operands(), [](const MachineOperand &MO) {
        return MO.isUse() && MO.getReg() == RVGPU::EXEC;
      }));
    } else {
      ...
      int Index = MI->findRegisterUseOperandIdx(RVGPU::EXEC);
      while (Index >= 0) {
        MI->removeOperand(Index);
        Index = MI->findRegisterUseOperandIdx(RVGPU::EXEC);
      }
      MI->setDesc(TII->get(RVGPU::COPY));
      LLVM_DEBUG(dbgs() << "  -> " << *MI);
    }
  }
  for (MachineInstr *MI : LowerToCopyInstrs) {
    if (MI->getOpcode() == RVGPU::V_SET_INACTIVE_B32 ||
        MI->getOpcode() == RVGPU::V_SET_INACTIVE_B64) {
...
bool SIWholeQuadMode::runOnMachineFunction(MachineFunction &MF) {
  ...
  if (ST->isWave32()) {
    AndOpc = RVGPU::S_AND_B32;
    AndTermOpc = RVGPU::S_AND_B32_term;
    AndN2Opc = RVGPU::S_ANDN2_B32;
    XorOpc = RVGPU::S_XOR_B32;
    AndSaveExecOpc = RVGPU::S_AND_SAVEEXEC_B32;
    AndSaveExecTermOpc = RVGPU::S_AND_SAVEEXEC_B32_term;
    WQMOpc = RVGPU::S_WQM_B32;
    Exec = RVGPU::EXEC_LO;
  } else {
    AndOpc = RVGPU::S_AND_B64;
    AndTermOpc = RVGPU::S_AND_B64_term;
    AndN2Opc = RVGPU::S_ANDN2_B64;
    XorOpc = RVGPU::S_XOR_B64;
    AndSaveExecOpc = RVGPU::S_AND_SAVEEXEC_B64;
    AndSaveExecTermOpc = RVGPU::S_AND_SAVEEXEC_B64_term;
    WQMOpc = RVGPU::S_WQM_B64;
    Exec = RVGPU::EXEC;
  }
  ...
  if (NeedsLiveMask || (GlobalFlags & StateWQM)) {
    ...
    MachineInstr *MI =
        BuildMI(Entry, EntryMI, DebugLoc(), TII->get(RVGPU::COPY), LiveMaskReg)
            .addReg(Exec);
    LIS->InsertMachineInstrInMaps(*MI);
  }
  ...
  LIS->removeAllRegUnitsForPhysReg(RVGPU::SCC);

  // If we performed any kills then recompute EXEC
  if (!KillInstrs.empty())
    LIS->removeAllRegUnitsForPhysReg(RVGPU::EXEC);
