...
static cl::opt<bool> DisableUnclusterHighRP(
    "rvgpu-disable-unclustered-high-rp-reschedule", cl::Hidden,
...
static cl::opt<bool>
    RelaxedOcc("rvgpu-schedule-relaxed-occupancy", cl::Hidden,
               cl::desc("Relax occupancy targets for kernels which are memory "
                        "bound (rvgpu-membound-threshold), or "
                        "Wave Limited (rvgpu-limit-wave-threshold)."),
               cl::init(false));
...
void GCNSchedStrategy::initialize(ScheduleDAGMI *DAG) {
  ...
  SGPRExcessLimit =
      Context->RegClassInfo->getNumAllocatableRegs(&RVGPU::SGPR_32RegClass);
  VGPRExcessLimit =
      Context->RegClassInfo->getNumAllocatableRegs(&RVGPU::VGPR_32RegClass);
  ...
  if (!KnownExcessRP) {
    ...
    unsigned Granule = RVGPU::IsaInfo::getVGPRAllocGranule(&ST);
    unsigned Addressable = RVGPU::IsaInfo::getAddressableNumVGPRs(&ST);
...
void GCNSchedStrategy::initCandidate(SchedCandidate &Cand, SUnit *SU,
  ...
  unsigned NewSGPRPressure = Pressure[RVGPU::RegisterPressureSets::SReg_32];
  unsigned NewVGPRPressure = Pressure[RVGPU::RegisterPressureSets::VGPR_32];
  ...
  if (ShouldTrackVGPRs && NewVGPRPressure >= VGPRExcessLimit) {
    ...
    Cand.RPDelta.Excess = PressureChange(RVGPU::RegisterPressureSets::VGPR_32);
    Cand.RPDelta.Excess.setUnitInc(NewVGPRPressure - VGPRExcessLimit);
  }

  if (ShouldTrackSGPRs && NewSGPRPressure >= SGPRExcessLimit) {
    HasHighPressure = true;
    Cand.RPDelta.Excess = PressureChange(RVGPU::RegisterPressureSets::SReg_32);
  ...
  if (SGPRDelta >= 0 || VGPRDelta >= 0) {
    ...
    if (SGPRDelta > VGPRDelta) {
      Cand.RPDelta.CriticalMax =
        PressureChange(RVGPU::RegisterPressureSets::SReg_32);
      Cand.RPDelta.CriticalMax.setUnitInc(SGPRDelta);
    } else {
      Cand.RPDelta.CriticalMax =
        PressureChange(RVGPU::RegisterPressureSets::VGPR_32);
...
void GCNSchedStrategy::pickNodeFromQueue(SchedBoundary &Zone,
                                         ...
                                         const RegPressureTracker &RPTracker,
                                         SchedCandidate &Cand) {
  const RVRegisterInfo *SRI = static_cast<const RVRegisterInfo*>(TRI);
  ArrayRef<unsigned> Pressure = RPTracker.getRegSetPressureAtPos();
  unsigned SGPRPressure = 0;
  unsigned VGPRPressure = 0;
  if (DAG->isTrackingPressure()) {
    SGPRPressure = Pressure[RVGPU::RegisterPressureSets::SReg_32];
    VGPRPressure = Pressure[RVGPU::RegisterPressureSets::VGPR_32];
  }
...
GCNScheduleDAGMILive::GCNScheduleDAGMILive(
    ...
    : ScheduleDAGMILive(C, std::move(S)), ST(MF.getSubtarget<RVSubtarget>()),
...
bool GCNSchedStage::initGCNRegion() {
  ...
  if (StageID == GCNSchedStageID::OccInitialSchedule ||
    ...
    for (auto &I : DAG) {
      Unsched.push_back(&I);
      if (I.getOpcode() == RVGPU::SCHED_GROUP_BARRIER ||
          I.getOpcode() == RVGPU::IGLP_OPT)
...
void PreRARematStage::collectRematerializableInstructions() {
  const RVRegisterInfo *SRI = static_cast<const RVRegisterInfo *>(DAG.TRI);
...
bool PreRARematStage::sinkTriviallyRematInsts(const RVSubtarget &ST,
  ...
  for (auto I : ImpactedRegions) {
    ...
    for (const auto &It : RematerializableInsts[I]) {
      ...
      TotalSinkableRegs +=
          RVRegisterInfo::getNumCoveredRegs(NewLiveIns[I][DefReg]);
    }
...
static bool hasIGLPInstrs(ScheduleDAGInstrs *DAG) {
  return std::any_of(
      DAG->begin(), DAG->end(), [](MachineBasicBlock::iterator MI) {
        ...
        return Opc == RVGPU::SCHED_GROUP_BARRIER || Opc == RVGPU::IGLP_OPT;
      });
}
