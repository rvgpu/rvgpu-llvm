//=== lib/CodeGen/GlobalISel/RVGPUCombinerHelper.cpp ---------------------===//
//
...
static bool isConstantCostlierToNegate(MachineInstr &MI, Register Reg,
  ...
  if (mi_match(Reg, MRI, m_GFCstOrSplat(FPValReg))) {
    ...
    const RVSubtarget &ST = MI.getMF()->getSubtarget<RVSubtarget>();
...
static unsigned inverseMinMax(unsigned Opc) {
  switch (Opc) {
  case RVGPU::G_FMAXNUM:
    return RVGPU::G_FMINNUM;
  case RVGPU::G_FMINNUM:
    return RVGPU::G_FMAXNUM;
  case RVGPU::G_FMAXNUM_IEEE:
    return RVGPU::G_FMINNUM_IEEE;
  case RVGPU::G_FMINNUM_IEEE:
    return RVGPU::G_FMAXNUM_IEEE;
  case RVGPU::G_FMAXIMUM:
    return RVGPU::G_FMINIMUM;
  case RVGPU::G_FMINIMUM:
    return RVGPU::G_FMAXIMUM;
  case RVGPU::G_RVGPU_FMAX_LEGACY:
    return RVGPU::G_RVGPU_FMIN_LEGACY;
  case RVGPU::G_RVGPU_FMIN_LEGACY:
    return RVGPU::G_RVGPU_FMAX_LEGACY;
  default:
    llvm_unreachable("invalid min/max opcode");
  }
}

bool RVGPUCombinerHelper::matchFoldableFneg(MachineInstr &MI,
...
void RVGPUCombinerHelper::applyFoldableFneg(MachineInstr &MI,
                                             MachineInstr *&MatchInfo) {
  // Transform:
  // %A = inst %Op1, ...
  // %B = fneg %A
...
bool RVGPUCombinerHelper::matchExpandPromotedF16FMed3(MachineInstr &MI,
                                                       Register Src0,
                                                       Register Src1,
                                                       Register Src2) {
...
void RVGPUCombinerHelper::applyExpandPromotedF16FMed3(MachineInstr &MI,
                                                       Register Src0,
                                                       Register Src1,
                                                       Register Src2) {
