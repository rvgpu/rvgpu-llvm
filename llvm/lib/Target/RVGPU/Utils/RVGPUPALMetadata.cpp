//===-- RVGPUPALMetadata.cpp - Accumulate and print RVGPU PAL metadata  -===//
//
...
/// \file
///
/// This class has methods called by RVGPUAsmPrinter to accumulate and print
/// the PAL metadata.
//
//===----------------------------------------------------------------------===//
//

#include "RVGPUPALMetadata.h"
...
void RVGPUPALMetadata::readFromIR(Module &M) {
  auto NamedMD = M.getNamedMetadata("rvgpu.pal.metadata.msgpack");
  if (NamedMD && NamedMD->getNumOperands()) {
    ...
    BlobType = ELF::NT_RVGPU_METADATA;
  ...
  BlobType = ELF::NT_RV_PAL_METADATA;
  NamedMD = M.getNamedMetadata("rvgpu.pal.metadata");
  if (!NamedMD || !NamedMD->getNumOperands()) {
...
// Metadata.
bool RVGPUPALMetadata::setFromBlob(unsigned Type, StringRef Blob) {
  BlobType = Type;
  if (Type == ELF::NT_RV_PAL_METADATA)
    return setFromLegacyBlob(Blob);
  return setFromMsgPackBlob(Blob);
}

// Set PAL metadata from legacy (array of key=value pairs) blob.
bool RVGPUPALMetadata::setFromLegacyBlob(StringRef Blob) {
...
bool RVGPUPALMetadata::setFromMsgPackBlob(StringRef Blob) {
...
static unsigned getRsrc1Reg(CallingConv::ID CC) {
  switch (CC) {
  default:
    return RVPALMD::R_2E12_COMPUTE_PGM_RSRC1;
  case CallingConv::RVGPU_LS:
    return RVPALMD::R_2D4A_SPI_SHADER_PGM_RSRC1_LS;
  case CallingConv::RVGPU_HS:
    return RVPALMD::R_2D0A_SPI_SHADER_PGM_RSRC1_HS;
  case CallingConv::RVGPU_ES:
    return RVPALMD::R_2CCA_SPI_SHADER_PGM_RSRC1_ES;
  case CallingConv::RVGPU_GS:
    return RVPALMD::R_2C8A_SPI_SHADER_PGM_RSRC1_GS;
  case CallingConv::RVGPU_VS:
    return RVPALMD::R_2C4A_SPI_SHADER_PGM_RSRC1_VS;
  case CallingConv::RVGPU_PS:
    return RVPALMD::R_2C0A_SPI_SHADER_PGM_RSRC1_PS;
  }
}
...
static unsigned getScratchSizeKey(CallingConv::ID CC) {
  switch (CC) {
  case CallingConv::RVGPU_PS:
    return RVPALMD::Key::PS_SCRATCH_SIZE;
  case CallingConv::RVGPU_VS:
    return RVPALMD::Key::VS_SCRATCH_SIZE;
  case CallingConv::RVGPU_GS:
    return RVPALMD::Key::GS_SCRATCH_SIZE;
  case CallingConv::RVGPU_ES:
    return RVPALMD::Key::ES_SCRATCH_SIZE;
  case CallingConv::RVGPU_HS:
    return RVPALMD::Key::HS_SCRATCH_SIZE;
  case CallingConv::RVGPU_LS:
    return RVPALMD::Key::LS_SCRATCH_SIZE;
  default:
    return RVPALMD::Key::CS_SCRATCH_SIZE;
  }
}
...
void RVGPUPALMetadata::setRsrc1(CallingConv::ID CC, unsigned Val) {
  setRegister(getRsrc1Reg(CC), Val);
}

// Set the rsrc2 register in the metadata for a particular shader stage.
// In fact this ORs the value into any previous setting of the register.
void RVGPUPALMetadata::setRsrc2(CallingConv::ID CC, unsigned Val) {
  setRegister(getRsrc1Reg(CC) + 1, Val);
}

// Set the SPI_PS_INPUT_ENA register in the metadata.
// In fact this ORs the value into any previous setting of the register.
void RVGPUPALMetadata::setSpiPsInputEna(unsigned Val) {
  setRegister(RVPALMD::R_A1B3_SPI_PS_INPUT_ENA, Val);
}

// Set the SPI_PS_INPUT_ADDR register in the metadata.
// In fact this ORs the value into any previous setting of the register.
void RVGPUPALMetadata::setSpiPsInputAddr(unsigned Val) {
  setRegister(RVPALMD::R_A1B4_SPI_PS_INPUT_ADDR, Val);
}

// Get a register from the metadata, or 0 if not currently set.
unsigned RVGPUPALMetadata::getRegister(unsigned Reg) {
...
// In fact this ORs the value into any previous setting of the register.
void RVGPUPALMetadata::setRegister(unsigned Reg, unsigned Val) {
  if (!isLegacy()) {
...
void RVGPUPALMetadata::setEntryPoint(unsigned CC, StringRef Name) {
  if (isLegacy())
    return;
  // Msgpack format.
...
// allocate.
void RVGPUPALMetadata::setNumUsedVgprs(CallingConv::ID CC, unsigned Val) {
  if (isLegacy()) {
    // Old non-msgpack format.
    unsigned NumUsedVgprsKey = getScratchSizeKey(CC) +
                               RVPALMD::Key::VS_NUM_USED_VGPRS -
                               RVPALMD::Key::VS_SCRATCH_SIZE;
...
void RVGPUPALMetadata::setNumUsedAgprs(CallingConv::ID CC, unsigned Val) {
...
void RVGPUPALMetadata::setNumUsedSgprs(CallingConv::ID CC, unsigned Val) {
  if (isLegacy()) {
    // Old non-msgpack format.
    unsigned NumUsedSgprsKey = getScratchSizeKey(CC) +
                               RVPALMD::Key::VS_NUM_USED_SGPRS -
                               RVPALMD::Key::VS_SCRATCH_SIZE;
...
void RVGPUPALMetadata::setScratchSize(CallingConv::ID CC, unsigned Val) {
  if (isLegacy()) {
    // Old non-msgpack format.
...
void RVGPUPALMetadata::setFunctionScratchSize(StringRef FnName, unsigned Val) {
...
void RVGPUPALMetadata::setFunctionLdsSize(StringRef FnName, unsigned Val) {
  auto Node = getShaderFunction(FnName);
  Node[".lds_size"] = MsgPackDoc.getNode(Val);
}

// Set the number of used vgprs in the metadata.
void RVGPUPALMetadata::setFunctionNumUsedVgprs(StringRef FnName,
                                                unsigned Val) {
  auto Node = getShaderFunction(FnName);
...
// Set the number of used vgprs in the metadata.
void RVGPUPALMetadata::setFunctionNumUsedSgprs(StringRef FnName,
                                                unsigned Val) {
  auto Node = getShaderFunction(FnName);
...
void RVGPUPALMetadata::setWave32(unsigned CC) {
  switch (CC) {
  case CallingConv::RVGPU_HS:
    setRegister(RVPALMD::R_A2D5_VGT_SHADER_STAGES_EN, S_028B54_HS_W32_EN(1));
    break;
  case CallingConv::RVGPU_GS:
    setRegister(RVPALMD::R_A2D5_VGT_SHADER_STAGES_EN, S_028B54_GS_W32_EN(1));
    break;
  case CallingConv::RVGPU_VS:
    setRegister(RVPALMD::R_A2D5_VGT_SHADER_STAGES_EN, S_028B54_VS_W32_EN(1));
    break;
  case CallingConv::RVGPU_PS:
    setRegister(RVPALMD::R_A1B6_SPI_PS_IN_CONTROL, S_0286D8_PS_W32_EN(1));
    break;
  case CallingConv::RVGPU_CS:
    setRegister(RVPALMD::R_2E00_COMPUTE_DISPATCH_INITIATOR,
...
static const char *getRegisterName(unsigned RegNum) {
  ...
  static const struct RegInfo {
    ...
    const char *Name;
      ...
      {RVPALMD::R_2C4A_SPI_SHADER_PGM_RSRC1_VS, "SPI_SHADER_PGM_RSRC1_VS"},
      {RVPALMD::R_2C4A_SPI_SHADER_PGM_RSRC1_VS + 1, "SPI_SHADER_PGM_RSRC2_VS"},
      {RVPALMD::R_2D4A_SPI_SHADER_PGM_RSRC1_LS, "SPI_SHADER_PGM_RSRC1_LS"},
      {RVPALMD::R_2D4A_SPI_SHADER_PGM_RSRC1_LS + 1, "SPI_SHADER_PGM_RSRC2_LS"},
      {RVPALMD::R_2D0A_SPI_SHADER_PGM_RSRC1_HS, "SPI_SHADER_PGM_RSRC1_HS"},
      {RVPALMD::R_2D0A_SPI_SHADER_PGM_RSRC1_HS + 1, "SPI_SHADER_PGM_RSRC2_HS"},
      {RVPALMD::R_2CCA_SPI_SHADER_PGM_RSRC1_ES, "SPI_SHADER_PGM_RSRC1_ES"},
      {RVPALMD::R_2CCA_SPI_SHADER_PGM_RSRC1_ES + 1, "SPI_SHADER_PGM_RSRC2_ES"},
      {RVPALMD::R_2C8A_SPI_SHADER_PGM_RSRC1_GS, "SPI_SHADER_PGM_RSRC1_GS"},
      {RVPALMD::R_2C8A_SPI_SHADER_PGM_RSRC1_GS + 1, "SPI_SHADER_PGM_RSRC2_GS"},
      {RVPALMD::R_2E00_COMPUTE_DISPATCH_INITIATOR, "COMPUTE_DISPATCH_INITIATOR"},
      {RVPALMD::R_2E12_COMPUTE_PGM_RSRC1, "COMPUTE_PGM_RSRC1"},
      {RVPALMD::R_2E12_COMPUTE_PGM_RSRC1 + 1, "COMPUTE_PGM_RSRC2"},
      {RVPALMD::R_2C0A_SPI_SHADER_PGM_RSRC1_PS, "SPI_SHADER_PGM_RSRC1_PS"},
      {RVPALMD::R_2C0A_SPI_SHADER_PGM_RSRC1_PS + 1, "SPI_SHADER_PGM_RSRC2_PS"},
      {RVPALMD::R_A1B3_SPI_PS_INPUT_ENA, "SPI_PS_INPUT_ENA"},
      {RVPALMD::R_A1B4_SPI_PS_INPUT_ADDR, "SPI_PS_INPUT_ADDR"},
      {RVPALMD::R_A1B6_SPI_PS_IN_CONTROL, "SPI_PS_IN_CONTROL"},
      {RVPALMD::R_A2D5_VGT_SHADER_STAGES_EN, "VGT_SHADER_STAGES_EN"},
...
void RVGPUPALMetadata::toString(std::string &String) {
  String.clear();
  if (!BlobType)
    return;
  ...
  if (isLegacy()) {
    ...
    Stream << '\t' << RVGPU::RVPALMD::AssemblerDirective << ' ';
    auto Regs = getRegisters();
  ...
  // Output as YAML.
  Stream << '\t' << RVGPU::RVPALMD::AssemblerDirectiveBegin << '\n';
  MsgPackDoc.toYAML(Stream);
  Stream << '\t' << RVGPU::RVPALMD::AssemblerDirectiveEnd << '\n';
...
// a .note record of the specified RV type. Returns an empty blob if
// there is no PAL metadata,
void RVGPUPALMetadata::toBlob(unsigned Type, std::string &Blob) {
  if (Type == ELF::NT_RV_PAL_METADATA)
    toLegacyBlob(Blob);
  else if (Type)
    toMsgPackBlob(Blob);
...
void RVGPUPALMetadata::toLegacyBlob(std::string &Blob) {
  Blob.clear();
...
void RVGPUPALMetadata::toMsgPackBlob(std::string &Blob) {
  Blob.clear();
  MsgPackDoc.writeToBlob(Blob);
}

// Set PAL metadata from YAML text. Returns false if failed.
bool RVGPUPALMetadata::setFromString(StringRef S) {
  BlobType = ELF::NT_RVGPU_METADATA;
...
msgpack::DocNode &RVGPUPALMetadata::refRegisters() {
  auto &N =
      MsgPackDoc.getRoot()
          .getMap(/*Convert=*/true)[MsgPackDoc.getNode("rvpal.pipelines")]
...
msgpack::MapDocNode RVGPUPALMetadata::getRegisters() {
...
msgpack::DocNode &RVGPUPALMetadata::refShaderFunctions() {
  auto &N =
      MsgPackDoc.getRoot()
          .getMap(/*Convert=*/true)[MsgPackDoc.getNode("rvpal.pipelines")]
          .getArray(/*Convert=*/true)[0]
...
msgpack::MapDocNode RVGPUPALMetadata::getShaderFunctions() {
...
msgpack::MapDocNode RVGPUPALMetadata::getShaderFunction(StringRef Name) {
  auto Functions = getShaderFunctions();
  return Functions[Name].getMap(/*Convert=*/true);
}

msgpack::DocNode &RVGPUPALMetadata::refComputeRegisters() {
  auto &N =
      MsgPackDoc.getRoot()
          .getMap(/*Convert=*/true)[MsgPackDoc.getNode("rvpal.pipelines")]
          .getArray(/*Convert=*/true)[0]
...
msgpack::MapDocNode RVGPUPALMetadata::getComputeRegisters() {
  if (ComputeRegisters.isEmpty())
    ComputeRegisters = refComputeRegisters();
  return ComputeRegisters.getMap();
}

msgpack::DocNode &RVGPUPALMetadata::refGraphicsRegisters() {
  auto &N =
      MsgPackDoc.getRoot()
          .getMap(/*Convert=*/true)[MsgPackDoc.getNode("rvpal.pipelines")]
          .getArray(/*Convert=*/true)[0]
...
msgpack::MapDocNode RVGPUPALMetadata::getGraphicsRegisters() {
...
msgpack::DocNode &RVGPUPALMetadata::refHwStage() {
  auto &N =
      MsgPackDoc.getRoot()
          .getMap(/*Convert=*/true)[MsgPackDoc.getNode("rvpal.pipelines")]
          .getArray(/*Convert=*/true)[0]
...
// convention.
msgpack::MapDocNode RVGPUPALMetadata::getHwStage(unsigned CC) {
  if (HwStages.isEmpty())
    HwStages = refHwStage();
...
const char *RVGPUPALMetadata::getVendor() const {
  return isLegacy() ? ElfNote::NoteNameV2 : ElfNote::NoteNameV3;
}

// Get .note record type of metadata blob to be emitted:
// ELF::NT_RV_PAL_METADATA (legacy key=val format), or
// ELF::NT_RVGPU_METADATA (MsgPack format), or
// 0 (no PAL metadata).
unsigned RVGPUPALMetadata::getType() const {
  return BlobType;
}

// Return whether the blob type is legacy PAL metadata.
bool RVGPUPALMetadata::isLegacy() const {
  return BlobType == ELF::NT_RV_PAL_METADATA;
}

// Set legacy PAL metadata format.
void RVGPUPALMetadata::setLegacy() {
  BlobType = ELF::NT_RV_PAL_METADATA;
}

// Erase all PAL metadata.
void RVGPUPALMetadata::reset() {
  MsgPackDoc.clear();
...
unsigned RVGPUPALMetadata::getPALMajorVersion() { return getPALVersion(0); }

unsigned RVGPUPALMetadata::getPALMinorVersion() { return getPALVersion(1); }

// Set the field in a given .hardware_stages entry
void RVGPUPALMetadata::setHwStage(unsigned CC, StringRef field, unsigned Val) {
  getHwStage(CC)[field] = Val;
}

void RVGPUPALMetadata::setHwStage(unsigned CC, StringRef field, bool Val) {
  getHwStage(CC)[field] = Val;
}

void RVGPUPALMetadata::setComputeRegisters(StringRef field, unsigned Val) {
  getComputeRegisters()[field] = Val;
}

void RVGPUPALMetadata::setComputeRegisters(StringRef field, bool Val) {
  getComputeRegisters()[field] = Val;
}

msgpack::DocNode *RVGPUPALMetadata::refComputeRegister(StringRef field) {
  auto M = getComputeRegisters();
  auto I = M.find(field);
  return I == M.end() ? nullptr : &I->second;
}

bool RVGPUPALMetadata::checkComputeRegisters(StringRef field, unsigned Val) {
  if (auto N = refComputeRegister(field))
    return N->getUInt() == Val;
  return false;
}

bool RVGPUPALMetadata::checkComputeRegisters(StringRef field, bool Val) {
  if (auto N = refComputeRegister(field))
    return N->getBool() == Val;
  return false;
}

void RVGPUPALMetadata::setGraphicsRegisters(StringRef field, unsigned Val) {
  getGraphicsRegisters()[field] = Val;
}

void RVGPUPALMetadata::setGraphicsRegisters(StringRef field, bool Val) {
  getGraphicsRegisters()[field] = Val;
}

void RVGPUPALMetadata::setGraphicsRegisters(StringRef field1, StringRef field2,
                                             unsigned Val) {
