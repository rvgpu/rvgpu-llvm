...
//===----------------------------------------------------------------------===//
//
/// \file - specifies tables for rv_kernel_code_t structure parsing/printing
//
//===----------------------------------------------------------------------===//

#define QNAME(name) rv_kernel_code_t::name
...
#define PRINTCODEPROP(name) \
  printBitField<FLD_T(code_properties),\
                RV_CODE_PROPERTY_##name##_SHIFT,\
                RV_CODE_PROPERTY_##name##_WIDTH>
...
[](StringRef Name, const rv_kernel_code_t &C, raw_ostream &OS) { \
   printName(OS, Name) << \
     (int)GetMacro(C.compute_pgm_resource_registers >> Shift); \
}
#define PARSECOMP(SetMacro, Shift) \
[](rv_kernel_code_t &C, MCAsmParser &MCParser, raw_ostream &Err) { \
   int64_t Value = 0; \
...
FIELD2(rv_code_version_major,        kernel_code_version_major,  rv_kernel_code_version_major),
FIELD2(rv_code_version_minor,        kernel_code_version_minor,  rv_kernel_code_version_minor),
FIELD2(rv_machine_kind,              machine_kind,               rv_machine_kind),
FIELD2(rv_machine_version_major,     machine_version_major,      rv_machine_version_major),
FIELD2(rv_machine_version_minor,     machine_version_minor,      rv_machine_version_minor),
FIELD2(rv_machine_version_stepping,  machine_version_stepping,   rv_machine_version_stepping),
