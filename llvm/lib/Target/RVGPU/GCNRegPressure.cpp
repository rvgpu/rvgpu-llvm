...
unsigned GCNRegPressure::getRegKind(Register Reg,
  ...
  auto STI = static_cast<const RVRegisterInfo*>(MRI.getTargetRegisterInfo());
...
void GCNRegPressure::inc(unsigned Reg,
                         ...
                         LaneBitmask NewMask,
                         const MachineRegisterInfo &MRI) {
  if (RVRegisterInfo::getNumCoveredRegs(NewMask) ==
      RVRegisterInfo::getNumCoveredRegs(PrevMask))
    return;
  ...
  case AGPR_TUPLE:
    ...
    Value[Kind == SGPR_TUPLE ? SGPR32 : Kind == AGPR_TUPLE ? AGPR32 : VGPR32] +=
      Sign * RVRegisterInfo::getNumCoveredRegs(~PrevMask & NewMask);
...
bool GCNRegPressure::less(const RVSubtarget &ST,
...
Printable llvm::print(const GCNRegPressure &RP, const RVSubtarget *ST) {
...
INITIALIZE_PASS(GCNRegPressurePrinter, "rvgpu-print-rp", "", true, true)
