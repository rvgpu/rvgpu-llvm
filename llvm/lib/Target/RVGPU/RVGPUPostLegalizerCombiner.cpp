//=== lib/CodeGen/GlobalISel/RVGPUPostLegalizerCombiner.cpp --------------===//
//
...
#define DEBUG_TYPE "rvgpu-postlegalizer-combiner"
...
public:
  RVGPUPostLegalizerCombinerImpl(
...
RVGPUPostLegalizerCombinerImpl::RVGPUPostLegalizerCombinerImpl(
    MachineFunction &MF, CombinerInfo &CInfo, const TargetPassConfig *TPC,
    GISelKnownBits &KB, GISelCSEInfo *CSEInfo,
    const RVGPUPostLegalizerCombinerImplRuleConfig &RuleConfig,
...
bool RVGPUPostLegalizerCombinerImpl::tryCombineAll(MachineInstr &MI) const {
...
bool RVGPUPostLegalizerCombinerImpl::matchFMinFMaxLegacy(
...
void RVGPUPostLegalizerCombinerImpl::applySelectFCmpToFMinToFMaxLegacy(
  ...
  case CmpInst::FCMP_ULE:
    if (Info.LHS == Info.True)
      buildNewInst(RVGPU::G_RVGPU_FMIN_LEGACY, Info.RHS, Info.LHS);
    else
      buildNewInst(RVGPU::G_RVGPU_FMAX_LEGACY, Info.LHS, Info.RHS);
    break;
  ...
  case CmpInst::FCMP_OLT: {
    ...
    if (Info.LHS == Info.True)
      buildNewInst(RVGPU::G_RVGPU_FMIN_LEGACY, Info.LHS, Info.RHS);
    else
      buildNewInst(RVGPU::G_RVGPU_FMAX_LEGACY, Info.RHS, Info.LHS);
    break;
  }
...
bool RVGPUPostLegalizerCombinerImpl::matchUCharToFloat(
...
void RVGPUPostLegalizerCombinerImpl::applyUCharToFloat(
    MachineInstr &MI) const {
  B.setInstrAndDebugLoc(MI);
  ...
  if (Ty == S32) {
    B.buildInstr(RVGPU::G_RVGPU_CVT_F32_UBYTE0, {DstReg}, {SrcReg},
                 MI.getFlags());
  } else {
    auto Cvt0 = B.buildInstr(RVGPU::G_RVGPU_CVT_F32_UBYTE0, {S32}, {SrcReg},
                             MI.getFlags());
...
bool RVGPUPostLegalizerCombinerImpl::matchRcpSqrtToRsq(
    MachineInstr &MI,
    std::function<void(MachineIRBuilder &)> &MatchInfo) const {
  ...
  if ((RcpSrcMI = getRcpSrc(MI)) && (SqrtSrcMI = getSqrtSrc(*RcpSrcMI))) {
    MatchInfo = [SqrtSrcMI, &MI](MachineIRBuilder &B) {
      B.buildIntrinsic(Intrinsic::rvgpu_rsq, {MI.getOperand(0)})
...
bool RVGPUPostLegalizerCombinerImpl::matchCvtF32UByteN(
  ...
  if (IsShr || mi_match(SrcReg, MRI, m_GShl(m_Reg(Src0), m_ICst(ShiftAmt)))) {
    const unsigned Offset = MI.getOpcode() - RVGPU::G_RVGPU_CVT_F32_UBYTE0;
...
void RVGPUPostLegalizerCombinerImpl::applyCvtF32UByteN(
    MachineInstr &MI, const CvtF32UByteMatchInfo &MatchInfo) const {
  B.setInstrAndDebugLoc(MI);
  unsigned NewOpc = RVGPU::G_RVGPU_CVT_F32_UBYTE0 + MatchInfo.ShiftOffset / 8;
...
bool RVGPUPostLegalizerCombinerImpl::matchRemoveFcanonicalize(
    MachineInstr &MI, Register &Reg) const {
  const RVTargetLowering *TLI = static_cast<const RVTargetLowering *>(
...
bool RVGPUPostLegalizerCombinerImpl::matchCombineSignExtendInReg(
  ...
  // instruction.
  return SubwordBufferLoad->getOpcode() == RVGPU::G_RVGPU_BUFFER_LOAD_UBYTE ||
         SubwordBufferLoad->getOpcode() == RVGPU::G_RVGPU_BUFFER_LOAD_USHORT;
}

// Combine buffer_load_{u8, u16} and the sign extension instruction to generate
// buffer_load_{i8, i16}.
void RVGPUPostLegalizerCombinerImpl::applyCombineSignExtendInReg(
    MachineInstr &MI, MachineInstr *&SubwordBufferLoad) const {
  // Modify the opcode and the destination of buffer_load_{u8, u16}:
  // Replace the opcode.
  unsigned Opc =
      SubwordBufferLoad->getOpcode() == RVGPU::G_RVGPU_BUFFER_LOAD_UBYTE
          ? RVGPU::G_RVGPU_BUFFER_LOAD_SBYTE
          : RVGPU::G_RVGPU_BUFFER_LOAD_SSHORT;
...
class RVGPUPostLegalizerCombiner : public MachineFunctionPass {
public:
  static char ID;

  RVGPUPostLegalizerCombiner(bool IsOptNone = false);

  StringRef getPassName() const override {
    return "RVGPUPostLegalizerCombiner";
  }
...
private:
  bool IsOptNone;
  RVGPUPostLegalizerCombinerImplRuleConfig RuleConfig;
};
} // end anonymous namespace

void RVGPUPostLegalizerCombiner::getAnalysisUsage(AnalysisUsage &AU) const {
...
RVGPUPostLegalizerCombiner::RVGPUPostLegalizerCombiner(bool IsOptNone)
    : MachineFunctionPass(ID), IsOptNone(IsOptNone) {
  initializeRVGPUPostLegalizerCombinerPass(*PassRegistry::getPassRegistry());

  if (!RuleConfig.parseCommandLineOption())
    report_fatal_error("Invalid rule identifier");
}

bool RVGPUPostLegalizerCombiner::runOnMachineFunction(MachineFunction &MF) {
  ...
  RVGPUPostLegalizerCombinerImpl Impl(MF, CInfo, TPC, *KB, /*CSEInfo*/ nullptr,
                                       RuleConfig, ST, MDT, LI);
  return Impl.combineMachineInstrs();
}

char RVGPUPostLegalizerCombiner::ID = 0;
INITIALIZE_PASS_BEGIN(RVGPUPostLegalizerCombiner, DEBUG_TYPE,
                      "Combine RVGPU machine instrs after legalization", false,
                      false)
INITIALIZE_PASS_DEPENDENCY(TargetPassConfig)
INITIALIZE_PASS_DEPENDENCY(GISelKnownBitsAnalysis)
INITIALIZE_PASS_END(RVGPUPostLegalizerCombiner, DEBUG_TYPE,
                    "Combine RVGPU machine instrs after legalization", false,
                    false)

namespace llvm {
FunctionPass *createRVGPUPostLegalizeCombiner(bool IsOptNone) {
  return new RVGPUPostLegalizerCombiner(IsOptNone);
}
} // end namespace llvm
