//===-- llvm/lib/Target/RVGPU/RVGPUCallLowering.cpp - Call lowering -----===//
//
...
#define DEBUG_TYPE "rvgpu-call-lowering"
...
struct RVGPUOutgoingValueHandler : public CallLowering::OutgoingValueHandler {
  RVGPUOutgoingValueHandler(MachineIRBuilder &B, MachineRegisterInfo &MRI,
  ...
  void assignValueToReg(Register ValVReg, Register PhysReg,
    ...
    const RVRegisterInfo *TRI
      = static_cast<const RVRegisterInfo *>(MRI.getTargetRegisterInfo());
    if (TRI->isSGPRReg(MRI, PhysReg)) {
      ...
      auto ToSGPR = MIRBuilder
                        .buildIntrinsic(Intrinsic::rvgpu_readfirstlane,
                                        {MRI.getType(ExtReg)})
                        .addReg(ExtReg);
...
struct RVGPUIncomingArgHandler : public CallLowering::IncomingValueHandler {
  uint64_t StackUsed = 0;

  RVGPUIncomingArgHandler(MachineIRBuilder &B, MachineRegisterInfo &MRI)
  ...
  Register getStackAddress(uint64_t Size, int64_t Offset,
    ...
    auto AddrReg = MIRBuilder.buildFrameIndex(
        LLT::pointer(RVGPUAS::PRIVATE_ADDRESS, 32), FI);
...
struct FormalArgHandler : public RVGPUIncomingArgHandler {
  FormalArgHandler(MachineIRBuilder &B, MachineRegisterInfo &MRI)
      : RVGPUIncomingArgHandler(B, MRI) {}
...
struct CallReturnHandler : public RVGPUIncomingArgHandler {
  CallReturnHandler(MachineIRBuilder &MIRBuilder, MachineRegisterInfo &MRI,
                    MachineInstrBuilder MIB)
      : RVGPUIncomingArgHandler(MIRBuilder, MRI), MIB(MIB) {}
...
struct RVGPUOutgoingArgHandler : public RVGPUOutgoingValueHandler {
  ...
  RVGPUOutgoingArgHandler(MachineIRBuilder &MIRBuilder,
                           MachineRegisterInfo &MRI, MachineInstrBuilder MIB,
                           bool IsTailCall = false, int FPDiff = 0)
      : RVGPUOutgoingValueHandler(MIRBuilder, MRI, MIB), FPDiff(FPDiff),
  ...
  Register getStackAddress(uint64_t Size, int64_t Offset,
                           ...
                           ISD::ArgFlagsTy Flags) override {
    MachineFunction &MF = MIRBuilder.getMF();
    const LLT PtrTy = LLT::pointer(RVGPUAS::PRIVATE_ADDRESS, 32);
    ...
    if (!SPReg) {
      const RVSubtarget &ST = MIRBuilder.getMF().getSubtarget<RVSubtarget>();
      if (ST.enableFlatScratch()) {
        ...
        // swizzled address.
        SPReg = MIRBuilder.buildInstr(RVGPU::G_RVGPU_WAVE_ADDRESS, {PtrTy},
                                      {MFI->getStackPtrOffsetReg()}).getReg(0);
      }
    }
  ...
  void assignValueToAddress(Register ValVReg, Register Addr, LLT MemTy,
    ...
    const auto &ST = MF.getSubtarget<RVSubtarget>();
...
RVGPUCallLowering::RVGPUCallLowering(const RVGPUTargetLowering &TLI)
  : CallLowering(&TLI) {
}
...
bool RVGPUCallLowering::canLowerReturn(MachineFunction &MF,
  ...
  const RVTargetLowering &TLI = *getTLI<RVTargetLowering>();
...
bool RVGPUCallLowering::lowerReturnVal(MachineIRBuilder &B,
  ...
  RVGPUOutgoingValueHandler RetHandler(B, *MRI, Ret);
  return determineAndHandleAssignments(RetHandler, Assigner, SplitRetInfos, B,
                                       CC, F.isVarArg());
}

bool RVGPUCallLowering::lowerReturn(MachineIRBuilder &B, const Value *Val,
  ...
  const bool IsShader = RVGPU::isShader(CC);
  const bool IsWaveEnd =
      (IsShader && MFI->returnsVoid()) || RVGPU::isKernel(CC);
  if (IsWaveEnd) {
    B.buildInstr(RVGPU::S_ENDPGM)
      .addImm(0);
    return true;
  }

  unsigned ReturnOpc =
      IsShader ? RVGPU::SI_RETURN_TO_EPILOG : RVGPU::SI_RETURN;
...
void RVGPUCallLowering::lowerParameterPtr(Register DstReg, MachineIRBuilder &B,
                                           uint64_t Offset) const {
  MachineFunction &MF = B.getMF();
  const RVMachineFunctionInfo *MFI = MF.getInfo<RVMachineFunctionInfo>();
  MachineRegisterInfo &MRI = MF.getRegInfo();
  Register KernArgSegmentPtr =
    MFI->getPreloadedReg(RVGPUFunctionArgInfo::KERNARG_SEGMENT_PTR);
...
void RVGPUCallLowering::lowerParameter(MachineIRBuilder &B, ArgInfo &OrigArg,
                                        uint64_t Offset,
  ...
  LLT PtrTy = LLT::pointer(RVGPUAS::CONSTANT_ADDRESS, 64);
...
static void allocateHSAUserSGPRs(CCState &CCInfo,
  ...
  if (UserSGPRInfo.hasKernargSegmentPtr()) {
    ...
    const LLT P4 = LLT::pointer(RVGPUAS::CONSTANT_ADDRESS, 64);
...
bool RVGPUCallLowering::lowerFormalArgumentsKernel(
  ...
  for (auto &Arg : F.args()) {
    ...
    if (IsByRef) {
      ...
      if (ByRefAS == RVGPUAS::CONSTANT_ADDRESS) {
        lowerParameterPtr(VRegs[i][0], B, ArgOffset);
      } else {
        const LLT ConstPtrTy = LLT::pointer(RVGPUAS::CONSTANT_ADDRESS, 64);
...
bool RVGPUCallLowering::lowerFormalArguments(
  ...
  const bool IsGraphics = RVGPU::isGraphics(CC);
  const bool IsEntryFunc = RVGPU::isEntryFunctionCC(CC);
  ...
  if (UserSGPRInfo.hasFlatScratchInit() && !Subtarget.isRvPalOS()) {
    Register FlatScratchInitReg = Info->addFlatScratchInit(*TRI);
    MF.addLiveIn(FlatScratchInitReg, &RVGPU::SGPR_64RegClass);
    CCInfo.AllocateReg(FlatScratchInitReg);
  }
  ...
  for (auto &Arg : F.args()) {
    ...
    if (CC == CallingConv::RVGPU_PS && !InReg && PSInputNum <= 15) {
...
bool RVGPUCallLowering::passSpecialInputs(MachineIRBuilder &MIRBuilder,
                                           CCState &CCInfo,
  ...
  const RVMachineFunctionInfo *MFI = MF.getInfo<RVMachineFunctionInfo>();
  const RVGPUFunctionArgInfo &CallerArgInfo = MFI->getArgInfo();


  // TODO: Unify with private memory register handling. This is complicated by
  // the fact that at least in kernels, the input argument is not necessarily
  // in the same location as the input.
  RVGPUFunctionArgInfo::PreloadedValue InputRegs[] = {
    RVGPUFunctionArgInfo::DISPATCH_PTR,
    RVGPUFunctionArgInfo::QUEUE_PTR,
    RVGPUFunctionArgInfo::IMPLICIT_ARG_PTR,
    RVGPUFunctionArgInfo::DISPATCH_ID,
  ...
  static constexpr StringLiteral ImplicitAttrNames[] = {
    "rvgpu-no-dispatch-ptr",
    "rvgpu-no-queue-ptr",
    "rvgpu-no-implicitarg-ptr",
    "rvgpu-no-dispatch-id",
    "rvgpu-no-workgroup-id-x",
    "rvgpu-no-workgroup-id-y",
    "rvgpu-no-workgroup-id-z",
    "rvgpu-no-lds-kernel-id",
  };
  ...
  for (auto InputID : InputRegs) {
    ...
    if (IncomingArg) {
      ...
      std::optional<uint32_t> Id =
          RVGPUMachineFunction::getLDSKernelIdMetadata(MF.getFunction());
      if (Id) {
  ...
  const bool NeedWorkItemIDY = !Info.CB->hasFnAttr("rvgpu-no-workitem-id-y");
  const bool NeedWorkItemIDZ = !Info.CB->hasFnAttr("rvgpu-no-workitem-id-z");
...
getAssignFnsForCC(CallingConv::ID CC, const RVTargetLowering &TLI) {
...
static unsigned getCallOpcode(const MachineFunction &CallerF, bool IsIndirect,
                              ...
                              CallingConv::ID CC) {
  // For calls to rvgpu_cs_chain functions, the address is known to be uniform.
  assert((RVGPU::isChainCC(CC) || !IsIndirect || !IsTailCall) &&
         "Indirect calls can't be tail calls, "
         "because the address can be divergent");
  if (!IsTailCall)
    return RVGPU::G_SI_CALL;

  if (RVGPU::isChainCC(CC))
    return isWave32 ? RVGPU::SI_CS_CHAIN_TC_W32 : RVGPU::SI_CS_CHAIN_TC_W64;

  return CC == CallingConv::RVGPU_Gfx ? RVGPU::SI_TCRETURN_GFX :
                                         RVGPU::SI_TCRETURN;
}
...
static bool addCallTargetOperands(MachineInstrBuilder &CallInst,
                                  ...
                                  RVGPUCallLowering::CallLoweringInfo &Info) {
...
bool RVGPUCallLowering::doCallerAndCalleePassArgsTheSameWay(
...
bool RVGPUCallLowering::areCalleeOutgoingArgsTailCallable(
    CallLoweringInfo &Info, MachineFunction &MF,
...
bool RVGPUCallLowering::isEligibleForTailCallOptimization(
  ...
  const RVRegisterInfo *TRI = MF.getSubtarget<RVSubtarget>().getRegisterInfo();
  const uint32_t *CallerPreserved = TRI->getCallPreservedMask(MF, CallerCC);
...
// call instruction.
void RVGPUCallLowering::handleImplicitCallArguments(
    MachineIRBuilder &MIRBuilder, MachineInstrBuilder &CallInst,
    const RVSubtarget &ST, const RVMachineFunctionInfo &FuncInfo,
  ...
  if (!ST.enableFlatScratch()) {
    ...
    auto CalleeRSrcReg = RVGPU::isChainCC(CalleeCC)
...
bool RVGPUCallLowering::lowerTailCall(
  ...
  if (!IsSibCall)
    CallSeqStart = MIRBuilder.buildInstr(RVGPU::ADJCALLSTACKUP);
  ...
  const RVRegisterInfo *TRI = ST.getRegisterInfo();
  if (RVGPU::isChainCC(Info.CallConv)) {
  ...
  if (Info.CallConv != CallingConv::RVGPU_Gfx &&
      !RVGPU::isChainCC(Info.CallConv)) {
  ...
  RVGPUOutgoingArgHandler Handler(MIRBuilder, MRI, MIB, true, FPDiff);
  ...
  if (!IsSibCall) {
    ...
    // location.
    MIRBuilder.buildInstr(RVGPU::ADJCALLSTACKDOWN).addImm(NumBytes).addImm(0);
  }
...
/// Lower a call to the @llvm.rvgpu.cs.chain intrinsic.
bool RVGPUCallLowering::lowerChainCall(MachineIRBuilder &MIRBuilder,
                                        CallLoweringInfo &Info) const {
  ...
  if (const Function *F = dyn_cast<Function>(CalleeV)) {
    ...
    Info.CallConv = CallingConv::RVGPU_CS_Chain; // rvgpu_cs_chain_preserve
...
bool RVGPUCallLowering::lowerCall(MachineIRBuilder &MIRBuilder,
                                   CallLoweringInfo &Info) const {
  if (Function *F = Info.CB->getCalledFunction())
    if (F->isIntrinsic()) {
      assert(F->getIntrinsicID() == Intrinsic::rvgpu_cs_chain &&
  ...
  MIRBuilder.buildInstr(RVGPU::ADJCALLSTACKUP)
    .addImm(0)
    .addImm(0);
  ...
  RVGPUOutgoingArgHandler Handler(MIRBuilder, MRI, MIB, false);
  if (!handleAssignments(Handler, OutArgs, CCInfo, ArgLocs, MIRBuilder))
    return false;
  ...
  MIRBuilder.buildInstr(RVGPU::ADJCALLSTACKDOWN)
            .addImm(0)
