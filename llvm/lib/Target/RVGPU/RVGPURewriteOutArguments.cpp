//===- RVGPURewriteOutArgumentsPass.cpp - Create struct returns ----------===//
//
...
#define DEBUG_TYPE "rvgpu-rewrite-out-arguments"

using namespace llvm;

static cl::opt<bool> AnyAddressSpace(
  "rvgpu-any-address-space-out-arguments",
...
class RVGPURewriteOutArguments : public FunctionPass {
private:
...
public:
  ...
  RVGPURewriteOutArguments() : FunctionPass(ID) {}
...
INITIALIZE_PASS_BEGIN(RVGPURewriteOutArguments, DEBUG_TYPE,
                      "RVGPU Rewrite Out Arguments", false, false)
INITIALIZE_PASS_DEPENDENCY(MemoryDependenceWrapperPass)
INITIALIZE_PASS_END(RVGPURewriteOutArguments, DEBUG_TYPE,
                    "RVGPU Rewrite Out Arguments", false, false)

char RVGPURewriteOutArguments::ID = 0;

Type *RVGPURewriteOutArguments::getStoredType(Value &Arg) const {
...
Type *RVGPURewriteOutArguments::getOutArgumentType(Argument &Arg) const {
...
bool RVGPURewriteOutArguments::doInitialization(Module &M) {
  DL = &M.getDataLayout();
  return false;
}

bool RVGPURewriteOutArguments::runOnFunction(Function &F) {
  if (skipFunction(F))
    return false;
...
FunctionPass *llvm::createRVGPURewriteOutArgumentsPass() {
  return new RVGPURewriteOutArguments();
}
