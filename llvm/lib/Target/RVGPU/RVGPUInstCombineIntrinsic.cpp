//===- RVGPInstCombineIntrinsic.cpp - RVGPU specific InstCombine pass ---===//
//
...
// RVGPU target machine. It uses the target's detailed information to provide
...
struct RVGPUImageDMaskIntrinsic {
  unsigned Intr;
};

#define GET_RVGPUImageDMaskIntrinsicTable_IMPL
#include "RVGPUInstCombineTables.inc"

} // end anonymous namespace

// Constant fold llvm.rvgpu.fmed3 intrinsics for standard inputs.
//
// A single NaN input is folded to minnum, so we rely on that folding for
// handling NaNs.
static APFloat fmed3RVGPU(const APFloat &Src0, const APFloat &Src1,
                           const APFloat &Src2) {
...
simplifyRVGPUImageIntrinsic(const RVSubtarget *ST,
                             const RVGPU::ImageDimIntrinsicInfo *ImageDimIntr,
                             IntrinsicInst &II, InstCombiner &IC) {
  // Optimize _L to _LZ when _L is zero
  if (const auto *LZMappingInfo =
          RVGPU::getMIMGLZMappingInfo(ImageDimIntr->BaseOpcode)) {
    if (auto *ConstantLod =
            dyn_cast<ConstantFP>(II.getOperand(ImageDimIntr->LodIndex))) {
      if (ConstantLod->isZero() || ConstantLod->isNegative()) {
        const RVGPU::ImageDimIntrinsicInfo *NewImageDimIntr =
            RVGPU::getImageDimIntrinsicByBaseOpcode(LZMappingInfo->LZ,
                                                     ImageDimIntr->Dim);
  ...
  if (const auto *MIPMappingInfo =
          RVGPU::getMIMGMIPMappingInfo(ImageDimIntr->BaseOpcode)) {
    if (auto *ConstantMip =
            dyn_cast<ConstantInt>(II.getOperand(ImageDimIntr->MipIndex))) {
      if (ConstantMip->isZero()) {
        const RVGPU::ImageDimIntrinsicInfo *NewImageDimIntr =
            RVGPU::getImageDimIntrinsicByBaseOpcode(MIPMappingInfo->NONMIP,
                                                     ImageDimIntr->Dim);
        return modifyIntrinsicCall(
            II, II, NewImageDimIntr->Intr, IC, [&](auto &Args, auto &ArgTys) {
  ...
  if (const auto *BiasMappingInfo =
          RVGPU::getMIMGBiasMappingInfo(ImageDimIntr->BaseOpcode)) {
    if (auto *ConstantBias =
            dyn_cast<ConstantFP>(II.getOperand(ImageDimIntr->BiasIndex))) {
      if (ConstantBias->isZero()) {
        const RVGPU::ImageDimIntrinsicInfo *NewImageDimIntr =
            RVGPU::getImageDimIntrinsicByBaseOpcode(BiasMappingInfo->NoBias,
                                                     ImageDimIntr->Dim);
        return modifyIntrinsicCall(
            II, II, NewImageDimIntr->Intr, IC, [&](auto &Args, auto &ArgTys) {
  ...
  if (const auto *OffsetMappingInfo =
          RVGPU::getMIMGOffsetMappingInfo(ImageDimIntr->BaseOpcode)) {
    if (auto *ConstantOffset =
            dyn_cast<ConstantInt>(II.getOperand(ImageDimIntr->OffsetIndex))) {
      if (ConstantOffset->isZero()) {
        const RVGPU::ImageDimIntrinsicInfo *NewImageDimIntr =
            RVGPU::getImageDimIntrinsicByBaseOpcode(
  ...
  if (ST->hasD16Images()) {
    ...
    const RVGPU::MIMGBaseOpcodeInfo *BaseOpcode =
        RVGPU::getMIMGBaseOpcodeInfo(ImageDimIntr->BaseOpcode);
  ...
  bool HasSampler =
      RVGPU::getMIMGBaseOpcodeInfo(ImageDimIntr->BaseOpcode)->Sampler;
...
static Value *simplifyRVGPUMemoryIntrinsicDemanded(InstCombiner &IC,
                                                    IntrinsicInst &II,
                                                    APInt DemandedElts,
                                                    int DMaskIdx = -1,
                                                    bool IsLoad = true);

/// Return true if it's legal to contract llvm.rvgpu.rcp(llvm.sqrt)
...
RVTTIImpl::instCombineIntrinsic(InstCombiner &IC, IntrinsicInst &II) const {
  ...
  case Intrinsic::rvgpu_rcp: {
    ...
    // llvm.rvgpu.rcp(llvm.rvgpu.sqrt(x)) -> llvm.rvgpu.rsq(x) if contractable
    //
    // llvm.rvgpu.rcp(llvm.sqrt(x)) -> llvm.rvgpu.rsq(x) if contractable and
    // relaxed.
    if (IID == Intrinsic::rvgpu_sqrt || IID == Intrinsic::sqrt) {
      ...
      Function *NewDecl = Intrinsic::getDeclaration(
          SrcCI->getModule(), Intrinsic::rvgpu_rsq, {SrcCI->getType()});
  ...
  case Intrinsic::rvgpu_rsq: {
    ...
    // f16 rvgpu.sqrt is identical to regular sqrt.
    if (IID == Intrinsic::rvgpu_sqrt && Src->getType()->isHalfTy()) {
      Function *NewDecl = Intrinsic::getDeclaration(
  ...
  case Intrinsic::rvgpu_exp2: {
    const bool IsLog = IID == Intrinsic::rvgpu_log;
    const bool IsExp = IID == Intrinsic::rvgpu_exp2;
    Value *Src = II.getArgOperand(0);
    Type *Ty = II.getType();
  ...
  case Intrinsic::rvgpu_frexp_exp: {
    ...
    if (const ConstantFP *C = dyn_cast<ConstantFP>(Src)) {
      ...
      if (IID == Intrinsic::rvgpu_frexp_mant) {
  ...
  case Intrinsic::rvgpu_sbfe: {
    ...
    bool Signed = IID == Intrinsic::rvgpu_sbfe;
  ...
  case Intrinsic::rvgpu_exp_compr: {
    ...
    bool IsCompr = IID == Intrinsic::rvgpu_exp_compr;
    bool Changed = false;
    ...
    if (const ConstantFP *C0 = dyn_cast<ConstantFP>(Src0)) {
      if (const ConstantFP *C1 = dyn_cast<ConstantFP>(Src1)) {
        if (const ConstantFP *C2 = dyn_cast<ConstantFP>(Src2)) {
          APFloat Result = fmed3RVGPU(C0->getValueAPF(), C1->getValueAPF(),
                                       C2->getValueAPF());
          return IC.replaceInstUsesWith(
  ...
  case Intrinsic::rvgpu_fcmp: {
    ...
    bool IsInteger = IID == Intrinsic::rvgpu_icmp;
    ...
    // llvm.rvgpu.icmp(zext (i1 x), 1, eq)
    //   -> llvm.rvgpu.icmp(zext (i1 x), 0, ne)
    // llvm.rvgpu.icmp(sext (i1 x), -1, eq)
    //   -> llvm.rvgpu.icmp(sext (i1 x), 0, ne)
    Value *ExtSrc;
    ...
    // llvm.rvgpu.icmp([sz]ext ([if]cmp pred a, b), 0, ne)
    //   -> llvm.rvgpu.[if]cmp(a, b, pred)
    //
    // llvm.rvgpu.icmp([sz]ext ([if]cmp pred a, b), 0, eq)
    //   -> llvm.rvgpu.[if]cmp(a, b, inv pred)
    if (match(Src1, PatternMatch::m_Zero()) &&
      ...
      Intrinsic::ID NewIID = CmpInst::isFPPredicate(SrcPred)
                                 ? Intrinsic::rvgpu_fcmp
                                 : Intrinsic::rvgpu_icmp;
  ...
  case Intrinsic::rvgpu_permlanex16_var: {
    ...
    unsigned int FiIdx = (IID == Intrinsic::rvgpu_permlane16 ||
                          IID == Intrinsic::rvgpu_permlanex16)
  ...
  case Intrinsic::rvgpu_readlane: {
    ...
    if (match(Src,
              PatternMatch::m_Intrinsic<Intrinsic::rvgpu_readfirstlane>())) {
      return IC.replaceInstUsesWith(II, Src);
    }

    if (IID == Intrinsic::rvgpu_readfirstlane) {
      // readfirstlane (readlane x, y) -> readlane x, y
      if (match(Src, PatternMatch::m_Intrinsic<Intrinsic::rvgpu_readlane>())) {
        return IC.replaceInstUsesWith(II, Src);
      }
    } else {
  ...
  case Intrinsic::rvgpu_image_store_1d:
  case Intrinsic::rvgpu_image_store_1darray:
  case Intrinsic::rvgpu_image_store_2d:
  case Intrinsic::rvgpu_image_store_2darray:
  case Intrinsic::rvgpu_image_store_2darraymsaa:
  case Intrinsic::rvgpu_image_store_2dmsaa:
  case Intrinsic::rvgpu_image_store_3d:
  case Intrinsic::rvgpu_image_store_cube:
  case Intrinsic::rvgpu_image_store_mip_1d:
  case Intrinsic::rvgpu_image_store_mip_1darray:
  case Intrinsic::rvgpu_image_store_mip_2d:
  case Intrinsic::rvgpu_image_store_mip_2darray:
  case Intrinsic::rvgpu_image_store_mip_3d:
  case Intrinsic::rvgpu_image_store_mip_cube: {
    ...
    int DMaskIdx = getRVGPUImageDMaskIntrinsic(II.getIntrinsicID()) ? 1 : -1;
    if (simplifyRVGPUMemoryIntrinsicDemanded(IC, II, DemandedElts, DMaskIdx,
                                              false)) {
      return IC.eraseInstFromFunction(II);
    }
  ...
  if (const RVGPU::ImageDimIntrinsicInfo *ImageDimIntr =
            RVGPU::getImageDimIntrinsicInfo(II.getIntrinsicID())) {
    return simplifyRVGPUImageIntrinsic(ST, ImageDimIntr, II, IC);
  }
  return std::nullopt;
}

/// Implement SimplifyDemandedVectorElts for rvgpu buffer and image intrinsics.
///
/// The result of simplifying rvgpu image and buffer store intrinsics is updating
...
std::optional<Value *> RVTTIImpl::simplifyDemandedVectorEltsIntrinsic(
  ...
  case Intrinsic::rvgpu_tbuffer_load:
    return simplifyRVGPUMemoryIntrinsicDemanded(IC, II, DemandedElts);
  default: {
    if (getRVGPUImageDMaskIntrinsic(II.getIntrinsicID())) {
      return simplifyRVGPUMemoryIntrinsicDemanded(IC, II, DemandedElts, 0);
    }
    break;
  }
  }
