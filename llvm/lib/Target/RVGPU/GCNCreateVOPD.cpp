...
public:
  ...
  bool doReplace(const RVInstrInfo *SII, VOPDCombineInfo &CI) {
    auto *FirstMI = CI.FirstMI;
    auto *SecondMI = CI.SecondMI;
    unsigned Opc1 = FirstMI->getOpcode();
    unsigned Opc2 = SecondMI->getOpcode();
    unsigned EncodingFamily =
        RVGPU::getVOPDEncodingFamily(SII->getSubtarget());
    int NewOpcode =
        RVGPU::getVOPDFull(RVGPU::getVOPDOpcode(Opc1),
                            RVGPU::getVOPDOpcode(Opc2), EncodingFamily);
    assert(NewOpcode != -1 &&
  ...
  bool runOnMachineFunction(MachineFunction &MF) override {
    ...
    const RVInstrInfo *SII = ST->getInstrInfo();
    bool Changed = false;
    ...
    for (auto &MBB : MF) {
      ...
      while (MII != E) {
        ...
        unsigned Opc2 = SecondMI->getOpcode();
        llvm::RVGPU::CanBeVOPD FirstCanBeVOPD = RVGPU::getCanBeVOPD(Opc);
        llvm::RVGPU::CanBeVOPD SecondCanBeVOPD = RVGPU::getCanBeVOPD(Opc2);
        VOPDCombineInfo CI;
