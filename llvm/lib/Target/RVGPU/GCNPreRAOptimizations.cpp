...
#define DEBUG_TYPE "rvgpu-pre-ra-optimizations"
...
public:
  ...
  StringRef getPassName() const override {
    return "RVGPU Pre-RA optimizations";
  }
...
INITIALIZE_PASS_BEGIN(GCNPreRAOptimizations, DEBUG_TYPE,
                      "RVGPU Pre-RA optimizations", false, false)
...
bool GCNPreRAOptimizations::processReg(Register Reg) {
  ...
  auto NewI = BuildMI(*Def0->getParent(), *Def0, Def0->getDebugLoc(),
                      TII->get(RVGPU::S_MOV_B64_IMM_PSEUDO), Reg)
                  .addImm(Init);
