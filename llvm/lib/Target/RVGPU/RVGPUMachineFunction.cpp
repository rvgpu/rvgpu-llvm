...
RVGPUMachineFunction::RVGPUMachineFunction(const Function &F,
    ...
    : IsEntryFunction(RVGPU::isEntryFunctionCC(F.getCallingConv())),
      ...
      IsChainFunction(RVGPU::isChainCC(F.getCallingConv())),
      NoSignedZerosFPMath(false) {

  // FIXME: Should initialize KernArgSize based on ExplicitKernelArgOffset,
  // except reserved size is not correctly aligned.

  Attribute MemBoundAttr = F.getFnAttribute("rvgpu-memory-bound");
  MemoryBound = MemBoundAttr.getValueAsBool();

  Attribute WaveLimitAttr = F.getFnAttribute("rvgpu-wave-limiter");
  WaveLimiter = WaveLimitAttr.getValueAsBool();

  // FIXME: How is this attribute supposed to interact with statically known
  // global sizes?
  StringRef S = F.getFnAttribute("rvgpu-gds-size").getValueAsString();
  if (!S.empty())
  ...
  // during codegen.
  std::pair<unsigned, unsigned> LDSSizeRange = RVGPU::getIntegerPairAttribute(
      F, "rvgpu-lds-size", {0, UINT32_MAX}, true);
  ...
  if (CC == CallingConv::RVGPU_KERNEL || CC == CallingConv::SPIR_KERNEL)
...
unsigned RVGPUMachineFunction::allocateLDSGlobal(const DataLayout &DL,
  ...
  if (GV.getAddressSpace() == RVGPUAS::LOCAL_ADDRESS) {
    ...
    assert(GV.getAddressSpace() == RVGPUAS::REGION_ADDRESS &&
...
getKernelDynLDSGlobalFromFunction(const Function &F) {
  ...
  std::string KernelDynLDSName = "llvm.rvgpu.";
...
RVGPUMachineFunction::getLDSKernelIdMetadata(const Function &F) {
  // TODO: Would be more consistent with the abs symbols to use a range
  MDNode *MD = F.getMetadata("llvm.rvgpu.lds.kernel.id");
...
std::optional<uint32_t>
RVGPUMachineFunction::getLDSAbsoluteAddress(const GlobalValue &GV) {
  if (GV.getAddressSpace() != RVGPUAS::LOCAL_ADDRESS)
    return {};
...
void RVGPUMachineFunction::setDynLDSAlign(const Function &F,
                                           const GlobalVariable &GV) {
  const Module *M = F.getParent();
