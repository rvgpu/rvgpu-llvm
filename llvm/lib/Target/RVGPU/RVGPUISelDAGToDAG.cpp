...
//
/// \file
/// Defines an instruction selector for the RVGPU target.
//
//===----------------------------------------------------------------------===//
...
INITIALIZE_PASS_BEGIN(RVGPUDAGToDAGISel, "rvgpu-isel",
                      "RVGPU DAG->DAG Pattern Instruction Selection", false, false)
INITIALIZE_PASS_DEPENDENCY(RVGPUArgumentUsageInfo)
INITIALIZE_PASS_DEPENDENCY(RVGPUPerfHintAnalysis)
INITIALIZE_PASS_DEPENDENCY(UniformityInfoWrapperPass)
#ifdef EXPENSIVE_CHECKS
INITIALIZE_PASS_DEPENDENCY(DominatorTreeWrapperPass)
INITIALIZE_PASS_DEPENDENCY(LoopInfoWrapperPass)
#endif
INITIALIZE_PASS_END(RVGPUDAGToDAGISel, "rvgpu-isel",
                    "RVGPU DAG->DAG Pattern Instruction Selection", false, false)

/// This pass converts a legalized DAG into a RVGPU-specific
// DAG, ready for instruction scheduling.
FunctionPass *llvm::createRVGPUISelDag(TargetMachine &TM,
                                        CodeGenOptLevel OptLevel) {
  return new RVGPUDAGToDAGISel(TM, OptLevel);
}

RVGPUDAGToDAGISel::RVGPUDAGToDAGISel(TargetMachine &TM,
                                       CodeGenOptLevel OptLevel)
    : SelectionDAGISel(ID, TM, OptLevel) {
  EnableLateStructurizeCFG = RVGPUTargetMachine::EnableLateStructurizeCFG;
}
...
#endif
  Subtarget = &MF.getSubtarget<RVSubtarget>();
  Mode = SIModeRegisterDefaults(MF.getFunction(), *Subtarget);
  return SelectionDAGISel::runOnMachineFunction(MF);
}

bool RVGPUDAGToDAGISel::fp16SrcZerosHighBits(unsigned Opc) const {
  ...
  case RVGPUISD::FRACT:
  case RVGPUISD::CLAMP:
  case RVGPUISD::COS_HW:
  case RVGPUISD::SIN_HW:
  case RVGPUISD::FMIN3:
  case RVGPUISD::FMAX3:
  case RVGPUISD::FMED3:
  ...
  case RVGPUISD::RCP_IFLAG:
    ...
    return Subtarget->getGeneration() <= RVGPUSubtarget::GFX9;
  case ISD::FP_ROUND:
    ...
    return Subtarget->getGeneration() == RVGPUSubtarget::VOLCANIC_ISLANDS;
  case ISD::FMA:
  case ISD::FMAD:
...
void RVGPUDAGToDAGISel::getAnalysisUsage(AnalysisUsage &AU) const {
  AU.addRequired<RVGPUArgumentUsageInfo>();
...
bool RVGPUDAGToDAGISel::matchLoadD16FromBuildVector(SDNode *N) const {
  ...
  if (LdHi && Hi.hasOneUse() && !LdHi->isPredecessorOf(Lo.getNode())) {
    ...
    unsigned LoadOp = RVGPUISD::LOAD_D16_HI;
    if (LdHi->getMemoryVT() == MVT::i8) {
      LoadOp = LdHi->getExtensionType() == ISD::SEXTLOAD ?
        RVGPUISD::LOAD_D16_HI_I8 : RVGPUISD::LOAD_D16_HI_U8;
    } else {
  ...
  if (LdLo && Lo.hasOneUse()) {
    ...
    SDVTList VTList = CurDAG->getVTList(VT, MVT::Other);
    unsigned LoadOp = RVGPUISD::LOAD_D16_LO;
    if (LdLo->getMemoryVT() == MVT::i8) {
      LoadOp = LdLo->getExtensionType() == ISD::SEXTLOAD ?
        RVGPUISD::LOAD_D16_LO_I8 : RVGPUISD::LOAD_D16_LO_U8;
    } else {
...
void RVGPUDAGToDAGISel::PreprocessISelDAG() {
...
bool RVGPUDAGToDAGISel::isInlineImmediate(const SDNode *N,
                                           bool Negated) const {
  if (N->isUndef())
    return true;
...
/// determined.
const TargetRegisterClass *RVGPUDAGToDAGISel::getOperandRegClass(SDNode *N,
                                                          unsigned OpNo) const {
  if (!N->isMachineOpcode()) {
    if (N->getOpcode() == ISD::CopyToReg) {
      ...
      const RVRegisterInfo *TRI
        = static_cast<const RVSubtarget *>(Subtarget)->getRegisterInfo();
...
SDNode *RVGPUDAGToDAGISel::glueCopyToOp(SDNode *N, SDValue NewChain,
                                         SDValue Glue) const {
...
SDNode *RVGPUDAGToDAGISel::glueCopyToM0(SDNode *N, SDValue Val) const {
  const RVTargetLowering& Lowering =
    *static_cast<const RVTargetLowering*>(getTargetLowering());
...
SDNode *RVGPUDAGToDAGISel::glueCopyToM0LDSInit(SDNode *N) const {
  ...
  } else if (AS == RVGPUAS::REGION_ADDRESS) {
    MachineFunction &MF = CurDAG->getMachineFunction();
    unsigned Value = MF.getInfo<RVMachineFunctionInfo>()->getGDSSize();
    return
...
MachineSDNode *RVGPUDAGToDAGISel::buildSMovImm64(SDLoc &DL, uint64_t Imm,
                                                  EVT VT) const {
  SDNode *Lo = CurDAG->getMachineNode(
      RVGPU::S_MOV_B32, DL, MVT::i32,
      CurDAG->getTargetConstant(Imm & 0xFFFFFFFF, DL, MVT::i32));
  SDNode *Hi =
      CurDAG->getMachineNode(RVGPU::S_MOV_B32, DL, MVT::i32,
                             CurDAG->getTargetConstant(Imm >> 32, DL, MVT::i32));
  const SDValue Ops[] = {
      CurDAG->getTargetConstant(RVGPU::SReg_64RegClassID, DL, MVT::i32),
      SDValue(Lo, 0), CurDAG->getTargetConstant(RVGPU::sub0, DL, MVT::i32),
      SDValue(Hi, 0), CurDAG->getTargetConstant(RVGPU::sub1, DL, MVT::i32)};

  return CurDAG->getMachineNode(TargetOpcode::REG_SEQUENCE, DL, VT, Ops);
}

void RVGPUDAGToDAGISel::SelectBuildVector(SDNode *N, unsigned RegClassID) {
  ...
  if (NumVectorElts == 1) {
    CurDAG->SelectNodeTo(N, RVGPU::COPY_TO_REGCLASS, EltVT, N->getOperand(0),
                         RegClass);
    return;
  }
  ...
  for (unsigned i = 0; i < NOps; i++) {
    ...
    unsigned Sub = RVRegisterInfo::getSubRegFromChannel(i);
  ...
  if (!IsRegSeq)
    SelectCode(N);
  CurDAG->SelectNodeTo(N, RVGPU::REG_SEQUENCE, N->getVTList(), RegSeqArgs);
}
...
void RVGPUDAGToDAGISel::Select(SDNode *N) {
  ...
  case RVGPUISD::FMUL_W_CHAIN: {
    SelectFMUL_W_CHAIN(N);
    return;
  }
  case RVGPUISD::FMA_W_CHAIN: {
    SelectFMA_W_CHAIN(N);
    return;
  }
  ...
  case ISD::BUILD_VECTOR: {
    ...
    unsigned RegClassID =
        RVRegisterInfo::getSGPRClassForBitWidth(NumVectorElts * 32)->getID();
  ...
  case ISD::BUILD_PAIR: {
    ...
    if (N->getValueType(0) == MVT::i128) {
      RC = CurDAG->getTargetConstant(RVGPU::SGPR_128RegClassID, DL, MVT::i32);
      SubReg0 = CurDAG->getTargetConstant(RVGPU::sub0_sub1, DL, MVT::i32);
      SubReg1 = CurDAG->getTargetConstant(RVGPU::sub2_sub3, DL, MVT::i32);
    } else if (N->getValueType(0) == MVT::i64) {
      RC = CurDAG->getTargetConstant(RVGPU::SReg_64RegClassID, DL, MVT::i32);
      SubReg0 = CurDAG->getTargetConstant(RVGPU::sub0, DL, MVT::i32);
      SubReg1 = CurDAG->getTargetConstant(RVGPU::sub1, DL, MVT::i32);
    } else {
  ...
  case ISD::ConstantFP: {
    ...
    if (ConstantFPSDNode *FP = dyn_cast<ConstantFPSDNode>(N)) {
      ...
      if (RVGPU::isValid32BitLiteral(Imm, true))
        break;
    } else {
      ConstantSDNode *C = cast<ConstantSDNode>(N);
      Imm = C->getZExtValue();
      if (RVGPU::isValid32BitLiteral(Imm, false))
        break;
    }
  ...
  case RVGPUISD::BFE_I32:
  case RVGPUISD::BFE_U32: {
    ...
    bool Signed = Opc == RVGPUISD::BFE_I32;
  ...
  case RVGPUISD::MAD_I64_I32:
  case RVGPUISD::MAD_U64_U32: {
    SelectMAD_64_32(N);
    return;
  }
  case ISD::SMUL_LOHI:
  ...
  case RVGPUISD::CVT_PKNORM_I16_F32:
  case RVGPUISD::CVT_PKNORM_U16_F32:
  case RVGPUISD::CVT_PK_U16_U32:
  case RVGPUISD::CVT_PK_I16_I32: {
    // Hack around using a legal type if f16 is illegal.
    if (N->getValueType(0) == MVT::i32) {
      MVT NewVT = Opc == RVGPUISD::CVT_PKRTZ_F16_F32 ? MVT::v2f16 : MVT::v2i16;
...
bool RVGPUDAGToDAGISel::isUniformBr(const SDNode *N) const {
  const BasicBlock *BB = FuncInfo->MBB->getBasicBlock();
  const Instruction *Term = BB->getTerminator();
  return Term->getMetadata("rvgpu.uniform") ||
         Term->getMetadata("structurizecfg.uniform");
}

bool RVGPUDAGToDAGISel::isUnneededShiftMask(const SDNode *N,
...
bool RVGPUDAGToDAGISel::isBaseWithConstantOffset64(SDValue Addr, SDValue &LHS,
                                                    SDValue &RHS) const {
...
StringRef RVGPUDAGToDAGISel::getPassName() const {
  return "RVGPU DAG->DAG Pattern Instruction Selection";
}
...
bool RVGPUDAGToDAGISel::SelectADDRVTX_READ(SDValue Addr, SDValue &Base,
                                            SDValue &Offset) {
  return false;
}

SDValue RVGPUDAGToDAGISel::getMaterializedScalarImm32(int64_t Val,
...
void RVGPUDAGToDAGISel::SelectADD_SUB_I64(SDNode *N) {
  SDLoc DL(N);
  ...
  SDValue Sub0 = CurDAG->getTargetConstant(RVGPU::sub0, DL, MVT::i32);
  SDValue Sub1 = CurDAG->getTargetConstant(RVGPU::sub1, DL, MVT::i32);
  ...
  static const unsigned OpcMap[2][2][2] = {
      {{RVGPU::S_SUB_U32, RVGPU::S_ADD_U32},
       {RVGPU::V_SUB_CO_U32_e32, RVGPU::V_ADD_CO_U32_e32}},
      {{RVGPU::S_SUBB_U32, RVGPU::S_ADDC_U32},
       {RVGPU::V_SUBB_U32_e32, RVGPU::V_ADDC_U32_e32}}};
  ...
  SDNode *RegSequence = CurDAG->getMachineNode(RVGPU::REG_SEQUENCE, DL,
...
void RVGPUDAGToDAGISel::SelectAddcSubb(SDNode *N) {
  SDLoc DL(N);
  SDValue LHS = N->getOperand(0);
  SDValue RHS = N->getOperand(1);
  ...
  if (N->isDivergent()) {
    unsigned Opc = N->getOpcode() == ISD::UADDO_CARRY ? RVGPU::V_ADDC_U32_e64
    ...
    unsigned Opc = N->getOpcode() == ISD::UADDO_CARRY ? RVGPU::S_ADD_CO_PSEUDO
                                                      : RVGPU::S_SUB_CO_PSEUDO;
    CurDAG->SelectNodeTo(N, Opc, N->getVTList(), {LHS, RHS, CI});
  }
}

void RVGPUDAGToDAGISel::SelectUADDO_USUBO(SDNode *N) {
  ...
  if (IsVALU) {
    unsigned Opc = IsAdd ? RVGPU::V_ADD_CO_U32_e64 : RVGPU::V_SUB_CO_U32_e64;
    ...
    unsigned Opc = N->getOpcode() == ISD::UADDO ? RVGPU::S_UADDO_PSEUDO
                                                : RVGPU::S_USUBO_PSEUDO;
...
void RVGPUDAGToDAGISel::SelectFMA_W_CHAIN(SDNode *N) {
  SDLoc SL(N);
  ...
  unsigned Opcode = UseFMAC ? RVGPU::V_FMAC_F32_e64 : RVGPU::V_FMA_F32_e64;
  CurDAG->SelectNodeTo(N, Opcode, N->getVTList(), Ops);
}

void RVGPUDAGToDAGISel::SelectFMUL_W_CHAIN(SDNode *N) {
  SDLoc SL(N);
  ...
  CurDAG->SelectNodeTo(N, RVGPU::V_MUL_F32_e64, N->getVTList(), Ops);
}

// We need to handle this here because tablegen doesn't support matching
// instructions with multiple outputs.
void RVGPUDAGToDAGISel::SelectDIV_SCALE(SDNode *N) {
  SDLoc SL(N);
  EVT VT = N->getValueType(0);
  ...
  unsigned Opc
    = (VT == MVT::f64) ? RVGPU::V_DIV_SCALE_F64_e64 : RVGPU::V_DIV_SCALE_F32_e64;
...
// We need to handle this here because tablegen doesn't support matching
// instructions with multiple outputs.
void RVGPUDAGToDAGISel::SelectMAD_64_32(SDNode *N) {
  SDLoc SL(N);
...
// We need to handle this here because tablegen doesn't support matching
// instructions with multiple outputs.
void RVGPUDAGToDAGISel::SelectMUL_LOHI(SDNode *N) {
  SDLoc SL(N);
  ...
  if (!SDValue(N, 0).use_empty()) {
    SDValue Sub0 = CurDAG->getTargetConstant(RVGPU::sub0, SL, MVT::i32);
  ...
  if (!SDValue(N, 1).use_empty()) {
    SDValue Sub1 = CurDAG->getTargetConstant(RVGPU::sub1, SL, MVT::i32);
...
bool RVGPUDAGToDAGISel::isDSOffsetLegal(SDValue Base, unsigned Offset) const {
  if (!isUInt<16>(Offset))
    return false;
...
bool RVGPUDAGToDAGISel::SelectDS1Addr1Offset(SDValue Addr, SDValue &Base,
  ...
  if (CurDAG->isBaseWithConstantOffset(Addr)) {
    ...
    if (const ConstantSDNode *C = dyn_cast<ConstantSDNode>(Addr.getOperand(0))) {
      ...
      if (isDSOffsetLegal(SDValue(), ByteOffset)) {
        ...
        if (isDSOffsetLegal(Sub, ByteOffset)) {
          ...
          unsigned SubOp = RVGPU::V_SUB_CO_U32_e32;
          if (Subtarget->hasAddNoCarry()) {
            SubOp = RVGPU::V_SUB_U32_e64;
            Opnds.push_back(
                CurDAG->getTargetConstant(0, {}, MVT::i1)); // clamp bit
          }
    ...
    if (isDSOffsetLegal(SDValue(), CAddr->getZExtValue())) {
      SDValue Zero = CurDAG->getTargetConstant(0, DL, MVT::i32);
      MachineSDNode *MovZero = CurDAG->getMachineNode(RVGPU::V_MOV_B32_e32,
                                 DL, MVT::i32, Zero);
...
bool RVGPUDAGToDAGISel::isDSOffset2Legal(SDValue Base, unsigned Offset0,
                                          unsigned Offset1,
                                          unsigned Size) const {
...
// of: SGPR + VGPR.
bool RVGPUDAGToDAGISel::isFlatScratchBaseLegalSV(SDValue Addr) const {
  if (isNoUnsignedWrap(Addr))
    return true;
...
// Check address value in SGPR/VGPR are legal for flat scratch in the form
// of: SGPR + VGPR + Imm.
bool RVGPUDAGToDAGISel::isFlatScratchBaseLegalSVImm(SDValue Addr) const {
...
bool RVGPUDAGToDAGISel::SelectDS64Bit4ByteAligned(SDValue Addr, SDValue &Base,
                                                   SDValue &Offset0,
                                                   SDValue &Offset1) const {
  return SelectDSReadWrite2(Addr, Base, Offset0, Offset1, 4);
}

bool RVGPUDAGToDAGISel::SelectDS128Bit8ByteAligned(SDValue Addr, SDValue &Base,
                                                    SDValue &Offset0,
                                                    SDValue &Offset1) const {
  return SelectDSReadWrite2(Addr, Base, Offset0, Offset1, 8);
}

bool RVGPUDAGToDAGISel::SelectDSReadWrite2(SDValue Addr, SDValue &Base,
  ...
  if (CurDAG->isBaseWithConstantOffset(Addr)) {
    ...
    if (isDSOffset2Legal(SDValue(), OffsetValue0, OffsetValue1, Size)) {
      ...
      MachineSDNode *MovZero =
          CurDAG->getMachineNode(RVGPU::V_MOV_B32_e32, DL, MVT::i32, Zero);
      Base = SDValue(MovZero, 0);
      Offset0 = CurDAG->getTargetConstant(OffsetValue0 / Size, DL, MVT::i8);
      Offset1 = CurDAG->getTargetConstant(OffsetValue1 / Size, DL, MVT::i8);
      return true;
...
bool RVGPUDAGToDAGISel::SelectMUBUF(SDValue Addr, SDValue &Ptr, SDValue &VAddr,
  ...
  SOffset = Subtarget->hasRestrictedSOffset()
                ? CurDAG->getRegister(RVGPU::SGPR_NULL, MVT::i32)
...
bool RVGPUDAGToDAGISel::SelectMUBUFAddr64(SDValue Addr, SDValue &SRsrc,
...
std::pair<SDValue, SDValue> RVGPUDAGToDAGISel::foldFrameIndex(SDValue N) const {
  SDLoc DL(N);
...
bool RVGPUDAGToDAGISel::SelectMUBUFScratchOffen(SDNode *Parent,
  ...
  if (ConstantSDNode *CAddr = dyn_cast<ConstantSDNode>(Addr)) {
    ...
    const int64_t NullPtr =
        RVGPUTargetMachine::getNullPointerValue(RVGPUAS::PRIVATE_ADDRESS);
    // Don't fold null pointer.
    if (Imm != NullPtr) {
      const uint32_t MaxOffset = RVInstrInfo::getMaxMUBUFImmOffset(*Subtarget);
      SDValue HighBits =
          CurDAG->getTargetConstant(Imm & ~MaxOffset, DL, MVT::i32);
      MachineSDNode *MovHighBits = CurDAG->getMachineNode(
        RVGPU::V_MOV_B32_e32, DL, MVT::i32, HighBits);
...
static bool IsCopyFromSGPR(const RVRegisterInfo &TRI, SDValue Val) {
...
bool RVGPUDAGToDAGISel::SelectMUBUFScratchOffset(SDNode *Parent,
                                                  SDValue Addr,
                                                  SDValue &SRsrc,
                                                  SDValue &SOffset,
                                                  SDValue &Offset) const {
  const RVRegisterInfo *TRI =
      static_cast<const RVRegisterInfo *>(Subtarget->getRegisterInfo());
  const RVInstrInfo *TII = Subtarget->getInstrInfo();
  MachineFunction &MF = CurDAG->getMachineFunction();
  const RVMachineFunctionInfo *Info = MF.getInfo<RVMachineFunctionInfo>();
  SDLoc DL(Addr);
...
bool RVGPUDAGToDAGISel::SelectMUBUFOffset(SDValue Addr, SDValue &SRsrc,
                                           SDValue &SOffset, SDValue &Offset
                                           ) const {
...
bool RVGPUDAGToDAGISel::SelectBUFSOffset(SDValue ByteOffsetNode,
  ...
  if (Subtarget->hasRestrictedSOffset()) {
    if (auto SOffsetConst = dyn_cast<ConstantSDNode>(ByteOffsetNode)) {
      if (SOffsetConst->isZero()) {
        SOffset = CurDAG->getRegister(RVGPU::SGPR_NULL, MVT::i32);
        return true;
      }
    }
  }
...
static MemSDNode* findMemSDNode(SDNode *N) {
  N = RVGPUTargetLowering::stripBitcast(SDValue(N,0)).getNode();
  if (MemSDNode *MN = dyn_cast<MemSDNode>(N))
    return MN;
  assert(isa<BuildVectorSDNode>(N));
  for (SDValue V : N->op_values())
    if (MemSDNode *MN =
          dyn_cast<MemSDNode>(RVGPUTargetLowering::stripBitcast(V)))
      return MN;
  llvm_unreachable("cannot find MemSDNode in the pattern!");
}

bool RVGPUDAGToDAGISel::SelectFlatOffsetImpl(SDNode *N, SDValue Addr,
  ...
  bool CanHaveFlatSegmentOffsetBug =
      ...
      (AS == RVGPUAS::FLAT_ADDRESS || AS == RVGPUAS::GLOBAL_ADDRESS);
    ...
    if (isBaseWithConstantOffset64(Addr, N0, N1) &&
      ...
      if (TII->isLegalFLATOffset(COffsetVal, AS, FlatVariant)) {
        ...
        if (Addr.getValueType().getSizeInBits() == 32) {
          ...
          unsigned AddOp = RVGPU::V_ADD_CO_U32_e32;
          if (Subtarget->hasAddNoCarry()) {
            AddOp = RVGPU::V_ADD_U32_e64;
          ...
          SDNode *Add =
              CurDAG->getMachineNode(RVGPU::V_ADD_CO_U32_e64, DL, VTs,
                                     {AddOffsetLo, SDValue(N0Lo, 0), Clamp});

          SDNode *Addc = CurDAG->getMachineNode(
              RVGPU::V_ADDC_U32_e64, DL, VTs,
              {AddOffsetHi, SDValue(N0Hi, 0), SDValue(Add, 1), Clamp});

          SDValue RegSequenceArgs[] = {
              CurDAG->getTargetConstant(RVGPU::VReg_64RegClassID, DL, MVT::i32),
              SDValue(Add, 0), Sub0, SDValue(Addc, 0), Sub1};

          Addr = SDValue(CurDAG->getMachineNode(RVGPU::REG_SEQUENCE, DL,
                                                MVT::i64, RegSequenceArgs),
                         0);
        }
      }
...
bool RVGPUDAGToDAGISel::SelectFlatOffset(SDNode *N, SDValue Addr,
                                          SDValue &VAddr,
                                          SDValue &Offset) const {
  return SelectFlatOffsetImpl(N, Addr, VAddr, Offset, SIInstrFlags::FLAT);
}

bool RVGPUDAGToDAGISel::SelectGlobalOffset(SDNode *N, SDValue Addr,
                                            SDValue &VAddr,
                                            SDValue &Offset) const {
  return SelectFlatOffsetImpl(N, Addr, VAddr, Offset, SIInstrFlags::FlatGlobal);
}

bool RVGPUDAGToDAGISel::SelectScratchOffset(SDNode *N, SDValue Addr,
                                             SDValue &VAddr,
                                             SDValue &Offset) const {
...
bool RVGPUDAGToDAGISel::SelectGlobalSAddr(SDNode *N,
                                           SDValue Addr,
                                           SDValue &SAddr,
                                           SDValue &VOffset,
                                           SDValue &Offset) const {
  ...
  if (isBaseWithConstantOffset64(Addr, LHS, RHS)) {
    ...
    if (TII->isLegalFLATOffset(COffsetVal, RVGPUAS::GLOBAL_ADDRESS,
      ...
      if (COffsetVal > 0) {
        ...
        std::tie(SplitImmOffset, RemainderOffset) = TII->splitFlatOffset(
            COffsetVal, RVGPUAS::GLOBAL_ADDRESS, SIInstrFlags::FlatGlobal);

        if (isUInt<32>(RemainderOffset)) {
          SDNode *VMov = CurDAG->getMachineNode(
              RVGPU::V_MOV_B32_e32, SL, MVT::i32,
      ...
      if (Subtarget->getConstantBusLimit(RVGPU::V_ADD_U32_e64) > NumLiterals)
        return false;
    }
  }
  ...
  SAddr = Addr;
  SDNode *VMov =
      CurDAG->getMachineNode(RVGPU::V_MOV_B32_e32, SDLoc(Addr), MVT::i32,
...
static SDValue SelectSAddrFI(SelectionDAG *CurDAG, SDValue SAddr) {
  if (auto FI = dyn_cast<FrameIndexSDNode>(SAddr)) {
    ...
    SDValue TFI = CurDAG->getTargetFrameIndex(FI->getIndex(),
                                              FI->getValueType(0));
    SAddr = SDValue(CurDAG->getMachineNode(RVGPU::S_ADD_I32, SDLoc(SAddr),
...
bool RVGPUDAGToDAGISel::SelectScratchSAddr(SDNode *Parent, SDValue Addr,
                                            SDValue &SAddr,
                                            SDValue &Offset) const {
  if (Addr->isDivergent())
    return false;
  ...
  if (!TII->isLegalFLATOffset(COffsetVal, RVGPUAS::PRIVATE_ADDRESS,
                              SIInstrFlags::FlatScratch)) {
    int64_t SplitImmOffset, RemainderOffset;
    std::tie(SplitImmOffset, RemainderOffset) = TII->splitFlatOffset(
        COffsetVal, RVGPUAS::PRIVATE_ADDRESS, SIInstrFlags::FlatScratch);
    ...
    SAddr = SDValue(CurDAG->getMachineNode(RVGPU::S_ADD_I32, DL, MVT::i32,
                                           SAddr, AddOffset),
                    0);
  }
...
bool RVGPUDAGToDAGISel::checkFlatScratchSVSSwizzleBug(
...
bool RVGPUDAGToDAGISel::SelectScratchSVAddr(SDNode *N, SDValue Addr,
  ...
  if (isBaseWithConstantOffset64(Addr, LHS, RHS)) {
    ...
    if (TII->isLegalFLATOffset(COffsetVal, RVGPUAS::PRIVATE_ADDRESS, true)) {
      Addr = LHS;
      ImmOffset = COffsetVal;
      ...
      std::tie(SplitImmOffset, RemainderOffset)
        = TII->splitFlatOffset(COffsetVal, RVGPUAS::PRIVATE_ADDRESS, true);
...
bool RVGPUDAGToDAGISel::SelectSMRDOffset(SDValue ByteOffsetNode,
                                          SDValue *SOffset, SDValue *Offset,
  ...
  std::optional<int64_t> EncodedOffset =
      RVGPU::getSMRDEncodedOffset(*Subtarget, ByteOffset, IsBuffer);
  ...
  EncodedOffset = RVGPU::getSMRDEncodedLiteralOffset32(*Subtarget, ByteOffset);
  if (EncodedOffset && Offset && Imm32Only) {
    *Offset = CurDAG->getTargetConstant(*EncodedOffset, SL, MVT::i32);
    return true;
  }
  ...
  if (SOffset) {
    ...
    *SOffset = SDValue(
        CurDAG->getMachineNode(RVGPU::S_MOV_B32, SL, MVT::i32, C32Bit), 0);
    return true;
  }
...
SDValue RVGPUDAGToDAGISel::Expand32BitAddress(SDValue Addr) const {
  ...
  const SDValue Ops[] = {
    CurDAG->getTargetConstant(RVGPU::SReg_64_XEXECRegClassID, SL, MVT::i32),
    Addr,
    CurDAG->getTargetConstant(RVGPU::sub0, SL, MVT::i32),
    SDValue(CurDAG->getMachineNode(RVGPU::S_MOV_B32, SL, MVT::i32, AddrHi),
            0),
    CurDAG->getTargetConstant(RVGPU::sub1, SL, MVT::i32),
  };

  return SDValue(CurDAG->getMachineNode(RVGPU::REG_SEQUENCE, SL, MVT::i64,
                                        Ops), 0);
}
...
bool RVGPUDAGToDAGISel::SelectSMRDBaseOffset(SDValue Addr, SDValue &SBase,
                                              SDValue *SOffset, SDValue *Offset,
                                              bool Imm32Only,
                                              bool IsBuffer) const {
  if (SOffset && Offset) {
...
bool RVGPUDAGToDAGISel::SelectSMRD(SDValue Addr, SDValue &SBase,
                                    SDValue *SOffset, SDValue *Offset,
...
bool RVGPUDAGToDAGISel::SelectSMRDImm(SDValue Addr, SDValue &SBase,
                                       SDValue &Offset) const {
  return SelectSMRD(Addr, SBase, /* SOffset */ nullptr, &Offset);
}

bool RVGPUDAGToDAGISel::SelectSMRDImm32(SDValue Addr, SDValue &SBase,
                                         SDValue &Offset) const {
  assert(Subtarget->getGeneration() == RVGPUSubtarget::SEA_ISLANDS);
  return SelectSMRD(Addr, SBase, /* SOffset */ nullptr, &Offset,
                    /* Imm32Only */ true);
}

bool RVGPUDAGToDAGISel::SelectSMRDSgpr(SDValue Addr, SDValue &SBase,
                                        SDValue &SOffset) const {
  return SelectSMRD(Addr, SBase, &SOffset, /* Offset */ nullptr);
}

bool RVGPUDAGToDAGISel::SelectSMRDSgprImm(SDValue Addr, SDValue &SBase,
                                           SDValue &SOffset,
                                           SDValue &Offset) const {
  return SelectSMRD(Addr, SBase, &SOffset, &Offset);
}

bool RVGPUDAGToDAGISel::SelectSMRDBufferImm(SDValue N, SDValue &Offset) const {
  return SelectSMRDOffset(N, /* SOffset */ nullptr, &Offset,
                          /* Imm32Only */ false, /* IsBuffer */ true);
}

bool RVGPUDAGToDAGISel::SelectSMRDBufferImm32(SDValue N,
                                               SDValue &Offset) const {
  assert(Subtarget->getGeneration() == RVGPUSubtarget::SEA_ISLANDS);
  return SelectSMRDOffset(N, /* SOffset */ nullptr, &Offset,
...
bool RVGPUDAGToDAGISel::SelectSMRDBufferSgprImm(SDValue N, SDValue &SOffset,
                                                 SDValue &Offset) const {
...
bool RVGPUDAGToDAGISel::SelectMOVRELOffset(SDValue Index,
                                            SDValue &Base,
                                            SDValue &Offset) const {
  SDLoc DL(Index);
...
SDNode *RVGPUDAGToDAGISel::getBFE32(bool IsSigned, const SDLoc &DL,
                                     SDValue Val, uint32_t Offset,
                                     uint32_t Width) {
  if (Val->isDivergent()) {
    unsigned Opcode = IsSigned ? RVGPU::V_BFE_I32_e64 : RVGPU::V_BFE_U32_e64;
  ...
  unsigned Opcode = IsSigned ? RVGPU::S_BFE_I32 : RVGPU::S_BFE_U32;
...
void RVGPUDAGToDAGISel::SelectS_BFEFromShifts(SDNode *N) {
...
void RVGPUDAGToDAGISel::SelectS_BFE(SDNode *N) {
  switch (N->getOpcode()) {
  case ISD::AND:
...
bool RVGPUDAGToDAGISel::isCBranchSCC(const SDNode *N) const {
  ...
  if (VT == MVT::i64) {
    auto ST = static_cast<const RVSubtarget *>(Subtarget);
...
static SDValue combineBallotPattern(SDValue VCMP, bool &Negate) {
  assert(VCMP->getOpcode() == RVGPUISD::SETCC);
  // Special case for rvgpu.ballot:
  // %Cond = i1 (and/or combination of i1 ISD::SETCCs)
  // %VCMP = i(WaveSize) RVGPUISD::SETCC (ext %Cond), 0, setne/seteq
  // =>
...
void RVGPUDAGToDAGISel::SelectBRCOND(SDNode *N) {
  SDValue Cond = N->getOperand(1);

  if (Cond.isUndef()) {
    CurDAG->SelectNodeTo(N, RVGPU::SI_BR_UNDEF, MVT::Other,
                         N->getOperand(2), N->getOperand(0));
    return;
  }

  const RVSubtarget *ST = static_cast<const RVSubtarget *>(Subtarget);
  ...
  if (Cond.getOpcode() == ISD::SETCC &&
      Cond->getOperand(0)->getOpcode() == RVGPUISD::SETCC) {
    ...
    if ((CC == ISD::SETEQ || CC == ISD::SETNE) &&
      ...
      // %VCMP = i(WaveSize) RVGPUISD::SETCC ...
      ...
      if (auto BallotCond = combineBallotPattern(VCMP, NegatedBallot)) {
        ...
        // TODO: don't use SCC here assuming that RVGPUISD::SETCC is always
        // selected as V_CMP, but this may change for uniform condition.
        Cond = VCMP;
        UseSCCBr = false;
      }
    }
    // Cond is either V_CMP resulted from RVGPUISD::SETCC or a combination of
  ...
  unsigned BrOp =
      UseSCCBr ? (Negate ? RVGPU::S_CBRANCH_SCC0 : RVGPU::S_CBRANCH_SCC1)
               : (Negate ? RVGPU::S_CBRANCH_VCCZ : RVGPU::S_CBRANCH_VCCNZ);
  Register CondReg = UseSCCBr ? RVGPU::SCC : TRI->getVCC();
  SDLoc SL(N);
  ...
  if (AndExec) {
    ...
    // RVInstrInfo::moveToVALU which inserts the S_AND).
    //
    ...
    // catches both cases.
    Cond = SDValue(CurDAG->getMachineNode(ST->isWave32() ? RVGPU::S_AND_B32
                                                         : RVGPU::S_AND_B64,
                     SL, MVT::i1,
                     CurDAG->getRegister(ST->isWave32() ? RVGPU::EXEC_LO
                                                        : RVGPU::EXEC,
                                         MVT::i1),
                    Cond),
                   0);
...
void RVGPUDAGToDAGISel::SelectFP_EXTEND(SDNode *N) {
  if (Subtarget->hasSALUFloatInsts() && N->getValueType(0) == MVT::f32 &&
      !N->isDivergent()) {
    SDValue Src = N->getOperand(0);
    if (Src.getValueType() == MVT::f16) {
      if (isExtractHiElt(Src, Src)) {
        CurDAG->SelectNodeTo(N, RVGPU::S_CVT_HI_F32_F16, N->getVTList(),
                             {Src});
        return;
      }
    }
...
void RVGPUDAGToDAGISel::SelectDSAppendConsume(SDNode *N, unsigned IntrID) {
  // The address is assumed to be uniform, so if it ends up in a VGPR, it will
  // be copied to an SGPR with readfirstlane.
  unsigned Opc = IntrID == Intrinsic::rvgpu_ds_append ?
    RVGPU::DS_APPEND : RVGPU::DS_CONSUME;

  SDValue Chain = N->getOperand(0);
  SDValue Ptr = N->getOperand(2);
  MemIntrinsicSDNode *M = cast<MemIntrinsicSDNode>(N);
  MachineMemOperand *MMO = M->getMemOperand();
  bool IsGDS = M->getAddressSpace() == RVGPUAS::REGION_ADDRESS;
...
// We need to handle this here because tablegen doesn't support matching
// instructions with multiple outputs.
void RVGPUDAGToDAGISel::SelectDSBvhStackIntrinsic(SDNode *N) {
  unsigned Opc = RVGPU::DS_BVH_STACK_RTN_B32;
...
static unsigned gwsIntrinToOpcode(unsigned IntrID) {
  ...
  case Intrinsic::rvgpu_ds_gws_init:
    return RVGPU::DS_GWS_INIT;
  case Intrinsic::rvgpu_ds_gws_barrier:
    return RVGPU::DS_GWS_BARRIER;
  case Intrinsic::rvgpu_ds_gws_sema_v:
    return RVGPU::DS_GWS_SEMA_V;
  case Intrinsic::rvgpu_ds_gws_sema_br:
    return RVGPU::DS_GWS_SEMA_BR;
  case Intrinsic::rvgpu_ds_gws_sema_p:
    return RVGPU::DS_GWS_SEMA_P;
  case Intrinsic::rvgpu_ds_gws_sema_release_all:
    return RVGPU::DS_GWS_SEMA_RELEASE_ALL;
  default:
    llvm_unreachable("not a gws intrinsic");
  }
}

void RVGPUDAGToDAGISel::SelectDS_GWS(SDNode *N, unsigned IntrID) {
  if (!Subtarget->hasGWS() ||
      (IntrID == Intrinsic::rvgpu_ds_gws_sema_release_all &&
  ...
  if (ConstantSDNode *ConstOffset = dyn_cast<ConstantSDNode>(BaseOffset)) {
    ...
    // later.
    SDNode *SGPROffset
      = CurDAG->getMachineNode(RVGPU::V_READFIRSTLANE_B32, SL, MVT::i32,
                               BaseOffset);
    // Shift to offset in m0
    SDNode *M0Base
      = CurDAG->getMachineNode(RVGPU::S_LSHL_B32, SL, MVT::i32,
...
void RVGPUDAGToDAGISel::SelectInterpP1F16(SDNode *N) {
  ...
  // 16 bank LDS
  SDValue ToM0 = CurDAG->getCopyToReg(CurDAG->getEntryNode(), DL, RVGPU::M0,
                                      N->getOperand(5), SDValue());

  SDVTList VTs = CurDAG->getVTList(MVT::f32, MVT::Other);

  SDNode *InterpMov =
    CurDAG->getMachineNode(RVGPU::V_INTERP_MOV_F32, DL, VTs, {
  ...
  SDNode *InterpP1LV =
    CurDAG->getMachineNode(RVGPU::V_INTERP_P1LV_F16, DL, MVT::f32, {
...
void RVGPUDAGToDAGISel::SelectINTRINSIC_W_CHAIN(SDNode *N) {
...
void RVGPUDAGToDAGISel::SelectINTRINSIC_WO_CHAIN(SDNode *N) {
  ...
  case Intrinsic::rvgpu_inverse_ballot:
    ...
    case 32:
      Opcode = RVGPU::S_INVERSE_BALLOT_U32;
      break;
    case 64:
      Opcode = RVGPU::S_INVERSE_BALLOT_U64;
      break;
    default:
...
void RVGPUDAGToDAGISel::SelectINTRINSIC_VOID(SDNode *N) {
  unsigned IntrID = cast<ConstantSDNode>(N->getOperand(1))->getZExtValue();
  switch (IntrID) {
  case Intrinsic::rvgpu_ds_gws_init:
  case Intrinsic::rvgpu_ds_gws_barrier:
...
void RVGPUDAGToDAGISel::SelectWAVE_ADDRESS(SDNode *N) {
  SDValue Log2WaveSize =
    CurDAG->getTargetConstant(Subtarget->getWavefrontSizeLog2(), SDLoc(N), MVT::i32);
  CurDAG->SelectNodeTo(N, RVGPU::S_LSHR_B32, N->getVTList(),
                       {N->getOperand(0), Log2WaveSize});
}

void RVGPUDAGToDAGISel::SelectSTACKRESTORE(SDNode *N) {
  ...
  if (SrcVal.getOpcode() == RVGPUISD::WAVE_ADDRESS) {
    ...
    if (N->isDivergent()) {
      SrcVal = SDValue(CurDAG->getMachineNode(RVGPU::V_READFIRSTLANE_B32, SL,
                                              MVT::i32, SrcVal),
                       0);
    }

    CopyVal = SDValue(CurDAG->getMachineNode(RVGPU::S_LSHL_B32, SL, MVT::i32,
                                             {SrcVal, Log2WaveSize}),
                      0);
  }
...
bool RVGPUDAGToDAGISel::SelectVOP3ModsImpl(SDValue In, SDValue &Src,
                                            unsigned &Mods,
...
bool RVGPUDAGToDAGISel::SelectVOP3Mods(SDValue In, SDValue &Src,
...
bool RVGPUDAGToDAGISel::SelectVOP3ModsNonCanonicalizing(
...
bool RVGPUDAGToDAGISel::SelectVOP3BMods(SDValue In, SDValue &Src,
                                         SDValue &SrcMods) const {
  unsigned Mods;
...
bool RVGPUDAGToDAGISel::SelectVOP3NoMods(SDValue In, SDValue &Src) const {
...
bool RVGPUDAGToDAGISel::SelectVINTERPModsImpl(SDValue In, SDValue &Src,
                                               SDValue &SrcMods,
                                               bool OpSel) const {
  unsigned Mods;
  if (SelectVOP3ModsImpl(In, Src, Mods,
...
bool RVGPUDAGToDAGISel::SelectVINTERPMods(SDValue In, SDValue &Src,
                                           SDValue &SrcMods) const {
  return SelectVINTERPModsImpl(In, Src, SrcMods, /* OpSel */ false);
}

bool RVGPUDAGToDAGISel::SelectVINTERPModsHi(SDValue In, SDValue &Src,
                                             SDValue &SrcMods) const {
  return SelectVINTERPModsImpl(In, Src, SrcMods, /* OpSel */ true);
}

bool RVGPUDAGToDAGISel::SelectVOP3Mods0(SDValue In, SDValue &Src,
...
bool RVGPUDAGToDAGISel::SelectVOP3BMods0(SDValue In, SDValue &Src,
                                          SDValue &SrcMods, SDValue &Clamp,
                                          SDValue &Omod) const {
  SDLoc DL(In);
  Clamp = CurDAG->getTargetConstant(0, DL, MVT::i1);
...
bool RVGPUDAGToDAGISel::SelectVOP3OMods(SDValue In, SDValue &Src,
...
bool RVGPUDAGToDAGISel::SelectVOP3PMods(SDValue In, SDValue &Src,
  ...
  if (Src.getOpcode() == ISD::BUILD_VECTOR && Src.getNumOperands() == 2 &&
    ...
    if (Lo.getValueSizeInBits() > VecSize) {
      Lo = CurDAG->getTargetExtractSubreg(
        (VecSize > 32) ? RVGPU::sub0_sub1 : RVGPU::sub0, SDLoc(In),
      ...
      if (VecSize == 32 || VecSize == Lo.getValueSizeInBits()) {
        ...
        SDValue Undef = SDValue(
          CurDAG->getMachineNode(TargetOpcode::IMPLICIT_DEF, SL,
                                 Lo.getValueType()), 0);
        auto RC = Lo->isDivergent() ? RVGPU::VReg_64RegClassID
                                    : RVGPU::SReg_64RegClassID;
        const SDValue Ops[] = {
          CurDAG->getTargetConstant(RC, SL, MVT::i32),
          Lo, CurDAG->getTargetConstant(RVGPU::sub0, SL, MVT::i32),
          Undef, CurDAG->getTargetConstant(RVGPU::sub1, SL, MVT::i32) };
    ...
    if (VecSize == 64 && Lo == Hi && isa<ConstantFPSDNode>(Lo)) {
      ...
      if (RVGPU::isInlinableLiteral32(Lit, Subtarget->hasInv2PiInlineImm())) {
...
bool RVGPUDAGToDAGISel::SelectVOP3PModsDOT(SDValue In, SDValue &Src,
                                            SDValue &SrcMods) const {
  return SelectVOP3PMods(In, Src, SrcMods, true);
}

bool RVGPUDAGToDAGISel::SelectDotIUVOP3PMods(SDValue In, SDValue &Src) const {
...
bool RVGPUDAGToDAGISel::SelectWMMAOpSelVOP3PMods(SDValue In,
                                                  SDValue &Src) const {
  const ConstantSDNode *C = cast<ConstantSDNode>(In);
  assert(C->getAPIntValue().getBitWidth() == 1 && "expected i1 value");
...
bool RVGPUDAGToDAGISel::SelectVOP3OpSel(SDValue In, SDValue &Src,
                                         SDValue &SrcMods) const {
  Src = In;
  // FIXME: Handle op_sel
...
bool RVGPUDAGToDAGISel::SelectVOP3OpSelMods(SDValue In, SDValue &Src,
                                             SDValue &SrcMods) const {
  // FIXME: Handle op_sel
  return SelectVOP3Mods(In, Src, SrcMods);
}
...
bool RVGPUDAGToDAGISel::SelectVOP3PMadMixModsImpl(SDValue In, SDValue &Src,
                                                   unsigned &Mods) const {
  Mods = 0;
...
bool RVGPUDAGToDAGISel::SelectVOP3PMadMixModsExt(SDValue In, SDValue &Src,
                                                  SDValue &SrcMods) const {
  unsigned Mods = 0;
...
bool RVGPUDAGToDAGISel::SelectVOP3PMadMixMods(SDValue In, SDValue &Src,
                                               SDValue &SrcMods) const {
  unsigned Mods = 0;
...
SDValue RVGPUDAGToDAGISel::getHi16Elt(SDValue In) const {
  if (In.isUndef())
...
bool RVGPUDAGToDAGISel::isVGPRImm(const SDNode * N) const {
  assert(CurDAG->getTarget().getTargetTriple().getArch() == Triple::rvgpu);

  const RVRegisterInfo *SIRI =
    static_cast<const RVRegisterInfo *>(Subtarget->getRegisterInfo());
  const RVInstrInfo * SII =
    static_cast<const RVInstrInfo *>(Subtarget->getInstrInfo());
  ...
  for (SDNode::use_iterator U = N->use_begin(), E = SDNode::use_end();
    ...
    if (RC != &RVGPU::VS_32RegClass && RC != &RVGPU::VS_64RegClass) {
      ...
      if (User->isMachineOpcode()) {
        ...
        if (Desc.isCommutable()) {
          ...
          if (SII->findCommutedOpIndices(Desc, OpIdx, CommuteIdx1)) {
            ...
            if (CommutedRC == &RVGPU::VS_32RegClass ||
                CommutedRC == &RVGPU::VS_64RegClass)
...
bool RVGPUDAGToDAGISel::isUniformLoad(const SDNode * N) const {
  auto Ld = cast<LoadSDNode>(N);

  if (N->isDivergent() && !RVGPUInstrInfo::isUniformMMO(Ld->getMemOperand()))
    return false;

  return Ld->getAlign() >= Align(4) &&
         ((Ld->getAddressSpace() == RVGPUAS::CONSTANT_ADDRESS ||
           Ld->getAddressSpace() == RVGPUAS::CONSTANT_ADDRESS_32BIT) ||
          (Subtarget->getScalarizeGlobalBehavior() &&
           Ld->getAddressSpace() == RVGPUAS::GLOBAL_ADDRESS &&
           Ld->isSimple() &&
           static_cast<const RVTargetLowering *>(getTargetLowering())
               ->isMemOpHasNoClobberedMemOperand(N)));
}

void RVGPUDAGToDAGISel::PostprocessISelDAG() {
  const RVGPUTargetLowering& Lowering =
    *static_cast<const RVGPUTargetLowering*>(getTargetLowering());
