...
static cl::opt<unsigned, false, MFMAPaddingRatioParser>
    MFMAPaddingRatio("rvgpu-mfma-padding-ratio", cl::init(0), cl::Hidden,
...
GCNHazardRecognizer::GCNHazardRecognizer(const MachineFunction &MF) :
  ...
  MaxLookAhead = MF.getRegInfo().isPhysRegUsed(RVGPU::AGPR0) ? 19 : 5;
  TSchedModel.init(&ST);
...
static bool isDivFMas(unsigned Opcode) {
  return Opcode == RVGPU::V_DIV_FMAS_F32_e64 || Opcode == RVGPU::V_DIV_FMAS_F64_e64;
}

static bool isSGetReg(unsigned Opcode) {
  return Opcode == RVGPU::S_GETREG_B32;
}

static bool isSSetReg(unsigned Opcode) {
  switch (Opcode) {
  case RVGPU::S_SETREG_B32:
  case RVGPU::S_SETREG_B32_mode:
  case RVGPU::S_SETREG_IMM32_B32:
  case RVGPU::S_SETREG_IMM32_B32_mode:
    return true;
  }
  return false;
}
...
static bool isRWLane(unsigned Opcode) {
  return Opcode == RVGPU::V_READLANE_B32 || Opcode == RVGPU::V_WRITELANE_B32;
}

static bool isRFE(unsigned Opcode) {
  return Opcode == RVGPU::S_RFE_B64;
}

static bool isSMovRel(unsigned Opcode) {
  switch (Opcode) {
  case RVGPU::S_MOVRELS_B32:
  case RVGPU::S_MOVRELS_B64:
  case RVGPU::S_MOVRELD_B32:
  case RVGPU::S_MOVRELD_B64:
    return true;
  default:
    return false;
  }
...
static bool isDGEMM(unsigned Opcode) {
  return RVGPU::getMAIIsDGEMM(Opcode);
}

static bool isXDL(const RVSubtarget &ST, const MachineInstr &MI) {
  unsigned Opcode = MI.getOpcode();

  if (!RVInstrInfo::isMAI(MI) ||
      isDGEMM(Opcode) ||
  ...
  return RVGPU::getMAIIsGFX940XDL(Opcode);
}

static bool isSendMsgTraceDataOrGDS(const RVInstrInfo &TII,
                                    const MachineInstr &MI) {
  if (TII.isAlwaysGDS(MI.getOpcode()))
    return true;

  switch (MI.getOpcode()) {
  case RVGPU::S_SENDMSG:
  case RVGPU::S_SENDMSGHALT:
  case RVGPU::S_TTRACEDATA:
    return true;
  // These DS opcodes don't support GDS.
  case RVGPU::DS_NOP:
  case RVGPU::DS_PERMUTE_B32:
  case RVGPU::DS_BPERMUTE_B32:
    return false;
  default:
    if (TII.isDS(MI.getOpcode())) {
      int GDS = RVGPU::getNamedOperandIdx(MI.getOpcode(),
                                           RVGPU::OpName::gds);
...
static bool isPermlane(const MachineInstr &MI) {
  unsigned Opcode = MI.getOpcode();
  return Opcode == RVGPU::V_PERMLANE16_B32_e64 ||
         Opcode == RVGPU::V_PERMLANEX16_B32_e64 ||
         Opcode == RVGPU::V_PERMLANE16_VAR_B32_e64 ||
         Opcode == RVGPU::V_PERMLANEX16_VAR_B32_e64;
}

static bool isLdsDma(const MachineInstr &MI) {
  return RVInstrInfo::isVALU(MI) &&
         (RVInstrInfo::isMUBUF(MI) || RVInstrInfo::isFLAT(MI));
}

static unsigned getHWReg(const RVInstrInfo *TII, const MachineInstr &RegInstr) {
  const MachineOperand *RegOp = TII->getNamedOperand(RegInstr,
                                                     RVGPU::OpName::simm16);
  return RegOp->getImm() & RVGPU::Hwreg::ID_MASK_;
}
...
GCNHazardRecognizer::getHazardType(SUnit *SU, int Stalls) {
  ...
  if (RVInstrInfo::isSMRD(*MI) && checkSMRDHazards(MI) > 0)
    return HazardType;
  ...
  if ((RVInstrInfo::isVMEM(*MI) ||
       RVInstrInfo::isFLAT(*MI))
      && checkVMEMHazards(MI) > 0)
    return HazardType;

  if (RVInstrInfo::isVALU(*MI) && checkVALUHazards(MI) > 0)
    return HazardType;

  if (RVInstrInfo::isDPP(*MI) && checkDPPHazards(MI) > 0)
    return HazardType;
  ...
  if ((RVInstrInfo::isVALU(*MI) || RVInstrInfo::isVMEM(*MI) ||
       RVInstrInfo::isFLAT(*MI) || RVInstrInfo::isDS(*MI) ||
       RVInstrInfo::isEXP(*MI)) && checkMAIVALUHazards(MI) > 0)
    return HazardType;
  ...
  if (((ST.hasReadM0MovRelInterpHazard() &&
        (TII.isVINTRP(*MI) || isSMovRel(MI->getOpcode()) ||
         MI->getOpcode() == RVGPU::DS_WRITE_ADDTID_B32 ||
         MI->getOpcode() == RVGPU::DS_READ_ADDTID_B32)) ||
       (ST.hasReadM0SendMsgHazard() && isSendMsgTraceDataOrGDS(TII, *MI)) ||
       (ST.hasReadM0LdsDmaHazard() && isLdsDma(*MI)) ||
       (ST.hasReadM0LdsDirectHazard() &&
        MI->readsRegister(RVGPU::LDS_DIRECT))) &&
      checkReadM0Hazards(MI) > 0)
    return HazardType;

  if (RVInstrInfo::isMAI(*MI) && checkMAIHazards(MI) > 0)
    return HazardType;

  if ((RVInstrInfo::isVMEM(*MI) ||
       RVInstrInfo::isFLAT(*MI) ||
       RVInstrInfo::isDS(*MI)) && checkMAILdStHazards(MI) > 0)
    return HazardType;
...
static void insertNoopsInBundle(MachineInstr *MI, const RVInstrInfo &TII,
                                unsigned Quantity) {
  while (Quantity > 0) {
    unsigned Arg = std::min(Quantity, 8u);
    Quantity -= Arg;
    BuildMI(*MI->getParent(), MI, MI->getDebugLoc(), TII.get(RVGPU::S_NOP))
        .addImm(Arg - 1);
  }
}
...
unsigned GCNHazardRecognizer::PreEmitNoopsCommon(MachineInstr *MI) {
  ...
  if (RVInstrInfo::isVMEM(*MI) || RVInstrInfo::isFLAT(*MI))
    WaitStates = std::max(WaitStates, checkVMEMHazards(MI));

  if (RVInstrInfo::isVALU(*MI))
    WaitStates = std::max(WaitStates, checkVALUHazards(MI));

  if (RVInstrInfo::isDPP(*MI))
  ...
  if ((ST.hasReadM0MovRelInterpHazard() &&
      ...
      (ST.hasReadM0SendMsgHazard() && isSendMsgTraceDataOrGDS(TII, *MI)) ||
      (ST.hasReadM0LdsDmaHazard() && isLdsDma(*MI)) ||
      (ST.hasReadM0LdsDirectHazard() && MI->readsRegister(RVGPU::LDS_DIRECT)))
...
static int getWaitStatesSince(
    ...
    GetNumWaitStatesFn GetNumWaitStates = RVInstrInfo::getNumWaitStates) {
  for (auto E = MBB->instr_rend(); I != E; ++I) {
...
static void addRegUnits(const RVRegisterInfo &TRI, BitVector &BV,
                        MCRegister Reg) {
  for (MCRegUnit Unit : TRI.regunits(Reg))
    BV.set(Unit);
}

static void addRegsToSet(const RVRegisterInfo &TRI,
...
static bool breaksSMEMSoftClause(MachineInstr *MI) {
  return !RVInstrInfo::isSMRD(*MI);
}

static bool breaksVMEMSoftClause(MachineInstr *MI) {
  return !RVInstrInfo::isVMEM(*MI) && !RVInstrInfo::isFLAT(*MI);
}
...
int GCNHazardRecognizer::checkDPPHazards(MachineInstr *DPP) {
  ...
  WaitStatesNeeded = std::max(
      WaitStatesNeeded,
      DppExecWaitStates - getWaitStatesSinceDef(RVGPU::EXEC, IsHazardDefFn,
                                                DppExecWaitStates));
...
int GCNHazardRecognizer::checkDivFMasHazards(MachineInstr *DivFMas) {
  ...
  int WaitStatesNeeded = getWaitStatesSinceDef(RVGPU::VCC, IsHazardDefFn,
                                               DivFMasWaitStates);
...
int GCNHazardRecognizer::createsVALUHazard(const MachineInstr &MI) {
  ...
  int VDataIdx = RVGPU::getNamedOperandIdx(Opcode, RVGPU::OpName::vdata);
  int VDataRCID = -1;
  if (VDataIdx != -1)
  ...
  if (TII->isMUBUF(MI) || TII->isMTBUF(MI)) {
    ...
    const MachineOperand *SOffset =
        TII->getNamedOperand(MI, RVGPU::OpName::soffset);
    // If we have no soffset operand, then assume this field has been
    // hardcoded to zero.
    if (RVGPU::getRegBitWidth(VDataRCID) > 64 &&
  ...
  if (TII->isMIMG(MI)) {
    int SRsrcIdx = RVGPU::getNamedOperandIdx(Opcode, RVGPU::OpName::srsrc);
    assert(SRsrcIdx != -1 &&
           RVGPU::getRegBitWidth(Desc.operands()[SRsrcIdx].RegClass) == 256);
    (void)SRsrcIdx;
  }

  if (TII->isFLAT(MI)) {
    int DataIdx = RVGPU::getNamedOperandIdx(Opcode, RVGPU::OpName::vdata);
    if (RVGPU::getRegBitWidth(Desc.operands()[DataIdx].RegClass) > 64)
      return DataIdx;
  }
...
int GCNHazardRecognizer::checkVALUHazards(MachineInstr *VALU) {
  ...
  if (ST.hasTransForwardingHazard() && !RVInstrInfo::isTRANS(*VALU)) {
    const int TransDefWaitstates = 1;

    auto IsTransDefFn = [this, VALU](const MachineInstr &MI) {
      if (!RVInstrInfo::isTRANS(MI))
        return false;
      const RVRegisterInfo *TRI = ST.getRegisterInfo();
      const RVInstrInfo *TII = ST.getInstrInfo();
      Register Def = TII->getNamedOperand(MI, RVGPU::OpName::vdst)->getReg();
  ...
  if (ST.hasDstSelForwardingHazard()) {
    ...
    auto IsShift16BitDefFn = [this, VALU](const MachineInstr &MI) {
      if (!RVInstrInfo::isVALU(MI))
        return false;
      const RVInstrInfo *TII = ST.getInstrInfo();
      if (RVInstrInfo::isSDWA(MI)) {
        if (auto *DstSel = TII->getNamedOperand(MI, RVGPU::OpName::dst_sel))
          if (DstSel->getImm() == RVGPU::SDWA::DWORD)
            return false;
      } else {
        if (!RVGPU::hasNamedOperand(MI.getOpcode(), RVGPU::OpName::op_sel) ||
            !(TII->getNamedOperand(MI, RVGPU::OpName::src0_modifiers)
                  ->getImm() &
              SISrcMods::DST_OP_SEL))
          return false;
      }
      const RVRegisterInfo *TRI = ST.getRegisterInfo();
      if (auto *Dst = TII->getNamedOperand(MI, RVGPU::OpName::vdst)) {
  ...
  if (ST.hasVDecCoExecHazard()) {
    ...
    if (VALU->readsRegister(RVGPU::VCC, TRI)) {
      UseReg = RVGPU::VCC;
      int WaitStatesNeededForDef =
          VALUWriteSGPRVALUReadWaitstates -
    ...
    case RVGPU::V_READFIRSTLANE_B32: {
      MachineOperand *Src = TII.getNamedOperand(*VALU, RVGPU::OpName::src0);
      UseReg = Src->getReg();
      int WaitStatesNeededForDef =
...
int GCNHazardRecognizer::checkRWLaneHazards(MachineInstr *RWLane) {
  ...
  const MachineOperand *LaneSelectOp =
      TII->getNamedOperand(*RWLane, RVGPU::OpName::src1);
...
int GCNHazardRecognizer::checkRFEHazards(MachineInstr *RFE) {
  ...
  auto IsHazardFn = [TII](const MachineInstr &MI) {
    return getHWReg(TII, MI) == RVGPU::Hwreg::ID_TRAPSTS;
  };
...
int GCNHazardRecognizer::checkReadM0Hazards(MachineInstr *MI) {
  ...
  return ReadM0WaitStates -
         getWaitStatesSinceDef(RVGPU::M0, IsHazardFn, ReadM0WaitStates);
}
...
bool GCNHazardRecognizer::fixVcmpxPermlaneHazards(MachineInstr *MI) {
  ...
  auto IsHazardFn = [TII, TRI](const MachineInstr &MI) {
    return (TII->isVOPC(MI) ||
           ...
           MI.modifiesRegister(RVGPU::EXEC, TRI);
  };

  auto IsExpiredFn = [](const MachineInstr &MI, int) {
    unsigned Opc = MI.getOpcode();
    return RVInstrInfo::isVALU(MI) && Opc != RVGPU::V_NOP_e32 &&
           Opc != RVGPU::V_NOP_e64 && Opc != RVGPU::V_NOP_sdwa;
  };
  ...
  auto *Src0 = TII->getNamedOperand(*MI, RVGPU::OpName::src0);
  Register Reg = Src0->getReg();
  bool IsUndef = Src0->isUndef();
  BuildMI(*MI->getParent(), MI, MI->getDebugLoc(),
          TII->get(RVGPU::V_MOV_B32_e32))
...
bool GCNHazardRecognizer::fixVMEMtoScalarWriteHazards(MachineInstr *MI) {
  ...
  if (!RVInstrInfo::isSALU(*MI) && !RVInstrInfo::isSMRD(*MI))
    return false;
  ...
  auto IsHazardFn = [TRI, MI](const MachineInstr &I) {
    if (!RVInstrInfo::isVMEM(I) && !RVInstrInfo::isDS(I) &&
        !RVInstrInfo::isFLAT(I))
      return false;
  ...
  auto IsExpiredFn = [](const MachineInstr &MI, int) {
    return RVInstrInfo::isVALU(MI) ||
           (MI.getOpcode() == RVGPU::S_WAITCNT &&
            !MI.getOperand(0).getImm()) ||
           (MI.getOpcode() == RVGPU::S_WAITCNT_DEPCTR &&
            RVGPU::DepCtr::decodeFieldVmVsrc(MI.getOperand(0).getImm()) == 0);
  };
  ...
  const RVInstrInfo *TII = ST.getInstrInfo();
  BuildMI(*MI->getParent(), MI, MI->getDebugLoc(),
          TII->get(RVGPU::S_WAITCNT_DEPCTR))
      .addImm(RVGPU::DepCtr::encodeFieldVmVsrc(0));
  return true;
}
...
bool GCNHazardRecognizer::fixSMEMtoVectorWriteHazards(MachineInstr *MI) {
  ...
  case RVGPU::V_READLANE_B32:
  case RVGPU::V_READFIRSTLANE_B32:
    SDSTName = RVGPU::OpName::vdst;
    break;
  default:
    SDSTName = RVGPU::OpName::sdst;
    break;
  }

  const RVInstrInfo *TII = ST.getInstrInfo();
  const RVRegisterInfo *TRI = ST.getRegisterInfo();
  const RVGPU::IsaVersion IV = RVGPU::getIsaVersion(ST.getCPU());
  ...
  auto IsHazardFn = [SDSTReg, TRI](const MachineInstr &I) {
    return RVInstrInfo::isSMRD(I) && I.readsRegister(SDSTReg, TRI);
  };

  auto IsExpiredFn = [TII, IV](const MachineInstr &MI, int) {
    if (TII->isSALU(MI)) {
      switch (MI.getOpcode()) {
      case RVGPU::S_SETVSKIP:
      case RVGPU::S_VERSION:
      case RVGPU::S_WAITCNT_VSCNT:
      case RVGPU::S_WAITCNT_VMCNT:
      case RVGPU::S_WAITCNT_EXPCNT:
        // These instructions cannot not mitigate the hazard.
        return false;
      case RVGPU::S_WAITCNT_LGKMCNT:
        // Reducing lgkmcnt count to 0 always mitigates the hazard.
        return (MI.getOperand(1).getImm() == 0) &&
               (MI.getOperand(0).getReg() == RVGPU::SGPR_NULL);
      case RVGPU::S_WAITCNT: {
        const int64_t Imm = MI.getOperand(0).getImm();
        RVGPU::Waitcnt Decoded = RVGPU::decodeWaitcnt(IV, Imm);
  ...
  BuildMI(*MI->getParent(), MI, MI->getDebugLoc(),
          TII->get(RVGPU::S_MOV_B32), RVGPU::SGPR_NULL)
      .addImm(0);
  return true;
}

bool GCNHazardRecognizer::fixVcmpxExecWARHazard(MachineInstr *MI) {
  if (!ST.hasVcmpxExecWARHazard() || !RVInstrInfo::isVALU(*MI))
    return false;

  const RVRegisterInfo *TRI = ST.getRegisterInfo();
  if (!MI->modifiesRegister(RVGPU::EXEC, TRI))
    return false;

  auto IsHazardFn = [TRI](const MachineInstr &I) {
    if (RVInstrInfo::isVALU(I))
      return false;
    return I.readsRegister(RVGPU::EXEC, TRI);
  };

  const RVInstrInfo *TII = ST.getInstrInfo();
  auto IsExpiredFn = [TII, TRI](const MachineInstr &MI, int) {
    if (RVInstrInfo::isVALU(MI)) {
      if (TII->getNamedOperand(MI, RVGPU::OpName::sdst))
        return true;
      for (auto MO : MI.implicit_operands())
        if (MO.isDef() && TRI->isSGPRClass(TRI->getPhysRegBaseClass(MO.getReg())))
          return true;
    }
    if (MI.getOpcode() == RVGPU::S_WAITCNT_DEPCTR &&
        RVGPU::DepCtr::decodeFieldSaSdst(MI.getOperand(0).getImm()) == 0)
      return true;
    return false;
  };
  ...
  BuildMI(*MI->getParent(), MI, MI->getDebugLoc(),
          TII->get(RVGPU::S_WAITCNT_DEPCTR))
      .addImm(RVGPU::DepCtr::encodeFieldSaSdst(0));
  return true;
}
...
static bool shouldRunLdsBranchVmemWARHazardFixup(const MachineFunction &MF,
  ...
  for (auto &MBB : MF) {
    for (auto &MI : MBB) {
      HasLds |= RVInstrInfo::isDS(MI);
      HasVmem |=
          RVInstrInfo::isVMEM(MI) || RVInstrInfo::isSegmentSpecificFLAT(MI);
      if (HasLds && HasVmem)
        return true;
    }
  }
...
static bool isStoreCountWaitZero(const MachineInstr &I) {
  return I.getOpcode() == RVGPU::S_WAITCNT_VSCNT &&
         I.getOperand(0).getReg() == RVGPU::SGPR_NULL &&
...
bool GCNHazardRecognizer::fixLdsBranchVmemWARHazard(MachineInstr *MI) {
  ...
  auto IsHazardInst = [](const MachineInstr &MI) {
    if (RVInstrInfo::isDS(MI))
      return 1;
    if (RVInstrInfo::isVMEM(MI) || RVInstrInfo::isSegmentSpecificFLAT(MI))
      return 2;
    return 0;
  };
  ...
  const RVInstrInfo *TII = ST.getInstrInfo();
  BuildMI(*MI->getParent(), MI, MI->getDebugLoc(),
          TII->get(RVGPU::S_WAITCNT_VSCNT))
    .addReg(RVGPU::SGPR_NULL, RegState::Undef)
    .addImm(0);
...
bool GCNHazardRecognizer::fixLdsDirectVALUHazard(MachineInstr *MI) {
  if (!RVInstrInfo::isLDSDIR(*MI))
    return false;

  const int NoHazardWaitStates = 15;
  const MachineOperand *VDST = TII.getNamedOperand(*MI, RVGPU::OpName::vdst);
  ...
  auto IsHazardFn = [this, VDSTReg, &VisitedTrans](const MachineInstr &I) {
    if (!RVInstrInfo::isVALU(I))
      return false;
    VisitedTrans = VisitedTrans || RVInstrInfo::isTRANS(I);
  ...
  auto IsExpiredFn = [&](const MachineInstr &I, int WaitStates) {
    ...
    return RVInstrInfo::isVMEM(I) || RVInstrInfo::isFLAT(I) ||
           RVInstrInfo::isDS(I) || RVInstrInfo::isEXP(I);
  };
  auto GetWaitStatesFn = [](const MachineInstr &MI) {
    return RVInstrInfo::isVALU(MI) ? 1 : 0;
  };
  ...
  MachineOperand *WaitVdstOp =
      TII.getNamedOperand(*MI, RVGPU::OpName::waitvdst);
...
bool GCNHazardRecognizer::fixLdsDirectVMEMHazard(MachineInstr *MI) {
  ...
  auto IsHazardFn = [this, VDSTReg](const MachineInstr &I) {
    if (!RVInstrInfo::isVMEM(I) && !RVInstrInfo::isFLAT(I) &&
        !RVInstrInfo::isDS(I))
      return false;
    return I.readsRegister(VDSTReg, &TRI) || I.modifiesRegister(VDSTReg, &TRI);
  };
  auto IsExpiredFn = [](const MachineInstr &I, int) {
    return RVInstrInfo::isVALU(I) || RVInstrInfo::isEXP(I) ||
           (I.getOpcode() == RVGPU::S_WAITCNT && !I.getOperand(0).getImm()) ||
           (I.getOpcode() == RVGPU::S_WAITCNT_DEPCTR &&
            RVGPU::DepCtr::decodeFieldVmVsrc(I.getOperand(0).getImm()) == 0);
  };
...
bool GCNHazardRecognizer::fixVALUPartialForwardingHazard(MachineInstr *MI) {
  ...
  auto IsHazardFn = [&, this](StateType &State, const MachineInstr &I) {
    ...
    if (RVInstrInfo::isVMEM(I) || RVInstrInfo::isFLAT(I) ||
        RVInstrInfo::isDS(I) || RVInstrInfo::isEXP(I) ||
        (I.getOpcode() == RVGPU::S_WAITCNT_DEPCTR &&
         RVGPU::DepCtr::decodeFieldVaVdst(I.getOperand(0).getImm()) == 0))
      return HazardExpired;
    ...
    } else if (RVInstrInfo::isSALU(I)) {
      if (State.ExecPos == std::numeric_limits<int>::max()) {
        if (!State.DefPos.empty() && I.modifiesRegister(RVGPU::EXEC, &TRI)) {
...
bool GCNHazardRecognizer::fixVALUTransUseHazard(MachineInstr *MI) {
  ...
  BuildMI(*MI->getParent(), MI, MI->getDebugLoc(),
          TII.get(RVGPU::S_WAITCNT_DEPCTR))
      .addImm(RVGPU::DepCtr::encodeFieldVaVdst(0));
...
bool GCNHazardRecognizer::fixWMMAHazards(MachineInstr *MI) {
  ...
  auto IsHazardFn = [MI, TII, TRI](const MachineInstr &I) {
    if (!RVInstrInfo::isWMMA(I))
      return false;

    // Src0 or Src1 of the current wmma instruction overlaps with the dest of
    // the previous wmma.
    const Register CurSrc0Reg =
        TII->getNamedOperand(*MI, RVGPU::OpName::src0)->getReg();
    const Register CurSrc1Reg =
        TII->getNamedOperand(*MI, RVGPU::OpName::src1)->getReg();

    const Register PrevDstReg =
        TII->getNamedOperand(I, RVGPU::OpName::vdst)->getReg();
  ...
  BuildMI(*MI->getParent(), MI, MI->getDebugLoc(), TII->get(RVGPU::V_NOP_e32));
...
bool GCNHazardRecognizer::fixShift64HighRegBug(MachineInstr *MI) {
  ...
  MachineOperand *Amt = TII.getNamedOperand(*MI, RVGPU::OpName::src0);
  if (!Amt->isReg())
    return false;
  ...
  if (!TRI.isVGPR(MRI, AmtReg) || ((AmtReg - RVGPU::VGPR0) & 7) != 7)
    return false;

  if (AmtReg != RVGPU::VGPR255 && MRI.isPhysRegUsed(AmtReg + 1))
    return false;

  MachineOperand *Src1 = TII.getNamedOperand(*MI, RVGPU::OpName::src1);
  ...
  static_assert(RVGPU::VGPR0 + 1 == RVGPU::VGPR1);

  Register NewReg;
  for (MCRegister Reg : Overlapped ? RVGPU::VReg_64_Align2RegClass
  ...
  Register NewAmt = Overlapped ? (Register)TRI.getSubReg(NewReg, RVGPU::sub1)
                               : NewReg;
  Register NewAmtLo;

  if (Overlapped)
    NewAmtLo = TRI.getSubReg(NewReg, RVGPU::sub0);

  DebugLoc DL = MI->getDebugLoc();
  MachineBasicBlock *MBB = MI->getParent();
  // Insert a full wait count because found register might be pending a wait.
  BuildMI(*MBB, MI, DL, TII.get(RVGPU::S_WAITCNT))
      .addImm(0);

  // Insert V_SWAP_B32 instruction(s) and run hazard recognizer on them.
  if (Overlapped)
    runOnInstruction(
        BuildMI(*MBB, MI, DL, TII.get(RVGPU::V_SWAP_B32), NewAmtLo)
            .addDef(AmtReg - 1)
            .addReg(AmtReg - 1, RegState::Undef)
            .addReg(NewAmtLo, RegState::Undef));
  runOnInstruction(BuildMI(*MBB, MI, DL, TII.get(RVGPU::V_SWAP_B32), NewAmt)
                       .addDef(AmtReg)
  ...
  BuildMI(*MBB, std::next(MI->getIterator()), DL, TII.get(RVGPU::V_SWAP_B32),
          AmtReg)
      .addDef(NewAmt)
      .addReg(NewAmt)
      .addReg(AmtReg);
...
int GCNHazardRecognizer::checkNSAtoVMEMHazard(MachineInstr *MI) {
  ...
  if (!RVInstrInfo::isMUBUF(*MI) && !RVInstrInfo::isMTBUF(*MI))
    return 0;

  const RVInstrInfo *TII = ST.getInstrInfo();
  const auto *Offset = TII->getNamedOperand(*MI, RVGPU::OpName::offset);
  if (!Offset || (Offset->getImm() & 6) == 0)
    return 0;

  auto IsHazardFn = [TII](const MachineInstr &I) {
    if (!RVInstrInfo::isMIMG(I))
      return false;
    const RVGPU::MIMGInfo *Info = RVGPU::getMIMGInfo(I.getOpcode());
    return Info->MIMGEncoding == RVGPU::MIMGEncGfx10NSA &&
...
int GCNHazardRecognizer::checkFPAtomicToDenormModeHazard(MachineInstr *MI) {
  ...
  if (MI->getOpcode() != RVGPU::S_DENORM_MODE)
    return 0;

  auto IsHazardFn = [](const MachineInstr &I) {
    if (!RVInstrInfo::isVMEM(I) && !RVInstrInfo::isFLAT(I))
      return false;
    return RVInstrInfo::isFPAtomic(I);
  };

  auto IsExpiredFn = [](const MachineInstr &MI, int WaitStates) {
    if (WaitStates >= 3 || RVInstrInfo::isVALU(MI))
      return true;
...
int GCNHazardRecognizer::checkMAIHazards908(MachineInstr *MI) {
  ...
  auto IsVALUFn = [](const MachineInstr &MI) {
    return RVInstrInfo::isVALU(MI) || MI.isInlineAsm();
  };

  if (Opc != RVGPU::V_ACCVGPR_READ_B32_e64) { // MFMA or v_accvgpr_write
    ...
    int WaitStatesNeededForUse = VALUWritesExecWaitStates -
      getWaitStatesSinceDef(RVGPU::EXEC, IsVALUFn, MaxWaitStates);
    WaitStatesNeeded = std::max(WaitStatesNeeded, WaitStatesNeededForUse);
  ...
  for (const MachineOperand &Op : MI->explicit_operands()) {
    ...
    if (Op.isDef() && Opc != RVGPU::V_ACCVGPR_WRITE_B32_e64)
      continue;
    ...
    int SrcCIdx = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::src2);
    int OpNo = Op.getOperandNo();
    if (OpNo == SrcCIdx) {
      NeedWaitStates = MFMAWritesAGPROverlappedSrcCWaitStates;
    } else if (Opc == RVGPU::V_ACCVGPR_READ_B32_e64) {
    ...
    } else if (Opc == RVGPU::V_ACCVGPR_WRITE_B32_e64) {
      switch (HazardDefLatency) {
  ...
  if (Opc == RVGPU::V_ACCVGPR_WRITE_B32_e64) {
    ...
    auto IsSrcCMFMAFn = [DstReg, &HazardDefLatency,
      ...
      if (!RVInstrInfo::isMFMA(MI))
        return false;
      Register Reg = TII.getNamedOperand(MI, RVGPU::OpName::src2)->getReg();
      HazardDefLatency =
          std::max(HazardDefLatency, TSchedModel.computeInstrLatency(&MI));
      return TRI.regsOverlap(Reg, DstReg);
    };
...
int GCNHazardRecognizer::checkMAIHazards90A(MachineInstr *MI) {
  ...
  auto IsLegacyVALUFn = [](const MachineInstr &MI) {
    return RVInstrInfo::isVALU(MI) && !RVInstrInfo::isMFMA(MI);
  };

  auto IsLegacyVALUNotDotFn = [](const MachineInstr &MI) {
    return RVInstrInfo::isVALU(MI) && !RVInstrInfo::isMFMA(MI) &&
           !RVInstrInfo::isDOT(MI);
  };
  ...
  const int VALUWritesExecWaitStates = 4;
  int WaitStatesNeededForUse = VALUWritesExecWaitStates -
    getWaitStatesSinceDef(RVGPU::EXEC, IsLegacyVALUFn,
  ...
  for (const MachineOperand &Use : MI->explicit_uses()) {
    ...
    if (OpNo == SrcCIdx) {
      if (!isDGEMM(Opc) && (!ST.hasGFX940Insts() && isDGEMM(Opc1))) {
        ...
        if ((Opc == RVGPU::V_MFMA_F64_4X4X4F64_e64 ||
             Opc == RVGPU::V_MFMA_F64_4X4X4F64_vgprcd_e64) &&
            (Opc1 == RVGPU::V_MFMA_F64_4X4X4F64_e64 ||
             Opc1 == RVGPU::V_MFMA_F64_4X4X4F64_vgprcd_e64))
        ...
        switch (Opc1) {
        case RVGPU::V_MFMA_F64_16X16X4F64_e64:
        case RVGPU::V_MFMA_F64_16X16X4F64_vgprcd_e64:
        case RVGPU::V_MFMA_F64_16X16X4F64_mac_e64:
        case RVGPU::V_MFMA_F64_16X16X4F64_mac_vgprcd_e64:
          if (!isXDL(ST, *MI))
            NeedWaitStates = DMFMA16x16WritesVGPROverlappedSrcCWaitStates;
          break;
        case RVGPU::V_MFMA_F64_4X4X4F64_e64:
        case RVGPU::V_MFMA_F64_4X4X4F64_vgprcd_e64:
          if (!isXDL(ST, *MI))
...
int GCNHazardRecognizer::checkMAILdStHazards(MachineInstr *MI) {
  ...
  auto IsAccVgprReadFn = [](const MachineInstr &MI) {
    return MI.getOpcode() == RVGPU::V_ACCVGPR_READ_B32_e64;
  };
  ...
  for (const MachineOperand &Op : MI->explicit_uses()) {
    ...
    auto IsVALUAccVgprRdWrCheckFn = [Reg, this](const MachineInstr &MI) {
      if (MI.getOpcode() != RVGPU::V_ACCVGPR_READ_B32_e64 &&
          MI.getOpcode() != RVGPU::V_ACCVGPR_WRITE_B32_e64)
        return false;
      auto IsVALUFn = [](const MachineInstr &MI) {
        return RVInstrInfo::isVALU(MI) && !RVInstrInfo::isMAI(MI);
      };
...
int GCNHazardRecognizer::checkMAIVALUHazards(MachineInstr *MI) {
  ...
  bool IsMem = RVInstrInfo::isVMEM(*MI) ||
               RVInstrInfo::isFLAT(*MI) ||
               RVInstrInfo::isDS(*MI);
  bool IsMemOrExport = IsMem || RVInstrInfo::isEXP(*MI);
  bool IsVALU = RVInstrInfo::isVALU(*MI);
  ...
  int SrcCIdx = RVGPU::getNamedOperandIdx(MI->getOpcode(),
                                           RVGPU::OpName::src2);
  ...
  if ((Opc == RVGPU::V_FMA_F64_e64 ||
       Opc == RVGPU::V_FMAC_F64_e32 || Opc == RVGPU::V_FMAC_F64_e64 ||
       Opc == RVGPU::V_FMAC_F64_dpp) &&
  ...
  for (const MachineOperand &Def : MI->defs()) {
    ...
    auto IsSMFMAReadAsCFn = [&Reg, &MFMA, this](const MachineInstr &MI) {
      if (!RVInstrInfo::isMFMA(MI) || isDGEMM(MI.getOpcode()) ||
...
bool GCNHazardRecognizer::fixVALUMaskWriteHazard(MachineInstr *MI) {
  ...
  const MachineOperand *SDSTOp = TII.getNamedOperand(*MI, RVGPU::OpName::sdst);
  if (!SDSTOp || !SDSTOp->isReg())
    return false;

  const Register HazardReg = SDSTOp->getReg();
  if (HazardReg == RVGPU::EXEC ||
      HazardReg == RVGPU::EXEC_LO ||
      HazardReg == RVGPU::EXEC_HI ||
      HazardReg == RVGPU::M0)
    return false;
  ...
  auto IsHazardFn = [HazardReg, this](const MachineInstr &I) {
    ...
    case RVGPU::V_ADDC_U32_dpp:
    case RVGPU::V_CNDMASK_B16_e32:
    case RVGPU::V_CNDMASK_B16_dpp:
    case RVGPU::V_CNDMASK_B32_e32:
    case RVGPU::V_CNDMASK_B32_dpp:
    case RVGPU::V_DIV_FMAS_F32_e64:
    case RVGPU::V_DIV_FMAS_F64_e64:
    case RVGPU::V_SUBB_U32_e32:
    case RVGPU::V_SUBB_U32_dpp:
    case RVGPU::V_SUBBREV_U32_e32:
    case RVGPU::V_SUBBREV_U32_dpp:
      // These implicitly read VCC as mask source.
      return HazardReg == RVGPU::VCC ||
             HazardReg == RVGPU::VCC_LO ||
             HazardReg == RVGPU::VCC_HI;
    case RVGPU::V_ADDC_U32_e64:
    case RVGPU::V_ADDC_U32_e64_dpp:
    case RVGPU::V_CNDMASK_B16_e64:
    case RVGPU::V_CNDMASK_B16_e64_dpp:
    case RVGPU::V_CNDMASK_B32_e64:
    case RVGPU::V_CNDMASK_B32_e64_dpp:
    case RVGPU::V_SUBB_U32_e64:
    case RVGPU::V_SUBB_U32_e64_dpp:
    case RVGPU::V_SUBBREV_U32_e64:
    case RVGPU::V_SUBBREV_U32_e64_dpp: {
      // Only check mask register overlaps.
      const MachineOperand *SSRCOp = TII.getNamedOperand(I, RVGPU::OpName::src2);
      assert(SSRCOp);
  ...
  auto IsExpiredFn = [&MRI, this](const MachineInstr &I, int) {
    ...
    if (I.getOpcode() == RVGPU::S_WAITCNT_DEPCTR &&
        RVGPU::DepCtr::decodeFieldSaSdst(I.getOperand(0).getImm()) == 0)
      return true;
  ...
  if (MI->getOpcode() == RVGPU::S_GETPC_B64) {
