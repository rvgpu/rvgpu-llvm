//===--- RVGPUExportClusting.cpp - RVGPU Export Clustering  -------------===//
//
...
static bool isExport(const SUnit &SU) {
  return RVInstrInfo::isEXP(*SU.getInstr());
}

static bool isPositionExport(const RVInstrInfo *TII, SUnit *SU) {
  const MachineInstr *MI = SU->getInstr();
  unsigned Imm = TII->getNamedOperand(*MI, RVGPU::OpName::tgt)->getImm();
  return Imm >= RVGPU::Exp::ET_POS0 && Imm <= RVGPU::Exp::ET_POS_LAST;
}

static void sortChain(const RVInstrInfo *TII, SmallVector<SUnit *, 8> &Chain,
                      unsigned PosCount) {
...
void ExportClustering::apply(ScheduleDAGInstrs *DAG) {
  const RVInstrInfo *TII = static_cast<const RVInstrInfo *>(DAG->TII);
...
std::unique_ptr<ScheduleDAGMutation> createRVGPUExportClusteringDAGMutation() {
