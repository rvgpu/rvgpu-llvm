//===- RVGPUMCInstLower.h - Lower MachineInstr to MCInst ------*- C++ -*--===//
//
...
//
/// \file
/// Header of lower RVGPU MachineInstrs to their corresponding MCInst.
//
//===----------------------------------------------------------------------===//
//

#ifndef LLVM_LIB_TARGET_RVGPU_RVGPUMCINSTLOWER_H
#define LLVM_LIB_TARGET_RVGPU_RVGPUMCINSTLOWER_H
...
class RVGPUMCInstLower {
  MCContext &Ctx;
  const TargetSubtargetInfo &ST;
  const AsmPrinter &AP;

public:
  RVGPUMCInstLower(MCContext &ctx, const TargetSubtargetInfo &ST,
                    const AsmPrinter &AP);
...
static inline const MCExpr *lowerAddrSpaceCast(const TargetMachine &TM,
  ...
  // This is safe since TM is always of type RVGPUTargetMachine or its
  // derived class.
  auto &AT = static_cast<const RVGPUTargetMachine &>(TM);
...
#endif // LLVM_LIB_TARGET_RVGPU_RVGPUMCINSTLOWER_H
