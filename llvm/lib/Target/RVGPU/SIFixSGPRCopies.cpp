...
static bool isVGPRToSGPRCopy(const TargetRegisterClass *SrcRC,
                             ...
                             const RVRegisterInfo &TRI) {
  return SrcRC != &RVGPU::VReg_1RegClass && TRI.isSGPRClass(DstRC) &&
...
static bool isSGPRToVGPRCopy(const TargetRegisterClass *SrcRC,
                             const TargetRegisterClass *DstRC,
                             const RVRegisterInfo &TRI) {
  return DstRC != &RVGPU::VReg_1RegClass && TRI.isSGPRClass(SrcRC) &&
...
static bool foldVGPRCopyIntoRegSequence(MachineInstr &MI,
  ...
  for (unsigned I = 1, N = MI.getNumOperands(); I != N; I += 2) {
    ...
    BuildMI(*MI.getParent(), &MI, MI.getDebugLoc(), TII->get(RVGPU::COPY),
            TmpReg)
        .add(MI.getOperand(I));
    ...
    if (IsAGPR) {
      ...
      unsigned Opc = NewSrcRC == &RVGPU::AGPR_32RegClass ?
        RVGPU::V_ACCVGPR_WRITE_B32_e64 : RVGPU::COPY;
...
static bool isSafeToFoldImmIntoCopy(const MachineInstr *Copy,
  ...
  const MachineOperand *ImmOp =
      TII->getNamedOperand(*MoveImm, RVGPU::OpName::src0);
  if (!ImmOp->isImm())
    return false;
  ...
  case RVGPU::V_MOV_B32_e32:
    SMovOp = RVGPU::S_MOV_B32;
    break;
  case RVGPU::V_MOV_B64_PSEUDO:
    SMovOp = RVGPU::S_MOV_B64_IMM_PSEUDO;
    break;
  }
  Imm = ImmOp->getImm();
  return true;
...
bool SIFixSGPRCopies::runOnMachineFunction(MachineFunction &MF) {
    ...
    for (MachineBasicBlock::iterator I = MBB->begin(), E = MBB->end(); I != E;
      ...
      case RVGPU::V_WRITELANE_B32: {
        ...
        if ((Src0.isReg() && TRI->isSGPRReg(*MRI, Src0.getReg()) &&
             Src0.getReg() != RVGPU::M0) &&
            (Src1.isReg() && TRI->isSGPRReg(*MRI, Src1.getReg()) &&
             Src1.getReg() != RVGPU::M0)) {
          ...
          if (!Resolved) {
            ...
            BuildMI(*MI.getParent(), MI, MI.getDebugLoc(),
                    TII->get(RVGPU::COPY), RVGPU::M0)
                .add(Src1);
            Src1.ChangeToRegister(RVGPU::M0, false);
          }
        }
        break;
      }
  ...
  if (MF.getTarget().getOptLevel() > CodeGenOptLevel::None && EnableM0Merge)
    hoistAndMergeSGPRInits(RVGPU::M0, *MRI, TRI, *MDT, TII);
...
bool SIFixSGPRCopies::tryMoveVGPRConstToSGPR(
  ...
  MachineOperand *SrcConst = TII->getNamedOperand(*DefMI, RVGPU::OpName::src0);
  if (SrcConst->isReg())
    return false;
  ...
  unsigned MoveOp = MoveSize == 64 ? RVGPU::S_MOV_B64 : RVGPU::S_MOV_B32;
...
bool SIFixSGPRCopies::lowerSpecialCase(MachineInstr &MI,
  ...
  if (!DstReg.isVirtual()) {
    ...
    if (DstReg == RVGPU::M0 &&
      ...
      Register TmpReg =
          MRI->createVirtualRegister(&RVGPU::SReg_32_XM0RegClass);
      BuildMI(*MI.getParent(), MI, MI.getDebugLoc(),
              TII->get(RVGPU::V_READFIRSTLANE_B32), TmpReg)
          .add(MI.getOperand(1));
...
void SIFixSGPRCopies::analyzeVGPRToSGPRCopy(MachineInstr* MI) {
  ...
  while (!AnalysisWorklist.empty()) {
    ...
    if ((TII->isSALU(*Inst) && Inst->isCompare()) ||
        (Inst->isCopy() && Inst->getOperand(0).getReg() == RVGPU::SCC)) {
      auto I = Inst->getIterator();
      auto E = Inst->getParent()->end();
      while (++I != E && !I->findRegisterDefOperand(RVGPU::SCC)) {
        if (I->readsRegister(RVGPU::SCC))
          Users.push_back(&*I);
      }
  ...
  for (auto C : V2SCopies) {
    ...
    if (SrcSize == 16) {
      ...
      auto MIB = BuildMI(*MBB, MI, MI->getDebugLoc(),
                         TII->get(RVGPU::V_READFIRSTLANE_B32), DstReg);
      MIB.addReg(SrcReg, 0, RVGPU::NoSubRegister);
    } else if (SrcSize == 32) {
      auto MIB = BuildMI(*MBB, MI, MI->getDebugLoc(),
                         TII->get(RVGPU::V_READFIRSTLANE_B32), DstReg);
      ...
      auto Result = BuildMI(*MBB, MI, MI->getDebugLoc(),
                            TII->get(RVGPU::REG_SEQUENCE), DstReg);
      int N = TRI->getRegSizeInBits(*SrcRC) / 32;
      for (int i = 0; i < N; i++) {
        Register PartialSrc = TII->buildExtractSubReg(
            Result, *MRI, MI->getOperand(1), SrcRC,
            TRI->getSubRegFromChannel(i), &RVGPU::VGPR_32RegClass);
        Register PartialDst =
            MRI->createVirtualRegister(&RVGPU::SReg_32RegClass);
        BuildMI(*MBB, *Result, Result->getDebugLoc(),
                TII->get(RVGPU::V_READFIRSTLANE_B32), PartialDst)
            .addReg(PartialSrc);
    ...
    for (MachineBasicBlock::iterator I = MBB->begin(), E = MBB->end(); I != E;
      ...
      if (SrcReg == RVGPU::SCC) {
        Register SCCCopy = MRI->createVirtualRegister(
            TRI->getRegClass(RVGPU::SReg_1_XEXECRegClassID));
        I = BuildMI(*MI.getParent(), std::next(MachineBasicBlock::iterator(MI)),
                    MI.getDebugLoc(),
                    TII->get(IsWave32 ? RVGPU::S_CSELECT_B32
                                      : RVGPU::S_CSELECT_B64),
                    SCCCopy)
                .addImm(-1)
                .addImm(0);
        I = BuildMI(*MI.getParent(), std::next(I), I->getDebugLoc(),
                    TII->get(RVGPU::COPY), DstReg)
                .addReg(SCCCopy);
        MI.eraseFromParent();
        continue;
      }
      if (DstReg == RVGPU::SCC) {
        unsigned Opcode = IsWave32 ? RVGPU::S_AND_B32 : RVGPU::S_AND_B64;
        Register Exec = IsWave32 ? RVGPU::EXEC_LO : RVGPU::EXEC;
