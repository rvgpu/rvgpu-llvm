//===--- RVGPUHSAMetadataStreamer.cpp --------------------------*- C++ -*-===//
//
...
static cl::opt<bool> DumpHSAMetadata(
    "rvgpu-dump-hsa-metadata",
    cl::desc("Dump RVGPU HSA Metadata"));
static cl::opt<bool> VerifyHSAMetadata(
    "rvgpu-verify-hsa-metadata",
    cl::desc("Verify RVGPU HSA Metadata"));
...
MetadataStreamerMsgPackV4::getValueKind(Type *Ty, StringRef TypeQual,
  ...
  return StringSwitch<StringRef>(BaseTypeName)
      ...
      .Default(isa<PointerType>(Ty)
                   ? (Ty->getPointerAddressSpace() == RVGPUAS::LOCAL_ADDRESS
...
void MetadataStreamerMsgPackV4::emitTargetID(
  ...
  getRootMetadata("rvhsa.target") =
      HSAMetadataDoc->getNode("rvgpu", /*Copy=*/true);
}
...
void MetadataStreamerMsgPackV4::emitKernelArg(const Argument &Arg,
  ...
  if (auto PtrTy = dyn_cast<PointerType>(Ty)) {
    if (PtrTy->getAddressSpace() == RVGPUAS::LOCAL_ADDRESS)
...
void MetadataStreamerMsgPackV4::emitHiddenKernelArgs(
  ...
  unsigned HiddenArgNumBytes = 256;
  ...
  auto Int8PtrTy =
      PointerType::get(Func.getContext(), RVGPUAS::GLOBAL_ADDRESS);
  ...
  if (HiddenArgNumBytes >= 32) {
    ...
    if (M->getNamedMetadata("llvm.printf.fmts"))
      emitKernelArg(DL, Int8PtrTy, Align(8), "hidden_printf_buffer", Offset,
                    Args);
    else if (!Func.hasFnAttribute("rvgpu-no-hostcall-ptr"))
  ...
  if (HiddenArgNumBytes >= 40) {
    if (!Func.hasFnAttribute("rvgpu-no-default-queue")) {
  ...
  if (HiddenArgNumBytes >= 48) {
    if (!Func.hasFnAttribute("rvgpu-no-completion-action")) {
  ...
  if (HiddenArgNumBytes >= 56) {
    if (!Func.hasFnAttribute("rvgpu-no-multigrid-sync-arg")) {
...
//  Kern[".vgpr_spill_count"] = Kern.getDocument()->getNode(MFI.getNumSpilledVGPRs());

  return Kern;
}

bool MetadataStreamerMsgPackV4::emitTo(RVGPUTargetStreamer &TargetStreamer) {
...
//  if (ST.getImplicitArgNumBytes(Func) == 0)
  ...
  if (!Func.hasFnAttribute("rvgpu-no-hostcall-ptr")) {
    emitKernelArg(DL, Int8PtrTy, Align(8), "hidden_hostcall_buffer", Offset,
                  Args);
  } else {
    Offset += 8; // Skipped.
  ...
  if (!Func.hasFnAttribute("rvgpu-no-heap-ptr"))
