...
//===----------------------------------------------------------------------===//
//
/// \file This file contains the RVGPU DAG scheduling
...
bool llvm::checkVOPDRegConstraints(const RVInstrInfo &TII,
                                   const MachineInstr &FirstMI,
                                   const MachineInstr &SecondMI) {
  namespace VOPD = RVGPU::VOPD;

  const MachineFunction *MF = FirstMI.getMF();
  const RVSubtarget &ST = MF->getSubtarget<RVSubtarget>();
  const RVRegisterInfo *TRI = dyn_cast<RVRegisterInfo>(ST.getRegisterInfo());
  ...
  auto InstInfo =
      RVGPU::getVOPDInstInfo(FirstMI.getDesc(), SecondMI.getDesc());
  ...
  for (auto CompIdx : VOPD::COMPONENTS) {
    ...
    if (MI.getDesc().hasImplicitUseOfPhysReg(RVGPU::VCC))
      UniqueScalarRegs.push_back(RVGPU::VCC_LO);
  }
  ...
  bool SkipSrc = ST.getGeneration() >= RVGPUSubtarget::GFX12 &&
                 FirstMI.getOpcode() == RVGPU::V_MOV_B32_e32 &&
                 SecondMI.getOpcode() == RVGPU::V_MOV_B32_e32;
...
static bool shouldScheduleVOPDAdjacent(const TargetInstrInfo &TII,
                                       ...
                                       const MachineInstr *FirstMI,
                                       const MachineInstr &SecondMI) {
  const RVInstrInfo &STII = static_cast<const RVInstrInfo &>(TII);
  unsigned Opc2 = SecondMI.getOpcode();
  auto SecondCanBeVOPD = RVGPU::getCanBeVOPD(Opc2);
  ...
  auto FirstCanBeVOPD = RVGPU::getCanBeVOPD(Opc);
...
struct VOPDPairingMutation : ScheduleDAGMutation {
  ...
  void apply(ScheduleDAGInstrs *DAG) override {
    ...
    const RVSubtarget &ST = DAG->MF.getSubtarget<RVSubtarget>();
    if (!RVGPU::hasVOPD(ST) || !ST.isWave32()) {
