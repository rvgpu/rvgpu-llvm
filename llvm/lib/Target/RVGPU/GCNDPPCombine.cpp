...
bool GCNDPPCombine::isShrinkable(MachineInstr &MI) const {
  ...
  if (RVGPU::isTrue16Inst(Op))
    return false;
  if (const auto *SDst = TII->getNamedOperand(MI, RVGPU::OpName::sdst)) {
  ...
  if (!hasNoImmOrEqual(MI, RVGPU::OpName::src0_modifiers, 0, Mask) ||
      !hasNoImmOrEqual(MI, RVGPU::OpName::src1_modifiers, 0, Mask) ||
      !hasNoImmOrEqual(MI, RVGPU::OpName::clamp, 0) ||
      !hasNoImmOrEqual(MI, RVGPU::OpName::omod, 0)) {
...
int GCNDPPCombine::getDPPOp(unsigned Op, bool IsShrinkable) const {
  int DPP32 = RVGPU::getDPPOp32(Op);
  if (IsShrinkable) {
    assert(DPP32 == -1);
    int E32 = RVGPU::getVOPe32(Op);
    DPP32 = (E32 == -1) ? -1 : RVGPU::getDPPOp32(E32);
  }
  if (DPP32 != -1 && TII->pseudoToMCOpcode(DPP32) != -1)
    return DPP32;
  int DPP64 = -1;
  if (ST->hasVOP3DPP())
    DPP64 = RVGPU::getDPPOp64(Op);
...
MachineInstr *GCNDPPCombine::createDPPInst(MachineInstr &OrigMI,
                                           ...
                                           bool CombBCZ,
                                           bool IsShrinkable) const {
  assert(MovMI.getOpcode() == RVGPU::V_MOV_B32_dpp ||
         MovMI.getOpcode() == RVGPU::V_MOV_B64_dpp ||
         MovMI.getOpcode() == RVGPU::V_MOV_B64_DPP_PSEUDO);
  ...
  int OrigOpE32 = RVGPU::getVOPe32(OrigOp);
  // Prior checks cover Mask with VOPC condition, but not on purpose
  auto *RowMaskOpnd = TII->getNamedOperand(MovMI, RVGPU::OpName::row_mask);
  assert(RowMaskOpnd && RowMaskOpnd->isImm());
  auto *BankMaskOpnd = TII->getNamedOperand(MovMI, RVGPU::OpName::bank_mask);
  ...
  do {
    int NumOperands = 0;
    if (auto *Dst = TII->getNamedOperand(OrigMI, RVGPU::OpName::vdst)) {
      DPPInst.add(*Dst);
      ++NumOperands;
    }
    if (auto *SDst = TII->getNamedOperand(OrigMI, RVGPU::OpName::sdst)) {
    ...
    const int OldIdx = RVGPU::getNamedOperandIdx(DPPOp, RVGPU::OpName::old);
    if (OldIdx != -1) {
    ...
    if (auto *Mod0 = TII->getNamedOperand(OrigMI,
      ...
      assert(NumOperands == RVGPU::getNamedOperandIdx(DPPOp,
                                          RVGPU::OpName::src0_modifiers));
      assert(HasVOP3DPP ||
             (0LL == (Mod0->getImm() & ~(SISrcMods::ABS | SISrcMods::NEG))));
      DPPInst.addImm(Mod0->getImm());
      ++NumOperands;
    } else if (RVGPU::hasNamedOperand(DPPOp, RVGPU::OpName::src0_modifiers)) {
      DPPInst.addImm(0);
      ++NumOperands;
    }
    auto *Src0 = TII->getNamedOperand(MovMI, RVGPU::OpName::src0);
    assert(Src0);
    ...
    if (auto *Mod1 = TII->getNamedOperand(OrigMI,
      ...
      ++NumOperands;
    } else if (RVGPU::hasNamedOperand(DPPOp, RVGPU::OpName::src1_modifiers)) {
      DPPInst.addImm(0);
      ++NumOperands;
    }
    auto *Src1 = TII->getNamedOperand(OrigMI, RVGPU::OpName::src1);
    if (Src1) {
    ...
    if (auto *Mod2 =
            TII->getNamedOperand(OrigMI, RVGPU::OpName::src2_modifiers)) {
      assert(NumOperands ==
             RVGPU::getNamedOperandIdx(DPPOp, RVGPU::OpName::src2_modifiers));
      assert(HasVOP3DPP ||
             (0LL == (Mod2->getImm() & ~(SISrcMods::ABS | SISrcMods::NEG))));
      DPPInst.addImm(Mod2->getImm());
      ++NumOperands;
    }
    auto *Src2 = TII->getNamedOperand(OrigMI, RVGPU::OpName::src2);
    if (Src2) {
      if (!TII->getNamedOperand(*DPPInst.getInstr(), RVGPU::OpName::src2) ||
    ...
    if (HasVOP3DPP) {
      auto *ClampOpr = TII->getNamedOperand(OrigMI, RVGPU::OpName::clamp);
      if (ClampOpr && RVGPU::hasNamedOperand(DPPOp, RVGPU::OpName::clamp)) {
        DPPInst.addImm(ClampOpr->getImm());
      }
      auto *VdstInOpr = TII->getNamedOperand(OrigMI, RVGPU::OpName::vdst_in);
      if (VdstInOpr &&
          RVGPU::hasNamedOperand(DPPOp, RVGPU::OpName::vdst_in)) {
        DPPInst.add(*VdstInOpr);
      }
      auto *OmodOpr = TII->getNamedOperand(OrigMI, RVGPU::OpName::omod);
      if (OmodOpr && RVGPU::hasNamedOperand(DPPOp, RVGPU::OpName::omod)) {
        DPPInst.addImm(OmodOpr->getImm());
      }
      // Validate OP_SEL has to be set to all 0 and OP_SEL_HI has to be set to
      // all 1.
      if (auto *OpSelOpr =
              TII->getNamedOperand(OrigMI, RVGPU::OpName::op_sel)) {
        ...
        if (RVGPU::hasNamedOperand(DPPOp, RVGPU::OpName::op_sel))
          DPPInst.addImm(OpSel);
      }
      if (auto *OpSelHiOpr =
              TII->getNamedOperand(OrigMI, RVGPU::OpName::op_sel_hi)) {
        ...
        if (RVGPU::hasNamedOperand(DPPOp, RVGPU::OpName::op_sel_hi))
          DPPInst.addImm(OpSelHi);
      }
      auto *NegOpr = TII->getNamedOperand(OrigMI, RVGPU::OpName::neg_lo);
      if (NegOpr && RVGPU::hasNamedOperand(DPPOp, RVGPU::OpName::neg_lo)) {
        DPPInst.addImm(NegOpr->getImm());
      }
      auto *NegHiOpr = TII->getNamedOperand(OrigMI, RVGPU::OpName::neg_hi);
      if (NegHiOpr && RVGPU::hasNamedOperand(DPPOp, RVGPU::OpName::neg_hi)) {
        DPPInst.addImm(NegHiOpr->getImm());
      }
    }
    DPPInst.add(*TII->getNamedOperand(MovMI, RVGPU::OpName::dpp_ctrl));
    DPPInst.add(*TII->getNamedOperand(MovMI, RVGPU::OpName::row_mask));
    DPPInst.add(*TII->getNamedOperand(MovMI, RVGPU::OpName::bank_mask));
...
static bool isIdentityValue(unsigned OrigMIOp, MachineOperand *OldOpnd) {
  ...
  switch (OrigMIOp) {
  default: break;
  case RVGPU::V_ADD_U32_e32:
  case RVGPU::V_ADD_U32_e64:
  case RVGPU::V_ADD_CO_U32_e32:
  case RVGPU::V_ADD_CO_U32_e64:
  ...
  case RVGPU::V_MAX_U32_e32:
  case RVGPU::V_MAX_U32_e64:
  ...
  case RVGPU::V_MIN_U32_e32:
  case RVGPU::V_MIN_U32_e64:
    if (static_cast<uint32_t>(OldOpnd->getImm()) ==
        std::numeric_limits<uint32_t>::max())
      return true;
    break;
  case RVGPU::V_MIN_I32_e32:
  case RVGPU::V_MIN_I32_e64:
    if (static_cast<int32_t>(OldOpnd->getImm()) ==
        std::numeric_limits<int32_t>::max())
      return true;
    break;
  case RVGPU::V_MAX_I32_e32:
  case RVGPU::V_MAX_I32_e64:
    if (static_cast<int32_t>(OldOpnd->getImm()) ==
        std::numeric_limits<int32_t>::min())
      return true;
    break;
  case RVGPU::V_MUL_I32_I24_e32:
  case RVGPU::V_MUL_I32_I24_e64:
  case RVGPU::V_MUL_U32_U24_e32:
  case RVGPU::V_MUL_U32_U24_e64:
    if (OldOpnd->getImm() == 1)
      return true;
    break;
  }
...
MachineInstr *GCNDPPCombine::createDPPInst(
  ...
  if (!CombBCZ && OldOpndValue && OldOpndValue->isImm()) {
    ...
    auto MovDst = TII->getNamedOperand(MovMI, RVGPU::OpName::vdst);
...
bool GCNDPPCombine::combineDPPMov(MachineInstr &MovMI) const {
  ...
  auto *DstOpnd = TII->getNamedOperand(MovMI, RVGPU::OpName::vdst);
  ...
  if (MovMI.getOpcode() == RVGPU::V_MOV_B64_DPP_PSEUDO ||
      MovMI.getOpcode() == RVGPU::V_MOV_B64_dpp) {
    auto *DppCtrl = TII->getNamedOperand(MovMI, RVGPU::OpName::dpp_ctrl);
    assert(DppCtrl && DppCtrl->isImm());
    if (!RVGPU::isLegalDPALU_DPPControl(DppCtrl->getImm())) {
  ...
  auto *BCZOpnd = TII->getNamedOperand(MovMI, RVGPU::OpName::bound_ctrl);
  assert(BCZOpnd && BCZOpnd->isImm());
  bool BoundCtrlZero = BCZOpnd->getImm();

  auto *OldOpnd = TII->getNamedOperand(MovMI, RVGPU::OpName::old);
  auto *SrcOpnd = TII->getNamedOperand(MovMI, RVGPU::OpName::src0);
  ...
  if (CombBCZ && OldOpndValue) { // CombOldVGPR should be undef
    ...
    auto UndefInst = BuildMI(*MovMI.getParent(), MovMI, MovMI.getDebugLoc(),
                             TII->get(RVGPU::IMPLICIT_DEF), CombOldVGPR.Reg);
  ...
  while (!Uses.empty()) {
    ...
    auto OrigOp = OrigMI.getOpcode();
    assert((TII->get(OrigOp).getSize() != 4 || !RVGPU::isTrue16Inst(OrigOp)) &&
           "There should not be e32 True16 instructions pre-RA");
    if (OrigOp == RVGPU::REG_SEQUENCE) {
    ...
    if (OrigMI.modifiesRegister(RVGPU::EXEC, ST->getRegisterInfo())) {
      LLVM_DEBUG(dbgs() << "  failed: can't combine v_cmpx\n");
      break;
    }

    auto *Src0 = TII->getNamedOperand(OrigMI, RVGPU::OpName::src0);
    auto *Src1 = TII->getNamedOperand(OrigMI, RVGPU::OpName::src1);
...
bool GCNDPPCombine::runOnMachineFunction(MachineFunction &MF) {
  ...
  for (auto &MBB : MF) {
    for (MachineInstr &MI : llvm::make_early_inc_range(llvm::reverse(MBB))) {
      if (MI.getOpcode() == RVGPU::V_MOV_B32_dpp && combineDPPMov(MI)) {
        Changed = true;
        ++NumDPPMovsCombined;
      } else if (MI.getOpcode() == RVGPU::V_MOV_B64_DPP_PSEUDO ||
                 MI.getOpcode() == RVGPU::V_MOV_B64_dpp) {
