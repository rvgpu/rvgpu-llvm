//===-- RVGPUISelDAGToDAG.h - A dag to dag inst selector for RVGPU ----===//
//
...
static inline SDNode *packConstantV2I16(const SDNode *N, SelectionDAG &DAG,
  ...
  if (getConstantValue(N->getOperand(0), LHSVal) &&
    ...
    return DAG.getMachineNode(RVGPU::S_MOV_B32, SL, N->getValueType(0),
...
/// RVGPU specific code to select RVGPU machine instructions for
/// SelectionDAG operations.
class RVGPUDAGToDAGISel : public SelectionDAGISel {
  // Subtarget - Keep a pointer to the RVGPU Subtarget around so that we can
...
public:
  ...
  RVGPUDAGToDAGISel() = delete;

  explicit RVGPUDAGToDAGISel(TargetMachine &TM, CodeGenOptLevel OptLevel);
  ~RVGPUDAGToDAGISel() override = default;
...
private:
  ...
  bool isInlineImmediate16(int64_t Imm) const {
    return RVGPU::isInlinableLiteral16(Imm, Subtarget->hasInv2PiInlineImm());
  }

  bool isInlineImmediate32(int64_t Imm) const {
    return RVGPU::isInlinableLiteral32(Imm, Subtarget->hasInv2PiInlineImm());
  }

  bool isInlineImmediate64(int64_t Imm) const {
    return RVGPU::isInlinableLiteral64(Imm, Subtarget->hasInv2PiInlineImm());
  }
...
#include "RVGPUGenDAGISel.inc"
};

#endif // LLVM_LIB_TARGET_RVGPU_RVGPUISELDAGTODAG_H
