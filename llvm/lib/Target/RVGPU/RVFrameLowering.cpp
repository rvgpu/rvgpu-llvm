...
static void getVGPRSpillLaneOrTempRegister(
    ...
    const TargetRegisterClass &RC = RVGPU::SReg_32_XM0_XEXECRegClass,
...
// use.
static void buildPrologSpill(const RVSubtarget &ST, const RVRegisterInfo &TRI,
                             const RVMachineFunctionInfo &FuncInfo,
...
static void buildEpilogRestore(const RVSubtarget &ST,
...
static void buildGitPtr(MachineBasicBlock &MBB, MachineBasicBlock::iterator I,
                        const DebugLoc &DL, const RVInstrInfo *TII,
                        Register TargetReg) {
  MachineFunction *MF = MBB.getParent();
  const RVMachineFunctionInfo *MFI = MF->getInfo<RVMachineFunctionInfo>();
  const RVRegisterInfo *TRI = &TII->getRegisterInfo();
  const MCInstrDesc &SMovB32 = TII->get(RVGPU::S_MOV_B32);
  Register TargetLo = TRI->getSubReg(TargetReg, RVGPU::sub0);
  Register TargetHi = TRI->getSubReg(TargetReg, RVGPU::sub1);
  ...
  if (MFI->getGITPtrHigh() != 0xffffffff) {
    ...
    const MCInstrDesc &GetPC64 = TII->get(RVGPU::S_GETPC_B64);
...
static void initLiveUnits(LiveRegUnits &LiveUnits, const RVRegisterInfo &TRI,
                          const RVMachineFunctionInfo *FuncInfo,
...
class PrologEpilogSGPRSpillBuilder {
  ...
  RVMachineFunctionInfo *FuncInfo;
  ...
  void saveToMemory(const int FI) const {
    ...
    MCPhysReg TmpVGPR = findScratchNonCalleeSaveRegister(
        MRI, LiveUnits, RVGPU::VGPR_32RegClass);
    if (!TmpVGPR)
  ...
  void saveToVGPRLane(const int FI) const {
    ...
    ArrayRef<RVRegisterInfo::SpilledReg> Spill =
    ...
    for (unsigned I = 0; I < NumSubRegs; ++I) {
      Register SubReg = NumSubRegs == 1
                            ? SuperReg
                            : Register(TRI.getSubReg(SuperReg, SplitParts[I]));
      BuildMI(MBB, MI, DL, TII->get(RVGPU::SI_SPILL_S32_TO_VGPR),
              Spill[I].VGPR)
          .addReg(SubReg)
          .addImm(Spill[I].Lane)
  ...
  void copyToScratchSGPR(Register DstReg) const {
    BuildMI(MBB, MI, DL, TII->get(RVGPU::COPY), DstReg)
        .addReg(SuperReg)
  ...
  void restoreFromMemory(const int FI) {
    ...
    for (unsigned I = 0, DwordOff = 0; I < NumSubRegs; ++I) {
      ...
      BuildMI(MBB, MI, DL, TII->get(RVGPU::V_READFIRSTLANE_B32), SubReg)
  ...
  void restoreFromVGPRLane(const int FI) {
    ...
    for (unsigned I = 0; I < NumSubRegs; ++I) {
      Register SubReg = NumSubRegs == 1
                            ? SuperReg
                            : Register(TRI.getSubReg(SuperReg, SplitParts[I]));
      BuildMI(MBB, MI, DL, TII->get(RVGPU::SI_RESTORE_S32_FROM_VGPR), SubReg)
          .addReg(Spill[I].VGPR)
          .addImm(Spill[I].Lane);
    }
  }
  ...
  void copyFromScratchSGPR(Register SrcReg) const {
    BuildMI(MBB, MI, DL, TII->get(RVGPU::COPY), SuperReg)
        .addReg(SrcReg)
...
public:
  PrologEpilogSGPRSpillBuilder(Register Reg,
      ...
      : MI(MI), MBB(MBB), MF(*MBB.getParent()),
        ST(MF.getSubtarget<RVSubtarget>()), MFI(MF.getFrameInfo()),
        FuncInfo(MF.getInfo<RVMachineFunctionInfo>()), TII(TII), TRI(TRI),
...
void RVFrameLowering::emitEntryFunctionFlatScratchInit(
  ...
  if (ST.isRvPalOS()) {
    ...
    MachineRegisterInfo &MRI = MF.getRegInfo();
    Register FlatScrInit = RVGPU::NoRegister;
    ...
    FlatScrInitLo = TRI->getSubReg(FlatScrInit, RVGPU::sub0);
    FlatScrInitHi = TRI->getSubReg(FlatScrInit, RVGPU::sub1);
    ...
    const MCInstrDesc &LoadDwordX2 = TII->get(RVGPU::S_LOAD_DWORDX2_IMM);
    auto *MMO = MF.getMachineMemOperand(
        ...
        8, Align(4));
    unsigned Offset =
        MF.getFunction().getCallingConv() == CallingConv::RVGPU_CS ? 16 : 0;
    const RVSubtarget &Subtarget = MF.getSubtarget<RVSubtarget>();
    unsigned EncodedOffset = RVGPU::convertSMRDOffsetUnits(Subtarget, Offset);
    ...
    const MCInstrDesc &SAndB32 = TII->get(RVGPU::S_AND_B32);
    ...
    Register FlatScratchInitReg =
        MFI->getPreloadedReg(RVGPUFunctionArgInfo::FLAT_SCRATCH_INIT);
    ...
    FlatScrInitLo = TRI->getSubReg(FlatScratchInitReg, RVGPU::sub0);
    FlatScrInitHi = TRI->getSubReg(FlatScratchInitReg, RVGPU::sub1);
  }

  // Do a 64-bit pointer add.
  if (ST.flatScratchIsPointer()) {
    if (ST.getGeneration() >= RVGPUSubtarget::GFX10) {
      BuildMI(MBB, I, DL, TII->get(RVGPU::S_ADD_U32), FlatScrInitLo)
        .addReg(FlatScrInitLo)
        .addReg(ScratchWaveOffsetReg);
      auto Addc = BuildMI(MBB, I, DL, TII->get(RVGPU::S_ADDC_U32),
                          FlatScrInitHi)
        .addReg(FlatScrInitHi)
        .addImm(0);
      Addc->getOperand(3).setIsDead(); // Mark SCC as dead.

      BuildMI(MBB, I, DL, TII->get(RVGPU::S_SETREG_B32)).
        addReg(FlatScrInitLo).
        addImm(int16_t(RVGPU::Hwreg::ID_FLAT_SCR_LO |
                       (31 << RVGPU::Hwreg::WIDTH_M1_SHIFT_)));
      BuildMI(MBB, I, DL, TII->get(RVGPU::S_SETREG_B32)).
        addReg(FlatScrInitHi).
        addImm(int16_t(RVGPU::Hwreg::ID_FLAT_SCR_HI |
                       (31 << RVGPU::Hwreg::WIDTH_M1_SHIFT_)));
      return;
    }

    // For GFX9.
    BuildMI(MBB, I, DL, TII->get(RVGPU::S_ADD_U32), RVGPU::FLAT_SCR_LO)
      .addReg(FlatScrInitLo)
      .addReg(ScratchWaveOffsetReg);
    auto Addc = BuildMI(MBB, I, DL, TII->get(RVGPU::S_ADDC_U32),
                        RVGPU::FLAT_SCR_HI)
  ...
  assert(ST.getGeneration() < RVGPUSubtarget::GFX9);

  // Copy the size in bytes.
  BuildMI(MBB, I, DL, TII->get(RVGPU::COPY), RVGPU::FLAT_SCR_LO)
    .addReg(FlatScrInitHi, RegState::Kill);

  // Add wave offset in bytes to private base offset.
  // See comment in RVKernelCodeT.h for enable_sgpr_flat_scratch_init.
  BuildMI(MBB, I, DL, TII->get(RVGPU::S_ADD_I32), FlatScrInitLo)
      .addReg(FlatScrInitLo)
      .addReg(ScratchWaveOffsetReg);

  // Convert offset to 256-byte units.
  auto LShr = BuildMI(MBB, I, DL, TII->get(RVGPU::S_LSHR_B32),
                      RVGPU::FLAT_SCR_HI)
...
Register RVFrameLowering::getEntryFunctionReservedScratchRsrcReg(
...
static unsigned getScratchScaleFactor(const RVSubtarget &ST) {
  return ST.enableFlatScratch() ? 1 : ST.getWavefrontSize();
}

void RVFrameLowering::emitEntryFunctionPrologue(MachineFunction &MF,
  ...
  Register PreloadedScratchWaveOffsetReg = MFI->getPreloadedReg(
      RVGPUFunctionArgInfo::PRIVATE_SEGMENT_WAVE_BYTE_OFFSET);
  ...
  if (ST.isRvHsaOrMesa(F)) {
    PreloadedScratchRsrcReg =
        MFI->getPreloadedReg(RVGPUFunctionArgInfo::PRIVATE_SEGMENT_BUFFER);
  ...
  // chosen by RVFrameLowering::allocateSystemSGPRs, COPY the scratch
  ...
  if (PreloadedScratchWaveOffsetReg &&
    ...
    for (MCPhysReg Reg : AllSGPRs) {
      if (!MRI.isPhysRegUsed(Reg) && MRI.isAllocatable(Reg) &&
        ...
        BuildMI(MBB, I, DL, TII->get(RVGPU::COPY), ScratchWaveOffsetReg)
  ...
  if (requiresStackPointerReference(MF)) {
    ...
    assert(SPReg != RVGPU::SP_REG);
    BuildMI(MBB, I, DL, TII->get(RVGPU::S_MOV_B32), SPReg)
        .addImm(FrameInfo.getStackSize() * getScratchScaleFactor(ST));
  }

  if (hasFP(MF)) {
    Register FPReg = MFI->getFrameOffsetReg();
    assert(FPReg != RVGPU::FP_REG);
    BuildMI(MBB, I, DL, TII->get(RVGPU::S_MOV_B32), FPReg).addImm(0);
  }

  bool NeedsFlatScratchInit =
      MFI->getUserSGPRInfo().hasFlatScratchInit() &&
      (MRI.isPhysRegUsed(RVGPU::FLAT_SCR) || FrameInfo.hasCalls() ||
...
// Emit scratch RSRC setup code, assuming `ScratchRsrcReg != RVGPU::NoReg`
void RVFrameLowering::emitEntryFunctionScratchRsrcRegSetup(
    MachineFunction &MF, MachineBasicBlock &MBB, MachineBasicBlock::iterator I,
  ...
  if (ST.isRvPalOS()) {
    ...
    // the rvgpu-git-ptr-high function attribute or the top part of the PC
    Register Rsrc01 = TRI->getSubReg(ScratchRsrcReg, RVGPU::sub0_sub1);
    Register Rsrc03 = TRI->getSubReg(ScratchRsrcReg, RVGPU::sub3);
    ...
    // at offset 0 (or offset 16 for a compute shader).
    MachinePointerInfo PtrInfo(RVGPUAS::CONSTANT_ADDRESS);
    const MCInstrDesc &LoadDwordX4 = TII->get(RVGPU::S_LOAD_DWORDX4_IMM);
    auto MMO = MF.getMachineMemOperand(PtrInfo,
                                       MachineMemOperand::MOLoad |
                                           MachineMemOperand::MOInvariant |
                                           MachineMemOperand::MODereferenceable,
                                       16, Align(4));
    unsigned Offset = Fn.getCallingConv() == CallingConv::RVGPU_CS ? 16 : 0;
    const RVSubtarget &Subtarget = MF.getSubtarget<RVSubtarget>();
    unsigned EncodedOffset = RVGPU::convertSMRDOffsetUnits(Subtarget, Offset);
    ...
    if (ST.isWave32()) {
      const MCInstrDesc &SBitsetB32 = TII->get(RVGPU::S_BITSET0_B32);
      BuildMI(MBB, I, DL, SBitsetB32, Rsrc03)
          .addImm(21)
          .addReg(Rsrc03);
    }
  } else if (ST.isMesaGfxShader(Fn) || !PreloadedScratchRsrcReg) {
    assert(!ST.isRvHsaOrMesa(Fn));
    const MCInstrDesc &SMovB32 = TII->get(RVGPU::S_MOV_B32);

    Register Rsrc2 = TRI->getSubReg(ScratchRsrcReg, RVGPU::sub2);
    Register Rsrc3 = TRI->getSubReg(ScratchRsrcReg, RVGPU::sub3);
    ...
    if (MFI->getUserSGPRInfo().hasImplicitBufferPtr()) {
      ...
      if (RVGPU::isCompute(MF.getFunction().getCallingConv())) {
        const MCInstrDesc &Mov64 = TII->get(RVGPU::S_MOV_B64);
      ...
      Register Rsrc0 = TRI->getSubReg(ScratchRsrcReg, RVGPU::sub0);
      Register Rsrc1 = TRI->getSubReg(ScratchRsrcReg, RVGPU::sub1);
    ...
    BuildMI(MBB, I, DL, SMovB32, Rsrc3)
      .addImm(Rsrc23 >> 32)
      .addReg(ScratchRsrcReg, RegState::ImplicitDefine);
  } else if (ST.isRvHsaOrMesa(Fn)) {
    assert(PreloadedScratchRsrcReg);

    if (ScratchRsrcReg != PreloadedScratchRsrcReg) {
      BuildMI(MBB, I, DL, TII->get(RVGPU::COPY), ScratchRsrcReg)
  ...
  Register ScratchRsrcSub0 = TRI->getSubReg(ScratchRsrcReg, RVGPU::sub0);
  Register ScratchRsrcSub1 = TRI->getSubReg(ScratchRsrcReg, RVGPU::sub1);

  // We cannot Kill ScratchWaveOffsetReg here because we allow it to be used in
  // the kernel body via inreg arguments.
  BuildMI(MBB, I, DL, TII->get(RVGPU::S_ADD_U32), ScratchRsrcSub0)
      .addReg(ScratchRsrcSub0)
      .addReg(ScratchWaveOffsetReg)
      .addReg(ScratchRsrcReg, RegState::ImplicitDefine);
  auto Addc = BuildMI(MBB, I, DL, TII->get(RVGPU::S_ADDC_U32), ScratchRsrcSub1)
...
bool RVFrameLowering::isSupportedStackID(TargetStackID::Value ID) const {
  switch (ID) {
...
static Register buildScratchExecCopy(LiveRegUnits &LiveUnits,
  ...
  const RVInstrInfo *TII = ST.getInstrInfo();
  const RVRegisterInfo &TRI = TII->getRegisterInfo();
  RVMachineFunctionInfo *FuncInfo = MF.getInfo<RVMachineFunctionInfo>();
  ...
  const unsigned SaveExecOpc =
      ST.isWave32() ? (EnableInactiveLanes ? RVGPU::S_XOR_SAVEEXEC_B32
                                           : RVGPU::S_OR_SAVEEXEC_B32)
                    : (EnableInactiveLanes ? RVGPU::S_XOR_SAVEEXEC_B64
...
void RVFrameLowering::emitCSRSpillStores(
    MachineFunction &MF, MachineBasicBlock &MBB,
  ...
  if (!WWMCalleeSavedRegs.empty()) {
    if (ScratchExecCopy) {
      unsigned MovOpc = ST.isWave32() ? RVGPU::S_MOV_B32 : RVGPU::S_MOV_B64;
  ...
  if (ScratchExecCopy) {
    ...
    unsigned ExecMov = ST.isWave32() ? RVGPU::S_MOV_B32 : RVGPU::S_MOV_B64;
...
void RVFrameLowering::emitCSRSpillRestores(
    MachineFunction &MF, MachineBasicBlock &MBB,
    MachineBasicBlock::iterator MBBI, DebugLoc &DL, LiveRegUnits &LiveUnits,
    Register FrameReg, Register FramePtrRegScratchCopy) const {
...
void RVFrameLowering::emitPrologue(MachineFunction &MF,
                                   MachineBasicBlock &MBB) const {
  RVMachineFunctionInfo *FuncInfo = MF.getInfo<RVMachineFunctionInfo>();
  ...
  if (FuncInfo->isChainFunction()) {
    // Functions with the rvgpu_cs_chain[_preserve] CC don't receive a SP, but
    // are free to set one up if they need it.
    bool UseSP = requiresStackPointerReference(MF);
    if (UseSP) {
      assert(StackPtrReg != RVGPU::SP_REG);

      BuildMI(MBB, MBBI, DL, TII->get(RVGPU::S_MOV_B32), StackPtrReg)
  ...
  if (!HasFP && !hasFP(MF)) {
    ...
    if (SGPRForFPSaveRestoreCopy) {
      ...
      FramePtrRegScratchCopy = findScratchNonCalleeSaveRegister(
          MRI, LiveUnits, RVGPU::SReg_32_XM0_XEXECRegClass);
      if (!FramePtrRegScratchCopy)
        report_fatal_error("failed to find free scratch register");

      LiveUnits.addReg(FramePtrRegScratchCopy);
      BuildMI(MBB, MBBI, DL, TII->get(RVGPU::COPY), FramePtrRegScratchCopy)
          .addReg(FramePtrReg);
    }
  }
  ...
  if (HasFP) {
    ...
    BuildMI(MBB, MBBI, DL, TII->get(RVGPU::S_ADD_I32), FramePtrReg)
        .addReg(StackPtrReg)
        .addImm((Alignment - 1) * getScratchScaleFactor(ST))
        .setMIFlag(MachineInstr::FrameSetup);
    auto And = BuildMI(MBB, MBBI, DL, TII->get(RVGPU::S_AND_B32), FramePtrReg)
    ...
    BuildMI(MBB, MBBI, DL, TII->get(RVGPU::COPY), FramePtrReg)
        .addReg(StackPtrReg)
        .setMIFlag(MachineInstr::FrameSetup);
  }
  ...
  if ((HasBP = TRI.hasBasePointer(MF))) {
    BuildMI(MBB, MBBI, DL, TII->get(RVGPU::COPY), BasePtrReg)
        .addReg(StackPtrReg)
        .setMIFlag(MachineInstr::FrameSetup);
  }

  if (HasFP && RoundedSize != 0) {
    auto Add = BuildMI(MBB, MBBI, DL, TII->get(RVGPU::S_ADD_I32), StackPtrReg)
        .addReg(StackPtrReg)
...
void RVFrameLowering::emitEpilogue(MachineFunction &MF,
                                   MachineBasicBlock &MBB) const {
  const RVMachineFunctionInfo *FuncInfo = MF.getInfo<RVMachineFunctionInfo>();
  if (FuncInfo->isEntryFunction())
    return;
...
StackOffset RVFrameLowering::getFrameIndexReference(const MachineFunction &MF,
                                                    int FI,
                                                    Register &FrameReg) const {
  const RVRegisterInfo *RI = MF.getSubtarget<RVSubtarget>().getRegisterInfo();

  FrameReg = RI->getFrameRegister(MF);
  return StackOffset::getFixed(MF.getFrameInfo().getObjectOffset(FI));
}

void RVFrameLowering::processFunctionBeforeFrameFinalized(
  MachineFunction &MF,
    ...
    for (MachineBasicBlock &MBB : MF) {
      for (MachineInstr &MI : llvm::make_early_inc_range(MBB)) {
        ...
        if (TII->isVGPRSpill(MI)) {
          ...
          // finalization.
          unsigned FIOp = RVGPU::getNamedOperandIdx(MI.getOpcode(),
                                                     RVGPU::OpName::vaddr);
          int FI = MI.getOperand(FIOp).getIndex();
          Register VReg =
            TII->getNamedOperand(MI, RVGPU::OpName::vdata)->getReg();
...
void RVFrameLowering::processFunctionBeforeFrameIndicesReplaced(
  ...
  if (ST.hasMAIInsts() && !ST.hasGFX90AInsts()) {
    ...
    Register UnusedLowVGPR =
        TRI->findUnusedRegister(MRI, &RVGPU::VGPR_32RegClass, MF);
  ...
  Register UnusedLowSGPR =
      TRI->findUnusedRegister(MRI, &RVGPU::SGPR_64RegClass, MF);
...
void RVFrameLowering::determinePrologEpilogSGPRSaves(
...
void RVFrameLowering::determineCalleeSaves(MachineFunction &MF,
                                           BitVector &SavedVGPRs,
                                           RegScavenger *RS) const {
  RVMachineFunctionInfo *MFI = MF.getInfo<RVMachineFunctionInfo>();

  // If this is a function with the rvgpu_cs_chain[_preserve] calling
  // convention and it doesn't contain any calls to llvm.rvgpu.cs.chain, then
  ...
  for (MachineBasicBlock &MBB : MF) {
    for (MachineInstr &MI : MBB) {
      ...
      if (MI.getOpcode() == RVGPU::SI_SPILL_S32_TO_VGPR)
        MFI->allocateWWMSpill(MF, MI.getOperand(0).getReg());
      else if (MI.getOpcode() == RVGPU::SI_RESTORE_S32_FROM_VGPR)
        MFI->allocateWWMSpill(MF, MI.getOperand(1).getReg());
      else if (TII->isWWMRegSpillOpcode(MI.getOpcode()))
        NeedExecCopyReservedReg = true;
      else if (MI.getOpcode() == RVGPU::SI_RETURN ||
               MI.getOpcode() == RVGPU::SI_RETURN_TO_EPILOG ||
               (MFI->isChainFunction() &&
...
void RVFrameLowering::determineCalleeSavesSGPR(MachineFunction &MF,
                                               BitVector &SavedRegs,
                                               RegScavenger *RS) const {
  ...
  if (!MFI->isEntryFunction() &&
    ...
    SavedRegs.set(TRI->getSubReg(RetAddrReg, RVGPU::sub0));
    SavedRegs.set(TRI->getSubReg(RetAddrReg, RVGPU::sub1));
  }
}

bool RVFrameLowering::assignCalleeSavedSpillSlots(
...
bool RVFrameLowering::allocateScavengingFrameIndexesNearIncomingSP(
  ...
  if (ST.enableFlatScratch()) {
    if (TII->isLegalFLATOffset(MaxOffset, RVGPUAS::PRIVATE_ADDRESS,
...
MachineBasicBlock::iterator RVFrameLowering::eliminateCallFramePseudoInstr(
  MachineFunction &MF,
  MachineBasicBlock &MBB,
  ...
  if (!hasReservedCallFrame(MF)) {
    ...
    if (IsDestroy)
      Amount = -Amount;
    auto Add = BuildMI(MBB, I, DL, TII->get(RVGPU::S_ADD_I32), SPReg)
        .addReg(SPReg)
        .addImm(Amount);
    Add->getOperand(3).setIsDead(); // Mark SCC as dead.
...
bool RVFrameLowering::hasFP(const MachineFunction &MF) const {
  const MachineFrameInfo &MFI = MF.getFrameInfo();
  ...
  if (MFI.hasCalls() &&
      !MF.getInfo<RVMachineFunctionInfo>()->isEntryFunction() &&
      !MF.getInfo<RVMachineFunctionInfo>()->isChainFunction()) {
  ...
  return frameTriviallyRequiresSP(MFI) || MFI.isFrameAddressTaken() ||
         MF.getSubtarget<RVSubtarget>().getRegisterInfo()->hasStackRealignment(
             MF) ||
...
bool RVFrameLowering::requiresStackPointerReference(
    const MachineFunction &MF) const {
  // Callable functions always require a stack pointer reference.
  assert((MF.getInfo<RVMachineFunctionInfo>()->isEntryFunction() ||
          MF.getInfo<RVMachineFunctionInfo>()->isChainFunction()) &&
