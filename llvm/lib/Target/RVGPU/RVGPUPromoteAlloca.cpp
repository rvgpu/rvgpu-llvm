//===-- RVGPUPromoteAlloca.cpp - Promote Allocas -------------------------===//
//
...
// declared in RVGPU.h and the legacy PM ones are declared here.s
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "rvgpu-promote-alloca"
...
static cl::opt<unsigned> PromoteAllocaToVectorLimit(
    "rvgpu-promote-alloca-to-vector-limit",
    cl::desc("Maximum byte size to consider promote alloca to vector"),
    cl::init(0));

// Shared implementation which can do both promotion to vector and to LDS.
class RVGPUPromoteAllocaImpl {
private:
...
class RVGPUPromoteAlloca : public FunctionPass {
public:
  static char ID;

  RVGPUPromoteAlloca() : FunctionPass(ID) {}

  bool runOnFunction(Function &F) override {
    if (skipFunction(F))
      return false;
    if (auto *TPC = getAnalysisIfAvailable<TargetPassConfig>())
      return RVGPUPromoteAllocaImpl(TPC->getTM<TargetMachine>())
          .run(F, /*PromoteToLDS*/ true);
    return false;
  }

  StringRef getPassName() const override { return "RVGPU Promote Alloca"; }
...
class RVGPUPromoteAllocaToVector : public FunctionPass {
public:
  static char ID;

  RVGPUPromoteAllocaToVector() : FunctionPass(ID) {}
  ...
  StringRef getPassName() const override {
    return "RVGPU Promote Alloca to vector";
  }
...
char RVGPUPromoteAlloca::ID = 0;
char RVGPUPromoteAllocaToVector::ID = 0;

INITIALIZE_PASS_BEGIN(RVGPUPromoteAlloca, DEBUG_TYPE,
                      "RVGPU promote alloca to vector or LDS", false, false)
// Move LDS uses from functions to kernels before promote alloca for accurate
// estimation of LDS available
INITIALIZE_PASS_DEPENDENCY(RVGPULowerModuleLDSLegacy)
INITIALIZE_PASS_END(RVGPUPromoteAlloca, DEBUG_TYPE,
                    "RVGPU promote alloca to vector or LDS", false, false)

INITIALIZE_PASS(RVGPUPromoteAllocaToVector, DEBUG_TYPE "-to-vector",
                "RVGPU promote alloca to vector", false, false)

char &llvm::RVGPUPromoteAllocaID = RVGPUPromoteAlloca::ID;
char &llvm::RVGPUPromoteAllocaToVectorID = RVGPUPromoteAllocaToVector::ID;

PreservedAnalyses RVGPUPromoteAllocaPass::run(Function &F,
                                               FunctionAnalysisManager &AM) {
  bool Changed = RVGPUPromoteAllocaImpl(TM).run(F, /*PromoteToLDS*/ true);
  if (Changed) {
...
PreservedAnalyses
RVGPUPromoteAllocaToVectorPass::run(Function &F, FunctionAnalysisManager &AM) {
  bool Changed = RVGPUPromoteAllocaImpl(TM).run(F, /*PromoteToLDS*/ false);
  if (Changed) {
    PreservedAnalyses PA;
    PA.preserveSet<CFGAnalyses>();
    return PA;
...
FunctionPass *llvm::createRVGPUPromoteAlloca() {
  return new RVGPUPromoteAlloca();
}

FunctionPass *llvm::createRVGPUPromoteAllocaToVector() {
  return new RVGPUPromoteAllocaToVector();
}

bool RVGPUPromoteAllocaImpl::run(Function &F, bool PromoteToLDS) {
  Mod = F.getParent();
  DL = &Mod->getDataLayout();

  const RVGPUSubtarget &ST = RVGPUSubtarget::get(TM, F);
...
bool RVGPUPromoteAllocaImpl::tryPromoteAllocaToVector(AllocaInst &Alloca) {
...
RVGPUPromoteAllocaImpl::getLocalSizeYZ(IRBuilder<> &Builder) {
  ...
  Function *DispatchPtrFn =
      Intrinsic::getDeclaration(Mod, Intrinsic::rvgpu_dispatch_ptr);

  CallInst *DispatchPtr = Builder.CreateCall(DispatchPtrFn, {});
  DispatchPtr->addRetAttr(Attribute::NoAlias);
  DispatchPtr->addRetAttr(Attribute::NonNull);
  F.removeFnAttr("rvgpu-no-dispatch-ptr");
  ...
  Value *CastDispatchPtr = Builder.CreateBitCast(
      DispatchPtr, PointerType::get(I32Ty, RVGPUAS::CONSTANT_ADDRESS));
...
Value *RVGPUPromoteAllocaImpl::getWorkitemID(IRBuilder<> &Builder,
                                              unsigned N) {
  Function *F = Builder.GetInsertBlock()->getParent();
  const RVGPUSubtarget &ST = RVGPUSubtarget::get(TM, *F);
  ...
  switch (N) {
  case 0:
    IntrID = (Intrinsic::ID)Intrinsic::rvgpu_workitem_id_x;
    AttrName = "rvgpu-no-workitem-id-x";
    break;
  case 1:
    IntrID = (Intrinsic::ID)Intrinsic::rvgpu_workitem_id_y;
    AttrName = "rvgpu-no-workitem-id-y";
    break;

  case 2:
    IntrID = (Intrinsic::ID)Intrinsic::rvgpu_workitem_id_z;
    AttrName = "rvgpu-no-workitem-id-z";
    break;
  default:
...
bool RVGPUPromoteAllocaImpl::binaryOpIsDerivedFromSameAlloca(
...
bool RVGPUPromoteAllocaImpl::collectUsesWithPtrTypes(
...
bool RVGPUPromoteAllocaImpl::hasSufficientLocalMem(const Function &F) {
  ...
  for (Type *ParamTy : FTy->params()) {
    ...
    if (PtrTy && PtrTy->getAddressSpace() == RVGPUAS::LOCAL_ADDRESS) {
      LocalMemLimit = 0;
...
// FIXME: Should try to pick the most likely to be profitable allocas first.
bool RVGPUPromoteAllocaImpl::tryPromoteAllocaToLDS(AllocaInst &I,
                                                    bool SufficientLDS) {
  ...
  const RVGPUSubtarget &ST = RVGPUSubtarget::get(TM, ContainingFunction);
  ...
  GlobalVariable *GV = new GlobalVariable(
      *Mod, GVTy, false, GlobalValue::InternalLinkage, PoisonValue::get(GVTy),
      ...
      GlobalVariable::NotThreadLocal, RVGPUAS::LOCAL_ADDRESS);
  ...
  for (Value *V : WorkList) {
    ...
    if (!Call) {
      if (ICmpInst *CI = dyn_cast<ICmpInst>(V)) {
        PointerType *NewTy = PointerType::get(Context, RVGPUAS::LOCAL_ADDRESS);
    ...
    case Intrinsic::objectsize: {
      ...
      Function *ObjectSize = Intrinsic::getDeclaration(
          ...
          {Intr->getType(),
           PointerType::get(Context, RVGPUAS::LOCAL_ADDRESS)});
