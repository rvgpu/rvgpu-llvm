//===-- RVRegisterInfo.cpp - SI Register Information ---------------------===//
//
...
static cl::opt<bool> EnableSpillSGPRToVGPR(
  "rvgpu-spill-sgpr-to-vgpr",
  cl::desc("Enable spilling SGPRs to VGPRs"),
  cl::ReallyHidden,
  cl::init(true));

std::array<std::vector<int16_t>, 16> RVRegisterInfo::RegSplitParts;
std::array<std::array<uint16_t, 32>, 9> RVRegisterInfo::SubRegFromChannelTable;
...
struct SGPRSpillBuilder {
  ...
  Register TmpVGPR = RVGPU::NoRegister;
  ...
  Register SavedExecReg = RVGPU::NoRegister;
  ...
  SGPRSpillBuilder(const RVRegisterInfo &TRI, const RVInstrInfo &TII,
  ...
  SGPRSpillBuilder(const RVRegisterInfo &TRI, const RVInstrInfo &TII,
      ...
      : SuperReg(Reg), MI(MI), IsKill(IsKill), DL(MI->getDebugLoc()),
        ...
        MFI(*MF.getInfo<RVMachineFunctionInfo>()), TII(TII), TRI(TRI),
        IsWave32(IsWave32) {
    ...
    if (IsWave32) {
      ExecReg = RVGPU::EXEC_LO;
      MovOpc = RVGPU::S_MOV_B32;
      NotOpc = RVGPU::S_NOT_B32;
    } else {
      ExecReg = RVGPU::EXEC;
      MovOpc = RVGPU::S_MOV_B64;
      NotOpc = RVGPU::S_NOT_B64;
    }

    assert(SuperReg != RVGPU::M0 && "m0 should never spill");
    assert(SuperReg != RVGPU::EXEC_LO && SuperReg != RVGPU::EXEC_HI &&
           SuperReg != RVGPU::EXEC && "exec should never spill");
  }
  ...
  void prepare() {
    ...
    TmpVGPR = RS->scavengeRegisterBackwards(RVGPU::VGPR_32RegClass, MI, false,
                                            0, false);
    ...
    const TargetRegisterClass &RC =
        IsWave32 ? RVGPU::SGPR_32RegClass : RVGPU::SGPR_64RegClass;
...
RVRegisterInfo::RVRegisterInfo(const GCNSubtarget &ST)
  ...
  assert(getSubRegIndexLaneMask(RVGPU::sub0).getAsInteger() == 3 &&
         getSubRegIndexLaneMask(RVGPU::sub31).getAsInteger() == (3ULL << 62) &&
         (getSubRegIndexLaneMask(RVGPU::lo16) |
          getSubRegIndexLaneMask(RVGPU::hi16)).getAsInteger() ==
           getSubRegIndexLaneMask(RVGPU::sub0).getAsInteger() &&
         "getNumCoveredRegs() will not work with generated subreg masks!");

  RegPressureIgnoredUnits.resize(getNumRegUnits());
  RegPressureIgnoredUnits.set(*regunits(MCRegister::from(RVGPU::M0)).begin());
  for (auto Reg : RVGPU::VGPR_HI16RegClass)
  ...
  static auto InitializeSubRegFromChannelTableOnce = [this]() {
    for (auto &Row : SubRegFromChannelTable)
      Row.fill(RVGPU::NoSubRegister);
    for (unsigned Idx = 1; Idx < getNumSubRegIndices(); ++Idx) {
      unsigned Width = RVGPUSubRegIdxRanges[Idx].Size / 32;
      unsigned Offset = RVGPUSubRegIdxRanges[Idx].Offset / 32;
...
void RVRegisterInfo::reserveRegisterTuples(BitVector &Reserved,
...
const MCPhysReg *RVRegisterInfo::getCalleeSavedRegs(
  ...
  case CallingConv::Cold:
    return ST.hasGFX90AInsts() ? CSR_RVGPU_GFX90AInsts_SaveList
                               : CSR_RVGPU_SaveList;
  case CallingConv::RVGPU_Gfx:
    return ST.hasGFX90AInsts() ? CSR_RVGPU_SI_Gfx_GFX90AInsts_SaveList
                               : CSR_RVGPU_SI_Gfx_SaveList;
  case CallingConv::RVGPU_CS_ChainPreserve:
    return CSR_RVGPU_CS_ChainPreserve_SaveList;
  default: {
    // Dummy to not crash RegisterClassInfo.
    static const MCPhysReg NoCalleeSavedReg = RVGPU::NoRegister;
...
const MCPhysReg *
RVRegisterInfo::getCalleeSavedRegsViaCopy(const MachineFunction *MF) const {
  return nullptr;
}

const uint32_t *RVRegisterInfo::getCallPreservedMask(const MachineFunction &MF,
                                                     CallingConv::ID CC) const {
  switch (CC) {
  case CallingConv::C:
  case CallingConv::Fast:
  case CallingConv::Cold:
    return ST.hasGFX90AInsts() ? CSR_RVGPU_GFX90AInsts_RegMask
                               : CSR_RVGPU_RegMask;
  case CallingConv::RVGPU_Gfx:
    return ST.hasGFX90AInsts() ? CSR_RVGPU_SI_Gfx_GFX90AInsts_RegMask
                               : CSR_RVGPU_SI_Gfx_RegMask;
  case CallingConv::RVGPU_CS_Chain:
  case CallingConv::RVGPU_CS_ChainPreserve:
    // Calls to these functions never return, so we can pretend everything is
    // preserved.
    return RVGPU_AllVGPRs_RegMask;
  default:
    return nullptr;
  }
}

const uint32_t *RVRegisterInfo::getNoPreservedMask() const {
  return CSR_RVGPU_NoRegs_RegMask;
}

bool RVRegisterInfo::isChainScratchRegister(Register VGPR) {
  return VGPR >= RVGPU::VGPR0 && VGPR < RVGPU::VGPR8;
}

const TargetRegisterClass *
RVRegisterInfo::getLargestLegalSuperClass(const TargetRegisterClass *RC,
                                          const MachineFunction &MF) const {
  ...
  if (ST.hasMAIInsts() && (isVGPRClass(RC) || isAGPRClass(RC))) {
    if (RC == &RVGPU::VGPR_32RegClass || RC == &RVGPU::AGPR_32RegClass)
      return &RVGPU::AV_32RegClass;
    if (RC == &RVGPU::VReg_64RegClass || RC == &RVGPU::AReg_64RegClass)
      return &RVGPU::AV_64RegClass;
    if (RC == &RVGPU::VReg_64_Align2RegClass ||
        RC == &RVGPU::AReg_64_Align2RegClass)
      return &RVGPU::AV_64_Align2RegClass;
    if (RC == &RVGPU::VReg_96RegClass || RC == &RVGPU::AReg_96RegClass)
      return &RVGPU::AV_96RegClass;
    if (RC == &RVGPU::VReg_96_Align2RegClass ||
        RC == &RVGPU::AReg_96_Align2RegClass)
      return &RVGPU::AV_96_Align2RegClass;
    if (RC == &RVGPU::VReg_128RegClass || RC == &RVGPU::AReg_128RegClass)
      return &RVGPU::AV_128RegClass;
    if (RC == &RVGPU::VReg_128_Align2RegClass ||
        RC == &RVGPU::AReg_128_Align2RegClass)
      return &RVGPU::AV_128_Align2RegClass;
    if (RC == &RVGPU::VReg_160RegClass || RC == &RVGPU::AReg_160RegClass)
      return &RVGPU::AV_160RegClass;
    if (RC == &RVGPU::VReg_160_Align2RegClass ||
        RC == &RVGPU::AReg_160_Align2RegClass)
      return &RVGPU::AV_160_Align2RegClass;
    if (RC == &RVGPU::VReg_192RegClass || RC == &RVGPU::AReg_192RegClass)
      return &RVGPU::AV_192RegClass;
    if (RC == &RVGPU::VReg_192_Align2RegClass ||
        RC == &RVGPU::AReg_192_Align2RegClass)
      return &RVGPU::AV_192_Align2RegClass;
    if (RC == &RVGPU::VReg_256RegClass || RC == &RVGPU::AReg_256RegClass)
      return &RVGPU::AV_256RegClass;
    if (RC == &RVGPU::VReg_256_Align2RegClass ||
        RC == &RVGPU::AReg_256_Align2RegClass)
      return &RVGPU::AV_256_Align2RegClass;
    if (RC == &RVGPU::VReg_512RegClass || RC == &RVGPU::AReg_512RegClass)
      return &RVGPU::AV_512RegClass;
    if (RC == &RVGPU::VReg_512_Align2RegClass ||
        RC == &RVGPU::AReg_512_Align2RegClass)
      return &RVGPU::AV_512_Align2RegClass;
    if (RC == &RVGPU::VReg_1024RegClass || RC == &RVGPU::AReg_1024RegClass)
      return &RVGPU::AV_1024RegClass;
    if (RC == &RVGPU::VReg_1024_Align2RegClass ||
        RC == &RVGPU::AReg_1024_Align2RegClass)
      return &RVGPU::AV_1024_Align2RegClass;
  }

  return TargetRegisterInfo::getLargestLegalSuperClass(RC, MF);
}

Register RVRegisterInfo::getFrameRegister(const MachineFunction &MF) const {
...
bool RVRegisterInfo::hasBasePointer(const MachineFunction &MF) const {
...
Register RVRegisterInfo::getBaseRegister() const { return RVGPU::SGPR34; }

const uint32_t *RVRegisterInfo::getAllVGPRRegMask() const {
  return RVGPU_AllVGPRs_RegMask;
}

const uint32_t *RVRegisterInfo::getAllAGPRRegMask() const {
  return RVGPU_AllAGPRs_RegMask;
}

const uint32_t *RVRegisterInfo::getAllVectorRegMask() const {
  return RVGPU_AllVectorRegs_RegMask;
}

const uint32_t *RVRegisterInfo::getAllAllocatableSRegMask() const {
  return RVGPU_AllAllocatableSRegs_RegMask;
}

unsigned RVRegisterInfo::getSubRegFromChannel(unsigned Channel,
                                              unsigned NumRegs) {
...
MCRegister
RVRegisterInfo::getAlignedHighSGPRForRC(const MachineFunction &MF,
                                        const unsigned Align,
                                        const TargetRegisterClass *RC) const {
  unsigned BaseIdx = alignDown(ST.getMaxNumSGPRs(MF), Align) - Align;
  MCRegister BaseReg(RVGPU::SGPR_32RegClass.getRegister(BaseIdx));
  return getMatchingSuperReg(BaseReg, RVGPU::sub0, RC);
}

MCRegister RVRegisterInfo::reservedPrivateSegmentBufferReg(
  const MachineFunction &MF) const {
  return getAlignedHighSGPRForRC(MF, /*Align=*/4, &RVGPU::SGPR_128RegClass);
}

BitVector RVRegisterInfo::getReservedRegs(const MachineFunction &MF) const {
  BitVector Reserved(getNumRegs());
  Reserved.set(RVGPU::MODE);
  ...
  reserveRegisterTuples(Reserved, RVGPU::EXEC);
  reserveRegisterTuples(Reserved, RVGPU::FLAT_SCR);

  // M0 has to be reserved so that llvm accepts it as a live-in into a block.
  reserveRegisterTuples(Reserved, RVGPU::M0);

  // Reserve src_vccz, src_execz, src_scc.
  reserveRegisterTuples(Reserved, RVGPU::SRC_VCCZ);
  reserveRegisterTuples(Reserved, RVGPU::SRC_EXECZ);
  reserveRegisterTuples(Reserved, RVGPU::SRC_SCC);

  // Reserve the memory aperture registers
  reserveRegisterTuples(Reserved, RVGPU::SRC_SHARED_BASE);
  reserveRegisterTuples(Reserved, RVGPU::SRC_SHARED_LIMIT);
  reserveRegisterTuples(Reserved, RVGPU::SRC_PRIVATE_BASE);
  reserveRegisterTuples(Reserved, RVGPU::SRC_PRIVATE_LIMIT);

  // Reserve src_pops_exiting_wave_id - support is not implemented in Codegen.
  reserveRegisterTuples(Reserved, RVGPU::SRC_POPS_EXITING_WAVE_ID);

  // Reserve xnack_mask registers - support is not implemented in Codegen.
  reserveRegisterTuples(Reserved, RVGPU::XNACK_MASK);

  // Reserve lds_direct register - support is not implemented in Codegen.
  reserveRegisterTuples(Reserved, RVGPU::LDS_DIRECT);

  // Reserve Trap Handler registers - support is not implemented in Codegen.
  reserveRegisterTuples(Reserved, RVGPU::TBA);
  reserveRegisterTuples(Reserved, RVGPU::TMA);
  reserveRegisterTuples(Reserved, RVGPU::TTMP0_TTMP1);
  reserveRegisterTuples(Reserved, RVGPU::TTMP2_TTMP3);
  reserveRegisterTuples(Reserved, RVGPU::TTMP4_TTMP5);
  reserveRegisterTuples(Reserved, RVGPU::TTMP6_TTMP7);
  reserveRegisterTuples(Reserved, RVGPU::TTMP8_TTMP9);
  reserveRegisterTuples(Reserved, RVGPU::TTMP10_TTMP11);
  reserveRegisterTuples(Reserved, RVGPU::TTMP12_TTMP13);
  reserveRegisterTuples(Reserved, RVGPU::TTMP14_TTMP15);

  // Reserve null register - it shall never be allocated
  reserveRegisterTuples(Reserved, RVGPU::SGPR_NULL64);

  // Disallow vcc_hi allocation in wave32. It may be allocated but most likely
  // will result in bugs.
  if (isWave32) {
    Reserved.set(RVGPU::VCC);
    Reserved.set(RVGPU::VCC_HI);
  }

  // Reserve SGPRs.
  //
  unsigned MaxNumSGPRs = ST.getMaxNumSGPRs(MF);
  unsigned TotalNumSGPRs = RVGPU::SGPR_32RegClass.getNumRegs();
  for (unsigned i = MaxNumSGPRs; i < TotalNumSGPRs; ++i) {
    unsigned Reg = RVGPU::SGPR_32RegClass.getRegister(i);
    reserveRegisterTuples(Reserved, Reg);
  }

  Register ScratchRSrcReg = MFI->getScratchRSrcReg();
  if (ScratchRSrcReg != RVGPU::NoRegister) {
  ...
  for (unsigned i = MaxNumVGPRs; i < TotalNumVGPRs; ++i) {
    unsigned Reg = RVGPU::VGPR_32RegClass.getRegister(i);
    reserveRegisterTuples(Reserved, Reg);
  }

  if (ST.hasMAIInsts()) {
    for (unsigned i = MaxNumAGPRs; i < TotalNumVGPRs; ++i) {
      unsigned Reg = RVGPU::AGPR_32RegClass.getRegister(i);
      reserveRegisterTuples(Reserved, Reg);
    }
  } else {
...
bool RVRegisterInfo::isAsmClobberable(const MachineFunction &MF,
                                      MCRegister PhysReg) const {
  return !MF.getRegInfo().isReserved(PhysReg);
}

bool RVRegisterInfo::shouldRealignStack(const MachineFunction &MF) const {
...
bool RVRegisterInfo::requiresRegisterScavenging(const MachineFunction &Fn) const {
  const RVMachineFunctionInfo *Info = Fn.getInfo<RVMachineFunctionInfo>();
...
bool RVRegisterInfo::requiresFrameIndexScavenging(
  const MachineFunction &MF) const {
...
bool RVRegisterInfo::requiresFrameIndexReplacementScavenging(
  const MachineFunction &MF) const {
  const MachineFrameInfo &MFI = MF.getFrameInfo();
  return MFI.hasStackObjects();
}

bool RVRegisterInfo::requiresVirtualBaseRegisters(
  const MachineFunction &) const {
  // There are no special dedicated stack or frame pointers.
  return true;
}

int64_t RVRegisterInfo::getScratchInstrOffset(const MachineInstr *MI) const {
  assert(RVInstrInfo::isMUBUF(*MI) || RVInstrInfo::isFLATScratch(*MI));

  int OffIdx = RVGPU::getNamedOperandIdx(MI->getOpcode(),
                                          RVGPU::OpName::offset);
  return MI->getOperand(OffIdx).getImm();
}

int64_t RVRegisterInfo::getFrameIndexInstrOffset(const MachineInstr *MI,
                                                 int Idx) const {
  if (!RVInstrInfo::isMUBUF(*MI) && !RVInstrInfo::isFLATScratch(*MI))
    return 0;

  assert((Idx == RVGPU::getNamedOperandIdx(MI->getOpcode(),
                                            RVGPU::OpName::vaddr) ||
         (Idx == RVGPU::getNamedOperandIdx(MI->getOpcode(),
                                            RVGPU::OpName::saddr))) &&
...
bool RVRegisterInfo::needsFrameBaseReg(MachineInstr *MI, int64_t Offset) const {
  if (!RVInstrInfo::isMUBUF(*MI) && !RVInstrInfo::isFLATScratch(*MI))
    return false;
  ...
  if (RVInstrInfo::isMUBUF(*MI))
    return !TII->isLegalMUBUFImmOffset(FullOffset);

  return !TII->isLegalFLATOffset(FullOffset, RVGPUAS::PRIVATE_ADDRESS,
                                 SIInstrFlags::FlatScratch);
}

Register RVRegisterInfo::materializeFrameBaseRegister(MachineBasicBlock *MBB,
                                                      int FrameIdx,
  ...
  unsigned MovOpc = ST.enableFlatScratch() ? RVGPU::S_MOV_B32
                                           : RVGPU::V_MOV_B32_e32;

  Register BaseReg = MRI.createVirtualRegister(
      ST.enableFlatScratch() ? &RVGPU::SReg_32_XEXEC_HIRegClass
  ...
  Register OffsetReg = MRI.createVirtualRegister(&RVGPU::SReg_32_XM0RegClass);

  Register FIReg = MRI.createVirtualRegister(
      ST.enableFlatScratch() ? &RVGPU::SReg_32_XM0RegClass
                             : &RVGPU::VGPR_32RegClass);

  BuildMI(*MBB, Ins, DL, TII->get(RVGPU::S_MOV_B32), OffsetReg)
    .addImm(Offset);
  BuildMI(*MBB, Ins, DL, TII->get(MovOpc), FIReg)
    .addFrameIndex(FrameIdx);

  if (ST.enableFlatScratch() ) {
    BuildMI(*MBB, Ins, DL, TII->get(RVGPU::S_ADD_I32), BaseReg)
...
void RVRegisterInfo::resolveFrameIndex(MachineInstr &MI, Register BaseReg,
                                       int64_t Offset) const {
  const RVInstrInfo *TII = ST.getInstrInfo();
  ...
  MachineOperand *FIOp =
      TII->getNamedOperand(MI, IsFlat ? RVGPU::OpName::saddr
                                      : RVGPU::OpName::vaddr);

  MachineOperand *OffsetOp = TII->getNamedOperand(MI, RVGPU::OpName::offset);
  ...
  if (IsFlat) {
    assert(TII->isLegalFLATOffset(NewOffset, RVGPUAS::PRIVATE_ADDRESS,
                                  SIInstrFlags::FlatScratch) &&
...
#ifndef NDEBUG
  MachineOperand *SOffset = TII->getNamedOperand(MI, RVGPU::OpName::soffset);
...
bool RVRegisterInfo::isFrameOffsetLegal(const MachineInstr *MI,
                                        Register BaseReg,
                                        int64_t Offset) const {
  if (!RVInstrInfo::isMUBUF(*MI) && !RVInstrInfo::isFLATScratch(*MI))
    return false;
  ...
  return TII->isLegalFLATOffset(NewOffset, RVGPUAS::PRIVATE_ADDRESS,
                                SIInstrFlags::FlatScratch);
}

const TargetRegisterClass *RVRegisterInfo::getPointerRegClass(
...
const TargetRegisterClass *
RVRegisterInfo::getCrossCopyRegClass(const TargetRegisterClass *RC) const {
  if (isAGPRClass(RC) && !ST.hasGFX90AInsts())
    return getEquivalentVGPRClass(RC);
  if (RC == &RVGPU::SCC_CLASSRegClass)
...
static unsigned getNumSubRegsForSpillOp(unsigned Op) {
  ...
  switch (Op) {
  case RVGPU::SI_SPILL_S1024_SAVE:
  case RVGPU::SI_SPILL_S1024_RESTORE:
  case RVGPU::SI_SPILL_V1024_SAVE:
  case RVGPU::SI_SPILL_V1024_RESTORE:
  case RVGPU::SI_SPILL_A1024_SAVE:
  case RVGPU::SI_SPILL_A1024_RESTORE:
  case RVGPU::SI_SPILL_AV1024_SAVE:
  case RVGPU::SI_SPILL_AV1024_RESTORE:
    return 32;
  case RVGPU::SI_SPILL_S512_SAVE:
  case RVGPU::SI_SPILL_S512_RESTORE:
  case RVGPU::SI_SPILL_V512_SAVE:
  case RVGPU::SI_SPILL_V512_RESTORE:
  case RVGPU::SI_SPILL_A512_SAVE:
  case RVGPU::SI_SPILL_A512_RESTORE:
  case RVGPU::SI_SPILL_AV512_SAVE:
  case RVGPU::SI_SPILL_AV512_RESTORE:
    return 16;
  case RVGPU::SI_SPILL_S384_SAVE:
  case RVGPU::SI_SPILL_S384_RESTORE:
  case RVGPU::SI_SPILL_V384_SAVE:
  case RVGPU::SI_SPILL_V384_RESTORE:
  case RVGPU::SI_SPILL_A384_SAVE:
  case RVGPU::SI_SPILL_A384_RESTORE:
  case RVGPU::SI_SPILL_AV384_SAVE:
  case RVGPU::SI_SPILL_AV384_RESTORE:
    return 12;
  case RVGPU::SI_SPILL_S352_SAVE:
  case RVGPU::SI_SPILL_S352_RESTORE:
  case RVGPU::SI_SPILL_V352_SAVE:
  case RVGPU::SI_SPILL_V352_RESTORE:
  case RVGPU::SI_SPILL_A352_SAVE:
  case RVGPU::SI_SPILL_A352_RESTORE:
  case RVGPU::SI_SPILL_AV352_SAVE:
  case RVGPU::SI_SPILL_AV352_RESTORE:
    return 11;
  case RVGPU::SI_SPILL_S320_SAVE:
  case RVGPU::SI_SPILL_S320_RESTORE:
  case RVGPU::SI_SPILL_V320_SAVE:
  case RVGPU::SI_SPILL_V320_RESTORE:
  case RVGPU::SI_SPILL_A320_SAVE:
  case RVGPU::SI_SPILL_A320_RESTORE:
  case RVGPU::SI_SPILL_AV320_SAVE:
  case RVGPU::SI_SPILL_AV320_RESTORE:
    return 10;
  case RVGPU::SI_SPILL_S288_SAVE:
  case RVGPU::SI_SPILL_S288_RESTORE:
  case RVGPU::SI_SPILL_V288_SAVE:
  case RVGPU::SI_SPILL_V288_RESTORE:
  case RVGPU::SI_SPILL_A288_SAVE:
  case RVGPU::SI_SPILL_A288_RESTORE:
  case RVGPU::SI_SPILL_AV288_SAVE:
  case RVGPU::SI_SPILL_AV288_RESTORE:
    return 9;
  case RVGPU::SI_SPILL_S256_SAVE:
  case RVGPU::SI_SPILL_S256_RESTORE:
  case RVGPU::SI_SPILL_V256_SAVE:
  case RVGPU::SI_SPILL_V256_RESTORE:
  case RVGPU::SI_SPILL_A256_SAVE:
  case RVGPU::SI_SPILL_A256_RESTORE:
  case RVGPU::SI_SPILL_AV256_SAVE:
  case RVGPU::SI_SPILL_AV256_RESTORE:
    return 8;
  case RVGPU::SI_SPILL_S224_SAVE:
  case RVGPU::SI_SPILL_S224_RESTORE:
  case RVGPU::SI_SPILL_V224_SAVE:
  case RVGPU::SI_SPILL_V224_RESTORE:
  case RVGPU::SI_SPILL_A224_SAVE:
  case RVGPU::SI_SPILL_A224_RESTORE:
  case RVGPU::SI_SPILL_AV224_SAVE:
  case RVGPU::SI_SPILL_AV224_RESTORE:
    return 7;
  case RVGPU::SI_SPILL_S192_SAVE:
  case RVGPU::SI_SPILL_S192_RESTORE:
  case RVGPU::SI_SPILL_V192_SAVE:
  case RVGPU::SI_SPILL_V192_RESTORE:
  case RVGPU::SI_SPILL_A192_SAVE:
  case RVGPU::SI_SPILL_A192_RESTORE:
  case RVGPU::SI_SPILL_AV192_SAVE:
  case RVGPU::SI_SPILL_AV192_RESTORE:
    return 6;
  case RVGPU::SI_SPILL_S160_SAVE:
  case RVGPU::SI_SPILL_S160_RESTORE:
  case RVGPU::SI_SPILL_V160_SAVE:
  case RVGPU::SI_SPILL_V160_RESTORE:
  case RVGPU::SI_SPILL_A160_SAVE:
  case RVGPU::SI_SPILL_A160_RESTORE:
  case RVGPU::SI_SPILL_AV160_SAVE:
  case RVGPU::SI_SPILL_AV160_RESTORE:
    return 5;
  case RVGPU::SI_SPILL_S128_SAVE:
  case RVGPU::SI_SPILL_S128_RESTORE:
  case RVGPU::SI_SPILL_V128_SAVE:
  case RVGPU::SI_SPILL_V128_RESTORE:
  case RVGPU::SI_SPILL_A128_SAVE:
  case RVGPU::SI_SPILL_A128_RESTORE:
  case RVGPU::SI_SPILL_AV128_SAVE:
  case RVGPU::SI_SPILL_AV128_RESTORE:
    return 4;
  case RVGPU::SI_SPILL_S96_SAVE:
  case RVGPU::SI_SPILL_S96_RESTORE:
  case RVGPU::SI_SPILL_V96_SAVE:
  case RVGPU::SI_SPILL_V96_RESTORE:
  case RVGPU::SI_SPILL_A96_SAVE:
  case RVGPU::SI_SPILL_A96_RESTORE:
  case RVGPU::SI_SPILL_AV96_SAVE:
  case RVGPU::SI_SPILL_AV96_RESTORE:
    return 3;
  case RVGPU::SI_SPILL_S64_SAVE:
  case RVGPU::SI_SPILL_S64_RESTORE:
  case RVGPU::SI_SPILL_V64_SAVE:
  case RVGPU::SI_SPILL_V64_RESTORE:
  case RVGPU::SI_SPILL_A64_SAVE:
  case RVGPU::SI_SPILL_A64_RESTORE:
  case RVGPU::SI_SPILL_AV64_SAVE:
  case RVGPU::SI_SPILL_AV64_RESTORE:
    return 2;
  case RVGPU::SI_SPILL_S32_SAVE:
  case RVGPU::SI_SPILL_S32_RESTORE:
  case RVGPU::SI_SPILL_V32_SAVE:
  case RVGPU::SI_SPILL_V32_RESTORE:
  case RVGPU::SI_SPILL_A32_SAVE:
  case RVGPU::SI_SPILL_A32_RESTORE:
  case RVGPU::SI_SPILL_AV32_SAVE:
  case RVGPU::SI_SPILL_AV32_RESTORE:
  case RVGPU::SI_SPILL_WWM_V32_SAVE:
  case RVGPU::SI_SPILL_WWM_V32_RESTORE:
  case RVGPU::SI_SPILL_WWM_AV32_SAVE:
  case RVGPU::SI_SPILL_WWM_AV32_RESTORE:
    return 1;
...
static int getOffsetMUBUFStore(unsigned Opc) {
  switch (Opc) {
  case RVGPU::BUFFER_STORE_DWORD_OFFEN:
    return RVGPU::BUFFER_STORE_DWORD_OFFSET;
  case RVGPU::BUFFER_STORE_BYTE_OFFEN:
    return RVGPU::BUFFER_STORE_BYTE_OFFSET;
  case RVGPU::BUFFER_STORE_SHORT_OFFEN:
    return RVGPU::BUFFER_STORE_SHORT_OFFSET;
  case RVGPU::BUFFER_STORE_DWORDX2_OFFEN:
    return RVGPU::BUFFER_STORE_DWORDX2_OFFSET;
  case RVGPU::BUFFER_STORE_DWORDX3_OFFEN:
    return RVGPU::BUFFER_STORE_DWORDX3_OFFSET;
  case RVGPU::BUFFER_STORE_DWORDX4_OFFEN:
    return RVGPU::BUFFER_STORE_DWORDX4_OFFSET;
  case RVGPU::BUFFER_STORE_SHORT_D16_HI_OFFEN:
    return RVGPU::BUFFER_STORE_SHORT_D16_HI_OFFSET;
  case RVGPU::BUFFER_STORE_BYTE_D16_HI_OFFEN:
    return RVGPU::BUFFER_STORE_BYTE_D16_HI_OFFSET;
  default:
    return -1;
  }
}
...
static int getOffsetMUBUFLoad(unsigned Opc) {
  switch (Opc) {
  case RVGPU::BUFFER_LOAD_DWORD_OFFEN:
    return RVGPU::BUFFER_LOAD_DWORD_OFFSET;
  case RVGPU::BUFFER_LOAD_UBYTE_OFFEN:
    return RVGPU::BUFFER_LOAD_UBYTE_OFFSET;
  case RVGPU::BUFFER_LOAD_SBYTE_OFFEN:
    return RVGPU::BUFFER_LOAD_SBYTE_OFFSET;
  case RVGPU::BUFFER_LOAD_USHORT_OFFEN:
    return RVGPU::BUFFER_LOAD_USHORT_OFFSET;
  case RVGPU::BUFFER_LOAD_SSHORT_OFFEN:
    return RVGPU::BUFFER_LOAD_SSHORT_OFFSET;
  case RVGPU::BUFFER_LOAD_DWORDX2_OFFEN:
    return RVGPU::BUFFER_LOAD_DWORDX2_OFFSET;
  case RVGPU::BUFFER_LOAD_DWORDX3_OFFEN:
    return RVGPU::BUFFER_LOAD_DWORDX3_OFFSET;
  case RVGPU::BUFFER_LOAD_DWORDX4_OFFEN:
    return RVGPU::BUFFER_LOAD_DWORDX4_OFFSET;
  case RVGPU::BUFFER_LOAD_UBYTE_D16_OFFEN:
    return RVGPU::BUFFER_LOAD_UBYTE_D16_OFFSET;
  case RVGPU::BUFFER_LOAD_UBYTE_D16_HI_OFFEN:
    return RVGPU::BUFFER_LOAD_UBYTE_D16_HI_OFFSET;
  case RVGPU::BUFFER_LOAD_SBYTE_D16_OFFEN:
    return RVGPU::BUFFER_LOAD_SBYTE_D16_OFFSET;
  case RVGPU::BUFFER_LOAD_SBYTE_D16_HI_OFFEN:
    return RVGPU::BUFFER_LOAD_SBYTE_D16_HI_OFFSET;
  case RVGPU::BUFFER_LOAD_SHORT_D16_OFFEN:
    return RVGPU::BUFFER_LOAD_SHORT_D16_OFFSET;
  case RVGPU::BUFFER_LOAD_SHORT_D16_HI_OFFEN:
    return RVGPU::BUFFER_LOAD_SHORT_D16_HI_OFFSET;
  default:
    return -1;
  }
}
...
static int getOffenMUBUFStore(unsigned Opc) {
  switch (Opc) {
  case RVGPU::BUFFER_STORE_DWORD_OFFSET:
    return RVGPU::BUFFER_STORE_DWORD_OFFEN;
  case RVGPU::BUFFER_STORE_BYTE_OFFSET:
    return RVGPU::BUFFER_STORE_BYTE_OFFEN;
  case RVGPU::BUFFER_STORE_SHORT_OFFSET:
    return RVGPU::BUFFER_STORE_SHORT_OFFEN;
  case RVGPU::BUFFER_STORE_DWORDX2_OFFSET:
    return RVGPU::BUFFER_STORE_DWORDX2_OFFEN;
  case RVGPU::BUFFER_STORE_DWORDX3_OFFSET:
    return RVGPU::BUFFER_STORE_DWORDX3_OFFEN;
  case RVGPU::BUFFER_STORE_DWORDX4_OFFSET:
    return RVGPU::BUFFER_STORE_DWORDX4_OFFEN;
  case RVGPU::BUFFER_STORE_SHORT_D16_HI_OFFSET:
    return RVGPU::BUFFER_STORE_SHORT_D16_HI_OFFEN;
  case RVGPU::BUFFER_STORE_BYTE_D16_HI_OFFSET:
    return RVGPU::BUFFER_STORE_BYTE_D16_HI_OFFEN;
  default:
    return -1;
  }
}
...
static int getOffenMUBUFLoad(unsigned Opc) {
  switch (Opc) {
  case RVGPU::BUFFER_LOAD_DWORD_OFFSET:
    return RVGPU::BUFFER_LOAD_DWORD_OFFEN;
  case RVGPU::BUFFER_LOAD_UBYTE_OFFSET:
    return RVGPU::BUFFER_LOAD_UBYTE_OFFEN;
  case RVGPU::BUFFER_LOAD_SBYTE_OFFSET:
    return RVGPU::BUFFER_LOAD_SBYTE_OFFEN;
  case RVGPU::BUFFER_LOAD_USHORT_OFFSET:
    return RVGPU::BUFFER_LOAD_USHORT_OFFEN;
  case RVGPU::BUFFER_LOAD_SSHORT_OFFSET:
    return RVGPU::BUFFER_LOAD_SSHORT_OFFEN;
  case RVGPU::BUFFER_LOAD_DWORDX2_OFFSET:
    return RVGPU::BUFFER_LOAD_DWORDX2_OFFEN;
  case RVGPU::BUFFER_LOAD_DWORDX3_OFFSET:
    return RVGPU::BUFFER_LOAD_DWORDX3_OFFEN;
  case RVGPU::BUFFER_LOAD_DWORDX4_OFFSET:
    return RVGPU::BUFFER_LOAD_DWORDX4_OFFEN;
  case RVGPU::BUFFER_LOAD_UBYTE_D16_OFFSET:
    return RVGPU::BUFFER_LOAD_UBYTE_D16_OFFEN;
  case RVGPU::BUFFER_LOAD_UBYTE_D16_HI_OFFSET:
    return RVGPU::BUFFER_LOAD_UBYTE_D16_HI_OFFEN;
  case RVGPU::BUFFER_LOAD_SBYTE_D16_OFFSET:
    return RVGPU::BUFFER_LOAD_SBYTE_D16_OFFEN;
  case RVGPU::BUFFER_LOAD_SBYTE_D16_HI_OFFSET:
    return RVGPU::BUFFER_LOAD_SBYTE_D16_HI_OFFEN;
  case RVGPU::BUFFER_LOAD_SHORT_D16_OFFSET:
    return RVGPU::BUFFER_LOAD_SHORT_D16_OFFEN;
  case RVGPU::BUFFER_LOAD_SHORT_D16_HI_OFFSET:
    return RVGPU::BUFFER_LOAD_SHORT_D16_HI_OFFEN;
  default:
    return -1;
  }
}
...
static MachineInstrBuilder spillVGPRtoAGPR(const GCNSubtarget &ST,
  ...
  MachineRegisterInfo &MRI = MF->getRegInfo();
  auto *TRI = static_cast<const RVRegisterInfo*>(MRI.getTargetRegisterInfo());
  ...
  if (IsVGPR == TRI->isVGPR(MRI, ValueReg)) {
    ...
    auto CopyMIB = BuildMI(MBB, MI, DL, TII->get(RVGPU::COPY), Dst)
                       .addReg(Src, getKillRegState(IsKill));
    CopyMIB->setAsmPrinterFlag(MachineInstr::ReloadReuse);
    return CopyMIB;
  }
  unsigned Opc = (IsStore ^ IsVGPR) ? RVGPU::V_ACCVGPR_WRITE_B32_e64
                                    : RVGPU::V_ACCVGPR_READ_B32_e64;
...
static bool buildMUBUFOffsetLoadStore(const GCNSubtarget &ST,
  ...
  const MachineOperand *Reg = TII->getNamedOperand(*MI, RVGPU::OpName::vdata);
  ...
  MachineInstrBuilder NewMI =
      BuildMI(*MBB, MI, DL, TII->get(LoadStoreOp))
          .add(*Reg)
          .add(*TII->getNamedOperand(*MI, RVGPU::OpName::srsrc))
          .add(*TII->getNamedOperand(*MI, RVGPU::OpName::soffset))
          .addImm(Offset)
          .addImm(0) // cpol
          .addImm(0) // swz
          .cloneMemRefs(*MI);
  ...
  const MachineOperand *VDataIn = TII->getNamedOperand(*MI,
                                                       RVGPU::OpName::vdata_in);
  if (VDataIn)
    NewMI.add(*VDataIn);
  return true;
}

static unsigned getFlatScratchSpillOpcode(const RVInstrInfo *TII,
                                          unsigned LoadStoreOp,
                                          unsigned EltSize) {
  bool IsStore = TII->get(LoadStoreOp).mayStore();
  bool HasVAddr = RVGPU::hasNamedOperand(LoadStoreOp, RVGPU::OpName::vaddr);
  bool UseST =
      !HasVAddr && !RVGPU::hasNamedOperand(LoadStoreOp, RVGPU::OpName::saddr);

  switch (EltSize) {
  case 4:
    LoadStoreOp = IsStore ? RVGPU::SCRATCH_STORE_DWORD_SADDR
                          : RVGPU::SCRATCH_LOAD_DWORD_SADDR;
    break;
  case 8:
    LoadStoreOp = IsStore ? RVGPU::SCRATCH_STORE_DWORDX2_SADDR
                          : RVGPU::SCRATCH_LOAD_DWORDX2_SADDR;
    break;
  case 12:
    LoadStoreOp = IsStore ? RVGPU::SCRATCH_STORE_DWORDX3_SADDR
                          : RVGPU::SCRATCH_LOAD_DWORDX3_SADDR;
    break;
  case 16:
    LoadStoreOp = IsStore ? RVGPU::SCRATCH_STORE_DWORDX4_SADDR
                          : RVGPU::SCRATCH_LOAD_DWORDX4_SADDR;
    break;
  default:
  ...
  if (HasVAddr)
    LoadStoreOp = RVGPU::getFlatScratchInstSVfromSS(LoadStoreOp);
  else if (UseST)
    LoadStoreOp = RVGPU::getFlatScratchInstSTfromSS(LoadStoreOp);

  return LoadStoreOp;
}

void RVRegisterInfo::buildSpillLoadStore(
  ...
  const unsigned RegWidth = RVGPU::getRegBitWidth(*RC) / 8;
  ...
  auto MaterializeVOffset = [&](Register SGPRBase, Register TmpVGPR,
    ...
    if (IsFlat && SGPRBase) {
      ...
      if (ST.getConstantBusLimit(RVGPU::V_ADD_U32_e64) >= 2) {
        BuildMI(MBB, MI, DL, TII->get(RVGPU::V_ADD_U32_e64), TmpVGPR)
          .addReg(SGPRBase)
          .addImm(VOffset)
          .addImm(0); // clamp
      } else {
        BuildMI(MBB, MI, DL, TII->get(RVGPU::V_MOV_B32_e32), TmpVGPR)
          .addReg(SGPRBase);
        BuildMI(MBB, MI, DL, TII->get(RVGPU::V_ADD_U32_e32), TmpVGPR)
          .addImm(VOffset)
  ...
  bool IsOffsetLegal =
      IsFlat ? TII->isLegalFLATOffset(MaxOffset, RVGPUAS::PRIVATE_ADDRESS,
                                      SIInstrFlags::FlatScratch)
  ...
  if (!IsOffsetLegal || (IsFlat && !SOffset && !ST.hasFlatScratchSTMode())) {
    ...
    // entry.
    if (RS) {
      SOffset = RS->scavengeRegisterBackwards(RVGPU::SGPR_32RegClass, MI, false, 0, false);

      // Piggy back on the liveness scan we just did see if SCC is dead.
      CanClobberSCC = !RS->isRegUsed(RVGPU::SCC);
    } else if (LiveUnits) {
      CanClobberSCC = LiveUnits->available(RVGPU::SCC);
      for (MCRegister Reg : RVGPU::SGPR_32RegClass) {
    ...
    if (ScratchOffsetReg != RVGPU::NoRegister && !CanClobberSCC)
      SOffset = Register();
    ...
    if (!SOffset) {
      ...
      if (RS) {
        TmpOffsetVGPR = RS->scavengeRegisterBackwards(RVGPU::VGPR_32RegClass, MI, false, 0);
      } else {
        assert(LiveUnits);
        for (MCRegister Reg : RVGPU::VGPR_32RegClass) {
          if (LiveUnits->available(Reg) && !MF->getRegInfo().isReserved(Reg)) {
            TmpOffsetVGPR = Reg;
            break;
          }
    ...
    } else if (ScratchOffsetReg == RVGPU::NoRegister) {
      BuildMI(MBB, MI, DL, TII->get(RVGPU::S_MOV_B32), SOffset).addImm(Offset);
    } else {
      assert(Offset != 0);
      auto Add = BuildMI(MBB, MI, DL, TII->get(RVGPU::S_ADD_I32), SOffset)
  ...
  if (IsFlat && SOffset == RVGPU::NoRegister) {
    assert(RVGPU::getNamedOperandIdx(LoadStoreOp, RVGPU::OpName::vaddr) < 0
  ...
  for (unsigned i = 0, e = NumSubRegs + NumRemSubRegs, RegOffset = 0; i != e;
    ...
    if (IsAGPR) {
      ...
      if (IsStore) {
        auto AccRead = BuildMI(MBB, MI, DL,
                               TII->get(RVGPU::V_ACCVGPR_READ_B32_e64),
                               TmpIntermediateVGPR)
      ...
      if (!TmpOffsetVGPR) {
        TmpOffsetVGPR = RS->scavengeRegisterBackwards(RVGPU::VGPR_32RegClass,
                                                      MI, false, 0);
    ...
    if (!IsStore && IsAGPR && TmpIntermediateVGPR != RVGPU::NoRegister) {
      MIB = BuildMI(MBB, MI, DL, TII->get(RVGPU::V_ACCVGPR_WRITE_B32_e64),
                    FinalReg)
  ...
  if (ScratchOffsetRegDelta != 0) {
    ...
    BuildMI(MBB, MI, DL, TII->get(RVGPU::S_ADD_I32), SOffset)
        .addReg(SOffset)
        .addImm(-ScratchOffsetRegDelta);
  }
}

void RVRegisterInfo::buildVGPRSpillLoadStore(SGPRSpillBuilder &SB, int Index,
  ...
  if (IsLoad) {
    unsigned Opc = ST.enableFlatScratch() ? RVGPU::SCRATCH_LOAD_DWORD_SADDR
                                          : RVGPU::BUFFER_LOAD_DWORD_OFFSET;
    buildSpillLoadStore(*SB.MBB, SB.MI, SB.DL, Opc, Index, SB.TmpVGPR, false,
                        FrameReg, Offset * SB.EltSize, MMO, SB.RS);
  } else {
    unsigned Opc = ST.enableFlatScratch() ? RVGPU::SCRATCH_STORE_DWORD_SADDR
                                          : RVGPU::BUFFER_STORE_DWORD_OFFSET;
...
bool RVRegisterInfo::spillSGPR(MachineBasicBlock::iterator MI, int Index,
  ...
  if (SpillToVGPR) {
    ...
    for (unsigned i = 0, e = SB.NumSubRegs; i < e; ++i) {
      ...
      auto MIB = BuildMI(*SB.MBB, MI, SB.DL,
                         SB.TII.get(RVGPU::SI_SPILL_S32_TO_VGPR), Spill.VGPR)
      ...
      for (unsigned i = Offset * PVD.PerVGPR,
        ...
        MachineInstrBuilder WriteLane =
            BuildMI(*SB.MBB, MI, SB.DL,
                    SB.TII.get(RVGPU::SI_SPILL_S32_TO_VGPR), SB.TmpVGPR)
...
bool RVRegisterInfo::restoreSGPR(MachineBasicBlock::iterator MI, int Index,
                                 RegScavenger *RS, SlotIndexes *Indexes,
                                 LiveIntervals *LIS, bool OnlyToVGPR,
                                 bool SpillToPhysVGPRLane) const {
  SGPRSpillBuilder SB(*this, *ST.getInstrInfo(), isWave32, MI, Index, RS);
  ...
  if (SpillToVGPR) {
    for (unsigned i = 0, e = SB.NumSubRegs; i < e; ++i) {
      ...
      SpilledReg Spill = VGPRSpills[i];
      auto MIB = BuildMI(*SB.MBB, MI, SB.DL,
                         SB.TII.get(RVGPU::SI_RESTORE_S32_FROM_VGPR), SubReg)
                     .addReg(Spill.VGPR)
                     .addImm(Spill.Lane);
...
bool RVRegisterInfo::spillEmergencySGPR(MachineBasicBlock::iterator MI,
  ...
  for (unsigned Offset = 0; Offset < PVD.NumVGPRs; ++Offset) {
    ...
    for (unsigned i = Offset * PVD.PerVGPR,
      ...
      MachineInstrBuilder WriteLane =
          BuildMI(*SB.MBB, MI, SB.DL, SB.TII.get(RVGPU::V_WRITELANE_B32),
                  SB.TmpVGPR)
              .addReg(SubReg, SubKillState)
              .addImm(i % PVD.PerVGPR)
              .addReg(SB.TmpVGPR, TmpVGPRFlags);
    ...
    for (unsigned i = Offset * PVD.PerVGPR,
      ...
      Register SubReg =
          SB.NumSubRegs == 1
              ...
              : Register(getSubReg(SB.SuperReg, SB.SplitParts[i]));
      bool LastSubReg = (i + 1 == e);
      auto MIB = BuildMI(*SB.MBB, MI, SB.DL, SB.TII.get(RVGPU::V_READLANE_B32),
                         SubReg)
                     .addReg(SB.TmpVGPR, getKillRegState(LastSubReg))
                     .addImm(i);
      if (SB.NumSubRegs > 1 && i == 0)
...
/// handled.
bool RVRegisterInfo::eliminateSGPRToVGPRSpillFrameIndex(
...
bool RVRegisterInfo::eliminateFrameIndex(MachineBasicBlock::iterator MI,
  ...
  switch (MI->getOpcode()) {
    ...
    case RVGPU::SI_SPILL_WWM_AV32_SAVE: {
      const MachineOperand *VData = TII->getNamedOperand(*MI,
                                                         RVGPU::OpName::vdata);
      assert(TII->getNamedOperand(*MI, RVGPU::OpName::soffset)->getReg() ==
      ...
      if (IsWWMRegSpill) {
        TII->insertScratchExecCopy(*MF, *MBB, MI, DL, MFI->getSGPRForEXECCopy(),
                                  RS->isRegUsed(RVGPU::SCC));
      }
      buildSpillLoadStore(
          *MBB, MI, DL, Opc, Index, VData->getReg(), VData->isKill(), FrameReg,
          TII->getNamedOperand(*MI, RVGPU::OpName::offset)->getImm(),
    ...
    default: {
      ...
      if (ST.enableFlatScratch()) {
        if (TII->isFLATScratch(*MI)) {
          assert((int16_t)FIOperandNum ==
                 RVGPU::getNamedOperandIdx(MI->getOpcode(),
                                            RVGPU::OpName::saddr));
          ...
          MachineOperand *OffsetOp =
            TII->getNamedOperand(*MI, RVGPU::OpName::offset);
          int64_t NewOffset = Offset + OffsetOp->getImm();
          if (TII->isLegalFLATOffset(NewOffset, RVGPUAS::PRIVATE_ADDRESS,
                                     SIInstrFlags::FlatScratch)) {
            OffsetOp->setImm(NewOffset);
            if (FrameReg)
              return false;
          ...
          if (!Offset) {
            unsigned Opc = MI->getOpcode();
            int NewOpc = -1;
            if (RVGPU::hasNamedOperand(Opc, RVGPU::OpName::vaddr)) {
              NewOpc = RVGPU::getFlatScratchInstSVfromSVS(Opc);
            ...
            if (NewOpc != -1) {
              ...
              MI->removeOperand(
                  RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::saddr));

              if (TiedVDst) {
                int NewVDst =
                    RVGPU::getNamedOperandIdx(NewOpc, RVGPU::OpName::vdst);
                int NewVDstIn =
                    RVGPU::getNamedOperandIdx(NewOpc, RVGPU::OpName::vdst_in);
        ...
        // a VGPR.
        FIOp.ChangeToRegister(RVGPU::M0, false);
        ...
        const TargetRegisterClass *RC = UseSGPR ? &RVGPU::SReg_32_XM0RegClass
                                                : &RVGPU::VGPR_32RegClass;
        ...
        if ((!FrameReg || !Offset) && TmpReg) {
          unsigned Opc = UseSGPR ? RVGPU::S_MOV_B32 : RVGPU::V_MOV_B32_e32;
        ...
        bool NeedSaveSCC =
            RS->isRegUsed(RVGPU::SCC) && !MI->definesRegister(RVGPU::SCC);

        Register TmpSReg =
            UseSGPR ? TmpReg
                    : RS->scavengeRegisterBackwards(RVGPU::SReg_32_XM0RegClass,
                                                    MI, false, 0, !UseSGPR);
        ...
        if (NeedSaveSCC) {
          ...
          BuildMI(*MBB, MI, DL, TII->get(RVGPU::S_ADDC_U32), TmpSReg)
              .addReg(FrameReg)
              .addImm(Offset);
          BuildMI(*MBB, MI, DL, TII->get(RVGPU::S_BITCMP1_B32))
              .addReg(TmpSReg)
              .addImm(0);
          BuildMI(*MBB, MI, DL, TII->get(RVGPU::S_BITSET0_B32), TmpSReg)
              .addImm(0)
              .addReg(TmpSReg);
        } else {
          BuildMI(*MBB, MI, DL, TII->get(RVGPU::S_ADD_I32), TmpSReg)
              .addReg(FrameReg)
              .addImm(Offset);
        }

        if (!UseSGPR)
          BuildMI(*MBB, MI, DL, TII->get(RVGPU::V_MOV_B32_e32), TmpReg)
            .addReg(TmpSReg, RegState::Kill);

        if (TmpSReg == FrameReg) {
          // Undo frame register modification.
          if (NeedSaveSCC && !MI->registerDefIsDead(RVGPU::SCC)) {
            MachineBasicBlock::iterator I =
                BuildMI(*MBB, std::next(MI), DL, TII->get(RVGPU::S_ADDC_U32),
                        TmpSReg)
                    .addReg(FrameReg)
                    .addImm(-Offset);
            I = BuildMI(*MBB, std::next(I), DL, TII->get(RVGPU::S_BITCMP1_B32))
                    .addReg(TmpSReg)
                    .addImm(0);
            BuildMI(*MBB, std::next(I), DL, TII->get(RVGPU::S_BITSET0_B32),
                    TmpSReg)
                .addImm(0)
                .addReg(TmpSReg);
          } else {
            BuildMI(*MBB, std::next(MI), DL, TII->get(RVGPU::S_ADD_I32),
                    FrameReg)
                .addReg(FrameReg)
                .addImm(-Offset);
          }
      ...
      if (!IsMUBUF && !MFI->isBottomOfStack()) {
        ...
        const TargetRegisterClass *RC = IsSALU && !LiveSCC
                                            ...
                                            : &RVGPU::VGPR_32RegClass;
        bool IsCopy = MI->getOpcode() == RVGPU::V_MOV_B32_e32 ||
                      MI->getOpcode() == RVGPU::V_MOV_B32_e64;
        Register ResultReg =
        ...
        int64_t Offset = FrameInfo.getObjectOffset(Index);
        if (Offset == 0) {
          unsigned OpCode = IsSALU && !LiveSCC ? RVGPU::S_LSHR_B32
                                               : RVGPU::V_LSHRREV_B32_e64;
          auto Shift = BuildMI(*MBB, MI, DL, TII->get(OpCode), ResultReg);
          if (OpCode == RVGPU::V_LSHRREV_B32_e64)
          ...
          if (IsSALU && LiveSCC) {
            Register NewDest = RS->scavengeRegisterBackwards(
                RVGPU::SReg_32RegClass, Shift, false, 0);
            BuildMI(*MBB, MI, DL, TII->get(RVGPU::V_READFIRSTLANE_B32),
                    NewDest)
                .addReg(ResultReg);
            ResultReg = NewDest;
          }
          ...
          if (!IsSALU) {
            if ((MIB = TII->getAddNoCarry(*MBB, MI, DL, ResultReg, *RS)) !=
              ...
              BuildMI(*MBB, *MIB, DL, TII->get(RVGPU::V_LSHRREV_B32_e64),
                      ScaledReg)
                .addImm(ST.getWavefrontSizeLog2())
                .addReg(FrameReg);

              const bool IsVOP2 = MIB->getOpcode() == RVGPU::V_ADD_U32_e32;

              // TODO: Fold if use instruction is another add of a constant.
              if (IsVOP2 || RVGPU::isInlinableLiteral32(Offset, ST.hasInv2PiInlineImm())) {
                // FIXME: This can fail
                MIB.addImm(Offset);
                ...
                assert(MIB->getOpcode() == RVGPU::V_ADD_CO_U32_e64 &&
                       "Need to reuse carry out register");

                // Use scavenged unused carry out as offset register.
                Register ConstOffsetReg;
                if (!isWave32)
                  ConstOffsetReg = getSubReg(MIB.getReg(1), RVGPU::sub0);
                else
                  ConstOffsetReg = MIB.getReg(1);

                BuildMI(*MBB, *MIB, DL, TII->get(RVGPU::S_MOV_B32), ConstOffsetReg)
                    .addImm(Offset);
          ...
          if (!MIB || IsSALU) {
            ...
            Register TmpScaledReg = RS->scavengeRegisterBackwards(
                RVGPU::SReg_32_XM0RegClass, MI, false, 0, false);
            Register ScaledReg = TmpScaledReg.isValid() ? TmpScaledReg : FrameReg;

            BuildMI(*MBB, MI, DL, TII->get(RVGPU::S_LSHR_B32), ScaledReg)
              .addReg(FrameReg)
              .addImm(ST.getWavefrontSizeLog2());
            BuildMI(*MBB, MI, DL, TII->get(RVGPU::S_ADD_I32), ScaledReg)
                .addReg(ScaledReg, RegState::Kill)
                .addImm(Offset);
            if (!IsSALU)
              BuildMI(*MBB, MI, DL, TII->get(RVGPU::COPY), ResultReg)
                  .addReg(ScaledReg, RegState::Kill);
            else
              ResultReg = ScaledReg;
            ...
            if (!TmpScaledReg.isValid()) {
              BuildMI(*MBB, MI, DL, TII->get(RVGPU::S_ADD_I32), ScaledReg)
                .addReg(ScaledReg, RegState::Kill)
                .addImm(-Offset);
              BuildMI(*MBB, MI, DL, TII->get(RVGPU::S_LSHL_B32), ScaledReg)
                .addReg(FrameReg)
                .addImm(ST.getWavefrontSizeLog2());
            }
          }
      ...
      if (IsMUBUF) {
        ...
        auto &SOffset = *TII->getNamedOperand(*MI, RVGPU::OpName::soffset);
        assert((SOffset.isImm() && SOffset.getImm() == 0));

        if (FrameReg != RVGPU::NoRegister)
      ...
      FIOp.ChangeToImmediate(Offset);
      if (!TII->isImmOperandLegal(*MI, FIOperandNum, FIOp)) {
        Register TmpReg = RS->scavengeRegisterBackwards(RVGPU::VGPR_32RegClass,
                                                        MI, false, 0);
        BuildMI(*MBB, MI, DL, TII->get(RVGPU::V_MOV_B32_e32), TmpReg)
          .addImm(Offset);
...
StringRef RVRegisterInfo::getRegAsmName(MCRegister Reg) const {
  return RVGPUInstPrinter::getRegisterName(Reg);
}

unsigned RVGPU::getRegBitWidth(const TargetRegisterClass &RC) {
...
getAnyVGPRClassForBitWidth(unsigned BitWidth) {
  if (BitWidth == 64)
    return &RVGPU::VReg_64RegClass;
  if (BitWidth == 96)
    return &RVGPU::VReg_96RegClass;
  if (BitWidth == 128)
    return &RVGPU::VReg_128RegClass;
  if (BitWidth == 160)
    return &RVGPU::VReg_160RegClass;
  if (BitWidth == 192)
    return &RVGPU::VReg_192RegClass;
  if (BitWidth == 224)
    return &RVGPU::VReg_224RegClass;
  if (BitWidth == 256)
    return &RVGPU::VReg_256RegClass;
  if (BitWidth == 288)
    return &RVGPU::VReg_288RegClass;
  if (BitWidth == 320)
    return &RVGPU::VReg_320RegClass;
  if (BitWidth == 352)
    return &RVGPU::VReg_352RegClass;
  if (BitWidth == 384)
    return &RVGPU::VReg_384RegClass;
  if (BitWidth == 512)
    return &RVGPU::VReg_512RegClass;
  if (BitWidth == 1024)
    return &RVGPU::VReg_1024RegClass;
...
getAlignedVGPRClassForBitWidth(unsigned BitWidth) {
  if (BitWidth == 64)
    return &RVGPU::VReg_64_Align2RegClass;
  if (BitWidth == 96)
    return &RVGPU::VReg_96_Align2RegClass;
  if (BitWidth == 128)
    return &RVGPU::VReg_128_Align2RegClass;
  if (BitWidth == 160)
    return &RVGPU::VReg_160_Align2RegClass;
  if (BitWidth == 192)
    return &RVGPU::VReg_192_Align2RegClass;
  if (BitWidth == 224)
    return &RVGPU::VReg_224_Align2RegClass;
  if (BitWidth == 256)
    return &RVGPU::VReg_256_Align2RegClass;
  if (BitWidth == 288)
    return &RVGPU::VReg_288_Align2RegClass;
  if (BitWidth == 320)
    return &RVGPU::VReg_320_Align2RegClass;
  if (BitWidth == 352)
    return &RVGPU::VReg_352_Align2RegClass;
  if (BitWidth == 384)
    return &RVGPU::VReg_384_Align2RegClass;
  if (BitWidth == 512)
    return &RVGPU::VReg_512_Align2RegClass;
  if (BitWidth == 1024)
    return &RVGPU::VReg_1024_Align2RegClass;

  return nullptr;
}

const TargetRegisterClass *
RVRegisterInfo::getVGPRClassForBitWidth(unsigned BitWidth) const {
  if (BitWidth == 1)
    return &RVGPU::VReg_1RegClass;
  if (BitWidth == 16)
    return &RVGPU::VGPR_LO16RegClass;
  if (BitWidth == 32)
    return &RVGPU::VGPR_32RegClass;
...
getAnyAGPRClassForBitWidth(unsigned BitWidth) {
  if (BitWidth == 64)
    return &RVGPU::AReg_64RegClass;
  if (BitWidth == 96)
    return &RVGPU::AReg_96RegClass;
  if (BitWidth == 128)
    return &RVGPU::AReg_128RegClass;
  if (BitWidth == 160)
    return &RVGPU::AReg_160RegClass;
  if (BitWidth == 192)
    return &RVGPU::AReg_192RegClass;
  if (BitWidth == 224)
    return &RVGPU::AReg_224RegClass;
  if (BitWidth == 256)
    return &RVGPU::AReg_256RegClass;
  if (BitWidth == 288)
    return &RVGPU::AReg_288RegClass;
  if (BitWidth == 320)
    return &RVGPU::AReg_320RegClass;
  if (BitWidth == 352)
    return &RVGPU::AReg_352RegClass;
  if (BitWidth == 384)
    return &RVGPU::AReg_384RegClass;
  if (BitWidth == 512)
    return &RVGPU::AReg_512RegClass;
  if (BitWidth == 1024)
    return &RVGPU::AReg_1024RegClass;
...
getAlignedAGPRClassForBitWidth(unsigned BitWidth) {
  if (BitWidth == 64)
    return &RVGPU::AReg_64_Align2RegClass;
  if (BitWidth == 96)
    return &RVGPU::AReg_96_Align2RegClass;
  if (BitWidth == 128)
    return &RVGPU::AReg_128_Align2RegClass;
  if (BitWidth == 160)
    return &RVGPU::AReg_160_Align2RegClass;
  if (BitWidth == 192)
    return &RVGPU::AReg_192_Align2RegClass;
  if (BitWidth == 224)
    return &RVGPU::AReg_224_Align2RegClass;
  if (BitWidth == 256)
    return &RVGPU::AReg_256_Align2RegClass;
  if (BitWidth == 288)
    return &RVGPU::AReg_288_Align2RegClass;
  if (BitWidth == 320)
    return &RVGPU::AReg_320_Align2RegClass;
  if (BitWidth == 352)
    return &RVGPU::AReg_352_Align2RegClass;
  if (BitWidth == 384)
    return &RVGPU::AReg_384_Align2RegClass;
  if (BitWidth == 512)
    return &RVGPU::AReg_512_Align2RegClass;
  if (BitWidth == 1024)
    return &RVGPU::AReg_1024_Align2RegClass;

  return nullptr;
}

const TargetRegisterClass *
RVRegisterInfo::getAGPRClassForBitWidth(unsigned BitWidth) const {
  if (BitWidth == 16)
    return &RVGPU::AGPR_LO16RegClass;
  if (BitWidth == 32)
    return &RVGPU::AGPR_32RegClass;
...
getAnyVectorSuperClassForBitWidth(unsigned BitWidth) {
  ...
  if (BitWidth == 192)
    return &RVGPU::AV_192RegClass;
  if (BitWidth == 224)
    return &RVGPU::AV_224RegClass;
  if (BitWidth == 256)
    return &RVGPU::AV_256RegClass;
  if (BitWidth == 288)
    return &RVGPU::AV_288RegClass;
  if (BitWidth == 320)
    return &RVGPU::AV_320RegClass;
  if (BitWidth == 352)
    return &RVGPU::AV_352RegClass;
  if (BitWidth == 384)
    return &RVGPU::AV_384RegClass;
  if (BitWidth == 512)
    return &RVGPU::AV_512RegClass;
  if (BitWidth == 1024)
    return &RVGPU::AV_1024RegClass;
...
getAlignedVectorSuperClassForBitWidth(unsigned BitWidth) {
  ...
  if (BitWidth == 192)
    return &RVGPU::AV_192_Align2RegClass;
  if (BitWidth == 224)
    return &RVGPU::AV_224_Align2RegClass;
  if (BitWidth == 256)
    return &RVGPU::AV_256_Align2RegClass;
  if (BitWidth == 288)
    return &RVGPU::AV_288_Align2RegClass;
  if (BitWidth == 320)
    return &RVGPU::AV_320_Align2RegClass;
  if (BitWidth == 352)
    return &RVGPU::AV_352_Align2RegClass;
  if (BitWidth == 384)
    return &RVGPU::AV_384_Align2RegClass;
  if (BitWidth == 512)
    return &RVGPU::AV_512_Align2RegClass;
  if (BitWidth == 1024)
    return &RVGPU::AV_1024_Align2RegClass;
...
const TargetRegisterClass *
RVRegisterInfo::getVectorSuperClassForBitWidth(unsigned BitWidth) const {
  if (BitWidth == 16)
    return &RVGPU::VGPR_LO16RegClass;
  if (BitWidth == 32)
    return &RVGPU::AV_32RegClass;
...
const TargetRegisterClass *
RVRegisterInfo::getSGPRClassForBitWidth(unsigned BitWidth) {
  if (BitWidth == 16)
    return &RVGPU::SGPR_LO16RegClass;
  if (BitWidth == 32)
    return &RVGPU::SReg_32RegClass;
  if (BitWidth == 64)
    return &RVGPU::SReg_64RegClass;
  if (BitWidth == 96)
    return &RVGPU::SGPR_96RegClass;
  if (BitWidth == 128)
    return &RVGPU::SGPR_128RegClass;
  if (BitWidth == 160)
    return &RVGPU::SGPR_160RegClass;
  if (BitWidth == 192)
    return &RVGPU::SGPR_192RegClass;
  if (BitWidth == 224)
    return &RVGPU::SGPR_224RegClass;
  if (BitWidth == 256)
    return &RVGPU::SGPR_256RegClass;
  if (BitWidth == 288)
    return &RVGPU::SGPR_288RegClass;
  if (BitWidth == 320)
    return &RVGPU::SGPR_320RegClass;
  if (BitWidth == 352)
    return &RVGPU::SGPR_352RegClass;
  if (BitWidth == 384)
    return &RVGPU::SGPR_384RegClass;
  if (BitWidth == 512)
    return &RVGPU::SGPR_512RegClass;
  if (BitWidth == 1024)
    return &RVGPU::SGPR_1024RegClass;

  return nullptr;
}

bool RVRegisterInfo::isSGPRReg(const MachineRegisterInfo &MRI,
                               Register Reg) const {
  const TargetRegisterClass *RC;
  if (Reg.isVirtual())
...
const TargetRegisterClass *
RVRegisterInfo::getEquivalentVGPRClass(const TargetRegisterClass *SRC) const {
...
const TargetRegisterClass *
RVRegisterInfo::getEquivalentAGPRClass(const TargetRegisterClass *SRC) const {
  unsigned Size = getRegSizeInBits(*SRC);
...
const TargetRegisterClass *
RVRegisterInfo::getEquivalentSGPRClass(const TargetRegisterClass *VRC) const {
  unsigned Size = getRegSizeInBits(*VRC);
  if (Size == 32)
    return &RVGPU::SGPR_32RegClass;
...
const TargetRegisterClass *
RVRegisterInfo::getCompatibleSubRegClass(const TargetRegisterClass *SuperRC,
...
bool RVRegisterInfo::opCanUseInlineConstant(unsigned OpType) const {
  if (OpType >= RVGPU::OPERAND_REG_INLINE_AC_FIRST &&
      OpType <= RVGPU::OPERAND_REG_INLINE_AC_LAST)
    return !ST.hasMFMAInlineLiteralBug();

  return OpType >= RVGPU::OPERAND_SRC_FIRST &&
         OpType <= RVGPU::OPERAND_SRC_LAST;
}

bool RVRegisterInfo::shouldRewriteCopySrc(
...
bool RVRegisterInfo::opCanUseLiteralConstant(unsigned OpType) const {
  // TODO: 64-bit operands have extending behavior from 32-bit literal.
  return OpType >= RVGPU::OPERAND_REG_IMM_FIRST &&
         OpType <= RVGPU::OPERAND_REG_IMM_LAST;
}

/// Returns a lowest register that is not used at any point in the function.
///        If all registers are used, then this function will return
///         RVGPU::NoRegister. If \p ReserveHighestRegister = true, then return
///         highest unused register.
MCRegister RVRegisterInfo::findUnusedRegister(
...
bool RVRegisterInfo::isUniformReg(const MachineRegisterInfo &MRI,
...
ArrayRef<int16_t> RVRegisterInfo::getRegSplitParts(const TargetRegisterClass *RC,
                                                   unsigned EltSize) const {
  const unsigned RegBitWidth = RVGPU::getRegBitWidth(*RC);
...
const TargetRegisterClass*
RVRegisterInfo::getRegClassForReg(const MachineRegisterInfo &MRI,
                                  Register Reg) const {
  return Reg.isVirtual() ? MRI.getRegClass(Reg) : getPhysRegBaseClass(Reg);
}

const TargetRegisterClass *
RVRegisterInfo::getRegClassForOperandReg(const MachineRegisterInfo &MRI,
                                         const MachineOperand &MO) const {
  const TargetRegisterClass *SrcRC = getRegClassForReg(MRI, MO.getReg());
  return getSubRegisterClass(SrcRC, MO.getSubReg());
}

bool RVRegisterInfo::isVGPR(const MachineRegisterInfo &MRI,
                            Register Reg) const {
...
bool RVRegisterInfo::isAGPR(const MachineRegisterInfo &MRI,
                            Register Reg) const {
  const TargetRegisterClass *RC = getRegClassForReg(MRI, Reg);
...
bool RVRegisterInfo::shouldCoalesce(MachineInstr *MI,
                                    const TargetRegisterClass *SrcRC,
                                    unsigned SubReg,
...
unsigned RVRegisterInfo::getRegPressureLimit(const TargetRegisterClass *RC,
  ...
  switch (RC->getID()) {
  default:
    return RVGPUGenRegisterInfo::getRegPressureLimit(RC, MF);
  case RVGPU::VGPR_32RegClassID:
  case RVGPU::VGPR_LO16RegClassID:
  case RVGPU::VGPR_HI16RegClassID:
    return std::min(ST.getMaxNumVGPRs(Occupancy), ST.getMaxNumVGPRs(MF));
  case RVGPU::SGPR_32RegClassID:
  case RVGPU::SGPR_LO16RegClassID:
    return std::min(ST.getMaxNumSGPRs(Occupancy, true), ST.getMaxNumSGPRs(MF));
  }
}

unsigned RVRegisterInfo::getRegPressureSetLimit(const MachineFunction &MF,
                                                unsigned Idx) const {
  if (Idx == RVGPU::RegisterPressureSets::VGPR_32 ||
      Idx == RVGPU::RegisterPressureSets::AGPR_32)
    return getRegPressureLimit(&RVGPU::VGPR_32RegClass,
                               const_cast<MachineFunction &>(MF));

  if (Idx == RVGPU::RegisterPressureSets::SReg_32)
    return getRegPressureLimit(&RVGPU::SGPR_32RegClass,
                               const_cast<MachineFunction &>(MF));

  llvm_unreachable("Unexpected register pressure set!");
}

const int *RVRegisterInfo::getRegUnitPressureSets(unsigned RegUnit) const {
  static const int Empty[] = { -1 };

  if (RegPressureIgnoredUnits[RegUnit])
    return Empty;

  return RVGPUGenRegisterInfo::getRegUnitPressureSets(RegUnit);
}

MCRegister RVRegisterInfo::getReturnAddressReg(const MachineFunction &MF) const {
  // Not a callee saved register.
  return RVGPU::SGPR30_SGPR31;
}

const TargetRegisterClass *
RVRegisterInfo::getRegClassForSizeOnBank(unsigned Size,
                                         const RegisterBank &RB) const {
  switch (RB.getID()) {
  case RVGPU::VGPRRegBankID:
    return getVGPRClassForBitWidth(
        std::max(ST.useRealTrue16Insts() ? 16u : 32u, Size));
  case RVGPU::VCCRegBankID:
    assert(Size == 1);
    return isWave32 ? &RVGPU::SReg_32_XM0_XEXECRegClass
                    : &RVGPU::SReg_64_XEXECRegClass;
  case RVGPU::SGPRRegBankID:
    return getSGPRClassForBitWidth(std::max(32u, Size));
  case RVGPU::AGPRRegBankID:
...
const TargetRegisterClass *
RVRegisterInfo::getConstrainedRegClassForOperand(const MachineOperand &MO,
...
MCRegister RVRegisterInfo::getVCC() const {
  return isWave32 ? RVGPU::VCC_LO : RVGPU::VCC;
}

MCRegister RVRegisterInfo::getExec() const {
  return isWave32 ? RVGPU::EXEC_LO : RVGPU::EXEC;
}

const TargetRegisterClass *RVRegisterInfo::getVGPR64Class() const {
  // VGPR tuples have an alignment requirement on gfx90a variants.
  return ST.needsAlignedVGPRs() ? &RVGPU::VReg_64_Align2RegClass
                                : &RVGPU::VReg_64RegClass;
}

const TargetRegisterClass *
RVRegisterInfo::getRegClass(unsigned RCID) const {
  switch ((int)RCID) {
  case RVGPU::SReg_1RegClassID:
    return getBoolRC();
  case RVGPU::SReg_1_XEXECRegClassID:
    return isWave32 ? &RVGPU::SReg_32_XM0_XEXECRegClass
      : &RVGPU::SReg_64_XEXECRegClass;
  case -1:
    return nullptr;
  default:
    return RVGPUGenRegisterInfo::getRegClass(RCID);
  }
}

// Find reaching register definition
MachineInstr *RVRegisterInfo::findReachingDef(Register Reg, unsigned SubReg,
                                              MachineInstr &Use,
...
MCPhysReg RVRegisterInfo::get32BitRegister(MCPhysReg Reg) const {
  assert(getRegSizeInBits(*getPhysRegBaseClass(Reg)) <= 32);

  for (const TargetRegisterClass &RC : { RVGPU::VGPR_32RegClass,
                                         RVGPU::SReg_32RegClass,
                                         RVGPU::AGPR_32RegClass } ) {
    if (MCPhysReg Super = getMatchingSuperReg(Reg, RVGPU::lo16, &RC))
      return Super;
  }
  if (MCPhysReg Super = getMatchingSuperReg(Reg, RVGPU::hi16,
                                            &RVGPU::VGPR_32RegClass)) {
      return Super;
  }
...
bool RVRegisterInfo::isProperlyAlignedRC(const TargetRegisterClass &RC) const {
...
const TargetRegisterClass *
RVRegisterInfo::getProperlyAlignedRC(const TargetRegisterClass *RC) const {
...
ArrayRef<MCPhysReg>
RVRegisterInfo::getAllSGPR128(const MachineFunction &MF) const {
  return ArrayRef(RVGPU::SGPR_128RegClass.begin(), ST.getMaxNumSGPRs(MF) / 4);
}

ArrayRef<MCPhysReg>
RVRegisterInfo::getAllSGPR64(const MachineFunction &MF) const {
  return ArrayRef(RVGPU::SGPR_64RegClass.begin(), ST.getMaxNumSGPRs(MF) / 2);
}

ArrayRef<MCPhysReg>
RVRegisterInfo::getAllSGPR32(const MachineFunction &MF) const {
  return ArrayRef(RVGPU::SGPR_32RegClass.begin(), ST.getMaxNumSGPRs(MF));
}

unsigned
RVRegisterInfo::getSubRegAlignmentNumBits(const TargetRegisterClass *RC,
