...
static void generateEndPgm(MachineBasicBlock &MBB,
                           ...
                           const RVInstrInfo *TII, MachineFunction &MF) {
  const Function &F = MF.getFunction();
  bool IsPS = F.getCallingConv() == CallingConv::RVGPU_PS;

  // Check if hardware has been configured to expect color or depth exports.
  bool HasColorExports = RVGPU::getHasColorExport(F);
  bool HasDepthExports = RVGPU::getHasDepthExport(F);
  bool HasExports = HasColorExports || HasDepthExports;

  // Prior to GFX10, hardware always expects at least one export for PS.
  bool MustExport = !RVGPU::isGFX10Plus(TII->getSubtarget());
  ...
  if (IsPS && (HasExports || MustExport)) {
    ...
    int Target =
        ST.hasNullExportTarget()
            ? RVGPU::Exp::ET_NULL
            : (HasColorExports ? RVGPU::Exp::ET_MRT0 : RVGPU::Exp::ET_MRTZ);
    BuildMI(MBB, I, DL, TII->get(RVGPU::EXP_DONE))
        .addImm(Target)
        .addReg(RVGPU::VGPR0, RegState::Undef)
        .addReg(RVGPU::VGPR0, RegState::Undef)
        .addReg(RVGPU::VGPR0, RegState::Undef)
        .addReg(RVGPU::VGPR0, RegState::Undef)
        .addImm(1)  // vm
  ...
  // s_endpgm
  BuildMI(MBB, I, DL, TII->get(RVGPU::S_ENDPGM)).addImm(0);
}
...
void SILateBranchLowering::expandChainCall(MachineInstr &MI) {
  ...
  MI.setDesc(TII->get(RVGPU::SI_TCRETURN));
}
...
void SILateBranchLowering::earlyTerm(MachineInstr &MI,
  ...
  auto BranchMI = BuildMI(MBB, MI, DL, TII->get(RVGPU::S_CBRANCH_SCC0))
...
bool SILateBranchLowering::runOnMachineFunction(MachineFunction &MF) {
  ...
  MovOpc = ST.isWave32() ? RVGPU::S_MOV_B32 : RVGPU::S_MOV_B64;
  ExecReg = ST.isWave32() ? RVGPU::EXEC_LO : RVGPU::EXEC;
  ...
  for (MachineBasicBlock &MBB : MF) {
    for (MachineInstr &MI : llvm::make_early_inc_range(MBB)) {
      ...
      case RVGPU::SI_CS_CHAIN_TC_W32:
      case RVGPU::SI_CS_CHAIN_TC_W64:
        expandChainCall(MI);
        MadeChange = true;
        break;

      case RVGPU::SI_EARLY_TERMINATE_SCC0:
        EarlyTermInstrs.push_back(&MI);
        break;

      case RVGPU::SI_RETURN_TO_EPILOG:
  ...
  if (!EarlyTermInstrs.empty()) {
    ...
    for (MachineInstr *Instr : EarlyTermInstrs) {
      ...
      if (MF.getFunction().getCallingConv() != CallingConv::RVGPU_GS)
  ...
  if (!EpilogInstrs.empty()) {
    ...
    assert(!MF.getInfo<RVMachineFunctionInfo>()->returnsVoid());
    ...
    for (auto *MI : EpilogInstrs) {
      ...
      BuildMI(*MBB, MI, MI->getDebugLoc(), TII->get(RVGPU::S_BRANCH))
