//===- RVGPUOpenCLEnqueuedBlockLowering.cpp - Lower enqueued block -------===//
//
...
#define DEBUG_TYPE "rvgpu-lower-enqueued-block"
...
class RVGPUOpenCLEnqueuedBlockLowering : public ModulePass {
public:
  static char ID;

  explicit RVGPUOpenCLEnqueuedBlockLowering() : ModulePass(ID) {}
...
char RVGPUOpenCLEnqueuedBlockLowering::ID = 0;

char &llvm::RVGPUOpenCLEnqueuedBlockLoweringID =
    RVGPUOpenCLEnqueuedBlockLowering::ID;

INITIALIZE_PASS(RVGPUOpenCLEnqueuedBlockLowering, DEBUG_TYPE,
                "Lower OpenCL enqueued blocks", false, false)

ModulePass* llvm::createRVGPUOpenCLEnqueuedBlockLoweringPass() {
  return new RVGPUOpenCLEnqueuedBlockLowering();
}

bool RVGPUOpenCLEnqueuedBlockLowering::runOnModule(Module &M) {
  ...
  for (auto &F : M.functions()) {
    if (F.hasFnAttribute("enqueued-block")) {
      if (!F.hasName()) {
        SmallString<64> Name;
        Mangler::getNameWithPrefix(Name, "__rvgpu_enqueued_kernel",
                                   M.getDataLayout());
        F.setName(Name);
      }
