//===-- RVGPUISelLowering.h - RVGPU Lowering Interface --------*- C++ -*-===//
//
...
private:
  ...
  /// \returns RVGPUISD::FFBH_U32 node if the incoming \p Op may have been
...
protected:
  ...
  virtual SDValue LowerGlobalAddress(RVGPUMachineFunction *MFI, SDValue Op,
...
public:
  RVGPUTargetLowering(const TargetMachine &TM, const RVGPUSubtarget &STI);
  ...
  // targets.  Enough RVGPU compiles go into an infinite loop (
...
enum NodeType : unsigned {
  ...
  LAST_RVGPU_ISD_NUMBER
};

} // End namespace RVGPUISD
