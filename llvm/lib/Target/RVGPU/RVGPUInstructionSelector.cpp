//===- RVGPUInstructionSelector.cpp ----------------------------*- C++ -*-==//
//
...
static cl::opt<bool> AllowRiskySelect(
  "rvgpu-global-isel-risky-select",
...
RVGPUInstructionSelector::RVGPUInstructionSelector(
    const RVSubtarget &STI, const RVGPURegisterBankInfo &RBI,
...
const char *RVGPUInstructionSelector::getName() { return DEBUG_TYPE; }

void RVGPUInstructionSelector::setupMF(MachineFunction &MF, GISelKnownBits *KB,
...
static Register getWaveAddress(const MachineInstr *Def) {
  return Def->getOpcode() == RVGPU::G_RVGPU_WAVE_ADDRESS
             ? Def->getOperand(1).getReg()
             : Register();
}

bool RVGPUInstructionSelector::isVCC(Register Reg,
  ...
  if (RC) {
    ...
    return MRI.getVRegDef(Reg)->getOpcode() != RVGPU::G_TRUNC &&
           RC->hasSuperClassEq(TRI.getBoolRC());
  }

  const RegisterBank *RB = RegClassOrBank.get<const RegisterBank *>();
  return RB->getID() == RVGPU::VCCRegBankID;
}

bool RVGPUInstructionSelector::constrainCopyLikeIntrin(MachineInstr &MI,
                                                        unsigned NewOpc) const {
  MI.setDesc(TII.get(NewOpc));
  MI.removeOperand(1); // Remove intrinsic ID.
  MI.addOperand(*MF, MachineOperand::CreateReg(RVGPU::EXEC, false, true));
...
bool RVGPUInstructionSelector::selectCOPY(MachineInstr &I) const {
  ...
  if (isVCC(DstReg, *MRI)) {
    ...
    if (!isVCC(SrcReg, *MRI)) {
      ...
      if (ConstVal) {
        unsigned MovOpc =
            STI.isWave64() ? RVGPU::S_MOV_B64 : RVGPU::S_MOV_B32;
        ...
        unsigned AndOpc =
            IsSGPR ? RVGPU::S_AND_B32 : RVGPU::V_AND_B32_e32;
        ...
        BuildMI(*BB, &I, DL, TII.get(RVGPU::V_CMP_NE_U32_e64), DstReg)
            .addImm(0)
            .addReg(MaskedReg);
      }
...
bool RVGPUInstructionSelector::selectPHI(MachineInstr &I) const {
...
MachineOperand
RVGPUInstructionSelector::getSubOperand64(MachineOperand &MO,
  ...
  if (MO.isReg()) {
    ...
    BuildMI(*BB, MI, MI->getDebugLoc(), TII.get(RVGPU::COPY), DstReg)
...
static unsigned getLogicalBitOpcode(unsigned Opc, bool Is64) {
  switch (Opc) {
  case RVGPU::G_AND:
    return Is64 ? RVGPU::S_AND_B64 : RVGPU::S_AND_B32;
  case RVGPU::G_OR:
    return Is64 ? RVGPU::S_OR_B64 : RVGPU::S_OR_B32;
  case RVGPU::G_XOR:
    return Is64 ? RVGPU::S_XOR_B64 : RVGPU::S_XOR_B32;
  default:
    llvm_unreachable("not a bit op");
  }
}

bool RVGPUInstructionSelector::selectG_AND_OR_XOR(MachineInstr &I) const {
  Register DstReg = I.getOperand(0).getReg();
  unsigned Size = RBI.getSizeInBits(DstReg, *MRI, TRI);

  const RegisterBank *DstRB = RBI.getRegBank(DstReg, *MRI, TRI);
  if (DstRB->getID() != RVGPU::SGPRRegBankID &&
      DstRB->getID() != RVGPU::VCCRegBankID)
    return false;

  bool Is64 = Size > 32 || (DstRB->getID() == RVGPU::VCCRegBankID &&
                            STI.isWave64());
  I.setDesc(TII.get(getLogicalBitOpcode(I.getOpcode(), Is64)));

  // Dead implicit-def of scc
  I.addOperand(MachineOperand::CreateReg(RVGPU::SCC, true, // isDef
                                         true, // isImp
                                         false, // isKill
                                         true)); // isDead
...
bool RVGPUInstructionSelector::selectG_ADD_SUB(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  ...
  const RegisterBank *DstRB = RBI.getRegBank(DstReg, *MRI, TRI);
  const bool IsSALU = DstRB->getID() == RVGPU::SGPRRegBankID;
  const bool Sub = I.getOpcode() == TargetOpcode::G_SUB;

  if (Size == 32) {
    if (IsSALU) {
      const unsigned Opc = Sub ? RVGPU::S_SUB_U32 : RVGPU::S_ADD_U32;
      MachineInstr *Add =
    ...
    if (STI.hasAddNoCarry()) {
      const unsigned Opc = Sub ? RVGPU::V_SUB_U32_e64 : RVGPU::V_ADD_U32_e64;
      I.setDesc(TII.get(Opc));
      I.addOperand(*MF, MachineOperand::CreateImm(0));
      I.addOperand(*MF, MachineOperand::CreateReg(RVGPU::EXEC, false, true));
      return constrainSelectedInstRegOperands(I, TII, TRI, RBI);
    }

    const unsigned Opc = Sub ? RVGPU::V_SUB_CO_U32_e64 : RVGPU::V_ADD_CO_U32_e64;
  ...
  const TargetRegisterClass &RC
    = IsSALU ? RVGPU::SReg_64_XEXECRegClass : RVGPU::VReg_64RegClass;
  const TargetRegisterClass &HalfRC
    = IsSALU ? RVGPU::SReg_32RegClass : RVGPU::VGPR_32RegClass;

  MachineOperand Lo1(getSubOperand64(I.getOperand(1), HalfRC, RVGPU::sub0));
  MachineOperand Lo2(getSubOperand64(I.getOperand(2), HalfRC, RVGPU::sub0));
  MachineOperand Hi1(getSubOperand64(I.getOperand(1), HalfRC, RVGPU::sub1));
  MachineOperand Hi2(getSubOperand64(I.getOperand(2), HalfRC, RVGPU::sub1));

  Register DstLo = MRI->createVirtualRegister(&HalfRC);
  Register DstHi = MRI->createVirtualRegister(&HalfRC);

  if (IsSALU) {
    BuildMI(*BB, &I, DL, TII.get(RVGPU::S_ADD_U32), DstLo)
      .add(Lo1)
      .add(Lo2);
    BuildMI(*BB, &I, DL, TII.get(RVGPU::S_ADDC_U32), DstHi)
      .add(Hi1)
      .add(Hi2)
      .setOperandDead(3); // Dead scc
  } else {
    ...
    BuildMI(*BB, &I, DL, TII.get(RVGPU::V_ADD_CO_U32_e64), DstLo)
      .addDef(CarryReg)
      .add(Lo1)
      .add(Lo2)
      .addImm(0);
    MachineInstr *Addc = BuildMI(*BB, &I, DL, TII.get(RVGPU::V_ADDC_U32_e64), DstHi)
  ...
  BuildMI(*BB, &I, DL, TII.get(RVGPU::REG_SEQUENCE), DstReg)
    .addReg(DstLo)
    .addImm(RVGPU::sub0)
    .addReg(DstHi)
    .addImm(RVGPU::sub1);
...
bool RVGPUInstructionSelector::selectG_UADDO_USUBO_UADDE_USUBE(
  ...
  const bool IsAdd = I.getOpcode() == RVGPU::G_UADDO ||
                     I.getOpcode() == RVGPU::G_UADDE;
  const bool HasCarryIn = I.getOpcode() == RVGPU::G_UADDE ||
                          I.getOpcode() == RVGPU::G_USUBE;

  if (isVCC(Dst1Reg, *MRI)) {
    unsigned NoCarryOpc =
        IsAdd ? RVGPU::V_ADD_CO_U32_e64 : RVGPU::V_SUB_CO_U32_e64;
    unsigned CarryOpc = IsAdd ? RVGPU::V_ADDC_U32_e64 : RVGPU::V_SUBB_U32_e64;
  ...
  if (HasCarryIn) {
    BuildMI(*BB, &I, DL, TII.get(RVGPU::COPY), RVGPU::SCC)
      .addReg(I.getOperand(4).getReg());
  }

  unsigned NoCarryOpc = IsAdd ? RVGPU::S_ADD_U32 : RVGPU::S_SUB_U32;
  unsigned CarryOpc = IsAdd ? RVGPU::S_ADDC_U32 : RVGPU::S_SUBB_U32;
  ...
  if (MRI->use_nodbg_empty(Dst1Reg)) {
    ...
    BuildMI(*BB, &I, DL, TII.get(RVGPU::COPY), Dst1Reg)
      .addReg(RVGPU::SCC);
    if (!MRI->getRegClassOrNull(Dst1Reg))
      MRI->setRegClass(Dst1Reg, &RVGPU::SReg_32RegClass);
  }

  if (!RBI.constrainGenericRegister(Dst0Reg, RVGPU::SReg_32RegClass, *MRI) ||
      !RBI.constrainGenericRegister(Src0Reg, RVGPU::SReg_32RegClass, *MRI) ||
      !RBI.constrainGenericRegister(Src1Reg, RVGPU::SReg_32RegClass, *MRI))
    return false;

  if (HasCarryIn &&
      !RBI.constrainGenericRegister(I.getOperand(4).getReg(),
                                    RVGPU::SReg_32RegClass, *MRI))
    return false;
...
bool RVGPUInstructionSelector::selectG_RVGPU_MAD_64_32(
    MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  MachineFunction *MF = BB->getParent();
  const bool IsUnsigned = I.getOpcode() == RVGPU::G_RVGPU_MAD_U64_U32;
...
bool RVGPUInstructionSelector::selectG_EXTRACT(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  Register DstReg = I.getOperand(0).getReg();
  ...
  if (!SrcRC)
    return false;
  unsigned SubReg = RVRegisterInfo::getSubRegFromChannel(Offset / 32,
                                                         DstSize / 32);
...
bool RVGPUInstructionSelector::selectG_MERGE_VALUES(MachineInstr &MI) const {
...
bool RVGPUInstructionSelector::selectG_UNMERGE_VALUES(MachineInstr &MI) const {
  MachineBasicBlock *BB = MI.getParent();
...
bool RVGPUInstructionSelector::selectG_BUILD_VECTOR(MachineInstr &MI) const {
  assert(MI.getOpcode() == RVGPU::G_BUILD_VECTOR_TRUNC ||
         MI.getOpcode() == RVGPU::G_BUILD_VECTOR);
  ...
  if (MI.getOpcode() == RVGPU::G_BUILD_VECTOR && SrcSize >= 32) {
  ...
  if (MRI->getType(Dst) != LLT::fixed_vector(2, 16) ||
      (MI.getOpcode() == RVGPU::G_BUILD_VECTOR_TRUNC &&
       SrcTy != LLT::scalar(32)))
    return selectImpl(MI, *CoverageInfo);

  const RegisterBank *DstBank = RBI.getRegBank(Dst, *MRI, TRI);
  if (DstBank->getID() == RVGPU::AGPRRegBankID)
    return false;

  assert(DstBank->getID() == RVGPU::SGPRRegBankID ||
         DstBank->getID() == RVGPU::VGPRRegBankID);
  const bool IsVector = DstBank->getID() == RVGPU::VGPRRegBankID;
  ...
  if (ConstSrc1) {
    ...
    if (ConstSrc0) {
      ...
      // VALU
      if (IsVector) {
        BuildMI(*BB, &MI, DL, TII.get(RVGPU::V_MOV_B32_e32), Dst).addImm(Imm);
        MI.eraseFromParent();
        return RBI.constrainGenericRegister(Dst, RVGPU::VGPR_32RegClass, *MRI);
      }

      // SALU
      BuildMI(*BB, &MI, DL, TII.get(RVGPU::S_MOV_B32), Dst).addImm(Imm);
      MI.eraseFromParent();
      return RBI.constrainGenericRegister(Dst, RVGPU::SReg_32RegClass, *MRI);
    }
  }
  ...
  if (Src1Def->getOpcode() == RVGPU::G_IMPLICIT_DEF) {
    MI.setDesc(TII.get(RVGPU::COPY));
    MI.removeOperand(2);
    const auto &RC =
        IsVector ? RVGPU::VGPR_32RegClass : RVGPU::SReg_32RegClass;
  ...
  if (IsVector) {
    ...
    auto MIB = BuildMI(*BB, MI, DL, TII.get(RVGPU::V_AND_B32_e32), TmpReg)
                   .addImm(0xFFFF)
                   .addReg(Src0);
    if (!constrainSelectedInstRegOperands(*MIB, TII, TRI, RBI))
      return false;

    MIB = BuildMI(*BB, MI, DL, TII.get(RVGPU::V_LSHL_OR_B32_e64), Dst)
              .addReg(Src1)
              .addImm(16)
              .addReg(TmpReg);
    if (!constrainSelectedInstRegOperands(*MIB, TII, TRI, RBI))
  ...
  unsigned Opc = RVGPU::S_PACK_LL_B32_B16;
  if (Shift0 && Shift1) {
    Opc = RVGPU::S_PACK_HH_B32_B16;
    MI.getOperand(1).setReg(ShiftSrc0);
    MI.getOperand(2).setReg(ShiftSrc1);
  } else if (Shift1) {
    Opc = RVGPU::S_PACK_LH_B32_B16;
    MI.getOperand(2).setReg(ShiftSrc1);
  } else if (Shift0) {
    auto ConstSrc1 =
    ...
    if (ConstSrc1 && ConstSrc1->Value == 0) {
      ...
      auto MIB = BuildMI(*BB, &MI, DL, TII.get(RVGPU::S_LSHR_B32), Dst)
                     .addReg(ShiftSrc0)
                     .addImm(16)
                     .setOperandDead(3); // Dead scc
...
bool RVGPUInstructionSelector::selectG_PTR_ADD(MachineInstr &I) const {
  return selectG_ADD_SUB(I);
}

bool RVGPUInstructionSelector::selectG_IMPLICIT_DEF(MachineInstr &I) const {
...
bool RVGPUInstructionSelector::selectG_INSERT(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
...
bool RVGPUInstructionSelector::selectG_SBFX_UBFX(MachineInstr &MI) const {
  Register DstReg = MI.getOperand(0).getReg();
  Register SrcReg = MI.getOperand(1).getReg();
  Register OffsetReg = MI.getOperand(2).getReg();
  Register WidthReg = MI.getOperand(3).getReg();

  assert(RBI.getRegBank(DstReg, *MRI, TRI)->getID() == RVGPU::VGPRRegBankID &&
  ...
  unsigned Opc = IsSigned ? RVGPU::V_BFE_I32_e64 : RVGPU::V_BFE_U32_e64;
...
bool RVGPUInstructionSelector::selectInterpP1F16(MachineInstr &MI) const {
  ...
  if (!RBI.constrainGenericRegister(M0Val, RVGPU::SReg_32RegClass, *MRI) ||
      !RBI.constrainGenericRegister(Dst, RVGPU::VGPR_32RegClass, *MRI) ||
      !RBI.constrainGenericRegister(Src0, RVGPU::VGPR_32RegClass, *MRI))
    return false;
  ...
  Register InterpMov = MRI->createVirtualRegister(&RVGPU::VGPR_32RegClass);
  const DebugLoc &DL = MI.getDebugLoc();
  MachineBasicBlock *MBB = MI.getParent();

  BuildMI(*MBB, &MI, DL, TII.get(RVGPU::COPY), RVGPU::M0)
    .addReg(M0Val);
  BuildMI(*MBB, &MI, DL, TII.get(RVGPU::V_INTERP_MOV_F32), InterpMov)
    .addImm(2)
    .addImm(MI.getOperand(4).getImm())  // $attr
    .addImm(MI.getOperand(3).getImm()); // $attrchan

  BuildMI(*MBB, &MI, DL, TII.get(RVGPU::V_INTERP_P1LV_F16), Dst)
...
// multiple SGPRs.
bool RVGPUInstructionSelector::selectWritelane(MachineInstr &MI) const {
  // With a constant bus limit of at least 2, there's no issue.
  if (STI.getConstantBusLimit(RVGPU::V_WRITELANE_B32) > 1)
    return selectImpl(MI, *CoverageInfo);
  ...
  auto MIB = BuildMI(*MBB, &MI, DL, TII.get(RVGPU::V_WRITELANE_B32), VDst);
  ...
  if (ConstSelect) {
    ...
    // copy to m0.
    if (ConstVal && RVGPU::isInlinableLiteral32(ConstVal->Value.getSExtValue(),
      ...
      RBI.constrainGenericRegister(LaneSelect, RVGPU::SReg_32_XM0RegClass, *MRI);

      BuildMI(*MBB, *MIB, DL, TII.get(RVGPU::COPY), RVGPU::M0)
        .addReg(LaneSelect);
      MIB.addReg(RVGPU::M0);
    }
  }
...
bool RVGPUInstructionSelector::selectDivScale(MachineInstr &MI) const {
  ...
  unsigned Opc;
  if (Ty == LLT::scalar(32))
    Opc = RVGPU::V_DIV_SCALE_F32_e64;
  else if (Ty == LLT::scalar(64))
    Opc = RVGPU::V_DIV_SCALE_F64_e64;
  else
    return false;
...
bool RVGPUInstructionSelector::selectG_INTRINSIC(MachineInstr &I) const {
  ...
  case Intrinsic::rvgpu_if_break: {
    ...
    BuildMI(*BB, &I, I.getDebugLoc(), TII.get(RVGPU::SI_IF_BREAK))
      .add(I.getOperand(0))
      .add(I.getOperand(2))
      .add(I.getOperand(3));
  ...
  case Intrinsic::rvgpu_wqm:
    return constrainCopyLikeIntrin(I, RVGPU::WQM);
  case Intrinsic::rvgpu_softwqm:
    return constrainCopyLikeIntrin(I, RVGPU::SOFT_WQM);
  case Intrinsic::rvgpu_strict_wwm:
  case Intrinsic::rvgpu_wwm:
    return constrainCopyLikeIntrin(I, RVGPU::STRICT_WWM);
  case Intrinsic::rvgpu_strict_wqm:
    return constrainCopyLikeIntrin(I, RVGPU::STRICT_WQM);
...
static int getV_CMPOpcode(CmpInst::Predicate P, unsigned Size,
  ...
  case CmpInst::ICMP_NE:
    return Select(RVGPU::V_CMP_NE_U16_e64, RVGPU::V_CMP_NE_U16_t16_e64,
                  RVGPU::V_CMP_NE_U32_e64, RVGPU::V_CMP_NE_U64_e64);
  case CmpInst::ICMP_EQ:
    return Select(RVGPU::V_CMP_EQ_U16_e64, RVGPU::V_CMP_EQ_U16_t16_e64,
                  RVGPU::V_CMP_EQ_U32_e64, RVGPU::V_CMP_EQ_U64_e64);
  case CmpInst::ICMP_SGT:
    return Select(RVGPU::V_CMP_GT_I16_e64, RVGPU::V_CMP_GT_I16_t16_e64,
                  RVGPU::V_CMP_GT_I32_e64, RVGPU::V_CMP_GT_I64_e64);
  case CmpInst::ICMP_SGE:
    return Select(RVGPU::V_CMP_GE_I16_e64, RVGPU::V_CMP_GE_I16_t16_e64,
                  RVGPU::V_CMP_GE_I32_e64, RVGPU::V_CMP_GE_I64_e64);
  case CmpInst::ICMP_SLT:
    return Select(RVGPU::V_CMP_LT_I16_e64, RVGPU::V_CMP_LT_I16_t16_e64,
                  RVGPU::V_CMP_LT_I32_e64, RVGPU::V_CMP_LT_I64_e64);
  case CmpInst::ICMP_SLE:
    return Select(RVGPU::V_CMP_LE_I16_e64, RVGPU::V_CMP_LE_I16_t16_e64,
                  RVGPU::V_CMP_LE_I32_e64, RVGPU::V_CMP_LE_I64_e64);
  case CmpInst::ICMP_UGT:
    return Select(RVGPU::V_CMP_GT_U16_e64, RVGPU::V_CMP_GT_U16_t16_e64,
                  RVGPU::V_CMP_GT_U32_e64, RVGPU::V_CMP_GT_U64_e64);
  case CmpInst::ICMP_UGE:
    return Select(RVGPU::V_CMP_GE_U16_e64, RVGPU::V_CMP_GE_U16_t16_e64,
                  RVGPU::V_CMP_GE_U32_e64, RVGPU::V_CMP_GE_U64_e64);
  case CmpInst::ICMP_ULT:
    return Select(RVGPU::V_CMP_LT_U16_e64, RVGPU::V_CMP_LT_U16_t16_e64,
                  RVGPU::V_CMP_LT_U32_e64, RVGPU::V_CMP_LT_U64_e64);
  case CmpInst::ICMP_ULE:
    return Select(RVGPU::V_CMP_LE_U16_e64, RVGPU::V_CMP_LE_U16_t16_e64,
                  RVGPU::V_CMP_LE_U32_e64, RVGPU::V_CMP_LE_U64_e64);

  case CmpInst::FCMP_OEQ:
    return Select(RVGPU::V_CMP_EQ_F16_e64, RVGPU::V_CMP_EQ_F16_t16_e64,
                  RVGPU::V_CMP_EQ_F32_e64, RVGPU::V_CMP_EQ_F64_e64);
  case CmpInst::FCMP_OGT:
    return Select(RVGPU::V_CMP_GT_F16_e64, RVGPU::V_CMP_GT_F16_t16_e64,
                  RVGPU::V_CMP_GT_F32_e64, RVGPU::V_CMP_GT_F64_e64);
  case CmpInst::FCMP_OGE:
    return Select(RVGPU::V_CMP_GE_F16_e64, RVGPU::V_CMP_GE_F16_t16_e64,
                  RVGPU::V_CMP_GE_F32_e64, RVGPU::V_CMP_GE_F64_e64);
  case CmpInst::FCMP_OLT:
    return Select(RVGPU::V_CMP_LT_F16_e64, RVGPU::V_CMP_LT_F16_t16_e64,
                  RVGPU::V_CMP_LT_F32_e64, RVGPU::V_CMP_LT_F64_e64);
  case CmpInst::FCMP_OLE:
    return Select(RVGPU::V_CMP_LE_F16_e64, RVGPU::V_CMP_LE_F16_t16_e64,
                  RVGPU::V_CMP_LE_F32_e64, RVGPU::V_CMP_LE_F64_e64);
  case CmpInst::FCMP_ONE:
    return Select(RVGPU::V_CMP_NEQ_F16_e64, RVGPU::V_CMP_NEQ_F16_t16_e64,
                  RVGPU::V_CMP_NEQ_F32_e64, RVGPU::V_CMP_NEQ_F64_e64);
  case CmpInst::FCMP_ORD:
    return Select(RVGPU::V_CMP_O_F16_e64, RVGPU::V_CMP_O_F16_t16_e64,
                  RVGPU::V_CMP_O_F32_e64, RVGPU::V_CMP_O_F64_e64);
  case CmpInst::FCMP_UNO:
    return Select(RVGPU::V_CMP_U_F16_e64, RVGPU::V_CMP_U_F16_t16_e64,
                  RVGPU::V_CMP_U_F32_e64, RVGPU::V_CMP_U_F64_e64);
  case CmpInst::FCMP_UEQ:
    return Select(RVGPU::V_CMP_NLG_F16_e64, RVGPU::V_CMP_NLG_F16_t16_e64,
                  RVGPU::V_CMP_NLG_F32_e64, RVGPU::V_CMP_NLG_F64_e64);
  case CmpInst::FCMP_UGT:
    return Select(RVGPU::V_CMP_NLE_F16_e64, RVGPU::V_CMP_NLE_F16_t16_e64,
                  RVGPU::V_CMP_NLE_F32_e64, RVGPU::V_CMP_NLE_F64_e64);
  case CmpInst::FCMP_UGE:
    return Select(RVGPU::V_CMP_NLT_F16_e64, RVGPU::V_CMP_NLT_F16_t16_e64,
                  RVGPU::V_CMP_NLT_F32_e64, RVGPU::V_CMP_NLT_F64_e64);
  case CmpInst::FCMP_ULT:
    return Select(RVGPU::V_CMP_NGE_F16_e64, RVGPU::V_CMP_NGE_F16_t16_e64,
                  RVGPU::V_CMP_NGE_F32_e64, RVGPU::V_CMP_NGE_F64_e64);
  case CmpInst::FCMP_ULE:
    return Select(RVGPU::V_CMP_NGT_F16_e64, RVGPU::V_CMP_NGT_F16_t16_e64,
                  RVGPU::V_CMP_NGT_F32_e64, RVGPU::V_CMP_NGT_F64_e64);
  case CmpInst::FCMP_UNE:
    return Select(RVGPU::V_CMP_NEQ_F16_e64, RVGPU::V_CMP_NEQ_F16_t16_e64,
                  RVGPU::V_CMP_NEQ_F32_e64, RVGPU::V_CMP_NEQ_F64_e64);
  case CmpInst::FCMP_TRUE:
    return Select(RVGPU::V_CMP_TRU_F16_e64, RVGPU::V_CMP_TRU_F16_t16_e64,
                  RVGPU::V_CMP_TRU_F32_e64, RVGPU::V_CMP_TRU_F64_e64);
  case CmpInst::FCMP_FALSE:
    return Select(RVGPU::V_CMP_F_F16_e64, RVGPU::V_CMP_F_F16_t16_e64,
                  RVGPU::V_CMP_F_F32_e64, RVGPU::V_CMP_F_F64_e64);
  }
}

int RVGPUInstructionSelector::getS_CMPOpcode(CmpInst::Predicate P,
                                              unsigned Size) const {
  if (Size == 64) {
    ...
    switch (P) {
    case CmpInst::ICMP_NE:
      return RVGPU::S_CMP_LG_U64;
    case CmpInst::ICMP_EQ:
      return RVGPU::S_CMP_EQ_U64;
    default:
      return -1;
    }
  }
  ...
  if (Size == 32) {
    switch (P) {
    case CmpInst::ICMP_NE:
      return RVGPU::S_CMP_LG_U32;
    case CmpInst::ICMP_EQ:
      return RVGPU::S_CMP_EQ_U32;
    case CmpInst::ICMP_SGT:
      return RVGPU::S_CMP_GT_I32;
    case CmpInst::ICMP_SGE:
      return RVGPU::S_CMP_GE_I32;
    case CmpInst::ICMP_SLT:
      return RVGPU::S_CMP_LT_I32;
    case CmpInst::ICMP_SLE:
      return RVGPU::S_CMP_LE_I32;
    case CmpInst::ICMP_UGT:
      return RVGPU::S_CMP_GT_U32;
    case CmpInst::ICMP_UGE:
      return RVGPU::S_CMP_GE_U32;
    case CmpInst::ICMP_ULT:
      return RVGPU::S_CMP_LT_U32;
    case CmpInst::ICMP_ULE:
      return RVGPU::S_CMP_LE_U32;
    case CmpInst::FCMP_OEQ:
      return RVGPU::S_CMP_EQ_F32;
    case CmpInst::FCMP_OGT:
      return RVGPU::S_CMP_GT_F32;
    case CmpInst::FCMP_OGE:
      return RVGPU::S_CMP_GE_F32;
    case CmpInst::FCMP_OLT:
      return RVGPU::S_CMP_LT_F32;
    case CmpInst::FCMP_OLE:
      return RVGPU::S_CMP_LE_F32;
    case CmpInst::FCMP_ONE:
      return RVGPU::S_CMP_LG_F32;
    case CmpInst::FCMP_ORD:
      return RVGPU::S_CMP_O_F32;
    case CmpInst::FCMP_UNO:
      return RVGPU::S_CMP_U_F32;
    case CmpInst::FCMP_UEQ:
      return RVGPU::S_CMP_NLG_F32;
    case CmpInst::FCMP_UGT:
      return RVGPU::S_CMP_NLE_F32;
    case CmpInst::FCMP_UGE:
      return RVGPU::S_CMP_NLT_F32;
    case CmpInst::FCMP_ULT:
      return RVGPU::S_CMP_NGE_F32;
    case CmpInst::FCMP_ULE:
      return RVGPU::S_CMP_NGT_F32;
    case CmpInst::FCMP_UNE:
      return RVGPU::S_CMP_NEQ_F32;
    default:
      llvm_unreachable("Unknown condition code!");
    }
  }
  ...
  if (Size == 16) {
    ...
    switch (P) {
    case CmpInst::FCMP_OEQ:
      return RVGPU::S_CMP_EQ_F16;
    case CmpInst::FCMP_OGT:
      return RVGPU::S_CMP_GT_F16;
    case CmpInst::FCMP_OGE:
      return RVGPU::S_CMP_GE_F16;
    case CmpInst::FCMP_OLT:
      return RVGPU::S_CMP_LT_F16;
    case CmpInst::FCMP_OLE:
      return RVGPU::S_CMP_LE_F16;
    case CmpInst::FCMP_ONE:
      return RVGPU::S_CMP_LG_F16;
    case CmpInst::FCMP_ORD:
      return RVGPU::S_CMP_O_F16;
    case CmpInst::FCMP_UNO:
      return RVGPU::S_CMP_U_F16;
    case CmpInst::FCMP_UEQ:
      return RVGPU::S_CMP_NLG_F16;
    case CmpInst::FCMP_UGT:
      return RVGPU::S_CMP_NLE_F16;
    case CmpInst::FCMP_UGE:
      return RVGPU::S_CMP_NLT_F16;
    case CmpInst::FCMP_ULT:
      return RVGPU::S_CMP_NGE_F16;
    case CmpInst::FCMP_ULE:
      return RVGPU::S_CMP_NGT_F16;
    case CmpInst::FCMP_UNE:
      return RVGPU::S_CMP_NEQ_F16;
    default:
      llvm_unreachable("Unknown condition code!");
    }
  }
...
bool RVGPUInstructionSelector::selectG_ICMP_or_FCMP(MachineInstr &I) const {
  ...
  if (!isVCC(CCReg, *MRI)) {
    ...
    MachineInstr *ICmp = BuildMI(*BB, &I, DL, TII.get(Opcode))
            .add(I.getOperand(2))
            .add(I.getOperand(3));
    BuildMI(*BB, &I, DL, TII.get(RVGPU::COPY), CCReg)
      .addReg(RVGPU::SCC);
    bool Ret =
        constrainSelectedInstRegOperands(*ICmp, TII, TRI, RBI) &&
        RBI.constrainGenericRegister(CCReg, RVGPU::SReg_32RegClass, *MRI);
    I.eraseFromParent();
    return Ret;
  }

  if (I.getOpcode() == RVGPU::G_FCMP)
    return false;
...
bool RVGPUInstructionSelector::selectIntrinsicCmp(MachineInstr &I) const {
  ...
  if (!CmpInst::isIntPredicate(Pred) && !CmpInst::isFPPredicate(Pred)) {
    BuildMI(*BB, &I, DL, TII.get(RVGPU::IMPLICIT_DEF), Dst);
    I.eraseFromParent();
  ...
  if (RVGPU::hasNamedOperand(Opcode, RVGPU::OpName::src0_modifiers))
    SelectedMI.addImm(Src0Mods);
  SelectedMI.addReg(Src0Reg);
  if (RVGPU::hasNamedOperand(Opcode, RVGPU::OpName::src1_modifiers))
    SelectedMI.addImm(Src1Mods);
  SelectedMI.addReg(Src1Reg);
  if (RVGPU::hasNamedOperand(Opcode, RVGPU::OpName::clamp))
    SelectedMI.addImm(0); // clamp
  if (RVGPU::hasNamedOperand(Opcode, RVGPU::OpName::op_sel))
...
bool RVGPUInstructionSelector::selectBallot(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  const DebugLoc &DL = I.getDebugLoc();
  Register DstReg = I.getOperand(0).getReg();
  ...
  const auto BuildCopy = [&](Register SrcReg) {
    if (Size == STI.getWavefrontSize()) {
      BuildMI(*BB, &I, DL, TII.get(RVGPU::COPY), DstReg)
          .addReg(SrcReg);
      return;
    }

    // If emitting a i64 ballot in wave32, fill the upper bits with zeroes.
    Register HiReg = MRI->createVirtualRegister(&RVGPU::SReg_32RegClass);
    BuildMI(*BB, &I, DL, TII.get(RVGPU::S_MOV_B32), HiReg).addImm(0);
    BuildMI(*BB, &I, DL, TII.get(RVGPU::REG_SEQUENCE), DstReg)
        .addReg(SrcReg)
        .addImm(RVGPU::sub0)
        .addReg(HiReg)
  ...
  if (Arg) {
    ...
    if (Value == 0) {
      unsigned Opcode = Is64 ? RVGPU::S_MOV_B64 : RVGPU::S_MOV_B32;
      BuildMI(*BB, &I, DL, TII.get(Opcode), DstReg).addImm(0);
    } else if (Value == -1) // all ones
      BuildCopy(IsWave32 ? RVGPU::EXEC_LO : RVGPU::EXEC);
    else
      return false;
  } else
...
bool RVGPUInstructionSelector::selectInverseBallot(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  const DebugLoc &DL = I.getDebugLoc();
  const Register DstReg = I.getOperand(0).getReg();
  const Register MaskReg = I.getOperand(2).getReg();

  BuildMI(*BB, &I, DL, TII.get(RVGPU::COPY), DstReg).addReg(MaskReg);
  I.eraseFromParent();
  return true;
}

bool RVGPUInstructionSelector::selectRelocConstant(MachineInstr &I) const {
  Register DstReg = I.getOperand(0).getReg();
  const RegisterBank *DstBank = RBI.getRegBank(DstReg, *MRI, TRI);
  ...
  const bool IsVALU = DstBank->getID() == RVGPU::VGPRRegBankID;
  ...
  BuildMI(*BB, &I, I.getDebugLoc(),
          TII.get(IsVALU ? RVGPU::V_MOV_B32_e32 : RVGPU::S_MOV_B32), DstReg)
    .addGlobalAddress(RelocSymbol, 0, RVInstrInfo::MO_ABS32_LO);

  I.eraseFromParent();
  return true;
}

bool RVGPUInstructionSelector::selectGroupStaticSize(MachineInstr &I) const {
  Triple::OSType OS = MF->getTarget().getTargetTriple().getOS();

  Register DstReg = I.getOperand(0).getReg();
  const RegisterBank *DstRB = RBI.getRegBank(DstReg, *MRI, TRI);
  unsigned Mov = DstRB->getID() == RVGPU::SGPRRegBankID ?
    RVGPU::S_MOV_B32 : RVGPU::V_MOV_B32_e32;
  ...
  if (OS == Triple::SS) {
    ...
    const GlobalValue *GV
      = Intrinsic::getDeclaration(M, Intrinsic::rvgpu_groupstaticsize);
    MIB.addGlobalAddress(GV, 0, RVInstrInfo::MO_ABS32_LO);
  }
...
bool RVGPUInstructionSelector::selectReturnAddress(MachineInstr &I) const {
  MachineBasicBlock *MBB = I.getParent();
  ...
  const TargetRegisterClass *RC
    = TRI.getConstrainedRegClassForOperand(Dst, *MRI);
  if (!RC->hasSubClassEq(&RVGPU::SGPR_64RegClass) ||
  ...
  if (Depth != 0 ||
    ...
    BuildMI(*MBB, &I, DL, TII.get(RVGPU::S_MOV_B64), DstReg)
      .addImm(0);
    I.eraseFromParent();
    return true;
  }
...
bool RVGPUInstructionSelector::selectEndCfIntrinsic(MachineInstr &MI) const {
  // FIXME: Manually selecting to avoid dealing with the SReg_1 trick
  // SelectionDAG uses for wave32 vs wave64.
  MachineBasicBlock *BB = MI.getParent();
  BuildMI(*BB, &MI, MI.getDebugLoc(), TII.get(RVGPU::SI_END_CF))
      .add(MI.getOperand(1));
...
bool RVGPUInstructionSelector::selectDSOrderedIntrinsic(
  ...
  unsigned ShaderType = RVInstrInfo::getDSShaderTypeValue(*MF);
  ...
  MachineInstrBuilder DS =
    BuildMI(*MBB, &MI, DL, TII.get(RVGPU::DS_ORDERED_COUNT), DstReg)
      .addReg(ValReg)
      .addImm(Offset)
      .cloneMemRefs(MI);
...
bool RVGPUInstructionSelector::selectDSGWSIntrinsic(MachineInstr &MI,
                                                     Intrinsic::ID IID) const {
  if (!STI.hasGWS() || (IID == Intrinsic::rvgpu_ds_gws_sema_release_all &&
  ...
  if (OffsetRB->getID() != RVGPU::SGPRRegBankID)
    return false;
  ...
  if (OffsetDef->getOpcode() == RVGPU::G_CONSTANT) {
    ...
    BuildMI(*MBB, &MI, DL, TII.get(RVGPU::S_MOV_B32), RVGPU::M0)
      .addImm(0);
  } else {
    std::tie(BaseOffset, ImmOffset) =
        RVGPU::getBaseWithConstantOffset(*MRI, BaseOffset, KB);

    if (Readfirstlane) {
      // We have the constant offset now, so put the readfirstlane back on the
      // variable component.
      if (!RBI.constrainGenericRegister(BaseOffset, RVGPU::VGPR_32RegClass, *MRI))
        return false;
    ...
    Register M0Base = MRI->createVirtualRegister(&RVGPU::SReg_32RegClass);
    BuildMI(*MBB, &MI, DL, TII.get(RVGPU::S_LSHL_B32), M0Base)
      .addReg(BaseOffset)
      .addImm(16)
      .setOperandDead(3); // Dead scc
  ...
  if (HasVSrc) {
    ...
    if (!RBI.constrainGenericRegister(VSrc, RVGPU::VGPR_32RegClass, *MRI))
      return false;
  }
  ...
  TII.enforceOperandRCAlignment(*MIB, RVGPU::OpName::data0);

  MI.eraseFromParent();
  return true;
}

bool RVGPUInstructionSelector::selectDSAppendConsume(MachineInstr &MI,
                                                      bool IsAppend) const {
  Register PtrBase = MI.getOperand(2).getReg();
  LLT PtrTy = MRI->getType(PtrBase);
  bool IsGDS = PtrTy.getAddressSpace() == RVGPUAS::REGION_ADDRESS;
  ...
  MachineBasicBlock *MBB = MI.getParent();
  const DebugLoc &DL = MI.getDebugLoc();
  const unsigned Opc = IsAppend ? RVGPU::DS_APPEND : RVGPU::DS_CONSUME;

  BuildMI(*MBB, &MI, DL, TII.get(RVGPU::COPY), RVGPU::M0)
    .addReg(PtrBase);
  if (!RBI.constrainGenericRegister(PtrBase, RVGPU::SReg_32RegClass, *MRI))
    return false;
...
bool RVGPUInstructionSelector::selectSBarrier(MachineInstr &MI) const {
  if (TM.getOptLevel() > CodeGenOptLevel::None) {
    unsigned WGSize = STI.getFlatWorkGroupSizes(MF->getFunction()).second;
    if (WGSize <= STI.getWavefrontSize()) {
      MachineBasicBlock *MBB = MI.getParent();
      const DebugLoc &DL = MI.getDebugLoc();
      BuildMI(*MBB, &MI, DL, TII.get(RVGPU::WAVE_BARRIER));
      MI.eraseFromParent();
      return true;
    }
  }
  ...
  if (STI.hasSplitBarriers()) {
    MachineBasicBlock *MBB = MI.getParent();
    const DebugLoc &DL = MI.getDebugLoc();
    BuildMI(*MBB, &MI, DL, TII.get(RVGPU::S_BARRIER_SIGNAL_IMM))
        .addImm(RVGPU::Barrier::WORKGROUP);
    BuildMI(*MBB, &MI, DL, TII.get(RVGPU::S_BARRIER_WAIT))
        .addImm(RVGPU::Barrier::WORKGROUP);
    MI.eraseFromParent();
    return true;
  }
...
bool RVGPUInstructionSelector::selectImageIntrinsic(
  MachineInstr &MI, const RVGPU::ImageDimIntrinsicInfo *Intr) const {
  MachineBasicBlock *MBB = MI.getParent();
  const DebugLoc &DL = MI.getDebugLoc();
  ...
  bool IsD16 = MI.getOpcode() == RVGPU::G_RVGPU_INTRIN_IMAGE_LOAD_D16 ||
               MI.getOpcode() == RVGPU::G_RVGPU_INTRIN_IMAGE_STORE_D16;
  ...
  if (BaseOpcode->Atomic) {
    ...
    if (BaseOpcode->AtomicX2) {
      assert(MI.getOperand(3).getReg() == RVGPU::NoRegister);
  ...
  if (UseNSA && !STI.hasFeature(RVGPU::FeatureNSAEncoding)) {
  ...
  if (IsGFX12Plus) {
    ...
    if (Opcode == -1 &&
        STI.getGeneration() >= RVGPUSubtarget::VOLCANIC_ISLANDS)
  ...
  if (VDataOut) {
    if (BaseOpcode->AtomicX2) {
      ...
      Register TmpReg = MRI->createVirtualRegister(
        Is64 ? &RVGPU::VReg_128RegClass : &RVGPU::VReg_64RegClass);
      unsigned SubReg = Is64 ? RVGPU::sub0_sub1 : RVGPU::sub0;

      MIB.addDef(TmpReg);
      if (!MRI->use_empty(VDataOut)) {
        BuildMI(*MBB, &MI, DL, TII.get(RVGPU::COPY), VDataOut)
  ...
  if (RVGPU::hasNamedOperand(Opcode, RVGPU::OpName::unorm))
    MIB.addImm(Unorm);

  MIB.addImm(CPol);
  MIB.addImm(IsA16 &&  // a16 or r128
             STI.hasFeature(RVGPU::FeatureR128A16) ? -1 : 0);
  if (IsGFX10Plus)
  ...
  if (RVGPU::hasNamedOperand(Opcode, RVGPU::OpName::lwe))
    MIB.addImm(LWE); // lwe
  if (!IsGFX10Plus)
  ...
  if (IsTexFail) {
    ...
    Register Zero = MRI->createVirtualRegister(&RVGPU::VGPR_32RegClass);
    BuildMI(*MBB, *MIB, DL, TII.get(RVGPU::V_MOV_B32_e32), Zero)
      .addImm(0);
    ...
    if (STI.usePRTStrictNull()) {
      ...
      // zero.
      auto RegSeq =
          BuildMI(*MBB, *MIB, DL, TII.get(RVGPU::REG_SEQUENCE), Tied);
      for (auto Sub : Parts)
        RegSeq.addReg(Zero).addImm(Sub);
    } else {
      // With enable-prt-strict-null disabled, only initialize the extra TFE/LWE
      // result register.
      Register Undef = MRI->createVirtualRegister(&RVGPU::VGPR_32RegClass);
      BuildMI(*MBB, *MIB, DL, TII.get(RVGPU::IMPLICIT_DEF), Undef);
      auto RegSeq =
          BuildMI(*MBB, *MIB, DL, TII.get(RVGPU::REG_SEQUENCE), Tied);
  ...
  TII.enforceOperandRCAlignment(*MIB, RVGPU::OpName::vaddr);
  return true;
}

// We need to handle this here because tablegen doesn't support matching
// instructions with multiple outputs.
bool RVGPUInstructionSelector::selectDSBvhStackIntrinsic(
  ...
  auto MIB = BuildMI(*MBB, &MI, DL, TII.get(RVGPU::DS_BVH_STACK_RTN_B32), Dst0)
                 .addDef(Dst1)
                 .addUse(Addr)
                 .addUse(Data0)
                 .addUse(Data1)
...
bool RVGPUInstructionSelector::selectG_INTRINSIC_W_SIDE_EFFECTS(
    MachineInstr &I) const {
  unsigned IntrinsicID = cast<GIntrinsic>(I).getIntrinsicID();
  switch (IntrinsicID) {
...
bool RVGPUInstructionSelector::selectG_SELECT(MachineInstr &I) const {
  if (selectImpl(I, *CoverageInfo))
    return true;
  ...
  if (!isVCC(CCReg, *MRI)) {
    unsigned SelectOpcode = Size == 64 ? RVGPU::S_CSELECT_B64 :
                                         RVGPU::S_CSELECT_B32;
    MachineInstr *CopySCC = BuildMI(*BB, &I, DL, TII.get(RVGPU::COPY), RVGPU::SCC)
            .addReg(CCReg);
  ...
  MachineInstr *Select =
      BuildMI(*BB, &I, DL, TII.get(RVGPU::V_CNDMASK_B32_e64), DstReg)
              .addImm(0)
              .add(I.getOperand(3))
              .addImm(0)
              .add(I.getOperand(2))
...
static int sizeToSubRegIndex(unsigned Size) {
  ...
  case 32:
    return RVGPU::sub0;
  case 64:
    return RVGPU::sub0_sub1;
  case 96:
    return RVGPU::sub0_sub1_sub2;
  case 128:
    return RVGPU::sub0_sub1_sub2_sub3;
  case 256:
    return RVGPU::sub0_sub1_sub2_sub3_sub4_sub5_sub6_sub7;
  default:
    if (Size < 32)
      return RVGPU::sub0;
    if (Size > 256)
...
bool RVGPUInstructionSelector::selectG_TRUNC(MachineInstr &I) const {
  Register DstReg = I.getOperand(0).getReg();
  Register SrcReg = I.getOperand(1).getReg();
  ...
  const bool IsVALU = DstRB->getID() == RVGPU::VGPRRegBankID;
  ...
  if (DstTy == LLT::fixed_vector(2, 16) && SrcTy == LLT::fixed_vector(2, 32)) {
    ...
    BuildMI(*MBB, I, DL, TII.get(RVGPU::COPY), LoReg)
      .addReg(SrcReg, 0, RVGPU::sub0);
    BuildMI(*MBB, I, DL, TII.get(RVGPU::COPY), HiReg)
      .addReg(SrcReg, 0, RVGPU::sub1);

    if (IsVALU && STI.hasSDWA()) {
      // Write the low 16-bits of the high element into the high 16-bits of the
      // low element.
      MachineInstr *MovSDWA =
        BuildMI(*MBB, I, DL, TII.get(RVGPU::V_MOV_B32_sdwa), DstReg)
        .addImm(0)                             // $src0_modifiers
        .addReg(HiReg)                         // $src0
        .addImm(0)                             // $clamp
        .addImm(RVGPU::SDWA::WORD_1)          // $dst_sel
        .addImm(RVGPU::SDWA::UNUSED_PRESERVE) // $dst_unused
        .addImm(RVGPU::SDWA::WORD_0)          // $src0_sel
      ...
      if (IsVALU) {
        BuildMI(*MBB, I, DL, TII.get(RVGPU::V_LSHLREV_B32_e64), TmpReg0)
          .addImm(16)
          .addReg(HiReg);
      } else {
        BuildMI(*MBB, I, DL, TII.get(RVGPU::S_LSHL_B32), TmpReg0)
          .addReg(HiReg)
          .addImm(16)
          .setOperandDead(3); // Dead scc
      }

      unsigned MovOpc = IsVALU ? RVGPU::V_MOV_B32_e32 : RVGPU::S_MOV_B32;
      unsigned AndOpc = IsVALU ? RVGPU::V_AND_B32_e64 : RVGPU::S_AND_B32;
      unsigned OrOpc = IsVALU ? RVGPU::V_OR_B32_e64 : RVGPU::S_OR_B32;
...
const RegisterBank *RVGPUInstructionSelector::getArtifactRegBank(
...
bool RVGPUInstructionSelector::selectG_SZA_EXT(MachineInstr &I) const {
  bool InReg = I.getOpcode() == RVGPU::G_SEXT_INREG;
  bool Signed = I.getOpcode() == RVGPU::G_SEXT || InReg;
  const DebugLoc &DL = I.getDebugLoc();
  MachineBasicBlock &MBB = *I.getParent();
  const Register DstReg = I.getOperand(0).getReg();
  ...
  const LLT DstTy = MRI->getType(DstReg);
  const LLT SrcTy = MRI->getType(SrcReg);
  const unsigned SrcSize = I.getOpcode() == RVGPU::G_SEXT_INREG ?
  ...
  if (I.getOpcode() == RVGPU::G_ANYEXT) {
    ...
    BuildMI(MBB, I, DL, TII.get(RVGPU::IMPLICIT_DEF), UndefReg);
    BuildMI(MBB, I, DL, TII.get(RVGPU::REG_SEQUENCE), DstReg)
      .addReg(SrcReg)
      .addImm(RVGPU::sub0)
      .addReg(UndefReg)
      .addImm(RVGPU::sub1);
  ...
  if (SrcBank->getID() == RVGPU::VGPRRegBankID && DstSize <= 32) {
    ...
    if (!Signed && shouldUseAndMask(SrcSize, Mask)) {
      MachineInstr *ExtI =
      BuildMI(MBB, I, DL, TII.get(RVGPU::V_AND_B32_e32), DstReg)
        .addImm(Mask)
        .addReg(SrcReg);
      I.eraseFromParent();
    ...
    const unsigned BFE = Signed ? RVGPU::V_BFE_I32_e64 : RVGPU::V_BFE_U32_e64;
    MachineInstr *ExtI =
  ...
  if (SrcBank->getID() == RVGPU::SGPRRegBankID && DstSize <= 64) {
    const TargetRegisterClass &SrcRC = InReg && DstSize > 32 ?
      RVGPU::SReg_64RegClass : RVGPU::SReg_32RegClass;
    if (!RBI.constrainGenericRegister(SrcReg, SrcRC, *MRI))
      return false;

    if (Signed && DstSize == 32 && (SrcSize == 8 || SrcSize == 16)) {
      const unsigned SextOpc = SrcSize == 8 ?
        RVGPU::S_SEXT_I32_I8 : RVGPU::S_SEXT_I32_I16;
      BuildMI(MBB, I, DL, TII.get(SextOpc), DstReg)
        .addReg(SrcReg);
      I.eraseFromParent();
      return RBI.constrainGenericRegister(DstReg, RVGPU::SReg_32RegClass, *MRI);
    }
    ...
    if (DstSize > 32 && SrcSize == 32) {
      Register HiReg = MRI->createVirtualRegister(&RVGPU::SReg_32RegClass);
      unsigned SubReg = InReg ? RVGPU::sub0 : RVGPU::NoSubRegister;
      if (Signed) {
        BuildMI(MBB, I, DL, TII.get(RVGPU::S_ASHR_I32), HiReg)
          .addReg(SrcReg, 0, SubReg)
          .addImm(31)
          .setOperandDead(3); // Dead scc
      } else {
        BuildMI(MBB, I, DL, TII.get(RVGPU::S_MOV_B32), HiReg)
          .addImm(0);
      }
      BuildMI(MBB, I, DL, TII.get(RVGPU::REG_SEQUENCE), DstReg)
        .addReg(SrcReg, 0, SubReg)
        ...
        .addImm(RVGPU::sub1);
      I.eraseFromParent();
      return RBI.constrainGenericRegister(DstReg, RVGPU::SReg_64RegClass,
                                          *MRI);
    }

    const unsigned BFE64 = Signed ? RVGPU::S_BFE_I64 : RVGPU::S_BFE_U64;
    const unsigned BFE32 = Signed ? RVGPU::S_BFE_I32 : RVGPU::S_BFE_U32;
    ...
    if (DstSize > 32 && (SrcSize <= 32 || InReg)) {
      ...
      Register ExtReg = MRI->createVirtualRegister(&RVGPU::SReg_64RegClass);
      Register UndefReg = MRI->createVirtualRegister(&RVGPU::SReg_32RegClass);
      unsigned SubReg = InReg ? RVGPU::sub0 : RVGPU::NoSubRegister;

      BuildMI(MBB, I, DL, TII.get(RVGPU::IMPLICIT_DEF), UndefReg);
      BuildMI(MBB, I, DL, TII.get(RVGPU::REG_SEQUENCE), ExtReg)
        .addReg(SrcReg, 0, SubReg)
        .addImm(RVGPU::sub0)
        .addReg(UndefReg)
        .addImm(RVGPU::sub1);
      ...
      I.eraseFromParent();
      return RBI.constrainGenericRegister(DstReg, RVGPU::SReg_64RegClass, *MRI);
    }

    unsigned Mask;
    if (!Signed && shouldUseAndMask(SrcSize, Mask)) {
      BuildMI(MBB, I, DL, TII.get(RVGPU::S_AND_B32), DstReg)
        .addReg(SrcReg)
        .addImm(Mask)
        .setOperandDead(3); // Dead scc
    } else {
...
bool RVGPUInstructionSelector::selectG_FPEXT(MachineInstr &I) const {
  ...
  if (MRI->getType(Dst) == LLT::scalar(32) &&
    ...
    if (isExtractHiElt(*MRI, Src, Src)) {
      MachineBasicBlock *BB = I.getParent();
      BuildMI(*BB, &I, I.getDebugLoc(), TII.get(RVGPU::S_CVT_HI_F32_F16), Dst)
          .addUse(Src);
      I.eraseFromParent();
      return RBI.constrainGenericRegister(Dst, RVGPU::SReg_32RegClass, *MRI);
    }
...
bool RVGPUInstructionSelector::selectG_CONSTANT(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  ...
  // The RVGPU backend only supports Imm operands and not CImm or FPImm.
  if (ImmOp.isFPImm()) {
  ...
  const RegisterBank *DstRB = RBI.getRegBank(DstReg, *MRI, TRI);
  const bool IsSgpr = DstRB->getID() == RVGPU::SGPRRegBankID;

  unsigned Opcode;
  if (DstRB->getID() == RVGPU::VCCRegBankID) {
    Opcode = STI.isWave32() ? RVGPU::S_MOV_B32 : RVGPU::S_MOV_B64;
  } else if (Size == 64 &&
             RVGPU::isValid32BitLiteral(I.getOperand(1).getImm(), IsFP)) {
    Opcode = IsSgpr ? RVGPU::S_MOV_B64_IMM_PSEUDO : RVGPU::V_MOV_B64_PSEUDO;
    I.setDesc(TII.get(Opcode));
    I.addImplicitDefUseOperands(*MF);
    return constrainSelectedInstRegOperands(I, TII, TRI, RBI);
  } else {
    Opcode = IsSgpr ? RVGPU::S_MOV_B32 : RVGPU::V_MOV_B32_e32;
  ...
  if (IsSgpr && TII.isInlineConstant(Imm)) {
    ResInst = BuildMI(*BB, &I, DL, TII.get(RVGPU::S_MOV_B64), DstReg)
      .addImm(I.getOperand(1).getImm());
  } else {
    const TargetRegisterClass *RC = IsSgpr ?
      &RVGPU::SReg_32RegClass : &RVGPU::VGPR_32RegClass;
    ...
    ResInst = BuildMI(*BB, &I, DL, TII.get(RVGPU::REG_SEQUENCE), DstReg)
      .addReg(LoReg)
      .addImm(RVGPU::sub0)
      .addReg(HiReg)
      .addImm(RVGPU::sub1);
...
bool RVGPUInstructionSelector::selectG_FNEG(MachineInstr &MI) const {
  ...
  if (!RBI.constrainGenericRegister(Src, RVGPU::SReg_64RegClass, *MRI) ||
      !RBI.constrainGenericRegister(Dst, RVGPU::SReg_64RegClass, *MRI))
    return false;

  MachineBasicBlock *BB = MI.getParent();
  const DebugLoc &DL = MI.getDebugLoc();
  Register LoReg = MRI->createVirtualRegister(&RVGPU::SReg_32RegClass);
  Register HiReg = MRI->createVirtualRegister(&RVGPU::SReg_32RegClass);
  Register ConstReg = MRI->createVirtualRegister(&RVGPU::SReg_32RegClass);
  Register OpReg = MRI->createVirtualRegister(&RVGPU::SReg_32RegClass);

  BuildMI(*BB, &MI, DL, TII.get(RVGPU::COPY), LoReg)
    .addReg(Src, 0, RVGPU::sub0);
  BuildMI(*BB, &MI, DL, TII.get(RVGPU::COPY), HiReg)
    .addReg(Src, 0, RVGPU::sub1);
  BuildMI(*BB, &MI, DL, TII.get(RVGPU::S_MOV_B32), ConstReg)
    .addImm(0x80000000);

  // Set or toggle sign bit.
  unsigned Opc = Fabs ? RVGPU::S_OR_B32 : RVGPU::S_XOR_B32;
...
bool RVGPUInstructionSelector::selectG_FABS(MachineInstr &MI) const {
  Register Dst = MI.getOperand(0).getReg();
  const RegisterBank *DstRB = RBI.getRegBank(Dst, *MRI, TRI);
  if (DstRB->getID() != RVGPU::SGPRRegBankID ||
      MRI->getType(Dst) != LLT::scalar(64))
  ...
  BuildMI(*BB, &MI, DL, TII.get(RVGPU::S_AND_B32), OpReg)
    .addReg(HiReg)
    .addReg(ConstReg)
    .setOperandDead(3); // Dead scc
  BuildMI(*BB, &MI, DL, TII.get(RVGPU::REG_SEQUENCE), Dst)
...
void RVGPUInstructionSelector::getAddrModeInfo(const MachineInstr &Load,
...
bool RVGPUInstructionSelector::isSGPR(Register Reg) const {
  return RBI.getRegBank(Reg, *MRI, TRI)->getID() == RVGPU::SGPRRegBankID;
}

bool RVGPUInstructionSelector::isInstrUniform(const MachineInstr &MI) const {
...
bool RVGPUInstructionSelector::hasVgprParts(ArrayRef<GEPInfo> AddrInfo) const {
...
void RVGPUInstructionSelector::initM0(MachineInstr &I) const {
  ...
  if ((AS == RVGPUAS::LOCAL_ADDRESS || AS == RVGPUAS::REGION_ADDRESS) &&
    ...
    BuildMI(*BB, &I, I.getDebugLoc(), TII.get(RVGPU::S_MOV_B32), RVGPU::M0)
      .addImm(-1);
  }
}

bool RVGPUInstructionSelector::selectG_LOAD_STORE_ATOMICRMW(
  MachineInstr &I) const {
  initM0(I);
  return selectImpl(I, *CoverageInfo);
}
...
static bool isVCmpResult(Register Reg, MachineRegisterInfo &MRI) {
  ...
  if (Opcode == RVGPU::COPY)
    return isVCmpResult(MI.getOperand(1).getReg(), MRI);

  if (Opcode == RVGPU::G_AND || Opcode == RVGPU::G_OR ||
      Opcode == RVGPU::G_XOR)
    return isVCmpResult(MI.getOperand(1).getReg(), MRI) &&
           isVCmpResult(MI.getOperand(2).getReg(), MRI);

  if (auto *GI = dyn_cast<GIntrinsic>(&MI))
    return GI->is(Intrinsic::rvgpu_class);

  return Opcode == RVGPU::G_ICMP || Opcode == RVGPU::G_FCMP;
}

bool RVGPUInstructionSelector::selectG_BRCOND(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  ...
  if (!isVCC(CondReg, *MRI)) {
    ...
    CondPhysReg = RVGPU::SCC;
    BrOpcode = RVGPU::S_CBRANCH_SCC1;
    ConstrainRC = &RVGPU::SReg_32RegClass;
  } else {
    ...
    if (!isVCmpResult(CondReg, *MRI)) {
      ...
      const unsigned Opcode = Is64 ? RVGPU::S_AND_B64 : RVGPU::S_AND_B32;
      const Register Exec = Is64 ? RVGPU::EXEC : RVGPU::EXEC_LO;
    ...
    BrOpcode = RVGPU::S_CBRANCH_VCCNZ;
  ...
  BuildMI(*BB, &I, DL, TII.get(RVGPU::COPY), CondPhysReg)
    .addReg(CondReg);
...
bool RVGPUInstructionSelector::selectG_GLOBAL_VALUE(
  MachineInstr &I) const {
  Register DstReg = I.getOperand(0).getReg();
  const RegisterBank *DstRB = RBI.getRegBank(DstReg, *MRI, TRI);
  const bool IsVGPR = DstRB->getID() == RVGPU::VGPRRegBankID;
  I.setDesc(TII.get(IsVGPR ? RVGPU::V_MOV_B32_e32 : RVGPU::S_MOV_B32));
  if (IsVGPR)
    I.addOperand(*MF, MachineOperand::CreateReg(RVGPU::EXEC, false, true));

  return RBI.constrainGenericRegister(
    DstReg, IsVGPR ? RVGPU::VGPR_32RegClass : RVGPU::SReg_32RegClass, *MRI);
}

bool RVGPUInstructionSelector::selectG_PTRMASK(MachineInstr &I) const {
  Register DstReg = I.getOperand(0).getReg();
  Register SrcReg = I.getOperand(1).getReg();
  ...
  if (!IsVGPR && Ty.getSizeInBits() == 64 &&
    ...
    auto MIB = BuildMI(*BB, &I, DL, TII.get(RVGPU::S_AND_B64), DstReg)
      .addReg(SrcReg)
      .addReg(MaskReg)
      .setOperandDead(3); // Dead scc
    I.eraseFromParent();
  ...
  unsigned NewOpc = IsVGPR ? RVGPU::V_AND_B32_e64 : RVGPU::S_AND_B32;
  const TargetRegisterClass &RegRC
    = IsVGPR ? RVGPU::VGPR_32RegClass : RVGPU::SReg_32RegClass;
  ...
  BuildMI(*BB, &I, DL, TII.get(RVGPU::COPY), LoReg)
    .addReg(SrcReg, 0, RVGPU::sub0);
  BuildMI(*BB, &I, DL, TII.get(RVGPU::COPY), HiReg)
    .addReg(SrcReg, 0, RVGPU::sub1);
  ...
  if (CanCopyLow32) {
    ...
    BuildMI(*BB, &I, DL, TII.get(RVGPU::COPY), MaskLo)
      .addReg(MaskReg, 0, RVGPU::sub0);
  ...
  if (CanCopyHi32) {
    ...
    BuildMI(*BB, &I, DL, TII.get(RVGPU::COPY), MaskHi)
      .addReg(MaskReg, 0, RVGPU::sub1);
...
computeIndirectRegIndex(MachineRegisterInfo &MRI, const RVRegisterInfo &TRI,
  ...
  std::tie(IdxBaseReg, Offset) =
      RVGPU::getBaseWithConstantOffset(MRI, IdxReg, &KnownBits);
  if (IdxBaseReg == RVGPU::NoRegister) {
...
bool RVGPUInstructionSelector::selectG_EXTRACT_VECTOR_ELT(
  MachineInstr &MI) const {
  Register DstReg = MI.getOperand(0).getReg();
  Register SrcReg = MI.getOperand(1).getReg();
  ...
  if (IdxRB->getID() != RVGPU::SGPRRegBankID)
    return false;
  ...
  if (!RBI.constrainGenericRegister(SrcReg, *SrcRC, *MRI) ||
      !RBI.constrainGenericRegister(DstReg, *DstRC, *MRI) ||
      !RBI.constrainGenericRegister(IdxReg, RVGPU::SReg_32RegClass, *MRI))
    return false;
  ...
  if (SrcRB->getID() == RVGPU::SGPRRegBankID) {
    if (DstTy.getSizeInBits() != 32 && !Is64)
      return false;

    BuildMI(*BB, &MI, DL, TII.get(RVGPU::COPY), RVGPU::M0)
      .addReg(IdxReg);

    unsigned Opc = Is64 ? RVGPU::S_MOVRELS_B64 : RVGPU::S_MOVRELS_B32;
  ...
  if (SrcRB->getID() != RVGPU::VGPRRegBankID || DstTy.getSizeInBits() != 32)
    return false;

  if (!STI.useVGPRIndexMode()) {
    BuildMI(*BB, &MI, DL, TII.get(RVGPU::COPY), RVGPU::M0)
      .addReg(IdxReg);
    BuildMI(*BB, &MI, DL, TII.get(RVGPU::V_MOVRELS_B32_e32), DstReg)
      .addReg(SrcReg, 0, SubReg)
      .addReg(SrcReg, RegState::Implicit);
    MI.eraseFromParent();
    return true;
...
bool RVGPUInstructionSelector::selectG_INSERT_VECTOR_ELT(
  MachineInstr &MI) const {
  Register DstReg = MI.getOperand(0).getReg();
  ...
  if (VecRB->getID() == RVGPU::VGPRRegBankID && ValSize != 32)
    return false;
  ...
  const bool IndexMode = VecRB->getID() == RVGPU::VGPRRegBankID &&
  ...
  if (!IndexMode) {
    ...
    const MCInstrDesc &RegWriteOp = TII.getIndirectRegWriteMovRelPseudo(
        VecSize, ValSize, VecRB->getID() == RVGPU::SGPRRegBankID);
...
bool RVGPUInstructionSelector::selectBufferLoadLds(MachineInstr &MI) const {
  assert(!RVGPU::isGFX12Plus(STI));
  unsigned Opc;
  ...
  if (HasVIndex && HasVOffset) {
    ...
    BuildMI(*MBB, &*MIB, DL, TII.get(RVGPU::REG_SEQUENCE), IdxReg)
      .addReg(VIndex)
      .addImm(RVGPU::sub0)
      .addReg(VOffset)
      .addImm(RVGPU::sub1);
  ...
  MIB.addImm(Aux & RVGPU::CPol::ALL);                  // cpol
  MIB.addImm(Aux & RVGPU::CPol::SWZ_pregfx12 ? 1 : 0); // swz
...
static Register matchZeroExtendFromS32(MachineRegisterInfo &MRI, Register Reg) {
  ...
  if (Def->getOpcode() != RVGPU::G_MERGE_VALUES)
    return Register();
...
bool RVGPUInstructionSelector::selectGlobalLoadLds(MachineInstr &MI) const{
  unsigned Opc;
  unsigned Size = MI.getOperand(3).getImm();
  ...
  if (!isSGPR(Addr)) {
    ...
    if (isSGPR(AddrDef->Reg)) {
      Addr = AddrDef->Reg;
    } else if (AddrDef->MI->getOpcode() == RVGPU::G_PTR_ADD) {
      Register SAddr =
  ...
  if (isSGPR(Addr)) {
    ...
    if (!VOffset) {
      VOffset = MRI->createVirtualRegister(&RVGPU::VGPR_32RegClass);
      BuildMI(*MBB, &MI, DL, TII.get(RVGPU::V_MOV_B32_e32), VOffset)
        .addImm(0);
    }
  }
...
bool RVGPUInstructionSelector::selectBVHIntrinsic(MachineInstr &MI) const{
...
bool RVGPUInstructionSelector::selectSMFMACIntrin(MachineInstr &MI) const {
  unsigned Opc;
  switch (cast<GIntrinsic>(MI).getIntrinsicID()) {
  case Intrinsic::rvgpu_smfmac_f32_16x16x32_f16:
    Opc = RVGPU::V_SMFMAC_F32_16X16X32_F16_e64;
    break;
  case Intrinsic::rvgpu_smfmac_f32_32x32x16_f16:
    Opc = RVGPU::V_SMFMAC_F32_32X32X16_F16_e64;
    break;
  case Intrinsic::rvgpu_smfmac_f32_16x16x32_bf16:
    Opc = RVGPU::V_SMFMAC_F32_16X16X32_BF16_e64;
    break;
  case Intrinsic::rvgpu_smfmac_f32_32x32x16_bf16:
    Opc = RVGPU::V_SMFMAC_F32_32X32X16_BF16_e64;
    break;
  case Intrinsic::rvgpu_smfmac_i32_16x16x64_i8:
    Opc = RVGPU::V_SMFMAC_I32_16X16X64_I8_e64;
    break;
  case Intrinsic::rvgpu_smfmac_i32_32x32x32_i8:
    Opc = RVGPU::V_SMFMAC_I32_32X32X32_I8_e64;
    break;
  case Intrinsic::rvgpu_smfmac_f32_16x16x64_bf8_bf8:
    Opc = RVGPU::V_SMFMAC_F32_16X16X64_BF8_BF8_e64;
    break;
  case Intrinsic::rvgpu_smfmac_f32_16x16x64_bf8_fp8:
    Opc = RVGPU::V_SMFMAC_F32_16X16X64_BF8_FP8_e64;
    break;
  case Intrinsic::rvgpu_smfmac_f32_16x16x64_fp8_bf8:
    Opc = RVGPU::V_SMFMAC_F32_16X16X64_FP8_BF8_e64;
    break;
  case Intrinsic::rvgpu_smfmac_f32_16x16x64_fp8_fp8:
    Opc = RVGPU::V_SMFMAC_F32_16X16X64_FP8_FP8_e64;
    break;
  case Intrinsic::rvgpu_smfmac_f32_32x32x32_bf8_bf8:
    Opc = RVGPU::V_SMFMAC_F32_32X32X32_BF8_BF8_e64;
    break;
  case Intrinsic::rvgpu_smfmac_f32_32x32x32_bf8_fp8:
    Opc = RVGPU::V_SMFMAC_F32_32X32X32_BF8_FP8_e64;
    break;
  case Intrinsic::rvgpu_smfmac_f32_32x32x32_fp8_bf8:
    Opc = RVGPU::V_SMFMAC_F32_32X32X32_FP8_BF8_e64;
    break;
  case Intrinsic::rvgpu_smfmac_f32_32x32x32_fp8_fp8:
    Opc = RVGPU::V_SMFMAC_F32_32X32X32_FP8_FP8_e64;
    break;
  default:
...
bool RVGPUInstructionSelector::selectWaveAddress(MachineInstr &MI) const {
  Register DstReg = MI.getOperand(0).getReg();
  Register SrcReg = MI.getOperand(1).getReg();
  const RegisterBank *DstRB = RBI.getRegBank(DstReg, *MRI, TRI);
  const bool IsVALU = DstRB->getID() == RVGPU::VGPRRegBankID;
  ...
  if (IsVALU) {
    BuildMI(*MBB, MI, DL, TII.get(RVGPU::V_LSHRREV_B32_e64), DstReg)
      .addImm(Subtarget->getWavefrontSizeLog2())
      .addReg(SrcReg);
  } else {
    BuildMI(*MBB, MI, DL, TII.get(RVGPU::S_LSHR_B32), DstReg)
      .addReg(SrcReg)
      .addImm(Subtarget->getWavefrontSizeLog2())
      .setOperandDead(3); // Dead scc
  }
  ...
  const TargetRegisterClass &RC =
      IsVALU ? RVGPU::VGPR_32RegClass : RVGPU::SReg_32RegClass;
  if (!RBI.constrainGenericRegister(DstReg, RC, *MRI))
    return false;
...
bool RVGPUInstructionSelector::selectStackRestore(MachineInstr &MI) const {
  Register SrcReg = MI.getOperand(0).getReg();
  if (!RBI.constrainGenericRegister(SrcReg, RVGPU::SReg_32RegClass, *MRI))
    return false;
  ...
  if (!WaveAddr) {
    WaveAddr = MRI->createVirtualRegister(&RVGPU::SReg_32RegClass);
    BuildMI(*MBB, MI, DL, TII.get(RVGPU::S_LSHR_B32), WaveAddr)
      .addReg(SrcReg)
      .addImm(Subtarget->getWavefrontSizeLog2())
      .setOperandDead(3); // Dead scc
  }

  BuildMI(*MBB, &MI, DL, TII.get(RVGPU::COPY), SP)
    .addReg(WaveAddr);
...
bool RVGPUInstructionSelector::select(MachineInstr &I) {
  if (I.isPHI())
    return selectPHI(I);
  ...
  case RVGPU::G_RVGPU_MAD_I64_I32:
    return selectG_RVGPU_MAD_64_32(I);
  ...
  case RVGPU::G_RVGPU_INTRIN_IMAGE_STORE_D16: {
    const RVGPU::ImageDimIntrinsicInfo *Intr =
        RVGPU::getImageDimIntrinsicInfo(RVGPU::getIntrinsicID(I));
  ...
  case RVGPU::G_SI_CALL:
    I.setDesc(TII.get(RVGPU::SI_CALL));
    return true;
...
RVGPUInstructionSelector::selectVCSRC(MachineOperand &Root) const {
  return {{
...
RVGPUInstructionSelector::selectVOP3ModsImpl(MachineOperand &Root,
  ...
  if (MI->getOpcode() == RVGPU::G_FNEG) {
    Src = MI->getOperand(1).getReg();
    Mods |= SISrcMods::NEG;
    MI = getDefIgnoringCopies(Src, *MRI);
  } else if (MI->getOpcode() == RVGPU::G_FSUB && IsCanonicalizing) {
  ...
  if (AllowAbs && MI->getOpcode() == RVGPU::G_FABS) {
    Src = MI->getOperand(1).getReg();
    Mods |= SISrcMods::ABS;
  }
...
Register RVGPUInstructionSelector::copyToVGPRIfSrcFolded(
    Register Src, unsigned Mods, MachineOperand Root, MachineInstr *InsertPt,
    bool ForceVGPR) const {
  if ((Mods != 0 || ForceVGPR) &&
      RBI.getRegBank(Src, *MRI, TRI)->getID() != RVGPU::VGPRRegBankID) {
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectVSRC0(MachineOperand &Root) const {
  return {{
      [=](MachineInstrBuilder &MIB) { MIB.add(Root); }
  }};
}
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectVOP3Mods0(MachineOperand &Root) const {
  Register Src;
  unsigned Mods;
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectVOP3BMods0(MachineOperand &Root) const {
  Register Src;
  unsigned Mods;
  std::tie(Src, Mods) = selectVOP3ModsImpl(Root,
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectVOP3OMods(MachineOperand &Root) const {
  return {{
      [=](MachineInstrBuilder &MIB) { MIB.add(Root); },
      [=](MachineInstrBuilder &MIB) { MIB.addImm(0); }, // clamp
      [=](MachineInstrBuilder &MIB) { MIB.addImm(0); }  // omod
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectVOP3Mods(MachineOperand &Root) const {
  Register Src;
  unsigned Mods;
  std::tie(Src, Mods) = selectVOP3ModsImpl(Root);
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectVOP3ModsNonCanonicalizing(
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectVOP3BMods(MachineOperand &Root) const {
  Register Src;
  unsigned Mods;
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectVOP3NoMods(MachineOperand &Root) const {
  Register Reg = Root.getReg();
  const MachineInstr *Def = getDefIgnoringCopies(Reg, *MRI);
  if (Def->getOpcode() == RVGPU::G_FNEG || Def->getOpcode() == RVGPU::G_FABS)
    return {};
  return {{
...
std::pair<Register, unsigned>
RVGPUInstructionSelector::selectVOP3PModsImpl(
  Register Src, const MachineRegisterInfo &MRI, bool IsDOT) const {
  unsigned Mods = 0;
  MachineInstr *MI = MRI.getVRegDef(Src);

  if (MI && MI->getOpcode() == RVGPU::G_FNEG &&
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectVOP3PMods(MachineOperand &Root) const {
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectVOP3PModsDOT(MachineOperand &Root) const {
  MachineRegisterInfo &MRI
    = Root.getParent()->getParent()->getParent()->getRegInfo();
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectDotIUVOP3PMods(MachineOperand &Root) const {
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectWMMAOpSelVOP3PMods(
    MachineOperand &Root) const {
  assert((Root.isImm() && (Root.getImm() == -1 || Root.getImm() == 0)) &&
         "expected i1 value");
  unsigned Mods = SISrcMods::OP_SEL_1;
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectVOP3OpSelMods(MachineOperand &Root) const {
  Register Src;
  unsigned Mods;
  std::tie(Src, Mods) = selectVOP3ModsImpl(Root);
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectVINTERPMods(MachineOperand &Root) const {
  Register Src;
  unsigned Mods;
  std::tie(Src, Mods) = selectVOP3ModsImpl(Root,
                                           /*IsCanonicalizing=*/true,
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectVINTERPModsHi(MachineOperand &Root) const {
  Register Src;
  unsigned Mods;
  std::tie(Src, Mods) = selectVOP3ModsImpl(Root,
                                           /*IsCanonicalizing=*/true,
...
bool RVGPUInstructionSelector::selectSmrdOffset(MachineOperand &Root,
                                                 Register &Base,
                                                 Register *SOffset,
  ...
  std::optional<int64_t> EncodedImm =
      RVGPU::getSMRDEncodedOffset(STI, GEPI.Imm, false);
  ...
  if (SOffset && GEPI.SgprParts.size() == 1 && isUInt<32>(GEPI.Imm) &&
    ...
    Base = GEPI.SgprParts[0];
    *SOffset = MRI->createVirtualRegister(&RVGPU::SReg_32RegClass);
    BuildMI(*MBB, MI, MI->getDebugLoc(), TII.get(RVGPU::S_MOV_B32), *SOffset)
        .addImm(GEPI.Imm);
    return true;
  }
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectSmrdImm(MachineOperand &Root) const {
  Register Base;
  int64_t Offset;
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectSmrdImm32(MachineOperand &Root) const {
  SmallVector<GEPInfo, 4> AddrInfo;
  ...
  std::optional<int64_t> EncodedImm =
      RVGPU::getSMRDEncodedLiteralOffset32(STI, GEPInfo.Imm);
  if (!EncodedImm)
    return std::nullopt;
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectSmrdSgpr(MachineOperand &Root) const {
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectSmrdSgprImm(MachineOperand &Root) const {
  Register Base, SOffset;
  int64_t Offset;
...
RVGPUInstructionSelector::selectFlatOffsetImpl(MachineOperand &Root,
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectFlatOffset(MachineOperand &Root) const {
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectGlobalOffset(MachineOperand &Root) const {
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectScratchOffset(MachineOperand &Root) const {
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectGlobalSAddr(MachineOperand &Root) const {
  ...
  if (ConstOffset != 0) {
    if (TII.isLegalFLATOffset(ConstOffset, RVGPUAS::GLOBAL_ADDRESS,
      ...
      if (isSGPR(PtrBaseDef->Reg)) {
        if (ConstOffset > 0) {
          ...
          std::tie(SplitImmOffset, RemainderOffset) = TII.splitFlatOffset(
              ConstOffset, RVGPUAS::GLOBAL_ADDRESS, SIInstrFlags::FlatGlobal);

          if (isUInt<32>(RemainderOffset)) {
            MachineInstr *MI = Root.getParent();
            MachineBasicBlock *MBB = MI->getParent();
            Register HighBits =
                MRI->createVirtualRegister(&RVGPU::VGPR_32RegClass);

            BuildMI(*MBB, MI, MI->getDebugLoc(), TII.get(RVGPU::V_MOV_B32_e32),
                    HighBits)
        ...
        if (STI.getConstantBusLimit(RVGPU::V_ADD_U32_e64) > NumLiterals)
          return std::nullopt;
      }
    }
  }
  ...
  auto AddrDef = getDefSrcRegIgnoringCopies(Addr, *MRI);
  if (AddrDef->MI->getOpcode() == RVGPU::G_PTR_ADD) {
  ...
  // drop this.
  if (AddrDef->MI->getOpcode() == RVGPU::G_IMPLICIT_DEF ||
      AddrDef->MI->getOpcode() == RVGPU::G_CONSTANT || !isSGPR(AddrDef->Reg))
    return std::nullopt;
  ...
  MachineInstr *MI = Root.getParent();
  MachineBasicBlock *MBB = MI->getParent();
  Register VOffset = MRI->createVirtualRegister(&RVGPU::VGPR_32RegClass);

  BuildMI(*MBB, MI, MI->getDebugLoc(), TII.get(RVGPU::V_MOV_B32_e32), VOffset)
      .addImm(0);
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectScratchSAddr(MachineOperand &Root) const {
  Register Addr = Root.getReg();
  Register PtrBase;
  int64_t ConstOffset;
  int64_t ImmOffset = 0;
  ...
  if (ConstOffset != 0 && isFlatScratchBaseLegal(Addr) &&
      TII.isLegalFLATOffset(ConstOffset, RVGPUAS::PRIVATE_ADDRESS,
  ...
  auto AddrDef = getDefSrcRegIgnoringCopies(Addr, *MRI);
  if (AddrDef->MI->getOpcode() == RVGPU::G_FRAME_INDEX) {
  ...
  if (AddrDef->MI->getOpcode() == RVGPU::G_PTR_ADD) {
    ...
    if (LHSDef->MI->getOpcode() == RVGPU::G_FRAME_INDEX &&
        isSGPR(RHSDef->Reg)) {
      int FI = LHSDef->MI->getOperand(1).getIndex();
      MachineInstr &I = *Root.getParent();
      MachineBasicBlock *BB = I.getParent();
      const DebugLoc &DL = I.getDebugLoc();
      SAddr = MRI->createVirtualRegister(&RVGPU::SReg_32RegClass);

      BuildMI(*BB, &I, DL, TII.get(RVGPU::S_ADD_I32), SAddr)
          .addFrameIndex(FI)
          .addReg(RHSDef->Reg)
          .setOperandDead(3); // Dead scc
    }
...
bool RVGPUInstructionSelector::checkFlatScratchSVSSwizzleBug(
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectScratchSVAddr(MachineOperand &Root) const {
  Register Addr = Root.getReg();
  Register PtrBase;
  int64_t ConstOffset;
  int64_t ImmOffset = 0;
  ...
  if (ConstOffset != 0 &&
      TII.isLegalFLATOffset(ConstOffset, RVGPUAS::PRIVATE_ADDRESS, true)) {
    Addr = PtrBase;
    ImmOffset = ConstOffset;
  }
  ...
  Register RHS = AddrDef->MI->getOperand(2).getReg();
  if (RBI.getRegBank(RHS, *MRI, TRI)->getID() != RVGPU::VGPRRegBankID)
    return std::nullopt;
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectMUBUFScratchOffen(MachineOperand &Root) const {
  MachineInstr *MI = Root.getParent();
  MachineBasicBlock *MBB = MI->getParent();
  MachineFunction *MF = MBB->getParent();
  ...
  if (mi_match(Root.getReg(), *MRI, m_ICst(Offset)) &&
      Offset != TM.getNullPointerValue(RVGPUAS::PRIVATE_ADDRESS)) {
    Register HighBits = MRI->createVirtualRegister(&RVGPU::VGPR_32RegClass);
  ...
  if (const MachineInstr *RootDef = MRI->getVRegDef(Root.getReg())) {
    ...
    if (ConstOffset != 0) {
      if (TII.isLegalMUBUFImmOffset(ConstOffset) &&
        ...
        if (PtrBaseDef->getOpcode() == RVGPU::G_FRAME_INDEX)
          FI = PtrBaseDef->getOperand(1).getIndex();
        else
          VAddr = PtrBase;
        Offset = ConstOffset;
      }
    } else if (RootDef->getOpcode() == RVGPU::G_FRAME_INDEX) {
...
bool RVGPUInstructionSelector::isDSOffsetLegal(Register Base,
                                                int64_t Offset) const {
  if (!isUInt<16>(Offset))
    return false;
...
bool RVGPUInstructionSelector::isDSOffset2Legal(Register Base, int64_t Offset0,
                                                 int64_t Offset1,
                                                 unsigned Size) const {
...
bool RVGPUInstructionSelector::isFlatScratchBaseLegal(Register Addr) const {
...
// of: SGPR + VGPR.
bool RVGPUInstructionSelector::isFlatScratchBaseLegalSV(Register Addr) const {
  MachineInstr *AddrMI = getDefIgnoringCopies(Addr, *MRI);
...
// Check address value in SGPR/VGPR are legal for flat scratch in the form
// of: SGPR + VGPR + Imm.
bool RVGPUInstructionSelector::isFlatScratchBaseLegalSVImm(
    Register Addr) const {
  MachineInstr *AddrMI = getDefIgnoringCopies(Addr, *MRI);
...
bool RVGPUInstructionSelector::isUnneededShiftMask(const MachineInstr &MI,
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectMUBUFScratchOffset(
    MachineOperand &Root) const {
  Register Reg = Root.getReg();
  const RVMachineFunctionInfo *Info = MF->getInfo<RVMachineFunctionInfo>();
...
std::pair<Register, unsigned>
RVGPUInstructionSelector::selectDS1Addr1OffsetImpl(MachineOperand &Root) const {
  ...
  } else if (RootDef->getOpcode() == RVGPU::G_SUB) {
    // TODO
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectDS1Addr1Offset(MachineOperand &Root) const {
  Register Reg;
  unsigned Offset;
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectDS64Bit4ByteAligned(MachineOperand &Root) const {
  return selectDSReadWrite2(Root, 4);
}

InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectDS128Bit8ByteAligned(MachineOperand &Root) const {
  return selectDSReadWrite2(Root, 8);
}

InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectDSReadWrite2(MachineOperand &Root,
                                              unsigned Size) const {
  Register Reg;
  unsigned Offset;
...
std::pair<Register, unsigned>
RVGPUInstructionSelector::selectDSReadWrite2Impl(MachineOperand &Root,
                                                  unsigned Size) const {
  const MachineInstr *RootDef = MRI->getVRegDef(Root.getReg());
  if (!RootDef)
    return std::pair(Root.getReg(), 0);
...
RVGPUInstructionSelector::getPtrBaseWithConstantOffset(
...
static Register buildRSRC(MachineIRBuilder &B, MachineRegisterInfo &MRI,
                          ...
                          Register BasePtr) {
  Register RSrc2 = MRI.createVirtualRegister(&RVGPU::SReg_32RegClass);
  Register RSrc3 = MRI.createVirtualRegister(&RVGPU::SReg_32RegClass);
  Register RSrcHi = MRI.createVirtualRegister(&RVGPU::SReg_64RegClass);
  Register RSrc = MRI.createVirtualRegister(&RVGPU::SGPR_128RegClass);

  B.buildInstr(RVGPU::S_MOV_B32)
    .addDef(RSrc2)
    .addImm(FormatLo);
  B.buildInstr(RVGPU::S_MOV_B32)
    .addDef(RSrc3)
  ...
  if (!BasePtr) {
    RSrcLo = MRI.createVirtualRegister(&RVGPU::SReg_64RegClass);
    B.buildInstr(RVGPU::S_MOV_B64)
      .addDef(RSrcLo)
      .addImm(0);
  }
...
RVGPUInstructionSelector::MUBUFAddressData
RVGPUInstructionSelector::parseMUBUFAddress(Register Src) const {
...
bool RVGPUInstructionSelector::shouldUseAddr64(MUBUFAddressData Addr) const {
  ...
  return N0Bank->getID() == RVGPU::VGPRRegBankID;
}
...
/// component.
void RVGPUInstructionSelector::splitIllegalMUBUFOffset(
...
bool RVGPUInstructionSelector::selectMUBUFAddr64Impl(
  ...
  if (N2) {
    if (RBI.getRegBank(N2, *MRI, TRI)->getID() == RVGPU::VGPRRegBankID) {
      assert(N3);
      if (RBI.getRegBank(N3, *MRI, TRI)->getID() == RVGPU::VGPRRegBankID) {
  ...
  } else if (RBI.getRegBank(N0, *MRI, TRI)->getID() == RVGPU::VGPRRegBankID) {
...
bool RVGPUInstructionSelector::selectMUBUFOffsetImpl(
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectMUBUFAddr64(MachineOperand &Root) const {
  Register VAddr;
  Register RSrcReg;
  Register SOffset;
  int64_t Offset = 0;
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectMUBUFOffset(MachineOperand &Root) const {
  Register RSrcReg;
  Register SOffset;
  int64_t Offset = 0;
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectBUFSOffset(MachineOperand &Root) const {

  Register SOffset = Root.getReg();

  if (STI.hasRestrictedSOffset() && mi_match(SOffset, *MRI, m_ZeroInt()))
    SOffset = RVGPU::SGPR_NULL;
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectSMRDBufferImm(MachineOperand &Root) const {
  std::optional<uint64_t> OffsetVal = getConstantZext32Val(Root.getReg(), *MRI);
  if (!OffsetVal)
    return {};

  std::optional<int64_t> EncodedImm =
      RVGPU::getSMRDEncodedOffset(STI, *OffsetVal, true);
  if (!EncodedImm)
    return {};
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectSMRDBufferImm32(MachineOperand &Root) const {
  assert(STI.getGeneration() == RVGPUSubtarget::SEA_ISLANDS);
  ...
  std::optional<int64_t> EncodedImm =
      RVGPU::getSMRDEncodedLiteralOffset32(STI, *OffsetVal);
  if (!EncodedImm)
    return {};
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectSMRDBufferSgprImm(MachineOperand &Root) const {
  // Match the (soffset + offset) pair as a 32-bit register base and
  // an immediate offset.
  Register SOffset;
  unsigned Offset;
  std::tie(SOffset, Offset) = RVGPU::getBaseWithConstantOffset(
      *MRI, Root.getReg(), KB, /*CheckNUW*/ true);
  if (!SOffset)
    return std::nullopt;

  std::optional<int64_t> EncodedOffset =
      RVGPU::getSMRDEncodedOffset(STI, Offset, /* IsBuffer */ true);
  if (!EncodedOffset)
    return std::nullopt;
...
static MachineInstr *stripBitCast(MachineInstr *MI, MachineRegisterInfo &MRI) {
  if (MI->getOpcode() == RVGPU::G_BITCAST)
...
static MachineInstr *isExtractHiElt(MachineInstr *Inst,
  ...
  if (Inst->getOpcode() != RVGPU::G_TRUNC)
    return nullptr;
  ...
  if (TruncOp->getOpcode() == RVGPU::G_LSHR) {
  ...
  if (TruncOp->getOpcode() == RVGPU::G_SHUFFLE_VECTOR) {
...
std::pair<Register, unsigned>
RVGPUInstructionSelector::selectVOP3PMadMixModsImpl(MachineOperand &Root,
                                                     bool &Matched) const {
  Matched = false;
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectVOP3PMadMixModsExt(
    MachineOperand &Root) const {
  Register Src;
  unsigned Mods;
  bool Matched;
...
InstructionSelector::ComplexRendererFns
RVGPUInstructionSelector::selectVOP3PMadMixMods(MachineOperand &Root) const {
  Register Src;
  unsigned Mods;
  bool Matched;
  std::tie(Src, Mods) = selectVOP3PMadMixModsImpl(Root, Matched);
...
bool RVGPUInstructionSelector::selectSBarrierSignalIsfirst(
    MachineInstr &I, Intrinsic::ID IntrID) const {
  MachineBasicBlock *MBB = I.getParent();
  const DebugLoc &DL = I.getDebugLoc();
  Register CCReg = I.getOperand(0).getReg();

  bool HasM0 = IntrID == Intrinsic::rvgpu_s_barrier_signal_isfirst_var;

  if (HasM0) {
    auto CopyMIB = BuildMI(*MBB, &I, DL, TII.get(RVGPU::COPY), RVGPU::M0)
                       .addReg(I.getOperand(2).getReg());
    BuildMI(*MBB, &I, DL, TII.get(RVGPU::S_BARRIER_SIGNAL_ISFIRST_M0));
    if (!constrainSelectedInstRegOperands(*CopyMIB, TII, TRI, RBI))
      return false;
  } else {
    BuildMI(*MBB, &I, DL, TII.get(RVGPU::S_BARRIER_SIGNAL_ISFIRST_IMM))
        .addImm(I.getOperand(2).getImm());
  }

  BuildMI(*MBB, &I, DL, TII.get(RVGPU::COPY), CCReg).addReg(RVGPU::SCC);

  I.eraseFromParent();
  return RBI.constrainGenericRegister(CCReg, RVGPU::SReg_32_XM0_XEXECRegClass,
                                      *MRI);
}
...
unsigned rvGetNamedBarrierOp(bool HasInlineConst, Intrinsic::ID IntrID) {
  if (HasInlineConst) {
    ...
    case Intrinsic::rvgpu_s_barrier_init:
      return RVGPU::S_BARRIER_INIT_IMM;
    case Intrinsic::rvgpu_s_barrier_join:
      return RVGPU::S_BARRIER_JOIN_IMM;
    case Intrinsic::rvgpu_s_wakeup_barrier:
      return RVGPU::S_WAKEUP_BARRIER_IMM;
    case Intrinsic::rvgpu_s_get_barrier_state:
      return RVGPU::S_GET_BARRIER_STATE_IMM;
    };
  } else {
    switch (IntrID) {
    default:
      llvm_unreachable("not a named barrier op");
    case Intrinsic::rvgpu_s_barrier_init:
      return RVGPU::S_BARRIER_INIT_M0;
    case Intrinsic::rvgpu_s_barrier_join:
      return RVGPU::S_BARRIER_JOIN_M0;
    case Intrinsic::rvgpu_s_wakeup_barrier:
      return RVGPU::S_WAKEUP_BARRIER_M0;
    case Intrinsic::rvgpu_s_get_barrier_state:
      return RVGPU::S_GET_BARRIER_STATE_M0;
    };
  }
}

bool RVGPUInstructionSelector::selectNamedBarrierInst(
    MachineInstr &I, Intrinsic::ID IntrID) const {
  MachineBasicBlock *MBB = I.getParent();
  const DebugLoc &DL = I.getDebugLoc();
  MachineOperand BarOp = IntrID == Intrinsic::rvgpu_s_get_barrier_state
                             ? I.getOperand(2)
                             : I.getOperand(1);
  ...
  if (IntrID == Intrinsic::rvgpu_s_barrier_init) {
    Register MemberCount = I.getOperand(2).getReg();
    TmpReg0 = MRI->createVirtualRegister(&RVGPU::SReg_32RegClass);
  ...
  if (!BarValImm) {
    if (IntrID == Intrinsic::rvgpu_s_barrier_init) {
      ...
      Register TmpReg1 = MRI->createVirtualRegister(&RVGPU::SReg_32RegClass);
      BuildMI(*MBB, &I, DL, TII.get(RVGPU::S_OR_B32), TmpReg1)
          .addReg(BarOp.getReg())
          .addReg(TmpReg0);
      M0Val = TmpReg1;
    } else {
  ...
  if (M0Val) {
    auto CopyMIB =
        BuildMI(*MBB, &I, DL, TII.get(RVGPU::COPY), RVGPU::M0).addReg(M0Val);
...
bool RVGPUInstructionSelector::selectSBarrierLeave(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  const DebugLoc &DL = I.getDebugLoc();
  Register CCReg = I.getOperand(0).getReg();

  BuildMI(*BB, &I, DL, TII.get(RVGPU::S_BARRIER_LEAVE));
  BuildMI(*BB, &I, DL, TII.get(RVGPU::COPY), CCReg).addReg(RVGPU::SCC);
...
void RVGPUInstructionSelector::renderTruncImm32(MachineInstrBuilder &MIB,
...
void RVGPUInstructionSelector::renderNegateImm(MachineInstrBuilder &MIB,
                                                const MachineInstr &MI,
                                                int OpIdx) const {
  assert(MI.getOpcode() == TargetOpcode::G_CONSTANT && OpIdx == -1 &&
         "Expected G_CONSTANT");
...
void RVGPUInstructionSelector::renderBitcastImm(MachineInstrBuilder &MIB,
                                                 const MachineInstr &MI,
                                                 int OpIdx) const {
  assert(OpIdx == -1);
...
void RVGPUInstructionSelector::renderPopcntImm(MachineInstrBuilder &MIB,
                                                const MachineInstr &MI,
                                                int OpIdx) const {
  assert(MI.getOpcode() == TargetOpcode::G_CONSTANT && OpIdx == -1 &&
         "Expected G_CONSTANT");
...
/// no-op here.
void RVGPUInstructionSelector::renderTruncTImm(MachineInstrBuilder &MIB,
                                                const MachineInstr &MI,
                                                int OpIdx) const {
  MIB.addImm(MI.getOperand(OpIdx).getImm());
}

void RVGPUInstructionSelector::renderOpSelTImm(MachineInstrBuilder &MIB,
                                                const MachineInstr &MI,
                                                int OpIdx) const {
...
void RVGPUInstructionSelector::renderExtractCPol(MachineInstrBuilder &MIB,
                                                  const MachineInstr &MI,
                                                  int OpIdx) const {
  assert(OpIdx >= 0 && "expected to match an immediate operand");
  MIB.addImm(MI.getOperand(OpIdx).getImm() &
             (RVGPU::isGFX12Plus(STI) ? RVGPU::CPol::ALL
                                       : RVGPU::CPol::ALL_pregfx12));
}

void RVGPUInstructionSelector::renderExtractSWZ(MachineInstrBuilder &MIB,
                                                 const MachineInstr &MI,
                                                 int OpIdx) const {
  assert(OpIdx >= 0 && "expected to match an immediate operand");
  const bool Swizzle = MI.getOperand(OpIdx).getImm() &
                       (RVGPU::isGFX12Plus(STI) ? RVGPU::CPol::SWZ
                                                 : RVGPU::CPol::SWZ_pregfx12);
  MIB.addImm(Swizzle);
}

void RVGPUInstructionSelector::renderSetGLC(MachineInstrBuilder &MIB,
                                             const MachineInstr &MI,
                                             int OpIdx) const {
  assert(OpIdx >= 0 && "expected to match an immediate operand");
  MIB.addImm(MI.getOperand(OpIdx).getImm() | RVGPU::CPol::GLC);
}

void RVGPUInstructionSelector::renderFrameIndex(MachineInstrBuilder &MIB,
                                                 const MachineInstr &MI,
                                                 int OpIdx) const {
  MIB.addFrameIndex(MI.getOperand(1).getIndex());
}

void RVGPUInstructionSelector::renderFPPow2ToExponent(MachineInstrBuilder &MIB,
                                                       const MachineInstr &MI,
                                                       int OpIdx) const {
...
bool RVGPUInstructionSelector::isInlineImmediate16(int64_t Imm) const {
  return RVGPU::isInlinableLiteral16(Imm, STI.hasInv2PiInlineImm());
}

bool RVGPUInstructionSelector::isInlineImmediate32(int64_t Imm) const {
  return RVGPU::isInlinableLiteral32(Imm, STI.hasInv2PiInlineImm());
}

bool RVGPUInstructionSelector::isInlineImmediate64(int64_t Imm) const {
  return RVGPU::isInlinableLiteral64(Imm, STI.hasInv2PiInlineImm());
}

bool RVGPUInstructionSelector::isInlineImmediate(const APFloat &Imm) const {
