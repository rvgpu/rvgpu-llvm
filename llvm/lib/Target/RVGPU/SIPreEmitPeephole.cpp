...
static cl::opt<unsigned, true> SkipThresholdFlag(
    "rvgpu-skip-threshold", cl::Hidden,
    cl::desc(
...
bool SIPreEmitPeephole::optimizeVccBranch(MachineInstr &MI) const {
  ...
  const unsigned And = IsWave32 ? RVGPU::S_AND_B32 : RVGPU::S_AND_B64;
  const unsigned AndN2 = IsWave32 ? RVGPU::S_ANDN2_B32 : RVGPU::S_ANDN2_B64;
  const unsigned Mov = IsWave32 ? RVGPU::S_MOV_B32 : RVGPU::S_MOV_B64;
  ...
  if (!ReadsCond && A->registerDefIsDead(RVGPU::SCC)) {
  ...
  bool IsVCCZ = MI.getOpcode() == RVGPU::S_CBRANCH_VCCZ;
  if (SReg == ExecReg) {
    ...
    MI.setDesc(TII->get(RVGPU::S_BRANCH));
    ...
    MI.setDesc(
        TII->get(IsVCCZ ? RVGPU::S_CBRANCH_EXECZ : RVGPU::S_CBRANCH_EXECNZ));
  }
...
bool SIPreEmitPeephole::optimizeSetGPR(MachineInstr &First,
  ...
  MachineOperand *Idx = TII->getNamedOperand(MI, RVGPU::OpName::src0);
  ...
  for (MachineBasicBlock::instr_iterator I = std::next(First.getIterator()),
    ...
    case RVGPU::S_SET_GPR_IDX_OFF:
      ...
      break;
    default:
      if (I->modifiesRegister(RVGPU::M0, TRI))
        return false;
      ...
      if (llvm::any_of(I->operands(),
        ...
        // with the same mode.
        if (!IdxOn || !(I->getOpcode() == RVGPU::V_MOV_B32_indirect_write ||
                        I->getOpcode() == RVGPU::V_MOV_B32_indirect_read))
          return false;
      }
    }
  }
  ...
  for (MachineFunction::const_iterator MBBI(&From), ToI(&To), End = MF->end();
    ...
    for (const MachineInstr &MI : MBB) {
      ...
      if (TII->isSMRD(MI) || TII->isVMEM(MI) || TII->isFLAT(MI) ||
          TII->isDS(MI) || MI.getOpcode() == RVGPU::S_WAITCNT)
        return true;
...
bool SIPreEmitPeephole::runOnMachineFunction(MachineFunction &MF) {
  ...
  for (MachineBasicBlock &MBB : MF) {
    ...
    for (auto &MI :
      ...
      if (MI.getOpcode() != RVGPU::S_SET_GPR_IDX_ON)
        continue;
