...
public:
  ...
  bool isVreg1(Register Reg) const {
    return Reg.isVirtual() && MRI->getRegClass(Reg) == &RVGPU::VReg_1RegClass;
  }
};
...
bool Vreg1LoweringHelper::cleanConstrainRegs(bool Changed) {
  ...
  for (Register Reg : ConstrainRegs)
    MRI->constrainRegClass(Reg, &RVGPU::SReg_1_XEXECRegClass);
  ConstrainRegs.clear();
...
public:
  PhiIncomingAnalysis(MachinePostDominatorTree &PDT, const RVInstrInfo *TII)
      : PDT(PDT), TII(TII) {}
...
#ifndef NDEBUG
static bool isVRegCompatibleReg(const RVRegisterInfo &TRI,
...
bool Vreg1LoweringHelper::lowerCopiesFromI1() {
  ...
  for (MachineBasicBlock &MBB : *MF) {
    for (MachineInstr &MI : MBB) {
      ...
      BuildMI(MBB, MI, DL, TII->get(RVGPU::V_CNDMASK_B32_e64), DstReg)
          .addImm(0)
          .addImm(0)
          .addImm(0)
          .addImm(-1)
...
PhiLoweringHelper::PhiLoweringHelper(MachineFunction *MF,
  ...
  if (IsWave32) {
    ...
    MovOp = RVGPU::S_MOV_B32;
    AndOp = RVGPU::S_AND_B32;
    OrOp = RVGPU::S_OR_B32;
    XorOp = RVGPU::S_XOR_B32;
    AndN2Op = RVGPU::S_ANDN2_B32;
    OrN2Op = RVGPU::S_ORN2_B32;
  } else {
    ExecReg = RVGPU::EXEC;
    MovOp = RVGPU::S_MOV_B64;
    AndOp = RVGPU::S_AND_B64;
    OrOp = RVGPU::S_OR_B64;
    XorOp = RVGPU::S_XOR_B64;
    AndN2Op = RVGPU::S_ANDN2_B64;
    OrN2Op = RVGPU::S_ORN2_B64;
  }
}
...
bool Vreg1LoweringHelper::lowerCopiesToI1() {
  ...
  for (MachineBasicBlock &MBB : *MF) {
    ...
    for (MachineInstr &MI : MBB) {
      if (MI.getOpcode() != RVGPU::IMPLICIT_DEF &&
      ...
      if (!SrcReg.isVirtual() || (!isLaneMaskReg(SrcReg) && !isVreg1(SrcReg))) {
        ...
        BuildMI(MBB, MI, DL, TII->get(RVGPU::V_CMP_NE_U32_e64), TmpReg)
            .addReg(SrcReg)
            .addImm(0);
...
static void instrDefsUsesSCC(const MachineInstr &MI, bool &Def, bool &Use) {
  ...
  for (const MachineOperand &MO : MI.operands()) {
    if (MO.isReg() && MO.getReg() == RVGPU::SCC) {
      if (MO.isUse())
        Use = true;
      else
        Def = true;
...
void Vreg1LoweringHelper::collectIncomingValuesFromPhi(
  ...
  for (unsigned i = 1; i < MI->getNumOperands(); i += 2) {
    ...
    if (IncomingDef->getOpcode() == RVGPU::COPY) {
    ...
    } else if (IncomingDef->getOpcode() == RVGPU::IMPLICIT_DEF) {
      continue;
    } else {
...
void Vreg1LoweringHelper::buildMergeLaneMasks(MachineBasicBlock &MBB,
  ...
  if (PrevConstant && CurConstant) {
    if (PrevVal == CurVal) {
      BuildMI(MBB, I, DL, TII->get(RVGPU::COPY), DstReg).addReg(CurReg);
    } else if (CurVal) {
      BuildMI(MBB, I, DL, TII->get(RVGPU::COPY), DstReg).addReg(ExecReg);
    } else {
  ...
  if (PrevConstant && !PrevVal) {
    BuildMI(MBB, I, DL, TII->get(RVGPU::COPY), DstReg)
        .addReg(CurMaskedReg);
