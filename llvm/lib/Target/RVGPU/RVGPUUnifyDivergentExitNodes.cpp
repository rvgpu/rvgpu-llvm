...
#define DEBUG_TYPE "rvgpu-unify-divergent-exit-nodes"

namespace {

class RVGPUUnifyDivergentExitNodesImpl {
private:
  const TargetTransformInfo *TTI = nullptr;

public:
  RVGPUUnifyDivergentExitNodesImpl() = delete;
  RVGPUUnifyDivergentExitNodesImpl(const TargetTransformInfo *TTI)
      : TTI(TTI) {}
...
class RVGPUUnifyDivergentExitNodes : public FunctionPass {
public:
  static char ID;
  RVGPUUnifyDivergentExitNodes() : FunctionPass(ID) {
    initializeRVGPUUnifyDivergentExitNodesPass(
...
char RVGPUUnifyDivergentExitNodes::ID = 0;

char &llvm::RVGPUUnifyDivergentExitNodesID = RVGPUUnifyDivergentExitNodes::ID;

INITIALIZE_PASS_BEGIN(RVGPUUnifyDivergentExitNodes, DEBUG_TYPE,
                      "Unify divergent function exit nodes", false, false)
INITIALIZE_PASS_DEPENDENCY(DominatorTreeWrapperPass)
INITIALIZE_PASS_DEPENDENCY(PostDominatorTreeWrapperPass)
INITIALIZE_PASS_DEPENDENCY(UniformityInfoWrapperPass)
INITIALIZE_PASS_END(RVGPUUnifyDivergentExitNodes, DEBUG_TYPE,
                    "Unify divergent function exit nodes", false, false)

void RVGPUUnifyDivergentExitNodes::getAnalysisUsage(AnalysisUsage &AU) const {
...
BasicBlock *RVGPUUnifyDivergentExitNodesImpl::unifyReturnBlockSet(
...
bool RVGPUUnifyDivergentExitNodesImpl::run(Function &F, DominatorTree *DT,
  ...
  if (!UnreachableBlocks.empty()) {
    ...
    if (!ReturningBlocks.empty()) {
      ...
      Function *UnreachableIntrin =
        Intrinsic::getDeclaration(F.getParent(), Intrinsic::rvgpu_unreachable);
...
bool RVGPUUnifyDivergentExitNodes::runOnFunction(Function &F) {
  ...
  return RVGPUUnifyDivergentExitNodesImpl(TranformInfo).run(F, DT, PDT, UA);
}

PreservedAnalyses
RVGPUUnifyDivergentExitNodesPass::run(Function &F,
  ...
  return RVGPUUnifyDivergentExitNodesImpl(TransformInfo).run(F, DT, PDT, UA)
