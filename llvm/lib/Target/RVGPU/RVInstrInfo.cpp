//===- RVInstrInfo.cpp - SI Instruction Information  ----------------------===//
//
...
BranchOffsetBits("rvgpu-s-branch-bits", cl::ReallyHidden, cl::init(16),
                 cl::desc("Restrict range of branch instructions (DEBUG)"));

static cl::opt<bool> Fix16BitCopies(
  "rvgpu-fix-16-bit-physreg-copies",
...
static bool nodesHaveSameOperandValue(SDNode *N0, SDNode* N1, unsigned OpName) {
  ...
  int Op0Idx = RVGPU::getNamedOperandIdx(Opc0, OpName);
  int Op1Idx = RVGPU::getNamedOperandIdx(Opc1, OpName);
...
static bool canRemat(const MachineInstr &MI) {
  ...
  if (RVInstrInfo::isVOP1(MI) || RVInstrInfo::isVOP2(MI) ||
      RVInstrInfo::isVOP3(MI) || RVInstrInfo::isSDWA(MI) ||
      RVInstrInfo::isSALU(MI))
    return true;

  if (RVInstrInfo::isSMRD(MI)) {
...
bool RVInstrInfo::isReallyTriviallyReMaterializable(
...
static bool resultDependsOnExec(const MachineInstr &MI) {
  ...
  if (MI.isCompare()) {
    ...
    for (MachineInstr &Use : MRI.use_nodbg_instructions(DstReg)) {
      ...
      case RVGPU::S_AND_SAVEEXEC_B32:
      case RVGPU::S_AND_SAVEEXEC_B64:
        break;
      case RVGPU::S_AND_B32:
      case RVGPU::S_AND_B64:
        if (!Use.readsRegister(RVGPU::EXEC))
          return true;
        break;
      default:
        return true;
  ...
  default:
    break;
  case RVGPU::V_READFIRSTLANE_B32:
    return true;
  }
...
bool RVInstrInfo::isIgnorableUse(const MachineOperand &MO) const {
  // Any implicit use of exec by VALU is not a real register read.
  return MO.getReg() == RVGPU::EXEC && MO.isImplicit() &&
         isVALU(*MO.getParent()) && !resultDependsOnExec(*MO.getParent());
}

bool RVInstrInfo::isSafeToSink(MachineInstr &MI,
                               MachineBasicBlock *SuccToSinkTo,
                               MachineCycleInfo *CI) const {
  // Allow sinking if MI edits lane mask (divergent i1 in sgpr).
  if (MI.getOpcode() == RVGPU::SI_IF_BREAK)
    return true;
...
bool RVInstrInfo::areLoadsFromSameBasePtr(SDNode *Load0, SDNode *Load1,
                                          int64_t &Offset0,
  ...
  if (isDS(Opc0) && isDS(Opc1)) {
    ...
    // st64 versions).
    int Offset0Idx = RVGPU::getNamedOperandIdx(Opc0, RVGPU::OpName::offset);
    int Offset1Idx = RVGPU::getNamedOperandIdx(Opc1, RVGPU::OpName::offset);
  ...
  if (isSMRD(Opc0) && isSMRD(Opc1)) {
    ...
    if (!RVGPU::hasNamedOperand(Opc0, RVGPU::OpName::sbase) ||
        !RVGPU::hasNamedOperand(Opc1, RVGPU::OpName::sbase))
      return false;
  ...
  if ((isMUBUF(Opc0) || isMTBUF(Opc0)) && (isMUBUF(Opc1) || isMTBUF(Opc1))) {
    ...
    if (!nodesHaveSameOperandValue(Load0, Load1, RVGPU::OpName::soffset) ||
        !nodesHaveSameOperandValue(Load0, Load1, RVGPU::OpName::vaddr) ||
        !nodesHaveSameOperandValue(Load0, Load1, RVGPU::OpName::srsrc))
      return false;

    int OffIdx0 = RVGPU::getNamedOperandIdx(Opc0, RVGPU::OpName::offset);
    int OffIdx1 = RVGPU::getNamedOperandIdx(Opc1, RVGPU::OpName::offset);
...
static bool isStride64(unsigned Opc) {
  switch (Opc) {
  case RVGPU::DS_READ2ST64_B32:
  case RVGPU::DS_READ2ST64_B64:
  case RVGPU::DS_WRITE2ST64_B32:
  case RVGPU::DS_WRITE2ST64_B64:
    return true;
  default:
    return false;
  }
...
bool RVInstrInfo::getMemOperandsWithOffsetWidth(
  ...
  if (isDS(LdSt)) {
    BaseOp = getNamedOperand(LdSt, RVGPU::OpName::addr);
    OffsetOp = getNamedOperand(LdSt, RVGPU::OpName::offset);
    if (OffsetOp) {
      ...
      DataOpIdx = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::vdst);
      if (DataOpIdx == -1)
        DataOpIdx = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::data0);
      ...
      const MachineOperand *Offset0Op =
          getNamedOperand(LdSt, RVGPU::OpName::offset0);
      const MachineOperand *Offset1Op =
          getNamedOperand(LdSt, RVGPU::OpName::offset1);
      ...
      if (LdSt.mayLoad())
        ...
        int Data0Idx = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::data0);
      ...
      if (DataOpIdx == -1) {
        DataOpIdx = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::data0);
        Width = getOpSize(LdSt, DataOpIdx);
        DataOpIdx = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::data1);
        Width += getOpSize(LdSt, DataOpIdx);
      } else {
        Width = getOpSize(LdSt, DataOpIdx);
      }
  ...
  if (isMUBUF(LdSt) || isMTBUF(LdSt)) {
    const MachineOperand *RSrc = getNamedOperand(LdSt, RVGPU::OpName::srsrc);
    if (!RSrc) // e.g. BUFFER_WBINVL1_VOL
      return false;
    BaseOps.push_back(RSrc);
    BaseOp = getNamedOperand(LdSt, RVGPU::OpName::vaddr);
    if (BaseOp && !BaseOp->isFI())
      BaseOps.push_back(BaseOp);
    const MachineOperand *OffsetImm =
        getNamedOperand(LdSt, RVGPU::OpName::offset);
    Offset = OffsetImm->getImm();
    const MachineOperand *SOffset =
        getNamedOperand(LdSt, RVGPU::OpName::soffset);
    if (SOffset) {
      if (SOffset->isReg())
    ...
    DataOpIdx = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::vdst);
    if (DataOpIdx == -1)
      DataOpIdx = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::vdata);
  ...
  if (isMIMG(LdSt)) {
    int SRsrcIdx = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::srsrc);
    BaseOps.push_back(&LdSt.getOperand(SRsrcIdx));
    int VAddr0Idx = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::vaddr0);
    if (VAddr0Idx >= 0) {
      // GFX10 possible NSA encoding.
      for (int I = VAddr0Idx; I < SRsrcIdx; ++I)
        BaseOps.push_back(&LdSt.getOperand(I));
    } else {
      BaseOps.push_back(getNamedOperand(LdSt, RVGPU::OpName::vaddr));
    }
    Offset = 0;
    // Get appropriate operand, and compute width accordingly.
    DataOpIdx = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::vdata);
  ...
  if (isSMRD(LdSt)) {
    BaseOp = getNamedOperand(LdSt, RVGPU::OpName::sbase);
    ...
    // Get appropriate operand, and compute width accordingly.
    DataOpIdx = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::sdst);
    if (DataOpIdx == -1)
      return false;
    Width = getOpSize(LdSt, DataOpIdx);
    return true;
  ...
  if (isFLAT(LdSt)) {
    ...
    if (BaseOp)
      BaseOps.push_back(BaseOp);
    BaseOp = getNamedOperand(LdSt, RVGPU::OpName::saddr);
    if (BaseOp)
      BaseOps.push_back(BaseOp);
    Offset = getNamedOperand(LdSt, RVGPU::OpName::offset)->getImm();
    // Get appropriate operand, and compute width accordingly.
    DataOpIdx = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::vdst);
    if (DataOpIdx == -1)
      DataOpIdx = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::vdata);
...
bool RVInstrInfo::shouldClusterMemOps(ArrayRef<const MachineOperand *> BaseOps1,
...
bool RVInstrInfo::shouldScheduleLoadsNear(SDNode *Load0, SDNode *Load1,
...
static void reportIllegalCopy(const RVInstrInfo *TII, MachineBasicBlock &MBB,
  ...
  BuildMI(MBB, MI, DL, TII->get(RVGPU::SI_ILLEGAL_COPY), DestReg)
...
static void indirectCopyToAGPR(const RVInstrInfo &TII,
  ...
  assert((RVGPU::SReg_32RegClass.contains(SrcReg) ||
          RVGPU::AGPR_32RegClass.contains(SrcReg)) &&
         "Source register of the copy should be either an SGPR or an AGPR.");

  assert(RVGPU::AGPR_32RegClass.contains(DestReg) &&
         "Destination register of the copy should be an AGPR.");

  const RVRegisterInfo &RI = TII.getRegisterInfo();
  ...
  if (!RegsOverlap) {
    for (auto Def = MI, E = MBB.begin(); Def != E; ) {
      ...
      if (Def->getOpcode() != RVGPU::V_ACCVGPR_WRITE_B32_e64 ||
      ...
      MachineInstrBuilder Builder =
        BuildMI(MBB, MI, DL, TII.get(RVGPU::V_ACCVGPR_WRITE_B32_e64), DestReg)
        .add(DefOp);
      if (ImpDefSuperReg)
  ...
  unsigned MaxVGPRs = RI.getRegPressureLimit(&RVGPU::VGPR_32RegClass,
                                             *MBB.getParent());
  ...
  unsigned RegNo = (DestReg - RVGPU::AGPR0) % 3;
  Register Tmp =
      MBB.getParent()->getInfo<RVMachineFunctionInfo>()->getVGPRForAGPRCopy();
  ...
  // spill.
  while (RegNo--) {
    Register Tmp2 = RS.scavengeRegisterBackwards(RVGPU::VGPR_32RegClass, MI,
  ...
  unsigned TmpCopyOp = RVGPU::V_MOV_B32_e32;
  if (RVGPU::AGPR_32RegClass.contains(SrcReg)) {
    TmpCopyOp = RVGPU::V_ACCVGPR_READ_B32_e64;
  } else {
    assert(RVGPU::SReg_32RegClass.contains(SrcReg));
  }
...
static void expandSGPRCopy(const RVInstrInfo &TII, MachineBasicBlock &MBB,
  ...
  for (unsigned Idx = 0; Idx < BaseIndices.size(); ++Idx) {
    ...
    unsigned Opcode = RVGPU::S_MOV_B32;

    // Is SGPR aligned? If so try to combine with next.
    bool AlignedDest = ((DestSubReg - RVGPU::SGPR0) % 2) == 0;
    bool AlignedSrc = ((SrcSubReg - RVGPU::SGPR0) % 2) == 0;
...
void RVInstrInfo::copyPhysReg(MachineBasicBlock &MBB,
                              MachineBasicBlock::iterator MI,
                              const DebugLoc &DL, MCRegister DestReg,
  ...
  if (Fix16BitCopies) {
    if (((Size == 16) != (SrcSize == 16))) {
      ...
      MCRegister SubReg = RI.getSubReg(RegToFix, RVGPU::lo16);
      RegToFix = SubReg;

      if (DestReg == SrcReg) {
        // Identity copy. Insert empty bundle since ExpandPostRA expects an
        // instruction here.
        BuildMI(MBB, MI, DL, get(RVGPU::BUNDLE));
        return;
      }
  ...
  if (RC == &RVGPU::VGPR_32RegClass) {
    assert(RVGPU::VGPR_32RegClass.contains(SrcReg) ||
           RVGPU::SReg_32RegClass.contains(SrcReg) ||
           RVGPU::AGPR_32RegClass.contains(SrcReg));
    unsigned Opc = RVGPU::AGPR_32RegClass.contains(SrcReg) ?
                     RVGPU::V_ACCVGPR_READ_B32_e64 : RVGPU::V_MOV_B32_e32;
    BuildMI(MBB, MI, DL, get(Opc), DestReg)
      .addReg(SrcReg, getKillRegState(KillSrc));
    return;
  }

  if (RC == &RVGPU::SReg_32_XM0RegClass ||
      RC == &RVGPU::SReg_32RegClass) {
    if (SrcReg == RVGPU::SCC) {
      BuildMI(MBB, MI, DL, get(RVGPU::S_CSELECT_B32), DestReg)
          .addImm(1)
          .addImm(0);
      return;
    }

    if (DestReg == RVGPU::VCC_LO) {
      if (RVGPU::SReg_32RegClass.contains(SrcReg)) {
        BuildMI(MBB, MI, DL, get(RVGPU::S_MOV_B32), RVGPU::VCC_LO)
          .addReg(SrcReg, getKillRegState(KillSrc));
      } else {
        // FIXME: Hack until VReg_1 removed.
        assert(RVGPU::VGPR_32RegClass.contains(SrcReg));
        BuildMI(MBB, MI, DL, get(RVGPU::V_CMP_NE_U32_e32))
          .addImm(0)
          .addReg(SrcReg, getKillRegState(KillSrc));
      }
    ...
    BuildMI(MBB, MI, DL, get(RVGPU::S_MOV_B32), DestReg)
            .addReg(SrcReg, getKillRegState(KillSrc));
    return;
  }

  if (RC == &RVGPU::SReg_64RegClass) {
    if (SrcReg == RVGPU::SCC) {
      BuildMI(MBB, MI, DL, get(RVGPU::S_CSELECT_B64), DestReg)
          .addImm(1)
          .addImm(0);
      return;
    }

    if (DestReg == RVGPU::VCC) {
      if (RVGPU::SReg_64RegClass.contains(SrcReg)) {
        BuildMI(MBB, MI, DL, get(RVGPU::S_MOV_B64), RVGPU::VCC)
          .addReg(SrcReg, getKillRegState(KillSrc));
      } else {
        // FIXME: Hack until VReg_1 removed.
        assert(RVGPU::VGPR_32RegClass.contains(SrcReg));
    ...
    BuildMI(MBB, MI, DL, get(RVGPU::S_MOV_B64), DestReg)
            .addReg(SrcReg, getKillRegState(KillSrc));
    return;
  }

  if (DestReg == RVGPU::SCC) {
    ...
    if (RVGPU::SReg_64RegClass.contains(SrcReg)) {
      ...
      BuildMI(MBB, MI, DL, get(RVGPU::S_CMP_LG_U64))
          .addReg(SrcReg, getKillRegState(KillSrc))
          .addImm(0);
    } else {
      assert(RVGPU::SReg_32RegClass.contains(SrcReg));
      BuildMI(MBB, MI, DL, get(RVGPU::S_CMP_LG_U32))
          .addReg(SrcReg, getKillRegState(KillSrc))
          .addImm(0);
    }
  ...
  if (RC == &RVGPU::AGPR_32RegClass) {
    if (RVGPU::VGPR_32RegClass.contains(SrcReg) ||
        (ST.hasGFX90AInsts() && RVGPU::SReg_32RegClass.contains(SrcReg))) {
      BuildMI(MBB, MI, DL, get(RVGPU::V_ACCVGPR_WRITE_B32_e64), DestReg)
        .addReg(SrcReg, getKillRegState(KillSrc));
      return;
    }

    if (RVGPU::AGPR_32RegClass.contains(SrcReg) && ST.hasGFX90AInsts()) {
      BuildMI(MBB, MI, DL, get(RVGPU::V_ACCVGPR_MOV_B32), DestReg)
        .addReg(SrcReg, getKillRegState(KillSrc));
      return;
    }
  ...
  if (Size == 16) {
    assert(RVGPU::VGPR_16RegClass.contains(SrcReg) ||
           RVGPU::SReg_LO16RegClass.contains(SrcReg) ||
           RVGPU::AGPR_LO16RegClass.contains(SrcReg));

    bool IsSGPRDst = RVGPU::SReg_LO16RegClass.contains(DestReg);
    bool IsSGPRSrc = RVGPU::SReg_LO16RegClass.contains(SrcReg);
    bool IsAGPRDst = RVGPU::AGPR_LO16RegClass.contains(DestReg);
    bool IsAGPRSrc = RVGPU::AGPR_LO16RegClass.contains(SrcReg);
    bool DstLow = RVGPU::VGPR_LO16RegClass.contains(DestReg) ||
                  RVGPU::SReg_LO16RegClass.contains(DestReg) ||
                  RVGPU::AGPR_LO16RegClass.contains(DestReg);
    bool SrcLow = RVGPU::VGPR_LO16RegClass.contains(SrcReg) ||
                  RVGPU::SReg_LO16RegClass.contains(SrcReg) ||
                  RVGPU::AGPR_LO16RegClass.contains(SrcReg);
    ...
    if (IsSGPRDst) {
      ...
      BuildMI(MBB, MI, DL, get(RVGPU::S_MOV_B32), NewDestReg)
    ...
    if (ST.hasTrue16BitInsts()) {
      ...
      if (RVGPU::VGPR_16_Lo128RegClass.contains(DestReg) &&
          (IsSGPRSrc || RVGPU::VGPR_16_Lo128RegClass.contains(SrcReg))) {
        BuildMI(MBB, MI, DL, get(RVGPU::V_MOV_B16_t16_e32), DestReg)
            .addReg(SrcReg);
      } else {
        BuildMI(MBB, MI, DL, get(RVGPU::V_MOV_B16_t16_e64), DestReg)
    ...
    if (IsSGPRSrc && !ST.hasSDWAScalar()) {
      ...
      BuildMI(MBB, MI, DL, get(RVGPU::V_MOV_B32_e32), NewDestReg)
        .addReg(NewSrcReg, getKillRegState(KillSrc));
      return;
    }

    auto MIB = BuildMI(MBB, MI, DL, get(RVGPU::V_MOV_B32_sdwa), NewDestReg)
      .addImm(0) // src0_modifiers
      .addReg(NewSrcReg)
      .addImm(0) // clamp
      .addImm(DstLow ? RVGPU::SDWA::SdwaSel::WORD_0
                     : RVGPU::SDWA::SdwaSel::WORD_1)
      .addImm(RVGPU::SDWA::DstUnused::UNUSED_PRESERVE)
      .addImm(SrcLow ? RVGPU::SDWA::SdwaSel::WORD_0
                     : RVGPU::SDWA::SdwaSel::WORD_1)
  ...
  if (RC == RI.getVGPR64Class() && (SrcRC == RC || RI.isSGPRClass(SrcRC))) {
    if (ST.hasMovB64()) {
      BuildMI(MBB, MI, DL, get(RVGPU::V_MOV_B64_e32), DestReg)
        .addReg(SrcReg, getKillRegState(KillSrc));
      return;
    }
    if (ST.hasPkMovB32()) {
      BuildMI(MBB, MI, DL, get(RVGPU::V_PK_MOV_B32), DestReg)
  ...
  unsigned EltSize = 4;
  unsigned Opcode = RVGPU::V_MOV_B32_e32;
  if (RI.isAGPRClass(RC)) {
    if (ST.hasGFX90AInsts() && RI.isAGPRClass(SrcRC))
      Opcode = RVGPU::V_ACCVGPR_MOV_B32;
    else if (RI.hasVGPRs(SrcRC) ||
             (ST.hasGFX90AInsts() && RI.isSGPRClass(SrcRC)))
      Opcode = RVGPU::V_ACCVGPR_WRITE_B32_e64;
    else
      Opcode = RVGPU::INSTRUCTION_LIST_END;
  } else if (RI.hasVGPRs(RC) && RI.isAGPRClass(SrcRC)) {
    Opcode = RVGPU::V_ACCVGPR_READ_B32_e64;
    ...
    if (ST.hasMovB64()) {
      Opcode = RVGPU::V_MOV_B64_e32;
      EltSize = 8;
    } else if (ST.hasPkMovB32()) {
      Opcode = RVGPU::V_PK_MOV_B32;
      EltSize = 8;
    }
  }
  ...
  if (Opcode == RVGPU::INSTRUCTION_LIST_END)
  ...
  for (unsigned Idx = 0; Idx < SubIndices.size(); ++Idx) {
    ...
    } else if (Opcode == RVGPU::V_PK_MOV_B32) {
      MachineInstrBuilder MIB =
          BuildMI(MBB, MI, DL, get(RVGPU::V_PK_MOV_B32), DestSubReg)
              .addImm(SISrcMods::OP_SEL_1)
              .addReg(SrcSubReg)
              .addImm(SISrcMods::OP_SEL_0 | SISrcMods::OP_SEL_1)
              .addReg(SrcSubReg)
...
int RVInstrInfo::commuteOpcode(unsigned Opcode) const {
  int NewOpc;

  // Try to map original to commuted opcode
  NewOpc = RVGPU::getCommuteRev(Opcode);
  if (NewOpc != -1)
    // Check if the commuted (REV) opcode exists on the target.
    return pseudoToMCOpcode(NewOpc) != -1 ? NewOpc : -1;

  // Try to map commuted to original opcode
  NewOpc = RVGPU::getCommuteOrig(Opcode);
  if (NewOpc != -1)
...
void RVInstrInfo::materializeImmediate(MachineBasicBlock &MBB,
                                       MachineBasicBlock::iterator MI,
                                       const DebugLoc &DL, Register DestReg,
                                       int64_t Value) const {
  MachineRegisterInfo &MRI = MBB.getParent()->getRegInfo();
  const TargetRegisterClass *RegClass = MRI.getRegClass(DestReg);
  if (RegClass == &RVGPU::SReg_32RegClass ||
      RegClass == &RVGPU::SGPR_32RegClass ||
      RegClass == &RVGPU::SReg_32_XM0RegClass ||
      RegClass == &RVGPU::SReg_32_XM0_XEXECRegClass) {
    BuildMI(MBB, MI, DL, get(RVGPU::S_MOV_B32), DestReg)
      .addImm(Value);
    return;
  }

  if (RegClass == &RVGPU::SReg_64RegClass ||
      RegClass == &RVGPU::SGPR_64RegClass ||
      RegClass == &RVGPU::SReg_64_XEXECRegClass) {
    BuildMI(MBB, MI, DL, get(RVGPU::S_MOV_B64), DestReg)
      .addImm(Value);
    return;
  }

  if (RegClass == &RVGPU::VGPR_32RegClass) {
    BuildMI(MBB, MI, DL, get(RVGPU::V_MOV_B32_e32), DestReg)
      .addImm(Value);
    return;
  }
  if (RegClass->hasSuperClassEq(&RVGPU::VReg_64RegClass)) {
    BuildMI(MBB, MI, DL, get(RVGPU::V_MOV_B64_PSEUDO), DestReg)
      .addImm(Value);
    return;
  }
...
RVInstrInfo::getPreferredSelectRegClass(unsigned Size) const {
  return &RVGPU::VGPR_32RegClass;
}

void RVInstrInfo::insertVectorSelect(MachineBasicBlock &MBB,
  ...
  const TargetRegisterClass *BoolXExecRC =
    RI.getRegClass(RVGPU::SReg_1_XEXECRegClassID);
  assert(MRI.getRegClass(DstReg) == &RVGPU::VGPR_32RegClass &&
         "Not a VGPR32 reg");

  if (Cond.size() == 1) {
    Register SReg = MRI.createVirtualRegister(BoolXExecRC);
    BuildMI(MBB, I, DL, get(RVGPU::COPY), SReg)
      .add(Cond[0]);
    BuildMI(MBB, I, DL, get(RVGPU::V_CNDMASK_B32_e64), DstReg)
      .addImm(0)
      .addReg(FalseReg)
      .addImm(0)
      .addReg(TrueReg)
    ...
    case RVInstrInfo::SCC_TRUE: {
      Register SReg = MRI.createVirtualRegister(BoolXExecRC);
      BuildMI(MBB, I, DL, get(ST.isWave32() ? RVGPU::S_CSELECT_B32
                                            : RVGPU::S_CSELECT_B64), SReg)
        .addImm(1)
        .addImm(0);
      BuildMI(MBB, I, DL, get(RVGPU::V_CNDMASK_B32_e64), DstReg)
        .addImm(0)
    ...
    case RVInstrInfo::EXECNZ: {
      Register SReg = MRI.createVirtualRegister(BoolXExecRC);
      Register SReg2 = MRI.createVirtualRegister(RI.getBoolRC());
      BuildMI(MBB, I, DL, get(ST.isWave32() ? RVGPU::S_OR_SAVEEXEC_B32
                                            : RVGPU::S_OR_SAVEEXEC_B64), SReg2)
        .addImm(0);
      BuildMI(MBB, I, DL, get(ST.isWave32() ? RVGPU::S_CSELECT_B32
                                            : RVGPU::S_CSELECT_B64), SReg)
        .addImm(1)
...
Register RVInstrInfo::insertEQ(MachineBasicBlock *MBB,
                               MachineBasicBlock::iterator I,
                               const DebugLoc &DL,
                               Register SrcReg, int Value) const {
  MachineRegisterInfo &MRI = MBB->getParent()->getRegInfo();
  Register Reg = MRI.createVirtualRegister(RI.getBoolRC());
  BuildMI(*MBB, I, DL, get(RVGPU::V_CMP_EQ_I32_e64), Reg)
    .addImm(Value)
    .addReg(SrcReg);
...
Register RVInstrInfo::insertNE(MachineBasicBlock *MBB,
                               MachineBasicBlock::iterator I,
                               const DebugLoc &DL,
                               Register SrcReg, int Value) const {
  MachineRegisterInfo &MRI = MBB->getParent()->getRegInfo();
  Register Reg = MRI.createVirtualRegister(RI.getBoolRC());
  BuildMI(*MBB, I, DL, get(RVGPU::V_CMP_NE_I32_e64), Reg)
    .addImm(Value)
    .addReg(SrcReg);
...
unsigned RVInstrInfo::getMovOpcode(const TargetRegisterClass *DstRC) const {
  ...
  if (RI.getRegSizeInBits(*DstRC) == 16) {
    ...
    // before RA.
    return RI.isSGPRClass(DstRC) ? RVGPU::COPY : RVGPU::V_MOV_B16_t16_e64;
  } else if (RI.getRegSizeInBits(*DstRC) == 32) {
    return RI.isSGPRClass(DstRC) ? RVGPU::S_MOV_B32 : RVGPU::V_MOV_B32_e32;
  } else if (RI.getRegSizeInBits(*DstRC) == 64 && RI.isSGPRClass(DstRC)) {
    return RVGPU::S_MOV_B64;
  } else if (RI.getRegSizeInBits(*DstRC) == 64 && !RI.isSGPRClass(DstRC)) {
    return  RVGPU::V_MOV_B64_PSEUDO;
  }
  return RVGPU::COPY;
}

const MCInstrDesc &
RVInstrInfo::getIndirectGPRIDXPseudo(unsigned VecSize,
                                     bool IsIndirectSrc) const {
  if (IsIndirectSrc) {
    if (VecSize <= 32) // 4 bytes
      return get(RVGPU::V_INDIRECT_REG_READ_GPR_IDX_B32_V1);
    if (VecSize <= 64) // 8 bytes
      return get(RVGPU::V_INDIRECT_REG_READ_GPR_IDX_B32_V2);
    if (VecSize <= 96) // 12 bytes
      return get(RVGPU::V_INDIRECT_REG_READ_GPR_IDX_B32_V3);
    if (VecSize <= 128) // 16 bytes
      return get(RVGPU::V_INDIRECT_REG_READ_GPR_IDX_B32_V4);
    if (VecSize <= 160) // 20 bytes
      return get(RVGPU::V_INDIRECT_REG_READ_GPR_IDX_B32_V5);
    if (VecSize <= 256) // 32 bytes
      return get(RVGPU::V_INDIRECT_REG_READ_GPR_IDX_B32_V8);
    if (VecSize <= 288) // 36 bytes
      return get(RVGPU::V_INDIRECT_REG_READ_GPR_IDX_B32_V9);
    if (VecSize <= 320) // 40 bytes
      return get(RVGPU::V_INDIRECT_REG_READ_GPR_IDX_B32_V10);
    if (VecSize <= 352) // 44 bytes
      return get(RVGPU::V_INDIRECT_REG_READ_GPR_IDX_B32_V11);
    if (VecSize <= 384) // 48 bytes
      return get(RVGPU::V_INDIRECT_REG_READ_GPR_IDX_B32_V12);
    if (VecSize <= 512) // 64 bytes
      return get(RVGPU::V_INDIRECT_REG_READ_GPR_IDX_B32_V16);
    if (VecSize <= 1024) // 128 bytes
      return get(RVGPU::V_INDIRECT_REG_READ_GPR_IDX_B32_V32);

    llvm_unreachable("unsupported size for IndirectRegReadGPRIDX pseudos");
  }

  if (VecSize <= 32) // 4 bytes
    return get(RVGPU::V_INDIRECT_REG_WRITE_GPR_IDX_B32_V1);
  if (VecSize <= 64) // 8 bytes
    return get(RVGPU::V_INDIRECT_REG_WRITE_GPR_IDX_B32_V2);
  if (VecSize <= 96) // 12 bytes
    return get(RVGPU::V_INDIRECT_REG_WRITE_GPR_IDX_B32_V3);
  if (VecSize <= 128) // 16 bytes
    return get(RVGPU::V_INDIRECT_REG_WRITE_GPR_IDX_B32_V4);
  if (VecSize <= 160) // 20 bytes
    return get(RVGPU::V_INDIRECT_REG_WRITE_GPR_IDX_B32_V5);
  if (VecSize <= 256) // 32 bytes
    return get(RVGPU::V_INDIRECT_REG_WRITE_GPR_IDX_B32_V8);
  if (VecSize <= 288) // 36 bytes
    return get(RVGPU::V_INDIRECT_REG_WRITE_GPR_IDX_B32_V9);
  if (VecSize <= 320) // 40 bytes
    return get(RVGPU::V_INDIRECT_REG_WRITE_GPR_IDX_B32_V10);
  if (VecSize <= 352) // 44 bytes
    return get(RVGPU::V_INDIRECT_REG_WRITE_GPR_IDX_B32_V11);
  if (VecSize <= 384) // 48 bytes
    return get(RVGPU::V_INDIRECT_REG_WRITE_GPR_IDX_B32_V12);
  if (VecSize <= 512) // 64 bytes
    return get(RVGPU::V_INDIRECT_REG_WRITE_GPR_IDX_B32_V16);
  if (VecSize <= 1024) // 128 bytes
    return get(RVGPU::V_INDIRECT_REG_WRITE_GPR_IDX_B32_V32);
...
static unsigned getIndirectVGPRWriteMovRelPseudoOpc(unsigned VecSize) {
  if (VecSize <= 32) // 4 bytes
    return RVGPU::V_INDIRECT_REG_WRITE_MOVREL_B32_V1;
  if (VecSize <= 64) // 8 bytes
    return RVGPU::V_INDIRECT_REG_WRITE_MOVREL_B32_V2;
  if (VecSize <= 96) // 12 bytes
    return RVGPU::V_INDIRECT_REG_WRITE_MOVREL_B32_V3;
  if (VecSize <= 128) // 16 bytes
    return RVGPU::V_INDIRECT_REG_WRITE_MOVREL_B32_V4;
  if (VecSize <= 160) // 20 bytes
    return RVGPU::V_INDIRECT_REG_WRITE_MOVREL_B32_V5;
  if (VecSize <= 256) // 32 bytes
    return RVGPU::V_INDIRECT_REG_WRITE_MOVREL_B32_V8;
  if (VecSize <= 288) // 36 bytes
    return RVGPU::V_INDIRECT_REG_WRITE_MOVREL_B32_V9;
  if (VecSize <= 320) // 40 bytes
    return RVGPU::V_INDIRECT_REG_WRITE_MOVREL_B32_V10;
  if (VecSize <= 352) // 44 bytes
    return RVGPU::V_INDIRECT_REG_WRITE_MOVREL_B32_V11;
  if (VecSize <= 384) // 48 bytes
    return RVGPU::V_INDIRECT_REG_WRITE_MOVREL_B32_V12;
  if (VecSize <= 512) // 64 bytes
    return RVGPU::V_INDIRECT_REG_WRITE_MOVREL_B32_V16;
  if (VecSize <= 1024) // 128 bytes
    return RVGPU::V_INDIRECT_REG_WRITE_MOVREL_B32_V32;
...
static unsigned getIndirectSGPRWriteMovRelPseudo32(unsigned VecSize) {
  if (VecSize <= 32) // 4 bytes
    return RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B32_V1;
  if (VecSize <= 64) // 8 bytes
    return RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B32_V2;
  if (VecSize <= 96) // 12 bytes
    return RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B32_V3;
  if (VecSize <= 128) // 16 bytes
    return RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B32_V4;
  if (VecSize <= 160) // 20 bytes
    return RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B32_V5;
  if (VecSize <= 256) // 32 bytes
    return RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B32_V8;
  if (VecSize <= 288) // 36 bytes
    return RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B32_V9;
  if (VecSize <= 320) // 40 bytes
    return RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B32_V10;
  if (VecSize <= 352) // 44 bytes
    return RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B32_V11;
  if (VecSize <= 384) // 48 bytes
    return RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B32_V12;
  if (VecSize <= 512) // 64 bytes
    return RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B32_V16;
  if (VecSize <= 1024) // 128 bytes
    return RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B32_V32;
...
static unsigned getIndirectSGPRWriteMovRelPseudo64(unsigned VecSize) {
  if (VecSize <= 64) // 8 bytes
    return RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B64_V1;
  if (VecSize <= 128) // 16 bytes
    return RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B64_V2;
  if (VecSize <= 256) // 32 bytes
    return RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B64_V4;
  if (VecSize <= 512) // 64 bytes
    return RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B64_V8;
  if (VecSize <= 1024) // 128 bytes
    return RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B64_V16;

  llvm_unreachable("unsupported size for IndirectRegWrite pseudos");
}

const MCInstrDesc &
RVInstrInfo::getIndirectRegWriteMovRelPseudo(unsigned VecSize, unsigned EltSize,
                                             bool IsSGPR) const {
  if (IsSGPR) {
    switch (EltSize) {
    case 32:
...
static unsigned getSGPRSpillSaveOpcode(unsigned Size) {
  switch (Size) {
  case 4:
    return RVGPU::SI_SPILL_S32_SAVE;
  case 8:
    return RVGPU::SI_SPILL_S64_SAVE;
  case 12:
    return RVGPU::SI_SPILL_S96_SAVE;
  case 16:
    return RVGPU::SI_SPILL_S128_SAVE;
  case 20:
    return RVGPU::SI_SPILL_S160_SAVE;
  case 24:
    return RVGPU::SI_SPILL_S192_SAVE;
  case 28:
    return RVGPU::SI_SPILL_S224_SAVE;
  case 32:
    return RVGPU::SI_SPILL_S256_SAVE;
  case 36:
    return RVGPU::SI_SPILL_S288_SAVE;
  case 40:
    return RVGPU::SI_SPILL_S320_SAVE;
  case 44:
    return RVGPU::SI_SPILL_S352_SAVE;
  case 48:
    return RVGPU::SI_SPILL_S384_SAVE;
  case 64:
    return RVGPU::SI_SPILL_S512_SAVE;
  case 128:
    return RVGPU::SI_SPILL_S1024_SAVE;
  default:
...
static unsigned getVGPRSpillSaveOpcode(unsigned Size) {
  switch (Size) {
  case 4:
    return RVGPU::SI_SPILL_V32_SAVE;
  case 8:
    return RVGPU::SI_SPILL_V64_SAVE;
  case 12:
    return RVGPU::SI_SPILL_V96_SAVE;
  case 16:
    return RVGPU::SI_SPILL_V128_SAVE;
  case 20:
    return RVGPU::SI_SPILL_V160_SAVE;
  case 24:
    return RVGPU::SI_SPILL_V192_SAVE;
  case 28:
    return RVGPU::SI_SPILL_V224_SAVE;
  case 32:
    return RVGPU::SI_SPILL_V256_SAVE;
  case 36:
    return RVGPU::SI_SPILL_V288_SAVE;
  case 40:
    return RVGPU::SI_SPILL_V320_SAVE;
  case 44:
    return RVGPU::SI_SPILL_V352_SAVE;
  case 48:
    return RVGPU::SI_SPILL_V384_SAVE;
  case 64:
    return RVGPU::SI_SPILL_V512_SAVE;
  case 128:
    return RVGPU::SI_SPILL_V1024_SAVE;
  default:
    llvm_unreachable("unknown register size");
  }
}
...
static unsigned getAGPRSpillSaveOpcode(unsigned Size) {
  switch (Size) {
  case 4:
    return RVGPU::SI_SPILL_A32_SAVE;
  case 8:
    return RVGPU::SI_SPILL_A64_SAVE;
  case 12:
    return RVGPU::SI_SPILL_A96_SAVE;
  case 16:
    return RVGPU::SI_SPILL_A128_SAVE;
  case 20:
    return RVGPU::SI_SPILL_A160_SAVE;
  case 24:
    return RVGPU::SI_SPILL_A192_SAVE;
  case 28:
    return RVGPU::SI_SPILL_A224_SAVE;
  case 32:
    return RVGPU::SI_SPILL_A256_SAVE;
  case 36:
    return RVGPU::SI_SPILL_A288_SAVE;
  case 40:
    return RVGPU::SI_SPILL_A320_SAVE;
  case 44:
    return RVGPU::SI_SPILL_A352_SAVE;
  case 48:
    return RVGPU::SI_SPILL_A384_SAVE;
  case 64:
    return RVGPU::SI_SPILL_A512_SAVE;
  case 128:
    return RVGPU::SI_SPILL_A1024_SAVE;
  default:
    llvm_unreachable("unknown register size");
  }
}
...
static unsigned getAVSpillSaveOpcode(unsigned Size) {
  switch (Size) {
  case 4:
    return RVGPU::SI_SPILL_AV32_SAVE;
  case 8:
    return RVGPU::SI_SPILL_AV64_SAVE;
  case 12:
    return RVGPU::SI_SPILL_AV96_SAVE;
  case 16:
    return RVGPU::SI_SPILL_AV128_SAVE;
  case 20:
    return RVGPU::SI_SPILL_AV160_SAVE;
  case 24:
    return RVGPU::SI_SPILL_AV192_SAVE;
  case 28:
    return RVGPU::SI_SPILL_AV224_SAVE;
  case 32:
    return RVGPU::SI_SPILL_AV256_SAVE;
  case 36:
    return RVGPU::SI_SPILL_AV288_SAVE;
  case 40:
    return RVGPU::SI_SPILL_AV320_SAVE;
  case 44:
    return RVGPU::SI_SPILL_AV352_SAVE;
  case 48:
    return RVGPU::SI_SPILL_AV384_SAVE;
  case 64:
    return RVGPU::SI_SPILL_AV512_SAVE;
  case 128:
    return RVGPU::SI_SPILL_AV1024_SAVE;
  default:
    llvm_unreachable("unknown register size");
  }
}
...
static unsigned getWWMRegSpillSaveOpcode(unsigned Size,
  ...
  if (IsVectorSuperClass)
    return RVGPU::SI_SPILL_WWM_AV32_SAVE;

  return RVGPU::SI_SPILL_WWM_V32_SAVE;
}
...
static unsigned getVectorRegSpillSaveOpcode(Register Reg,
  ...
  if (MFI.checkFlag(Reg, RVGPU::VirtRegFlag::WWM_REG))
...
void RVInstrInfo::storeRegToStackSlot(
  ...
  if (RI.isSGPRClass(RC)) {
    ...
    assert(SrcReg != RVGPU::M0 && "m0 should not be spilled");
    assert(SrcReg != RVGPU::EXEC_LO && SrcReg != RVGPU::EXEC_HI &&
           SrcReg != RVGPU::EXEC && "exec should not be spilled");
    ...
    if (SrcReg.isVirtual() && SpillSize == 4) {
      MRI.constrainRegClass(SrcReg, &RVGPU::SReg_32_XM0_XEXECRegClass);
    }
...
static unsigned getSGPRSpillRestoreOpcode(unsigned Size) {
  switch (Size) {
  case 4:
    return RVGPU::SI_SPILL_S32_RESTORE;
  case 8:
    return RVGPU::SI_SPILL_S64_RESTORE;
  case 12:
    return RVGPU::SI_SPILL_S96_RESTORE;
  case 16:
    return RVGPU::SI_SPILL_S128_RESTORE;
  case 20:
    return RVGPU::SI_SPILL_S160_RESTORE;
  case 24:
    return RVGPU::SI_SPILL_S192_RESTORE;
  case 28:
    return RVGPU::SI_SPILL_S224_RESTORE;
  case 32:
    return RVGPU::SI_SPILL_S256_RESTORE;
  case 36:
    return RVGPU::SI_SPILL_S288_RESTORE;
  case 40:
    return RVGPU::SI_SPILL_S320_RESTORE;
  case 44:
    return RVGPU::SI_SPILL_S352_RESTORE;
  case 48:
    return RVGPU::SI_SPILL_S384_RESTORE;
  case 64:
    return RVGPU::SI_SPILL_S512_RESTORE;
  case 128:
    return RVGPU::SI_SPILL_S1024_RESTORE;
  default:
    llvm_unreachable("unknown register size");
  }
}
...
static unsigned getVGPRSpillRestoreOpcode(unsigned Size) {
  switch (Size) {
  case 4:
    return RVGPU::SI_SPILL_V32_RESTORE;
  case 8:
    return RVGPU::SI_SPILL_V64_RESTORE;
  case 12:
    return RVGPU::SI_SPILL_V96_RESTORE;
  case 16:
    return RVGPU::SI_SPILL_V128_RESTORE;
  case 20:
    return RVGPU::SI_SPILL_V160_RESTORE;
  case 24:
    return RVGPU::SI_SPILL_V192_RESTORE;
  case 28:
    return RVGPU::SI_SPILL_V224_RESTORE;
  case 32:
    return RVGPU::SI_SPILL_V256_RESTORE;
  case 36:
    return RVGPU::SI_SPILL_V288_RESTORE;
  case 40:
    return RVGPU::SI_SPILL_V320_RESTORE;
  case 44:
    return RVGPU::SI_SPILL_V352_RESTORE;
  case 48:
    return RVGPU::SI_SPILL_V384_RESTORE;
  case 64:
    return RVGPU::SI_SPILL_V512_RESTORE;
  case 128:
    return RVGPU::SI_SPILL_V1024_RESTORE;
  default:
    llvm_unreachable("unknown register size");
  }
}
...
static unsigned getAGPRSpillRestoreOpcode(unsigned Size) {
  switch (Size) {
  case 4:
    return RVGPU::SI_SPILL_A32_RESTORE;
  case 8:
    return RVGPU::SI_SPILL_A64_RESTORE;
  case 12:
    return RVGPU::SI_SPILL_A96_RESTORE;
  case 16:
    return RVGPU::SI_SPILL_A128_RESTORE;
  case 20:
    return RVGPU::SI_SPILL_A160_RESTORE;
  case 24:
    return RVGPU::SI_SPILL_A192_RESTORE;
  case 28:
    return RVGPU::SI_SPILL_A224_RESTORE;
  case 32:
    return RVGPU::SI_SPILL_A256_RESTORE;
  case 36:
    return RVGPU::SI_SPILL_A288_RESTORE;
  case 40:
    return RVGPU::SI_SPILL_A320_RESTORE;
  case 44:
    return RVGPU::SI_SPILL_A352_RESTORE;
  case 48:
    return RVGPU::SI_SPILL_A384_RESTORE;
  case 64:
    return RVGPU::SI_SPILL_A512_RESTORE;
  case 128:
    return RVGPU::SI_SPILL_A1024_RESTORE;
  default:
    llvm_unreachable("unknown register size");
  }
}
...
static unsigned getAVSpillRestoreOpcode(unsigned Size) {
  switch (Size) {
  case 4:
    return RVGPU::SI_SPILL_AV32_RESTORE;
  case 8:
    return RVGPU::SI_SPILL_AV64_RESTORE;
  case 12:
    return RVGPU::SI_SPILL_AV96_RESTORE;
  case 16:
    return RVGPU::SI_SPILL_AV128_RESTORE;
  case 20:
    return RVGPU::SI_SPILL_AV160_RESTORE;
  case 24:
    return RVGPU::SI_SPILL_AV192_RESTORE;
  case 28:
    return RVGPU::SI_SPILL_AV224_RESTORE;
  case 32:
    return RVGPU::SI_SPILL_AV256_RESTORE;
  case 36:
    return RVGPU::SI_SPILL_AV288_RESTORE;
  case 40:
    return RVGPU::SI_SPILL_AV320_RESTORE;
  case 44:
    return RVGPU::SI_SPILL_AV352_RESTORE;
  case 48:
    return RVGPU::SI_SPILL_AV384_RESTORE;
  case 64:
    return RVGPU::SI_SPILL_AV512_RESTORE;
  case 128:
    return RVGPU::SI_SPILL_AV1024_RESTORE;
  default:
    llvm_unreachable("unknown register size");
  }
}
...
static unsigned getWWMRegSpillRestoreOpcode(unsigned Size,
  ...
  if (IsVectorSuperClass)
    return RVGPU::SI_SPILL_WWM_AV32_RESTORE;

  return RVGPU::SI_SPILL_WWM_V32_RESTORE;
}

static unsigned
getVectorRegSpillRestoreOpcode(Register Reg, const TargetRegisterClass *RC,
                               unsigned Size, const RVRegisterInfo &TRI,
                               const RVMachineFunctionInfo &MFI) {
  bool IsVectorSuperClass = TRI.isVectorSuperClass(RC);
...
void RVInstrInfo::loadRegFromStackSlot(MachineBasicBlock &MBB,
                                       MachineBasicBlock::iterator MI,
  ...
  if (RI.isSGPRClass(RC)) {
    MFI->setHasSpilledSGPRs();
    assert(DestReg != RVGPU::M0 && "m0 should not be reloaded into");
    assert(DestReg != RVGPU::EXEC_LO && DestReg != RVGPU::EXEC_HI &&
           DestReg != RVGPU::EXEC && "exec should not be spilled");
    ...
    if (DestReg.isVirtual() && SpillSize == 4) {
      MachineRegisterInfo &MRI = MF->getRegInfo();
      MRI.constrainRegClass(DestReg, &RVGPU::SReg_32_XM0_XEXECRegClass);
    }
...
void RVInstrInfo::insertNoop(MachineBasicBlock &MBB,
                             MachineBasicBlock::iterator MI) const {
  insertNoops(MBB, MI, 1);
}

void RVInstrInfo::insertNoops(MachineBasicBlock &MBB,
                              MachineBasicBlock::iterator MI,
  ...
  while (Quantity > 0) {
    ...
    Quantity -= Arg;
    BuildMI(MBB, MI, DL, get(RVGPU::S_NOP)).addImm(Arg - 1);
  }
}

void RVInstrInfo::insertReturn(MachineBasicBlock &MBB) const {
  ...
  if (MBB.succ_empty()) {
    ...
    if (HasNoTerminator) {
      if (Info->returnsVoid()) {
        BuildMI(MBB, MBB.end(), DebugLoc(), get(RVGPU::S_ENDPGM)).addImm(0);
      } else {
        BuildMI(MBB, MBB.end(), DebugLoc(), get(RVGPU::SI_RETURN_TO_EPILOG));
      }
    }
  }
}

unsigned RVInstrInfo::getNumWaitStates(const MachineInstr &MI) {
  switch (MI.getOpcode()) {
  default:
...
bool RVInstrInfo::expandPostRAPseudo(MachineInstr &MI) const {
  const RVRegisterInfo *TRI = ST.getRegisterInfo();
  MachineBasicBlock &MBB = *MI.getParent();
  DebugLoc DL = MBB.findDebugLoc(MI);
  switch (MI.getOpcode()) {
  default: return TargetInstrInfo::expandPostRAPseudo(MI);
  case RVGPU::S_MOV_B64_term:
    // This is only a terminator to get the correct spill code placement during
    // register allocation.
    MI.setDesc(get(RVGPU::S_MOV_B64));
    break;

  case RVGPU::S_MOV_B32_term:
    // This is only a terminator to get the correct spill code placement during
    // register allocation.
    MI.setDesc(get(RVGPU::S_MOV_B32));
    break;

  case RVGPU::S_XOR_B64_term:
    // This is only a terminator to get the correct spill code placement during
    // register allocation.
    MI.setDesc(get(RVGPU::S_XOR_B64));
    break;

  case RVGPU::S_XOR_B32_term:
    // This is only a terminator to get the correct spill code placement during
    // register allocation.
    MI.setDesc(get(RVGPU::S_XOR_B32));
    break;
  case RVGPU::S_OR_B64_term:
    // This is only a terminator to get the correct spill code placement during
    // register allocation.
    MI.setDesc(get(RVGPU::S_OR_B64));
    break;
  case RVGPU::S_OR_B32_term:
    // This is only a terminator to get the correct spill code placement during
    // register allocation.
    MI.setDesc(get(RVGPU::S_OR_B32));
    break;

  case RVGPU::S_ANDN2_B64_term:
    // This is only a terminator to get the correct spill code placement during
    // register allocation.
    MI.setDesc(get(RVGPU::S_ANDN2_B64));
    break;

  case RVGPU::S_ANDN2_B32_term:
    // This is only a terminator to get the correct spill code placement during
    // register allocation.
    MI.setDesc(get(RVGPU::S_ANDN2_B32));
    break;

  case RVGPU::S_AND_B64_term:
    // This is only a terminator to get the correct spill code placement during
    // register allocation.
    MI.setDesc(get(RVGPU::S_AND_B64));
    break;

  case RVGPU::S_AND_B32_term:
    // This is only a terminator to get the correct spill code placement during
    // register allocation.
    MI.setDesc(get(RVGPU::S_AND_B32));
    break;

  case RVGPU::S_AND_SAVEEXEC_B64_term:
    // This is only a terminator to get the correct spill code placement during
    // register allocation.
    MI.setDesc(get(RVGPU::S_AND_SAVEEXEC_B64));
    break;

  case RVGPU::S_AND_SAVEEXEC_B32_term:
    // This is only a terminator to get the correct spill code placement during
    // register allocation.
    MI.setDesc(get(RVGPU::S_AND_SAVEEXEC_B32));
    break;

  case RVGPU::SI_SPILL_S32_TO_VGPR:
    MI.setDesc(get(RVGPU::V_WRITELANE_B32));
    break;

  case RVGPU::SI_RESTORE_S32_FROM_VGPR:
    MI.setDesc(get(RVGPU::V_READLANE_B32));
    break;

  case RVGPU::V_MOV_B64_PSEUDO: {
    Register Dst = MI.getOperand(0).getReg();
    Register DstLo = RI.getSubReg(Dst, RVGPU::sub0);
    Register DstHi = RI.getSubReg(Dst, RVGPU::sub1);

    const MachineOperand &SrcOp = MI.getOperand(1);
    // FIXME: Will this work for 64-bit floating point immediates?
    assert(!SrcOp.isFPImm());
    if (ST.hasMovB64()) {
      MI.setDesc(get(RVGPU::V_MOV_B64_e32));
    ...
    if (SrcOp.isImm()) {
      ...
      if (ST.hasPkMovB32() && Lo == Hi && isInlineConstant(Lo)) {
        BuildMI(MBB, MI, DL, get(RVGPU::V_PK_MOV_B32), Dst)
          .addImm(SISrcMods::OP_SEL_1)
        ...
        BuildMI(MBB, MI, DL, get(RVGPU::V_MOV_B32_e32), DstLo)
          .addImm(Lo.getSExtValue())
          .addReg(Dst, RegState::Implicit | RegState::Define);
        BuildMI(MBB, MI, DL, get(RVGPU::V_MOV_B32_e32), DstHi)
      ...
      if (ST.hasPkMovB32() &&
        ...
        BuildMI(MBB, MI, DL, get(RVGPU::V_MOV_B32_e32), DstLo)
          .addReg(RI.getSubReg(SrcOp.getReg(), RVGPU::sub0))
          .addReg(Dst, RegState::Implicit | RegState::Define);
        BuildMI(MBB, MI, DL, get(RVGPU::V_MOV_B32_e32), DstHi)
          .addReg(RI.getSubReg(SrcOp.getReg(), RVGPU::sub1))
          .addReg(Dst, RegState::Implicit | RegState::Define);
      }
    }
    MI.eraseFromParent();
  ...
  case RVGPU::V_MOV_B64_DPP_PSEUDO: {
    expandMovDPP64(MI);
    break;
  }
  case RVGPU::S_MOV_B64_IMM_PSEUDO: {
    const MachineOperand &SrcOp = MI.getOperand(1);
    assert(!SrcOp.isFPImm());
    APInt Imm(64, SrcOp.getImm());
    ...
    APInt Lo(32, Imm.getLoBits(32).getZExtValue());
    APInt Hi(32, Imm.getHiBits(32).getZExtValue());
    BuildMI(MBB, MI, DL, get(RVGPU::S_MOV_B32), DstLo)
      .addImm(Lo.getSExtValue())
      .addReg(Dst, RegState::Implicit | RegState::Define);
    BuildMI(MBB, MI, DL, get(RVGPU::S_MOV_B32), DstHi)
      .addImm(Hi.getSExtValue())
      .addReg(Dst, RegState::Implicit | RegState::Define);
    MI.eraseFromParent();
    break;
  }
  case RVGPU::V_SET_INACTIVE_B32: {
    unsigned NotOpc = ST.isWave32() ? RVGPU::S_NOT_B32 : RVGPU::S_NOT_B64;
    ...
    BuildMI(MBB, MI, DL, get(RVGPU::V_MOV_B32_e32), MI.getOperand(0).getReg())
        .add(MI.getOperand(1));
    auto FirstNot = BuildMI(MBB, MI, DL, get(NotOpc), Exec).addReg(Exec);
    FirstNot->addRegisterDead(RVGPU::SCC, TRI); // SCC is overwritten
    BuildMI(MBB, MI, DL, get(RVGPU::V_MOV_B32_e32), MI.getOperand(0).getReg())
      .add(MI.getOperand(2));
  ...
  case RVGPU::V_SET_INACTIVE_B64: {
    unsigned NotOpc = ST.isWave32() ? RVGPU::S_NOT_B32 : RVGPU::S_NOT_B64;
    unsigned Exec = ST.isWave32() ? RVGPU::EXEC_LO : RVGPU::EXEC;
    MachineInstr *Copy = BuildMI(MBB, MI, DL, get(RVGPU::V_MOV_B64_PSEUDO),
                                 MI.getOperand(0).getReg())
                             .add(MI.getOperand(1));
    expandPostRAPseudo(*Copy);
    auto FirstNot = BuildMI(MBB, MI, DL, get(NotOpc), Exec).addReg(Exec);
    FirstNot->addRegisterDead(RVGPU::SCC, TRI); // SCC is overwritten
    Copy = BuildMI(MBB, MI, DL, get(RVGPU::V_MOV_B64_PSEUDO),
                   MI.getOperand(0).getReg())
               .add(MI.getOperand(2));
    expandPostRAPseudo(*Copy);
    BuildMI(MBB, MI, DL, get(NotOpc), Exec)
  ...
  case RVGPU::V_INDIRECT_REG_WRITE_MOVREL_B32_V1:
  case RVGPU::V_INDIRECT_REG_WRITE_MOVREL_B32_V2:
  case RVGPU::V_INDIRECT_REG_WRITE_MOVREL_B32_V3:
  case RVGPU::V_INDIRECT_REG_WRITE_MOVREL_B32_V4:
  case RVGPU::V_INDIRECT_REG_WRITE_MOVREL_B32_V5:
  case RVGPU::V_INDIRECT_REG_WRITE_MOVREL_B32_V8:
  case RVGPU::V_INDIRECT_REG_WRITE_MOVREL_B32_V9:
  case RVGPU::V_INDIRECT_REG_WRITE_MOVREL_B32_V10:
  case RVGPU::V_INDIRECT_REG_WRITE_MOVREL_B32_V11:
  case RVGPU::V_INDIRECT_REG_WRITE_MOVREL_B32_V12:
  case RVGPU::V_INDIRECT_REG_WRITE_MOVREL_B32_V16:
  case RVGPU::V_INDIRECT_REG_WRITE_MOVREL_B32_V32:
  case RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B32_V1:
  case RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B32_V2:
  case RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B32_V3:
  case RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B32_V4:
  case RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B32_V5:
  case RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B32_V8:
  case RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B32_V9:
  case RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B32_V10:
  case RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B32_V11:
  case RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B32_V12:
  case RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B32_V16:
  case RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B32_V32:
  case RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B64_V1:
  case RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B64_V2:
  case RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B64_V4:
  case RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B64_V8:
  case RVGPU::S_INDIRECT_REG_WRITE_MOVREL_B64_V16: {
    const TargetRegisterClass *EltRC = getOpRegClass(MI, 2);

    unsigned Opc;
    if (RI.hasVGPRs(EltRC)) {
      Opc = RVGPU::V_MOVRELD_B32_e32;
    } else {
      Opc = RI.getRegSizeInBits(*EltRC) == 64 ? RVGPU::S_MOVRELD_B64
                                              : RVGPU::S_MOVRELD_B32;
    }
  ...
  case RVGPU::V_INDIRECT_REG_WRITE_GPR_IDX_B32_V1:
  case RVGPU::V_INDIRECT_REG_WRITE_GPR_IDX_B32_V2:
  case RVGPU::V_INDIRECT_REG_WRITE_GPR_IDX_B32_V3:
  case RVGPU::V_INDIRECT_REG_WRITE_GPR_IDX_B32_V4:
  case RVGPU::V_INDIRECT_REG_WRITE_GPR_IDX_B32_V5:
  case RVGPU::V_INDIRECT_REG_WRITE_GPR_IDX_B32_V8:
  case RVGPU::V_INDIRECT_REG_WRITE_GPR_IDX_B32_V9:
  case RVGPU::V_INDIRECT_REG_WRITE_GPR_IDX_B32_V10:
  case RVGPU::V_INDIRECT_REG_WRITE_GPR_IDX_B32_V11:
  case RVGPU::V_INDIRECT_REG_WRITE_GPR_IDX_B32_V12:
  case RVGPU::V_INDIRECT_REG_WRITE_GPR_IDX_B32_V16:
  case RVGPU::V_INDIRECT_REG_WRITE_GPR_IDX_B32_V32: {
    ...
    MachineInstr *SetOn = BuildMI(MBB, MI, DL, get(RVGPU::S_SET_GPR_IDX_ON))
                              .addReg(Idx)
                              .addImm(RVGPU::VGPRIndexMode::DST_ENABLE);
    SetOn->getOperand(3).setIsUndef();

    const MCInstrDesc &OpDesc = get(RVGPU::V_MOV_B32_indirect_write);
    MachineInstrBuilder MIB =
        BuildMI(MBB, MI, DL, OpDesc)
            .addReg(RI.getSubReg(VecReg, SubReg), RegState::Undef)
            .add(MI.getOperand(2))
    ...
    MachineInstr *SetOff = BuildMI(MBB, MI, DL, get(RVGPU::S_SET_GPR_IDX_OFF));
  ...
  case RVGPU::V_INDIRECT_REG_READ_GPR_IDX_B32_V1:
  case RVGPU::V_INDIRECT_REG_READ_GPR_IDX_B32_V2:
  case RVGPU::V_INDIRECT_REG_READ_GPR_IDX_B32_V3:
  case RVGPU::V_INDIRECT_REG_READ_GPR_IDX_B32_V4:
  case RVGPU::V_INDIRECT_REG_READ_GPR_IDX_B32_V5:
  case RVGPU::V_INDIRECT_REG_READ_GPR_IDX_B32_V8:
  case RVGPU::V_INDIRECT_REG_READ_GPR_IDX_B32_V9:
  case RVGPU::V_INDIRECT_REG_READ_GPR_IDX_B32_V10:
  case RVGPU::V_INDIRECT_REG_READ_GPR_IDX_B32_V11:
  case RVGPU::V_INDIRECT_REG_READ_GPR_IDX_B32_V12:
  case RVGPU::V_INDIRECT_REG_READ_GPR_IDX_B32_V16:
  case RVGPU::V_INDIRECT_REG_READ_GPR_IDX_B32_V32: {
    assert(ST.useVGPRIndexMode());
    Register Dst = MI.getOperand(0).getReg();
    ...
    MachineInstr *SetOn = BuildMI(MBB, MI, DL, get(RVGPU::S_SET_GPR_IDX_ON))
                              .addReg(Idx)
                              .addImm(RVGPU::VGPRIndexMode::SRC0_ENABLE);
    SetOn->getOperand(3).setIsUndef();

    BuildMI(MBB, MI, DL, get(RVGPU::V_MOV_B32_indirect_read))
        .addDef(Dst)
        .addReg(RI.getSubReg(VecReg, SubReg), RegState::Undef)
        .addReg(VecReg, RegState::Implicit | (IsUndef ? RegState::Undef : 0));
  ...
  case RVGPU::SI_PC_ADD_REL_OFFSET: {
    MachineFunction &MF = *MBB.getParent();
    Register Reg = MI.getOperand(0).getReg();
    Register RegLo = RI.getSubReg(Reg, RVGPU::sub0);
    Register RegHi = RI.getSubReg(Reg, RVGPU::sub1);
    ...
    Bundler.append(BuildMI(MF, DL, get(RVGPU::S_GETPC_B64), Reg));
    ...
    Bundler.append(
        BuildMI(MF, DL, get(RVGPU::S_ADD_U32), RegLo).addReg(RegLo).add(OpLo));

    if (OpHi.isGlobal())
      OpHi.setOffset(OpHi.getOffset() + 12);
    Bundler.append(BuildMI(MF, DL, get(RVGPU::S_ADDC_U32), RegHi)
                       .addReg(RegHi)
                       .add(OpHi));
  ...
  case RVGPU::ENTER_STRICT_WWM: {
    // This only gets its own opcode so that SIPreAllocateWWMRegs can tell when
    // Whole Wave Mode is entered.
    MI.setDesc(get(ST.isWave32() ? RVGPU::S_OR_SAVEEXEC_B32
                                 : RVGPU::S_OR_SAVEEXEC_B64));
    break;
  }
  case RVGPU::ENTER_STRICT_WQM: {
    // This only gets its own opcode so that SIPreAllocateWWMRegs can tell when
    // STRICT_WQM is entered.
    const unsigned Exec = ST.isWave32() ? RVGPU::EXEC_LO : RVGPU::EXEC;
    const unsigned WQMOp = ST.isWave32() ? RVGPU::S_WQM_B32 : RVGPU::S_WQM_B64;
    const unsigned MovOp = ST.isWave32() ? RVGPU::S_MOV_B32 : RVGPU::S_MOV_B64;
  ...
  case RVGPU::EXIT_STRICT_WWM:
  case RVGPU::EXIT_STRICT_WQM: {
    // This only gets its own opcode so that SIPreAllocateWWMRegs can tell when
    // WWM/STICT_WQM is exited.
    MI.setDesc(get(ST.isWave32() ? RVGPU::S_MOV_B32 : RVGPU::S_MOV_B64));
    break;
  }
  case RVGPU::ENTER_PSEUDO_WM:
  case RVGPU::EXIT_PSEUDO_WM: {
    // These do nothing.
    MI.eraseFromParent();
    break;
  }
  case RVGPU::SI_RETURN: {
    ...
    // liveness checks.
    MachineInstrBuilder MIB =
        BuildMI(MBB, MI, DL, get(RVGPU::S_SETPC_B64_return))
...
void RVInstrInfo::reMaterialize(MachineBasicBlock &MBB,
  ...
  switch (Opcode) {
  case RVGPU::S_LOAD_DWORDX16_IMM:
  case RVGPU::S_LOAD_DWORDX8_IMM: {
    if (SubIdx != 0)
      break;
    ...
    if (!UseMO || UseMO->getSubReg() == RVGPU::NoSubRegister)
      break;
    ...
    if (SubregSize == 256)
      NewOpcode = RVGPU::S_LOAD_DWORDX8_IMM;
    else if (SubregSize == 128)
      NewOpcode = RVGPU::S_LOAD_DWORDX4_IMM;
    else
      break;
    ...
    UseMO->setSubReg(RVGPU::NoSubRegister);

    // Use a smaller load with the desired size, possibly with updated offset.
    MachineInstr *MI = MF->CloneMachineInstr(&Orig);
    MI->setDesc(TID);
    MI->getOperand(0).setReg(DestReg);
    MI->getOperand(0).setSubReg(RVGPU::NoSubRegister);
    if (Offset) {
      MachineOperand *OffsetMO = getNamedOperand(*MI, RVGPU::OpName::offset);
...
RVInstrInfo::expandMovDPP64(MachineInstr &MI) const {
  assert (MI.getOpcode() == RVGPU::V_MOV_B64_DPP_PSEUDO);

  if (ST.hasMovB64() &&
      RVGPU::isLegalDPALU_DPPControl(
        getNamedOperand(MI, RVGPU::OpName::dpp_ctrl)->getImm())) {
    MI.setDesc(get(RVGPU::V_MOV_B64_dpp));
  ...
  for (auto Sub : { RVGPU::sub0, RVGPU::sub1 }) {
    auto MovDPP = BuildMI(MBB, MI, DL, get(RVGPU::V_MOV_B32_dpp));
    if (Dst.isPhysical()) {
      MovDPP.addDef(RI.getSubReg(Dst, Sub));
    } else {
      assert(MRI.isSSA());
      auto Tmp = MRI.createVirtualRegister(&RVGPU::VGPR_32RegClass);
      MovDPP.addDef(Tmp);
    }
  ...
  if (Dst.isVirtual())
    BuildMI(MBB, MI, DL, get(RVGPU::REG_SEQUENCE), Dst)
...
RVInstrInfo::isCopyInstrImpl(const MachineInstr &MI) const {
  if (MI.getOpcode() == RVGPU::WWM_COPY)
    return DestSourcePair{MI.getOperand(0), MI.getOperand(1)};

  return std::nullopt;
}

bool RVInstrInfo::swapSourceModifiers(MachineInstr &MI,
                                      MachineOperand &Src0,
                                      unsigned Src0OpName,
                                      MachineOperand &Src1,
...
MachineInstr *RVInstrInfo::commuteInstructionImpl(MachineInstr &MI, bool NewMI,
                                                  unsigned Src0Idx,
  ...
  assert(RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::src0) ==
           static_cast<int>(Src0Idx) &&
         RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::src1) ==
  ...
  if (CommutedMI) {
    swapSourceModifiers(MI, Src0, RVGPU::OpName::src0_modifiers,
                        Src1, RVGPU::OpName::src1_modifiers);
...
bool RVInstrInfo::findCommutedOpIndices(const MachineInstr &MI,
                                        unsigned &SrcOpIdx0,
                                        unsigned &SrcOpIdx1) const {
  return findCommutedOpIndices(MI.getDesc(), SrcOpIdx0, SrcOpIdx1);
}

bool RVInstrInfo::findCommutedOpIndices(const MCInstrDesc &Desc,
                                        unsigned &SrcOpIdx0,
                                        unsigned &SrcOpIdx1) const {
  ...
  int Src0Idx = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::src0);
  if (Src0Idx == -1)
    return false;

  int Src1Idx = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::src1);
  if (Src1Idx == -1)
    return false;
...
MachineBasicBlock *
RVInstrInfo::getBranchDestBlock(const MachineInstr &MI) const {
  return MI.getOperand(0).getMBB();
}

bool RVInstrInfo::hasDivergentBranch(const MachineBasicBlock *MBB) const {
  for (const MachineInstr &MI : MBB->terminators()) {
    if (MI.getOpcode() == RVGPU::SI_NON_UNIFORM_BRCOND_PSEUDO ||
        MI.getOpcode() == RVGPU::SI_IF || MI.getOpcode() == RVGPU::SI_ELSE ||
        MI.getOpcode() == RVGPU::SI_LOOP)
      return true;
  }
  return false;
}

void RVInstrInfo::insertIndirectBranch(MachineBasicBlock &MBB,
  ...
  // blocks.
  Register PCReg = MRI.createVirtualRegister(&RVGPU::SReg_64RegClass);
  ...
  MachineInstr *GetPC = BuildMI(MBB, I, DL, get(RVGPU::S_GETPC_B64), PCReg);
  ...
  BuildMI(MBB, I, DL, get(RVGPU::S_ADD_U32))
      .addReg(PCReg, RegState::Define, RVGPU::sub0)
      .addReg(PCReg, 0, RVGPU::sub0)
      .addSym(OffsetLo, MO_FAR_BRANCH_OFFSET);
  BuildMI(MBB, I, DL, get(RVGPU::S_ADDC_U32))
      .addReg(PCReg, RegState::Define, RVGPU::sub1)
      .addReg(PCReg, 0, RVGPU::sub1)
      .addSym(OffsetHi, MO_FAR_BRANCH_OFFSET);

  // Insert the indirect branch after the other terminator.
  BuildMI(&MBB, DL, get(RVGPU::S_SETPC_B64))
    .addReg(PCReg);
  ...
  if (LongBranchReservedReg) {
    ...
    Scav = RS->scavengeRegisterBackwards(
        RVGPU::SReg_64RegClass, MachineBasicBlock::iterator(GetPC),
...
unsigned RVInstrInfo::getBranchOpcode(RVInstrInfo::BranchPredicate Cond) {
  switch (Cond) {
  case RVInstrInfo::SCC_TRUE:
    return RVGPU::S_CBRANCH_SCC1;
  case RVInstrInfo::SCC_FALSE:
    return RVGPU::S_CBRANCH_SCC0;
  case RVInstrInfo::VCCNZ:
    return RVGPU::S_CBRANCH_VCCNZ;
  case RVInstrInfo::VCCZ:
    return RVGPU::S_CBRANCH_VCCZ;
  case RVInstrInfo::EXECNZ:
    return RVGPU::S_CBRANCH_EXECNZ;
  case RVInstrInfo::EXECZ:
    return RVGPU::S_CBRANCH_EXECZ;
  default:
    llvm_unreachable("invalid branch predicate");
  }
}

RVInstrInfo::BranchPredicate RVInstrInfo::getBranchPredicate(unsigned Opcode) {
  switch (Opcode) {
  case RVGPU::S_CBRANCH_SCC0:
    return SCC_FALSE;
  case RVGPU::S_CBRANCH_SCC1:
    return SCC_TRUE;
  case RVGPU::S_CBRANCH_VCCNZ:
    return VCCNZ;
  case RVGPU::S_CBRANCH_VCCZ:
    return VCCZ;
  case RVGPU::S_CBRANCH_EXECNZ:
    return EXECNZ;
  case RVGPU::S_CBRANCH_EXECZ:
    return EXECZ;
  default:
    return INVALID_BR;
  }
...
bool RVInstrInfo::analyzeBranchImpl(MachineBasicBlock &MBB,
                                    MachineBasicBlock::iterator I,
                                    MachineBasicBlock *&TBB,
                                    MachineBasicBlock *&FBB,
                                    SmallVectorImpl<MachineOperand> &Cond,
                                    bool AllowModify) const {
  if (I->getOpcode() == RVGPU::S_BRANCH) {
  ...
  if (I->getOpcode() == RVGPU::SI_NON_UNIFORM_BRCOND_PSEUDO) {
...
bool RVInstrInfo::analyzeBranch(MachineBasicBlock &MBB, MachineBasicBlock *&TBB,
                                MachineBasicBlock *&FBB,
                                SmallVectorImpl<MachineOperand> &Cond,
                                bool AllowModify) const {
  ...
  while (I != E && !I->isBranch() && !I->isReturn()) {
    ...
    case RVGPU::SI_IF:
    case RVGPU::SI_ELSE:
    case RVGPU::SI_KILL_I1_TERMINATOR:
    case RVGPU::SI_KILL_F32_COND_IMM_TERMINATOR:
...
unsigned RVInstrInfo::removeBranch(MachineBasicBlock &MBB,
...
unsigned RVInstrInfo::insertBranch(MachineBasicBlock &MBB,
                                   MachineBasicBlock *TBB,
                                   MachineBasicBlock *FBB,
                                   ArrayRef<MachineOperand> Cond,
                                   const DebugLoc &DL,
  ...
  if (!FBB && Cond.empty()) {
    BuildMI(&MBB, DL, get(RVGPU::S_BRANCH))
      .addMBB(TBB);
    if (BytesAdded)
  ...
  if(Cond.size() == 1 && Cond[0].isReg()) {
     BuildMI(&MBB, DL, get(RVGPU::SI_NON_UNIFORM_BRCOND_PSEUDO))
       .add(Cond[0])
       .addMBB(TBB);
     return 1;
  }
...
bool RVInstrInfo::reverseBranchCondition(
...
bool RVInstrInfo::canInsertSelect(const MachineBasicBlock &MBB,
                                  ArrayRef<MachineOperand> Cond,
  ...
  case VCCZ: {
    ...
    int NumInsts = RVGPU::getRegBitWidth(*RC) / 32;
...
void RVInstrInfo::insertSelect(MachineBasicBlock &MBB,
  ...
  if (DstSize == 32) {
    MachineInstr *Select;
    if (Pred == SCC_TRUE) {
      Select = BuildMI(MBB, I, DL, get(RVGPU::S_CSELECT_B32), DstReg)
        .addReg(TrueReg)
        .addReg(FalseReg);
    } else {
      // Instruction's operands are backwards from what is expected.
      Select = BuildMI(MBB, I, DL, get(RVGPU::V_CNDMASK_B32_e32), DstReg)
        .addReg(FalseReg)
        .addReg(TrueReg);
    }
  ...
  if (DstSize == 64 && Pred == SCC_TRUE) {
    MachineInstr *Select =
      BuildMI(MBB, I, DL, get(RVGPU::S_CSELECT_B64), DstReg)
      .addReg(TrueReg)
      .addReg(FalseReg);
  ...
  static const int16_t Sub0_15[] = {
    RVGPU::sub0, RVGPU::sub1, RVGPU::sub2, RVGPU::sub3,
    RVGPU::sub4, RVGPU::sub5, RVGPU::sub6, RVGPU::sub7,
    RVGPU::sub8, RVGPU::sub9, RVGPU::sub10, RVGPU::sub11,
    RVGPU::sub12, RVGPU::sub13, RVGPU::sub14, RVGPU::sub15,
  };

  static const int16_t Sub0_15_64[] = {
    RVGPU::sub0_sub1, RVGPU::sub2_sub3,
    RVGPU::sub4_sub5, RVGPU::sub6_sub7,
    RVGPU::sub8_sub9, RVGPU::sub10_sub11,
    RVGPU::sub12_sub13, RVGPU::sub14_sub15,
  };

  unsigned SelOp = RVGPU::V_CNDMASK_B32_e32;
  const TargetRegisterClass *EltRC = &RVGPU::VGPR_32RegClass;
  ...
  if (Pred == SCC_TRUE) {
    if (NElts % 2) {
      SelOp = RVGPU::S_CSELECT_B32;
      EltRC = &RVGPU::SGPR_32RegClass;
    } else {
      SelOp = RVGPU::S_CSELECT_B64;
      EltRC = &RVGPU::SGPR_64RegClass;
  ...
  MachineInstrBuilder MIB = BuildMI(
    MBB, I, DL, get(RVGPU::REG_SEQUENCE), DstReg);
  ...
  for (int Idx = 0; Idx != NElts; ++Idx) {
    ...
    MachineInstr *Select;
    if (SelOp == RVGPU::V_CNDMASK_B32_e32) {
      Select =
...
bool RVInstrInfo::isFoldableCopy(const MachineInstr &MI) {
  switch (MI.getOpcode()) {
  case RVGPU::V_MOV_B32_e32:
  case RVGPU::V_MOV_B32_e64:
  case RVGPU::V_MOV_B64_PSEUDO:
  case RVGPU::V_MOV_B64_e32:
  case RVGPU::V_MOV_B64_e64:
  case RVGPU::S_MOV_B32:
  case RVGPU::S_MOV_B64:
  case RVGPU::S_MOV_B64_IMM_PSEUDO:
  case RVGPU::COPY:
  case RVGPU::WWM_COPY:
  case RVGPU::V_ACCVGPR_WRITE_B32_e64:
  case RVGPU::V_ACCVGPR_READ_B32_e64:
  case RVGPU::V_ACCVGPR_MOV_B32:
    return true;
  default:
    return false;
  }
...
static constexpr unsigned ModifierOpNames[] = {
    RVGPU::OpName::src0_modifiers, RVGPU::OpName::src1_modifiers,
    RVGPU::OpName::src2_modifiers, RVGPU::OpName::clamp,
    RVGPU::OpName::omod,           RVGPU::OpName::op_sel};

void RVInstrInfo::removeModOperands(MachineInstr &MI) const {
  unsigned Opc = MI.getOpcode();
  for (unsigned Name : reverse(ModifierOpNames)) {
    int Idx = RVGPU::getNamedOperandIdx(Opc, Name);
    if (Idx >= 0)
      MI.removeOperand(Idx);
  }
}

bool RVInstrInfo::FoldImmediate(MachineInstr &UseMI, MachineInstr &DefMI,
  ...
  const MachineOperand *ImmOp = getNamedOperand(DefMI, RVGPU::OpName::src0);
  assert(ImmOp);
  ...
  auto getImmFor = [ImmOp](const MachineOperand &UseOp) -> int64_t {
    ...
    case RVGPU::sub1_lo16:
      return APInt(16, Hi_32(Imm)).getSExtValue();
    case RVGPU::sub1_hi16:
  ...
  if (Opc == RVGPU::COPY) {
    ...
    unsigned NewOpc = isVGPRCopy ? Is64Bit ? RVGPU::V_MOV_B64_PSEUDO
                                           : RVGPU::V_MOV_B32_e32
                                 : Is64Bit ? RVGPU::S_MOV_B64_IMM_PSEUDO
                                           : RVGPU::S_MOV_B32;
    ...
    if (RI.isAGPR(*MRI, DstReg)) {
      if (Is64Bit || !isInlineConstant(Imm))
        return false;
      NewOpc = RVGPU::V_ACCVGPR_WRITE_B32_e64;
    }
    ...
    if (Is16Bit) {
      ...
      if (DstReg.isVirtual() && UseMI.getOperand(0).getSubReg() != RVGPU::lo16)
        return false;
  ...
  if (Opc == RVGPU::V_MAD_F32_e64 || Opc == RVGPU::V_MAC_F32_e64 ||
      Opc == RVGPU::V_MAD_F16_e64 || Opc == RVGPU::V_MAC_F16_e64 ||
      Opc == RVGPU::V_FMA_F32_e64 || Opc == RVGPU::V_FMAC_F32_e64 ||
      Opc == RVGPU::V_FMA_F16_e64 || Opc == RVGPU::V_FMAC_F16_e64 ||
      Opc == RVGPU::V_FMAC_F16_t16_e64) {
    ...
    // later.
    MachineOperand *Src0 = getNamedOperand(UseMI, RVGPU::OpName::src0);

    // Any src operand can be used for the legality check.
    if (isInlineConstant(UseMI, *Src0, *ImmOp))
      return false;

    bool IsF32 = Opc == RVGPU::V_MAD_F32_e64 || Opc == RVGPU::V_MAC_F32_e64 ||
                 Opc == RVGPU::V_FMA_F32_e64 || Opc == RVGPU::V_FMAC_F32_e64;
    bool IsFMA =
        Opc == RVGPU::V_FMA_F32_e64 || Opc == RVGPU::V_FMAC_F32_e64 ||
        Opc == RVGPU::V_FMA_F16_e64 || Opc == RVGPU::V_FMAC_F16_e64 ||
        Opc == RVGPU::V_FMAC_F16_t16_e64;
    MachineOperand *Src1 = getNamedOperand(UseMI, RVGPU::OpName::src1);
    MachineOperand *Src2 = getNamedOperand(UseMI, RVGPU::OpName::src2);
    ...
    if ((Src0->isReg() && Src0->getReg() == Reg) ||
      ...
      unsigned NewOpc =
          IsFMA ? (IsF32                    ? RVGPU::V_FMAMK_F32
                   : ST.hasTrue16BitInsts() ? RVGPU::V_FMAMK_F16_t16
                                            : RVGPU::V_FMAMK_F16)
                : (IsF32 ? RVGPU::V_MADMK_F32 : RVGPU::V_MADMK_F16);
      ...
      // just bail out.
      if (NewOpc == RVGPU::V_FMAMK_F16_t16)
        return false;
      ...
      if (Opc == RVGPU::V_MAC_F32_e64 || Opc == RVGPU::V_MAC_F16_e64 ||
          Opc == RVGPU::V_FMAC_F32_e64 || Opc == RVGPU::V_FMAC_F16_t16_e64 ||
          Opc == RVGPU::V_FMAC_F16_e64)
        UseMI.untieRegOperand(
            RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::src2));
    ...
    if (Src2->isReg() && Src2->getReg() == Reg) {
      ...
      unsigned NewOpc =
          IsFMA ? (IsF32                    ? RVGPU::V_FMAAK_F32
                   : ST.hasTrue16BitInsts() ? RVGPU::V_FMAAK_F16_t16
                                            : RVGPU::V_FMAAK_F16)
                : (IsF32 ? RVGPU::V_MADAK_F32 : RVGPU::V_MADAK_F16);
      if (pseudoToMCOpcode(NewOpc) == -1)
        return false;
      ...
      // would also require restricting their register classes. For now
      // just bail out.
      if (NewOpc == RVGPU::V_FMAAK_F16_t16)
        return false;
...
bool RVInstrInfo::checkInstOffsetsDoNotOverlap(const MachineInstr &MIa,
...
bool RVInstrInfo::areMemAccessesTriviallyDisjoint(const MachineInstr &MIa,
                                                  const MachineInstr &MIb) const {
...
static bool getFoldableImm(Register Reg, const MachineRegisterInfo &MRI,
  ...
  if (Def && RVInstrInfo::isFoldableCopy(*Def) && Def->getOperand(1).isImm()) {
...
MachineInstr *RVInstrInfo::convertToThreeAddress(MachineInstr &MI,
                                                 LiveVariables *LV,
  ...
  // Handle MFMA.
  int NewMFMAOpc = RVGPU::getMFMAEarlyClobberOp(Opc);
  if (NewMFMAOpc != -1) {
    MachineInstrBuilder MIB =
  ...
  if (RVInstrInfo::isWMMA(MI)) {
    unsigned NewOpc = RVGPU::mapWMMA2AddrTo3AddrOpcode(MI.getOpcode());
  ...
  assert(Opc != RVGPU::V_FMAC_F16_t16_e32 &&
         "V_FMAC_F16_t16_e32 is not supported and not expected to be present "
         "pre-RA");

  // Handle MAC/FMAC.
  bool IsF16 = Opc == RVGPU::V_MAC_F16_e32 || Opc == RVGPU::V_MAC_F16_e64 ||
               Opc == RVGPU::V_FMAC_F16_e32 || Opc == RVGPU::V_FMAC_F16_e64 ||
               Opc == RVGPU::V_FMAC_F16_t16_e64;
  bool IsFMA = Opc == RVGPU::V_FMAC_F32_e32 || Opc == RVGPU::V_FMAC_F32_e64 ||
               Opc == RVGPU::V_FMAC_LEGACY_F32_e32 ||
               Opc == RVGPU::V_FMAC_LEGACY_F32_e64 ||
               Opc == RVGPU::V_FMAC_F16_e32 || Opc == RVGPU::V_FMAC_F16_e64 ||
               Opc == RVGPU::V_FMAC_F16_t16_e64 ||
               Opc == RVGPU::V_FMAC_F64_e32 || Opc == RVGPU::V_FMAC_F64_e64;
  bool IsF64 = Opc == RVGPU::V_FMAC_F64_e32 || Opc == RVGPU::V_FMAC_F64_e64;
  bool IsLegacy = Opc == RVGPU::V_MAC_LEGACY_F32_e32 ||
                  Opc == RVGPU::V_MAC_LEGACY_F32_e64 ||
                  Opc == RVGPU::V_FMAC_LEGACY_F32_e32 ||
                  Opc == RVGPU::V_FMAC_LEGACY_F32_e64;
  ...
  default:
    return nullptr;
  case RVGPU::V_MAC_F16_e64:
  case RVGPU::V_FMAC_F16_e64:
  case RVGPU::V_FMAC_F16_t16_e64:
  case RVGPU::V_MAC_F32_e64:
  case RVGPU::V_MAC_LEGACY_F32_e64:
  case RVGPU::V_FMAC_F32_e64:
  case RVGPU::V_FMAC_LEGACY_F32_e64:
  case RVGPU::V_FMAC_F64_e64:
    break;
  case RVGPU::V_MAC_F16_e32:
  case RVGPU::V_FMAC_F16_e32:
  case RVGPU::V_MAC_F32_e32:
  case RVGPU::V_MAC_LEGACY_F32_e32:
  case RVGPU::V_FMAC_F32_e32:
  case RVGPU::V_FMAC_LEGACY_F32_e32:
  case RVGPU::V_FMAC_F64_e32: {
    int Src0Idx = RVGPU::getNamedOperandIdx(MI.getOpcode(),
                                             RVGPU::OpName::src0);
  ...
  MachineInstrBuilder MIB;
  const MachineOperand *Dst = getNamedOperand(MI, RVGPU::OpName::vdst);
  const MachineOperand *Src0 = getNamedOperand(MI, RVGPU::OpName::src0);
  const MachineOperand *Src0Mods =
    getNamedOperand(MI, RVGPU::OpName::src0_modifiers);
  const MachineOperand *Src1 = getNamedOperand(MI, RVGPU::OpName::src1);
  const MachineOperand *Src1Mods =
    getNamedOperand(MI, RVGPU::OpName::src1_modifiers);
  const MachineOperand *Src2 = getNamedOperand(MI, RVGPU::OpName::src2);
  const MachineOperand *Src2Mods =
      getNamedOperand(MI, RVGPU::OpName::src2_modifiers);
  const MachineOperand *Clamp = getNamedOperand(MI, RVGPU::OpName::clamp);
  const MachineOperand *Omod = getNamedOperand(MI, RVGPU::OpName::omod);
  const MachineOperand *OpSel = getNamedOperand(MI, RVGPU::OpName::op_sel);
  ...
  if (!Src0Mods && !Src1Mods && !Src2Mods && !Clamp && !Omod && !IsF64 &&
    ...
    const auto killDef = [&]() -> void {
      ...
      DefMI->setDesc(get(RVGPU::IMPLICIT_DEF));
    ...
    if (!Src0Literal && getFoldableImm(Src2, Imm, &DefMI)) {
      unsigned NewOpc =
          IsFMA ? (IsF16 ? (ST.hasTrue16BitInsts() ? RVGPU::V_FMAAK_F16_t16
                                                   : RVGPU::V_FMAAK_F16)
                         : RVGPU::V_FMAAK_F32)
                : (IsF16 ? RVGPU::V_MADAK_F16 : RVGPU::V_MADAK_F32);
      if (pseudoToMCOpcode(NewOpc) != -1) {
    ...
    unsigned NewOpc =
        IsFMA ? (IsF16 ? (ST.hasTrue16BitInsts() ? RVGPU::V_FMAMK_F16_t16
                                                 : RVGPU::V_FMAMK_F16)
                       : RVGPU::V_FMAMK_F32)
              : (IsF16 ? RVGPU::V_MADMK_F16 : RVGPU::V_MADMK_F32);
    ...
    if (Src0Literal || getFoldableImm(Src0, Imm, &DefMI)) {
      ...
      if (pseudoToMCOpcode(NewOpc) != -1 &&
          isOperandLegal(
              MI, RVGPU::getNamedOperandIdx(NewOpc, RVGPU::OpName::src0),
              Src1)) {
        MIB = BuildMI(MBB, MI, MI.getDebugLoc(), get(NewOpc))
                  .add(*Dst)
                  .add(*Src1)
  ...
  unsigned NewOpc = IsFMA ? IsF16 ? RVGPU::V_FMA_F16_gfx9_e64
                                  : IsF64 ? RVGPU::V_FMA_F64_e64
                                          : IsLegacy
                                                ? RVGPU::V_FMA_LEGACY_F32_e64
                                                : RVGPU::V_FMA_F32_e64
                          : IsF16 ? RVGPU::V_MAD_F16_e64
                                  : IsLegacy ? RVGPU::V_MAD_LEGACY_F32_e64
                                             : RVGPU::V_MAD_F32_e64;
  if (pseudoToMCOpcode(NewOpc) == -1)
    return nullptr;
  ...
  if (RVGPU::hasNamedOperand(NewOpc, RVGPU::OpName::op_sel))
...
static bool changesVGPRIndexingMode(const MachineInstr &MI) {
  switch (MI.getOpcode()) {
  case RVGPU::S_SET_GPR_IDX_ON:
  case RVGPU::S_SET_GPR_IDX_MODE:
  case RVGPU::S_SET_GPR_IDX_OFF:
    return true;
  default:
    return false;
  }
...
bool RVInstrInfo::isSchedulingBoundary(const MachineInstr &MI,
  ...
  if (MI.getOpcode() == RVGPU::SCHED_BARRIER && MI.getOperand(0).getImm() == 0)
    return true;

  // Target-independent instructions do not have an implicit-use of EXEC, even
  // when they operate on VGPRs. Treating EXEC modifications as scheduling
  // boundaries prevents incorrect movements of such instructions.
  return MI.modifiesRegister(RVGPU::EXEC, &RI) ||
         MI.getOpcode() == RVGPU::S_SETREG_IMM32_B32 ||
         MI.getOpcode() == RVGPU::S_SETREG_B32 ||
         MI.getOpcode() == RVGPU::S_SETPRIO ||
         changesVGPRIndexingMode(MI);
}

bool RVInstrInfo::isAlwaysGDS(uint16_t Opcode) const {
  return Opcode == RVGPU::DS_ORDERED_COUNT || isGWS(Opcode);
}

bool RVInstrInfo::modifiesModeRegister(const MachineInstr &MI) {
  ...
  return is_contained(MI.getDesc().implicit_defs(), RVGPU::MODE);
}

bool RVInstrInfo::hasUnwantedEffectsWhenEXECEmpty(const MachineInstr &MI) const {
  ...
  if (Opcode == RVGPU::S_SENDMSG || Opcode == RVGPU::S_SENDMSGHALT ||
      isEXP(Opcode) ||
      Opcode == RVGPU::DS_ORDERED_COUNT || Opcode == RVGPU::S_TRAP ||
      Opcode == RVGPU::DS_GWS_INIT || Opcode == RVGPU::DS_GWS_BARRIER)
    return true;
  ...
  if (Opcode == RVGPU::V_READFIRSTLANE_B32 ||
      Opcode == RVGPU::V_READLANE_B32 || Opcode == RVGPU::V_WRITELANE_B32 ||
      Opcode == RVGPU::SI_RESTORE_S32_FROM_VGPR ||
      Opcode == RVGPU::SI_SPILL_S32_TO_VGPR)
    return true;

  return false;
}

bool RVInstrInfo::mayReadEXEC(const MachineRegisterInfo &MRI,
                              const MachineInstr &MI) const {
  if (MI.isMetaInstruction())
    return false;
  ...
  if (MI.isCopyLike()) {
    ...
    return MI.readsRegister(RVGPU::EXEC, &RI);
  }
  ...
  return !isSALU(MI) || MI.readsRegister(RVGPU::EXEC, &RI);
}

bool RVInstrInfo::isInlineConstant(const APInt &Imm) const {
  switch (Imm.getBitWidth()) {
  case 1: // This likely will be a condition code mask.
    return true;

  case 32:
    return RVGPU::isInlinableLiteral32(Imm.getSExtValue(),
                                        ST.hasInv2PiInlineImm());
  case 64:
    return RVGPU::isInlinableLiteral64(Imm.getSExtValue(),
                                        ST.hasInv2PiInlineImm());
  case 16:
    return ST.has16BitInsts() &&
           RVGPU::isInlinableLiteral16(Imm.getSExtValue(),
                                        ST.hasInv2PiInlineImm());
  default:
...
bool RVInstrInfo::isInlineConstant(const MachineOperand &MO,
  ...
  case RVGPU::OPERAND_INLINE_SPLIT_BARRIER_INT32: {
    ...
    return RVGPU::isInlinableLiteral32(Trunc, ST.hasInv2PiInlineImm());
  }
  ...
  case RVGPU::OPERAND_REG_INLINE_AC_FP64:
    return RVGPU::isInlinableLiteral64(MO.getImm(),
                                        ST.hasInv2PiInlineImm());
  ...
  case RVGPU::OPERAND_REG_INLINE_AC_V2FP16: {
    if (isInt<16>(Imm) || isUInt<16>(Imm)) {
      ...
      return ST.has16BitInsts() &&
             RVGPU::isInlinableLiteral16(Trunc, ST.hasInv2PiInlineImm());
    }
...
bool RVInstrInfo::isImmOperandLegal(const MachineInstr &MI, unsigned OpNo,
  ...
  if (MO.isImm() && isInlineConstant(MO, OpInfo)) {
    if (isMAI(MI) && ST.hasMFMAInlineLiteralBug() &&
        OpNo ==(unsigned)RVGPU::getNamedOperandIdx(MI.getOpcode(),
                                                    RVGPU::OpName::src2))
      return false;
  ...
  if (!isVOP3(MI) || !RVGPU::isSISrcOperand(InstDesc, OpNo))
    return true;

  return ST.hasVOP3Literal();
}

bool RVInstrInfo::hasVALU32BitEncoding(unsigned Opcode) const {
  // GFX90A does not have V_MUL_LEGACY_F32_e32.
  if (Opcode == RVGPU::V_MUL_LEGACY_F32_e64 && ST.hasGFX90AInsts())
    return false;

  int Op32 = RVGPU::getVOPe32(Opcode);
  if (Op32 == -1)
    return false;
...
bool RVInstrInfo::hasModifiers(unsigned Opcode) const {
  // The src0_modifier operand is present on all instructions
  // that have modifiers.

  return RVGPU::hasNamedOperand(Opcode, RVGPU::OpName::src0_modifiers);
}

bool RVInstrInfo::hasModifiersSet(const MachineInstr &MI,
                                  unsigned OpName) const {
  const MachineOperand *Mods = getNamedOperand(MI, OpName);
  return Mods && Mods->getImm();
}

bool RVInstrInfo::hasAnyModifiersSet(const MachineInstr &MI) const {
  return any_of(ModifierOpNames,
                [&](unsigned Name) { return hasModifiersSet(MI, Name); });
}

bool RVInstrInfo::canShrink(const MachineInstr &MI,
    ...
    switch (MI.getOpcode()) {
      ...
      case RVGPU::V_SUBBREV_U32_e64: {
        const MachineOperand *Src1
          = getNamedOperand(MI, RVGPU::OpName::src1);
      ...
      case RVGPU::V_FMAC_LEGACY_F32_e64:
        if (!Src2->isReg() || !RI.isVGPR(MRI, Src2->getReg()) ||
            hasModifiersSet(MI, RVGPU::OpName::src2_modifiers))
          return false;
        break;
  ...
  if (Src1 && (!Src1->isReg() || !RI.isVGPR(MRI, Src1->getReg()) ||
               hasModifiersSet(MI, RVGPU::OpName::src1_modifiers)))
    return false;

  // We don't need to check src0, all input types are legal, so just make sure
  // src0 isn't using any modifiers.
  if (hasModifiersSet(MI, RVGPU::OpName::src0_modifiers))
    return false;
  ...
  return !hasModifiersSet(MI, RVGPU::OpName::omod) &&
         !hasModifiersSet(MI, RVGPU::OpName::clamp);
}
...
static void copyFlagsToImplicitVCC(MachineInstr &MI,
  ...
  for (MachineOperand &Use : MI.implicit_operands()) {
    if (Use.isUse() &&
        (Use.getReg() == RVGPU::VCC || Use.getReg() == RVGPU::VCC_LO)) {
...
MachineInstr *RVInstrInfo::buildShrunkInst(MachineInstr &MI,
                                           unsigned Op32) const {
  MachineBasicBlock *MBB = MI.getParent();
  ...
  if (RVGPU::hasNamedOperand(Op32, RVGPU::OpName::vdst)) {
    // dst
    Inst32.add(MI.getOperand(0));
  } else if (RVGPU::hasNamedOperand(Op32, RVGPU::OpName::sdst)) {
    // VOPCX instructions won't be writing to an explicit dst, so this should
    // not fail for these instructions.
    assert(((MI.getOperand(0).getReg() == RVGPU::VCC) ||
            (MI.getOperand(0).getReg() == RVGPU::VCC_LO)) &&
           "Unexpected case");
  }

  Inst32.add(*getNamedOperand(MI, RVGPU::OpName::src0));
  ...
  if (Src2) {
    int Op32Src2Idx = RVGPU::getNamedOperandIdx(Op32, RVGPU::OpName::src2);
    if (Op32Src2Idx != -1) {
      Inst32.add(*Src2);
    } else {
...
bool RVInstrInfo::usesConstantBus(const MachineRegisterInfo &MRI,
  ...
  // Null is free
  if (MO.getReg() == RVGPU::SGPR_NULL || MO.getReg() == RVGPU::SGPR_NULL64)
    return false;

  // SGPRs use the constant bus
  if (MO.isImplicit()) {
    return MO.getReg() == RVGPU::M0 ||
           MO.getReg() == RVGPU::VCC ||
           MO.getReg() == RVGPU::VCC_LO;
  } else {
    return RVGPU::SReg_32RegClass.contains(MO.getReg()) ||
           RVGPU::SReg_64RegClass.contains(MO.getReg());
  }
}
...
static bool shouldReadExec(const MachineInstr &MI) {
  if (RVInstrInfo::isVALU(MI)) {
    switch (MI.getOpcode()) {
    case RVGPU::V_READLANE_B32:
    case RVGPU::SI_RESTORE_S32_FROM_VGPR:
    case RVGPU::V_WRITELANE_B32:
    case RVGPU::SI_SPILL_S32_TO_VGPR:
      return false;
    }
  ...
  if (MI.isPreISelOpcode() ||
      RVInstrInfo::isGenericOpcode(MI.getOpcode()) ||
      RVInstrInfo::isSALU(MI) ||
      RVInstrInfo::isSMRD(MI))
    return false;

  return true;
}

static bool isSubRegOf(const RVRegisterInfo &TRI,
                       const MachineOperand &SuperVec,
                       const MachineOperand &SubReg) {
  if (SubReg.getReg().isPhysical())
    return TRI.isSubRegister(SuperVec.getReg(), SubReg.getReg());

  return SubReg.getSubReg() != RVGPU::NoSubRegister &&
         SubReg.getReg() == SuperVec.getReg();
}

bool RVInstrInfo::verifyInstruction(const MachineInstr &MI,
                                    StringRef &ErrInfo) const {
  uint16_t Opcode = MI.getOpcode();
  if (RVInstrInfo::isGenericOpcode(MI.getOpcode()))
    return true;

  const MachineFunction *MF = MI.getParent()->getParent();
  const MachineRegisterInfo &MRI = MF->getRegInfo();

  int Src0Idx = RVGPU::getNamedOperandIdx(Opcode, RVGPU::OpName::src0);
  int Src1Idx = RVGPU::getNamedOperandIdx(Opcode, RVGPU::OpName::src1);
  int Src2Idx = RVGPU::getNamedOperandIdx(Opcode, RVGPU::OpName::src2);
  int Src3Idx = -1;
  if (Src0Idx == -1) {
    // VOPD V_DUAL_* instructions use different operand names.
    Src0Idx = RVGPU::getNamedOperandIdx(Opcode, RVGPU::OpName::src0X);
    Src1Idx = RVGPU::getNamedOperandIdx(Opcode, RVGPU::OpName::vsrc1X);
    Src2Idx = RVGPU::getNamedOperandIdx(Opcode, RVGPU::OpName::src0Y);
    Src3Idx = RVGPU::getNamedOperandIdx(Opcode, RVGPU::OpName::vsrc1Y);
  }
  ...
  if (isSDWA(MI)) {
    ...
    int DstIdx = RVGPU::getNamedOperandIdx(Opcode, RVGPU::OpName::vdst);
    ...
    const MachineOperand *DstUnused = getNamedOperand(MI, RVGPU::OpName::dst_unused);
    if (DstUnused && DstUnused->isImm() &&
        DstUnused->getImm() == RVGPU::SDWA::UNUSED_PRESERVE) {
      const MachineOperand &Dst = MI.getOperand(DstIdx);
  ...
  if (isImage(MI.getOpcode()) && !MI.mayStore()) {
    ...
    const MachineOperand *DMask = getNamedOperand(MI, RVGPU::OpName::dmask);
    if (DMask) {
      uint64_t DMaskImm = DMask->getImm();
      uint32_t RegCount =
          isGather4(MI.getOpcode()) ? 4 : llvm::popcount(DMaskImm);
      const MachineOperand *TFE = getNamedOperand(MI, RVGPU::OpName::tfe);
      const MachineOperand *LWE = getNamedOperand(MI, RVGPU::OpName::lwe);
      const MachineOperand *D16 = getNamedOperand(MI, RVGPU::OpName::d16);
      ...
      const uint32_t DstIdx =
          RVGPU::getNamedOperandIdx(MI.getOpcode(), RVGPU::OpName::vdata);
      const MachineOperand &Dst = MI.getOperand(DstIdx);
      if (Dst.isReg()) {
  ...
  if (isVALU(MI) && Desc.getOpcode() != RVGPU::V_WRITELANE_B32) {
    unsigned ConstantBusCount = 0;
    bool UsesLiteral = false;
    const MachineOperand *LiteralVal = nullptr;

    int ImmIdx = RVGPU::getNamedOperandIdx(Opcode, RVGPU::OpName::imm);
    if (ImmIdx != -1) {
      ++ConstantBusCount;
      UsesLiteral = true;
  ...
  if (Desc.getOpcode() == RVGPU::V_WRITELANE_B32) {
    ...
    for (int OpIdx : {Src0Idx, Src1Idx}) {
      ...
      if (usesConstantBus(MRI, MO, MI.getDesc().operands()[OpIdx])) {
        if (MO.isReg() && MO.getReg() != RVGPU::M0) {
  ...
  if (Desc.getOpcode() == RVGPU::V_DIV_SCALE_F32_e64 ||
      Desc.getOpcode() == RVGPU::V_DIV_SCALE_F64_e64) {
    const MachineOperand &Src0 = MI.getOperand(Src0Idx);
    ...
    if ((getNamedOperand(MI, RVGPU::OpName::src0_modifiers)->getImm() &
         SISrcMods::ABS) ||
        (getNamedOperand(MI, RVGPU::OpName::src1_modifiers)->getImm() &
         SISrcMods::ABS) ||
        (getNamedOperand(MI, RVGPU::OpName::src2_modifiers)->getImm() &
         SISrcMods::ABS)) {
  ...
  if (isSOPK(MI)) {
    auto Op = getNamedOperand(MI, RVGPU::OpName::simm16);
    if (Desc.isBranch()) {
      if (!Op->isMBB()) {
  ...
  if (Desc.getOpcode() == RVGPU::V_MOVRELS_B32_e32 ||
      Desc.getOpcode() == RVGPU::V_MOVRELS_B32_e64 ||
      Desc.getOpcode() == RVGPU::V_MOVRELD_B32_e32 ||
      Desc.getOpcode() == RVGPU::V_MOVRELD_B32_e64) {
    const bool IsDst = Desc.getOpcode() == RVGPU::V_MOVRELD_B32_e32 ||
                       Desc.getOpcode() == RVGPU::V_MOVRELD_B32_e64;
  ...
  if (shouldReadExec(MI)) {
    if (!MI.hasRegisterImplicitUseOperand(RVGPU::EXEC)) {
  ...
  if (isSMRD(MI)) {
    if (MI.mayStore() &&
        ST.getGeneration() == RVGPUSubtarget::VOLCANIC_ISLANDS) {
      // The register offset form of scalar stores may only use m0 as the
      // soffset register.
      const MachineOperand *Soff = getNamedOperand(MI, RVGPU::OpName::soffset);
      if (Soff && Soff->getReg() != RVGPU::M0) {
  ...
  if (isFLAT(MI) && !ST.hasFlatInstOffsets()) {
    const MachineOperand *Offset = getNamedOperand(MI, RVGPU::OpName::offset);
    if (Offset->getImm() != 0) {
  ...
  if (isImage(MI)) {
    const MachineOperand *DimOp = getNamedOperand(MI, RVGPU::OpName::dim);
    if (DimOp) {
      int VAddr0Idx = RVGPU::getNamedOperandIdx(Opcode,
                                                 RVGPU::OpName::vaddr0);
      int RSrcOpName =
          isMIMG(MI) ? RVGPU::OpName::srsrc : RVGPU::OpName::rsrc;
      int RsrcIdx = RVGPU::getNamedOperandIdx(Opcode, RSrcOpName);
      const RVGPU::MIMGInfo *Info = RVGPU::getMIMGInfo(Opcode);
      const RVGPU::MIMGBaseOpcodeInfo *BaseOpcode =
          RVGPU::getMIMGBaseOpcodeInfo(Info->BaseOpcode);
      const RVGPU::MIMGDimInfo *Dim =
          RVGPU::getMIMGDimInfoByEncoding(DimOp->getImm());
      ...
      bool IsA16 = false;
      if (ST.hasR128A16()) {
        const MachineOperand *R128A16 = getNamedOperand(MI, RVGPU::OpName::r128);
        IsA16 = R128A16->getImm() != 0;
      } else if (ST.hasA16()) {
        const MachineOperand *A16 = getNamedOperand(MI, RVGPU::OpName::a16);
        IsA16 = A16->getImm() != 0;
      }
      ...
      unsigned AddrWords =
          RVGPU::getAddrSizeMIMGOp(BaseOpcode, Dim, IsA16, ST.hasG16());
  ...
  const MachineOperand *DppCt = getNamedOperand(MI, RVGPU::OpName::dpp_ctrl);
  if (DppCt) {
    using namespace RVGPU::DPP;
    ...
    if (DC >= DppCtrl::WAVE_SHL1 && DC <= DppCtrl::WAVE_ROR1 &&
        ST.getGeneration() >= RVGPUSubtarget::GFX10) {
      ErrInfo = "Invalid dpp_ctrl value: "
    ...
    if (Opcode != RVGPU::V_MOV_B64_DPP_PSEUDO &&
        !RVGPU::isLegalDPALU_DPPControl(DC) && RVGPU::isDPALU_DPP(Desc)) {
      ErrInfo = "Invalid dpp_ctrl value: "
  ...
  if ((MI.mayStore() || MI.mayLoad()) && !isVGPRSpill(MI)) {
    const MachineOperand *Dst = getNamedOperand(MI, RVGPU::OpName::vdst);
    uint16_t DataNameIdx = isDS(Opcode) ? RVGPU::OpName::data0
                                        : RVGPU::OpName::vdata;
    const MachineOperand *Data = getNamedOperand(MI, DataNameIdx);
    const MachineOperand *Data2 = getNamedOperand(MI, RVGPU::OpName::data1);
    if (Data && !Data->isReg())
      Data = nullptr;
  ...
  if (ST.needsAlignedVGPRs()) {
    ...
    if (MI.getOpcode() == RVGPU::DS_GWS_INIT ||
        MI.getOpcode() == RVGPU::DS_GWS_SEMA_BR ||
        MI.getOpcode() == RVGPU::DS_GWS_BARRIER) {

      if (!isAlignedReg(RVGPU::OpName::data0)) {
        ErrInfo = "Subtarget requires even aligned vector registers "
    ...
    if (isMIMG(MI)) {
      if (!isAlignedReg(RVGPU::OpName::vaddr)) {
        ErrInfo = "Subtarget requires even aligned vector registers "
  ...
  if (MI.getOpcode() == RVGPU::V_ACCVGPR_WRITE_B32_e64 &&
      !ST.hasGFX90AInsts()) {
    const MachineOperand *Src = getNamedOperand(MI, RVGPU::OpName::src0);
  ...
  if (Desc.getOpcode() == RVGPU::G_RVGPU_WAVE_ADDRESS) {
    const MachineOperand &SrcOp = MI.getOperand(1);
...
unsigned RVInstrInfo::getVALUOp(const MachineInstr &MI) const {
  switch (MI.getOpcode()) {
  default: return RVGPU::INSTRUCTION_LIST_END;
  case RVGPU::REG_SEQUENCE: return RVGPU::REG_SEQUENCE;
  case RVGPU::COPY: return RVGPU::COPY;
  case RVGPU::PHI: return RVGPU::PHI;
  case RVGPU::INSERT_SUBREG: return RVGPU::INSERT_SUBREG;
  case RVGPU::WQM: return RVGPU::WQM;
  case RVGPU::SOFT_WQM: return RVGPU::SOFT_WQM;
  case RVGPU::STRICT_WWM: return RVGPU::STRICT_WWM;
  case RVGPU::STRICT_WQM: return RVGPU::STRICT_WQM;
  case RVGPU::S_MOV_B32: {
    const MachineRegisterInfo &MRI = MI.getParent()->getParent()->getRegInfo();
    return MI.getOperand(1).isReg() ||
           RI.isAGPR(MRI, MI.getOperand(0).getReg()) ?
           RVGPU::COPY : RVGPU::V_MOV_B32_e32;
  }
  case RVGPU::S_ADD_I32:
    return ST.hasAddNoCarry() ? RVGPU::V_ADD_U32_e64 : RVGPU::V_ADD_CO_U32_e32;
  case RVGPU::S_ADDC_U32:
    return RVGPU::V_ADDC_U32_e32;
  case RVGPU::S_SUB_I32:
    return ST.hasAddNoCarry() ? RVGPU::V_SUB_U32_e64 : RVGPU::V_SUB_CO_U32_e32;
    // FIXME: These are not consistently handled, and selected when the carry is
    // used.
  case RVGPU::S_ADD_U32:
    return RVGPU::V_ADD_CO_U32_e32;
  case RVGPU::S_SUB_U32:
    return RVGPU::V_SUB_CO_U32_e32;
  case RVGPU::S_SUBB_U32: return RVGPU::V_SUBB_U32_e32;
  case RVGPU::S_MUL_I32: return RVGPU::V_MUL_LO_U32_e64;
  case RVGPU::S_MUL_HI_U32: return RVGPU::V_MUL_HI_U32_e64;
  case RVGPU::S_MUL_HI_I32: return RVGPU::V_MUL_HI_I32_e64;
  case RVGPU::S_AND_B32: return RVGPU::V_AND_B32_e64;
  case RVGPU::S_OR_B32: return RVGPU::V_OR_B32_e64;
  case RVGPU::S_XOR_B32: return RVGPU::V_XOR_B32_e64;
  case RVGPU::S_XNOR_B32:
    return ST.hasDLInsts() ? RVGPU::V_XNOR_B32_e64 : RVGPU::INSTRUCTION_LIST_END;
  case RVGPU::S_MIN_I32: return RVGPU::V_MIN_I32_e64;
  case RVGPU::S_MIN_U32: return RVGPU::V_MIN_U32_e64;
  case RVGPU::S_MAX_I32: return RVGPU::V_MAX_I32_e64;
  case RVGPU::S_MAX_U32: return RVGPU::V_MAX_U32_e64;
  case RVGPU::S_ASHR_I32: return RVGPU::V_ASHR_I32_e32;
  case RVGPU::S_ASHR_I64: return RVGPU::V_ASHR_I64_e64;
  case RVGPU::S_LSHL_B32: return RVGPU::V_LSHL_B32_e32;
  case RVGPU::S_LSHL_B64: return RVGPU::V_LSHL_B64_e64;
  case RVGPU::S_LSHR_B32: return RVGPU::V_LSHR_B32_e32;
  case RVGPU::S_LSHR_B64: return RVGPU::V_LSHR_B64_e64;
  case RVGPU::S_SEXT_I32_I8: return RVGPU::V_BFE_I32_e64;
  case RVGPU::S_SEXT_I32_I16: return RVGPU::V_BFE_I32_e64;
  case RVGPU::S_BFE_U32: return RVGPU::V_BFE_U32_e64;
  case RVGPU::S_BFE_I32: return RVGPU::V_BFE_I32_e64;
  case RVGPU::S_BFM_B32: return RVGPU::V_BFM_B32_e64;
  case RVGPU::S_BREV_B32: return RVGPU::V_BFREV_B32_e32;
  case RVGPU::S_NOT_B32: return RVGPU::V_NOT_B32_e32;
  case RVGPU::S_NOT_B64: return RVGPU::V_NOT_B32_e32;
  case RVGPU::S_CMP_EQ_I32: return RVGPU::V_CMP_EQ_I32_e64;
  case RVGPU::S_CMP_LG_I32: return RVGPU::V_CMP_NE_I32_e64;
  case RVGPU::S_CMP_GT_I32: return RVGPU::V_CMP_GT_I32_e64;
  case RVGPU::S_CMP_GE_I32: return RVGPU::V_CMP_GE_I32_e64;
  case RVGPU::S_CMP_LT_I32: return RVGPU::V_CMP_LT_I32_e64;
  case RVGPU::S_CMP_LE_I32: return RVGPU::V_CMP_LE_I32_e64;
  case RVGPU::S_CMP_EQ_U32: return RVGPU::V_CMP_EQ_U32_e64;
  case RVGPU::S_CMP_LG_U32: return RVGPU::V_CMP_NE_U32_e64;
  case RVGPU::S_CMP_GT_U32: return RVGPU::V_CMP_GT_U32_e64;
  case RVGPU::S_CMP_GE_U32: return RVGPU::V_CMP_GE_U32_e64;
  case RVGPU::S_CMP_LT_U32: return RVGPU::V_CMP_LT_U32_e64;
  case RVGPU::S_CMP_LE_U32: return RVGPU::V_CMP_LE_U32_e64;
  case RVGPU::S_CMP_EQ_U64: return RVGPU::V_CMP_EQ_U64_e64;
  case RVGPU::S_CMP_LG_U64: return RVGPU::V_CMP_NE_U64_e64;
  case RVGPU::S_BCNT1_I32_B32: return RVGPU::V_BCNT_U32_B32_e64;
  case RVGPU::S_FF1_I32_B32: return RVGPU::V_FFBL_B32_e32;
  case RVGPU::S_FLBIT_I32_B32: return RVGPU::V_FFBH_U32_e32;
  case RVGPU::S_FLBIT_I32: return RVGPU::V_FFBH_I32_e64;
  case RVGPU::S_CBRANCH_SCC0: return RVGPU::S_CBRANCH_VCCZ;
  case RVGPU::S_CBRANCH_SCC1: return RVGPU::S_CBRANCH_VCCNZ;
  case RVGPU::S_CVT_F32_I32: return RVGPU::V_CVT_F32_I32_e64;
  case RVGPU::S_CVT_F32_U32: return RVGPU::V_CVT_F32_U32_e64;
  case RVGPU::S_CVT_I32_F32: return RVGPU::V_CVT_I32_F32_e64;
  case RVGPU::S_CVT_U32_F32: return RVGPU::V_CVT_U32_F32_e64;
  case RVGPU::S_CVT_F32_F16: return RVGPU::V_CVT_F32_F16_t16_e64;
  case RVGPU::S_CVT_HI_F32_F16: return RVGPU::V_CVT_F32_F16_t16_e64;
  case RVGPU::S_CVT_F16_F32: return RVGPU::V_CVT_F16_F32_t16_e64;
  case RVGPU::S_CEIL_F32: return RVGPU::V_CEIL_F32_e64;
  case RVGPU::S_FLOOR_F32: return RVGPU::V_FLOOR_F32_e64;
  case RVGPU::S_TRUNC_F32: return RVGPU::V_TRUNC_F32_e64;
  case RVGPU::S_RNDNE_F32: return RVGPU::V_RNDNE_F32_e64;
  case RVGPU::S_CEIL_F16: return RVGPU::V_CEIL_F16_t16_e64;
  case RVGPU::S_FLOOR_F16: return RVGPU::V_FLOOR_F16_t16_e64;
  case RVGPU::S_TRUNC_F16: return RVGPU::V_TRUNC_F16_t16_e64;
  case RVGPU::S_RNDNE_F16: return RVGPU::V_RNDNE_F16_t16_e64;
  case RVGPU::S_ADD_F32: return RVGPU::V_ADD_F32_e64;
  case RVGPU::S_SUB_F32: return RVGPU::V_SUB_F32_e64;
  case RVGPU::S_MIN_F32: return RVGPU::V_MIN_F32_e64;
  case RVGPU::S_MAX_F32: return RVGPU::V_MAX_F32_e64;
  case RVGPU::S_MINIMUM_F32: return RVGPU::V_MINIMUM_F32_e64;
  case RVGPU::S_MAXIMUM_F32: return RVGPU::V_MAXIMUM_F32_e64;
  case RVGPU::S_MUL_F32: return RVGPU::V_MUL_F32_e64;
  case RVGPU::S_ADD_F16: return RVGPU::V_ADD_F16_fake16_e64;
  case RVGPU::S_SUB_F16: return RVGPU::V_SUB_F16_fake16_e64;
  case RVGPU::S_MIN_F16: return RVGPU::V_MIN_F16_fake16_e64;
  case RVGPU::S_MAX_F16: return RVGPU::V_MAX_F16_fake16_e64;
  case RVGPU::S_MINIMUM_F16: return RVGPU::V_MINIMUM_F16_e64;
  case RVGPU::S_MAXIMUM_F16: return RVGPU::V_MAXIMUM_F16_e64;
  case RVGPU::S_MUL_F16: return RVGPU::V_MUL_F16_fake16_e64;
  case RVGPU::S_CVT_PK_RTZ_F16_F32: return RVGPU::V_CVT_PKRTZ_F16_F32_e64;
  case RVGPU::S_FMAC_F32: return RVGPU::V_FMAC_F32_e64;
  case RVGPU::S_FMAC_F16: return RVGPU::V_FMAC_F16_t16_e64;
  case RVGPU::S_FMAMK_F32: return RVGPU::V_FMAMK_F32;
  case RVGPU::S_FMAAK_F32: return RVGPU::V_FMAAK_F32;
  case RVGPU::S_CMP_LT_F32: return RVGPU::V_CMP_LT_F32_e64;
  case RVGPU::S_CMP_EQ_F32: return RVGPU::V_CMP_EQ_F32_e64;
  case RVGPU::S_CMP_LE_F32: return RVGPU::V_CMP_LE_F32_e64;
  case RVGPU::S_CMP_GT_F32: return RVGPU::V_CMP_GT_F32_e64;
  case RVGPU::S_CMP_LG_F32: return RVGPU::V_CMP_LG_F32_e64;
  case RVGPU::S_CMP_GE_F32: return RVGPU::V_CMP_GE_F32_e64;
  case RVGPU::S_CMP_O_F32: return RVGPU::V_CMP_O_F32_e64;
  case RVGPU::S_CMP_U_F32: return RVGPU::V_CMP_U_F32_e64;
  case RVGPU::S_CMP_NGE_F32: return RVGPU::V_CMP_NGE_F32_e64;
  case RVGPU::S_CMP_NLG_F32: return RVGPU::V_CMP_NLG_F32_e64;
  case RVGPU::S_CMP_NGT_F32: return RVGPU::V_CMP_NGT_F32_e64;
  case RVGPU::S_CMP_NLE_F32: return RVGPU::V_CMP_NLE_F32_e64;
  case RVGPU::S_CMP_NEQ_F32: return RVGPU::V_CMP_NEQ_F32_e64;
  case RVGPU::S_CMP_NLT_F32: return RVGPU::V_CMP_NLT_F32_e64;
  case RVGPU::S_CMP_LT_F16: return RVGPU::V_CMP_LT_F16_t16_e64;
  case RVGPU::S_CMP_EQ_F16: return RVGPU::V_CMP_EQ_F16_t16_e64;
  case RVGPU::S_CMP_LE_F16: return RVGPU::V_CMP_LE_F16_t16_e64;
  case RVGPU::S_CMP_GT_F16: return RVGPU::V_CMP_GT_F16_t16_e64;
  case RVGPU::S_CMP_LG_F16: return RVGPU::V_CMP_LG_F16_t16_e64;
  case RVGPU::S_CMP_GE_F16: return RVGPU::V_CMP_GE_F16_t16_e64;
  case RVGPU::S_CMP_O_F16: return RVGPU::V_CMP_O_F16_t16_e64;
  case RVGPU::S_CMP_U_F16: return RVGPU::V_CMP_U_F16_t16_e64;
  case RVGPU::S_CMP_NGE_F16: return RVGPU::V_CMP_NGE_F16_t16_e64;
  case RVGPU::S_CMP_NLG_F16: return RVGPU::V_CMP_NLG_F16_t16_e64;
  case RVGPU::S_CMP_NGT_F16: return RVGPU::V_CMP_NGT_F16_t16_e64;
  case RVGPU::S_CMP_NLE_F16: return RVGPU::V_CMP_NLE_F16_t16_e64;
  case RVGPU::S_CMP_NEQ_F16: return RVGPU::V_CMP_NEQ_F16_t16_e64;
  case RVGPU::S_CMP_NLT_F16: return RVGPU::V_CMP_NLT_F16_t16_e64;
  case RVGPU::V_S_EXP_F32_e64: return RVGPU::V_EXP_F32_e64;
  case RVGPU::V_S_EXP_F16_e64: return RVGPU::V_EXP_F16_t16_e64;
  case RVGPU::V_S_LOG_F32_e64: return RVGPU::V_LOG_F32_e64;
  case RVGPU::V_S_LOG_F16_e64: return RVGPU::V_LOG_F16_t16_e64;
  case RVGPU::V_S_RCP_F32_e64: return RVGPU::V_RCP_F32_e64;
  case RVGPU::V_S_RCP_F16_e64: return RVGPU::V_RCP_F16_t16_e64;
  case RVGPU::V_S_RSQ_F32_e64: return RVGPU::V_RSQ_F32_e64;
  case RVGPU::V_S_RSQ_F16_e64: return RVGPU::V_RSQ_F16_t16_e64;
  case RVGPU::V_S_SQRT_F32_e64: return RVGPU::V_SQRT_F32_e64;
  case RVGPU::V_S_SQRT_F16_e64: return RVGPU::V_SQRT_F16_t16_e64;
  }
  llvm_unreachable(
...
void RVInstrInfo::insertScratchExecCopy(MachineFunction &MF,
                                        MachineBasicBlock &MBB,
  ...
  if (IsSCCLive) {
    ...
    unsigned MovOpc = IsWave32 ? RVGPU::S_MOV_B32 : RVGPU::S_MOV_B64;
    MCRegister Exec = IsWave32 ? RVGPU::EXEC_LO : RVGPU::EXEC;
    ...
    const unsigned OrSaveExec =
        IsWave32 ? RVGPU::S_OR_SAVEEXEC_B32 : RVGPU::S_OR_SAVEEXEC_B64;
    auto SaveExec =
...
void RVInstrInfo::restoreExec(MachineFunction &MF, MachineBasicBlock &MBB,
                              MachineBasicBlock::iterator MBBI,
                              const DebugLoc &DL, Register Reg,
                              SlotIndexes *Indexes) const {
  unsigned ExecMov = isWave32() ? RVGPU::S_MOV_B32 : RVGPU::S_MOV_B64;
  MCRegister Exec = isWave32() ? RVGPU::EXEC_LO : RVGPU::EXEC;
  auto ExecRestoreMI =
...
adjustAllocatableRegClass(const RVSubtarget &ST, const RVRegisterInfo &RI,
  ...
  if ((IsAllocatable || !ST.hasGFX90AInsts() || !MRI.reservedRegsFrozen()) &&
    ...
    switch (RCID) {
    case RVGPU::AV_32RegClassID:
      RCID = RVGPU::VGPR_32RegClassID;
      break;
    case RVGPU::AV_64RegClassID:
      RCID = RVGPU::VReg_64RegClassID;
      break;
    case RVGPU::AV_96RegClassID:
      RCID = RVGPU::VReg_96RegClassID;
      break;
    case RVGPU::AV_128RegClassID:
      RCID = RVGPU::VReg_128RegClassID;
      break;
    case RVGPU::AV_160RegClassID:
      RCID = RVGPU::VReg_160RegClassID;
      break;
    case RVGPU::AV_512RegClassID:
      RCID = RVGPU::VReg_512RegClassID;
      break;
    default:
      break;
    }
...
const TargetRegisterClass *RVInstrInfo::getRegClass(const MCInstrDesc &TID,
  ...
  if (TID.TSFlags & (SIInstrFlags::DS | SIInstrFlags::FLAT)) {
    ...
    const int VDstIdx = RVGPU::getNamedOperandIdx(TID.Opcode,
                                                   RVGPU::OpName::vdst);
    const int DataIdx = RVGPU::getNamedOperandIdx(TID.Opcode,
        (TID.TSFlags & SIInstrFlags::DS) ? RVGPU::OpName::data0
                                         : RVGPU::OpName::vdata);
    if (DataIdx != -1) {
      IsAllocatable = VDstIdx != -1 || RVGPU::hasNamedOperand(
                                           TID.Opcode, RVGPU::OpName::data1);
    }
  }
...
const TargetRegisterClass *RVInstrInfo::getOpRegClass(const MachineInstr &MI,
                                                      unsigned OpNo) const {
  const MachineRegisterInfo &MRI = MI.getParent()->getParent()->getRegInfo();
...
void RVInstrInfo::legalizeOpWithMove(MachineInstr &MI, unsigned OpIdx) const {
  MachineBasicBlock::iterator I = MI;
  MachineBasicBlock *MBB = MI.getParent();
  ...
  unsigned Size = RI.getRegSizeInBits(*RC);
  unsigned Opcode = (Size == 64) ? RVGPU::V_MOV_B64_PSEUDO : RVGPU::V_MOV_B32_e32;
  if (MO.isReg())
    Opcode = RVGPU::COPY;
  else if (RI.isSGPRClass(RC))
    Opcode = (Size == 64) ? RVGPU::S_MOV_B64 : RVGPU::S_MOV_B32;
...
unsigned RVInstrInfo::buildExtractSubReg(
  ...
  if (SuperReg.getSubReg() == RVGPU::NoSubRegister) {
...
void RVInstrInfo::swapOperands(MachineInstr &Inst) const {
...
bool RVInstrInfo::isLegalRegOperand(const MachineRegisterInfo &MRI,
...
bool RVInstrInfo::isLegalVSrcOperand(const MachineRegisterInfo &MRI,
                                     const MCOperandInfo &OpInfo,
                                     const MachineOperand &MO) const {
  if (MO.isReg())
...
bool RVInstrInfo::isOperandLegal(const MachineInstr &MI, unsigned OpIdx,
                                 const MachineOperand *MO) const {
  const MachineFunction &MF = *MI.getParent()->getParent();
  const MachineRegisterInfo &MRI = MF.getRegInfo();
  const MCInstrDesc &InstDesc = MI.getDesc();
  ...
  if (isVALU(MI) && usesConstantBus(MRI, *MO, OpInfo)) {
    ...
    for (unsigned i = 0, e = MI.getNumOperands(); i != e; ++i) {
      ...
      } else if (RVGPU::isSISrcOperand(InstDesc, i) &&
  ...
  if (MO->isReg()) {
    ...
    const int VDstIdx = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::vdst);
    const int DataIdx = RVGPU::getNamedOperandIdx(Opc,
        isDS(Opc) ? RVGPU::OpName::data0 : RVGPU::OpName::vdata);
    ...
    if ((int)OpIdx == DataIdx) {
      ...
      const int Data1Idx = RVGPU::getNamedOperandIdx(Opc,
                                                      RVGPU::OpName::data1);
      if (Data1Idx != -1 && MI.getOperand(Data1Idx).isReg() &&
          RI.isAGPR(MRI, MI.getOperand(Data1Idx).getReg()) != IsAGPR)
        return false;
    }
    if (Opc == RVGPU::V_ACCVGPR_WRITE_B32_e64 && !ST.hasGFX90AInsts() &&
        (int)OpIdx == RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::src0) &&
  ...
  if (MO->isImm()) {
    ...
    bool Is64BitFPOp = OpInfo.OperandType == RVGPU::OPERAND_REG_IMM_FP64;
    bool Is64BitOp = Is64BitFPOp ||
                     OpInfo.OperandType == RVGPU::OPERAND_REG_IMM_INT64 ||
                     OpInfo.OperandType == RVGPU::OPERAND_REG_IMM_V2INT32 ||
                     OpInfo.OperandType == RVGPU::OPERAND_REG_IMM_V2FP32;
    if (Is64BitOp &&
        !RVGPU::isInlinableLiteral64(Imm, ST.hasInv2PiInlineImm())) {
      if (!RVGPU::isValid32BitLiteral(Imm, Is64BitFPOp))
        return false;
...
void RVInstrInfo::legalizeOperandsVOP2(MachineRegisterInfo &MRI,
  ...
  if (Opc == RVGPU::V_WRITELANE_B32) {
    const DebugLoc &DL = MI.getDebugLoc();
    if (Src0.isReg() && RI.isVGPR(MRI, Src0.getReg())) {
      Register Reg = MRI.createVirtualRegister(&RVGPU::SReg_32_XM0RegClass);
      BuildMI(*MI.getParent(), MI, DL, get(RVGPU::V_READFIRSTLANE_B32), Reg)
          .add(Src0);
  ...
  if (Opc == RVGPU::V_FMAC_F32_e32 || Opc == RVGPU::V_FMAC_F16_e32) {
    int Src2Idx = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::src2);
  ...
  // select is uniform.
  if (Opc == RVGPU::V_READLANE_B32 && Src1.isReg() &&
...
void RVInstrInfo::legalizeOperandsVOP3(MachineRegisterInfo &MRI,
                                       MachineInstr &MI) const {
  unsigned Opc = MI.getOpcode();

  int VOP3Idx[3] = {
    RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::src0),
    RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::src1),
    RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::src2)
  };

  if (Opc == RVGPU::V_PERMLANE16_B32_e64 ||
      Opc == RVGPU::V_PERMLANEX16_B32_e64) {
  ...
  if ((Opc == RVGPU::V_FMAC_F32_e64 || Opc == RVGPU::V_FMAC_F16_e64) &&
      !RI.isVGPR(MRI, MI.getOperand(VOP3Idx[2]).getReg()))
    legalizeOpWithMove(MI, VOP3Idx[2]);
}

Register RVInstrInfo::readlaneVGPRToSGPR(Register SrcReg, MachineInstr &UseMI,
  ...
  if (SubRegs == 1) {
    BuildMI(*UseMI.getParent(), UseMI, UseMI.getDebugLoc(),
            get(RVGPU::V_READFIRSTLANE_B32), DstReg)
        .addReg(SrcReg);
    return DstReg;
  }
  ...
  for (unsigned i = 0; i < SubRegs; ++i) {
    Register SGPR = MRI.createVirtualRegister(&RVGPU::SGPR_32RegClass);
    BuildMI(*UseMI.getParent(), UseMI, UseMI.getDebugLoc(),
            get(RVGPU::V_READFIRSTLANE_B32), SGPR)
...
void RVInstrInfo::legalizeOperandsSMRD(MachineRegisterInfo &MRI,
                                       MachineInstr &MI) const {
  ...
  MachineOperand *SBase = getNamedOperand(MI, RVGPU::OpName::sbase);
  if (SBase && !RI.isSGPRClass(MRI.getRegClass(SBase->getReg()))) {
    Register SGPR = readlaneVGPRToSGPR(SBase->getReg(), MI, MRI);
    SBase->setReg(SGPR);
  }
  MachineOperand *SOff = getNamedOperand(MI, RVGPU::OpName::soffset);
...
bool RVInstrInfo::moveFlatAddrToVGPR(MachineInstr &Inst) const {
  unsigned Opc = Inst.getOpcode();
  int OldSAddrIdx = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::saddr);
  if (OldSAddrIdx < 0)
    return false;

  assert(isSegmentSpecificFLAT(Inst));

  int NewOpc = RVGPU::getGlobalVaddrOp(Opc);
  if (NewOpc < 0)
    NewOpc = RVGPU::getFlatScratchInstSVfromSS(Opc);
  if (NewOpc < 0)
    return false;
  ...
  int NewVAddrIdx = RVGPU::getNamedOperandIdx(NewOpc, RVGPU::OpName::vaddr);
  if (NewVAddrIdx < 0)
    return false;

  int OldVAddrIdx = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::vaddr);
  ...
  if (OldVAddrIdx >= 0) {
    ...
    if (!VAddrDef || VAddrDef->getOpcode() != RVGPU::V_MOV_B32_e32 ||
  ...
  if (OldVAddrIdx == NewVAddrIdx) {
    ...
    if (OldVAddrIdx >= 0) {
      int NewVDstIn = RVGPU::getNamedOperandIdx(NewOpc,
      ...
      if (NewVDstIn != -1) {
        int OldVDstIn = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::vdst_in);
      ...
      if (NewVDstIn != -1) {
        int NewVDst = RVGPU::getNamedOperandIdx(NewOpc, RVGPU::OpName::vdst);
...
void RVInstrInfo::legalizeOperandsFLAT(MachineRegisterInfo &MRI,
                                       MachineInstr &MI) const {
  ...
  MachineOperand *SAddr = getNamedOperand(MI, RVGPU::OpName::saddr);
...
void RVInstrInfo::legalizeGenericOperand(MachineBasicBlock &InsertMBB,
                                         MachineBasicBlock::iterator I,
  ...
  auto Copy = BuildMI(InsertMBB, I, DL, get(RVGPU::COPY), DstReg).add(Op);
  ...
  if (Def->isMoveImmediate() && DstRC != &RVGPU::VReg_1RegClass)
  ...
  if (!RI.isSGPRClass(DstRC) && !Copy->readsRegister(RVGPU::EXEC, &RI) &&
      !ImpDef)
    Copy.addReg(RVGPU::EXEC, RegState::Implicit);
}
...
static void emitLoadScalarOpsFromVGPRLoop(
    const RVInstrInfo &TII, MachineRegisterInfo &MRI, MachineBasicBlock &OrigBB,
  ...
  unsigned SaveExecOpc =
      ST.isWave32() ? RVGPU::S_AND_SAVEEXEC_B32 : RVGPU::S_AND_SAVEEXEC_B64;
  unsigned XorTermOpc =
      ST.isWave32() ? RVGPU::S_XOR_B32_term : RVGPU::S_XOR_B64_term;
  unsigned AndOpc =
      ST.isWave32() ? RVGPU::S_AND_B32 : RVGPU::S_AND_B64;
  const auto *BoolXExecRC = TRI->getRegClass(RVGPU::SReg_1_XEXECRegClassID);
  ...
  for (MachineOperand *ScalarOp : ScalarOps) {
    ...
    if (NumSubRegs == 1) {
      Register CurReg = MRI.createVirtualRegister(&RVGPU::SGPR_32RegClass);

      BuildMI(LoopBB, I, DL, TII.get(RVGPU::V_READFIRSTLANE_B32), CurReg)
          .addReg(VScalarOp);

      Register NewCondReg = MRI.createVirtualRegister(BoolXExecRC);

      BuildMI(LoopBB, I, DL, TII.get(RVGPU::V_CMP_EQ_U32_e64), NewCondReg)
          .addReg(CurReg)
          .addReg(VScalarOp);
      ...
      for (unsigned Idx = 0; Idx < NumSubRegs; Idx += 2) {
        Register CurRegLo = MRI.createVirtualRegister(&RVGPU::SGPR_32RegClass);
        Register CurRegHi = MRI.createVirtualRegister(&RVGPU::SGPR_32RegClass);

        // Read the next variant <- also loop target.
        BuildMI(LoopBB, I, DL, TII.get(RVGPU::V_READFIRSTLANE_B32), CurRegLo)
            .addReg(VScalarOp, VScalarOpUndef, TRI->getSubRegFromChannel(Idx));

        // Read the next variant <- also loop target.
        BuildMI(LoopBB, I, DL, TII.get(RVGPU::V_READFIRSTLANE_B32), CurRegHi)
        ...
        Register CurReg = MRI.createVirtualRegister(&RVGPU::SGPR_64RegClass);
        BuildMI(LoopBB, I, DL, TII.get(RVGPU::REG_SEQUENCE), CurReg)
            .addReg(CurRegLo)
            .addImm(RVGPU::sub0)
            .addReg(CurRegHi)
            .addImm(RVGPU::sub1);
        ...
        Register NewCondReg = MRI.createVirtualRegister(BoolXExecRC);
        auto Cmp = BuildMI(LoopBB, I, DL, TII.get(RVGPU::V_CMP_EQ_U64_e64),
                           NewCondReg)
                       .addReg(CurReg);
        if (NumSubRegs <= 2)
      ...
      auto Merge =
          BuildMI(LoopBB, I, DL, TII.get(RVGPU::REG_SEQUENCE), SScalarOp);
      unsigned Channel = 0;
  ...
  BuildMI(BodyBB, I, DL, TII.get(RVGPU::SI_WATERFALL_LOOP)).addMBB(&LoopBB);
}
...
loadMBUFScalarOperandsFromVGPR(const RVInstrInfo &TII, MachineInstr &MI,
                               ArrayRef<MachineOperand *> ScalarOps,
  ...
  const DebugLoc &DL = MI.getDebugLoc();
  unsigned Exec = ST.isWave32() ? RVGPU::EXEC_LO : RVGPU::EXEC;
  unsigned MovExecOpc = ST.isWave32() ? RVGPU::S_MOV_B32 : RVGPU::S_MOV_B64;
  const auto *BoolXExecRC = TRI->getRegClass(RVGPU::SReg_1_XEXECRegClassID);

  // Save SCC. Waterfall Loop may overwrite SCC.
  Register SaveSCCReg;
  bool SCCNotDead = (MBB.computeRegisterLiveness(TRI, RVGPU::SCC, MI, 30) !=
                     MachineBasicBlock::LQR_Dead);
  if (SCCNotDead) {
    SaveSCCReg = MRI.createVirtualRegister(&RVGPU::SReg_32RegClass);
    BuildMI(MBB, Begin, DL, TII.get(RVGPU::S_CSELECT_B32), SaveSCCReg)
        .addImm(1)
        .addImm(0);
  }
  ...
  // Restore SCC
  if (SCCNotDead) {
    BuildMI(*RemainderBB, First, DL, TII.get(RVGPU::S_CMP_LG_U32))
...
extractRsrcPtr(const RVInstrInfo &TII, MachineInstr &MI, MachineOperand &Rsrc) {
  MachineBasicBlock &MBB = *MI.getParent();
  MachineFunction &MF = *MBB.getParent();
  MachineRegisterInfo &MRI = MF.getRegInfo();
  ...
  unsigned RsrcPtr =
      TII.buildExtractSubReg(MI, MRI, Rsrc, &RVGPU::VReg_128RegClass,
                             RVGPU::sub0_sub1, &RVGPU::VReg_64RegClass);

  // Create an empty resource descriptor
  Register Zero64 = MRI.createVirtualRegister(&RVGPU::SReg_64RegClass);
  Register SRsrcFormatLo = MRI.createVirtualRegister(&RVGPU::SGPR_32RegClass);
  Register SRsrcFormatHi = MRI.createVirtualRegister(&RVGPU::SGPR_32RegClass);
  Register NewSRsrc = MRI.createVirtualRegister(&RVGPU::SGPR_128RegClass);
  uint64_t RsrcDataFormat = TII.getDefaultRsrcDataFormat();

  // Zero64 = 0
  BuildMI(MBB, MI, MI.getDebugLoc(), TII.get(RVGPU::S_MOV_B64), Zero64)
      .addImm(0);

  // SRsrcFormatLo = RSRC_DATA_FORMAT{31-0}
  BuildMI(MBB, MI, MI.getDebugLoc(), TII.get(RVGPU::S_MOV_B32), SRsrcFormatLo)
      .addImm(RsrcDataFormat & 0xFFFFFFFF);

  // SRsrcFormatHi = RSRC_DATA_FORMAT{63-32}
  BuildMI(MBB, MI, MI.getDebugLoc(), TII.get(RVGPU::S_MOV_B32), SRsrcFormatHi)
      .addImm(RsrcDataFormat >> 32);

  // NewSRsrc = {Zero64, SRsrcFormat}
  BuildMI(MBB, MI, MI.getDebugLoc(), TII.get(RVGPU::REG_SEQUENCE), NewSRsrc)
      .addReg(Zero64)
      .addImm(RVGPU::sub0_sub1)
...
MachineBasicBlock *
RVInstrInfo::legalizeOperands(MachineInstr &MI,
  ...
  if (MI.getOpcode() == RVGPU::PHI) {
    ...
    if (VRC || !RI.isSGPRClass(getOpRegClass(MI, 0))) {
      if (!VRC) {
        assert(SRC);
        if (getOpRegClass(MI, 0) == &RVGPU::VReg_1RegClass) {
          VRC = &RVGPU::VReg_1RegClass;
        } else
  ...
  if (MI.getOpcode() == RVGPU::REG_SEQUENCE) {
    MachineBasicBlock *MBB = MI.getParent();
  ...
  if (MI.getOpcode() == RVGPU::INSERT_SUBREG) {
    Register Dst = MI.getOperand(0).getReg();
  ...
  // Legalize SI_INIT_M0
  if (MI.getOpcode() == RVGPU::SI_INIT_M0) {
  ...
  if (MI.getOpcode() == RVGPU::S_BITREPLICATE_B64_B32 ||
      MI.getOpcode() == RVGPU::S_QUADMASK_B32 ||
      MI.getOpcode() == RVGPU::S_QUADMASK_B64 ||
      MI.getOpcode() == RVGPU::S_WQM_B32 ||
      MI.getOpcode() == RVGPU::S_WQM_B64) {
  ...
  if (isImage(MI) || (RVGPU::isGraphics(MF.getFunction().getCallingConv()) &&
                      (isMUBUF(MI) || isMTBUF(MI)))) {
    int RSrcOpName = (isVIMAGE(MI) || isVSAMPLE(MI)) ? RVGPU::OpName::rsrc
                                                     : RVGPU::OpName::srsrc;
    MachineOperand *SRsrc = getNamedOperand(MI, RSrcOpName);
    if (SRsrc && !RI.isSGPRClass(MRI.getRegClass(SRsrc->getReg())))
      CreatedBB = loadMBUFScalarOperandsFromVGPR(*this, MI, {SRsrc}, MDT);

    int SampOpName = isMIMG(MI) ? RVGPU::OpName::ssamp : RVGPU::OpName::samp;
  ...
  // Legalize SI_CALL
  if (MI.getOpcode() == RVGPU::SI_CALL_ISEL) {
  ...
  if (MI.getOpcode() == RVGPU::S_SLEEP_VAR) {
    const DebugLoc &DL = MI.getDebugLoc();
    Register Reg = MRI.createVirtualRegister(&RVGPU::SReg_32_XM0RegClass);
    int Src0Idx =
        RVGPU::getNamedOperandIdx(MI.getOpcode(), RVGPU::OpName::src0);
    MachineOperand &Src0 = MI.getOperand(Src0Idx);
    BuildMI(*MI.getParent(), MI, DL, get(RVGPU::V_READFIRSTLANE_B32), Reg)
        .add(Src0);
    Src0.ChangeToRegister(Reg, false);
  ...
  int SoffsetIdx =
      RVGPU::getNamedOperandIdx(MI.getOpcode(), RVGPU::OpName::soffset);
  if (SoffsetIdx != -1) {
  ...
  if (!isRsrcLegal) {
    ...
    MachineOperand *VAddr = getNamedOperand(MI, RVGPU::OpName::vaddr);
    if (VAddr && RVGPU::getIfAddr64Inst(MI.getOpcode()) != -1) {
      // This is already an ADDR64 instruction so we need to add the pointer
      // extracted from the resource descriptor to the current value of VAddr.
      Register NewVAddrLo = MRI.createVirtualRegister(&RVGPU::VGPR_32RegClass);
      Register NewVAddrHi = MRI.createVirtualRegister(&RVGPU::VGPR_32RegClass);
      Register NewVAddr = MRI.createVirtualRegister(&RVGPU::VReg_64RegClass);

      const auto *BoolXExecRC = RI.getRegClass(RVGPU::SReg_1_XEXECRegClassID);
      ...
      const DebugLoc &DL = MI.getDebugLoc();
      BuildMI(MBB, MI, DL, get(RVGPU::V_ADD_CO_U32_e64), NewVAddrLo)
        .addDef(CondReg0)
        .addReg(RsrcPtr, 0, RVGPU::sub0)
        .addReg(VAddr->getReg(), 0, RVGPU::sub0)
        .addImm(0);

      // NewVaddrHi = RsrcPtr:sub1 + VAddr:sub1
      BuildMI(MBB, MI, DL, get(RVGPU::V_ADDC_U32_e64), NewVAddrHi)
        .addDef(CondReg1, RegState::Dead)
        .addReg(RsrcPtr, 0, RVGPU::sub1)
        .addReg(VAddr->getReg(), 0, RVGPU::sub1)
        .addReg(CondReg0, RegState::Kill)
        .addImm(0);

      // NewVaddr = {NewVaddrHi, NewVaddrLo}
      BuildMI(MBB, MI, MI.getDebugLoc(), get(RVGPU::REG_SEQUENCE), NewVAddr)
          .addReg(NewVAddrLo)
          .addImm(RVGPU::sub0)
          .addReg(NewVAddrHi)
          .addImm(RVGPU::sub1);
      ...
      // ADDR64.
      assert(ST.getGeneration() < RVGPUSubtarget::VOLCANIC_ISLANDS &&
      ...
      Register NewVAddr = MRI.createVirtualRegister(&RVGPU::VReg_64RegClass);
      MachineOperand *VData = getNamedOperand(MI, RVGPU::OpName::vdata);
      MachineOperand *Offset = getNamedOperand(MI, RVGPU::OpName::offset);
      MachineOperand *SOffset = getNamedOperand(MI, RVGPU::OpName::soffset);
      unsigned Addr64Opcode = RVGPU::getAddr64Inst(MI.getOpcode());

      // Atomics with return have an additional tied operand and are
      // missing some of the special bits.
      MachineOperand *VDataIn = getNamedOperand(MI, RVGPU::OpName::vdata_in);
      MachineInstr *Addr64;
      ...
      if (!VDataIn) {
        ...
        if (const MachineOperand *CPol =
                getNamedOperand(MI, RVGPU::OpName::cpol)) {
          MIB.addImm(CPol->getImm());
        }

        if (const MachineOperand *TFE =
                getNamedOperand(MI, RVGPU::OpName::tfe)) {
          MIB.addImm(TFE->getImm());
        }

        MIB.addImm(getNamedImmOperand(MI, RVGPU::OpName::swz));
        ...
        Addr64 = BuildMI(MBB, MI, MI.getDebugLoc(), get(Addr64Opcode))
                     ...
                     .add(*SOffset)
                     .add(*Offset)
                     .addImm(getNamedImmOperand(MI, RVGPU::OpName::cpol))
                     .cloneMemRefs(MI);
      }
      ...
      // NewVaddr = {NewVaddrHi, NewVaddrLo}
      BuildMI(MBB, Addr64, Addr64->getDebugLoc(), get(RVGPU::REG_SEQUENCE),
              NewVAddr)
          .addReg(RsrcPtr, 0, RVGPU::sub0)
          .addImm(RVGPU::sub0)
          .addReg(RsrcPtr, 0, RVGPU::sub1)
...
void RVInstrInfo::moveToVALUImpl(RVInstrWorklist &Worklist,
  ...
  default:
    break;
  case RVGPU::S_ADD_U64_PSEUDO:
    NewOpcode = RVGPU::V_ADD_U64_PSEUDO;
    break;
  case RVGPU::S_SUB_U64_PSEUDO:
    NewOpcode = RVGPU::V_SUB_U64_PSEUDO;
    break;
  case RVGPU::S_ADD_I32:
  case RVGPU::S_SUB_I32: {
  ...
  case RVGPU::S_AND_B64:
    splitScalar64BitBinaryOp(Worklist, Inst, RVGPU::S_AND_B32, MDT);
    Inst.eraseFromParent();
    return;

  case RVGPU::S_OR_B64:
    splitScalar64BitBinaryOp(Worklist, Inst, RVGPU::S_OR_B32, MDT);
    Inst.eraseFromParent();
    return;

  case RVGPU::S_XOR_B64:
    splitScalar64BitBinaryOp(Worklist, Inst, RVGPU::S_XOR_B32, MDT);
    Inst.eraseFromParent();
    return;

  case RVGPU::S_NAND_B64:
    splitScalar64BitBinaryOp(Worklist, Inst, RVGPU::S_NAND_B32, MDT);
    Inst.eraseFromParent();
    return;

  case RVGPU::S_NOR_B64:
    splitScalar64BitBinaryOp(Worklist, Inst, RVGPU::S_NOR_B32, MDT);
    Inst.eraseFromParent();
    return;

  case RVGPU::S_XNOR_B64:
    if (ST.hasDLInsts())
      splitScalar64BitBinaryOp(Worklist, Inst, RVGPU::S_XNOR_B32, MDT);
    else
      splitScalar64BitXnor(Worklist, Inst, MDT);
    Inst.eraseFromParent();
    return;

  case RVGPU::S_ANDN2_B64:
    splitScalar64BitBinaryOp(Worklist, Inst, RVGPU::S_ANDN2_B32, MDT);
    Inst.eraseFromParent();
    return;

  case RVGPU::S_ORN2_B64:
    splitScalar64BitBinaryOp(Worklist, Inst, RVGPU::S_ORN2_B32, MDT);
    Inst.eraseFromParent();
    return;

  case RVGPU::S_BREV_B64:
    splitScalar64BitUnaryOp(Worklist, Inst, RVGPU::S_BREV_B32, true);
    Inst.eraseFromParent();
    return;

  case RVGPU::S_NOT_B64:
    splitScalar64BitUnaryOp(Worklist, Inst, RVGPU::S_NOT_B32);
    Inst.eraseFromParent();
    return;

  case RVGPU::S_BCNT1_I32_B64:
    splitScalar64BitBCNT(Worklist, Inst);
    Inst.eraseFromParent();
    return;

  case RVGPU::S_BFE_I64:
    splitScalar64BitBFE(Worklist, Inst);
    Inst.eraseFromParent();
    return;

  case RVGPU::S_LSHL_B32:
    if (ST.hasOnlyRevVALUShifts()) {
      NewOpcode = RVGPU::V_LSHLREV_B32_e64;
      swapOperands(Inst);
    }
    break;
  case RVGPU::S_ASHR_I32:
    if (ST.hasOnlyRevVALUShifts()) {
      NewOpcode = RVGPU::V_ASHRREV_I32_e64;
      swapOperands(Inst);
    }
    break;
  case RVGPU::S_LSHR_B32:
    if (ST.hasOnlyRevVALUShifts()) {
      NewOpcode = RVGPU::V_LSHRREV_B32_e64;
      swapOperands(Inst);
    }
    break;
  case RVGPU::S_LSHL_B64:
    if (ST.hasOnlyRevVALUShifts()) {
      NewOpcode = ST.getGeneration() >= RVGPUSubtarget::GFX12
                      ? RVGPU::V_LSHLREV_B64_pseudo_e64
                      : RVGPU::V_LSHLREV_B64_e64;
      swapOperands(Inst);
    }
    break;
  case RVGPU::S_ASHR_I64:
    if (ST.hasOnlyRevVALUShifts()) {
      NewOpcode = RVGPU::V_ASHRREV_I64_e64;
      swapOperands(Inst);
    }
    break;
  case RVGPU::S_LSHR_B64:
    if (ST.hasOnlyRevVALUShifts()) {
      NewOpcode = RVGPU::V_LSHRREV_B64_e64;
      swapOperands(Inst);
    }
    break;

  case RVGPU::S_ABS_I32:
  ...
  case RVGPU::S_CBRANCH_SCC1: {
    ...
    bool IsSCC = CondReg == RVGPU::SCC;
    Register VCC = RI.getVCC();
    Register EXEC = ST.isWave32() ? RVGPU::EXEC_LO : RVGPU::EXEC;
    unsigned Opc = ST.isWave32() ? RVGPU::S_AND_B32 : RVGPU::S_AND_B64;
  ...
  case RVGPU::S_BFE_U64:
  case RVGPU::S_BFM_B64:
    llvm_unreachable("Moving this op to VALU not implemented");

  case RVGPU::S_PACK_LL_B32_B16:
  case RVGPU::S_PACK_LH_B32_B16:
  case RVGPU::S_PACK_HL_B32_B16:
  case RVGPU::S_PACK_HH_B32_B16:
  ...
  case RVGPU::S_NAND_B32:
    splitScalarNotBinop(Worklist, Inst, RVGPU::S_AND_B32);
    Inst.eraseFromParent();
    return;

  case RVGPU::S_NOR_B32:
    splitScalarNotBinop(Worklist, Inst, RVGPU::S_OR_B32);
    Inst.eraseFromParent();
    return;

  case RVGPU::S_ANDN2_B32:
    splitScalarBinOpN2(Worklist, Inst, RVGPU::S_AND_B32);
    Inst.eraseFromParent();
    return;

  case RVGPU::S_ORN2_B32:
    splitScalarBinOpN2(Worklist, Inst, RVGPU::S_OR_B32);
    Inst.eraseFromParent();
    return;
  ...
  case RVGPU::S_ADD_CO_PSEUDO:
  case RVGPU::S_SUB_CO_PSEUDO: {
    unsigned Opc = (Inst.getOpcode() == RVGPU::S_ADD_CO_PSEUDO)
                       ? RVGPU::V_ADDC_U32_e64
                       : RVGPU::V_SUBB_U32_e64;
    const auto *CarryRC = RI.getRegClass(RVGPU::SReg_1_XEXECRegClassID);
    ...
    if (!MRI.constrainRegClass(CarryInReg, CarryRC)) {
      ...
      BuildMI(*MBB, Inst, Inst.getDebugLoc(), get(RVGPU::COPY), NewCarryReg)
          .addReg(CarryInReg);
    }
    ...
    return;
  case RVGPU::S_UADDO_PSEUDO:
  case RVGPU::S_USUBO_PSEUDO: {
    ...
    unsigned Opc = (Inst.getOpcode() == RVGPU::S_UADDO_PSEUDO)
                       ? RVGPU::V_ADD_CO_U32_e64
                       : RVGPU::V_SUB_CO_U32_e64;
    const TargetRegisterClass *NewRC =
  ...
  case RVGPU::S_CSELECT_B32:
  case RVGPU::S_CSELECT_B64:
    lowerSelect(Worklist, Inst, MDT);
    Inst.eraseFromParent();
    return;
  case RVGPU::S_CMP_EQ_I32:
  case RVGPU::S_CMP_LG_I32:
  case RVGPU::S_CMP_GT_I32:
  case RVGPU::S_CMP_GE_I32:
  case RVGPU::S_CMP_LT_I32:
  case RVGPU::S_CMP_LE_I32:
  case RVGPU::S_CMP_EQ_U32:
  case RVGPU::S_CMP_LG_U32:
  case RVGPU::S_CMP_GT_U32:
  case RVGPU::S_CMP_GE_U32:
  case RVGPU::S_CMP_LT_U32:
  case RVGPU::S_CMP_LE_U32:
  case RVGPU::S_CMP_EQ_U64:
  case RVGPU::S_CMP_LG_U64:
  case RVGPU::S_CMP_LT_F32:
  case RVGPU::S_CMP_EQ_F32:
  case RVGPU::S_CMP_LE_F32:
  case RVGPU::S_CMP_GT_F32:
  case RVGPU::S_CMP_LG_F32:
  case RVGPU::S_CMP_GE_F32:
  case RVGPU::S_CMP_O_F32:
  case RVGPU::S_CMP_U_F32:
  case RVGPU::S_CMP_NGE_F32:
  case RVGPU::S_CMP_NLG_F32:
  case RVGPU::S_CMP_NGT_F32:
  case RVGPU::S_CMP_NLE_F32:
  case RVGPU::S_CMP_NEQ_F32:
  case RVGPU::S_CMP_NLT_F32:
  case RVGPU::S_CMP_LT_F16:
  case RVGPU::S_CMP_EQ_F16:
  case RVGPU::S_CMP_LE_F16:
  case RVGPU::S_CMP_GT_F16:
  case RVGPU::S_CMP_LG_F16:
  case RVGPU::S_CMP_GE_F16:
  case RVGPU::S_CMP_O_F16:
  case RVGPU::S_CMP_U_F16:
  case RVGPU::S_CMP_NGE_F16:
  case RVGPU::S_CMP_NLG_F16:
  case RVGPU::S_CMP_NGT_F16:
  case RVGPU::S_CMP_NLE_F16:
  case RVGPU::S_CMP_NEQ_F16:
  case RVGPU::S_CMP_NLT_F16: {
    ...
    if (RVGPU::getNamedOperandIdx(NewOpcode,
                                   RVGPU::OpName::src0_modifiers) >= 0) {
      NewInstr
          .addImm(0)               // src0_modifiers
    ...
    legalizeOperands(*NewInstr, MDT);
    int SCCIdx = Inst.findRegisterDefOperandIdx(RVGPU::SCC);
    MachineOperand SCCOp = Inst.getOperand(SCCIdx);
    addSCCDefUsersToVALUWorklist(SCCOp, Inst, Worklist, CondReg);
    Inst.eraseFromParent();
    return;
  }
  case RVGPU::S_CVT_HI_F32_F16: {
    const DebugLoc &DL = Inst.getDebugLoc();
    Register TmpReg = MRI.createVirtualRegister(&RVGPU::VGPR_32RegClass);
    Register NewDst = MRI.createVirtualRegister(&RVGPU::VGPR_32RegClass);
    BuildMI(*MBB, Inst, DL, get(RVGPU::V_LSHRREV_B32_e64), TmpReg)
        .addImm(16)
        .add(Inst.getOperand(1));
  ...
  case RVGPU::S_MINIMUM_F32:
  case RVGPU::S_MAXIMUM_F32:
  case RVGPU::S_MINIMUM_F16:
  case RVGPU::S_MAXIMUM_F16: {
    const DebugLoc &DL = Inst.getDebugLoc();
    Register NewDst = MRI.createVirtualRegister(&RVGPU::VGPR_32RegClass);
  ...
  if (NewOpcode == RVGPU::INSTRUCTION_LIST_END) {
  ...
  if (NewOpcode == Opcode) {
    ...
    if (Inst.isCopy() && Inst.getOperand(1).getReg().isVirtual() &&
      ...
      Inst.setDesc(get(RVGPU::IMPLICIT_DEF));
      return;
    }
  ...
  if (isVOP3(NewOpcode) && !isVOP3(Opcode)) {
    ...
    if (RVGPU::getNamedOperandIdx(NewOpcode,
                                   RVGPU::OpName::src0_modifiers) >= 0)
      NewInstr.addImm(0);
    if (RVGPU::getNamedOperandIdx(NewOpcode, RVGPU::OpName::src0) >= 0)
      NewInstr->addOperand(Inst.getOperand(1));

    if (Opcode == RVGPU::S_SEXT_I32_I8 || Opcode == RVGPU::S_SEXT_I32_I16) {
      // We are converting these to a BFE, so we need to add the missing
      // operands for the size and offset.
      unsigned Size = (Opcode == RVGPU::S_SEXT_I32_I8) ? 8 : 16;
      NewInstr.addImm(0);
      NewInstr.addImm(Size);
    } else if (Opcode == RVGPU::S_BCNT1_I32_B32) {
      // The VALU version adds the second operand to the result, so insert an
      // extra 0 operand.
      NewInstr.addImm(0);
    } else if (Opcode == RVGPU::S_BFE_I32 || Opcode == RVGPU::S_BFE_U32) {
      ...
      if (RVGPU::getNamedOperandIdx(NewOpcode,
                                     RVGPU::OpName::src1_modifiers) >= 0)
        NewInstr.addImm(0);
      if (RVGPU::getNamedOperandIdx(NewOpcode, RVGPU::OpName::src1) >= 0)
        NewInstr->addOperand(Inst.getOperand(2));
      if (RVGPU::getNamedOperandIdx(NewOpcode,
                                     RVGPU::OpName::src2_modifiers) >= 0)
        NewInstr.addImm(0);
      if (RVGPU::getNamedOperandIdx(NewOpcode, RVGPU::OpName::src2) >= 0)
        NewInstr->addOperand(Inst.getOperand(3));
      if (RVGPU::getNamedOperandIdx(NewOpcode, RVGPU::OpName::clamp) >= 0)
        NewInstr.addImm(0);
      if (RVGPU::getNamedOperandIdx(NewOpcode, RVGPU::OpName::omod) >= 0)
        NewInstr.addImm(0);
      if (RVGPU::getNamedOperandIdx(NewOpcode, RVGPU::OpName::op_sel) >= 0)
        NewInstr.addImm(0);
    }
  } else {
...
RVInstrInfo::moveScalarAddSub(RVInstrWorklist &Worklist, MachineInstr &Inst,
  ...
  if (ST.hasAddNoCarry()) {
    ...
    unsigned Opc = Inst.getOpcode();
    assert(Opc == RVGPU::S_ADD_I32 || Opc == RVGPU::S_SUB_I32);

    unsigned NewOpc = Opc == RVGPU::S_ADD_I32 ?
      RVGPU::V_ADD_U32_e64 : RVGPU::V_SUB_U32_e64;

    assert(Inst.getOperand(3).getReg() == RVGPU::SCC);
...
void RVInstrInfo::lowerSelect(RVInstrWorklist &Worklist, MachineInstr &Inst,
  ...
  if (IsSCC) {
    ...
    for (MachineInstr &CandI :
         make_range(std::next(MachineBasicBlock::reverse_iterator(Inst)),
                    Inst.getParent()->rend())) {
      if (CandI.findRegisterDefOperandIdx(RVGPU::SCC, false, false, &RI) !=
          -1) {
        if (CandI.isCopy() && CandI.getOperand(0).getReg() == RVGPU::SCC) {
          BuildMI(MBB, MII, DL, get(RVGPU::COPY), NewCondReg)
    ...
    if (!CopyFound) {
      ...
      unsigned Opcode = (ST.getWavefrontSize() == 64) ? RVGPU::S_CSELECT_B64
                                                      : RVGPU::S_CSELECT_B32;
      auto NewSelect =
  ...
  if (Inst.getOpcode() == RVGPU::S_CSELECT_B32) {
    NewInst = BuildMI(MBB, MII, DL, get(RVGPU::V_CNDMASK_B32_e64), NewDestReg)
                  .addImm(0)
                  .add(Src1) // False
                  .addImm(0)
                  .add(Src0) // True
    ...
    NewInst =
        BuildMI(MBB, MII, DL, get(RVGPU::V_CNDMASK_B64_PSEUDO), NewDestReg)
            .add(Src1) // False
            .add(Src0) // True
            .addReg(NewCondReg);
  }
...
void RVInstrInfo::lowerScalarAbs(RVInstrWorklist &Worklist,
  ...
  unsigned SubOp = ST.hasAddNoCarry() ?
    RVGPU::V_SUB_U32_e32 : RVGPU::V_SUB_CO_U32_e32;

  BuildMI(MBB, MII, DL, get(SubOp), TmpReg)
    .addImm(0)
    .addReg(Src.getReg());

  BuildMI(MBB, MII, DL, get(RVGPU::V_MAX_I32_e64), ResultReg)
    .addReg(Src.getReg())
    .addReg(TmpReg);
...
void RVInstrInfo::lowerScalarXnor(RVInstrWorklist &Worklist,
  ...
  if (ST.hasDLInsts()) {
    Register NewDest = MRI.createVirtualRegister(&RVGPU::VGPR_32RegClass);
    legalizeGenericOperand(MBB, MII, &RVGPU::VGPR_32RegClass, Src0, MRI, DL);
    legalizeGenericOperand(MBB, MII, &RVGPU::VGPR_32RegClass, Src1, MRI, DL);

    BuildMI(MBB, MII, DL, get(RVGPU::V_XNOR_B32_e64), NewDest)
      .add(Src0)
      .add(Src1);
    ...
    MachineInstr *Xor;
    Register Temp = MRI.createVirtualRegister(&RVGPU::SReg_32RegClass);
    Register NewDest = MRI.createVirtualRegister(&RVGPU::SReg_32RegClass);
    ...
    // unit as necessary.
    if (Src0IsSGPR) {
      BuildMI(MBB, MII, DL, get(RVGPU::S_NOT_B32), Temp).add(Src0);
      Xor = BuildMI(MBB, MII, DL, get(RVGPU::S_XOR_B32), NewDest)
      .addReg(Temp)
      .add(Src1);
    } else if (Src1IsSGPR) {
      BuildMI(MBB, MII, DL, get(RVGPU::S_NOT_B32), Temp).add(Src1);
      Xor = BuildMI(MBB, MII, DL, get(RVGPU::S_XOR_B32), NewDest)
      .add(Src0)
      .addReg(Temp);
    } else {
      Xor = BuildMI(MBB, MII, DL, get(RVGPU::S_XOR_B32), Temp)
        .add(Src0)
        .add(Src1);
      MachineInstr *Not =
          BuildMI(MBB, MII, DL, get(RVGPU::S_NOT_B32), NewDest).addReg(Temp);
      Worklist.insert(Not);
    }
...
void RVInstrInfo::splitScalarNotBinop(RVInstrWorklist &Worklist,
  ...
  Register NewDest = MRI.createVirtualRegister(&RVGPU::SReg_32RegClass);
  Register Interm = MRI.createVirtualRegister(&RVGPU::SReg_32RegClass);

  MachineInstr &Op = *BuildMI(MBB, MII, DL, get(Opcode), Interm)
    .add(Src0)
    .add(Src1);

  MachineInstr &Not = *BuildMI(MBB, MII, DL, get(RVGPU::S_NOT_B32), NewDest)
    .addReg(Interm);
...
void RVInstrInfo::splitScalarBinOpN2(RVInstrWorklist &Worklist,
  ...
  Register NewDest = MRI.createVirtualRegister(&RVGPU::SReg_32_XM0RegClass);
  Register Interm = MRI.createVirtualRegister(&RVGPU::SReg_32_XM0RegClass);

  MachineInstr &Not = *BuildMI(MBB, MII, DL, get(RVGPU::S_NOT_B32), Interm)
    .add(Src1);
...
void RVInstrInfo::splitScalar64BitUnaryOp(RVInstrWorklist &Worklist,
  ...
  const TargetRegisterClass *Src0SubRC =
      RI.getSubRegisterClass(Src0RC, RVGPU::sub0);

  MachineOperand SrcReg0Sub0 = buildExtractSubRegOrImm(MII, MRI, Src0, Src0RC,
                                                       RVGPU::sub0, Src0SubRC);

  const TargetRegisterClass *DestRC = MRI.getRegClass(Dest.getReg());
  const TargetRegisterClass *NewDestRC = RI.getEquivalentVGPRClass(DestRC);
  const TargetRegisterClass *NewDestSubRC =
      RI.getSubRegisterClass(NewDestRC, RVGPU::sub0);
...
void RVInstrInfo::splitScalar64BitBinaryOp(RVInstrWorklist &Worklist,
  ...
  const TargetRegisterClass *Src1SubRC =
      RI.getSubRegisterClass(Src1RC, RVGPU::sub0);

  MachineOperand SrcReg0Sub0 = buildExtractSubRegOrImm(MII, MRI, Src0, Src0RC,
                                                       RVGPU::sub0, Src0SubRC);
  MachineOperand SrcReg1Sub0 = buildExtractSubRegOrImm(MII, MRI, Src1, Src1RC,
                                                       RVGPU::sub0, Src1SubRC);
  MachineOperand SrcReg0Sub1 = buildExtractSubRegOrImm(MII, MRI, Src0, Src0RC,
                                                       RVGPU::sub1, Src0SubRC);
  MachineOperand SrcReg1Sub1 = buildExtractSubRegOrImm(MII, MRI, Src1, Src1RC,
                                                       RVGPU::sub1, Src1SubRC);
...
void RVInstrInfo::splitScalar64BitXnor(RVInstrWorklist &Worklist,
  ...
  Register Interm = MRI.createVirtualRegister(&RVGPU::SReg_64RegClass);
  ...
  BuildMI(MBB, MII, DL, get(RVGPU::S_NOT_B64), Interm)
    .add(*Op0);

  Register NewDest = MRI.createVirtualRegister(DestRC);

  MachineInstr &Xor = *BuildMI(MBB, MII, DL, get(RVGPU::S_XOR_B64), NewDest)
    .addReg(Interm)
    .add(*Op1);
...
void RVInstrInfo::splitScalar64BitBCNT(RVInstrWorklist &Worklist,
  ...
  const MCInstrDesc &InstDesc = get(RVGPU::V_BCNT_U32_B32_e64);
  const TargetRegisterClass *SrcRC = Src.isReg() ?
    MRI.getRegClass(Src.getReg()) :
    &RVGPU::SGPR_32RegClass;

  Register MidReg = MRI.createVirtualRegister(&RVGPU::VGPR_32RegClass);
  Register ResultReg = MRI.createVirtualRegister(&RVGPU::VGPR_32RegClass);

  const TargetRegisterClass *SrcSubRC =
      RI.getSubRegisterClass(SrcRC, RVGPU::sub0);
...
void RVInstrInfo::splitScalar64BitBFE(RVInstrWorklist &Worklist,
  ...
  assert(Inst.getOpcode() == RVGPU::S_BFE_I64 && BitWidth <= 32 &&
         Offset == 0 && "Not implemented");

  if (BitWidth < 32) {
    Register MidRegLo = MRI.createVirtualRegister(&RVGPU::VGPR_32RegClass);
    Register MidRegHi = MRI.createVirtualRegister(&RVGPU::VGPR_32RegClass);
    Register ResultReg = MRI.createVirtualRegister(&RVGPU::VReg_64RegClass);

    BuildMI(MBB, MII, DL, get(RVGPU::V_BFE_I32_e64), MidRegLo)
        .addReg(Inst.getOperand(1).getReg(), 0, RVGPU::sub0)
        .addImm(0)
        .addImm(BitWidth);

    BuildMI(MBB, MII, DL, get(RVGPU::V_ASHRREV_I32_e32), MidRegHi)
      .addImm(31)
      .addReg(MidRegLo);
  ...
  BuildMI(MBB, MII, DL, get(RVGPU::V_ASHRREV_I32_e64), TmpReg)
    .addImm(31)
    .addReg(Src.getReg(), 0, RVGPU::sub0);
...
void RVInstrInfo::addUsersToMoveToVALUWorklist(
  ...
  for (MachineRegisterInfo::use_iterator I = MRI.use_begin(DstReg),
    ...
    case RVGPU::WQM:
    case RVGPU::SOFT_WQM:
    case RVGPU::STRICT_WWM:
    case RVGPU::STRICT_WQM:
...
void RVInstrInfo::movePackToVALU(RVInstrWorklist &Worklist,
  ...
  case RVGPU::S_PACK_LL_B32_B16: {
    Register ImmReg = MRI.createVirtualRegister(&RVGPU::VGPR_32RegClass);
    Register TmpReg = MRI.createVirtualRegister(&RVGPU::VGPR_32RegClass);

    // FIXME: Can do a lot better if we know the high bits of src0 or src1 are
    // 0.
    BuildMI(*MBB, Inst, DL, get(RVGPU::V_MOV_B32_e32), ImmReg)
      .addImm(0xffff);

    BuildMI(*MBB, Inst, DL, get(RVGPU::V_AND_B32_e64), TmpReg)
      .addReg(ImmReg, RegState::Kill)
      .add(Src0);

    BuildMI(*MBB, Inst, DL, get(RVGPU::V_LSHL_OR_B32_e64), ResultReg)
      .add(Src1)
      .addImm(16)
  ...
  case RVGPU::S_PACK_LH_B32_B16: {
    ...
    BuildMI(*MBB, Inst, DL, get(RVGPU::V_MOV_B32_e32), ImmReg)
      .addImm(0xffff);
    BuildMI(*MBB, Inst, DL, get(RVGPU::V_BFI_B32_e64), ResultReg)
      .addReg(ImmReg, RegState::Kill)
      .add(Src0)
      .add(Src1);
    break;
  ...
  case RVGPU::S_PACK_HH_B32_B16: {
    ...
    BuildMI(*MBB, Inst, DL, get(RVGPU::V_MOV_B32_e32), ImmReg)
      .addImm(0xffff0000);
    BuildMI(*MBB, Inst, DL, get(RVGPU::V_AND_OR_B32_e64), ResultReg)
      .add(Src1)
      .addReg(ImmReg, RegState::Kill)
      .addReg(TmpReg, RegState::Kill);
    break;
...
void RVInstrInfo::addSCCDefUsersToVALUWorklist(MachineOperand &Op,
  ...
  assert(Op.isReg() && Op.getReg() == RVGPU::SCC && Op.isDef() &&
  ...
  for (MachineInstr &MI : // Skip the def inst itself.
    ...
    int SCCIdx = MI.findRegisterUseOperandIdx(RVGPU::SCC, false, &RI);
    if (SCCIdx != -1) {
      if (MI.isCopy()) {
    ...
    if (MI.findRegisterDefOperandIdx(RVGPU::SCC, false, false, &RI) != -1)
      break;
  }
...
void RVInstrInfo::addSCCDefsToVALUWorklist(MachineInstr *SCCUseInst,
  ...
  for (MachineInstr &MI :
    ...
    if (MI.modifiesRegister(RVGPU::VCC, &RI))
      break;
    if (MI.definesRegister(RVGPU::SCC, &RI)) {
      Worklist.insert(&MI);
      break;
    }
  }
...
const TargetRegisterClass *RVInstrInfo::getDestEquivalentVGPRClass(
  ...
  case RVGPU::STRICT_WQM: {
    ...
    if (RI.isAGPRClass(SrcRC)) {
      ...
      if (RI.isVGPRClass(NewDstRC) || NewDstRC == &RVGPU::VReg_1RegClass)
        return nullptr;
...
// Find the one SGPR operand we are allowed to use.
Register RVInstrInfo::findUsedSGPR(const MachineInstr &MI,
...
MachineOperand *RVInstrInfo::getNamedOperand(MachineInstr &MI,
...
uint64_t RVInstrInfo::getDefaultRsrcDataFormat() const {
  ...
  uint64_t RsrcDataFormat = RVGPU::RSRC_DATA_FORMAT;
  if (ST.isRvHsaOS()) {
    // Set ATC = 1. GFX9 doesn't have this bit.
    if (ST.getGeneration() <= RVGPUSubtarget::VOLCANIC_ISLANDS)
...
uint64_t RVInstrInfo::getScratchRsrcWords23() const {
  uint64_t Rsrc23 = getDefaultRsrcDataFormat() |
                    RVGPU::RSRC_TID_ENABLE |
                    0xffffffff; // Size;

  // GFX9 doesn't have ELEMENT_SIZE.
  if (ST.getGeneration() <= RVGPUSubtarget::VOLCANIC_ISLANDS) {
    uint64_t EltSizeValue = Log2_32(ST.getMaxPrivateElementSize(true)) - 1;
    Rsrc23 |= EltSizeValue << RVGPU::RSRC_ELEMENT_SIZE_SHIFT;
  }

  // IndexStride = 64 / 32.
  uint64_t IndexStride = ST.getWavefrontSize() == 64 ? 3 : 2;
  Rsrc23 |= IndexStride << RVGPU::RSRC_INDEX_STRIDE_SHIFT;

  // If TID_ENABLE is set, DATA_FORMAT specifies stride bits [14:17].
  // Clear them unless we want a huge stride.
  if (ST.getGeneration() >= RVGPUSubtarget::VOLCANIC_ISLANDS &&
      ST.getGeneration() <= RVGPUSubtarget::GFX9)
    Rsrc23 &= ~RVGPU::RSRC_DATA_FORMAT;

  return Rsrc23;
}

bool RVInstrInfo::isLowLatencyInstruction(const MachineInstr &MI) const {
  unsigned Opc = MI.getOpcode();

  return isSMRD(Opc);
}

bool RVInstrInfo::isHighLatencyDef(int Opc) const {
  return get(Opc).mayLoad() &&
         (isMUBUF(Opc) || isMTBUF(Opc) || isMIMG(Opc) || isFLAT(Opc));
}

unsigned RVInstrInfo::isStackAccess(const MachineInstr &MI,
                                    int &FrameIndex) const {
  const MachineOperand *Addr = getNamedOperand(MI, RVGPU::OpName::vaddr);
  if (!Addr || !Addr->isFI())
    return Register();

  assert(!MI.memoperands_empty() &&
         (*MI.memoperands_begin())->getAddrSpace() == RVGPUAS::PRIVATE_ADDRESS);

  FrameIndex = Addr->getIndex();
  return getNamedOperand(MI, RVGPU::OpName::vdata)->getReg();
}

unsigned RVInstrInfo::isSGPRStackAccess(const MachineInstr &MI,
                                        int &FrameIndex) const {
  const MachineOperand *Addr = getNamedOperand(MI, RVGPU::OpName::addr);
  assert(Addr && Addr->isFI());
  FrameIndex = Addr->getIndex();
  return getNamedOperand(MI, RVGPU::OpName::data)->getReg();
}

unsigned RVInstrInfo::isLoadFromStackSlot(const MachineInstr &MI,
                                          int &FrameIndex) const {
  if (!MI.mayLoad())
    return Register();
...
unsigned RVInstrInfo::isStoreToStackSlot(const MachineInstr &MI,
                                         int &FrameIndex) const {
  if (!MI.mayStore())
    return Register();
...
unsigned RVInstrInfo::getInstBundleSize(const MachineInstr &MI) const {
  unsigned Size = 0;
...
unsigned RVInstrInfo::getInstSizeInBytes(const MachineInstr &MI) const {
  unsigned Opc = MI.getOpcode();
  ...
  if (isMIMG(MI)) {
    ...
    int RSrcIdx = RVGPU::getNamedOperandIdx(Opc, RVGPU::OpName::srsrc);
...
bool RVInstrInfo::mayAccessFlatAddressSpace(const MachineInstr &MI) const {
  if (!isFLAT(MI))
    return false;
  ...
  for (const MachineMemOperand *MMO : MI.memoperands()) {
    if (MMO->getAddrSpace() == RVGPUAS::FLAT_ADDRESS)
      return true;
  }
  return false;
}

bool RVInstrInfo::isNonUniformBranchInstr(MachineInstr &Branch) const {
  return Branch.getOpcode() == RVGPU::SI_NON_UNIFORM_BRCOND_PSEUDO;
}

void RVInstrInfo::convertNonUniformIfRegion(MachineBasicBlock *IfEntry,
  ...
  if (Branch->getOpcode() == RVGPU::SI_NON_UNIFORM_BRCOND_PSEUDO) {
    Register DstReg = MRI.createVirtualRegister(RI.getBoolRC());
    MachineInstr *SIIF =
        BuildMI(*MF, Branch->getDebugLoc(), get(RVGPU::SI_IF), DstReg)
            .add(Branch->getOperand(0))
            .add(Branch->getOperand(1));
    MachineInstr *SIEND =
        BuildMI(*MF, Branch->getDebugLoc(), get(RVGPU::SI_END_CF))
            .addReg(DstReg);
...
void RVInstrInfo::convertNonUniformLoopRegion(
  ...
  if (Branch->getOpcode() == RVGPU::SI_NON_UNIFORM_BRCOND_PSEUDO) {
    ...
    MachineInstr *SIIFBREAK = BuildMI(*(MF), Branch->getDebugLoc(),
                                      get(RVGPU::SI_IF_BREAK), BackEdgeReg)
                                  .addReg(DstReg)
                                  .add(Branch->getOperand(0));
    MachineInstr *SILOOP =
        BuildMI(*(MF), Branch->getDebugLoc(), get(RVGPU::SI_LOOP))
            .addReg(BackEdgeReg)
            .addMBB(LoopEntry);
...
RVInstrInfo::getSerializableTargetIndices() const {
  static const std::pair<int, const char *> TargetIndices[] = {
      {RVGPU::TI_CONSTDATA_START, "rvgpu-constdata-start"},
      {RVGPU::TI_SCRATCH_RSRC_DWORD0, "rvgpu-scratch-rsrc-dword0"},
      {RVGPU::TI_SCRATCH_RSRC_DWORD1, "rvgpu-scratch-rsrc-dword1"},
      {RVGPU::TI_SCRATCH_RSRC_DWORD2, "rvgpu-scratch-rsrc-dword2"},
      {RVGPU::TI_SCRATCH_RSRC_DWORD3, "rvgpu-scratch-rsrc-dword3"}};
...
RVInstrInfo::CreateTargetPostRAHazardRecognizer(const InstrItineraryData *II,
...
/// pass.
ScheduleHazardRecognizer *
RVInstrInfo::CreateTargetPostRAHazardRecognizer(const MachineFunction &MF) const {
...
ScheduleHazardRecognizer *
RVInstrInfo::CreateTargetMIHazardRecognizer(const InstrItineraryData *II,
...
RVInstrInfo::decomposeMachineOperandsTargetFlags(unsigned TF) const {
  return std::pair(TF & MO_MASK, TF & ~MO_MASK);
}

ArrayRef<std::pair<unsigned, const char *>>
RVInstrInfo::getSerializableDirectMachineOperandTargetFlags() const {
  static const std::pair<unsigned, const char *> TargetFlags[] = {
    { MO_GOTPCREL, "rvgpu-gotprel" },
    { MO_GOTPCREL32_LO, "rvgpu-gotprel32-lo" },
    { MO_GOTPCREL32_HI, "rvgpu-gotprel32-hi" },
    { MO_REL32_LO, "rvgpu-rel32-lo" },
    { MO_REL32_HI, "rvgpu-rel32-hi" },
    { MO_ABS32_LO, "rvgpu-abs32-lo" },
    { MO_ABS32_HI, "rvgpu-abs32-hi" },
  };
...
RVInstrInfo::getSerializableMachineMemOperandTargetFlags() const {
  static const std::pair<MachineMemOperand::Flags, const char *> TargetFlags[] =
      {
          {MONoClobber, "rvgpu-noclobber"},
      };

  return ArrayRef(TargetFlags);
}

unsigned RVInstrInfo::getLiveRangeSplitOpcode(Register SrcReg,
                                              const MachineFunction &MF) const {
  const RVMachineFunctionInfo *MFI = MF.getInfo<RVMachineFunctionInfo>();
  assert(SrcReg.isVirtual());
  if (MFI->checkFlag(SrcReg, RVGPU::VirtRegFlag::WWM_REG))
    return RVGPU::WWM_COPY;

  return RVGPU::COPY;
}

bool RVInstrInfo::isBasicBlockPrologue(const MachineInstr &MI,
                                       Register Reg) const {
  ...
  return IsNullOrVectorRegister &&
         (isSpillOpcode(Opc) || (!MI.isTerminator() && Opc != RVGPU::COPY &&
                                 MI.modifiesRegister(RVGPU::EXEC, &RI)));
}

MachineInstrBuilder
RVInstrInfo::getAddNoCarry(MachineBasicBlock &MBB,
                           MachineBasicBlock::iterator I,
                           const DebugLoc &DL,
                           Register DestReg) const {
  if (ST.hasAddNoCarry())
    return BuildMI(MBB, I, DL, get(RVGPU::V_ADD_U32_e64), DestReg);

  MachineRegisterInfo &MRI = MBB.getParent()->getRegInfo();
  Register UnusedCarry = MRI.createVirtualRegister(RI.getBoolRC());
  MRI.setRegAllocationHint(UnusedCarry, 0, RI.getVCC());

  return BuildMI(MBB, I, DL, get(RVGPU::V_ADD_CO_U32_e64), DestReg)
           .addReg(UnusedCarry, RegState::Define | RegState::Dead);
}

MachineInstrBuilder RVInstrInfo::getAddNoCarry(MachineBasicBlock &MBB,
                                               MachineBasicBlock::iterator I,
                                               const DebugLoc &DL,
                                               Register DestReg,
                                               RegScavenger &RS) const {
  if (ST.hasAddNoCarry())
    return BuildMI(MBB, I, DL, get(RVGPU::V_ADD_U32_e32), DestReg);

  // If available, prefer to use vcc.
  Register UnusedCarry = !RS.isRegUsed(RVGPU::VCC)
                             ? Register(RI.getVCC())
...
bool RVInstrInfo::isKillTerminator(unsigned Opcode) {
  switch (Opcode) {
  case RVGPU::SI_KILL_F32_COND_IMM_TERMINATOR:
  case RVGPU::SI_KILL_I1_TERMINATOR:
    return true;
...
const MCInstrDesc &RVInstrInfo::getKillTerminatorFromPseudo(unsigned Opcode) const {
  switch (Opcode) {
  case RVGPU::SI_KILL_F32_COND_IMM_PSEUDO:
    return get(RVGPU::SI_KILL_F32_COND_IMM_TERMINATOR);
  case RVGPU::SI_KILL_I1_PSEUDO:
    return get(RVGPU::SI_KILL_I1_TERMINATOR);
  default:
    llvm_unreachable("invalid opcode, expected SI_KILL_*_PSEUDO");
  }
}

bool RVInstrInfo::isLegalMUBUFImmOffset(unsigned Imm) const {
...
void RVInstrInfo::fixImplicitOperands(MachineInstr &MI) const {
  if (!ST.isWave32())
    return;
  ...
  for (auto &Op : MI.implicit_operands()) {
    if (Op.isReg() && Op.getReg() == RVGPU::VCC)
      Op.setReg(RVGPU::VCC_LO);
  }
}

bool RVInstrInfo::isBufferSMRD(const MachineInstr &MI) const {
  if (!isSMRD(MI))
    return false;

  // Check that it is using a buffer resource.
  int Idx = RVGPU::getNamedOperandIdx(MI.getOpcode(), RVGPU::OpName::sbase);
  if (Idx == -1) // e.g. s_memtime
    return false;

  const auto RCID = MI.getDesc().operands()[Idx].RegClass;
  return RI.getRegClass(RCID)->hasSubClassEq(&RVGPU::SGPR_128RegClass);
}
...
bool RVInstrInfo::splitMUBUFOffset(uint32_t Imm, uint32_t &SOffset,
                                   uint32_t &ImmOffset, Align Alignment) const {
  const uint32_t MaxOffset = RVInstrInfo::getMaxMUBUFImmOffset(ST);
  ...
  if (Overflow > 0) {
    ...
    if (ST.getGeneration() <= RVGPUSubtarget::SEA_ISLANDS)
      return false;
...
bool RVInstrInfo::isLegalFLATOffset(int64_t Offset, unsigned AddrSpace,
  ...
  if (ST.hasFlatSegmentOffsetBug() && FlatVariant == SIInstrFlags::FLAT &&
      (AddrSpace == RVGPUAS::FLAT_ADDRESS ||
       AddrSpace == RVGPUAS::GLOBAL_ADDRESS))
    return false;
  ...
  unsigned N = RVGPU::getNumFlatOffsetBits(ST);
  return isIntN(N, Offset) && (AllowNegative || Offset >= 0);
}

// See comment on RVInstrInfo::isLegalFLATOffset for what is legal and what not.
std::pair<int64_t, int64_t>
RVInstrInfo::splitFlatOffset(int64_t COffsetVal, unsigned AddrSpace,
                             uint64_t FlatVariant) const {
  int64_t RemainderOffset = COffsetVal;
  int64_t ImmField = 0;

  bool AllowNegative = allowNegativeFlatOffset(FlatVariant);
  const unsigned NumBits = RVGPU::getNumFlatOffsetBits(ST) - 1;
...
bool RVInstrInfo::allowNegativeFlatOffset(uint64_t FlatVariant) const {
...
static unsigned subtargetEncodingFamily(const RVSubtarget &ST) {
  ...
  default:
    break;
  case RVGPUSubtarget::SOUTHERN_ISLANDS:
  case RVGPUSubtarget::SEA_ISLANDS:
    return SIEncodingFamily::SI;
  case RVGPUSubtarget::VOLCANIC_ISLANDS:
  case RVGPUSubtarget::GFX9:
...
bool RVInstrInfo::isAsmOnlyOpcode(int MCOp) const {
  switch(MCOp) {
  ...
  case RVGPU::V_MOVRELS_B32_dpp_gfx10:
  case RVGPU::V_MOVRELS_B32_sdwa_gfx10:
  case RVGPU::V_MOVRELD_B32_dpp_gfx10:
  case RVGPU::V_MOVRELD_B32_sdwa_gfx10:
  case RVGPU::V_MOVRELSD_B32_dpp_gfx10:
  case RVGPU::V_MOVRELSD_B32_sdwa_gfx10:
  case RVGPU::V_MOVRELSD_2_B32_dpp_gfx10:
  case RVGPU::V_MOVRELSD_2_B32_sdwa_gfx10:
    return true;
  default:
    return false;
  }
...
int RVInstrInfo::pseudoToMCOpcode(int Opcode) const {
  if (RVInstrInfo::isSoftWaitcnt(Opcode))
    Opcode = RVInstrInfo::getNonSoftWaitcntOpcode(Opcode);
  ...
  if (isMAI(Opcode)) {
    int MFMAOp = RVGPU::getMFMAEarlyClobberOp(Opcode);
    if (MFMAOp != -1)
      Opcode = MFMAOp;
  }

  int MCOp = RVGPU::getMCOpcode(Opcode, Gen);
  ...
  if (ST.hasGFX90AInsts()) {
    ...
    if (ST.hasGFX940Insts())
      NMCOp = RVGPU::getMCOpcode(Opcode, SIEncodingFamily::GFX940);
    if (NMCOp == (uint16_t)-1)
      NMCOp = RVGPU::getMCOpcode(Opcode, SIEncodingFamily::GFX90A);
    if (NMCOp == (uint16_t)-1)
      NMCOp = RVGPU::getMCOpcode(Opcode, SIEncodingFamily::GFX9);
    if (NMCOp != (uint16_t)-1)
      MCOp = NMCOp;
  }
...
bool llvm::rvExecMayBeModifiedBeforeUse(const MachineRegisterInfo &MRI,
  ...
  for (auto I = std::next(DefMI.getIterator()); I != E; ++I) {
    ...
    if (I->modifiesRegister(RVGPU::EXEC, TRI))
      return true;
  }
    ...
    for (const MachineOperand &Op : I->operands()) {
      ...
      if (Op.isUse()) {
        if (Reg == VReg && --NumUse == 0)
          return false;
      } else if (TRI->regsOverlap(Reg, RVGPU::EXEC))
        return true;
    }
  }
}

MachineInstr *RVInstrInfo::createPHIDestinationCopy(
...
MachineInstr *RVInstrInfo::createPHISourceCopy(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator InsPt,
    const DebugLoc &DL, Register Src, unsigned SrcSubReg, Register Dst) const {
  if (InsPt != MBB.end() &&
      (InsPt->getOpcode() == RVGPU::SI_IF ||
       InsPt->getOpcode() == RVGPU::SI_ELSE ||
       InsPt->getOpcode() == RVGPU::SI_IF_BREAK) &&
      InsPt->definesRegister(Src)) {
    InsPt++;
    return BuildMI(MBB, InsPt, DL,
                   get(ST.isWave32() ? RVGPU::S_MOV_B32_term
                                     : RVGPU::S_MOV_B64_term),
                   Dst)
        .addReg(Src, 0, SrcSubReg)
        .addReg(RVGPU::EXEC, RegState::Implicit);
  }
  return TargetInstrInfo::createPHISourceCopy(MBB, InsPt, DL, Src, SrcSubReg,
                                              Dst);
}

bool llvm::RVInstrInfo::isWave32() const { return ST.isWave32(); }

MachineInstr *RVInstrInfo::foldMemoryOperandImpl(
  ...
  if (isFullCopyInstr(MI)) {
    ...
    if ((DstReg.isVirtual() || SrcReg.isVirtual()) &&
      ...
      if (RC->hasSuperClassEq(&RVGPU::SReg_32RegClass)) {
        MRI.constrainRegClass(VirtReg, &RVGPU::SReg_32_XM0_XEXECRegClass);
        return nullptr;
      } else if (RC->hasSuperClassEq(&RVGPU::SReg_64RegClass)) {
        MRI.constrainRegClass(VirtReg, &RVGPU::SReg_64_XEXECRegClass);
        return nullptr;
      }
    }
  }
...
unsigned RVInstrInfo::getInstrLatency(const InstrItineraryData *ItinData,
...
RVInstrInfo::getGenericInstructionUniformity(const MachineInstr &MI) const {
  unsigned opcode = MI.getOpcode();
  if (auto *GI = dyn_cast<GIntrinsic>(&MI)) {
    auto IID = GI->getIntrinsicID();
    if (RVGPU::isIntrinsicSourceOfDivergence(IID))
      return InstructionUniformity::NeverUniform;
    if (RVGPU::isIntrinsicAlwaysUniform(IID))
      return InstructionUniformity::AlwaysUniform;

    switch (IID) {
    case Intrinsic::rvgpu_if:
  ...
  if (opcode == RVGPU::G_LOAD) {
    ...
    if (llvm::any_of(MI.memoperands(), [](const MachineMemOperand *mmo) {
          return mmo->getAddrSpace() == RVGPUAS::PRIVATE_ADDRESS ||
                 mmo->getAddrSpace() == RVGPUAS::FLAT_ADDRESS;
        })) {
  ...
  if (RVInstrInfo::isGenericAtomicRMWOpcode(opcode) ||
      opcode == RVGPU::G_ATOMIC_CMPXCHG ||
      opcode == RVGPU::G_ATOMIC_CMPXCHG_WITH_SUCCESS ||
      RVGPU::isGenericAtomic(opcode)) {
    return InstructionUniformity::NeverUniform;
  }
  return InstructionUniformity::Default;
}
...
InstructionUniformity
RVInstrInfo::getInstructionUniformity(const MachineInstr &MI) const {

  if (isNeverUniform(MI))
    return InstructionUniformity::NeverUniform;

  unsigned opcode = MI.getOpcode();
  if (opcode == RVGPU::V_READLANE_B32 ||
      opcode == RVGPU::V_READFIRSTLANE_B32 ||
      opcode == RVGPU::SI_RESTORE_S32_FROM_VGPR)
    return InstructionUniformity::AlwaysUniform;
  ...
  // GMIR handling
  if (MI.isPreISelOpcode())
    return RVInstrInfo::getGenericInstructionUniformity(MI);
  ...
  const MachineRegisterInfo &MRI = MI.getParent()->getParent()->getRegInfo();
  const RVGPURegisterBankInfo *RBI = ST.getRegBankInfo();
  ...
  for (unsigned I = 0, E = MI.getNumOperands(); I != E; ++I) {
    ...
    if (RegBank && RegBank->getID() != RVGPU::SGPRRegBankID)
      return InstructionUniformity::NeverUniform;
  }
  ...
  // TODO: rvgpu.{ballot, [if]cmp} should be AlwaysUniform, but they are
...
unsigned RVInstrInfo::getDSShaderTypeValue(const MachineFunction &MF) {
...
bool RVInstrInfo::analyzeCompare(const MachineInstr &MI, Register &SrcReg,
  ...
  case RVGPU::S_CMP_LG_U64:
    ...
    CmpMask = ~0;
    return true;
  case RVGPU::S_CMPK_EQ_U32:
  case RVGPU::S_CMPK_EQ_I32:
  case RVGPU::S_CMPK_LG_U32:
  case RVGPU::S_CMPK_LG_I32:
  case RVGPU::S_CMPK_LT_U32:
  case RVGPU::S_CMPK_LT_I32:
  case RVGPU::S_CMPK_GT_U32:
  case RVGPU::S_CMPK_GT_I32:
  case RVGPU::S_CMPK_LE_U32:
  case RVGPU::S_CMPK_LE_I32:
  case RVGPU::S_CMPK_GE_U32:
  case RVGPU::S_CMPK_GE_I32:
    SrcReg = MI.getOperand(0).getReg();
    SrcReg2 = Register();
    CmpValue = MI.getOperand(1).getImm();
    CmpMask = ~0;
...
bool RVInstrInfo::optimizeCompareInstr(MachineInstr &CmpInstr, Register SrcReg,
                                       Register SrcReg2, int64_t CmpMask,
                                       int64_t CmpValue,
                                       const MachineRegisterInfo *MRI) const {
  ...
  const auto optimizeCmpAnd = [&CmpInstr, SrcReg, CmpValue, MRI,
    ...
    if (Def->getOpcode() != RVGPU::S_AND_B32 &&
        Def->getOpcode() != RVGPU::S_AND_B64)
      return false;
    ...
    for (auto I = std::next(Def->getIterator()), E = CmpInstr.getIterator();
         I != E; ++I) {
      if (I->modifiesRegister(RVGPU::SCC, &RI) ||
          I->killsRegister(RVGPU::SCC, &RI))
        return false;
    }

    MachineOperand *SccDef = Def->findRegisterDefOperand(RVGPU::SCC);
    ...
    unsigned NewOpc = (SrcSize == 32) ? IsReversedCC ? RVGPU::S_BITCMP0_B32
                                                     : RVGPU::S_BITCMP1_B32
                                      : IsReversedCC ? RVGPU::S_BITCMP0_B64
                                                     : RVGPU::S_BITCMP1_B64;
...
void RVInstrInfo::enforceOperandRCAlignment(MachineInstr &MI,
                                            unsigned OpName) const {
  if (!ST.needsAlignedVGPRs())
    return;

  int OpNo = RVGPU::getNamedOperandIdx(MI.getOpcode(), OpName);
  if (OpNo < 0)
    return;
  ...
  Register Undef = MRI.createVirtualRegister(
      IsAGPR ? &RVGPU::AGPR_32RegClass : &RVGPU::VGPR_32RegClass);
  BuildMI(*BB, MI, DL, get(RVGPU::IMPLICIT_DEF), Undef);
  Register NewVR =
      MRI.createVirtualRegister(IsAGPR ? &RVGPU::AReg_64_Align2RegClass
                                       : &RVGPU::VReg_64_Align2RegClass);
  BuildMI(*BB, MI, DL, get(RVGPU::REG_SEQUENCE), NewVR)
      .addReg(DataReg, 0, Op.getSubReg())
      .addImm(RVGPU::sub0)
      .addReg(Undef)
      .addImm(RVGPU::sub1);
  Op.setReg(NewVR);
  Op.setSubReg(RVGPU::sub0);
