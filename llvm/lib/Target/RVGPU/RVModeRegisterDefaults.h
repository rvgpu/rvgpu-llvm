...
#ifndef LLVM_LIB_TARGET_RVGPU_SIMODEREGISTERDEFAULTS_H
#define LLVM_LIB_TARGET_RVGPU_SIMODEREGISTERDEFAULTS_H
...
struct RVModeRegisterDefaults {
  ...
  RVModeRegisterDefaults() :
    IEEE(true),
    DX10Clamp(true),
    FP32Denormals(DenormalMode::getIEEE()),
    FP64FP16Denormals(DenormalMode::getIEEE()) {}

  RVModeRegisterDefaults(const Function &F, const RVGPUSubtarget &ST);

  static RVModeRegisterDefaults getDefaultForCallingConv(CallingConv::ID CC) {
    RVModeRegisterDefaults Mode;
    Mode.IEEE = 0;
    return Mode;
  }

  bool operator==(const RVModeRegisterDefaults Other) const {
  ...
  // be able to override.
  bool isInlineCompatible(RVModeRegisterDefaults CalleeMode) const {
...
#endif // LLVM_LIB_TARGET_RVGPU_SIMODEREGISTERDEFAULTS_H
