//===- RVInstrInfo.h - SI Instruction Info Interface ------------*- C++ -*-===//
//
...
//
/// \file
/// Interface definition for RVInstrInfo.
//
//===----------------------------------------------------------------------===//

#ifndef LLVM_LIB_TARGET_RVGPU_SIINSTRINFO_H
#define LLVM_LIB_TARGET_RVGPU_SIINSTRINFO_H
...
public:
  enum TargetOperandFlags {
    ...
    MO_NONE = 0,
    // MO_GOTPCREL -> symbol@GOTPCREL -> R_RVGPU_GOTPCREL.
    MO_GOTPCREL = 1,
    // MO_GOTPCREL32_LO -> symbol@gotpcrel32@lo -> R_RVGPU_GOTPCREL32_LO.
    MO_GOTPCREL32 = 2,
    MO_GOTPCREL32_LO = 2,
    // MO_GOTPCREL32_HI -> symbol@gotpcrel32@hi -> R_RVGPU_GOTPCREL32_HI.
    MO_GOTPCREL32_HI = 3,
    // MO_REL32_LO -> symbol@rel32@lo -> R_RVGPU_REL32_LO.
    MO_REL32 = 4,
    MO_REL32_LO = 4,
    // MO_REL32_HI -> symbol@rel32@hi -> R_RVGPU_REL32_HI.
    MO_REL32_HI = 5,
  ...
  // DstRC, then RVGPU::COPY is returned.
  ...
  static bool isDualSourceBlendEXP(const MachineInstr &MI) {
    ...
    return Target == RVGPU::Exp::ET_DUAL_SRC_BLEND0 ||
           Target == RVGPU::Exp::ET_DUAL_SRC_BLEND1;
  }
  ...
  static bool mayWriteLDSThroughDMA(const MachineInstr &MI) {
    return isLDSDMA(MI) && MI.getOpcode() != RVGPU::BUFFER_STORE_LDS_DWORD;
  }
  ...
  static bool isWWMRegSpillOpcode(uint16_t Opcode) {
    return Opcode == RVGPU::SI_SPILL_WWM_V32_SAVE ||
           Opcode == RVGPU::SI_SPILL_WWM_AV32_SAVE ||
           Opcode == RVGPU::SI_SPILL_WWM_V32_RESTORE ||
           Opcode == RVGPU::SI_SPILL_WWM_AV32_RESTORE;
  }

  static bool isChainCallOpcode(uint64_t Opcode) {
    return Opcode == RVGPU::SI_CS_CHAIN_TC_W32 ||
           Opcode == RVGPU::SI_CS_CHAIN_TC_W64;
  }
  ...
  static bool isMFMA(const MachineInstr &MI) {
    return isMAI(MI) && MI.getOpcode() != RVGPU::V_ACCVGPR_WRITE_B32_e64 &&
           MI.getOpcode() != RVGPU::V_ACCVGPR_READ_B32_e64;
  }
  ...
  bool isInlineConstant(const MachineInstr &MI, unsigned OpIdx,
    ...
    if (isCopyInstr(MI)) {
      ...
      uint8_t OpType = (Size == 8) ?
        RVGPU::OPERAND_REG_IMM_INT64 : RVGPU::OPERAND_REG_IMM_INT32;
  ...
  int64_t getNamedImmOperand(const MachineInstr &MI, unsigned OpName) const {
    int Idx = RVGPU::getNamedOperandIdx(MI.getOpcode(), OpName);
  ...
  const MIRFormatter *getMIRFormatter() const override {
    if (!Formatter.get())
      Formatter = std::make_unique<RVGPUMIRFormatter>();
...
#endif // LLVM_LIB_TARGET_RVGPU_SIINSTRINFO_H
