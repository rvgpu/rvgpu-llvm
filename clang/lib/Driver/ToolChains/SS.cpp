...
void SSInstallationDetector::WarnIfUnsupportedVersion() {
...
void RVGPU::Assembler::ConstructJob(Compilation &C, const JobAction &JA,
...
// correspond to.
void RVGPU::FatBinary::ConstructJob(Compilation &C, const JobAction &JA,
                                    const InputInfo &Output,
                                    const InputInfoList &Inputs,
                                    const ArgList &Args,
                                    const char *LinkingOutput) const {
...
void RVGPU::getRVGPUTargetFeatures(const Driver &D, const llvm::Triple &Triple,
                                   const llvm::opt::ArgList &Args,
  ...
  SSInstallationDetector CudaInstallation(D, Triple, Args);
...
RVGPUToolChain::RVGPUToolChain(const Driver &D, const llvm::Triple &Triple,
                               const ArgList &Args)
    : RVGPUToolChain(D, Triple, llvm::Triple(LLVM_HOST_TRIPLE), Args,
                     /*Freestanding=*/true) {}

llvm::opt::DerivedArgList *
RVGPUToolChain::TranslateArgs(const llvm::opt::DerivedArgList &Args,
                              StringRef BoundArch,
...
bool RVGPUToolChain::supportsDebugInfoOption(const llvm::opt::Arg *A) const {
...
void RVGPUToolChain::adjustDebugInfoKind(
...
SSToolChain::SSToolChain(const Driver &D, const llvm::Triple &Triple,
    ...
    : RVGPUToolChain(D, Triple, HostTC.getTriple(), Args), HostTC(HostTC) {}
...
llvm::opt::DerivedArgList *
SSToolChain::TranslateArgs(const llvm::opt::DerivedArgList &Args,
                             StringRef BoundArch,
                             Action::OffloadKind DeviceOffloadKind) const {
  DerivedArgList *DAL =
...
SSToolChain::getSystemGPUArchs(const ArgList &Args) const {
...
void SSToolChain::addClangWarningOptions(ArgStringList &CC1Args) const {
  HostTC.addClangWarningOptions(CC1Args);
}

ToolChain::CXXStdlibType
SSToolChain::GetCXXStdlibType(const ArgList &Args) const {
  return HostTC.GetCXXStdlibType(Args);
}

void SSToolChain::AddClangSystemIncludeArgs(const ArgList &DriverArgs,
                                            ArgStringList &CC1Args) const {
...
void SSToolChain::AddClangCXXStdlibIncludeArgs(const ArgList &Args,
                                               ArgStringList &CC1Args) const {
  HostTC.AddClangCXXStdlibIncludeArgs(Args, CC1Args);
}

void SSToolChain::AddIAMCUIncludeArgs(const ArgList &Args,
                                      ArgStringList &CC1Args) const {
  HostTC.AddIAMCUIncludeArgs(Args, CC1Args);
}

SanitizerMask SSToolChain::getSupportedSanitizers() const {
...
VersionTuple SSToolChain::computeMSVCVersion(const Driver *D,
                                             const ArgList &Args) const {
