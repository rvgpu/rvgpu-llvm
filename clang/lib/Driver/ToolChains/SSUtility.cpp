//===--- SSUtility.cpp - Common SS Tool Chain Utilities -------*- C++ -*-===//
//
...
namespace {
const unsigned SSCodeObjectAlign = 4096;
} // namespace
...
// different devices into a SS fat binary.
void SS::constructSSFatbinCommand(Compilation &C, const JobAction &JA,
  ...
  BundlerArgs.push_back(
      Args.MakeArgString("-bundle-align=" + Twine(SSCodeObjectAlign)));
  ...
  //if (TT.isRVGPU() && getRVGPUCodeObjectVersion(C.getDriver(), Args) >= 4)
...
/// Add Generated SS Object File which has device images embedded into the
...
void SS::constructGenerateObjFileFromSSFatBinary(
  ...
  SS::constructSSFatbinCommand(C, JA, BundleFile, Inputs, Args, T);
  ...
  ObjStream << "  .p2align " << llvm::Log2(llvm::Align(SSCodeObjectAlign))
            << "\n";
