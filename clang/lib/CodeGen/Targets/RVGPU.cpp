...
class RVGPUTargetCodeGenInfo;

class RVGPUABIInfo : public ABIInfo {
  RVGPUTargetCodeGenInfo &CGInfo;

public:
  RVGPUABIInfo(CodeGenTypes &CGT, RVGPUTargetCodeGenInfo &Info)
...
public:
  RVGPUTargetCodeGenInfo(CodeGenTypes &CGT)
      : TargetCodeGenInfo(std::make_unique<RVGPUABIInfo>(CGT, *this)) {}
  ...
  static void emitBuiltinSurfTexDeviceCopy(CodeGenFunction &CGF, LValue Dst,
    ...
    if (auto *GV = llvm::dyn_cast_or_null<llvm::GlobalVariable>(C)) {
      ...
      Handle = CGF.EmitRuntimeCall(
          CGF.CGM.getIntrinsic(llvm::Intrinsic::rvgpu_texsurf_handle_internal,
                               {GV->getType()}),
          {GV}, "texsurf_handle");
    } else
...
bool RVGPUABIInfo::isUnsupportedType(QualType T) const {
...
ABIArgInfo RVGPUABIInfo::coerceToIntArrayWithLimit(QualType Ty,
...
ABIArgInfo RVGPUABIInfo::classifyArgumentType(QualType Ty) const {
  // Treat an enum type as its underlying type.
...
void RVGPUTargetCodeGenInfo::setTargetAttributes(
  ...
  // Attach kernel metadata directly if compiling for RVGPU.
  if (FD->hasAttr<NVPTXKernelAttr>()) {
    addNVVMMetadata(F, "kernel", 1);
  }
}

void RVGPUTargetCodeGenInfo::addNVVMMetadata(llvm::GlobalValue *GV,
...
void CodeGenModule::handleSSLaunchBoundsAttr(llvm::Function *F,
  ...
  if (MaxThreads > 0) {
    ...
    if (F) {
      // Create !{<func-ref>, metadata !"maxntidx", i32 <val>} node
      RVGPUTargetCodeGenInfo::addNVVMMetadata(F, "maxntidx",
  ...
  if (Attr->getMinBlocks()) {
    ...
    if (MinBlocks > 0) {
      ...
      if (F) {
        ...
        RVGPUTargetCodeGenInfo::addNVVMMetadata(F, "minctasm",
  ...
  if (Attr->getMaxBlocks()) {
    ...
    if (MaxBlocks > 0) {
      ...
      if (F) {
        ...
        RVGPUTargetCodeGenInfo::addNVVMMetadata(F, "maxclusterrank",
