...
//===----------------------------------------------------------------------===//
// RVGPU ABI Implementation
//===----------------------------------------------------------------------===//

namespace {

class RVGPUABIInfo final : public DefaultABIInfo {
private:
...
public:
  explicit RVGPUABIInfo(CodeGen::CodeGenTypes &CGT) :
    DefaultABIInfo(CGT) {}
...
bool RVGPUABIInfo::isHomogeneousAggregateBaseType(QualType Ty) const {
  return true;
}

bool RVGPUABIInfo::isHomogeneousAggregateSmallEnough(
...
unsigned RVGPUABIInfo::numRegsForType(QualType Ty) const {
  unsigned NumRegs = 0;
...
void RVGPUABIInfo::computeInfo(CGFunctionInfo &FI) const {
  ...
  for (auto &Arg : FI.arguments()) {
    if (CC == llvm::CallingConv::RVGPU_KERNEL) {
...
Address RVGPUABIInfo::EmitVAArg(CodeGenFunction &CGF, Address VAListAddr,
                                 QualType Ty) const {
  llvm_unreachable("RVGPU does not support varargs");
}

ABIArgInfo RVGPUABIInfo::classifyReturnType(QualType RetTy) const {
...
ABIArgInfo RVGPUABIInfo::classifyKernelArgumentType(QualType Ty) const {
...
ABIArgInfo RVGPUABIInfo::classifyArgumentType(QualType Ty,
...
class RVGPUTargetCodeGenInfo : public TargetCodeGenInfo {
public:
  RVGPUTargetCodeGenInfo(CodeGenTypes &CGT)
      : TargetCodeGenInfo(std::make_unique<RVGPUABIInfo>(CGT)) {}
...
static bool requiresRVGPUProtectedVisibility(const Decl *D,
                                              llvm::GlobalValue *GV) {
...
void RVGPUTargetCodeGenInfo::setFunctionDeclAttributes(
  ...
  const auto *FlatWGS = FD->getAttr<RVGPUFlatWorkGroupSizeAttr>();
  if (ReqdWGS || FlatWGS) {
    M.handleRVGPUFlatWorkGroupSizeAttr(F, FlatWGS, ReqdWGS);
    ...
    F->addFnAttr("rvgpu-flat-work-group-size", AttrVal);
  }

  if (const auto *Attr = FD->getAttr<RVGPUWavesPerEUAttr>())
    M.handleRVGPUWavesPerEUAttr(F, Attr);

  if (const auto *Attr = FD->getAttr<RVGPUNumSGPRAttr>()) {
    unsigned NumSGPR = Attr->getNumSGPR();

    if (NumSGPR != 0)
      F->addFnAttr("rvgpu-num-sgpr", llvm::utostr(NumSGPR));
  }

  if (const auto *Attr = FD->getAttr<RVGPUNumVGPRAttr>()) {
    uint32_t NumVGPR = Attr->getNumVGPR();

    if (NumVGPR != 0)
      F->addFnAttr("rvgpu-num-vgpr", llvm::utostr(NumVGPR));
  }
}

/// Emits control constants used to change per-architecture behaviour in the
/// RVGPU ROCm device libraries.
void RVGPUTargetCodeGenInfo::emitTargetGlobals(
...
void RVGPUTargetCodeGenInfo::setTargetAttributes(
    const Decl *D, llvm::GlobalValue *GV, CodeGen::CodeGenModule &M) const {
  if (requiresRVGPUProtectedVisibility(D, GV)) {
  ...
  if (M.getContext().getTargetInfo().allowRVGPUUnsafeFPAtomics())
    F->addFnAttr("rvgpu-unsafe-fp-atomics", "true");

  if (!getABIInfo().getCodeGenOpts().EmitIEEENaNCompliantInsts)
    F->addFnAttr("rvgpu-ieee", "false");
}

unsigned RVGPUTargetCodeGenInfo::getOpenCLKernelCallingConv() const {
  return llvm::CallingConv::RVGPU_KERNEL;
}
...
llvm::Constant *RVGPUTargetCodeGenInfo::getNullPointer(
...
LangAS
RVGPUTargetCodeGenInfo::getGlobalVarAddressSpace(CodeGenModule &CGM,
...
llvm::SyncScope::ID
RVGPUTargetCodeGenInfo::getLLVMSyncScopeID(const LangOptions &LangOpts,
                                            SyncScope Scope,
                                            llvm::AtomicOrdering Ordering,
...
bool RVGPUTargetCodeGenInfo::shouldEmitStaticExternCAliases() const {
  return false;
}

bool RVGPUTargetCodeGenInfo::shouldEmitDWARFBitFieldSeparators() const {
  return true;
}

void RVGPUTargetCodeGenInfo::setCUDAKernelCallingConvention(
...
llvm::Value *RVGPUTargetCodeGenInfo::createEnqueuedBlockKernel(
  ...
  auto *F = llvm::Function::Create(FT, llvm::GlobalValue::InternalLinkage, Name,
                                   &CGF.CGM.getModule());
  F->setCallingConv(llvm::CallingConv::RVGPU_KERNEL);
...
void CodeGenModule::handleRVGPUFlatWorkGroupSizeAttr(
    llvm::Function *F, const RVGPUFlatWorkGroupSizeAttr *FlatWGS,
...
void CodeGenModule::handleRVGPUWavesPerEUAttr(
    llvm::Function *F, const RVGPUWavesPerEUAttr *Attr) {
  unsigned Min =
  ...
  if (Min != 0) {
    ...
    F->addFnAttr("rvgpu-waves-per-eu", AttrVal);
  } else
    assert(Max == 0 && "Max must be zero");
}

std::unique_ptr<TargetCodeGenInfo>
CodeGen::createRVGPUTargetCodeGenInfo(CodeGenModule &CGM) {
  return std::make_unique<RVGPUTargetCodeGenInfo>(CGM.getTypes());
}
